...
// --experimental-strip-types` executes this file without pnpm resolving a
// runtime dependency. The strip-regex below is spliced verbatim from the
// packaged github-slugger@2.0.0 regex.js (a root devDependency), and the
// snapshot pin is CI-ENFORCED, not merely documented: the oracle suite in
// `__tests__/slug.test.ts` asserts parity with the package over every
// committed corpus heading plus adversarial unicode, so any drift —
// truncation, partial paste, formatter mangling — fails tests instead of
// silently diverging from GitHub's rendered anchors (the 2026-07-09
// incident: a truncated snapshot kept U+2192, splitting "3 → 2" anchors).
//
...
// and spaces (which the downstream replace turns into hyphens). Verbatim
// reproduction is intentional — bit-for-bit fidelity with the packaged
// upstream keeps divergence visible at diff time and enforceable by the
// oracle suite. ESLint suppressions match the upstream source's own: the
// character class uses defensive escapes (`no-useless-escape`) and matches
// astral codepoints via surrogate pairs without the `u` flag
// (`no-misleading-character-class`) — upstream's exact, deliberate form.
//
...
export class SluggerSession {
  ...
  next(headingText: string): string {
    // Mirrors github-slugger's BananaSlug#slug collision walk: every emitted
    // id is registered, and a candidate colliding with ANY earlier id (base
    // or generated) walks forward — "Foo", "Foo-1", "Foo" yields foo, foo-1,
    // foo-2 exactly as GitHub renders it. A count-by-base-only dedup emits a
    // duplicate id in that sequence and diverges from every later anchor.
    let result = slug(headingText);
    const originalSlug = result;
    while (this.occurrences.has(result)) {
      const nextCount = (this.occurrences.get(originalSlug) ?? 0) + 1;
      this.occurrences.set(originalSlug, nextCount);
