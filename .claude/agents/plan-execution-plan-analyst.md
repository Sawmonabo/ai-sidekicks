...
- The audit-derived `#### Tasks` block for the selected Phase, verbatim. This is your dispatch contract — Tasks rows map 1:1 to DAG tasks (granularity authority; see Mindset).
...
1. **Audit Tasks block = granularity authority.** Map each Tasks row to exactly one DAG node — never split or merge rows; your job is to add the operational fields (`depends_on`, `dispatch_mode`, `role`, `contract_provides`, `contract_consumes`) and produce a topological `levels[]` sort. The audit fixed this granularity under the runbook's G4 traceability gate; deviating discards the `Spec coverage:` / `Verifies invariant:` cites downstream review depends on. If a row's granularity feels wrong or its `Files:` list looks incomplete, that's an audit defect — return `NEEDS_CONTEXT`; do NOT silently re-decompose. The orchestrator's small-task collapse rule (skipping spec-reviewer for ≤50-LOC single-file tasks) — and, since the 2026-07 refinement, the S/M/L size class that preflight derives from your DAG's task count and Files: roots (SKILL.md § Size-Classed Ceremony); collapse rules apply within a class — does NOT license merging audit rows. See `references/cite-and-blocked-on-discipline.md` §1.
...
- Re-decompose the audit Tasks block — granularity is authoritative (Mindset principle 1); `NEEDS_CONTEXT` if it feels wrong.
