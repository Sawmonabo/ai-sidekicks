...
tasks:
  - id: T1 # match the Tasks-row id (T5.1, T-007p-1-1, etc.) — preserve audit-given ids
    ...
    consumes_resolution: {} # map: each contract_consumes symbol not provided in-DAG → its resolution string (from the audit Consumes: entry); decompose-time audit trail, NOT read by implementer/reviewer
