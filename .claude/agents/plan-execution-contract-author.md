...
- Per-package test scope: if the plan asks for a tooling-sanity test, **write** it scoped to your target package — no workspace-wide test files. You have no `Bash` tool, so test execution is the orchestrator's responsibility; it runs typecheck and per-package tests against your work after Phase C clears and before committing (see Phase B.1 in `.claude/skills/plan-execution/SKILL.md`). If either step fails the orchestrator round-trips the failure to you as a follow-on dispatch.
