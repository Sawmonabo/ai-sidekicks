...
Reconciliation is boundary-aware, not substring: an id occurrence counts only when not followed by an id-extending character (digit, letter, `_`, `.`, `-` — the corpus uses lettered ids like `T-025d…`), so a parsed `T1.1` cannot cover an unparseable `T1.10` row, nor `T-025` a `T-025d-14-1` row (Codex P2, PR #192 rounds 2-3). Erring toward "not covered" is the fail-closed direction — a false anomaly is visible, a false pass is the miss class the screen exists for.
