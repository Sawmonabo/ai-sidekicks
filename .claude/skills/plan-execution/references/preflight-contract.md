...
2. **Code-touching filter.** Only candidates whose diff touches `packages/` or `apps/` count as shipments (`gh pr view N --json files,changedFiles` per candidate). Doc-governance PRs citing a plan in the title are not manifest material.
...
- All merged in-title code PRs present in `shipped[].pr` → **pass**.
- Missing code PR(s) → **halt** listing each PR (number, merge date, code-file count, title) + the rebuild remediation + the escape flag.
