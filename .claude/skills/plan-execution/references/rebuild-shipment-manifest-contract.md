...
2. **Fetch merged PRs.** `gh pr list --state merged --search "Plan-NNN" --json number --limit 200`. The 200 ceiling is intentional — plans rarely exceed ~30 PRs, and the recovery context is one-time, so a high ceiling is cheaper than pagination.
...
4. **Parse heuristics.** `parsePhaseFromPr` and `parseTaskFromPr` extract the phase number and task ID(s) from title/body (title wins). Recognized shapes:
   ...
   - **Task:** `T-NNN-N-N` / `T-NNNp-N-N` (audit-runbook style, `plan`-prefixed); `TN.M` (Plan-001 phase-task style).
