...
### Phase E — Post-merge housekeeping (script)

20. **Housekeeper script exits 0** → proceed. The orchestrator reads the script's `proposed_manifest_entry`, merges in the DAG task's `verifies_invariant` and `spec_coverage`, appends the entry to the plan's `### Shipment Manifest`, and lands the housekeeping commit.

21. **Housekeeper script exits 6 or higher (crash / IO error / argument-validation failure)** → halt; surface the script's stderr and the manifest path to the user. `decideHousekeeperRouting` returns the same halt for any exit code other than 0, unrecognized ones included — the default-deny posture is deliberate: continuing over a state the script never documents is the bug class that helper exists to prevent.
...
> # → GATE verdict=<ack_clean|ack_with_findings|ack_findings_no_threads|ack_unattributable|ack_baseline_unavailable|ack_unsettled|ack_without_verdict|ack_predates_baseline|signal_truncated|no_ack_yet|rate_limited|draft_not_eligible|head_moved> ack= unresolved= ci= state= merge_state= merge_ok= head_sha=
> ```
>
...
>
> The script encodes every condition this section describes — all three ack legs as a disjunction (shape (2)'s clean-verdict comment included), the `created_at >= BASELINE_TS` freshness binding on shapes (1)/(2) and on the usage-limits non-ack — where `BASELINE_TS` is TWO floors, not one, because they answer different questions and conflating them is a defect the review rounds hit three times. The ACK floor is `max(gate's own first sighting of the sha as this PR's HEAD, earliest check_suite.created_at, committedDate)` and asks "could this ack predate the moment this sha became this PR's head" (see the `BASELINE_TS` paragraph for the commit-to-push gap the second closes, and the floor discussion later in this paragraph for why the first had to be added). The PUSH floor is `max(earliest check_suite.created_at, committedDate)` — the ack floor without the first sighting — and is what the stale-run detector and the usage-limits non-ack read, because neither asks an attribution question: "was a run for an older commit in flight across the push" is anchored on the push, and "is Codex out of quota" is a recency claim about the bot that names no commit at all. Raising either to the first sighting silently narrows it — a stale review landing in the push-to-sighting gap becomes invisible, and a genuine quota notice in that same gap is dropped so the gate prints "keep polling" precisely when polling cannot help, `--paginate --slurp`, REST-not-GraphQL reactions, an empty check rollup as `ci=none` rather than `green`, and the ack-lands-before-its-threads race (§ Loop structure) as a distinct `ack_unsettled` verdict that can never score `merge_ok=1` — scoped to the two ack legs that CAN be followed by inline threads, the review object and the acking comment, and deliberately not to the 👍 (a `+1` means "no suggestions", so nothing is pending behind it). It also separates "acks HEAD" from "asserts HEAD is clean": a `Reviewed commit:` citation proves only that Codex read this commit, so a citation that declares nothing scores `ack_without_verdict`, never `ack_clean`. It separates a third thing on the same axis — whether an ack can be ATTRIBUTED to HEAD at all — because the two sha-less legs, the 👍 and shape (2)'s clean-verdict comment, rest on `created_at >= BASELINE_TS` alone, and a run started on the PREVIOUS head that overlaps a push and finishes afterwards satisfies that by construction. Moving the anchor cannot separate those: the stale ack arrives AFTER the push, not before it, so the commit-to-push-gap fix does not reach this one. Two things do. A clean verdict that names a commit other than HEAD in its own `Reviewed commit:` line is refused outright — a positive test on the comment's own words, never a requirement that a clean verdict cite a sha. The reason is contract volatility, not present-day breakage: a full-corpus survey (259 PRs, 36 clean verdicts, 2026-07-27) found the sha-less and sha-bearing forms split cleanly by DATE with zero interleaving — 28 sha-less from 2026-04-30 to 2026-06-09, then 8 sha-bearing from 2026-06-22 on — so requiring the sha would work perfectly today and break the day Codex reverts a format it has already changed once, while testing it when present costs nothing in either regime. The general lesson outlives this fix: **Codex's comment format is an external contract that has already changed once mid-corpus**, so any predicate keyed to its wording needs a dated observation window, and a survey that pools the whole corpus averages across the change and reports a ratio describing no period that ever existed — the pooled "28 of 36 are sha-less" reads as a live majority and is in fact a retired format. And a Codex artefact for a NON-head commit published at or after the anchor — a review whose `commit_id` is not HEAD, or a citation naming another sha — is evidence such a run was in flight, so while it stands and NO sha-bound ack corroborates them, the sha-less legs cannot carry a clean verdict and the gate scores `ack_unattributable` (never `merge_ok=1`; cleared by any HEAD-citing verdict, which is what a genuine pass for this commit posts). Named residual, dated because the halves are not equally live: a run for the previous head finishing with ONLY a bare 👍, or ONLY a sha-less clean comment, leaves no trace and is still accepted — the 👍 half is open, the comment half has not been observed since 2026-06-09 and is dormant only while Codex keeps its current format. **A separate question is the FLOOR rather than the rival run, and it needed a separate mechanism.** Every server-side candidate for "when did this sha become this PR's head" is a proxy that has been predated in review — the author-controlled commit clock, then the earliest check suite, which dates the sha's first visibility ANYWHERE in the repo and so predates this PR entirely for a sha pushed on another branch first. GitHub exposes no head-update timestamp to replace them with (`Commit.pushedDate` null; `PullRequest.timelineItems` `PullRequestCommit` nodes carry no `createdAt` — verified on #259, all seven commits, 2026-07-27; the REST timeline's only sha-bearing event is `committed`, which is that same author-controlled field). So the gate records its OWN first sighting of the sha as this PR's HEAD, per `(pr, sha)`, under `.cache/codex-gate/` — an instant at or after the head update by construction, with no proxy in between. A timestamp-only ack older than that sighting scores `ack_predates_baseline` rather than `no_ack_yet`, because Codex HAS looked and the gate is declining to bind the result; re-polling never clears it (Codex does not re-ack a head it already acked) so the remediation is `@codex review`. A store that is unwritable, corrupt, or records another sha scores `ack_baseline_unavailable` and never `merge_ok=1` — missing state fails toward "cannot vouch", never toward "anchor unknown, allow". Both are bypassed entirely by a sha-bound ack, which is what stops the strict floor from being a blanket stall: a review on HEAD or a sha-citing comment needs no floor, and every clean verdict since 2026-06-22 is in that set. The refusal is also not sticky — it blocks a merge only when it is WHY no ack survives. A refused stale 👍 sitting beside a review that names HEAD does not block, because the review is dispositive and the 👍 is evidence about nothing; blocking on it anyway produced a verdict of `ack_clean` with the merge refused and no remediation printed, which is a silent block and therefore the same defect class as the false merges, pointing the other way. The rejected alternative — seed the baseline from the old anchor on first sight, so nothing ever stalls — is refused because its failure mode is a SILENT false merge on the first gate call for a sha, where the strict floor's is a loud stall that prints its own remediation; under fail-closed doctrine those costs are not comparable. Honest scope in the other direction too: against currently observed shapes the baseline's protective value is near zero, since every modern clean verdict is sha-bound and bypasses it — it exists for the timestamp-only legs, the bare 👍 above all. Separately, an ack whose OWN timestamp is missing or unparseable is treated as having landed this instant rather than as infinitely old: unknown recency is the absence of evidence, and reading it as "safely outside the settle window" produced `ack_clean` + `merge_ok=1` before any delayed thread could materialise. That ack never ages out, so the gate reports it as undatable and prints a remediation other than "keep re-polling". Exactly ONE shape is excluded from carrying a clean verdict — a comment that acks HEAD without asserting HEAD is clean, when it is the only ack — deliberately stated as a single exclusion rather than a general "an ack must assert clean" rule, which would drop the bare 👍 (Codex's own "no suggestions") and stall every clean merge. It reads findings in BOTH places Codex files them: inline review threads, and a `### 💡 Codex Review` comment whose body carries the findings with no thread at all (shape (2b) below), the second scoring `ack_findings_no_threads` — the shape with no backstop, since zero threads leaves `required_conversation_resolution` nothing to hold and GitHub would allow the merge. Past this section it also drains `reviewThreads` AND the check rollup to completion, reporting a shortfall against `totalCount`, a non-advancing cursor, a ceiling exit with pages outstanding, or an absent `totalCount` as a fail-closed `signal_truncated` verdict rather than a printed warning; gates CI on branch-protection-required checks only (`isRequired(pullRequestNumber:)` is the sole authority — names lie), printing advisory failures without blocking on them and falling back to gating every check when no row reports required; and conjoins the PR's own `state` (a CLOSED or MERGED PR can never score `merge_ok=1`), a push anchor it could actually date, and GitHub's own `mergeStateStatus`, which is what still catches a required check that has no rollup row at all. Every probe hangs off the sha read before any of them ran, so the gate re-reads HEAD after the last one and reports `head_moved` — never a downstream symptom — when a push landed mid-probe; the sha it validated is printed as `head_sha=` on every verdict, and step 4's `--match-head-commit` closes the residual window between that print and the merge. Wrap the script in Monitor for polling; read `merge_ok=1` as the only green light. Signal derivation + decision table: `scripts/lib/codex-verdict.mjs`; `gh` response handling: `scripts/lib/gh-api.mjs`; tests: `scripts/__tests__/codex-verdict.test.mjs` + `scripts/__tests__/gh-api.test.mjs`.
...
- **Terminal ack — any of the Codex verdict shapes below, each conjoined with `reviewThreads.totalCount == BASELINE_THREADS`** (no new findings landed). Codex acks in multiple shapes that are NOT mutually exclusive, so OR across them — a monitor watching only one false-times-out when Codex uses another. On PR #120 the final-HEAD ack arrived as a `+1` reaction **and** a clean-verdict comment while Codex's review objects sat on earlier (pre-fix) commits, not the final HEAD — a review-only monitor would have hung. The `totalCount == BASELINE_THREADS` conjunct applies to every shape (a review can itself carry findings, which trips the new-threads intermediate exit above rather than acking):
  ...
  - **(2) clean-verdict comment (observed — #120 / #121)** — a `chatgpt-codex-connector[bot]` issue comment matching `Didn't find any major issues`, **with `created_at >= BASELINE_TS`** (same freshness binding as (1) — a clean verdict from an earlier HEAD does not ack the pushed one). **Freshness is necessary and NOT sufficient on this shape or on (1).** A run started on the previous HEAD that overlaps a push and finishes afterwards posts a clean verdict whose `created_at` post-dates the new anchor, so no timestamp floor can reject it — the stale ack arrives after the push, which is what distinguishes this from the commit-to-push gap below. `codex-gate.mjs` therefore refuses a clean verdict whose `Reviewed commit:` line names a sha OTHER than HEAD (today's clean comment carries that line — every clean verdict in the corpus from 2026-06-22 on does — so this is a positive test on the comment's own words, NOT a requirement that a clean verdict cite a sha; requiring it would in fact pass against current behaviour and is refused for contract volatility, since the sha-less form ran from 2026-04-30 to 2026-06-09 and the format could move back), and withholds cleanliness from (1) and the sha-less form of (2) entirely while a non-head Codex artefact dated at/after the anchor shows such a run was in flight, scoring `ack_unattributable` until a HEAD-citing verdict clears it. A previous-head run that finishes with ONLY a bare 👍 or a sha-less clean comment leaves no trace and is still accepted — the named residual, live on the 👍 and dormant on the comment since 2026-06-09. **Both (1) and the sha-less form of (2) are additionally floored on the gate's own first sighting of the sha as this PR's HEAD**, not on a server-side proxy for it: an ack older than that sighting scores `ack_predates_baseline` (Codex looked, the gate cannot bind the result, `@codex review` is the only fix), and an unusable baseline store scores `ack_baseline_unavailable`. Neither applies to a sha-bound ack, which needs no floor.
  - **(2b) findings-summary comment — a NON-ack of cleanliness (observed — #28 / #218 / #235)** — a `chatgpt-codex-connector[bot]` issue comment headed `### 💡 Codex Review` whose body carries the findings themselves (severity badge, blob permalink, prose) with **no inline thread anywhere**, no `Reviewed commit:` line and no clean verdict. It matched no ack leg at all until 2026-07-27, so `codex-gate.mjs` reported `no_ack_yet` — "Codex has not looked at this yet" — about a commit Codex had reviewed and filed a P1 against. Observed on PR #28: `f67a7bb` became HEAD 02:11:29Z, the summary landed 02:18:54Z citing that exact sha, the next push was 02:27:03Z, the first bot review 02:30:39Z on a LATER sha, and the only `+1` 04:33:50Z — 8 minutes with every ack leg false. **HEAD-bound by the sha in its permalinks**, exactly like a `Reviewed commit:` citation and with no timestamp involved; a summary naming an older sha is findings against a commit that has since been rewritten and must stay silent (PR #235 pushed 108s before its summary landed). Scores `ack_findings_no_threads`: an ack, never a clean one, and never `merge_ok=1`. Detected on either of two independently-sufficient markers — the heading, or a `![P<n> Badge]` image — because they fail independently; surveyed 2026-07-27 across all 48 bot comments in the repo, each marker matched the same 5 summaries and neither matched any of the 36 clean verdicts, the 6 usage-limits notices, or the 1 conversational reply (which a permalink-based test WOULD have captured, so the permalink is deliberately not a marker).
  - **(3) review-on-HEAD (documented; unobserved on a final HEAD in the #105–#121 sample)** — a Codex review (REST `pulls/<PR#>/reviews[]`, login WITH the `[bot]` suffix per sub-step b: `chatgpt-codex-connector[bot]`) whose `.commit_id == HEAD_SHA` (intrinsically HEAD-bound — no `BASELINE_TS` check needed; `.commit_id` is the reliable HEAD anchor — GraphQL `latestReviews[].commit.oid` is often `""` for the bot, and a body-grep breaks silently under a wrong-form author filter). **Ack-vs-threads race (PR #171 r5):** the review object lands seconds BEFORE its threads materialize, so a poll tick can read fresh-review + `totalCount == BASELINE_THREADS` and mis-signal clean — any ack that can be FOLLOWED by inline threads MUST be confirmed by re-polling `reviewThreads.totalCount` after a settle window before advancing to SKILL.md Phase D.5 step 4. That is this shape AND a HEAD-citing comment, not just this one; `codex-gate.mjs` applies `DEFAULT_SETTLE_WINDOW_MS` (120s) to both, and to neither the 👍. In the sample Codex reviewed intermediate pushes (those reviews carry findings → new-threads exit) and acked the final fixed HEAD via (1)/(2) rather than re-reviewing; include defensively in case that behavior changes.

  `BASELINE_TS` is the current HEAD commit's GH-issued `committedDate`, fetched via GraphQL `object(oid:)` anchored by `$HEAD_SHA` (the captured form is at sub-step (a) above — NEVER `gh pr view --json commits --jq '.commits[-1].committedDate'`, which is truncation-vulnerable per `cli/cli#5415` and silently returns the 100th commit's date instead of HEAD's on >100-commit PRs), captured alongside `HEAD_SHA` / `BASELINE_THREADS` and re-captured on every re-baseline (the new-push intermediate exit above). It is earlier than any reaction/comment Codex posts about that HEAD, so `created_at >= BASELINE_TS` is what binds the otherwise-unanchored shapes (1)/(2) to the current HEAD — without it a clean verdict from a pre-fix HEAD false-passes the gate, which is the ack-side instance of the HEAD-SHA ack-anchoring rule (a stale-HEAD ack is the same class as a stale-HEAD review). **`committedDate` is not a sufficient floor on its own**, and a hand-rolled poll inherits the hole: it is the LOCAL commit time, written by the author's clock before the push, so an ack of the PREVIOUS HEAD landing in the commit-to-push gap still clears it and falsely acks a commit Codex never saw — 112 seconds of exposure measured on `c8bcdc1` (`committedDate` 18:19:07Z, first check suite 18:20:59Z), and a commit-then-verify-then-push workflow widens it to minutes. `Commit.pushedDate` would answer this exactly but GitHub no longer populates it (null on that same commit). `codex-gate.mjs` therefore anchors on `max(committedDate, earliest check_suite.created_at)` — the earliest server-side sighting of the sha, check suites rather than `actions/runs` because they cover non-Actions apps too — and refuses `merge_ok` outright when no check suite dates the sha at all.

  On any one of (1)/(2)/(3) AND `totalCount == BASELINE_THREADS`, advance to SKILL.md Phase D.5 step 4. **(2b) is explicitly NOT in that set** — it acks HEAD but reports findings, so it satisfies `totalCount == BASELINE_THREADS` trivially (it opens no thread) while being the opposite of a clean pass. Advancing on it is the false pass `ack_findings_no_threads` exists to refuse.

- **Terminal NON-ack on usage limits** — a `chatgpt-codex-connector[bot]` issue comment matching `usage limits for code reviews`, **with `created_at >= ` the PUSH floor** (a per-HEAD freshness binding, so a stale `usage limits` comment from a prior HEAD or a manual trigger must not halt the current HEAD's gate — but deliberately the push floor rather than the ack floor shapes (1)/(2) use, since a quota notice makes no claim about any commit and needs recency, not attribution; anchoring it on the gate's first sighting would drop a genuine notice posted in the push-to-sighting gap and report `no_ack_yet` instead of `rate_limited`), means Codex is out of review credits and will not ack this HEAD; halt Phase D.5 and surface to the user for a merge decision (do not wait out the budget). Documented but not observed in the #117–#122 sample. Observed 2026-08-31 (PRs #395–#397): the bot's SECURITY-review quota emits a near-identical comment — "You have reached your Codex usage limits for security reviews." — while the code review runs to Completed with findings, so the match is deliberately narrowed to the code-review limit text and a security-review notice is never a terminal.
...
A verification step that **mutates source in place** — a negative-control driver, mutation testing, any perturb-run-restore loop — leaves real files broken on disk for the length of each run, with nothing about the file announcing it. Every concurrent reader is a silent victim: a `git add`, a `git commit`, a parallel test run, an editor's autosave-format. What gets captured is a plausible-looking constant or a deleted branch, and it ships as a permanent false-clean of whatever the mutated code existed to detect. Observed 2026-07-27 on PR #259, where a stage-and-commit racing a live perturbation captured `const signalTruncated = false;` — a gate that could never report truncation, which is the exact class that PR existed to close.

**Neither the static gates nor a working-tree test run can catch it.** A constant-folded predicate is valid code, so hooks, lint, format and commit-message checks all pass on a perturbed staged snapshot — on the observed commit, pre-commit hooks, eslint, prettier and commitlint were all green. And once the harness restores, the tree is correct; it is the _staged snapshot_ that is broken, so a test run against the working tree is not looking at the artifact that carries the defect. (A crash mid-run is the easy case — the tree is left perturbed and the next run fails loudly.) That asymmetry is the part a reader will not derive on their own: the only detection is running the committed bytes in a clean checkout, from a detached worktree at that sha rather than the checkout you have been working in.

**Coverage-shaped invariants catch what behavioural ones miss.** A mutation that pins a predicate to a constant makes the branch behind it unreachable, so an "every arm of this decision table is reachable" assertion fails even though no assertion about that branch's _behaviour_ was hunting a constant-folded predicate. Deleting the behaviour deletes the arm — an invariant over the shape of the decision space sees that; invariants over its outputs do not. On the observed commit that assertion was itself brand new — absent at `bdfd3d0`, present at `bcc3ddb` — so it caught a defect in its own commit, firing alongside the two tests that assert truncation behaviour directly. Both are PR-branch commits, and `develop` squash-merges: verify them through PR #259 rather than `git log`, which will not resolve either sha once the branch is gone. That applies to every branch sha cited from a merged PR in this file. This is a general argument for arm-coverage assertions on any decision table, not only a recovery aid.

**Recovery and discipline.** Compare the suspect file against the pre-round commit, restore it, then re-verify every leg from a detached worktree at the new sha with exit codes read bare. `git commit --amend` is the repair only while the bad commit is unpushed; once it is on the remote, stack a follow-on commit carrying the restore rather than rewriting history. Rewriting is available — § Reading subagent responses above documents `--force-with-lease` for a stray subagent commit — but that case pulls a whole unwanted diff back, whereas a perturbed commit is one the PR wants and one line of which is wrong. On a PR under review the perturbed sha is what Codex anchored its verdict to, so rewriting it converts a landed review into findings against a commit that no longer exists: the stale-sha hazard § Codex Verdict Gate (Phase D.5 step 3) already names. While a harness is live, do not stage, do not commit, and do not point a second harness at the same files; when two agents share a working tree, one of them owns git for the duration.
