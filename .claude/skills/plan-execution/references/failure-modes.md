...
>
> The script encodes every condition this section describes — all three ack legs as a disjunction (shape (2)'s clean-verdict comment included), the `created_at >= BASELINE_TS` freshness binding on shapes (1)/(2) and on the usage-limits non-ack, `--paginate --slurp`, REST-not-GraphQL reactions, an empty check rollup as `ci=none` rather than `green`, and the review-lands-before-its-threads race (§ Loop structure) as a distinct `ack_unsettled` verdict that can never score `merge_ok=1`. Past this section it also drains `reviewThreads` AND the check rollup to completion, reporting any shortfall against `totalCount` as a fail-closed `signal_truncated` verdict rather than a printed warning; gates CI on branch-protection-required checks only (`isRequired(pullRequestNumber:)` is the sole authority — names lie), printing advisory failures without blocking on them and falling back to gating every check when no row reports required; and conjoins both the PR's own `state` (a CLOSED or MERGED PR can never score `merge_ok=1`) and GitHub's own `mergeStateStatus`, which is what still catches a required check that has no rollup row at all. Wrap the script in Monitor for polling; read `merge_ok=1` as the only green light. Signal derivation + decision table: `scripts/lib/codex-verdict.mjs`; `gh` response handling: `scripts/lib/gh-api.mjs`; tests: `scripts/__tests__/codex-verdict.test.mjs` + `scripts/__tests__/gh-api.test.mjs`.
