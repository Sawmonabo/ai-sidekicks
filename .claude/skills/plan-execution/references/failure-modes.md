...
- **Terminal NON-ack on usage limits** — a `chatgpt-codex-connector[bot]` issue comment matching `usage limits`, **with `created_at >= ` the PUSH floor** (a per-HEAD freshness binding, so a stale `usage limits` comment from a prior HEAD or a manual trigger must not halt the current HEAD's gate — but deliberately the push floor rather than the ack floor shapes (1)/(2) use, since a quota notice makes no claim about any commit and needs recency, not attribution; anchoring it on the gate's first sighting would drop a genuine notice posted in the push-to-sighting gap and report `no_ack_yet` instead of `rate_limited`), means Codex is out of review credits and will not ack this HEAD; halt Phase D.5 and surface to the user for a merge decision (do not wait out the budget). Documented but not observed in the #117–#122 sample — verify against current Codex behavior.
