...
- **Session naming aligned to plan.** Phase 0.3 MUST invoke `/rename Plan-NNN Phase N` (compact form) before issuing the confirm-to-user sentence — this makes the session tab self-identifying when the user has multiple parallel orchestrator sessions open. The rename is a skill-prescribed action under the carve-out above; do not call `AskUserQuestion`. Failure to rename is not a hard halt — if `/rename` is unavailable in the current Claude Code build, surface the gap and proceed.
...
**Set the session name.** Invoke `/rename Plan-NNN Phase N` (e.g., `/rename Plan-023 Phase 1`). Use the compact form — descriptive titles risk IDE tab truncation; the descriptive context appears in the confirm sentence below.
