---
name: plan-execution
description: Execute implementation plans (docs/plans/NNN-*.md) one Phase per PR by consuming the audit-derived `#### Tasks` block as the dispatch contract, expanding it into a task DAG, dispatching task-scoped implementer subagents (sequential by default, worktree-isolated when wall-clock parallelism is required), gating each task with a small-scope reviewer pipeline (spec / quality / correctness), and gating the merged PR with a final integration-coverage review pipeline. OPT-IN ONLY — use when the user explicitly invokes /plan-execution, explicitly asks for this pipeline by name ("run plan-execution on Plan-NNN", "execute Plan-NNN through the plan-execution pipeline"), or asks to resume an in-flight plan-execution run (a feat/plan-NNN-* branch whose draft PR body carries this skill's Task DAG block). A request that merely names a plan ("work on Plan-NNN", "fix X from Plan-NNN", "continue Plan-NNN") is NOT a trigger — handle it as normal development (branch off develop, implement, PR per CONTRIBUTING.md) unless the user asks for this pipeline. When this skill does run, it auto-detects the next eligible Phase by walking the plan's Implementation Phase Sequence and checking each Phase's `**Precondition:**` against branch + merged-PR + cross-plan state; an explicit "PR #M" or "Phase N" overrides the inference. The audit precondition gate (Status Promotion Gate from `docs/operations/plan-implementation-readiness-audit-runbook.md`) blocks dispatch on un-audited plans; doc-first ordering and audit gates are repo policy for ALL code work regardless of execution path.
requires_files:
...
4. **Squash-merge into `develop`** — `gh pr merge <PR#> --squash --delete-branch`. This produces the canonical squash-commit on `develop` (whose SHA Phase E step 6 references in the Progress Log) and deletes the feature branch in one atomic action. Immediately before firing, re-confirm two things: (a) the step-3 gate still holds at this instant (a new thread or push between the ack and now regresses the state — re-baseline rather than merge); (b) the PR title still summarizes the final diff — the squash-commit subject on `develop` IS the PR title, so when review rounds outgrew it, pass an explicit commitlint-valid `--subject` (and `--body-file` if the body needs the same) instead of accepting the stale default. On merge failure (e.g. mergeStateStatus regressed to BLOCKED between step 3 and step 4 because a new review thread fired), halt and surface to user.
