...
4. **Decide routing on `script_exit_code`, then dispatch xor halt** — call `decideHousekeeperRouting({ scriptExitCode })` from `lib/housekeeper-orchestrator-helpers.mjs` (the canonical mapping; mirror in `references/post-merge-housekeeper-contract.md` § Exit codes; pinned by 10 unit tests in `scripts/__tests__/post-merge-housekeeper-orchestrator-helpers.test.mjs`). The helper returns either `{ action: "dispatch", exitClass: "subagent-handled" }` (exits 0 / 2 / 3 / 5 — happy path, subagent-handled BLOCKED, no-checklist, schema-violation surfacing) or `{ action: "halt", exitClass, reason, surfacePromptTemplate }` (exits 1 / 4 — orchestrator misdispatch; exit ≥6 — script crash; defensive fallback for unrecognized codes). [^d8]
   - `action === "halt"` → relay `surfacePromptTemplate` verbatim to the user, halt Phase E, do NOT dispatch the subagent. The manifest reflects a script-stage / orchestrator-stage failure that needs operator action; routing it through the subagent would force the LLM to interpret garbage and emit a `RESULT:` tag based on hallucinated state (Codex P1 PR #33 — `gate housekeeper dispatch on script exit-code class`).
...
[^d8]: `decideHousekeeperRouting` is the single source of truth for the dispatch/halt mapping. Edit the helper + its tests when the contract's exit-code semantics change; the prose tables in this skill and the contract reference cite the helper rather than re-stating the mapping inline (avoids the prose-drift hazard that surfaced before this fix landed).
...
5. **Validate the subagent-stage manifest**:
   ...
   - `manifest._script_stage` (script-embedded immutable snapshot of `affected_files` / `schema_violations` / `verification_failures` / `semantic_work_pending`) is preserved verbatim — the validator reads this field as the **A2 design** snapshot source so the four preservation/iteration checks (#7/#9/#10/#11) work end-to-end without requiring orchestrator-explicit `scriptXXX` plumbing. The subagent contract treats `_script_stage` as READ-ONLY; structural tampering (removed key, non-object value, non-array fields) surfaces in the validator as a `Codex F-AMbIV` gap. See `references/post-merge-housekeeper-contract.md` § `_script_stage` immutability.
