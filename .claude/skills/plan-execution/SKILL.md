...
**Memory-freshness sanity check.** Before relying on the Codex-related memory slugs cited downstream in this skill (`feedback_codex_bot_login`, `feedback_codex_requires_non_draft`, `feedback_pr_polling_cadence`, `feedback_thread_reply_discipline`, `project_codex_review_integration`), open each one and check the date / last-updated context. These slugs encode the Codex bot's external contract (login form per endpoint, draft-required-for-auto-review behavior, polling cadence, thread-reply ordering) — Anthropic does not control that contract, and it has shifted before. If any cited memory looks stale (>30 days without verification) or contradicts behavior observed in this session, surface to the user before proceeding to Phase D.5. The skill's Codex-handling steps assume those memories are current; silent breakage looks identical to "Codex is slow today."
...
3. **Wait for the Codex external reviewer's verdict on HEAD** — Codex auto-reviews push events on **non-draft** PRs since ~2026-05-05 (per `feedback_codex_bot_login.md` + `feedback_codex_requires_non_draft.md` + `project_codex_review_integration.md` memories). Step 1's `gh pr ready` transition is what makes the PR visible to Codex; the eyes-poll budget below begins from that ready transition, NOT from push time. The orchestrator MUST wait for that verdict before queuing the squash-merge, because `required_conversation_resolution` only blocks merge if Codex has left unresolved threads — silence (Codex still working) is observably indistinguishable from "passed with no findings" without an explicit wait. Three sub-steps:

   **Loop structure.** Step 3 is itself a loop, not a one-shot. Each `git push` after `gh pr ready` re-triggers Codex's auto-review path (per `feedback_codex_requires_non_draft.md` — auto-fires on every push to a non-draft PR, not just the ready-transition). The outer loop is per-HEAD: `(a) baseline → (b) eyes-poll → (c) verdict-poll`, with `(c)`'s "new push" intermediate-exit restarting from `(a)` on the new HEAD. After Phase D returns DONE the first iteration may complete on the unchanged HEAD; after any round-trip fix-up push, the next iteration MUST re-baseline (Codex's next verdict is on the new HEAD, not the old). A monitor that doesn't re-baseline silently waits for a verdict that never lands. Equally load-bearing: the resubmit-detection signal is `reviewThreads.totalCount > BASELINE_THREADS` (per `feedback_pr_polling_cadence.md`) — NEVER `latestReviews.length` (returns one row per author, stays at 1 across a Codex resubmit, silently fails). Both regressions burned in PR #83 (2026-05-20): a `latestReviews`-length-keyed Monitor missed the post-push Codex resubmit; the user surfaced the miss 20 min later.

   a. **Capture HEAD-SHA + thread baseline + commit timestamp** — `HEAD_SHA=$(gh pr view <PR#> --json headRefOid -q .headRefOid)`; `BASELINE_THREADS=$(gh api graphql -f query='{repository(owner:"<o>",name:"<r>"){pullRequest(number:<PR#>){reviewThreads{totalCount}}}}' --jq '.data.repository.pullRequest.reviewThreads.totalCount')`; `BASELINE_TS=$(gh pr view <PR#> --json commits --jq '.commits[-1].committedDate')`. These three values anchor the poll to the current commit, the current thread count, and the commit timestamp (the floor for the `created_at >= BASELINE_TS` freshness binding on ack shapes (1)/(2) and the usage-limits non-ack at step (c) below); any new push or any new thread is an intermediate-exit event per `feedback_pr_polling_cadence.md`, and re-baselining on intermediate exit captures the new commit's `committedDate` so freshness checks bind to the new HEAD, not the old one.
   ...
   **Bot-login form varies by endpoint — verify before constructing any ad-hoc Codex-author filter.** The reactions API above returns `user.login` WITH the `[bot]` suffix (`chatgpt-codex-connector[bot]`); GraphQL `latestReviews.author.login` and REST `pulls/{n}/reviews[].user.login` return `chatgpt-codex-connector` WITHOUT the suffix. A `[bot]`-suffixed filter on those review-author endpoints silently returns 0 hits and the poll loop never terminates. Enumerate once before filtering any new endpoint: `gh api repos/<o>/<r>/pulls/<PR#>/reviews --jq '[.[] | .user.login] | unique'`. If no eyes after 5 min, post `gh pr comment <PR#> --body "@codex review"` as a manual fallback trigger, then resume the eyes-poll for another 5 min budget. Still no eyes after the fallback → halt and surface to user (Codex bot may be down or de-installed).
   ...
   - **Terminal ack — any of the Codex verdict shapes below, each conjoined with `reviewThreads.totalCount == BASELINE_THREADS`** (no new findings landed). Codex acks in multiple shapes that are NOT mutually exclusive, so OR across them — a monitor watching only one false-times-out when Codex uses another. On PR #120 the final-HEAD ack arrived as a `+1` reaction **and** a clean-verdict comment while Codex's review objects sat on earlier (pre-fix) commits, not the final HEAD — a review-only monitor would have hung. The `totalCount == BASELINE_THREADS` conjunct applies to every shape (a review can itself carry findings, which trips the new-threads intermediate exit above rather than acking):
     ...
     `BASELINE_TS` is the current HEAD commit's GH-issued `committedDate` (`gh pr view <PR#> --json commits --jq '.commits[-1].committedDate'`), captured alongside `HEAD_SHA` / `BASELINE_THREADS` and re-captured on every re-baseline (the new-push intermediate exit above). It is strictly earlier than any reaction/comment Codex posts about that HEAD, so `created_at >= BASELINE_TS` is what binds the otherwise-unanchored shapes (1)/(2) to the current HEAD — without it a clean verdict from a pre-fix HEAD false-passes the gate, which is the ack-side instance of the `feedback_codex_head_sha_anchor.md` rule (a stale-HEAD ack is the same class as a stale-HEAD review).
