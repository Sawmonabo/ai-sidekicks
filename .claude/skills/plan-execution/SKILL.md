...
**Codex-contract freshness check.** The Codex-handling steps in Phase D.5 below assume a specific external contract for the bot — login form per endpoint, draft-required-for-auto-review behavior, polling cadence, thread-reply ordering, HEAD-SHA ack-anchoring. Anthropic does not control that contract, and it has shifted before. If behavior observed in this session contradicts the Phase D.5 model (e.g. no `eyes` reaction on a non-draft PR, or an ack shape not listed there), surface to the user before relying on the gate — silent breakage looks identical to "Codex is slow today."
