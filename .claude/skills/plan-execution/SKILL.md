...
5. **Validate the subagent-stage manifest**:
   ...
   - `manifest._script_stage` (script-embedded immutable snapshot of `affected_files` / `schema_violations` / `verification_failures` / `semantic_work_pending`) is preserved verbatim — the validator reads this field as the **A2 design** snapshot source so the four preservation/iteration checks (#7/#9/#10/#11) work end-to-end without requiring orchestrator-explicit `scriptXXX` plumbing. The subagent contract treats `_script_stage` as READ-ONLY; structural tampering (removed key, non-object value, non-array fields) surfaces in the validator as a structural-tampering gap. See `references/post-merge-housekeeper-contract.md` § `_script_stage` immutability.
