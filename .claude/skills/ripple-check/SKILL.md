...
4. **Map the residual diff signals to subagents.** Use this lookup; dispatch only the subagents whose signals are present (typical invocations dispatch 1–3, full sweeps dispatch 5):
   ...
   **Subagent D inbound enumeration — `.md` citers.** A line-shifting edit to a governance doc silently invalidates inbound line-cites in other `.md` docs. Build D's inbound list from the `.md` corpus across every line-bearing form:
   ...
   2. **Keep every line-bearing form** in `.md` citers: colon `Spec-003:178`; line-word `Spec-003 line 178` / `lines 81, 107-113`; parenthesized `(line 178)`; named-section `§Acceptance Criteria line 81`; acceptance-criterion `AC4:108`; full path `docs/domain/session-model.md:61-77`; bare basename `api-payload-contracts.md:120`; and their line-word variants.
   3. **Normalize** each hit to `<doc-path>:NNN` — a range / list (`lines 81, 107-113`, `:61-77`) expands to one entry per endpoint. Pass the union as Subagent D's inbound list.

   Post-2026-07 sweep, code comments cite governance docs ONLY via backticked `§Heading` anchors (gate-verified; raw colon/path forms are denied at pre-commit + CI), so there is no code-side line-cite population to enumerate — the lone code-side residual is the bare-basename `.md:NN` wire-doc annotation class (label-less architecture-doc targets; documented in CAT-07). Enumerate inbound ANCHOR exposure instead:
