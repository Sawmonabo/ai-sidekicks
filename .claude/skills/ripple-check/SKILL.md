...
The static pre-commit + CI hooks (`lychee`, `path-canonical-ripple`, `mermaid-set-coherence`, `cite-target-existence` — including `#symbol` verification + the volatile-tree line-pin deny — and `label-cite` — `§Heading` verification + the raw volatile-cite deny for code AND `.md` citers, wrap-split pairs included) catch the deterministic 80% — they always fire, run in <5s, and gate via branch protection. They cannot catch the semantic 20%:
...
- **CAT-07** — line-citation drift when both old and new lines are non-empty (`Spec-024:6 → :5`) — now confined to the deny's exemption namespaces (plan Tasks-block grammar, waiver-marked example lines, `docs/superpowers/` + `.claude/` citers, frozen-tree pins, label-less continuations): both code (2026-07-08) and `.md` (2026-07-17) citers are deny-gated for new volatile cites, so the dominant residual is semantic drift UNDER an intact anchor.
- **Cross-doc narrative coherence** — "ADR-022 says X; Plan-020 implicitly assumes ¬X." Spans multiple catalog rows and is keyed on a _semantic relationship_, not a single structural action.
...
4. **Map the residual diff signals to subagents.** Use this lookup; dispatch only the subagents whose signals are present (typical invocations dispatch 1–3, full sweeps dispatch 5):
   ...
   | A modified `.md` file is referenced by another doc — by path / stem, by label token (`Spec-005`), or by a `§Heading` anchor — OR it cites a section of ITSELF that the diff modifies | **Subagent E** (cross-doc coherence) | `ripple-check-cross-doc` |

   CAT-08 (outbound HTTP / file-link breakage), CAT-09 (table-total arithmetic drift), and CAT-11 (a formatter normalizing malformed markup into format-stable corruption) fire no subagent — each is covered by a deterministic hook: `lychee` at CI for CAT-08, `table-total-coherence` at pre-commit + CI for CAT-09, and `table-arity` at pre-commit + CI for CAT-11. (A marked-table row change still dispatches Subagent C for its broader CAT-05 set-quantifier ripple; only the within-document arithmetic is the hook's deterministic slice. CAT-11's hook covers its GFM-table slice; the row's other-markup residual carries no diff signal distinct from CAT-05's.) CAT-10 (a gate reporting clean over work it did not do) also fires no subagent, but because it has no doc-diff signal at all — it lives in a gate's logic rather than in the corpus, and is a code-review finding. See § What this skill does NOT do.

   **Subagent D inbound enumeration — `.md` AND code citers.** A line-shifting edit to a governance doc silently invalidates inbound line-cites in other `.md` docs AND in `packages/**`+`apps/**` code comments. Build D's inbound list from BOTH surfaces across every line-bearing form:
   1. **Identify the amended doc by token AND path.** Label token (when the doc has one): `docs/specs/002-…` → `Spec-002`, `docs/plans/006-…` → `Plan-006`, `docs/decisions/018-…` → `ADR-018`. Plus its repo-relative path (`docs/domain/session-model.md`) and basename (`session-model.md`) — a domain / architecture / ops doc has only these. Grep BOTH the `.md` corpus and the code tree (`packages/` + `apps/`, `*.ts|*.tsx|*.mts|*.cts`, excluding `dist/`) for the token and the basename.
   2. **Keep every line-bearing form** in `.md` and code citers: colon `Spec-002:178`; line-word `Spec-002 line 178` / `lines 81, 107-113`; parenthesized `(line 178)`; named-section `§Acceptance Criteria line 81`; acceptance-criterion `AC4:108`; full path `docs/domain/session-model.md:61-77`; bare basename `api-payload-contracts.md:120`; and their line-word variants.
   3. **Normalize** each hit to `<doc-path>:NNN` — a range / list (`lines 81, 107-113`, `:61-77`) expands to one entry per endpoint. Pass the union as Subagent D's inbound list, tagging each entry's origin (`.md` vs code).

   Post-2026-07-08 ratchet (label-cite passes 5-6), NEW code-side raw cites are denied at pre-commit + CI in ALL single-line spellings — colon, docs-path, label line-word (`Spec-002 line 128`), bare-basename (`api-payload-contracts.md:120` / `line N`), and docs-rooted `§Heading line N` — and the 2026-07-17 md-lane deny (`checkMarkdownVolatileCites`) extends the same ratchet to `.md` citers, with WRAPPED pairs (label or `.md` path on one line, the `line N` anchor on the next) join-scanned in BOTH lanes; the backticked `§Heading` anchors (label `Spec-NNN §…` and path `docs/….md §…` forms) are gate-verified. Both trees STAY in D's enumeration for the deny-invisible residual: 3+-line wrap spans, label-less continuations (`line 86 (PresenceRead)` under a prior line's label), the deny's exemption namespaces (plan Tasks-block grammar, waiver-marked lines, `docs/superpowers/` + `.claude/` citers, frozen pins), and semantic drift of surviving durable `§`-cites (heading still exists but the claim under it changed). All documented in CAT-07. Additionally enumerate inbound ANCHOR exposure:
   - For each renamed / deleted **heading** in the diff: `grep -rn "§ *<old-heading>"` over `**/*.md` AND `packages/ apps/` (backticked and unbackticked) — route hits to Subagent B.
   - For each renamed / removed **export**: `grep -rn "#<oldName>\b"` over `docs/` and root md — route hits to Subagent A.

   Drop bare mentions with neither line nor anchor (a path or token with no `:NNN`, no `§`, no `#symbol`) — there is nothing cited to drift.

   **Subagent E dependent-doc enumeration — path, label, AND self.** Grepping the corpus for the modified file's stem under-builds E's input in two directions, and both directions are the stale-forward-reference shape CAT-07 records. Build E's list from three passes, run **per target** — once for each modified doc, excluding from passes 1-2 only THAT target and never the modified set as a whole — and dispatch E when ANY pass returns a hit:
   1. **Path / stem** — grep `**/*.md` for the target's repo-relative path and basename stem, **excluding the target itself**.
   2. **Label token** — for a target that has one (`docs/specs/005-…` → `Spec-005`), grep the token as well, **again excluding the target itself**. The self-exclusion is load-bearing, not hygiene: a labeled doc carries its own token in its title and throughout its body, so without it every labeled-doc edit self-matches, dispatches E unconditionally, and hands E its own target as an untagged dependent — noise that also disguises the genuine self-citation case pass 3 exists to surface. A citer writing `Spec-005 §Compacted Event Format` never mentions the stem, so pass 1 cannot see it, and in this corpus the label-only spelling is the COMMON one, not an edge case — the 2026-07-17 md-lane deny pushed volatile cites toward exactly this durable form.
   3. **Self** — grep the target for cites to its OWN sections, in every durable spelling: `§Heading` anchors naming a heading in the same file, self-label cites (a backticked `Spec-NNN §Heading` anchor written inside Spec-NNN itself), and **local Markdown fragment links** (`[§Status Flip Rule](#status-flip-rule)`) whose slug resolves to a heading this diff modified. The fragment form is a supported durable docs→docs cite, so a self-fragment goes stale under an intact anchor exactly as a `§` cite does — and it names neither the stem nor the label, so passes 1-2 cannot see it and a `§`-only pass 3 skips it. A target section and a stale citer of that section routinely coexist in one file; PR #256 produced two such instances, and file-scoped sweeping is precisely what missed them.

   Excluding only the target, rather than every modified file, is the load-bearing half of that rule. When one changed doc cites another, the citer is a genuine dependent and the likeliest place for a claim this very PR made stale; a blanket exclusion drops it, pass 3 cannot recover it (that pass sees only a file's cites to its OWN sections), and where no unchanged citer exists E may not dispatch at all.
...
- **It does not enforce CAT-08, CAT-09, or CAT-11** (outbound HTTP / file-link breakage; table-total arithmetic drift; a formatter normalizing malformed markup into format-stable corruption). A deterministic hook already covers each — `lychee` at CI for CAT-08, `table-total-coherence` at pre-commit + CI for CAT-09, `table-arity` at pre-commit + CI for CAT-11 — so dispatching a subagent would duplicate effort. CAT-11's hook covers its GFM-table slice; the row's other-markup residual carries no diff signal distinct from CAT-05's, so it stays audit-layer rather than earning a subagent.
- **It does not enforce CAT-10** (a gate reporting clean over work it did not do) — for the opposite reason. There is no doc-diff signal to key a subagent on: the regression lives in a gate's own logic, not in the corpus the staged diff touches, so this skill would be reading the wrong artifact. CAT-10 is discovered in code review and closed per instance by a gate of its own. The one place it touches this skill is inverted — a `/ripple-check` run that returns zero findings is itself a CAT-10 candidate until the dispatch actually covered the diff's signals, which is what the Phase 0 signal-to-subagent mapping exists to make checkable.
