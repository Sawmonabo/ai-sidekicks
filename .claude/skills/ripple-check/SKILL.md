...
The static pre-commit + CI hooks (`lychee`, `path-canonical-ripple`, `mermaid-set-coherence`, `cite-target-existence` — including `#symbol` verification + the volatile-tree line-pin deny — and `label-cite` — `§Heading` verification + the raw code→docs deny) catch the deterministic 80% — they always fire, run in <5s, and gate via branch protection. They cannot catch the semantic 20%:
...
- **CAT-07** — line-citation drift when both old and new lines are non-empty (`Spec-027:6 → :5`) — in `.md` citers; code citers now use gate-verified anchors, so their residual is semantic drift UNDER an intact anchor.
...
4. **Map the residual diff signals to subagents.** Use this lookup; dispatch only the subagents whose signals are present (typical invocations dispatch 1–3, full sweeps dispatch 5):
   ...
   **Subagent D inbound enumeration — `.md` AND code citers.** A line-shifting edit to a governance doc silently invalidates inbound line-cites in other `.md` docs AND in `packages/**`+`apps/**` code comments. Build D's inbound list from BOTH surfaces across every line-bearing form:
   ...
   Post-2026-07-08 ratchet (label-cite passes 5-6), NEW code-side raw cites are denied at pre-commit + CI in ALL single-line spellings — colon, docs-path, label line-word (`Spec-003 line 128`), bare-basename (`api-payload-contracts.md:120` / `line N`), and docs-rooted `§Heading line N` — and the swept code trees hold zero occurrences; the backticked `§Heading` anchors (label `Spec-NNN §…` and path `docs/….md §…` forms) are gate-verified. The code tree STAYS in D's enumeration for the deny-invisible residual: WRAPPED cites (label or `.md` path on one comment line, the `line N` anchor on the next), label-less continuations (`line 86 (PresenceRead)` under a prior line's label), and semantic drift of surviving durable `§`-cites (heading still exists but the claim under it changed). All documented in CAT-07. Additionally enumerate inbound ANCHOR exposure:
