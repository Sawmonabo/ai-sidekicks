...
4. **Map the residual diff signals to subagents.** Use this lookup; dispatch only the subagents whose signals are present (typical invocations dispatch 1–3, full sweeps dispatch 5):
   ...
   **Subagent E dependent-doc enumeration — path, label, AND self.** Grepping the corpus for the modified file's stem under-builds E's input in two directions, and both directions are the stale-forward-reference shape CAT-07 records. Build E's list from three passes, run **per target** — once for each modified doc, excluding from passes 1-2 only THAT target and never the modified set as a whole — and dispatch E when ANY pass returns a hit:
   ...
   2. **Label token** — for a target that has one (`docs/specs/006-…` → `Spec-006`), grep the token as well, **again excluding the target itself**. The self-exclusion is load-bearing, not hygiene: a labeled doc carries its own token in its title and throughout its body, so without it every labeled-doc edit self-matches, dispatches E unconditionally, and hands E its own target as an untagged dependent — noise that also disguises the genuine self-citation case pass 3 exists to surface. A citer writing `Spec-006 §Compacted Event Format` never mentions the stem, so pass 1 cannot see it, and in this corpus the label-only spelling is the COMMON one, not an edge case — the 2026-07-17 md-lane deny pushed volatile cites toward exactly this durable form.
