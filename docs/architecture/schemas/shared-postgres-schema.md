...
CREATE TABLE webauthn_credentials (
  ...
  prf_eval_input    BYTEA NOT NULL,         -- 32 random bytes minted once at registration; served back on every options reply
...
**`webauthn_credentials.prf_eval_input` is minted once and never regenerated (Plan-018 I-018-18).** PRF output is a function of the credential secret _and_ this input, so generating one per ceremony derives a different wrapping key each sign-in and leaves every previously wrapped envelope unopenable — a failure that first appears after a restart, not at the ceremony. It is `NOT NULL` because there is no correct value to backfill onto a row that lacks one: whatever input the first derivation used is already bound into what it wrapped.
