...
CREATE TABLE session_channel_directory (
  ...
  origin_node_id        TEXT NOT NULL,                 -- winning record's origin daemon (payload `originNodeId`)
  origin_seq            BIGINT NOT NULL,               -- winning record's per-(session, origin) channel-lifecycle counter
  origin_occurred_at    TIMESTAMPTZ NOT NULL,          -- winning record's envelope occurredAt (cross-origin comparator)
  origin_event_id       TEXT NOT NULL,                 -- winning record's envelope id (cross-origin comparator tiebreak)
  updated_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
