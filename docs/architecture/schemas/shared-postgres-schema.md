...
-- Extended by: Plan-016 (identity/profile columns via ALTER TABLE — see below)
CREATE TABLE users (
...
CREATE TABLE sessions (
  ...
  min_client_version TEXT,                       -- NULL = no floor; semver "MAJOR.MINOR" per ADR-018 §Decision #1
                                                 ...
                                                 -- Enforcement owned by Plan-002 attach flow.
  created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
...
-- Invariant: `sessions.id` is daemon-assigned UUID v7 per RFC 9562 for the
...
Plan-016 extends the [Plan-001 Users Identity Anchor](#users-identity-anchor-plan-001) with identity/profile columns via additive ALTER migrations, and adds the `identity_mappings` and `user_identity_keys` side tables (the latter T5.1, its own migration file). The base `users(id, created_at)` table is already present from Plan-001's first migration — Plan-016 does not re-create it.

```sql
-- Owner: Plan-016 (additive extension of the Plan-001 users anchor)
-- Strategy: add columns as NULL-able, backfill from Plan-016 registration flow, then
...
ALTER TABLE users
  ...
  ADD COLUMN identity_ref TEXT UNIQUE,           -- synthetic primary ref (PASETO kid / minted handle), NOT a {provider}:{external_id} projection — Plan-016 D-016-2; set NOT NULL after backfill
...
-- Owner: Plan-016 (T5.1 — sibling of identity_mappings)
CREATE TABLE user_identity_keys (
  id               UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id   UUID NOT NULL REFERENCES users(id),
  key_fingerprint  TEXT NOT NULL,        -- per-key selector; the Plan-012 identityKeyFingerprint value
...
**`user_identity_keys` is the device roster, and is multi-row per user by construction (Plan-016 T5.1/I-016-12).** One row per linked device's identity key — a user with N linked devices holds N rows, which is the ordinary Remote Control case rather than an edge case ([trust-and-identity.md §Edge Cases](../../domain/trust-and-identity.md#edge-cases), [user-and-device-model.md](../../domain/user-and-device-model.md)); `UNIQUE(user_id, key_fingerprint)` mirrors `identity_mappings`' `UNIQUE(provider, external_id)` shape (same owner, same anchor, same multi-row idiom). Registration is register-once: a different `public_key` under an existing fingerprint is refused before any row mutation (the control-plane half of ADR-021's Refuse-On-Rotation Invariant). The `REFERENCES users(id)` FK deliberately places the table inside the [Spec-020 §Path 2](../../specs/020-data-retention-and-gdpr.md#shred-fan-out) exhaustive inbound-FK erasure closure — the opposite of `daemon_signing_public_keys`' FK-free choice, because a device's long-term identity key is person-bound (the machine-key exemption does not transfer) and every consumer (Plan-028 bundle admission, Plan-012 attestation delivery, Plan-024 dispatch intake) verifies at live time, so no retained row needs post-erasure re-verification. Reads are scoped to the owning user via `UserIdentityKeyRoster` (Plan-016 I-016-13); key bytes never ride `UserProjection` (I-016-14).
...
Backs the relying party's half of the WebAuthn ceremony [ADR-010](../../decisions/010-paseto-webauthn-mls-auth.md) requires, owned by Plan-016 Phase 6 (T6.1 — CP-016-14). The Electron main process calls these routes over its own authenticated control-plane channel per [Spec-021 §Main Process Responsibilities](../../specs/021-desktop-shell-and-renderer.md#main-process-responsibilities); no JSON-RPC method string is involved.
...
Both tables carry `REFERENCES users(id)` for the same reason `user_identity_keys` does — it places them inside the [Spec-020 §Path 2](../../specs/020-data-retention-and-gdpr.md#shred-fan-out) exhaustive inbound-FK erasure closure, and the closure costs nothing because a WebAuthn credential is verified live and no retained row re-verifies one post-erasure.

---

## Token Revocation

Backs `POST /auth/revoke-all-for-user` (see [security-architecture.md §Bulk Revoke All For User](../security-architecture.md#bulk-revoke-all-for-user)). Cross-plan auth infrastructure, not Plan-016 identity schema.

```sql
-- Owner: the post-V1 token-revocation build
...
**Multi-region propagation:** The control plane writes a revocation row to the local region, then propagates via Postgres logical replication (publication/subscription) to peer regions. Propagation is best-effort and eventually consistent; see [security-architecture.md §Bulk Revoke All For User](../security-architecture.md#bulk-revoke-all-for-user) for the eventual-consistency window analysis.

---

## Runtime Node Attachments (Plan-002)
...
-- One-active-session enforcement (Plan-002 I-002-5; Spec-002 §Resolved Questions and V1 Scope Decisions — "one active session at a time in v1"):
...
-- on reconnect, while a 'revoked' row is refused — revocation is terminal (Plan-002 T3.2/P10).
...
## Session Directory and Relay (Plan-028)
...
-- Owner: Plan-028
-- Durable cross-session ephemeral-key reuse guard: Spec-028 §The encryption envelope requires the
...
-- Each endpoint mints a fresh ephemeral X25519 key pair per session (Spec-028), and the
...
-- is that store (OD-008r-2). The PK on the public key is the DB-level
...
-- with relay.bundle_rejected (Spec-028) and treating a SAME-session
-- re-presentation as an idempotent admit ONLY for the endpoint that first claimed the key
-- (Spec-028's reconnect-resume step; a different endpoint
...
-- Spec-028 requires a reused key to be rejected across ALL distinct sessions, so
...
-- the security property. Bounding retention would weaken the Spec-028
-- forward-secrecy guarantee and is therefore a decision of Spec-028's own, not a code-level
-- optimization.
...
## Session Channel Directory (Plan-014)

The control-plane half of the channel directory ([Spec-014 §Interfaces And Contracts](../../specs/014-multi-agent-channels-and-orchestration.md#interfaces-and-contracts), D-014-22): the store the `channel.directoryPublish` ingest mutation writes and the `ChannelList` read serves, so a device that did not create a channel can still list it. **Plan-014 owns the whole directory** — the daemon-side producer and this store alike — so the ingest mutation, this DDL, and the read path have one owner and one shape. **Forward-declared:** the additive migration ships with Plan-014's channel-directory control-plane task; the DDL is pinned here so that task, the ingest mutation, and the `ChannelList` read share one canonical shape.

Channel **content** never reaches the control plane — it transits the relay end-to-end sealed. What this table holds is channel existence metadata only, which is the deliberate, scoped disclosure Spec-014 records.

```sql
-- Owner: Plan-014 (channel-directory control-plane leg per D-014-22)
...
CREATE TABLE session_channel_directory (
  ...
  session_id        UUID NOT NULL REFERENCES sessions(id), -- scopes the ChannelList read and the Spec-020 erasure closure
...
--  predicate rather than trusting state alone, because the sweep is hourly and a row can sit past its TTL until it runs — Spec-012
--  §TTL sweep disposition (V1));
-- 'expired' records a TTL sweep. Value set = the storage-lifecycle SUBSET of the Spec-012 replicationStatus wire enum:
-- the degradation states ('over_cap' / 'quota_exceeded') mean NO relay upload happened (Spec-012 failure table), so
...
-- ROW disposition differs by trigger (Spec-012 §TTL sweep disposition (V1)):
...
--   transaction then stamps bytes_reclaimed_at and NULLs publisher_user_id together.
--   That order is mandatory: this row is the ONLY index into the stored object,
...
-- always) — so neither needs an index of its own. The RECLAIM-RETRY pass does:
-- it must find tombstones whose object-store delete has not confirmed, and bytes_reclaimed_at IS NULL
...
CREATE TABLE artifact_relay_blobs (
  ...
  session_id               UUID NOT NULL REFERENCES sessions(id),
  publisher_user_id UUID REFERENCES users(id) ON DELETE SET NULL,  -- anonymize-class (CP-020-6); NULL after publisher erasure.
                           -- Second, lifecycle-driven writer: the TTL sweep NULLs it — but in the
                           ...
                           -- (Spec-012 §TTL sweep disposition (V1)). An unreclaimed tombstone therefore still names
                           -- its publisher, and is covered by this same ON DELETE SET NULL if erasure arrives first.
  size_bytes               BIGINT NOT NULL,
  chunk_size_bytes         INTEGER NOT NULL,   -- fixed 8 MiB in V1 (Spec-012 artifact_relay_chunk_bytes)
  ...
  expires_at               TIMESTAMPTZ NOT NULL,          -- tier-derived TTL deletion trigger; re-anchored to now + tier TTL on every successful re-pin (Spec-012 Publish steps 3-4: a re-pin is a fresh grant of the same bytes)
  bytes_reclaimed_at       TIMESTAMPTZ,        -- NULL until the sweep confirms the object-store bytes are gone; the
                                               ...
                                               -- reclaim is a plain idempotent object-store delete.
  created_at               TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- Owner: Plan-012
-- One row per (blob, intended recipient node): carries the wrapped CEK for one attested (user, node) — encrypted to that node's DURABLE artifact-encryption X25519 key (Spec-012 Publish step 3; never the ADR-010 session-ephemeral keys, which are zeroed at session end and would orphan the CEK on restart), thumbprint-tagged so the fetching daemon selects the right private key after restart/rotation — the relay cannot unwrap;
-- per Spec-012 Publish step 3 this row is the wrapped CEK's ONLY store, never the durable artifact.published event — deleting it is a true shred), delivery state (delivered_at NULL = undelivered;
...
-- never inferred from the last chunk GET — Spec-012 Fetch step 6; the acked row is resolved from the
-- fetch token's own (user, node) DPoP-bound claims, never a caller-supplied node_id, so a node presenting a token minted for itself cannot mark a sibling delivered; mint-time authorization is user-granular (Spec-012 Fetch step 5) and this ack proves no CEK unwrap, so a COMPROMISED
...
--  the retention bound. Spec-012 §TTL sweep disposition (V1)). Hard-DELETE class in the CP-020-6
...
-- consistent after erasure. The SAME hard delete has a second, lifecycle-driven trigger: the TTL
...
-- neither the Spec-020 PII data map nor the CP-020-6 closure at either stage. Backup honesty (Spec-012 §State And Data Implications): Postgres
...
-- separately-destroyable KEK (Spec-020 §Daemon Master Key precedent); otherwise shred is incomplete.
CREATE TABLE artifact_relay_recipients (
  ...
  user_id    UUID NOT NULL REFERENCES users(id),  -- hard-DELETE class (CP-020-6); erasure removes ALL of a user's node rows
...
-- Thumbprint→node injectivity within one user's rows for one blob.
-- This is what makes the Spec-012 Fetch step 5 mint selector resolvable: the caller presents the
...
-- attestation spoof Spec-012 Publish step 3 names as a V1 residual — and this index converts it from a silent
...
-- violate the PK and a second row per node would break the I-012-7 per-node delivery refcount. The screen
-- covers SURVIVING rows as well as the submitted set (Spec-012 Publish step 3):
...
## Rate Limiting Tables (Plan-019)

Admin bans (`admin_bans`) are shared by both deployments. Escalation state (`rate_limit_escalations`) is self-host only; hosted deployments use Cloudflare Durable Objects (`RateLimitEscalationDO`) for escalation state and persist nothing in Postgres for that path. The self-host sliding-window counters live in `ratelimit_*`-prefixed tables that `rate-limiter-flexible` auto-creates on first use — library-managed, deliberately absent from this hand-authored schema and from the migration sequence ([Plan-019 §Data And Storage Changes](../../plans/019-rate-limiting-policy.md#data-and-storage-changes)).
...
CREATE TABLE admin_bans (
  ...
  issued_by       TEXT NOT NULL,                  -- operator attribution, server-derived from the operator-token context (Plan-019 D-019-1: 'deployment-operator' in V1 — no user principal exists on this surface); deliberately no FK — rows survive user deletion (Plan-019 D-019-13)
...
-- the issue path supersedes it by atomic revoke-then-insert (Plan-019 D-019-12). This index also
...
## Notification Queue (Plan-017)

Durable per-user delivery records for the offline leg of [Spec-017 §Cross-Device Delivery](../../specs/017-notifications-and-attention-model.md#cross-device-delivery): V1 delivers notifications to currently-connected devices over the SSE subscription, and **if no device is connected the notification is queued in the control plane** and delivered as a batch on the user's next connect, replayed from the last delivered position. This table is that queue — the substrate the spec's catch-up sentence assumes. **Forward-declared:** the additive migration ships with the Plan-017 Phase 2 preference-and-queue-storage leg ([Plan-017 §Implementation Phase Sequence](../../plans/017-notifications-and-attention-model.md#implementation-phase-sequence)); the DDL is pinned here so the emit path, the reconnect catch-up read, the expiry sweep, and the migration share one canonical shape.

**Deliberately absent: no cursor table, no per-device delivery state, no delivery-attempt or backoff column, no coalescing or cross-device dedup key.** Each omission tracks a behavior Spec-017 does not state, and columns are not added ahead of the behavior that would write them. The catch-up cursor is derived, not stored (see the delivery model below). Per-device state has no V1 writer: the spec scopes V1 delivery to the user and defers per-device fan-out — and with it any duplicate-suppression across devices — to the V2 push/digest leg. Redelivery attempts, backoff, and coalescing of related notifications are likewise unstated: a queued row is either pushed once (and stamped) or stays owed until it expires.

```sql
-- Owner: Plan-017 (Phase 2 ships the additive control-plane migration)
...
The `attention_trigger` and `severity` domains are byte-identical to the `AttentionItem` contract's `trigger` and `severity` unions in [api-payload-contracts.md](../contracts/api-payload-contracts.md), which ground them in [Spec-017 §Required Behavior](../../specs/017-notifications-and-attention-model.md#required-behavior)'s minimum trigger set and [Spec-017 §Default Behavior](../../specs/017-notifications-and-attention-model.md#default-behavior)'s actionable/informational split; the CHECK constraints exist so a queued row cannot carry a trigger the client has no rendering for. The column-name qualification is the only divergence from the wire shape, and it is a keyword-avoidance rename, not a domain change. `summary` is carried rather than re-derived at delivery time because the queue's entire purpose is delivery to a client that was absent when the attention state was derived.

**Delivery model (a cursor without a cursor table).** The catch-up position is the pair (`delivered_at IS NULL`, `queue_sequence`): the reconnect read selects the user's undelivered rows in `queue_sequence` order, pushes them as one batch over the [Spec-017 §Desktop-to-Desktop Delivery](../../specs/017-notifications-and-attention-model.md#desktop-to-desktop-delivery) SSE subscription, and stamps `delivered_at` on exactly the rows it pushed — so the next connect resumes where this one stopped without a stored per-user cursor row to keep in sync. `queue_sequence` rather than `queued_at` carries the order because rows inserted in one transaction share a single `now()`: timestamps tie, and a timestamp-keyed resume across a tie can skip or repeat a row. `notification_preferences` filtering happens at emit time, before a row is written ([Spec-017 §Desktop-to-Desktop Delivery](../../specs/017-notifications-and-attention-model.md#desktop-to-desktop-delivery) — non-matching events are dropped at the control plane), so the queue holds only notifications the user has already opted into and the catch-up read applies no further filter.

**Retention (7 days, then permanent deletion).** `expires_at` defaults to `queued_at + 7 days`, the one retention figure [Spec-017 §Cross-Device Delivery](../../specs/017-notifications-and-attention-model.md#cross-device-delivery) states, and the CHECK keeps it strictly after `queued_at` so a row can never be born expired. The purge sweep deletes every row past `expires_at`, delivered or not: an undelivered row is "expired and permanently deleted" by the spec's own words, and a delivered row has discharged its purpose with no stated longer-retention obligation. Deletion is permanent and leaves no tombstone — this queue is a delivery buffer, not an audit surface; the canonical event each row references stays in the emitting daemon's local log per ADR-017.
...
-- is a V1.1 extension (ADR-017 §Node-Scope Anchor Witnessing; Spec-005 §Daemon-Scope Event Binding).
...
**Verification**: an audit reader resolves the emitting daemon's Ed25519 public key by `node_id` from the [§Daemon Signing Public Keys](#daemon-signing-public-keys-plan-005--verification-key-roster) verification-key roster below and checks `root_signature` over the anchor claim — the row's coordinates and root together, per [Spec-005 §Anchoring Cadence](../../specs/005-session-event-taxonomy-and-audit-log.md#anchoring-cadence) — so a stored row whose span or log identity was relabeled after signing fails verification rather than passing a coverage test on unsigned coordinates. V1 ships no daemon signing-key rotation ceremony — registration is register-once per `(session_id, node_id)` and a different-key registration is refused, per [Security Architecture §Per-Event Daemon Signature](../security-architecture.md#per-event-daemon-signature); validity-window resolution of superseded keys is the V1.1+ extension a specified rotation ceremony would unlock. Anchor cadence defaults (`ANCHOR_INTERVAL_EVENTS = 1000` events or `ANCHOR_INTERVAL_SECONDS = 300` seconds, whichever first) are set in [Spec-005 § Integrity Protocol](../../specs/005-session-event-taxonomy-and-audit-log.md#integrity-protocol).

---

## Daemon Signing Public Keys (Plan-005 — Verification-Key Roster)

The `NodeId`-keyed resolution surface behind [Security Architecture §Per-Event Daemon Signature](../security-architecture.md#per-event-daemon-signature): one session-scoped Ed25519 PUBLIC key per `(session, node)`, registered by the emitting daemon after a successful attach via `runtimenode.signingkeyregister` (daemon-called mutation) and resolved by verifiers via `runtimenode.signingkeyroster` (query) — both registered in the [api-payload §Signing-Key Registration Method Registry](../contracts/api-payload-contracts.md#signing-key-registration-method-registry). **Forward-declared:** the additive migration ships with Plan-005 T4.10 (Phase 4, CP-005-7 leg B / CP-002-5); the DDL is pinned here so the registration service, the resolution query, and the migration share one canonical shape.

```sql
-- Owner: Plan-005 (T4.10 additive control-plane migration per CP-005-7 leg B / CP-002-5)
...
-- refused with typed runtimenode.signingkeyregister_conflict, never overwritten (the Plan-005 T4.2 refuse_on_rotation
...
-- so this table sits outside the Spec-020 §Shred Fan-Out Path-2 REFERENCES users(id)
...
## Session Terminal Lease (Plan-022)

Per-shell terminal write-lease coordination record ([Spec-002 §Required Behavior](../../specs/002-runtime-node-attach.md#required-behavior)) — the current holding **device** only, one row per shell of a session, projected by the `runtimenode.roster` join as `RuntimeNodeRosterResponse.controlHolder`, which carries one entry per held shell. **Forward-declared:** the additive migration ships with the Plan-022 Phase 3B lease leg; the DDL is pinned here so the roster projection, the erasure fan-out, and the Phase 3B migration share one canonical shape. Invariant-compliant by construction: a single current-state row per shell (same coordination tier as `runtime_node_presence`), never an event log — the `pty.control_changed` transition stream stays in the daemon-local event store per ADR-017.

**Deliberately absent: no lease-expiry or authority-epoch column.** A time-bounded lease with an authority-issued fence is the correct shape when the _control plane_ is the lease authority; in V1 it is not ([Spec-002 §Required Behavior](../../specs/002-runtime-node-attach.md#required-behavior) makes the terminal-owning daemon the sole authority). Server-owned columns here would make the control plane a **second writer** on this row, contradicting the single-producer write model below, its no-cross-writer-lock-ordering property, and ADR-017's daemon-local transition stream — and they would fence the record rather than the terminal, since enforcement lives on the holder's own machine. The time-bounded design belongs with the future relay-borne remote-take leg Spec-002's forward constraint names, which ships its own additive migration when that write leg lands; dormant columns are not added ahead of it.

```sql
-- Owner: Plan-022 (Phase 3B ships the additive migration)
CREATE TABLE session_terminal_leases (
  session_id            UUID NOT NULL REFERENCES sessions(id),
  terminal_id           TEXT NOT NULL,     -- the shell this lease is for: the daemon's own handle for it, never client-minted. The lease is one per shell, so the row identity is the pair below and a take on one shell moves no other shell's lease
  holder_device_id      TEXT,              -- the DEVICE holding this shell, in the identifier shape the presence surface carries; NULL = lease free (writes refused; Spec-002 null-holder-refuses-writes). A device and not an account: one account's several devices contend for the same shell, so an account-grained holder could not tell the device that must go read-only from the one that may write. Deliberately NO user FK, which places this table outside the Spec-020 §Shred Fan-Out Path-2 REFERENCES users(id) closure — its erasure disposition rides its own session_id edge instead (below)
  ...
  transition_seq        BIGINT NOT NULL,   -- daemon-owned strictly-increasing per shell (persisted daemon-side across restarts); stale transport retries compare lower and are discarded. Per shell rather than per session, so two shells' transitions never serialize against each other
  transitioned_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY (session_id, terminal_id)
);

CREATE INDEX idx_session_terminal_leases_holder ON session_terminal_leases(holder_device_id);
```

**Write model (single producer).** The terminal-owning daemon is the sole lease authority ([Spec-002 §Required Behavior](../../specs/002-runtime-node-attach.md#required-behavior)): it adjudicates every take/release and publishes each transition to the control plane via the `runtimenode.leaseupdate` projection-sync mutation ([api-payload-contracts §Session Terminal-Control Method Registry](../contracts/api-payload-contracts.md#session-terminal-control-method-registry)); the control plane persists what the daemon publishes through a **producer-bound, monotonic conditional upsert** carrying the api-payload contract's **caller-authorization** predicates **inside** the write statement rather than in a preceding probe (active attachment for `(node_id, session_id)` owned by the verified caller), so the form is `INSERT … SELECT … WHERE EXISTS (…) ON CONFLICT (session_id, terminal_id) DO UPDATE … WHERE ((session_terminal_leases.node_id = EXCLUDED.node_id AND session_terminal_leases.transition_seq < EXCLUDED.transition_seq) OR (session_terminal_leases.node_id <> EXCLUDED.node_id AND <recorded-producer-departed>)) AND EXISTS (…)` — the authorization `EXISTS` repeated on **both** arms, because `ON CONFLICT … DO UPDATE … WHERE` is evaluated only on conflict and a bare `VALUES` insert would bypass it against an absent row (same producer ⇒ the sequence must increase — an equal-or-lower retry is acknowledged and discarded; an authorized producer re-bind re-baselines the sequence). The **producer re-bind** condition rides **inside** that statement, closing a TOCTOU: `<recorded-producer-departed>` is the departure disjunction over the row's **recorded** producer — `NOT EXISTS` an active `runtime_node_attachments` row for `(session_terminal_leases.node_id, session_id)`, `OR EXISTS` a `runtime_node_presence` row for it reading `offline` — so a different node is accepted only when the recorded producer has stopped being a live terminal host, evaluated **under the conflicting row's lock in the same statement as the write**, never in a preceding probe. A probe-then-write split here is exploitable in both directions: two replacement nodes that both observed the producer offline would both pass the probe and each overwrite the other (the second write must instead re-evaluate against the first's committed re-bind and refuse, because the new recorded producer is live), and a returning former producer's delayed publish would overwrite its successor (it must instead be refused on the different-node arm while the successor is live). With the predicate in the statement both races serialize on the lease row's conflict lock and re-evaluate against the current row, so exactly one recorded producer survives any interleaving. A zero-row result is therefore ambiguous across all three refusal causes — stale `transition_seq` (acknowledged and discarded), failed caller authorization, refused re-bind — and needs in-transaction classification (re-read and distinguish; never collapse them). Because exactly one daemon owns a session's shells, writes to a given row are serialized at the producer — no cross-writer lock ordering exists on this table, and the `runtime_node_attachments` / `runtime_node_presence` facts the predicates consult are **read, never written** (each an `EXISTS` inside the single write statement — one snapshot, no lock taken on those tables), so this table registers no entry in [§Lock Ordering Across Shared Tables](#lock-ordering-across-shared-tables) below; the `node_id` binding and monotonic `transition_seq` make that single-producer assumption enforced rather than assumed (a stale delivery or a non-owning attached node's daemon cannot overwrite the projection). The holder is cleared by the daemon-published auto-releases (holder liveness/attachment drop; holder authorization loss — the holding device suspended or revoked; and the agent-run write-burst release on the acquiring run's first lifecycle transition out of `running` — `auto_released_run_idle`, [Spec-002 §Required Behavior](../../specs/002-runtime-node-attach.md#required-behavior)), and `idx_session_terminal_leases_holder` serves exactly the two holder-keyed sweeps: the liveness-drop clear and the authorization-loss clear, each of which names the departing device (the run-idle release needs no holder-keyed sweep — the daemon resolves it through the acquiring-run tag on its daemon-local lease record — re-bound on each agent-path acquisition, so it names the most recent acquiring run — never a holder lookup). The roster's `controlHolder` is not a byte-mirror of this column: the read leaves a row's entry out while that row's `node_id` carries `runtime_node_presence.health_state = 'offline'` — a read-time predicate over the verdict Plan-002's heartbeat sweep already derived, writing nothing, so the read-side surface stays honest during the window in which no successor host has exercised the `offline` re-bind disjunct above ([api-payload §Session Terminal-Control Method Registry](../contracts/api-payload-contracts.md#session-terminal-control-method-registry)).

**GDPR erasure (hard-DELETE class, reached through `session_id`).** The row's only identity reference is the session it belongs to, so it carries **no user FK and no anonymize-class mechanism**: `session_id UUID NOT NULL REFERENCES sessions(id)` places it among the session-scoped rows the [Spec-020 §Shred Fan-Out](../../specs/020-data-retention-and-gdpr.md#shred-fan-out) Path-2 hard delete of `sessions` removes transitively, and `holder_device_id` names a device rather than pointing into `users`, so no reference to the erased person survives for a `SET NULL` to sever. **Ordering (the daemon stays the sole lease authority):** this projection row never changes without the daemon's `pty.control_changed` broadcast, so the documented erasure flow revokes the person's devices and node attachments **before** the `DELETE` — the authorization-loss force-clear has then already freed the lease daemon-side and published the transition, and the row that is deleted afterwards is an already-free one (the [GDPR Manual Erasure Runbook](../../operations/gdpr-manual-erasure-runbook.md) Precondition carries the operator check; an out-of-flow direct DELETE still ends with the lease gone but bypasses the broadcast — exactly why the ordering is required, never the primary transition). Canonical dispositions: [Spec-020 §Shred Fan-Out](../../specs/020-data-retention-and-gdpr.md#shred-fan-out); operator verification in the [GDPR Manual Erasure Runbook](../../operations/gdpr-manual-erasure-runbook.md).
