...
Admin bans (`admin_bans`) are shared by both deployments. Escalation state (`rate_limit_escalations`) is self-host only; hosted deployments use Cloudflare Durable Objects (`RateLimitEscalationDO`) for escalation state and persist nothing in Postgres for that path. The self-host sliding-window counters live in `ratelimit_*`-prefixed tables that `rate-limiter-flexible` auto-creates on first use — library-managed, deliberately absent from this hand-authored schema and from the migration sequence (`Plan-019 §Data And Storage Changes`).
