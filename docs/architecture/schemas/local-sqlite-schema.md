...
The wrapped home of the key that seals every `content_payload` in one session — deliberately a mirror of `participant_keys` rather than a new custody idea, so the machinery that already re-wraps that table covers this one. Keyed by session, because the body is machine-authored session work product rather than anything a person typed, so there is no per-participant erasure axis to key it on ([Spec-022 §PII Data Map](../../specs/022-data-retention-and-gdpr.md#pii-data-map), the artifact-payload posture). The row is created lazily on the session's first content-bearing append, so a session that never runs an agent stores no key.
