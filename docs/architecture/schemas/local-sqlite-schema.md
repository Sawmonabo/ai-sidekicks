...
CREATE TABLE artifact_manifests (
  ...
  content_hash       TEXT NOT NULL,              -- SHA-256 content address (OCI `digest`); intrinsic to a content-addressed manifest (I-014-1), set at insert from the Task-2 ingest hash — D-014-1
  size_bytes         INTEGER NOT NULL,           -- OCI manifest-descriptor `size` (payload byte length); set at insert from Task-2 ingest, never a payload-less row — D-014-1
