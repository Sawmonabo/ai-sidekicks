...
-- schema work). SQLite has no ALTER TABLE ... ADD CHECK on an existing table (sqlite.org/lang_altertable.html),
-- so a trigger pair is the idiomatic equivalent: abort any terminal run_lifecycle write whose runId/runVersion
-- key is NULL — the UNIQUE index above cannot catch it (NULLs are distinct). The BEFORE UPDATE OF payload leg
-- also enforces stub-preservation against the compactor (the key is kept for the row's whole retention life).
