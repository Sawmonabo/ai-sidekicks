...
```ts
// RuntimeNodeSigningKeyRegister — control-plane tRPC ONLY, daemon-called post-attach
// (Plan-006 T4.10 per CP-006-7 leg B / CP-003-5; producer-bound like runtimenode.leaseupdate)
...
// a different-key registration is refused with a typed conflict error and no row write.

// RuntimeNodeSigningKeyRoster — control-plane tRPC ONLY (the NodeId-keyed verification-key
// resolution surface per Security Architecture §Per-Event Daemon Signature)
