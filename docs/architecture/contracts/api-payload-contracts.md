...
interface SessionCreateResponse {
  sessionId: SessionId;
  state: SessionState;
  ownerUserId: ParticipantId; // = Postgres `sessions.owner_user_id`; set once at creation and immutable
...
// HistoryBackfill (Spec-031, 2026-08-03 amendment; typed by
// Plan-031's history-backfill leg, 2026-08-11). On the RELAY these shapes are
// application-level payloads INSIDE the established pairwise ciphertext envelope (Spec-031
...
// member and demultiplexed through the Plan-031-owned RelayApplicationPayload discriminated
// union — see the §Envelope-Interior Application-Payload Kind Registry below (Plan-031,
...
// JSON-RPC bridge methods in the §History-Backfill Bridge Method Registry below (Plan-031 —
...
interface HistoryBackfillRequest {
  kind: "history_backfill_request"; // the RelayApplicationPayload discriminant — see the §Envelope-Interior Application-Payload Kind Registry below (Plan-031, ratified 2026-08-12 as Plan-014 D-014-5); required, and an absent or unrecognized kind is dropped fail-closed by the coordinator demux rather than dispatched
...
interface HistoryBackfillChunk {
  ...
  servesForOrigins?: NodeId[]; // present iff terminal: the origin NodeIds this response claims to serve for — its own always, a migrated predecessor's only alongside rows/stubs actually verifying under that origin's key (an empty foreign claim is never credited; Spec-031). Inside the responder-signed member set below, so a claim list cannot be stripped or padded in carriage
...
interface RelayedSessionEventPayload {
  ...
  entry: HistoryBackfillEntry; // the origin triple verbatim (originNodeId, canonicalBytes, daemonSignature; stub carried for a compaction racing the live send) — forwarded as-is into the Plan-031 verify-and-append engine by session.relayEventDeliver
}
```

### History-Backfill Bridge Method Registry (Tier 5, Plan-031)

The client↔daemon bridge for relay-decrypted payloads (registered 2026-08-11, Codex PR #323 round 2 — Plan-031): pairwise decryption is client-side where bundle custody lives, while roster-resolved verification and log append are daemon authority, so the Spec-031 payloads cross the local [Spec-007](../../specs/007-local-ipc-and-daemon-control.md) JSON-RPC boundary through four daemon methods — an EXTEND of the Tier-1 `session.*` namespace root under the cross-plan-dependencies.md root-extension class (the Plan-024 lease-pair / Plan-016 goal-pair precedent), handler files under `packages/runtime-daemon/src/ipc/handlers/`, backed by Plan-031's own `packages/runtime-daemon/src/backfill/` engines. The first three are Plan-031's; the fourth is the live-relay sibling on Plan-031's inbound leg, riding the **same** verify-and-append engine (ADR-017's receive semantics are one path, live or deferred — backfill is that path deferred, not a second one).
...
| --- | --- | --- |
| `history_backfill_request` | `HistoryBackfillRequest` (above) | Plan-031 (owner) |
| `history_backfill_chunk` | `HistoryBackfillChunk` (above) | Plan-031 (owner) |
| `relayed_session_event` | `RelayedSessionEventPayload` (above) — the tagged wrapper nesting one `HistoryBackfillEntry` verbatim, consumed by the §History-Backfill Bridge Method Registry's `session.relayEventDeliver` row (Plan-031's inbound leg) | Plan-031 (owner) |
...
// ParticipantIdentityKeyRegister (Plan-018 T5.2, 2026-08-15 pass)
...
// ParticipantIdentityKeyRoster (Plan-018 T5.3, 2026-08-15 pass) — the DEVICE roster:
...
// participant.permission_denied 403s (I-018-13). Key bytes never ride
...
interface RememberedRuleListResponse {
  rules: Array<{
    ...
    sessionId: SessionId;
    participantId: ParticipantId; // the GRANTOR (audit key, not a match key — D-012-10)
    nodeId: NodeId;
...
//     row named the arm with no named schema, so Plan-031's relay union had no type to build the
...
//     and Plan-031's union references it (CP-014-4).
...
interface ArtifactKeyAttestationPayload {
  kind: "artifact_key_attestation"; // the RelayApplicationPayload discriminant (Plan-031, ratified as Plan-014 D-014-5)
  sessionId: SessionId;
...
type ChildRunIncompleteCause =
  ...
  | "pending_backfill" // the producing node's rows have not reached this node yet. Spec-031's
  //   coverage-state term, reused verbatim rather than paraphrased — it is the same
...
// coverage state — Spec-031 rules it out by name), NodeState "offline" (a node lifecycle state, which
...
interface NotificationEmitParams {
  participantId: ParticipantId; // recipient — must own sessionId (registry predicate)
...
// constrain. A single nullable participant id was rejected for the reason rejected it on the
