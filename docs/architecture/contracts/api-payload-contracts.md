...
// a caller without a live owned attachment (missing or foreign — or an unknown session, collapsed) is refused
// typed runtimenode.permission_denied (403/FORBIDDEN) before the floor or key comparison is reached.
...
// the caller must hold an ACTIVE session_memberships row (any role); a non-member or a caller
// naming an unknown session is refused typed runtimenode.permission_denied (403), evaluated
// BEFORE any existence or roster read — empty ({ entries: [] }) and denied stay distinguishable.
...
interface RuntimeNodeSigningKeyRosterEntry {
  ...
  registeredAt: string; // ISO 8601, daemon_signing_public_keys.registered_at verbatim
}
