...
**Server-side attachment-transition backstop (Plan-024 CP-024-5 audit delta, Codex PR #283 round 1, 2026-08-02).** The three producer-bound conditions above cannot repair one case by themselves. When the recorded producer's own attachment leaves the active set, condition (1) refuses that node's publishes until it re-attaches — a reconnecting daemon converges (condition (1) tests for A live attachment on `(nodeId, sessionId)` and pins no `attachmentId`, unlike the sibling §Signing-Key Registration Method Registry below), but a producer that never returns — host crash, machine powered off — never republishes at all, so a lease held at the moment of loss would stay advertised on the roster indefinitely, contrary to [Spec-003 §Required Behavior](../../specs/003-runtime-node-attach.md#required-behavior)'s disconnect auto-release. The control plane therefore clears the holder itself: in the SAME transaction that moves a `runtime_node_attachments` row out of the active set — an explicit `runtimenode.detach` or the heartbeat-expiry sweep — it applies `UPDATE session_terminal_leases SET holder_participant_id = NULL WHERE session_id = $1 AND node_id = $2 AND holder_participant_id IS NOT NULL`. That statement has exactly ONE implementation — a Plan-024-owned helper exported from `lease-projection-service.ts` beside the upsert it complements — and both Plan-003 departure sites call it with their open transaction handle rather than inlining the SQL, so this table keeps a single writer MODULE and each Plan-003 seam edit stays one call line. Four properties make this safe rather than a second writer: it is keyed on the lease PRIMARY KEY plus the `node_id` producer binding and never on `holder_participant_id` (so it adds no holder-keyed sweep to the [schema's enumerated three](../schemas/shared-postgres-schema.md#session-terminal-lease-plan-024)); it is FAIL-CLOSED-ONLY, moving `controlHolder` from a participant to NULL and never the reverse, so it manufactures no write authority; it leaves `node_id` and `transition_seq` UNTOUCHED, so the producer's monotonic sequence keeps working on return and condition (2)'s replacement-node re-bind carve-out is unaffected; and it is idempotent — a repeat, an already-free lease, or a lease bound to a different producer is a zero-row no-op. No `pty.control_changed` event is authored: the departed daemon is the sole author of that stream ([ADR-017](../../decisions/017-shared-event-sourcing-scope.md)) and is not present to author one, so the clear is a coordination-record change on the `runtime_node.*` server-derived-transition precedent (`degraded` / `revoked` are coordination records whose durable events are V1.1-gated — [cross-plan-dependencies §3](../cross-plan-dependencies.md#3-inter-plan-dependency-graph)). The governing spec sanctions exactly this shape rather than merely tolerating it: [Spec-003 §Required Behavior](../../specs/003-runtime-node-attach.md#required-behavior) already assigns node-departure detection to the control plane's heartbeat service as **server-derived** transitions the node never self-reports, already records that class as coordination records authoring no durable event, and states the lease requirement this backstop serves — auto-release on holder disconnect, "no dead-holder lockout". "Sole lease authority" in that spec scopes ADJUDICATION (who decides a take or release, and why no control-plane→daemon command path may forward one), not who may free a stranded coordination row — and this column already had a non-daemon writer in the `ON DELETE SET NULL` erasure backstop below. The window it covers is exactly the window in which no producing node is attached — the terminal is unreachable and no terminal write could be served — so converging to the fail-closed lease-free state loses nothing; when the producer returns it re-attaches, republishes its snapshot at a higher `transitionSeq`, and the true state (still-held or released) is restored by the ordinary monotonic path.
