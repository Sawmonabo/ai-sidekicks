...
**What the replies carry, and why the split is where it is.** The authentication-**options** reply carries the `rpId`, the origin, the challenge, the transaction id, and each candidate credential's persisted `prf_eval_input` (Plan-018 I-018-18), because the caller needs the PRF evaluation input **before** it runs the ceremony. The authentication-**verify** reply carries the verdict and the signing credential's stored `uv_mode` (I-018-16), because that one cannot be known any earlier: the desktop's credentials are discoverable, so the options leg offers no credential list and does not know which credential will answer. Refusals are `participant.webauthn_challenge_invalid` (400) for an unknown, consumed, or expired challenge or transaction id, and `participant.webauthn_verification_failed` (400) for every verification arm — bad signature, wrong origin, wrong `rpId`, UV mismatch, regressed counter — so the reply is no oracle for which check failed.
...
**What it carries, and where the HKDF runs (pinned 2026-09-01, Codex round 5).** The request carries two inputs — the credential's persisted PRF evaluation input, which the daemon received on the authentication-options reply and passes through unmodified (Plan-018 I-018-18), and the [Plan-022](../../plans/022-data-retention-and-gdpr.md) daemon-master-key HKDF salt. The response carries the finished **32-byte key-encryption key**. The derivation is therefore performed **inside the shell-hosted method**, so raw PRF output never crosses the RPC: main runs the ceremony, feeds the PRF bytes straight into HKDF-SHA-256 with the caller's salt and the contract-fixed info string `"ai-sidekicks/daemon-master/v1"`, and returns only the derived key. Putting the HKDF on the daemon side would have sent the authenticator's raw secret across a process boundary to be transformed by the party that does not own it, which is the shape this whole retirement existed to remove. The info string is fixed **by this contract** rather than supplied by the caller, because a caller-chosen info is a second axis on which two honest implementations silently derive different keys from the same authenticator. The two inputs are not interchangeable and neither substitutes for the other: the evaluation input selects **which secret the authenticator computes**, the salt selects **which key that secret derives**, and conflating them would move the wrapping key whenever either changed. Main answers it with the injected `PrfKeyDerivationService` of [Plan-023](../../plans/023-desktop-shell-and-renderer.md) CP-023-9 (producer task T-023r-4-7), which runs the WebAuthn PRF ceremony through the resolved per-platform binding. The KEK crosses this one channel and no other; it appears in no `SidekicksBridge` type and reaches no renderer. This registry replaces the retired `webAuthn.deriveKeyMaterial` preload-bridge method, which let the untrusted renderer choose the PRF salt and returned derived key material across the bridge.
