...
| `session.history_backfill_unavailable` | A peer history backfill source refuses the request at response level — a transient serving-side refusal, never the coverage state itself: the requester keeps its two-axis `pending_backfill` degraded state and re-requests on retry, restart, or presence transition, converging idempotently on canonical `id`s under first-delivery-wins (`Spec-008`; registered by Plan-008's R4 backfill task T-008r-4-14 per the OD-008r-1 registration shape) | 503 |
