...
Chunk-integrity mismatch on relay fetch reuses `artifact.hash_mismatch` (409). The three reserved rows are named by Spec-014's 2026-07-08 cross-node relay amendment (the no-access-key row by its 2026-07-09 extension) and become live registrations with Plan-014's relay legs when tier order reaches them — that plan's relay-scope readiness-audit delta landed 2026-08-12 and restored it and Spec-014 `approved`, binding `artifact.relay_expired` to Task 9's TTL-sweep assertion, `artifact.too_large` to Task 7's over-cap arm, and both of the fetch rows to Task 8's.
