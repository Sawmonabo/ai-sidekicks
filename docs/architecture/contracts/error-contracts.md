...
| `mcp.config_scope_unsupported` | The mutation targets a config scope V1 does not accept writes for — only user scope is writable; Codex project-local `.codex/config.toml` and Claude `.mcp.json` / `local`-scope entries are read-only, and guidance names the file to edit, in the message only, never in event payloads (`Spec-028 §Fallback Behavior`, campaign B18) | 400 |
