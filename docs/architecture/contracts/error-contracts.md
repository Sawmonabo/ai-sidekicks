...
| `runtimenode.permission_denied` | Caller is not authorized for the signing-key surface — on `runtimenode.signingkeyregister`, the caller's verified principal holds no live active attachment for the named `(session, node)` pair (missing and foreign-owned attachments collapse into this one refusal, and a nonexistent session refuses identically — no attachment-existence or session-existence oracle, per this family's no-info-leak header); on `runtimenode.signingkeyroster`, the caller is not an active member of the session (any active role reads; evaluated before any session-existence or roster read, so non-member and nonexistent-session refusals are byte-identical). One code, two call sites with distinct messages — the `runtimenode.capabilityupdate_conflict` precedent; domain authz code, HTTP row per the `membership.permission_denied` convention; deliberately never tRPC `NOT_FOUND`, which this namespace reserves as the old-control-plane procedure-absence discovery signal (Plan-006 T4.10, Codex PR #274 round 3, 2026-07-31) | 403 |
