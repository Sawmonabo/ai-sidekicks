...
Artifact Storage uses an OCI-inspired manifest envelope with content-addressable storage (CAS) keyed by SHA-256 for deduplication. Locally, artifacts are stored on the filesystem. For shared artifacts, the relay eagerly pins participant-encrypted ciphertext in a digest-addressed, TTL-bounded relay blob store at publish time — so cross-node fetch works while the publishing node is offline — threat-model-scoped 2026-08-08 ([ADR-015 §Decision Log](../decisions/015-v1-feature-scope-definition.md#decision-log)): given an operational relay, and not against a compromised node of the fetching participant — with coordination rows and per-`(participant, node)` wrapped CEKs in Postgres (`artifact_relay_*`) and never payload bytes — wrapped to durable per-node artifact-encryption keys held daemon-local (`artifact_encryption_keys`, Spec-022 master-key custody), never to session-ephemeral keys; see [Spec-014 §Cross-Node Artifact Relay](../specs/014-artifacts-files-and-attachments.md#cross-node-artifact-relay-v1).
