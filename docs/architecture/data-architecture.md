...
Artifact Storage uses an OCI-inspired manifest envelope with content-addressable storage (CAS) keyed by SHA-256 for deduplication. Locally, artifacts are stored on the filesystem. For shared artifacts, the relay eagerly pins participant-encrypted ciphertext in a digest-addressed, TTL-bounded relay blob store at publish time — so cross-node fetch works while the publishing node is offline — with coordination rows and per-participant wrapped CEKs in Postgres (`artifact_relay_*`) and never payload bytes; see [Spec-014 §Cross-Node Artifact Relay](../specs/014-artifacts-files-and-attachments.md#cross-node-artifact-relay-v1).
