...
Plan-008-bootstrap (Tier 1: tRPC + sessionRouter + SSE) ┴─▶ Plan-001 Phase 5 (sessionClient)
...
| Plan | Why Standalone |
| --- | --- |
| Plan-001 | Session foundation — everything depends on this. _PR-level exception:_ Plan-001 Phase 5 (`sessionClient`) consumes Plan-007-partial + Plan-008-bootstrap as Tier 1 substrate prerequisites — see §3 row and §5 Tier 1 carve-out. Phase 1–Phase 4 remain dependency-free. |
| Plan-007 | Local IPC — daemon control surface, no shared tables, no upstream plan dependencies. _Carve-out:_ Plan-007-partial (Spec-007 §Wire Format substrate + `session.*` namespace + SDK Zod layer) is a Tier 1 prerequisite for Plan-001 Phase 5 — see §5 Tier 1 carve-out and Plan-007 §Execution Windows. |
