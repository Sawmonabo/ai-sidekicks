...
3. A `user.tokens_revoked_all` event is emitted per [Spec-006](../specs/006-session-event-taxonomy-and-audit-log.md) (payload `base + {revokedAt, tokenCount}`).
...
3. The control plane verifies each `SessionKeyBundle` signature against the peer's registered Ed25519 identity key before distribution — the resolution surface is the `user_identity_keys` roster ([shared-postgres-schema.md §Users and Identity](./schemas/shared-postgres-schema.md#users-and-identity-plan-018); Plan-018 T5.1/T5.3, registered 2026-08-15 — the store Plan-031's bundle-admission `RegisteredIdentityResolver` reads, CP-018-13 consumer c)
...
2. Client presents a PASETO v4.public connection token in the initial WebSocket handshake via two `Sec-WebSocket-Protocol` subprotocol values — `paseto-v4, <base64url(connectionToken)>` (value 1 `paseto-v4` names the scheme and is echoed by the relay as the negotiated subprotocol; value 2 carries the base64url-encoded token, since browsers cannot set custom WebSocket request headers — per Spec-031/Plan-031)
...
2. Signs the **anchor claim** with its session-scoped Ed25519 key (same key used for `daemon_signature`): the RFC 8785 canonicalization of `{endSequence, merkleRoot, nodeId, sessionId, startSequence}` per [Spec-006 §Anchoring Cadence](../specs/006-session-event-taxonomy-and-audit-log.md#anchoring-cadence)'s 2026-08-11 amendment — coordinates inside the signature, not beside it, so a stored or carried anchor record cannot be relabeled onto a different span or log after signing (previously the root alone was signed, which left the coverage test any consumer runs against the coordinates vacuous).
