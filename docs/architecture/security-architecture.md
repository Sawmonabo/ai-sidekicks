...
| Start workflow runs (adjudicated per start as the named Cedar operation action — never an approval category, never remembered; the chat-borne path carries a daemon-resolved authoring participant, never a client-supplied field, and refuses while that carrier is unregistered — [ADR-027](../decisions/027-chat-invoked-workflow-start.md)) | Yes | Yes | No | No |
| Steer/interrupt/cancel/rollback, pause/resume runs, and compact a bound run's provider context (authorized by session write access, never by run authorship — any participant holding a write-access role may steer, interrupt, or cancel any run in the session, including runs authored by another participant and runs targeting another participant's agent; "own" never scopes this row. Participant-triggered context compaction joined this row 2026-08-29, sharing the identical adjudication rather than minting a Cedar action of its own, per `Spec-005 §Participant-triggered context compaction` — a compaction target is always a binding this daemon holds, so its hosting node is always local and the cross-node clause below cannot arise for it. A run hosted on another participant's runtime node additionally routes through that node owner's authority per §Inter-Node Trust Boundaries, so cross-node steering is a request, never standing authority) | Yes | Yes | No | No |
