...
| --- | --- | --- |
| Desktop shell (same machine) | Socket access + 256-bit session token | Shell is the daemon client for the desktop tier; holds all session-scoped auth material per [Spec-021 §Trust Stance](../specs/021-desktop-shell-and-renderer.md#trust-stance); forwards renderer-originated requests with auth headers attached and response payloads sanitized. |
...
- **Not applicable** to the desktop renderer, which is not a direct daemon client per [Spec-021 §Trust Stance](../specs/021-desktop-shell-and-renderer.md#trust-stance).
