...
Every V1 feature has a governing spec, with one campaign-pending exception and one campaign caveat. **Feature #18** is the one feature with **no governing spec yet in the corpus** — Spec-028 is authored by the capability-enhancement campaign's B18 bundle and is not yet landed (see its row). **Features #19–#23** each have an existing governing spec that is cited today, and each depended on a **campaign amendment** to that spec: the B1/B3/B20/B6/B2 amendments are all **in-tree** with their specs re-promoted `approved` via the W1.5 batch gate (campaign Task 28, 2026-07-18); the remaining campaign dependency is each feature's W2 plan-task bundle — see each row. Cross-cutting V1 specs (identity, observability, rate limiting, data retention, relay) are listed separately in §Supporting V1 Specs below.
