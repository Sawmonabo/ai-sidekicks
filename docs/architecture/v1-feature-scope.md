...
Every V1 feature has a governing spec, with campaign caveats on #18–#23; feature #24 (Remote Control) is governed by [Spec-031](../specs/031-remote-control.md). Feature numbers are stable identifiers cited from other documents, so the list skips the numbers of features the product dropped rather than renumbering the survivors. **Feature #18**'s Spec-028 landed 2026-07-22 via the capability-enhancement campaign's B18 bundle and was promoted `approved` the same day via the campaign's W3 gate; Plan-028's doc gates closed 2026-08-14 — its targeted readiness audit landed 2026-08-12 and its `review → approved` promotion followed as its own PR (see its row). **Features #19–#23** each have an existing governing spec that is cited today, and each depended on a **campaign amendment** to that spec: the B1/B3/B20/B6/B2 amendments are all **in-tree**; their specs were re-promoted `approved` via the W1.5 batch gate (campaign Task 28, 2026-07-18), and Spec-004, Spec-012, and Spec-016 were re-flipped `review` 2026-08-03 by the V1 product-vision reconciliation amendment bundle — each restoring `approved` via its queued targeted readiness-audit delta, of which Spec-004's landed 2026-08-08 (PR #299), Spec-012's 2026-08-10 (PR #317), and Spec-016's 2026-08-11 (PR #321) — the reconciliation-flipped spec set fully restored, with Spec-005 remaining `approved` and Spec-006 flip-and-restored `approved` in the same PR #321 swap for its channel-lifecycle payload growth (the `channel.created` kind + member-pair mirror, the D-016-22 origin-ordering keys, and the run-interrupt trigger-union member); the remaining campaign dependency is each feature's W2 plan-task bundle — see each row. Cross-cutting V1 specs (identity, observability, rate limiting, data retention) are listed separately in §Supporting V1 Specs below.
...
| 18 | MCP server configuration and governance | Spec-028 + Plan-028 — **landed 2026-07-22** via the capability-enhancement campaign's B18 bundle (Spec-028 promoted `approved` the same day via the campaign's W3 gate; Plan-028 landed at `draft`, was promoted `review` 2026-08-12 by its targeted readiness audit — — and `approved` 2026-08-14 by its §Rollout Order step-2 promotion, completing the promotions feature #18's implementability required; code dispatches in tier order per Plan-028 §Preconditions). Scope: operator-managed trusted-server store, Cedar-gated per-tool overrides |
| 19 | Session time-travel (run rollback) | [Spec-004](../specs/004-queue-steer-pause-resume.md) (`rollback` intervention — campaign B2, **in-tree**: merged via PR #205; Spec-004 re-promoted `approved` 2026-07-18, the W1.5 gate / Task 28; re-flipped `review` 2026-08-03 by the V1 product-vision reconciliation amendment bundle, its restoring targeted readiness-audit delta queued) + forward `run.rolled_back` event ([Spec-006](../specs/006-session-event-taxonomy-and-audit-log.md), 2026-07-02 B1 amendment, **in-tree**: merged via PR #173, Spec-006 re-promoted `approved` 2026-07-18 — the W1.5 gate / Task 28) + the daemon-side turn-snapshot **file-restore** leg ([Spec-010](../specs/010-worktree-lifecycle-and-execution-modes.md) + [Plan-010](../plans/010-worktree-lifecycle-and-execution-modes.md), campaign B21→B22→B23 / Task 26, **landed 2026-08-09** via PR #303: the gate before Plan-004 rollback dispatch is met — the rollback cluster rides Plan-004's Phase-3B explicit-label supplement since the round-1 re-home, on tier order plus its precondition legs — the Codex conversation rewind reverts conversation only, so worktree restoration is the daemon's — re-verified 2026-08-26 and rebound: `thread/rollback` carries its own deprecation in the generated type, and Spec-005 binds the non-experimental successor `thread/fork` at an inclusive `lastTurnId` boundary, which mints a **new thread** and so re-points the run's live provider binding in the same operation) |
| 20 | Session goals | [Spec-016](../specs/016-multi-agent-channels-and-orchestration.md) (goal set/clear RPC — campaign B6, **in-tree**: landed 2026-07-06, `Spec-016 §Session Goals`; `session.goal_*` events per the 2026-07-02 Spec-006 B1 amendment, **in-tree**: merged via PR #173 — both specs re-promoted `approved` 2026-07-18, the W1.5 gate / Task 28; Spec-016 was briefly re-flipped `review` 2026-07-21 by the campaign B15 §Stop Conditions amendment and restored `approved` the same day by Plan-016's W2.5 targeted re-audit; re-flipped `review` again 2026-08-03 by the V1 product-vision reconciliation amendment, restored `approved` 2026-08-11 — Plan-016 with it — by that bundle's channel-directory targeted readiness-audit delta (PR #321)) |
| 21 | Session callback tools | [Spec-005](../specs/005-provider-driver-contract-and-capabilities.md) (registry shape — campaign B3, **in-tree**: merged 2026-07-05; Spec-005 re-promoted `approved` 2026-07-18, the W1.5 gate / Task 28) + [Spec-012](../specs/012-approvals-permissions-and-trust-boundaries.md) (Cedar governance — 2026-07-02 B20 amendment, **in-tree**: merged via PR #175; Spec-012 re-promoted `approved` 2026-07-18, the W1.5 gate / Task 28; re-flipped `review` 2026-08-03 by the V1 product-vision reconciliation amendment bundle and restored `approved` 2026-08-10 by its cross-user run-control authorization targeted readiness-audit delta, PR #317) |
| 22 | Execution postures and sandbox profiles | [Spec-012](../specs/012-approvals-permissions-and-trust-boundaries.md) (`executionPosture` authorization semantics — 2026-07-02 B20 amendment, **in-tree**: merged via PR #175; Spec-012 re-promoted `approved` 2026-07-18, the W1.5 gate / Task 28; re-flipped `review` 2026-08-03 by the V1 product-vision reconciliation amendment bundle and restored `approved` 2026-08-10 by its cross-user run-control authorization targeted readiness-audit delta, PR #317) + [Spec-005](../specs/005-provider-driver-contract-and-capabilities.md) (driver legs — campaign B3, **in-tree**: merged 2026-07-05; Spec-005 re-promoted `approved` 2026-07-18, the W1.5 gate / Task 28) |
| 23 | Realtime voice channels (capability-gated) | [Spec-016](../specs/016-multi-agent-channels-and-orchestration.md) (V1-scope-decision reservation — campaign B6, **in-tree**: landed 2026-07-06, `Spec-016 §Resolved Questions and V1 Scope Decisions`) + [Spec-006](../specs/006-session-event-taxonomy-and-audit-log.md) (reserved `realtime_*` family — 2026-07-02 B1 amendment, **in-tree**: merged via PR #173, Spec-006 re-promoted `approved` 2026-07-18 — the W1.5 gate / Task 28); gated on upstream Codex realtime-flag stabilization |
| 24 | Remote Control — linked devices, device liveness, and full parity from any device | [Spec-031](../specs/031-remote-control.md) |
...
| --- | --- | --- | --- |
| 1 | MLS relay E2EE | Pending audit of an MLS implementation (OpenMLS, mls-rs, or a post-audit TypeScript implementation); V1 ships pairwise X25519 + XChaCha20-Poly1305 per [ADR-010](../decisions/010-paseto-webauthn-mls-auth.md). | [Spec-031](../specs/031-remote-control.md) |
