...
Every V1 feature has a governing spec, with campaign caveats on #18–#23. **Feature #18**'s Spec-028 landed 2026-07-22 via the capability-enhancement campaign's B18 bundle and was promoted `approved` the same day via the campaign's W3 gate; implementation still gates on Plan-028's targeted readiness audit (see its row). **Features #19–#23** each have an existing governing spec that is cited today, and each depended on a **campaign amendment** to that spec: the B1/B3/B20/B6/B2 amendments are all **in-tree**; their specs were re-promoted `approved` via the W1.5 batch gate (campaign Task 28, 2026-07-18), and Spec-004, Spec-012, and Spec-016 were re-flipped `review` 2026-08-03 by the V1 product-vision reconciliation amendment bundle — each restoring `approved` via its queued targeted readiness-audit delta, of which Spec-004's landed 2026-08-08 (PR #299 / §6 NS-49) and Spec-012's 2026-08-10 (PR #NNN / §6 NS-54), leaving Spec-016 alone outstanding, with Spec-005 and Spec-006 remaining `approved`; the remaining campaign dependency is each feature's W2 plan-task bundle — see each row. Cross-cutting V1 specs (identity, observability, rate limiting, data retention, relay) are listed separately in §Supporting V1 Specs below.
