...
| `revoked` | The material has been replaced or invalidated. Old wrapped blobs from before rotation cannot be unwrapped after the fact (this is the mechanism that prevents crypto-shred circumvention via backup restore). For tokens: `jti` or `family` is on the revocation list. | [security-architecture.md §Daemon Master Key Rotation, §Token Revocation](../architecture/security-architecture.md#daemon-master-key-rotation) |
