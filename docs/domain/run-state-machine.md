...
- A liveness exemption granted for a run's pending blocking work — a pending driver ask or a pending pipeline approval request (the idle sweep's pending-blocking-work hard-skip) — is **expiry-bounded**: every such block carries a concrete deadline at creation (asks stamp `expiresAt`; run-blocking approval mints stamp `expiryAt` per Plan-010's run-blocking mint rule, campaign B15), and the exemption ends the moment the block expires or resolves — `driver_ask.expired` / `approval.expired` closes the exemption together with the block, and an expired block never counts as pending blocking work — so an unanswered ask or approval is a bounded wait, never an indefinite shield against reaping ([Spec-010 §Required Behavior](../specs/010-approvals-permissions-and-trust-boundaries.md#required-behavior)'s decoupled-but-coordinated rule; campaign B13/B15). The startup-reconciliation divergence halt (`waiting_for_input` carrying `recovery-needed`) is outside this bound by design — a human-action halt, not pending ask/approval work ([Spec-013 §Fallback Behavior](../specs/013-persistence-recovery-and-replay.md#fallback-behavior), campaign B5/B14).
...
- `waiting_for_input -> paused` (rollback intervention — campaign B2; driver input-ask expiry — Spec-010 Part-B fail-closed follow-up, 2026-07-17)
...
- `waiting_for_input`: recovery resumed the provider session but the reported position diverged from the daemon-recorded position; the local log is authoritative and a human decides how to reconcile (`recovery-needed` — `Spec-013 §Fallback Behavior`, campaign B5).
...
- From `completed` / `interrupted` / `failed`: rollback is the only exit from a terminal state. The run re-opens in `paused`; any later terminal event carries a higher `runVersion` (§Invariants). The terminal exit had released the run's execution root (workspace `busy` → `ready` — [Spec-008 §State And Data Implications](../specs/008-worktree-lifecycle-and-execution-modes.md#state-and-data-implications)), so a terminal-source rollback first re-acquires it; a root `busy` under another run refuses the whole intervention pre-dispatch ([Spec-003 §Required Behavior](../specs/003-queue-steer-pause-resume.md#required-behavior)), and the re-opened run holds the root again — re-acquisition **reactivates the run's durable execution context** (the terminal exit had stamped `run_execution_contexts.released_at`; the rollback's durable commit clears it atomically with the re-open, so active-root provenance never reports a re-acquired `paused` run as released (the conversation-only convergence below is the deliberate exception: a vanished-checkout re-open stays released, which is exactly what its resume refusal keys on)), while every path that does not confirm the conversation rewind releases the re-acquired hold and restores the release marker — the run stays terminal and the workspace returns to `ready` (same Spec-003 contract). A run whose root no longer exists (disposed ephemeral clone, retired worktree) — or whose recorded checkout root itself is gone, the vanished-checkout convergence (Codex PR #346 round 2: a live context is released at the confirmed re-open with that cause recorded; a terminal source arrives already released and keeps its release record) — rolls back **conversation-only** and skips re-acquisition — there is no root to hold — per the same Spec-003 contract; such a re-opened run is **non-resumable**: the `paused → running` resume is guarded on a live execution root, and a resume against a released, root-less context is refused with a typed error — re-execution is a fresh run through normal admission, never an activation against a nonexistent root ([Spec-003 §Required Behavior](../specs/003-queue-steer-pause-resume.md#required-behavior)). The resume guard carries a second refusal arm (the 2026-08-16 amendment's two-point classification backstop, same Spec-003 section): a run whose current position sits strictly below its newest current provider-side compaction boundary — reachable only through a rewind settled before the boundary's late delivery, the `boundary-diverged` class — is refused with its own typed error and is likewise non-resumable in V1, re-execution taking a fresh run through normal admission.
- From `running`: not directly legal — the orchestration layer first pauses the run (the existing pause path: interrupt + persist), then applies rollback from `paused` (pause-first, [Spec-003 §Required Behavior](../specs/003-queue-steer-pause-resume.md#required-behavior)); pause-first covers a **bare** rollback only — a rollback carrying the 2026-08-16 amendment's optional `replacementSend` edit-and-resend leg refuses a `running` source outright at admission instead, leaving it running untouched, because the message being replaced is still being answered (same Spec-003 section), so that composite reaches this machine only from the states below; the pause is an unguarded internal sub-step of the atomic rollback application, whose `expectedRunVersion` guard was evaluated once at admission against the pre-pause version. Every fail-closed pre-dispatch validation completes **before** the pause sub-step — a refused rollback of a `running` run leaves it running, untouched — and the internal pause queues **no resume event**; a queued resume already pending against the run is canceled as moot at application (`Spec-003 §Required Behavior`).
...
| `starting` | `interrupted` | Interrupt or cancel intervention | User-initiated stop while run setup is in progress or parked (e.g. blocked-in-setup per `Spec-008 §Fallback Behavior` — Tier-5 audit) |
...
| `waiting_for_approval` | `running` | Approval resolved | Resolution outcome (approved, rejected, or expired) delivered to the run; a rejected or expired outcome continues the run with the action refused — it does not terminate the run (Spec-010 — Tier-5 audit); exception: a denied — or expired: silence is denial (campaign B15) — pre-turn moderation gate (`category: 'gate'`) does not continue — Plan-014's moderation gate system-cancels it with `trigger: 'moderation_denied'` (Spec-014 D-014-10), exiting via the interrupt row below |
...
| `waiting_for_approval` | `interrupted` | Interrupt or cancel intervention | User-initiated stop while waiting, or the system-cancel of a denied or expired pre-turn moderation gate (`trigger: 'moderation_denied'` — Spec-014 D-014-10; the expired-gate arm, campaign B15) |
| `waiting_for_approval` | `failed` | Provider or transport failure | Failure occurs while run is blocked on approval |
| `waiting_for_input` | `running` | Input received | Valid user input received, or a blocking `user_input` / `mcp_elicitation` approval-category request resolves — rejected/expired outcomes continue-with-refusal (Spec-010, CP-010-5 seam); a driver input-ask expiry never takes this row — it takes the park row below (Spec-010 Part-B fail-closed follow-up, 2026-07-17) |
...
V1 run nesting is depth-1 (`Spec-014 §Default Behavior`): a child run cannot create its own children, so "subtree" is always a flat set of direct children.
...
- Guards required: version checks for interventions (ensuring stale interventions do not apply), recovery eligibility checks (determining whether a stale run can be safely resumed or must fail), rollback state-gating guards (legal source states per §Rollback Transitions, with pause-first for `running` on a bare rollback and outright refusal of a `running` source when the request carries the 2026-08-16 amendment's optional `replacementSend` — campaign B2), and child-run independence guards (ensuring no code path auto-propagates a parent state change to children — non-cascade per §Child-Run Behavior; Tier-5 audit, Plan-014 A-014-17).
