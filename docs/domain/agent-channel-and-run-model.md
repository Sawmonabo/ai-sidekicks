...
This 4-state enum is the canonical `AgentState` adopted verbatim by the Plan-014 contract surface. There is no attach or detach verb: a session's main sidekick is the one it starts with, and every other sidekick in the session arrives because that one delegated to it or because the user named one in the composer, each recorded by the daemon on the session's own sidekick tree index as it starts ([Spec-014 §Interfaces And Contracts](../specs/014-multi-agent-channels-and-orchestration.md#interfaces-and-contracts)). V1 wire mapping: a sidekick the daemon is running stands at `ready`, and one whose named default node is not currently attached at `configured`; `disabled` and `archived` are registered in the contract enum and no V1 wire mutation reaches either. Only a `ready` agent can take a run (`agent.not_ready` otherwise). The agent persona (`name`, `driverName`, `modelId`, `defaultNodeId?`, `config?`, `providerAccountId?`, `effort?`, `executionPostureMode?`, `toolAllowlist?`, `instructions?`, `goal?`) is durable via `agent.*` events — the `providerAccountId?` / `effort?` pair and the four axes a definition resolves into (Plan-027 CP-027-7), each named rather than counted so this enumeration stops tracking a position that moves; all six outside `config` because surfaces beyond the driver read them; and `driverName`, `providerAccountId`, `modelId`, and `effort` are mutable axes of `agent.configUpdate` applied at the next boundary the axis permits, not facts fixed when the run started ([Spec-014 §Same-Agent Provider Switch](../specs/014-multi-agent-channels-and-orchestration.md#same-agent-provider-switch)) ([Spec-005 §Channel and Agent Lifecycle](../specs/005-session-event-taxonomy-and-audit-log.md#channel-and-agent-lifecycle-session_lifecycle)).
...
Run lifecycle is defined in `run-state-machine.md`. Parent-child run links carry one of three caller-declared link types — `spawn`, `delegate`, `handoff` ([Spec-014 §Interfaces And Contracts](../specs/014-multi-agent-channels-and-orchestration.md#interfaces-and-contracts), D-014-17) — and nesting is unbounded by default: a child run may create child runs of its own, and those may create their own, to any depth. A depth refusal exists only where a depth limit has been configured, and every budget, approval, and visibility rule binds at every level ([Spec-014 §Default Behavior](../specs/014-multi-agent-channels-and-orchestration.md#default-behavior)).
