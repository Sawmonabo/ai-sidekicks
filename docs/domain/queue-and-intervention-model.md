...
- Every intervention records the **origin** it was admitted through — `user` or `system` — and, on the `user` arm only, the **admitting principal**: the identity the daemon itself resolved from the transport at acceptance. The initiator is routing and audit metadata supplied by the caller; the admitting principal is the daemon's own finding and is the sole identity any later authorization decision may read ([Spec-003 §Required Behavior](../specs/003-queue-steer-pause-resume.md#required-behavior)). The two arms are exhaustive and mutually exclusive, so a system-originated intervention carries no principal and can never be mistaken for a user's act.
...
| `degraded` | The intervention took partial or fallback effect, with the outcome detail naming what degraded: the driver could not deliver the type at all — neither through a provider verb of its own nor through the orchestration leg above it — and the orchestration layer fell back (e.g., a driver that can accept nothing mid-run degrades steer to queue plus interrupt; neither V1 provider is such a driver), or a multi-leg intervention's later leg failed after a confirmed earlier leg (rollback's file leg after a confirmed conversation rewind — a confirmed floor the settlement-time compaction-boundary reclassification finds crossing, the `boundary-diverged` arm, or, on a rollback carrying the optional `replacementSend`, its replacement-send leg: the `resend-unapplied` arm in §Driver Result To Lifecycle Mapping). |
...
| `accepted` | `degraded` | Driver fallback used, or multi-leg partial effect | Driver can deliver this type neither natively nor through the orchestration leg above it, and the orchestration layer fell back — a path neither V1 provider reaches; or a later leg failed after a confirmed earlier leg (rollback file leg) |
...
The `rollback` type is the deliberate exception at the driver boundary: it is a full member of the `InterventionRequestPayload` union on the client→daemon wire, but its driver leg dispatches through the dedicated capability-gated `rollbackTo(RollbackToParams {sessionId, position, bindingId})` parity operation returning `DriverRollbackResult` — not through `ApplyInterventionParams` ([Spec-003 §Interfaces And Contracts](../specs/003-queue-steer-pause-resume.md#interfaces-and-contracts); operation contract per [Spec-004](../specs/004-provider-driver-contract-and-capabilities.md)). The daemon enforces both guards pre-dispatch and resolves `targetRunId` into the run's live provider binding (`bindingId`) plus session context — which is why neither guard field appears in `RollbackToParams`, and why clients never send a binding key. `DriverRollbackResult` maps into the same 6-state lifecycle per the normative table below (`applied {sessionPosition, bindingId?}` → `applied`, the optional `bindingId` repointing the run's live binding where a mechanism mints a new provider session — which neither V1 leg does, both cutting in place; `degraded {fallbackAction?}` → `degraded`), and the daemon's file-restore leg composes fail-closed around the driver leg per [Spec-003 §Required Behavior](../specs/003-queue-steer-pause-resume.md#required-behavior). That file leg is the daemon's own per-session file checkpoint store on both providers ([Spec-013 §Required Behavior](../specs/013-persistence-recovery-and-replay.md#required-behavior)), never the provider's own file rewind; a restore aimed at a named snapshot instead of at a message goes through the turn-boundary snapshots ([Spec-008 §Turn-Boundary Snapshots](../specs/008-worktree-lifecycle-and-execution-modes.md#turn-boundary-snapshots)).
...
- `rollback` intervention targets `paused`, the two waiting states (voiding the pending approval or input block), and the three terminal states — the only intervention legal against a terminal run; a `running` target is paused first, on a bare rollback and on the edit-and-resend composite alike, because the pencil is offered through a running turn ([Spec-003 §Required Behavior](../specs/003-queue-steer-pause-resume.md#required-behavior); [Run State Machine §Rollback Transitions](run-state-machine.md#rollback-transitions)).
...
- A steer intervention against a run whose driver can deliver the type neither natively nor through the orchestration leg above it must be rejected or degraded to a new queue item explicitly. A driver whose `steer` flag is `false` because the capability is delivered above it is not that case: it delivers and settles `applied`, which is where both V1 providers sit.
- A queued item can expire if its required workspace, branch, or user authority is no longer valid. Nothing else expires it; a waiting message has no deadline.
