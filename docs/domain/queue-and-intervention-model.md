...
| `replacementSend` | no (2026-08-16 amendment) | `InterventionRequestPayload` | daemon-only — persisted on the durable intervention row before dispatch, then re-**admitted** as a queue item through the ordinary participant-send path after the confirmed rewind (the run still lands `paused`; no auto-resume, no second driver call); never forwarded to the driver's `rollbackTo` leg |
