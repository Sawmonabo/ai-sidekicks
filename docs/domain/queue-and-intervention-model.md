...
| `accepted` | `degraded` | Driver fallback used, or multi-leg partial effect | Driver can deliver this type neither natively nor through the orchestration leg above it, and the orchestration layer fell back — a path neither V1 provider reaches; or a later leg failed after a confirmed earlier leg (rollback file leg, campaign B2) |
...
- `rollback` intervention (campaign B2) targets `paused`, the two waiting states (voiding the pending approval or input block), and the three terminal states — the only intervention legal against a terminal run; a `running` target is paused first, on a bare rollback and on the edit-and-resend composite alike, because the pencil is offered through a running turn ([Spec-003 §Required Behavior](../specs/003-queue-steer-pause-resume.md#required-behavior); [Run State Machine §Rollback Transitions](run-state-machine.md#rollback-transitions-campaign-b2)).
