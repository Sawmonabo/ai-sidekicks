...
| Skill | [`/ripple-check`](../../.claude/skills/ripple-check/SKILL.md) dispatches Subagent D for the semantic line-cite case (`.md` citers; the bare-basename `.md:NN` annotations in code remain a documented audit-only residual with no static floor); heading-anchor and symbol-anchor semantic exposure routes to Subagents B / A. |
