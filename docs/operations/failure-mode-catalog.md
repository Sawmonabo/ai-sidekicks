...
| Field | Value |
| --- | --- |
| Structural Action | A line cite that still RESOLVES but no longer means what it meant: a `file.md:NNN` cite (in a doc — including the label-form `Spec-NNN:LL` in doc prose, 661 occ @ 2026-07-07; the bare-basename `.md:NN` wire-doc annotations in `packages/**` code, 79 occ; and the line-word forms `Spec-003 line 128` / `lines 109, 155` in `packages/**`+`apps/**` code, ~362 occ @ 2026-07-08 — the deny gate sees only colon-adjacent spellings) whose target line moved within the file; or a durable-form cite whose anchor survived while the CONTENT changed contract — the section under a cited `§Heading` was rewritten, or an exported symbol kept its name with different semantics. |
...
| Skill | [`/ripple-check`](../../.claude/skills/ripple-check/SKILL.md) dispatches Subagent D for the semantic line-cite case (`.md` AND code citers; the line-word and bare-basename `.md:NN` code populations remain documented audit-only residuals with no static floor); heading-anchor and symbol-anchor semantic exposure routes to Subagents B / A. |
