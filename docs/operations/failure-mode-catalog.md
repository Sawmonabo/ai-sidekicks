...
- **Slug-algorithm divergence on niche Unicode.** lychee's filter and GitHub's slugger strip math-symbol characters (U+2192 `→` included), but the strip class in [`tools/docs-corpus/lib/slug.ts`](../../tools/docs-corpus/lib/slug.ts) is a truncated snapshot ending at U+205E — it KEEPS the arrow, so for this character class the local slug diverges from what GitHub actually renders (authority inverted from this row's original claim). First live exercise 2026-07-08: the ADR-015 `### Amendment 2026-07-08: V1.1 deferred features 3 → 2 …` heading carries 14 inbound `…-3--2-…` fragment anchors across 11 files — all GitHub-correct, all lychee-validated, all computed WRONG by the current `slug.ts`. Until the full upstream github-slugger strip regex is restored in `slug.ts` (follow-up tooling PR), do NOT "repair" anchors against the local slug for symbol-bearing headings — GitHub/lychee are authoritative for this class.
