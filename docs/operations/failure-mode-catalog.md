...
| Known Gaps | 2026-07-09 (cross-node scope-amendment PR): the floor grammar covers only path-form `file.md:NNN` cites — label-form `Spec-NNN:LL` and doc-prose line-word forms (`AC line NN`, `Spec-NNN line NN`) are not floor-checked, so such cites to blank or shifted lines pass pre-commit + CI (observed: `Spec-025:119` and `AC line 170` citers passed the full-repo runner while both targets were blank after an upstream +1 insertion). Blank-target and semantic drift in that population are audit-layer only — `/ripple-check` Subagent D plus a scripted inbound-pair scan are the covering layers until the floor learns the label / line-word grammar. |
| What Discipline Restores Correctness | When you intend to cite a specific line, prefer to cite by content (inline anchor or quoting a unique substring) over by line number. When line number is required, run the hook. Line numbers are a docs→docs form only — code targets are cited `path#symbol`; governance targets in code are cited via backticked `§Heading`. |

### CAT-07 — Line-citation drift (semantic)

| Field | Value |
| --- | --- |
| Structural Action | A line cite that still RESOLVES but no longer means what it meant: a `file.md:NNN` cite (in a doc — including the label-form `Spec-NNN:LL` in doc prose, 661 occ @ 2026-07-07; the bare-basename `.md:NN` and line-word code populations: swept to durable §-forms and deny-gated for NEW instances 2026-07-08, 0 occ remaining in `packages/**`+`apps/**`) whose target line moved within the file; or a durable-form cite whose anchor survived while the CONTENT changed contract — the section under a cited `§Heading` was rewritten, or an exported symbol kept its name with different semantics. |
...
| Hook | Semantic drift: none possible without semantic understanding (NLP / model invocation). NEW line-word / bare-basename instances in code citers are statically denied by [`tools/docs-corpus/lib/label-cite.ts`](../../tools/docs-corpus/lib/label-cite.ts) passes 5-6 (`line-anchored-cite-in-code`, 2026-07-08), so that once-gate-invisible population can no longer regrow. The floor (docs→docs `:NNN`) + anchor-survival is CAT-06's beat; the SEMANTIC drift under an intact `§Heading` / `#symbol` anchor stays audit-only (this row). |
| Skill | [`/ripple-check`](../../.claude/skills/ripple-check/SKILL.md) dispatches Subagent D for the semantic line-cite case (`.md` AND code citers; the code trees' line-word / bare-basename populations were swept to §-forms and are deny-gated for new instances @ 2026-07-08 — subagent D's code-side work is now the §-cites' semantic match, plus `.md`-citer line forms which keep their own conventions); heading-anchor and symbol-anchor semantic exposure routes to Subagents B / A. |
