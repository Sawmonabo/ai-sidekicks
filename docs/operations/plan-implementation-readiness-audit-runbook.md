...
| G5 | PR references (bare `PR #N` or qualified `Plan-NNN PR #N`) may not appear inside audit-authored `#### Tasks` blocks — for each tier plan run `awk '/^#### Tasks/{f=1;next} /^#{1,4} /{f=0} f' <plan>.md \| rg "PR #\d+"` and require empty output (the bare pattern subsumes the qualified form). Provenance surfaces (Progress Log, Shipment Manifest, Preconditions, Decision Log, dependency-map dispatch state) legitimately carry PR references and are exempt. `pr_preparations` table name preserved |
