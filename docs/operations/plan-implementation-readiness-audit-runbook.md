...
1. **Phase-level dep-ordering gaps** — a Phase imports from a plan/Phase in a later tier than its own (the Plan-001 Phase 5 → Plan-006/008 substrate gap GitHub PR-#11 surfaced retroactively).
...
For each Plan in Tier:
  1. Main agent reads plan + spec + cited ADRs + the cross-plan-deps graph
...
The audit has three validation surfaces: **Tier 1 pilot acceptance gate** (one-time, validates the methodology itself), **per-tier coverage gates G1-G7** (every tier, mechanical, before swap), and **final synthesis verification** (after Tier 8, validates the full corpus).

### Tier 1 Pilot — Two-Part Acceptance Gate

Tier 1 doubles as a methodology pilot. Both parts must pass before the next tier starts.
...
- F-001-5-XX (critical, dimension 10) — Phase 5 imports JSON-RPC wire substrate from Plan-006 (Tier 3); breaks build-order
- F-001-5-XX (critical, dimension 10) — Phase 5 imports the tRPC v11 server skeleton from a later-tier control-plane plan; breaks build-order

If the methodology cannot reproduce this finding on the pre-carve-out state, it is broken; fix before proceeding.

| Criterion | Pass condition |
| --- | --- |
| A1 — Canonical-finding reproduction | Pre-PR-#11 audit produces F-001-5-XX critical findings citing both Plan-006 (JSON-RPC wire) and a later-tier control-plane plan (tRPC v11 server skeleton) substrate imports |
...
- All within band: record metrics in §Lessons Learned; proceed to the next tier unmodified.
- One metric out of band: document in §Lessons Learned; adjust corresponding dimension/threshold; proceed to the next tier with adjustment noted.
- Multiple metrics out of band: pause; revise §Subagent Prompt Template and/or §Main-Agent Dep-Trace Dimensions before the next tier starts; re-run Tier 1 calibration on the pilot tier.
...
| `substrate_exempt` | `{ type: audit_status, status: substrate_exempt }` | The phase is a substrate slice of a substrate-vs-namespace carve-out declared in the plan's own phase table. Three criteria below — criterion (3) is mechanically enforced. |

### The `substrate_exempt` three-criterion predicate

A phase qualifies as `substrate_exempt` only if ALL THREE hold:

1. **Substrate is single-owned and load-bearing for an earlier-tier consumer** ([Plan-006 §Execution Windows](../plans/006-local-ipc-and-daemon-control.md#execution-windows-v1-carve-out) criterion (a)). The substrate phase ships infrastructure that a later-tier or sibling plan's behavioral phases depend on; no other plan owns it.
...
3. File path concreteness — file/module to create lacks exact path, or path
   conflicts with another plan's declared §Target Areas ownership.
...
- **Consumes:** each contract symbol this Task consumes that is NOT produced within the same Phase (RPC/IPC method, event channel, request-param shape, exported type), each with its resolution — `presence.subscribe({ sessionId }) ← Plan-006 daemon namespace (Tier-3 §Precondition)`. Names an upstream Plan/Phase provider, a declared §Precondition, or a tracked `BL-NNN` (closes Dimension 11 loop). A `BL-NNN` resolution means the consume is blocked on that BL — record it as a §Precondition so the Phase is not execution-ready until the BL ships, not a satisfied dependency. Resolution is by provider SHAPE — a symbol that exists but does not satisfy the consumer need is a Finding (severity: critical, dimension: 11), not an entry.
...
| D4 | Phase-DAG alignment | Does `cross-plan-dependencies.md` show every edge Phase-N needs? |
...
| D6 | Forward-declared schema bidirectionality | If Phase-N references a forward-declared column/table, do the CREATE-owner's plan and the semantics-owner's plan each cite the other for that element? |
| D7 | Cross-plan ownership consistency | Does Phase-N create or modify a path/table that another plan's own Target Areas claim ownership of? |
| D8 | Substrate-vs-namespace pattern | If Phase-N depends on a substrate-deliverable from a later-tier plan, is the carve-out documented in the owning plan's carve-out declaration? |
...
| ---------- | -------- | --------- | ---------------- | --------- |
| F-XXX-1-01 | critical | 10        | dep-map graph    | line NNN  |
...
### Tier 5 (Plans 007/010/012/016/021) — five-plan bundle calibration (2026-06-12)
...
### Tier 6 (Plans 009/014/015) — backfill-dominant audit, long-convergence contract lessons (2026-06-19)

(PR #160) audited three plans — Plan-009 (gitflow PR & diff attribution), Plan-012 (artifacts/files/attachments), and Plan-013 (persistence/recovery/replay). **Finding-volume actuals** ran well under prior tiers: 22 per-plan findings + 1 cross-cutting (C-7-01) — Plan-009 7, Plan-012 9, Plan-013 6 — all three **under the B2 5–50 ceiling** (contrast the Tier-5 five-plan bundle's 52–76), because two of three were backfill-dominant (Plan-009 + Plan-013 stay `approved`: §Invariants / §Cross-Plan Obligations / §Tasks record EXISTING relationships, not behavior inventions), so the pre-Tasks-format stub-backlog cluster that inflated the B1/B2 bands did not recur. **B6 status-flip rate 1/3**: Plan-012 `review → approved` (OCI-envelope manifest contract ratified); Plan-009 + Plan-013 unchanged. The audit produced audit-traceable amendment-tasks (A-009/014/015-\*) plus the cross-cutting D-decision set. **Convergence cadence**: 20 Codex rounds / 75 threads on a +673/−221, 21-file doc diff — comparable to the Tier-5 bundle's 21/85 despite a far smaller diff, because the rounds were driven by cross-cutting _contract_ decisions, not finding volume. Cross-cutting lessons (runbook v.next): **(1) method-string vs. TS-symbol confusion (D-009-5, surfaced round 19)** — a plan naming wire operations by their PascalCase request-type symbols (`PRPrepare`, `GitActionExecute`) reads as if those were the method strings, but the method string is dotted-camelCase (`gitflow.prPrepare`), which the canonical `METHOD_NAME_FORMAT` permits — though registering these methods in the daemon `MethodRegistry` is gated on [BL-142](../archive/backlog-archive.md) until its deployed `registry.ts` regex (lowercase-only today) conforms, as [api-payload-contracts.md](../architecture/contracts/api-payload-contracts.md) already records; recommend a per-plan dimension asserting every wire operation's method string resolves in the [api-payload-contracts.md](../architecture/contracts/api-payload-contracts.md) method registry, not merely its request/response schema name. **(2) optional-field parity across lift surfaces (rounds 19–20)** — a field optional in its canonical source (`security.default.override.row` at Spec-024) must read optional in every downstream lift (the Spec-005 taxonomy) AND the code interface (`SecurityDefaultOverrideEvent.row?`); a required-vs-optional mismatch is a real defect (a row-less override event is rejected by any sink built from the stricter surface) — recommend the cross-cutting-contract sweep assert optionality parity across canonical-spec ↔ taxonomy-lift ↔ code-interface for every touched field. **(3) audit-PR self-citation drift (CAT-07)** — inserting the api-payload method-name table (+11 lines) shifted five inbound Plan-013 line-cites, re-anchored in-PR; an audit that adds rows/tables to a doc cited by line elsewhere must re-anchor those inbound cites in the same PR (the ripple-check CAT-07 axis), not defer to housekeeping.

### Tier 7 (Plans 011/017/019/020/023-remainder) — five-plan bundle with a first-time promotion (2026-08-10)
...
### Tier 8 (Plans 023/027) — terminal-tier audit, chain closure (2026-08-12)

(PR #331) audited the two Tier-8 plans — Plan-023 (first-run onboarding, paired Spec-023) and Plan-024 (cross-node dispatch, paired Spec-022) — closing the tier-PR chain. **Calibration band actuals** (per §Calibration Bands B1–B6 definitions above): **B1 — Critical findings per Phase**: Plan-023 0.67 avg (4 criticals / 6 backfilled Phases), Plan-024 0.5 avg (3 / 6) — both within the 0–2 target for the first time since, because both plans were authored after the Tasks-block convention existed (no pre-format stub backlog). **B2 — Total findings per plan**: Plan-023 18 (4 critical / 8 major / 6 minor), Plan-024 9 (3 critical / 4 major / 2 minor) — both within the 5–50 band; Plan-024 additionally produced 3 verification no-ops (claims checked true, no edit owed), recorded outside the finding triple so the band compares like with like. **B3 — Tasks-authored vs. blocking-finding ratio**: 34 Tasks authored (22 + 12) / 7 criticals ≈ 4.9:1 — above the 2:1 target. **B4 — User-review walltime**: the tier's two carve-out adjudications (the ADR-010 Option-3 scope-out; the no-third-rejection-reason ruling) were ratified by the orchestrating lead under the session's standing authorization at dispatch time — no separate user pause was required. **B5 — advisor signal-to-noise**: substantive at both checkpoints (walk-time package-shape review; fold-time pre-dispatch execution review surfacing the combined entry shape and the tag-chain gap) — within target. **B6 — Status flip rate**: 0/2 (both plans stay `approved`; both satellite spec amendments non-flip). Terminal-tier housekeeping the Final Synthesis check surfaced: the `plan-readiness-audit-tier-7-complete` / `tier-7-complete` tags were never pushed at their PRs' merges — backfilled at the PR #160 / PR #318 merge commits in this PR's swap, with `tier-7-baseline` reconstructed at PR #318's first parent; the "Tags exist" row now passes on the full chain rather than asserting over a gap. Cross-cutting lessons: **(1) re-derive closed-set quantifiers after the swap, never before** — Plan-023's "exactly the six §Touched But Not Owned entries" quantifier was falsified by three of the same audit's own findings, each adding a member; the final count depends on which sibling repairs land, so the number must be counted from the post-repair block as the swap's last edit (the F-26-04 count-derivation table is the template). **(2) Ownership edges must be walked in both directions** — the map-obliges-plan direction (dep-map §2 promised a Plan-023 redirect-handler extension the plan never carried — struck) and the plan-obliges-map direction (the plan authored CLI commands no §2 row owned and no `main.ts` registration reached — row + extender added) are distinct failure modes; auditing only one direction passes both defects. **(3) Path-prefix greps need their terminal anchor** — bare `packages/cli` matches `packages/client-sdk`, so the rename sweep both over-counts (false client-sdk hits) and under-counts (missing-slash spellings) without the trailing-slash form; anchor every path-population grep before trusting its count. **(4) A JSON round-trip launders exactly what a `toJSON` guard exists to catch** — `JSON.parse(JSON.stringify(x))` invokes `toJSON` during stringify, so "normalize before guarding" silently removes the override and the guard passes vacuously; guard-reachability must be argued at the type level (what shapes can arrive at the guard), and any proposal to precede a guard with a serialization round-trip is a soundness bug, not a hardening.
