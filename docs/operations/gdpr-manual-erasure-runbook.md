...
1. Hard-DELETE the participant's rows from the no-retention-basis tables present in your deployment. The predicate differs by table shape:
   ...
   - **`cross_node_dispatch_coordination`** has **no `participant_id` column** — it references the participant twice, as `caller_participant_id` **and** `target_participant_id` ([shared-postgres-schema.md:337-341](../architecture/schemas/shared-postgres-schema.md#cross-node-dispatch-coordination-plan-027)), each `NOT NULL REFERENCES participants(id)` with the default `NO ACTION`. Use a two-column predicate: `DELETE FROM cross_node_dispatch_coordination WHERE caller_participant_id = :pid OR target_participant_id = :pid;`. The generic `WHERE participant_id = :pid` would error on the missing column and leave these rows behind — and because both FKs are `NO ACTION`, any survivor blocks the `DELETE FROM participants` anchor in step 2 (the check fires at transaction commit). Skip if absent per the Detection deployment-scope note — it is Tier 9 (Plan-027).
