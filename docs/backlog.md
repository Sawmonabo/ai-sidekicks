...
- Summary: One BL-102 sub-item remains open: the JSON-RPC handshake `protocolVersion` field is typed as **integer** in Spec-007:54 and as **string** in prior api-payload-contracts.md text. The substrate at `packages/contracts/src/jsonrpc.ts` parameterizes `number | string` until reconciled. Spec-007 amendment (or matching ratification on api-payload-contracts.md) is required to narrow the type.
