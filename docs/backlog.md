...
### BL-137: Generalize the daemon-scope event sentinel binding across all categories + define per-node anchor upload

- Status: `todo`
- Priority: `P2`
- Owner: `unassigned`
- References: [Spec-006 §Security Events / §Audit Integrity / §Event Maintenance](./specs/006-session-event-taxonomy-and-audit-log.md), [Plan-022 §Ratified Design Decisions D-022-8](./plans/022-data-retention-and-gdpr.md#ratified-design-decisions-tier-5-audit-2026-05-30), [ADR-017 Shared Event-Sourcing Scope](./decisions/017-shared-event-sourcing-scope.md) — surfaced + partially resolved at the Tier-5 plan-readiness audit (PR #129) Codex review
- Summary: The daemon-scope-event session-binding seam is now **partially closed**. Spec-006 §Security Events defines the node-scope sentinel `session_id` binding (FK-free local store, sentinel-as-partition-key, reusing per-session sequence/hash-chain/anchor machinery — no new replay mechanism) for the two Plan-022 `daemon.*` events (`daemon.master_key_source`, `daemon.pii_split_ambiguous`), reflected in the local-sqlite `session_events.session_id` comment, per Plan-022 D-022-8. The **residual** is: (a) generalizing that same sentinel binding to the remaining daemon-scope categories Spec-006 still describes as "not session-scoped" — `audit_integrity`, `event_maintenance`, and the daemon-startup `runtime_node_lifecycle` clock events; (b) propagating the now-pinned sentinel constant value (the RFC 9562 §5.10 Max UUID, reserved in Spec-006 §Security Events for the two registered `daemon.*` events at the round-5 audit swap) to the remaining categories' bindings when they are defined; and (c) defining the per-node anchor-upload model for sentinel-partitioned hash chains. Owned by Spec-006, not by any Tier-5 plan.
- Exit Criteria: (a) Spec-006 specifies the sentinel binding for the three remaining daemon-scope categories (`audit_integrity`, `event_maintenance`, `runtime_node_lifecycle` clock events); (b) the now-pinned sentinel constant (RFC 9562 §5.10 Max UUID, reserved in Spec-006 §Security Events) is applied across all daemon-scope category bindings, not only the two registered `daemon.*` events; (c) the per-node anchor-upload model for sentinel-partitioned chains is defined; (d) no Spec-006 category is described as "not session-scoped" while the store requires a non-null `session_id` without an explained reconciliation.
- Revisit Trigger: Spec-006 reaches a status promotion; OR a daemon-scope category beyond Plan-022's two `daemon.*` events is next edited; OR Plan-006 (the daemon-scope event emitter) reaches implementation.

### BL-138: V1 GDPR manual-erasure operator runbook

- Status: `todo`
- Priority: `P2`
- Owner: `unassigned`
- References: [Spec-022 §Non-Goals V1 scope-boundary](./specs/022-data-retention-and-gdpr.md#non-goals), [Plan-022 §Non-Goals](./plans/022-data-retention-and-gdpr.md#non-goals), [Spec-022 §Shred Fan-Out](./specs/022-data-retention-and-gdpr.md#shred-fan-out) — surfaced at the Tier-5 plan-readiness audit (PR #129) Codex review (round-5 GDPR hardening)
- Summary: V1 defers the automated `gdpr.*` deletion/export/purge endpoint to V1.1 but ships the erasure _capability_ (schema + write-path + the `ON DELETE SET NULL` severance migration), so a data-subject erasure request must be satisfiable by hand in V1. That manual operator procedure currently lives only in scattered scope notes (crypto-shred via `DELETE FROM participant_keys`; Postgres severance via the SET NULL migration; the Path-1/2/3 fan-out + ordering). It is owed as a single `docs/operations/` runbook so an operator can execute a compliant erasure without reading the spec internals.
- Exit Criteria: (a) a `docs/operations/` runbook enumerates the manual Path-1 (crypto-shred) → Path-2 (Postgres hard-DELETE + `ON DELETE SET NULL` severance over the CP-022-6 FK closure) → Path-3 (diagnostic-bucket flush) sequence with the documented ordering + idempotency guarantees; (b) it names the per-table disposition (hard-DELETE vs SET-NULL-anonymize vs denylist-survival); (c) it states the `participant.purged` audit-emission step; (d) it is cross-linked from Spec-022 §Non-Goals + Plan-022 §Non-Goals.
- Revisit Trigger: a V1 GDPR data-subject erasure request must be serviced; OR Plan-022 reaches implementation; OR the automated endpoint is promoted to a V1.1 plan.
