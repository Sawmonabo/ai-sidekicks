...
- Summary: Plan-007-partial Phase 3 (PR #19) shipped four `session.*` namespace handlers but only `session.create` (`I-007-3-T1` + `I-007-3-T2`) and `session.subscribe` (`I-007-3-T3` + `I-007-3-T6` + `I-007-3-T7`) carry direct handler round-trip tests. Spec-007 AC-N2 (`session.read`) and AC-N3 (`session.join`) cite indirect coverage only: wire-shape inherits from Spec-001 §Interfaces And Contracts; structural invariants inherit via I-007-3-T5 substrate-symmetry; AC-N3 has secondary end-to-end observability through I-007-3-T3's subscribe streaming-primitive integration. BL-114 ratified this honest-deferral as adequate for closing the BL-113 retroactive audit scope. This BL tracks the eventual direct-coverage debt.
- Exit Criteria: Two new test cases (or test files) land under `packages/runtime-daemon/src/ipc/handlers/__tests__/` that exercise (a) `session.read` happy-path round-trip (known-id request → `SessionRead`-shape response) + unknown-id rejection (`-32602 InvalidParams` + `data.type: "session.not_found"`); (b) `session.join` joined-event emission + Spec-001 AC4 second-client replay shape. Test IDs follow Plan-007 Phase 3 `I-007-3-T*` numbering (next free `T8` + `T9` at time of authoring). Spec-007 AC-N2 + AC-N3 amended to upgrade citations from indirect to direct once tests land; Plan-007 Phase 3 Coverage scope note amended to drop the BL-117 deferral.
