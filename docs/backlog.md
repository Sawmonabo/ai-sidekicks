...
- Summary: The daemon-scope-event session-binding seam is now **partially closed**. Spec-006 §Security Events defines the node-scope sentinel `session_id` binding (FK-free local store, sentinel-as-partition-key, reusing per-session sequence/hash-chain/anchor machinery — no new replay mechanism) for the two Plan-022 `daemon.*` events (`daemon.master_key_source`, `daemon.pii_split_ambiguous`), reflected in the local-sqlite `session_events.session_id` comment, per Plan-022 D-022-8. The **residual** is: (a) generalizing that same sentinel binding to the remaining daemon-scope categories Spec-006 still describes as "not session-scoped" — `audit_integrity`, `event_maintenance`, and the daemon-startup `runtime_node_lifecycle` clock events; (b) formally reserving the sentinel constant value in the taxonomy rather than leaving it an implementation-chosen well-known id; and (c) defining the per-node anchor-upload model for sentinel-partitioned hash chains. Owned by Spec-006, not by any Tier-5 plan.
- Exit Criteria: (a) Spec-006 specifies the sentinel binding for the three remaining daemon-scope categories (`audit_integrity`, `event_maintenance`, `runtime_node_lifecycle` clock events); (b) the reserved sentinel constant is formally reserved in the taxonomy; (c) the per-node anchor-upload model for sentinel-partitioned chains is defined; (d) no Spec-006 category is described as "not session-scoped" while the store requires a non-null `session_id` without an explained reconciliation.
