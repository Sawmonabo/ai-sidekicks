...
- Summary: One BL-102 sub-item remains open: the JSON-RPC handshake `protocolVersion` field type — [Spec-007:54](./specs/007-local-ipc-and-daemon-control.md) declares integer; substrate at `packages/contracts/src/jsonrpc.ts` parameterizes `number | string` pending Spec-007 amendment or matching ratification on api-payload-contracts.md. (Note: api-payload-contracts.md previously closed this sub-item by conflating handshake `protocolVersion` with the `EventEnvelopeVersion` semver brand; that closure was rolled back in commit `735b069` as a mis-conflation — the handshake field and the envelope-version brand are distinct surfaces.)
  ...
  - **Resolution Direction (2026-05-01):** Adopt **date-string format (ISO 8601 `YYYY-MM-DD`)** for `protocolVersion` (e.g. `"2026-05-01"`), aligning with the [MCP specification §Architecture overview](https://modelcontextprotocol.io/docs/learn/architecture) and [MCP §Connection Lifecycle](https://deepwiki.com/modelcontextprotocol/modelcontextprotocol/2.4-client-features) precedent for AI-agent IPC handshake versioning. Spec-007:54 amends `integer field` → `date-string field (ISO 8601 YYYY-MM-DD)`; substrate at `packages/contracts/src/jsonrpc.ts` narrows `number | string` → `z.string().regex(/^\d{4}-\d{2}-\d{2}$/)`; api-payload-contracts.md §Tier 1 (cont.): Plan-007 ratifies the date-string format with MCP citation. **Rationale:** this project is an AI-agent IPC running `claude-driver` + `codex-driver` providers; MCP is the closest-analog 2024-2026 convention from Anthropic. Date strings provide natural lexicographic ordering (ISO 8601 = chronological), encode release date inherently, and dodge the semver "v1.5 with no v1.4" ambiguity. The handshake `protocolVersion` and the per-envelope `EventEnvelopeVersion` semver brand remain distinct surfaces (per the line-281 rollback narrative).
...
- Summary: Plan-007 Phase 2 JSON-RPC error model is unspecified. `error.ts` shapes are not mapped to JSON-RPC standard codes (-32700 parse error / -32600 invalid request / -32601 method not found / -32602 invalid params / -32603 internal error) plus custom domain codes (`unknown_setting`, `resource.limit_exceeded`, etc.). Register codes in `error-contracts.md`; map daemon-side error envelopes to canonical wire shapes.
  - **Resolution Direction (2026-05-01):** Adopt a **two-layer error envelope** per [JSON-RPC 2.0 §5.1](https://www.jsonrpc.org/specification) (`code` MUST be `Number` — strict requirement), [LSP 3.17 §ResponseError](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/) (`data` carries optional structured detail, primitive or structured), and [RFC 7807 Problem Details for HTTP APIs](https://datatracker.ietf.org/doc/html/rfc7807) (`type` is the canonical machine-readable identifier). Wire shape: `{ code: <jsonrpc-numeric>, message: <human-readable>, data: { type: "<dotted.namespace>", fields: { ... } } }`. error-contracts.md adds a top-level "JSON-RPC Wire Mapping" section registering the standard JSON-RPC numeric codes (`-32700` parse error / `-32600` invalid request / `-32601` method not found / `-32602` invalid params / `-32603` internal error) plus envelope shapes for the Plan-007 Tier 1 domain identifiers (`unknown_setting`, `transport.unavailable`, `resource.limit_exceeded`). T-007p-1-4 widens from code-string-only assertion to full-envelope-shape assertion. **Rationale:** JSON-RPC 2.0 §5.1 strictly requires numeric `code`, so the dotted-namespace identifier MUST live in `data` regardless of preference. Naming the field `type` (vs. the more verbose `domainCode`) aligns with the broader 2025-2026 HTTP-API community convention (RFC 7807 is the de-facto standard for structured API error responses) and the ecosystem tooling that consumes it.
...
- Summary: Spec-006 event registry needs two amendments: (1) define `MemberJoined` event semantics OR rename to `MembershipRoleChanged` (Plan-001 Phase 2 references with no source-of-truth definition); (2) register `security.default.override` and `security.update.available` events (Plan-007 Phase 3 references).
  - **Resolution Direction (2026-05-01):** **(1)** Register **`membership.created`** in [Spec-006 §Invite and Membership](./specs/006-session-event-taxonomy-and-audit-log.md) for "direct first-time membership creation outside the invite flow" (paired with the Plan-001 Phase 4 `joinSession` non-invite-mediated emission point), preserving the existing `<object>.<past-tense-action>` namespace pattern (`invite.created`, `membership.role_changed`, `membership.suspended`, `membership.revoked`, `membership.reactivated`). Rename Plan-001 Phase 2's discriminant `MemberJoined` → `MembershipCreated` in `packages/contracts/src/event.ts`. **(2)** Register `security.default.override` and `security.update.available` in Spec-006 §Event Type Summary; add Plan-006 emitter-table rows listing Plan-007 as the emitter. **Rationale:** the original "rename to `MembershipRoleChanged`" framing was a misread — Spec-006 already has `membership.role_changed` (semantically role-change ≠ creation, so the rename would lose information). The `member.*` prefix doesn't exist anywhere else in the registry; staying within the established `membership.*` namespace preserves consistency. Past-tense + namespace consistency follows modern event-sourcing convention per [Confluent — Designing Events and Event Streams](https://developer.confluent.io/courses/event-design/best-practices/) and [Kurrent — What's in an (event) name?](https://www.kurrent.io/blog/whats-in-an-event-name) (events named `<Object><PastTenseVerb>`; specific over generic; namespace-scoped).
