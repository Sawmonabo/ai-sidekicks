...
### BL-117: Direct `session.read` + `session.join` handler round-trip tests (Plan-007 Phase 3 deferred)

- Status: `todo`
- Priority: `P3`
- Owner: `unassigned`
- References:
  - [Spec-007 §Acceptance Criteria AC-N2 + AC-N3](./specs/007-local-ipc-and-daemon-control.md#acceptance-criteria) — honest-deferral citations land here
  - [Plan-007 §Phase 3 test plan Coverage scope note](./plans/007-local-ipc-and-daemon-control.md) — coverage map names BL-117 as the deferral surface
  - `packages/runtime-daemon/src/ipc/handlers/session-read.ts` — per-method binding (test-target)
  - `packages/runtime-daemon/src/ipc/handlers/session-join.ts` — per-method binding (test-target)
  - `packages/runtime-daemon/src/ipc/handlers/__tests__/session-handlers.test.ts` lines 58-63 — file-header acknowledgment that no direct `session.read` / `session.join` round-trip exists in Plan-007-partial Phase 3
  - [BL-114 archive entry](./archive/backlog-archive.md) — parent closure that filed this deferral
- Summary: Plan-007-partial Phase 3 (PR #19) shipped four `session.*` namespace handlers but only `session.create` (`I-007-3-T1` + `I-007-3-T2`) and `session.subscribe` (`I-007-3-T3` + `I-007-3-T6` + `I-007-3-T7`) carry direct handler round-trip tests. Spec-007 AC-N2 (`session.read`) and AC-N3 (`session.join`) cite indirect coverage only: wire-shape inherits from Spec-001 §Interfaces And Contracts; structural invariants inherit via I-007-3-T5 substrate-symmetry; AC-N3 has secondary end-to-end observability through I-007-3-T3's subscribe streaming-primitive integration. BL-114 ratified this honest-deferral as adequate for closing the BL-113 retroactive audit scope. Codex review of PR #76 (2026-05-19) surfaced a second gap behind AC-N2: `session.read`'s unknown-id throw currently default-collapses to `-32603 InternalError` rather than the typed `-32602` + `data.type: "session.not_found"` shape promised by [error-contracts.md §JSON-RPC Wire Mapping](./architecture/contracts/error-contracts.md#json-rpc-wire-mapping); registering a `SessionNotFoundError` subclass + adding its `mapJsonRpcError` discriminator entry closes the gap and is now part of this BL's scope. This BL tracks the eventual direct-coverage debt + the typed-error-mapping debt together.
- Exit Criteria: **(1) Production code — typed `session.not_found` error mapping.** Register a `SessionNotFoundError` subclass (mirroring the registered substrate-error pattern in `packages/runtime-daemon/src/ipc/jsonrpc-error-mapping.ts`) and add its `mapJsonRpcError` discriminator entry so `session.read`'s unknown-id throw surfaces `-32602 InvalidParams` + `data.type: "session.not_found"` per [error-contracts.md §JSON-RPC Wire Mapping](./architecture/contracts/error-contracts.md#json-rpc-wire-mapping) — replacing the current default-collapse path's `-32603 InternalError`. **(2) Tests.** Two new test cases (or test files) land under `packages/runtime-daemon/src/ipc/handlers/__tests__/` that exercise (a) `session.read` happy-path round-trip (known-id request → `SessionRead`-shape response) + unknown-id rejection (verifies the typed wire shape produced by #1); (b) `session.join` `membership.created` event emission (the canonical V1 join-admission event per `SessionEventSchema` at `packages/contracts/src/event.ts`) + Spec-001 AC4 second-client replay shape. Test IDs follow Plan-007 Phase 3 `I-007-3-T*` numbering (next free `T8` + `T9` at time of authoring). **(3) Doc updates.** Spec-007 AC-N2 + AC-N3 amended to upgrade citations from indirect to direct once tests land; AC-N2's "current daemon behavior" caveat removed once #1 lands; Plan-007 Phase 3 Coverage scope note amended to drop the BL-117 deferral.
- Revisit Trigger: Plan-007-remainder Tier 4 dispatch (the natural execution surface). Tier 4 ships `run.*` / `repo.*` / `artifact.*` handlers — adding the deferred `session.read` + `session.join` direct tests as Tier 4's first or second PR is the recommended sequencing per BL-114 archive entry. Also revisit opportunistically if a Plan-007-related bug in production surfaces a need for tighter handler test coverage.
...
- Owner: `unassigned`
- References:
  - [Plan-023](./plans/023-desktop-shell-and-renderer.md) — Electron shell + smoke gate substrate
  - [Spec-023 §webPreferences locks](./specs/023-desktop-shell-and-renderer.md) — the contract `assert-webprefs.ts` enforces
  - `apps/desktop/package.json` — current `"test": "pnpm run build:smoke && vitest run --passWithNoTests"` wiring
  - `apps/desktop/build/assert-webprefs.ts` — Spec-023 webPreferences-lock assertion run after `build:smoke`
  - `apps/desktop/test/launch.smoke.test.ts` — the sole consumer of the built smoke bundle (spawns Electron against `out/main/index.js`)
  - PR #77 Phase D first-pass POLISH that surfaced the inefficiency
- Summary: Plan-001 Phase 5 T5.2 (PR #77) added a renderer-unit-test suite under `apps/desktop/src/renderer/src/**/__tests__/**` that runs under happy-dom — none of those tests need the built Electron smoke bundle. But `apps/desktop/package.json`'s `test` script unconditionally runs `pnpm run build:smoke && vitest run --passWithNoTests` so EVERY test invocation (root `pnpm test`, `pnpm --filter @ai-sidekicks/desktop test`, the future watch-mode loop) rebuilds the full electron-vite bundle + re-runs the `assert-webprefs.ts` gate before vitest even starts. The smoke bundle is only consumed by `test/launch.smoke.test.ts` — the renderer suite's happy-dom env reaches the SessionBootstrap component via plain module import, not via the built `out/` tree. Three fixes are viable, in increasing decoupling: (a) **stale-detection prepass** — wrap `build:smoke` in a script that checks `out/main/index.js` mtime + source-hash and skips when fresh; (b) **vitest-project per-test-file gating** — split `pnpm test` into `pnpm test:renderer` (no build) + `pnpm test:smoke` (build + smoke) and have root `pnpm test` Turbo-orchestrate both; (c) **lazy build-on-demand** — move the build into `test/launch.smoke.test.ts`'s `beforeAll` so only the smoke test pays the cost, and only if it actually runs. (b) is the Turbo-idiomatic path and matches how `packages/contracts` already splits unit + integration via vitest projects. (a) is the cheapest immediate hack. None of these are required to ship V1 — the existing wiring is correct, just slow.
- Exit Criteria: `pnpm --filter @ai-sidekicks/desktop test` (or its successor invocation) no longer unconditionally invokes `electron-vite build --mode=smoke` when only the renderer suite is being exercised; renderer-unit-test feedback latency drops from ~25-30s (full vite SSR + client build + assert-webprefs gate) to ~3s (vitest + happy-dom only). The `test/launch.smoke.test.ts` path STILL exercises the built bundle (so the Spec-023 webPreferences assertion still gates merge via the smoke test) — the change is only to the invocation contract, not to the smoke gate's authority. Plan-023's testing posture documentation amended to describe the split.
- Revisit Trigger: Any of — (1) Plan-023 Tier 8 Phase scheduled for delivery (good window to revisit the desktop package's test wiring as part of that phase's substrate work); (2) developer feedback that the test loop's slowness is materially blocking flow; (3) renderer-unit-test suite grows beyond a handful of files and the build-prefix cost compounds proportionally.
