...
- Exit Criteria: (a) an ADR (new ADR-NNN, or an ADR-010 / ADR-007 amendment) decides the runtime-node caller-authorization model — how the authenticated caller identity (ADR-010 PASETO/DPoP on the tRPC ctx) is resolved, and the ownership predicate each of the four procedures enforces before mutating an attachment row (caller participant == attachment owner, or a Cedar policy per ADR-012); (b) Spec-003 §Interfaces And Contracts records the authorization precondition for all four procedures; (c) Plan-003 carries an implementation task wiring the guard into each of attach / heartbeat / capabilityupdate / detach, with tests proving a cross-owner mutation is refused with a typed authorization error; (d) the guard is enforced in code, not merely documented — a cross-owner attempt returns the typed error.
