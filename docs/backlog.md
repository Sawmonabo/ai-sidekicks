...
- Priority: `P1`
- Owner: `unassigned`
- Resolution (resolved 2026-04-18): Registered a canonical 13-event `cross_node_dispatch` category in Spec-006, reconciling Spec-024's pre-existing namespace drift in the same landing pass per the BL's required single-audit-pass discipline. Canonical set (trusts Spec-024 §Execution And Result Emission body text as authoritative, supersedes the partial §Interfaces And Contracts line-165 enumeration): caller-side (3) `.sent`, `.approval_observed`, `.result_observed`; target-side intermediate (5) `.received`, `.approval_requested`, `.approved`, `.executed`, `.result_buffered`; target-side terminal (5) `.rejected` (pre-approval), `.denied` (approval refusal), `.failed` (post-approval execution fault — new, added to Spec-024 §Cross-Node Failure Semantics in the same pass), `.expired` (token timeout), `.completed` (success). Lifecycle discipline: exactly one terminal per dispatch; `.executed` is intermediate (not terminal) so audit can distinguish executed-but-undelivered from fully-completed. Spec-024 reconciliation edits: line 97 `approval_requested` → `dispatch.approval_requested`; line 140 `result_buffered` → `dispatch.result_buffered`; line 165 enumeration expanded to 13 events with side-labels. Per-event "When Fired" clause landed in Spec-006 category body. Teleport precedent cross-referenced with verified live-check on 2026-04-18 — T5000I (`access_request.create`) and T5002I (`access_request.review`) confirmed, additional lifecycle events T5001I (`access_request.update`), T5003I (`access_request.delete`), T5004I (`access_request.search`), T5005I (`access_request.expire`) cited; Teleport's single-event-with-state-transition model contrasted against AI Sidekicks' more granular distinct-event model. No PASETO tokens or secret material carried in event payloads; ApprovalRecord envelope referenced by `approvalRecordRef` only. ADR-017 federated-log semantics cited in category preamble.
