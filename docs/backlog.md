...
- Summary: Plan-001 is the tier-entry plan for shared session core and owns the `0001-initial.sql` migration in `packages/runtime-daemon/src/persistence/migrations/`. After the post-P0 spec cascade (BL-050 integrity protocol, BL-064 event taxonomy extension, BL-065 ADR-018, BL-066 PII data map), Plan-001 must forward-declare schema shape that downstream plans assume is already present at tier entry. Required edits: (1) add `participant_keys` table (columns: `participant_id` PK, `session_id`, `key_material` BLOB, `created_at`, `destroyed_at NULLABLE`) to `0001-initial.sql` as per-participant AES-256-GCM key custody per Spec-022 §Shred Fan-Out Path 1; (2) add `session_events.pii_payload BLOB NULLABLE` column forward-declared per cross-plan-dependencies.md §1 Contested rows carve-out; (3) add integrity columns to `session_events`: `prev_hash BLOB`, `row_hash BLOB NOT NULL`, `daemon_signature BLOB NOT NULL`, `monotonic_ns INTEGER NOT NULL` per Spec-006 §Integrity Protocol (BLAKE3 + Ed25519 + RFC 8785 JCS canonical-serialization hash chain); (4) add `min_client_version TEXT` column to `sessions` table per ADR-018 §Decision for monotonic session-floor enforcement; (5) add ADR-018 to Plan-001 `Required ADRs` header row; (6) add a §Cross-Plan Forward-Declared Schema section in the plan body listing which downstream plans own the semantics of each forward-declared column (Plan-006 owns integrity columns, Plan-022 owns `pii_payload` + `participant_keys`, Plan-003 owns `min_client_version` enforcement). Rework magnitude: Data And Storage Changes section expands to ~3x current length; API surface unchanged; Test Plan adds migration-integrity tests (column presence + NULL-default checks). Plan status flip `review` → `approved` gates on this landing.
...
- Summary: Plan-003 governs participant-owned RuntimeNode registration, attach/detach, capability declaration, and presence heartbeat between the Local Runtime Daemon and the Collaboration Control Plane. Plan-003 currently cites ADR-002, ADR-008, ADR-015 in `Required ADRs` but under-cites the governing decisions surfaced by post-P0 work. Required edits: (1) add ADR-001 (session as primary domain object — RuntimeNode attaches to a session, so session-centric invariants bind the attach contract), ADR-007 (collaboration trust model — the plan's local `node_trust_state` table materializes the trust classification defined in ADR-007), and ADR-018 (cross-version compatibility — capability declarations carry version info and `min_client_version` gates attach eligibility) to the `Required ADRs` header row; (2) add §Event Emission subsection (or extend §Data And Storage Changes) documenting that Plan-003's node-registry and attach services are the canonical emitters of the 9-event `runtime_node_lifecycle` category introduced by BL-064 — specifically `runtime_node.registered/online/degraded/offline/revoked/capability_declared/capability_updated` (7 events) plus the 2 `session.clock_*` wire-stable event names reclassified into this category per ADR-018 §Decision #3 — with a cross-reference to Spec-006 §Runtime Node Lifecycle; (3) add `min_client_version` check to the attach flow so below-floor daemons are rejected at attach-time with `VERSION_FLOOR_EXCEEDED` per ADR-018. Rework magnitude: header row edit + ~2 paragraph additions to body; no API or test-plan structural rewrite.
...
#### BL-091: Rewrite Plan-006 for 120 events / 15 categories + integrity protocol + PII columns
...
- Summary: Plan-006 is the plan most heavily impacted by the post-P0 spec cascade and requires rewrite rather than amendment. Spec-006 grew from 93 → 120 events across 6 new categories (runtime_node_lifecycle, recovery_events, participant_lifecycle, audit_integrity, event_maintenance, policy_events); category count 9 → 15. Spec-006 §Integrity Protocol landed the BLAKE3 + Ed25519 + RFC 8785 JCS canonical-serialization hash chain with Merkle anchors to `event_log_anchors`. Spec-022 added the §Shred Fan-Out Path 1/2/3 ordering contract and §Signature Safety Under Shred proof. Required edits: (1) update event enumeration target count from 93 → 120 and category count 9 → 15 in plan header/summary; (2) add §Integrity Protocol subsection describing how Plan-006 implements the BLAKE3 + Ed25519 + JCS hash chain + Merkle anchor emission into `event_log_anchors`; (3) add §PII Columns subsection documenting `session_events.pii_payload` read/write semantics (forward-declared in Plan-001 per BL-089, owned by Plan-006) and `pii_ciphertext_digest` one-way indirection for signature safety; (4) add §Shred Fan-Out Cross-References subsection naming which events must respect Path 1 ordering (crypto-shred before delete) — specifically `participant.purged` aggregate event emission timing; (5) add ADR-012 (Cedar authorization — policy_events category), ADR-017 (no shared event log in V1 — federation semantics), ADR-018 (cross-version compatibility — version stubs and min_client_version), ADR-020 (V1 deployment + OSS license — audit log visibility surface) to `Required ADRs`; (6) add §Audit Integrity Invariant subsection stating `audit_integrity` category events are never compacted and never shredded (per Spec-006 §Audit Integrity category invariant). Rework magnitude: full plan body rewrite (not amendment) — scope/data/API/test sections all expand ~2-3x.
- Exit Criteria: Plan-006 header reflects 120 events / 15 categories; §Integrity Protocol, §PII Columns, §Shred Fan-Out Cross-References, §Audit Integrity Invariant subsections exist; `Required ADRs` row includes ADR-012, ADR-017, ADR-018, ADR-020; Plan-006 status header reads `approved`; no retrospective/amendment phrasing.
...
- Summary: Plan-020 governs observability and failure recovery. Spec-020 §PII in Diagnostics (landed under BL-066) introduced a 5-point Required Behavior policy: (1) default-deny outbound telemetry, (2) opt-in for raw content, (3) bounded local retention ≤ 7-day default with `retention_policy_override` warning metric on > 30-day overrides, (4) shred fan-out coverage mandatory for all 4 diagnostic buckets, (5) summary-only retention from non-PII signals by construction. Plan-020 currently has no implementation surface for this policy. Required edits: (1) add §PII in Diagnostics subsection under §Target Areas covering the 4 diagnostic buckets (`driver_raw_events`, `command_output`, `tool_traces`, `reasoning_detail`) and their TTL bucket purge paths; (2) add `DiagnosticRedactionPolicy` API surface reference to §API And Transport Changes; (3) add TTL bucket schema reference to §Data And Storage Changes (schema owned by Plan-020 since these are runtime-local diagnostic stores); (4) add §Implementation Step for default-deny outbound telemetry wiring + opt-in raw content configuration knob; (5) add 3-4 test cases to §Test And Verification Plan: outbound-telemetry-default-deny, raw-content-opt-in-explicit-only, TTL-bucket-purge-coverage, `retention_policy_override` warning emission on > 30-day config. Rework magnitude: ~1-2 paragraphs added to each of Target Areas / API / Data / Implementation / Tests sections; no structural rewrite.
