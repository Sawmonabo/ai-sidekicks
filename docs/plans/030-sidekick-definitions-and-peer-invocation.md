...
The following invariants are **load-bearing** and MUST be preserved across all Plan-030 PRs and downstream extensions. Any change that would weaken or remove an invariant requires a coordinated cross-plan amendment (see cross-plan-dependencies.md).
...
- `packages/contracts/src/session.ts` (EXTEND, Plan-001-owned) — the additive-optional `peerInvocationEnabled` member on `SessionReadResponse` and its key on the `.strict()` `SessionReadResponseSchema` (T4.2, CP-030-8). A sanctioned additive-member edit, not ownership.
...
- **T4.2 — Enablement lifecycle.**
  - **Files:** `packages/runtime-daemon/src/sidekicks/peer-invocation-handler.ts` (NEW — enablement leg), `packages/contracts/src/session.ts` (EXTEND — the additive-optional `peerInvocationEnabled` member on `SessionReadResponse` **and** the matching key on the shipped `SessionReadResponseSchema`, per CP-030-8; Plan-001-owned, a sanctioned additive-member edit rather than ownership), `packages/contracts/src/event.ts` (EXTEND — the `session.peer_invocation_set` type literal, its `SESSION_EVENT_CATEGORY_BY_TYPE` row, and its `SessionEventSchema` payload arm, through the additive union-registration seam per CP-030-5).
