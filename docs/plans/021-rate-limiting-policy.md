...
- **Wiring endpoint groups owned by later-landing plans.** `invite.*` wiring is Plan-002 Phase 4 (T4.1 window rows, T4.3 the `invite.pending_cap` store-side cap); the six `artifact.*` relay rows are [Plan-014](./014-artifacts-files-and-attachments.md) T14.14 / T14.15 at Tier 7; `event.subscribe` concurrency-cap enforcement is the SSE subscription surface (Plan-008 Phase R5); `artifact.publish` is **dormant in V1** — no control-plane method string or tRPC procedure for artifact publication is registered anywhere in the corpus, publication being a client↔daemon call, so the `Spec-021 §ADR Triggers` network-reachable-publication bullet is that row's gate (no backlog item); `approval.resolve` is **dormant in V1** — Plan-012's ratified transport is daemon JSON-RPC only (D-012-5) and Spec-021 excludes the daemon IPC path from rate limiting, so no V1 wiring surface exists (the `Spec-021 §ADR Triggers` conditions are the re-arm gate — no backlog item). See §Endpoint Wiring Ownership.

## Preconditions

- [x] **Plan-readiness audit complete per [runbook](../operations/plan-implementation-readiness-audit-runbook.md)** — Tier-6 audit (2026-06-10): 74 findings adjudicated via A-021-1..24, D-021-1..17 ratified; full rewrite to the four-phase audited Tasks structure; companion amendments in Spec-021, api-payload-contracts.md, error-contracts.md, Plan-008 (CP-008-9), Plan-020, Plan-025, Spec-027, deployment-topology.md, shared-postgres-schema.md, Spec-022, the GDPR erasure runbook, backlog.md (BL-120 rewrite + BL-144/145/146), and cross-plan-dependencies.md §2/§3/§5. **Re-opened 2026-08-25, scoped to the rate-limit wiring amendment (§6 node NS-80):** this plan mints **CP-021-6**, the concurrency-cap half of the enforcement contract owed to the two store-side cap owners, and materially rewrites four §Endpoint Wiring Ownership rows — `event.subscribe` (re-priced 10 concurrent, assigned to Plan-008 Phase R5 with the pre-tRPC refusal requirement stated), `artifact.publish` (moved into the dormant class), the six `artifact.*` relay rows (split T14.14 / T14.15 by landing phase, with the transport-invariant admission rule), and the `invite.*` row (T4.3 added for the store-side pending cap) — so it flips `approved → review` under the [Status Flip Rule](../operations/plan-implementation-readiness-audit-runbook.md#status-flip-rule)'s new-CP row. Spec-021 flips beside it for the registry, §Overflow Response, §Fallback Behavior, §ADR Triggers, and §Acceptance Criteria growth. **Delivered 2026-08-25, in the same diff:** the rate-limit-wiring targeted readiness-audit delta (§6 node NS-80) audited the re-open's named scope from this side — that CP-021-6 provides only what the cap owners cannot author themselves and duplicates no `rateLimitProcedure` responsibility, that no cap owner re-transcribes a registry limit (I-021-2 holds), that the four rewritten rows leave the registry at 25 keys and every key still assigned exactly once (the `invite.*` row's key list gains `invite.preview`, a repair of a pre-existing omission rather than a new assignment — the row already owned every `invite.*` key by scope), that Spec-021's §Fallback Behavior is left **unmodified** for cap rows (a draft status deviation was withdrawn at Codex round 2 on primary-source grounds, so the fail-open-then-fail-closed-`503` behavior the T21.2-8 wrapper and the T21.4 outage rows implement is untouched and no Plan-021 task is invalidated) and that the appended acceptance criterion is **last** so the AC ordinals this plan cites positionally (`Spec-021 AC1`..`AC8`) do not move, and that the two Tier-6-deferred consumers (Plan-008 Phase R5, Plan-002 Phase 4) are scoped so no Plan-021 → Plan-008 or Plan-021 → Plan-002 edge inverts — re-checked this box, and restored `approved`. The restoration clears the audit and Status gates only; the two unchecked Plan-008 substrate boxes below stand unchanged.
...
- [ ] Plan-008 R3 exposes the per-frame admission seam per CP-008-9 (cross-plan amendment executed at this audit's swap). Gates the **hosted** half of Phase 3 WS wiring only; the self-host path is independently wired by Plan-025 steps 7-8 and is not gated on this precondition (D-021-2).
...
export type RateLimitEndpointGroup =
  ...
  | "invite.pending_cap"
  | "invite.accept"
  | "invite.preview" // 2026-08-11 invite.preview amendment (BL-133); stacks with invite.redeem_ip
  | "invite.redeem_ip"
  | "presence.heartbeat"
  | "event.query"
  | "event.subscribe"
...
- **Per-frame endpoint routing:** `endpointFor` selects the registry group from the decoded envelope's 1-byte `type` discriminator only (ciphertext-envelope metadata — the CP-008-9 seam exposes no payload). The relay's 1-byte `RelayFrameType` (Spec-008 §Message Framing; the contracts wire enum per Plan-008 T-008r-1-3) carries **two** V1 frame categories — **forwardable pairwise-ciphertext envelopes** and **broker-handled control frames** (bundle post/distribution, keepalive, close). Only the forwardable-envelope category transits the CP-008-9 opaque-forward seam this hook binds into (T-008r-3-3 decodes ciphertext-envelope metadata only), so `endpointFor` maps it → `ws.message`; the broker-handled control frames are dispatched on Plan-008's broker control path (Plan-008 T-008r-3-9) — **not** metered as `ws.message`, **not** a Plan-021 registry row (their admission is Plan-008's). The seam is retained (not inlined into `wsRateLimit`) because it is the CP-021-3-exported contract Plan-025 consumes — a ratified Tier-6-audit signature, out of scope to change here. `presence.heartbeat` is **not** reachable here: presence heartbeats ride the WebSocket (JSON-RPC 2.0) collaboration channel (Spec-002 §Heartbeat Transport; Spec-008 §Control-Plane Transport Protocol), not the relay WSS binary frames, so no relay `type` byte can carry one.
...
| --- | --- | --- | --- | --- |
| CP-021-1 | consumes ← | Plan-008-remainder (Tier 5) | CP-008-5 stable tRPC middleware-mount surface that `rateLimitProcedure` mounts onto; Plan-008 authors no rate-limit **middleware** task — `rateLimitProcedure` authorship stays here. **Scope note (2026-08-25, §6 node NS-80, mirroring the narrowing on the CP-008-5 side):** the `event.subscribe` `concurrency_cap` row is the carve-out — `Spec-021`'s enforcement-class rule assigns a cap to its owning resource surface and D-021-6 assigns this one to Plan-008 by name, so the Tier-6-deferred Plan-008 Phase R5 authors a lease store and a pre-dispatch refusal under CP-008-17 ⇄ CP-021-6, consuming no `rateLimitProcedure`. The admin-route pre-tRPC match and the DO class export are export-only edits inside the same stable-mount seam. | Plan-008 CP-008-5 (return-cite) |
| CP-021-2 | provides → | Plan-002 Phase 4 (within-Tier-6, lands after this plan) + Plan-014 T14.14 / T14.15 (Tier 7, the tRPC-procedure arm of the six relay rows — CP-014-9) | `rateLimitProcedure({ endpoint })` middleware factory + the canonical 429 `RateLimitResponse` (typed export) that the invite-endpoint tests assert; `invite.*` registry keys reserved in `RateLimitEndpointGroup`. | Plan-002 CP-002-3 (return-cite); cross-plan-dependencies.md §3 Plan-002 row |
| CP-021-3 | provides → | Plan-025 remainder (Tier 7) + Plan-014 T14.15 (Tier 7, the raw-route arm — `artifact.fetch.chunk` consumes `checkAdmission` at the route match per CP-014-9) | `RateLimiter` contract + `PostgresRateLimiter` (constructor: injected `pg.Pool` + `endpoint-limits` + `EscalationStore`) + `AdminBansStore` + admin-bans routes + `wsRateLimit`, instantiated not re-implemented; the `AIS_RATELIMIT_BACKEND` / `AIS_RATELIMIT_FAILOPEN_SECONDS` / `AIS_RATELIMIT_MODE` env contract; the exported contract suite at `packages/control-plane/src/rate-limit/rate-limiter-contract-suite.ts` (`Plan-025` re-runs it). | Plan-025 §Dependencies / §Preconditions / step 7 / §Test |
...
| CP-021-5 | consumes ← | Plan-008 R3 (Tier 5) | CP-008-9 per-decoded-frame admission seam on the zero-knowledge DO relay broker (ciphertext-envelope metadata + connection principal only) into which `wsRateLimit` injects on the hosted path. | Plan-008 CP-008-9 |
| CP-021-6 | provides → | Plan-008 Phase R5 (Tier 6) + Plan-002 Phase 4 (within-Tier-6; both land after this plan) | **The concurrency-cap half of the enforcement contract**, for the two store-side cap owners the §Endpoint Wiring Ownership table names (2026-08-25, §6 node NS-80). Three exported pieces, none of them `rateLimitProcedure`: (a) the `endpoint-limits.ts` cap entries (`event.subscribe`, `invite.pending_cap`) **read** by the cap owner rather than re-transcribed, so the registry stays the single enumeration (I-021-2); (b) the canonical 429 `RateLimitResponse` builder together with the concurrency-cap header subset from `Spec-021 §Overflow Response` (truthful `X-RateLimit-Limit` + `X-RateLimit-Remaining: 0`; `Retry-After` / `X-RateLimit-Reset` / `retryAfter` / `resetAt` omitted), **exported rather than module-private**, so a cap refusal is byte-identical to a window refusal and neither owner re-derives the envelope; (c) the `rate_limit_trip_total{endpoint,tier}` registration the cap owner increments on a genuine cap refusal (and `rate_limit_backend_error_total{backend}` for a backend-fault refusal, never both on one refusal), so cap trips and window trips report through one telemetry family; and (d) **`checkCapAdmission` + `recordCapViolation`** (added 2026-08-25 at the Codex round-2 fold) — the ban → active-block legs **without** the counter stage, plus the violation hook. This pair is not a convenience: the shipped `check` runs block lookup and counter consumption inseparably and `EscalationStore.recordViolation` is internal (A-021-19), so without it a cap owner must either double-price its row through a sliding-window bucket or bypass the `Spec-021 §Escalation` ladder entirely — neither acceptable, and both invisible in review. `rateLimitProcedure` is deliberately **not** the vehicle — a cap is enforced at its owning resource surface, and the `event.subscribe` surface must additionally refuse **before** tRPC dispatch (the D-021-10 raw-route precedent), which no procedure middleware can do. | Plan-008 CP-008-17; Plan-002 CP-002-3 |
...
| D-021-6 | `Spec-021 §Canonical Endpoint Group Registry` is the single limit enumeration with canonical keys; stacked invite-redemption rows (`invite.redeem_ip` 5/IP/min + `invite.accept` 10/min/token-hash); `general.api`/`unauthenticated.request` fallback buckets; concurrency-cap rows enforced store-side by named owners; step-10-style census replaced by the §Endpoint Wiring Ownership table; the later-landing wiring obligations are named tasks in their owning plans as of 2026-08-25 (Plan-008 Phase R5, Plan-014 T14.14 / T14.15, Plan-002 T4.1 / T4.3), and the three sentinels that had tracked them — BL-120, BL-144, BL-146 — closed into those tasks in the same swap, joining BL-145 (the dormant-row sentinel, withdrawn 2026-08-25); every dormant row's gate is a `Spec-021 §ADR Triggers` condition or a named V1.1 surface, never a backlog item. |
...
- [ ] **T21.1-6 — Contracts schema tests (`packages/contracts/src/__tests__/`).** AdminBan\* request/response Zod schemas round-trip the documented example payloads; `RateLimitResponseSchema` parses the 5-field envelope and rejects the legacy 4-field shape and any half-timed envelope (`retryAfter` without `resetAt`, and vice versa — the both-or-neither refinement); `RateLimitEndpointGroup` union matches the registry key census (snapshot of the 25 keys — the six `artifact.*` relay keys joined via the 2026-07-08 Spec-021 amendment and its 2026-07-09 `artifact.fetch.complete` + `artifact.upload.complete` additions, and `invite.preview` via the 2026-08-11 BL-133 invite-preview amendment).
...
- [ ] **T21.2-6 — `postgres-rate-limiter.ts`.** `PostgresRateLimiter implements RateLimiter` wrapping `RateLimiterPostgres`, one instance per `(endpoint group, tier variant)` (D-021-4), `pg.Pool` constructor-injected (`storeClient` — never constructed; Plan-025 is the production instantiator, CP-021-3), points/duration from `endpoint-limits.ts`, `keyPrefix` namespacing into `ratelimit_*` with library-managed creation. Same internal composition as T21.2-5 (block pre-check → consume → recordViolation on trip). Add `rate-limiter-flexible: ^11.0.0` to `packages/control-plane/package.json`; contract suite asserts `semver.gte(installedVersion, '11.0.0')` (documented-pin → enforced-pin). Postgres TLS posture (`sslmode=verify-full`; refusal table per Spec-027 row 5, CVE-2024-10977 — fixed in PG 17.1/16.5/15.9/14.14/13.17/12.21) is enforced where the connection string is parsed: Plan-025's config loader (Spec-027 row-5 ownership). This class documents the requirement on the injected pool and the contract suite exercises a `parsePostgresConfig()` refusal-table assertion; it does not parse connection strings.
  - **Spec coverage:** Spec-021 §Deployment-Aware Abstraction (self-host = rate-limiter-flexible + Postgres), Spec-021 §Implementation Notes (sliding window), Spec-021 AC7 (identical limits, via shared suite)
...
- [ ] **T21.2-9 — `rate-limiter-contract-suite.ts` + runners.** Export `describeRateLimiterContract(makeLimiter: () => Promise<RateLimiter>)` — scenario set: under-limit allow; at-limit deny; header-source fields present + internally consistent; window expiry re-allow; per-endpoint-group isolation; per-tier variant selection (elevated 3×, D-021-4); **block-window behavioral rows (I-021-3):** after the 3/5-min ladder trips, advance fake time past the counter window (capacity restored) but within the 15-min block → next check REJECTED with `resetAt = blockUntil` and `blockUntil` set (the trip-vs-block discriminator); advance past expiry → allowed; same for the 10/1-hr ladder asserting `escalatedTo1h` fires exactly once. Runners: `postgres-rate-limiter.contract.test.ts` (PGlite/testcontainer per harness note) and `cloudflare-rate-limiter.contract.test.ts` (hand-rolled `env.<LIMITER>.limit()` fake honoring `endpoint-limits.ts` + DO-storage fake — `@cloudflare/workers-types` is types-only; fidelity caveat recorded: local emulation does not reproduce production per-edge-location counting; I-021-2 parity is asserted at the contract level, not edge-distribution level).
  - **Spec coverage:** Spec-021 §Deployment-Aware Abstraction (identical limits — the parity proof), Spec-021 AC4 (behavioral half), Spec-021 AC5 (behavioral half), Spec-021 AC7, Spec-021 AC9 (elevated)
  - **Verifies invariant:** I-021-2, I-021-3
...
- 2026-08-11 — **Spec-021 invite-preview registry true-up (PR #322 / NS-57 — no Status flip).** The BL-133 amendment inserts the anonymous non-consuming `invite.preview` registry row (24 → 25 keys), stacked with `invite.redeem_ip` as the registry's one deliberate exception to the most-specific-row rule, and this plan's dependent surfaces true up in the same PR: the §API And Transport Changes union sketch gains the `"invite.preview"` arm and T21.1-6's snapshot census reads 25. The row insertion also shifted Spec-021's line numbering, so every line-anchored Spec-021 `Spec coverage:` cite on this plan converted to `§Heading (descriptor)` form (and T21.4-1's touched Spec-027 cite dropped its raw line locator) under the never-renumber rule. Cite-form and census corrections only: no task, invariant, phase, or scope change, and the `approved` Status holds — the 2026-07-08/09 `artifact.*` registry growths set the no-flip precedent for registry-row additions landing on this plan's census surfaces.
...
- [ ] `rateLimitProcedure` is wired on every endpoint group the §Endpoint Wiring Ownership table assigns to this plan; the groups it assigns elsewhere are owned by named tasks (Plan-002 T4.1 / T4.3, Plan-008 Phase R5, Plan-014 T14.14 / T14.15), and the dormant rows are covered by the dormant-row item below.
- [ ] The CP-021-6 concurrency-cap surface is exported for the store-side cap owners and not left module-private: the `endpoint-limits.ts` cap entries are readable, the canonical 429 builder and its concurrency-cap header subset are importable outside the tRPC middleware, and `rate_limit_trip_total{endpoint,tier}` accepts an increment from a caller that is not `rateLimitProcedure` — verified by a test that builds a cap refusal through the exported surface and asserts it byte-matches the window refusal minus the timing pair.
...
Tier 6 per [cross-plan-dependencies.md §5 Canonical Build Order](../architecture/cross-plan-dependencies.md#5-canonical-build-order). Strictly **downstream of Plan-008** for the CP-008-5 tRPC mount + CP-008-9 frame seam this plan consumes — with **Plan-008 Phase R5 the one Plan-008 surface that is downstream of this plan** (2026-08-25, §6 node NS-80) and **upstream of Plan-025** (the self-hostable relay instantiates `PostgresRateLimiter` + `wsRateLimit` + the admin routes inside its compose-deployed process, CP-021-3). Additionally: (a) within-Tier-6 upstream of **Plan-002 Phase 4**, which consumes `rateLimitProcedure` for invite-endpoint wiring per CP-021-2 and the cap surface per CP-021-6, **and of Plan-008 Phase R5**, which consumes the cap surface per CP-021-6 ⇄ CP-008-17 — a Tier-5 plan's phase deliberately deferred into Tier 6 on the Plan-002 Phase 4 precedent, so the edge stays acyclic rather than inverting this plan's own Plan-008 dependency; (b) registers its metric families against Plan-020's §Prometheus /metrics Exposition contract (Tier 8 — doc-contract compliance only, no code consumed; CP-021-4) with self-host exposition mounted by Plan-025 (Tier 7).
