...
- **Wiring endpoint groups owned by later-landing plans.** `invite.*` wiring is Plan-002 Phase 4 (BL-120); `artifact.publish` is Plan-014 at Tier 7 (BL-146); `event.subscribe` concurrency-cap enforcement is the SSE subscription surface (BL-144); `approval.resolve` is **dormant in V1** — Plan-012's ratified transport is daemon JSON-RPC only (D-012-5) and Spec-021 excludes the daemon IPC path from rate limiting, so no V1 wiring surface exists (the `Spec-021 §ADR Triggers` condition is the re-arm gate — no backlog item). See §Endpoint Wiring Ownership.
...
| `approval.resolve` | — (dormant in V1) | No V1 wiring surface: Plan-012's transport is daemon JSON-RPC only (D-012-5; api-payload §Approval Method-Name Registry) and Spec-021 excludes daemon IPC from rate limiting (§Scope, §Non-Goals, AC). Row stays priced/reserved; it re-arms on the `Spec-021 §ADR Triggers` topology condition (the daemon becoming network-reachable), wired then by the surface-owning plan at the registry price (the `presence.heartbeat` / `keypackage.upload` reserved-row posture; no backlog item) |
...
- [ ] The dormant registry rows are not wired and no V1 surface has armed them — `approval.resolve` gated on the `Spec-021 §ADR Triggers` condition (the daemon becoming network-reachable; a delegated/remote resolution surface, should one be introduced, would itself be an ADR trigger to register at that time — not a second condition here), `presence.heartbeat` on the V1.1 JSON-RPC-WS message-admission surface (`Spec-021 §Canonical Endpoint Group Registry` dormant-row semantics).
