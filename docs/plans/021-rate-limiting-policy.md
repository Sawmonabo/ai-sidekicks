...
| **Dependencies** | the shipped control-plane host (the stable tRPC middleware-mount surface consumed here; `wrangler.toml` deployment config extended); [Plan-031](./031-remote-control.md) (the relay's per-frame admission seam, consumed on the hosted path); Plan-018 (`AuthenticatedIdentityContext` — the `ctx.userId` producer for identity resolution; the former "role-claim surface for admin endpoints" dependency is removed by D-021-1). Non-blocking context, not dependencies: Plan-007 daemon-IPC scope exclusion (§Explicitly Out of Scope); the Plan-020 metric-name doc contract (CP-021-4 — Tier 8, no code consumed, no tier inversion); Plan-031's self-host relay node as a downstream consumer (see §Cross-Plan Obligations) |
...
- Elevated tier (3×) for session owners on the registry's elevated-eligible rows, resolved via a per-endpoint `resolveTier` hook with bounded-staleness session-ownership reads (D-021-4, D-021-5).
...
- WebSocket per-frame admission consumed by [Plan-031](./031-remote-control.md)'s relay (hosted) and by its self-host frame handler: drop-frame on counter trip with one in-band `rate_limited` frame; close `4029` only on active escalation block; ban refusals close with distinct code `4003` (D-021-9).
...
- **Wiring endpoint groups owned by later-landing plans.** The six `artifact.*` relay rows are [Plan-014](./014-artifacts-files-and-attachments.md) T14.14 / T14.15 at Tier 7; `event.subscribe` concurrency-cap enforcement is the SSE subscription surface's own ([Plan-031](./031-remote-control.md), CP-021-6); `artifact.publish` is **dormant in V1** — no control-plane method string or tRPC procedure for artifact publication is registered anywhere in the corpus, publication being a client↔daemon call, so the `Spec-021 §ADR Triggers` network-reachable-publication bullet is that row's gate (no backlog item); `approval.resolve` is **dormant in V1** — Plan-012's ratified transport is daemon JSON-RPC only (D-012-5) and Spec-021 excludes the daemon IPC path from rate limiting, so no V1 wiring surface exists (the `Spec-021 §ADR Triggers` conditions are the re-arm gate — no backlog item). See §Endpoint Wiring Ownership.

## Preconditions

- [x] **Plan-readiness audit complete per [runbook](../operations/plan-implementation-readiness-audit-runbook.md)** — Tier-6 audit (2026-06-10): 74 findings adjudicated via A-021-1..24, D-021-1..17 ratified; full rewrite to the four-phase audited Tasks structure; companion amendments in Spec-021, api-payload-contracts.md, error-contracts.md, Plan-020, Spec-027, deployment-topology.md, shared-postgres-schema.md, Spec-022, the GDPR erasure runbook, backlog.md (BL-120 rewrite + BL-144/145/146), and cross-plan-dependencies.md/§3/§5. **Re-opened 2026-08-25, scoped to the rate-limit wiring amendment:** this plan mints **CP-021-6**, the concurrency-cap half of the enforcement contract owed to the store-side cap owner, and materially rewrites four §Endpoint Wiring Ownership rows — `event.subscribe` (re-priced 10 concurrent, assigned to the SSE subscription surface with the pre-tRPC refusal requirement stated), `artifact.publish` (moved into the dormant class), and the six `artifact.*` relay rows (split T14.14 / T14.15 by landing phase, with the transport-invariant admission rule) — so it flips `approved → review` under the [Status Flip Rule](../operations/plan-implementation-readiness-audit-runbook.md#status-flip-rule)'s new-CP row. Spec-021 flips beside it for the registry, §Overflow Response, §Fallback Behavior, §ADR Triggers, and §Acceptance Criteria growth. **Delivered 2026-08-25, in the same diff:** the rate-limit-wiring targeted readiness-audit delta audited the re-open's named scope from this side — that CP-021-6 provides only what the cap owner cannot author itself and duplicates no `rateLimitProcedure` responsibility, that no cap owner re-transcribes a registry limit (I-021-2 holds), that the rewritten rows leave every registry key assigned exactly once, that Spec-021's §Fallback Behavior is left **unmodified** for cap rows (a draft status deviation was withdrawn at Codex round 2 on primary-source grounds, so the fail-open-then-fail-closed-`503` behavior the T21.2-8 wrapper and the T21.4 outage rows implement is untouched and no Plan-021 task is invalidated) and that the appended acceptance criterion is **last** so the AC ordinals this plan cites positionally (`Spec-021 AC1`..`AC8`) do not move, and that the deferred cap consumer is scoped so no build-order edge inverts — re-checked this box, and restored `approved`. The restoration clears the audit and Status gates only; the two unchecked substrate boxes below stand unchanged.
...
- [x] ADR-014 (tRPC control-plane API) is accepted — establishes the tRPC router surface; the concrete middleware-mount seam is the control-plane host's (see CP-021-1). The admin-bans routes are raw HTTP matched before tRPC dispatch per D-021-10 (the ADR-014 "narrow REST facade" posture for an operator-facing surface; route shape mandated by `Spec-021 §Interfaces And Contracts`).
- [x] ADR-020 (V1 deployment model) is accepted — declares both rate-limiter backends as V1-scope.
- [x] ADR-010 (PASETO v4.public) is accepted — provides the token primitive behind `AuthenticatedIdentityContext` (user identity resolution for the middleware). The admin-bans surface does NOT use PASETO (D-021-1).
- [ ] The Tier-5 control-plane surfaces are shipped: the stable middleware-mount surface (PASETO middleware + router host) that `rateLimitProcedure` mounts onto.
- [ ] [Plan-031](./031-remote-control.md) Phase 3 exposes the relay's per-frame admission seam (CP-021-5). Gates the **hosted** half of Phase 3 WS wiring only; the self-host path is independently wired on the self-host relay node and is not gated on this precondition (D-021-2).
- [ ] Plan-018 ships `AuthenticatedIdentityContext` (the `ctx.userId` producer). The former Cedar-consumable role-model precondition is removed (D-021-1).
- [x] BL-135 (resolved): canonical self-host admin-token path is `./data/admin-token`. Gates nothing in this plan's code (the operator token is constructor-injected config); recorded because the self-host relay node's token generator is the self-host credential provider for the admin-bans surface (D-021-1).
...
- `packages/control-plane/src/rate-limit/` — **created by this plan.**
  ...
  - `rate-limiter-contract-suite.ts` — exported shared contract suite (`describeRateLimiterContract`), the I-021-2 parity proof the self-host relay node re-runs (A-021-20).
...
- `packages/control-plane/src/migrations/NNNN-rate-limit-tables.ts` — **extends the control-plane migration-runner series (v1 = Plan-001).** NNNN = next free version at landing time, append-order per cross-plan-dependencies.md (D-021-11). Two new tables: `admin_bans`, `rate_limit_escalations`.
- `packages/control-plane/src/server/host.ts` — **extended by this plan (export-only edit):** re-export `RateLimitEscalationDO` from the Worker entry module (Cloudflare requires DO classes exported from the deployed script; A-021-19) and mount the admin-bans route match ahead of tRPC dispatch (D-021-10), both inside the CP-021-1 stable-mount seam.
- `packages/control-plane/package.json` — **extended by this plan:** `rate-limiter-flexible: ^11.0.0`, `prom-client` dependencies.
- `packages/runtime-daemon/src/__tests__/no-rate-limit-import.test.ts` — **created by this plan.** Daemon-exclusion structural test (A-021-20).
...
issued_by       TEXT         NOT NULL              -- operator attribution per D-021-1 ('deployment-operator' in V1; no user principal exists on this surface) — no FK; rows survive user deletion, D-021-13
...
- One binding per **60s-window** sliding-window endpoint group from the Spec-021 registry — the CF binding supports only 10s/60s periods, so the hourly row (`keypackage.upload`) is DO-tracked on hosted instead — the T21.2-5 step-f `checkAndConsume` branch (see §Open Questions binding-period-cap note), and the **dormant rows** (`presence.heartbeat`, `approval.resolve`, `artifact.publish`) declare no binding in V1 — named as a class rather than one instance so the carve-out stays correct the next time a row goes dormant (`Spec-021 §Canonical Endpoint Group Registry` dormant-row semantics; the class re-derived 2026-08-25). Elevated-eligible groups declare a second `<NAME>_ELEVATED` binding with `limit = 3 × <threshold>` (D-021-4). Each binding declared with `period: 60` (or `period: 10` for sub-minute limits) and `limit: <threshold>` per the registry.
- Example (partial):
  ```toml
  [[ratelimits]]
...
export type RateLimitIdentityType = "user" | "ip" | "token_hash" | "session" | "user"; // D-021-17 — 'user' reserved dormant for V1.1 keypackage.upload
...
export type RateLimitEndpointGroup =
  ...
  | "artifact.upload.complete" // 2026-07-09 upload finalization (Spec-014 Publish step 4); Plan-014 self-wires at Tier 7
...
// --- concurrency_cap admission types (CP-021-6, 2026-08-25) ----------------
...
export function createAdmissionCheck(deps: {
  ...
  // --- concurrency_cap surface (CP-021-6, 2026-08-25) -------------------------
  ...
  // (Spec-021: concurrency caps are not counters). These two are that seam; they
  // exist because `check` cannot be decomposed by its caller and `EscalationStore`
  // is internal (A-021-19), so without them a cap owner must either double-price
...
- **Authorization (D-021-1):** all three routes require the deployment's operator admin token as a bearer credential over TLS, compared constant-time, read once at startup. Self-host: the Spec-027 Row 3 first-run relay admin token (generated by the self-host relay node; canonical on-disk path `./data/admin-token` per BL-135). Hosted: operator-provisioned Workers secret `AIS_ADMIN_TOKEN` (`wrangler secret put`; the deploy-time-secret precedent). Trust-model rationale: ban issuance is a deployment-operator action, not an in-product user action — no user role model is required.
...
### WebSocket per-frame admission (consumed hosted and self-host per CP-021-5)
...
export const wsRateLimit =
  (
    ...
    endpointFor: (frame: WsFrame) => RateLimitEndpointGroup = () => "ws.message", // forwardable ciphertext-envelope routing — 1-byte type discriminator only (CP-021-5 metadata, never payload); control frames are broker-dispatched upstream by the relay and never reach this seam
  ) =>
...
- **Per-frame endpoint routing:** `endpointFor` selects the registry group from the decoded envelope's 1-byte `type` discriminator only (ciphertext-envelope metadata — the CP-021-5 seam exposes no payload). The relay's 1-byte `RelayFrameType` ([Spec-031](../specs/031-remote-control.md); the contracts wire enum the relay owns) carries **two** V1 frame categories — **forwardable pairwise-ciphertext envelopes** and **broker-handled control frames** (bundle post/distribution, keepalive, close). Only the forwardable-envelope category transits the CP-021-5 opaque-forward seam this hook binds into (the relay decodes ciphertext-envelope metadata only), so `endpointFor` maps it → `ws.message`; the broker-handled control frames are dispatched on the relay's broker control path — **not** metered as `ws.message`, **not** a Plan-021 registry row (their admission is the relay's). The seam is retained (not inlined into `wsRateLimit`) because it is the CP-021-3-exported contract the self-host relay node consumes — a ratified Tier-6-audit signature, out of scope to change here. `presence.heartbeat` is **not** reachable here: presence heartbeats ride the WebSocket (JSON-RPC 2.0) control-plane channel ([Spec-031](../specs/031-remote-control.md)), not the relay WSS binary frames, so no relay `type` byte can carry one.
- Zero-knowledge constraint (CP-021-5): the hosted seam exposes ciphertext-envelope metadata + the authenticated connection principal only; identity = the connection's PASETO-resolved user.
...
| I-021-2 | Both `RateLimiter` backends enforce identical limits and expose identical `check()` semantics for every canonical endpoint group; backend selection is configuration-only. | `rate-limiter-contract-suite.ts` (T21.2-9) green against both backends in CI; the self-host relay node re-runs it (CP-021-3). |
...
| I-021-7 | The check request/response and the 429 `RateLimitResponse` envelope have exactly one canonical shape — identical in `Spec-021 §Interfaces And Contracts`, error-contracts.md, api-payload-contracts.md, and `packages/contracts/src/rate-limiter.ts`. | contracts docs pre-propagated at the Tier-6 audit; T21.1-4/T21.1-5 parity verification at implementation; T21.1-6 schema tests (5-field parses; legacy 4-field + half-timed envelopes rejected); cross-doc drift re-checked at PR review. |
| I-021-8 | Rate-limit metric labels are compile-time-enumerable, PII-free, and emission-time-enforced per `Plan-020 §Prometheus /metrics Exposition (Spec-027 row 9)`; total series across the plan's families < 50. | T21.4-1 unit tests (out-of-allow-list label throws per family; series-count assertion). |

## Cross-Plan Obligations

| ID | Direction | Counterparty | Obligation | Anchor |
| --- | --- | --- | --- | --- |
| CP-021-1 | consumes ← | the control-plane host (Tier 5) | The stable tRPC middleware-mount surface that `rateLimitProcedure` mounts onto; the host authors no rate-limit **middleware** task — `rateLimitProcedure` authorship stays here. **Scope note (2026-08-25):** the `event.subscribe` `concurrency_cap` row is the carve-out — `Spec-021`'s enforcement-class rule assigns a cap to its owning resource surface and D-021-6 assigns this one to the SSE subscription surface, which authors a lease store and a pre-dispatch refusal under CP-021-6, consuming no `rateLimitProcedure`. The admin-route pre-tRPC match and the DO class export are export-only edits inside the same stable-mount seam. | cross-plan-dependencies.md `src/server/` row |
| CP-021-2 | provides → | Plan-014 T14.14 / T14.15 (Tier 7, the tRPC-procedure arm of the six relay rows — CP-014-9) | `rateLimitProcedure({ endpoint })` middleware factory + the canonical 429 `RateLimitResponse` (typed export) that the endpoint tests assert; the relay registry keys reserved in `RateLimitEndpointGroup`. | cross-plan-dependencies.md Plan-014 row |
| CP-021-3 | provides → | [Plan-031](./031-remote-control.md)'s self-host relay node (Tier 10) + Plan-014 T14.15 (Tier 7, the raw-route arm — `artifact.fetch.chunk` consumes `checkAdmission` at the route match per CP-014-9) | `RateLimiter` contract + `PostgresRateLimiter` (constructor: injected `pg.Pool` + `endpoint-limits` + `EscalationStore`) + `AdminBansStore` + admin-bans routes + `wsRateLimit`, instantiated not re-implemented; the `AIS_RATELIMIT_BACKEND` / `AIS_RATELIMIT_FAILOPEN_SECONDS` / `AIS_RATELIMIT_MODE` env contract; the exported contract suite at `packages/control-plane/src/rate-limit/rate-limiter-contract-suite.ts` (the relay node re-runs it). | [Spec-027](../specs/027-self-host-secure-defaults.md) rows 3 / 5 / 9 |
...
| CP-021-5 | consumes ← | [Plan-031](./031-remote-control.md) Phase 3 (the relay) | The per-decoded-frame admission seam on the zero-knowledge relay broker (ciphertext-envelope metadata + connection principal only) into which `wsRateLimit` injects on the hosted path. | Plan-031 §Phase 3 — The relay |
| CP-021-6 | provides → | the SSE subscription surface ([Plan-031](./031-remote-control.md); lands after this plan) | **The concurrency-cap half of the enforcement contract**, for the store-side cap owner the §Endpoint Wiring Ownership table names (2026-08-25). Three exported pieces, none of them `rateLimitProcedure`: (a) the `endpoint-limits.ts` cap entry (`event.subscribe`) **read** by the cap owner rather than re-transcribed, so the registry stays the single enumeration (I-021-2); (b) the canonical 429 `RateLimitResponse` builder together with the concurrency-cap header subset from `Spec-021 §Overflow Response` (truthful `X-RateLimit-Limit` + `X-RateLimit-Remaining: 0`; `Retry-After` / `X-RateLimit-Reset` / `retryAfter` / `resetAt` omitted), **exported rather than module-private**, so a cap refusal is byte-identical to a window refusal and neither owner re-derives the envelope; (c) the `rate_limit_trip_total{endpoint,tier}` registration the cap owner increments on a genuine cap refusal (and `rate_limit_backend_error_total{backend}` for a backend-fault refusal, never both on one refusal), so cap trips and window trips report through one telemetry family; and (d) **`checkCapAdmission` + `recordCapViolation`** (added 2026-08-25 at the Codex round-2 fold) — the ban → active-block legs **without** the counter stage, plus the violation hook. This pair is not a convenience: the shipped `check` runs block lookup and counter consumption inseparably and `EscalationStore.recordViolation` is internal (A-021-19), so without it a cap owner must either double-price its row through a sliding-window bucket or bypass the `Spec-021 §Escalation` ladder entirely — neither acceptable, and both invisible in review. `rateLimitProcedure` is deliberately **not** the vehicle — a cap is enforced at its owning resource surface, and the `event.subscribe` surface must additionally refuse **before** tRPC dispatch (the D-021-10 raw-route precedent), which no procedure middleware can do. | Plan-031 §Phase 4 |
...
| --- | --- | --- |
| `session.create` + `general.api` fallback | the shipped session router | Plan-021 T21.3-6 (Tier 6) |
| `presence.heartbeat` | the control-plane presence surface (Tier 5) | — (not wired in V1): device-liveness heartbeats ride the **WebSocket (JSON-RPC 2.0)** control-plane channel ([Spec-031](../specs/031-remote-control.md)), which is neither a tRPC procedure (nothing for `rateLimitProcedure` to wrap) nor a relay binary frame (the `endpointFor` seam decodes only the 1-byte ciphertext-envelope `type` and cannot admit a JSON-RPC message); no JSON-RPC-WS message-admission surface ships in V1, so the row stays priced/reserved (10/min) — the JSON-RPC-WS admission surface is the V1.1 wiring point (mirrors the `approval.resolve` / `keypackage.upload` reserved-row pattern — see [Spec-021](../specs/021-rate-limiting-policy.md) §Registry semantics for the live-channel distinction from `approval.resolve`) |
| `event.query` | the control-plane SSE/query surface | Plan-021 T21.3-6 for shipped procedures |
| `event.subscribe` (10 concurrent — concurrency_cap) | the control-plane SSE subscription registry | The subscription surface itself ([Plan-031](./031-remote-control.md), CP-021-6): a store-side lease registry keyed per user, refusing **pre-tRPC** in the host fetch handler because a tRPC middleware `TRPCError` on a subscription procedure is emitted as an in-stream SSE error under an already-`200` `text/event-stream` response and can never become an HTTP 429 (the D-021-10 raw-route precedent). Deferred past this plan so the edge stays acyclic |
| `health.check` | the control-plane host (+ the self-host relay node) | Plan-021 T21.3-6 (hosted); the relay node's own bootstrap (self-host) |
| `approval.resolve` | — (dormant in V1) | No V1 wiring surface: Plan-012's transport is daemon JSON-RPC only (D-012-5; api-payload §Approval Method-Name Registry) and Spec-021 excludes daemon IPC from rate limiting (§Scope, §Non-Goals, AC). Row stays priced/reserved; it re-arms on either `Spec-021 §ADR Triggers` condition — the topology trigger or the delegated/remote `approval.resolve` surface trigger — wired then by the surface-owning plan at the registry price (the `presence.heartbeat` / `keypackage.upload` reserved-row posture; no backlog item) |
| `artifact.publish` | — (dormant in V1) | No V1 wiring surface: **no control-plane method string or tRPC procedure for artifact publication is registered anywhere in the corpus** — [API Payload Contracts](../architecture/contracts/api-payload-contracts.md) shapes the `ArtifactPublish` family and binds its ingest sibling to the **local IPC transport**, `Spec-014 §Interfaces And Contracts` places publication at the client↔daemon boundary, Plan-014 T14.5 registers `artifact.*` under Plan-007's daemon JSON-RPC registry, and Spec-021 excludes daemon IPC from rate limiting (§Scope, §Non-Goals, AC). The network-reachable counterpart is the six relay rows below, which is what the `artifact.upload.init` registry note means by pricing itself against `artifact.publish` cadence. Row stays priced/reserved; it re-arms on the `Spec-021 §ADR Triggers` network-reachable-publication bullet, wired then by the surface-owning plan at the registry price (the `approval.resolve` / `presence.heartbeat` / `keypackage.upload` reserved-row posture; no backlog item) |
| `artifact.upload.init` / `artifact.upload.chunk` / `artifact.upload.complete` / `artifact.fetch.authorize` / `artifact.fetch.chunk` / `artifact.fetch.complete` | Plan-014 (Tier 7) | [Plan-014](./014-artifacts-files-and-attachments.md) self-wires at Tier 7, split by phase because the rows ship in different PRs: **T14.14** (Phase 4) the three upload rows plus the shared admission helper, **T14.15** (Phase 5) the three fetch rows (CP-014-9 ⇄ CP-021-2 + CP-021-3 — both arms, since the raw-route half consumes `checkAdmission`, not the middleware). Enforcement is stated transport-invariantly — admission resolves **before any request-body byte is consumed**. That property picks the mounting rather than adapting to it: a **body-bearing** row is admitted at the host/raw-route seam through `checkAdmission` before the body parser, because tRPC's fetch adapter parses a procedure's input before its middleware runs and a `rateLimitProcedure` wrap would therefore admit only after the payload was already spent; `rateLimitProcedure` serves the rows whose admission key resolves without reading a body; `artifact.fetch.chunk` is definitively the latter (`Spec-014` serves fetch chunks as per-chunk authenticated GETs), and the self-host relay node's Fastify hook is the standing precedent for consuming `createAdmissionCheck` outside tRPC (2026-07-08 Spec-021 cross-node relay amendment, extended 2026-07-09 with the fetch-complete ack and upload-finalize rows) |
| `ws.message` | the relay ([Plan-031](./031-remote-control.md), hosted) / the self-host relay node | Plan-021 T21.3-4 via CP-021-5 (hosted); the relay node's own bootstrap (self-host) |
| `auth.endpoint` / `unauthenticated.request` | Plan-018 auth + the host's unauthenticated procedures | Plan-021 T21.3-6 (fallback buckets + auth group over shipped procedures) |
...
| D-021-4 | Elevated tier implemented for session owners: per-`(endpoint group, tier)` limiter variants (CF `_ELEVATED` bindings at 3×; PG per-pair instances); `resolveTier` session-ownership hook on elevated-eligible rows; absent tier never defaults to elevated; Spec-021 narrows `elevated` to session-owner ("system service" removed — no V1 service principal). |
| D-021-5 | Bounded-staleness identity reads: ban lookups + session-ownership reads cached in-memory ≤60s per process/isolate, write-through invalidation in the issuing process; ban/revoke propagation bound ≤60s documented. |
| D-021-6 | `Spec-021 §Canonical Endpoint Group Registry` is the single limit enumeration with canonical keys; `general.api`/`unauthenticated.request` fallback buckets; concurrency-cap rows enforced store-side by named owners; step-10-style census replaced by the §Endpoint Wiring Ownership table; the later-landing wiring obligations are owned by their own surfaces as of 2026-08-25 (the SSE subscription surface, Plan-014 T14.14 / T14.15), and the three sentinels that had tracked them — BL-120, BL-144, BL-146 — closed into those tasks in the same swap, joining BL-145 (the dormant-row sentinel, withdrawn 2026-08-25); every dormant row's gate is a `Spec-021 §ADR Triggers` condition or a named V1.1 surface, never a backlog item. |
...
| D-021-8 | Canonical metric family set (snake spelling per Spec-027; labels per this plan): `rate_limit_trip_total{endpoint,tier}`, `rate_limit_block_total{window_size}`, `rate_limit_backend_error_total{backend}`, `rate_limit_failclosed_total{backend}`, `admin_ban_total{action}`. Plan-020's daemon-side `rate_limit_trip_total{bucket}` registry row removed (daemon has no enforcer); Spec-027 rows 9a/9b reconciled; the self-host export list collapsed to the canonical set. |
...
| D-021-10 | Admin transport = raw HTTP routes pattern-matched before tRPC dispatch on the host (hosted Workers fetch; self-host Fastify on the relay node). Spec-021 route shape satisfied verbatim; ADR-014 narrow-REST-facade posture recorded. |
...
| D-021-14 | Typed identity pair from `identityKeyFn`; canonical forms (user UUID; IPv4 exact / IPv6 /64; the token-hash convention; session UUID; reserved `'user'` arm pinned at V1.1 activation); IP provenance = `CF-Connecting-IP` (hosted) / `X-Forwarded-For` under explicit trust-proxy (self-host relay node); missing IP on anonymous endpoint → 400. |
| D-021-15 | Hosted metrics exposition = explicit V1 gap: hosted counters degrade to structured-log emission; self-host exposition = the relay node's `GET /metrics`; Plan-021 ships registration + emission only (injectable registry). |
...
- [ ] **T21.1-1 — `packages/contracts/src/rate-limiter.ts`.** Author `RateLimitIdentityType` (5 values incl. the reserved dormant `'user'` arm, D-021-17), `RateLimitTier`, `RateLimitEndpointGroup` (registry-key union, D-021-6), `RateLimitCheckRequest` (5-field), `RateLimitCheckResponse` (two-arm union — window arm: 4-field + `blockUntil?`; fail-open degraded arm: `{allowed: true, degraded: true, graceEndsAt}`, no window fields), `RateLimiter` interface (interface-only, PtyHost precedent), `RateLimitResponse` wire envelope + `RateLimitResponseSchema` (Zod; timing pair `.optional()` with a both-or-neither refinement — concurrency-cap refusals omit `retryAfter`/`resetAt`, sliding-window/escalation refusals carry both, `Spec-021 §Overflow Response`); re-export from `packages/contracts/src/index.ts`.
  - **Spec coverage:** Spec-021 §Interfaces And Contracts (RateLimitCheck five-field shape), Spec-021 §Deployment-Aware Abstraction (same programmatic interface both backends), Spec-021 §Implementation Notes (single RateLimiter interface), Spec-021 §Rate Limit Tiers (standard RateLimitResponse envelope)
...
- [ ] **T21.1-2 — `packages/contracts/src/admin-bans.ts`.** Author `AdminBan`, `AdminBansStore` (typed signatures: `issue` with 23505 → already-exists semantics; non-idempotent `revoke`; paginated `list({ activeOnly = true, limit = 100, cursor })`; indexed `findActive`), and the `AdminBanCreateRequest`/`AdminBanCreateResponse`/`AdminBanListResponse` wire pairs + Zod schemas; reuse `RateLimitIdentityType` (no `BanIdentityType`); `issuedBy` server-derived, never body-supplied; index.ts re-export.
  - **Spec coverage:** Spec-021 §Interfaces And Contracts (admin API POST + GET + DELETE shapes), Spec-021 §Escalation (permanent bans exclusively via admin API; admin-ban expiry semantics), Spec-021 §State And Data Implications (durable ban shape)
...
- [ ] **T21.1-4 — api-payload-contracts.md parity verification.** The Tier-6 audit pre-propagated the reconciliation (5-field `interface RateLimitResponse` (timing pair optional) under §Error Responses; 5-field `RateLimitCheckRequest` / two-arm `RateLimitCheckResponse` (window arm: 4-field + `blockUntil?`; degraded arm: `{allowed: true, degraded: true, graceEndsAt}`) under §GDPR And Rate Limiting; AdminBan\* payloads under §Admin Bans API; `"admin"`/`"ratelimit"` in the illustrative `ErrorNamespace` union; §Relay Rate-Limit Signalling frame + close codes `4029` (block) / `4003` (ban) per D-021-9). This task verifies, field-for-field, that each documented shape matches the T21.1-1/T21.1-2 typed exports, and lands ONLY drift fixes (any divergence that crept in between audit-merge and implementation). No-drift outcome = no doc edit; record the parity check in the PR description.
  - **Spec coverage:** Spec-021 §Interfaces And Contracts (api-payload-contracts holds the typed request/response schemas), Spec-021 §Rate Limit Tiers (standard envelope), Spec-021 §WebSocket Overflow Response (WS overflow signalling shape)
  - **Verifies invariant:** I-021-7
...
- [ ] **T21.1-5 — error-contracts.md parity verification.** The Tier-6 audit pre-propagated the code tables: `### Admin` (`admin.forbidden` 403 — operator-token mismatch on the admin-bans surface per D-021-1; `admin.ban_not_found` 404; `admin.ban_already_exists` 409 — losing side of the one-active-ban race, Postgres 23505) and the §Rate Limiting enforcement-layer rows (`ratelimit.banned` 403 — active admin ban matches the identity; `ratelimit.backend_unavailable` 503 — fail-closed after grace). This task verifies every code the implementation emits resolves to a registered row (the admin-route 401 path maps to the EXISTING `auth.token_invalid` row — no new auth code) and lands ONLY drift fixes. No-drift outcome = no doc edit; record the parity check in the PR description.
  - **Spec coverage:** Spec-021 §Interfaces And Contracts (error-contracts holds error response schemas and error codes), Spec-021 §Example Flows (banned identity receives 403)
...
- [ ] **T21.1-6 — Contracts schema tests (`packages/contracts/src/__tests__/`).** AdminBan\* request/response Zod schemas round-trip the documented example payloads; `RateLimitResponseSchema` parses the 5-field envelope and rejects the legacy 4-field shape and any half-timed envelope (`retryAfter` without `resetAt`, and vice versa — the both-or-neither refinement); `RateLimitEndpointGroup` union matches the registry key census (snapshot of the 18 keys).
  - **Spec coverage:** Spec-021 §Overflow Response (canonical envelope), Spec-021 §Interfaces And Contracts (check shape)
  - **Verifies invariant:** I-021-7
...
- [ ] **T21.2-1 — `endpoint-limits.ts`.** Canonical endpoint-group → `{ limit, periodSeconds, tierEligible, enforcementClass, namespaceId }` table transcribed from the Spec-021 registry; exported as the single config source for both backends, wrangler authoring, and header values. Unit test parses the repo's wrangler.toml (when present) and asserts every `[[ratelimits]]` binding's `simple = { limit, period }` + `namespace_id` matches this module (drift guard).
  - **Spec coverage:** Spec-021 §Canonical Endpoint Group Registry (registry table header — single enumeration), Spec-021 §Deployment-Aware Abstraction (identical limits)
...
- [ ] **T21.2-2 — `escalation/escalation-store.ts`.** Author the internal `EscalationStore` + `EscalationDecision` contract per §API And Transport Changes (atomic `recordViolation` ladder evaluation; `getActiveBlock` hot-path read).
  - **Spec coverage:** Spec-021 §Escalation (3/5-min → 15-min block ladder; 10/1-hr → 1-hr block ladder; block keying on (identity, identityType) + full-window enforcement)
...
- [ ] **T21.2-3 — `escalation/postgres-escalation-store.ts`.** Append-and-prune upsert on `rate_limit_escalations.violation_timestamps` per violation inside `recordViolation` (single statement or transaction — atomic ladder evaluation: prune to the 1-hr horizon, then array-filter both windows exactly per §Data And Storage Changes); `getActiveBlock` indexed read; `sweepExpired()` bulk-DELETE of fully-expired rows (`GREATEST(max(violation_timestamps) + interval '1 hour', COALESCE(active_block_until, '-infinity')) < now()`) — the self-host mirror of the DO self-eviction alarm (D-021-13 ephemerality; live cardinality self-bounds to recently-violating identities, so the periodic full-scan DELETE needs no companion column or index). Sweep test: a fully-expired row is deleted; a recent-violation or active-block row survives.
  - **Spec coverage:** Spec-021 §State And Data Implications (escalation state persisted for the window), Spec-021 §Escalation (15-min block; 1-hr block)
...
- [ ] **T21.2-7 — `factory.ts`.** `createRateLimiterFactory(config)` with the discriminated config (`{ kind: 'workers'; env } | { kind: 'node'; pool }`), returning `{ forEndpoint(endpoint: RateLimitEndpointGroup): RateLimiter; mode: 'enforce' | 'observe' }`. Backend resolution: `cloudflare` / `postgres`; ABSENT or unknown `AIS_RATELIMIT_BACKEND` throws at startup naming the env var (no implicit default — self-host declares via `.env`, hosted via wrangler `[vars]`). `AIS_RATELIMIT_MODE` read once here (D-021-16; absent → `enforce`). On Workers, values come from the injected `env`, never `process.env`. Table-driven tests: `cloudflare` → CF registry; `postgres` → PG registry; `undefined`/`''`/`'redis'` → throw; `observe` mode surfaces on the factory.
  - **Spec coverage:** Spec-021 §Deployment-Aware Abstraction (swap via deployment configuration), Spec-021 §Implementation Notes (configuration selects the backend at startup)
  - **Verifies invariant:** I-021-2
...
- [ ] **T21.2-8 — `fail-open.ts`.** Grace wrapper per A-021-18/A-021-19: try/catch around `check()`; on backend error start the grace window (`AIS_RATELIMIT_FAILOPEN_SECONDS`, default 60; monotonic clock); during grace return the degraded union arm `{ allowed: true, degraded: true, graceEndsAt: <now + grace_remaining> }` (the arm carries no window fields — nothing is sentinel-fabricated) + structured warn log `{ backend, error, grace_remaining_ms }`; consumers suppress all `X-RateLimit-*` headers when `degraded` is set; after grace, throw → middleware maps to 503 `ratelimit.backend_unavailable`, increments `rate_limit_failclosed_total{backend}` (emission point lands with T21.4-1), and emits ONE structured error log at the grace-expiry transition. Grace scope = per wrapper instance (per process self-host / per isolate hosted) — the per-isolate deviation is ACCEPTED for V1 and recorded in §Risks. Unit rows: allow-for-grace then throw; degraded → zero headers; transition log exactly once.
  - **Spec coverage:** Spec-021 §Fallback Behavior (bounded-grace fail-open + warning log; fail-closed 503 after grace), Spec-021 §Default Behavior (degraded header suppression)
...
- [ ] **T21.2-9 — `rate-limiter-contract-suite.ts` + runners.** Export `describeRateLimiterContract(makeLimiter: () => Promise<RateLimiter>)` — scenario set: under-limit allow; at-limit deny; header-source fields present + internally consistent; window expiry re-allow; per-endpoint-group isolation; per-tier variant selection (elevated 3×, D-021-4); **block-window behavioral rows (I-021-3):** after the 3/5-min ladder trips, advance fake time past the counter window (capacity restored) but within the 15-min block → next check REJECTED with `resetAt = blockUntil` and `blockUntil` set (the trip-vs-block discriminator); advance past expiry → allowed; same for the 10/1-hr ladder asserting `escalatedTo1h` fires exactly once. Runners: `postgres-rate-limiter.contract.test.ts` (PGlite/testcontainer per harness note) and `cloudflare-rate-limiter.contract.test.ts` (hand-rolled `env.<LIMITER>.limit()` fake honoring `endpoint-limits.ts` + DO-storage fake — `@cloudflare/workers-types` is types-only; fidelity caveat recorded: local emulation does not reproduce production per-edge-location counting; I-021-2 parity is asserted at the contract level, not edge-distribution level).
  - **Spec coverage:** Spec-021 §Deployment-Aware Abstraction (identical limits — the parity proof), Spec-021 AC3 (behavioral half), Spec-021 AC4 (behavioral half), Spec-021 AC6, Spec-021 AC8 (elevated)
  - **Verifies invariant:** I-021-2, I-021-3
  - **Consumes:** all Phase-2 tasks; the self-host relay node re-runs this suite (CP-021-3).
...
**Precondition:** Phase 2 shipped. CP-021-5 (hosted frame seam) gates only the hosted half of T21.3-4's consumption — the middleware module itself + self-host path are not gated. D-021-1 operator token available as injected config.
...
- [ ] **T21.3-1 — `admin/bans-store.ts`.** Postgres `AdminBansStore`: `issue` (insert; 23505 conflict with an active ban → typed already-exists error → 409; conflict with an expired non-revoked ban → atomic revoke-then-insert supersession per D-021-12), `revoke` (set revoked_at/revoked_by; missing/already-revoked → not-found), `list` (paginated, `activeOnly` default), `findActive` (single indexed lookup via the `idx_admin_bans_one_active` partial index; expiry filtered in the query — `now()` cannot appear in an index predicate). Unit tests: issue → findActive returns ban; revoke → null; expired auto-filtered; issue over expired ban supersedes (old row revoked, new row active); 23505 mapping (I-021-6); pagination cursor.
  - **Spec coverage:** Spec-021 §Escalation (admin-API-exclusive ban management), Spec-021 §State And Data Implications (durable bans), Spec-021 §Interfaces And Contracts (route-backing store shapes)
  - **Verifies invariant:** I-021-6
  - **Consumes:** `AdminBansStore` contract ← T21.1-2; `admin_bans` ← T21.1-3; pg client seam (injected; hosted production = the Tier-5 Hyperdrive-backed Querier substrate, self-host = the relay node's pool).
...
- [ ] **T21.3-5 — `admin/bans-routes.ts`.** Raw HTTP routes (D-021-10) registered ahead of tRPC dispatch in the host fetch handler: `POST /admin/bans` (validate via `AdminBanCreateRequestSchema`; 201), `GET /admin/bans` (pagination), `DELETE /admin/bans/:id` (204/404). Operator-token auth per D-021-1 (constant-time compare; absent/malformed → 401 `auth.token_invalid`; mismatch → 403 `admin.forbidden`). On issue/revoke: ban-cache write-through invalidation (call T21.3-2's `invalidateBanCache` before returning) + `admin_ban_total{action}` emission point (lands with T21.4-1).
  - **Spec coverage:** Spec-021 §Interfaces And Contracts (admin API POST + GET + DELETE routes), Spec-021 §Escalation (bans managed exclusively via admin API), Spec-021 §Example Flows (ban example flow)
  - **Verifies invariant:** I-021-6
  - **Consumes:** `AdminBansStore` ← T21.3-1; `invalidateBanCache` ← T21.3-2; wire schemas ← T21.1-2; operator token (injected config per D-021-1; self-host provider = Spec-027 Row 3, path `./data/admin-token` (BL-135); hosted = `AIS_ADMIN_TOKEN` secret).
- [ ] **T21.3-6 — Wire shipped procedures + daemon-exclusion test.** Apply `rateLimitProcedure` per the §Endpoint Wiring Ownership table to the procedures shipped at Tier 6: `session.create`, `event.query`, `health.check`, the `general.api`/`unauthenticated.request` fallback buckets, and `auth.endpoint` over the shipped auth procedures — inside the CP-021-1 mount seam (export-only host edit). Bind `wsRateLimit` into the CP-021-5 frame seam for `ws.message` relay-frame admission (`endpointFor` maps the **forwardable pairwise-ciphertext-envelope** category — the one `RelayFrameType` category that transits the opaque-forward seam — → `ws.message`, [`Spec-031`](../specs/031-remote-control.md); broker-handled control frames are dispatched on the relay's broker path, not metered here). `presence.heartbeat` is **neither** a tRPC procedure **nor** a relay frame — presence heartbeats ride the WebSocket (JSON-RPC 2.0) control-plane channel ([`Spec-031`](../specs/031-remote-control.md)), piggybacking the existing subscription connection with no dedicated endpoint, so nothing wraps via `rateLimitProcedure` and no relay `type` byte meters it; its 10/min row (`Spec-021 §Canonical Endpoint Group Registry`) stays priced/reserved as no JSON-RPC-WS message-admission surface ships in V1 (§Endpoint Wiring Ownership `presence.heartbeat` row). Author `packages/runtime-daemon/src/__tests__/no-rate-limit-import.test.ts`: glob `packages/runtime-daemon/src/**/*.ts` (excluding the test file itself, which now lives inside that glob) and assert zero import specifiers matching `control-plane/src/middleware`, `control-plane/src/rate-limit`, or the `rateLimitProcedure`/`wsRateLimit` symbols (type-only `packages/contracts` imports permitted).
  - **Spec coverage:** Spec-021 AC1 (general.api), Spec-021 AC2 (auth.endpoint), Spec-021 AC7 (daemon exclusion), Spec-021 §Scope (daemon scope exclusion), Spec-021 §Canonical Endpoint Group Registry (presence.heartbeat — priced/reserved, not wired in V1: a JSON-RPC-WS control-channel message, neither tRPC nor relay frame)
  - **Verifies invariant:** I-021-1 (daemon-exclusion companion)
  - **Consumes:** `rateLimitProcedure` ← T21.3-3; `wsRateLimit` + `endpointFor` ← T21.3-4; the CP-021-5 seam ← Plan-031's relay; the shipped control-plane procedure surfaces (Tier 5); the NOT-wired groups that have a V1 wiring surface are owned by their own surfaces and are not satisfied here — the SSE subscription surface (`event.subscribe`), Plan-014 T14.14 / T14.15 (the six `artifact.*` relay rows); `artifact.publish`, `approval.resolve`, `presence.heartbeat`, and `keypackage.upload` have no V1 wiring surface and stay dormant per the `Spec-021 §Canonical Endpoint Group Registry` dormant-row semantics.
...
- [ ] **T21.4-1 — `rate-limit/metrics.ts`.** `RateLimitMetrics` class wrapping an injected `prom-client` `Registry`; register the five canonical families (D-021-8); emission map: trip → the T21.3-2 pipeline `onTrip` seam (fires in observe + enforce — the middleware/WS deny paths go dark in observe and MUST NOT own this counter); block → `escalatedTo1h` decisions consumed inside both limiters' `check()` (the seam holding the request `endpoint`; hosted DO-side emission cannot reach the Worker registry); failclosed → post-grace 503 path; backend_error → fail-open catch; admin_ban → store issue/revoke. Emission-time label-allow-list guard throws on out-of-list values (Plan-020 invariants). Add `prom-client` to `packages/control-plane/package.json`. Hosted disposition: no exposition — counters degrade to structured-log emission of the same bounded fields (D-021-15); self-host exposition = the relay node's `GET /metrics` (CP-021-3). Unit tests: each emission point increments exactly its family; out-of-allow-list label throws; total series across families < 50 (I-021-8).
  - **Spec coverage:** Spec-027 §Required Behavior (row 9b relay family set — Plan-021's counters); Spec-021 §Escalation (ops-alert telemetry pair)
  - **Verifies invariant:** I-021-8
  - **Consumes:** `onTrip` + `escalatedTo1h` seams ← Phases 2-3; `Plan-020 §Prometheus /metrics Exposition (Spec-027 row 9)` label invariants (doc contract, CP-021-4); `prom-client` (npm).
- [ ] **T21.4-2 — Escalation alert telemetry.** On `escalatedTo1h: true` from `recordViolation` — observed inside BOTH limiters' `check()` composition, the seam that still holds the request's `endpoint` (the stores never receive it, and hosted DO-side emission cannot reach the Worker registry): increment `rate_limit_block_total{window_size="1h"}` + emit one structured warning log with bounded fields `{ identity_type, endpoint, window_size: "1h", block_until }` (no raw identity — PII-free; exactly-once per crossing rides the store's atomic ladder evaluation). Integration row: ten violations in 1 hr → `active_block_until ≈ now+1h` AND the counter incremented by exactly 1 AND one warning log with the bounded field set.
  - **Spec coverage:** Spec-021 §Escalation (1-hr block + ops alert; ops-alert definition), Spec-021 AC4
  - **Verifies invariant:** I-021-8
  - **Consumes:** `escalatedTo1h` ← T21.2-3/T21.2-4 (surfaced through the T21.2-5/T21.2-6 limiters); `RateLimitMetrics` ← T21.4-1.
- [ ] **T21.4-3 — AC-anchored integration verification (`packages/control-plane/integration/`).** Named rows, all metric assertions via in-process registry reads (D-021-15 — no scrape exists at Tier 6): (1) `event.query` hammer 61× in 60s → 61st = 429 with `X-RateLimit-Limit: 60`, `Retry-After` per formula; (2) AC-1: 101st `general.api` request in 60s → 429 + all four headers; (3) AC-2: 21st `auth.endpoint` request from one IP → 429 + `Retry-After`; (4) AC-3/AC-4 ladder rows incl. blocked-while-capacity (I-021-3) via the contract suite; (5) AC-5 exclusivity negative: ≥3 consecutive 10/1-hr escalation cycles → zero `admin_bans` rows + ladder ceiling ≤ 1h; (6) admin flow: 201 issue → 403 `ratelimit.banned` on next request from the banned identity (no counter consumed, I-021-1) → 204 revoke → allowed; 409 race row; (7) WS: 61 frames in 60s → 61st refused with in-band frame + connection alive; post-block frame → close 4029 (D-021-9); (8) grace-expiry → 503 + `rate_limit_failclosed_total` increment + one transition log; (9) per-scenario counter deltas (trip/block/admin_ban/backend_error families).
  - **Spec coverage:** Spec-021 AC1, Spec-021 AC2, Spec-021 AC3, Spec-021 AC4, Spec-021 AC5, Spec-021 AC7 (via T21.3-6's structural test), Spec-021 §Example Flows (61-message example; ban example)
...
- **Contract parity suite (`rate-limiter-contract-suite.ts`):** the I-021-2 proof, run against both backends in CI and re-run by the self-host relay node (T21.2-9; CP-021-3).
...
3. Land Phase 3 wiring (tRPC + admin routes; WS hosted half gated on CP-021-5, self-host rides the relay node).
4. Land Phase 4 telemetry + verification suite.
5. Pre-enable soak (D-021-16; no staging environment exists — ADR-023's `environment: production` is a release gate, and the host's dev-environment allow-list refuses `'staging'`): deploy with `AIS_RATELIMIT_MODE=observe` for 24h. Monitor `rate_limit_trip_total`, `rate_limit_block_total`, `admin_ban_total`, `rate_limit_backend_error_total`, `rate_limit_failclosed_total`. Gate: false-positive rate < 0.1%, measured as would-be-429s issued to identities whose structured-log request rate never exceeded the configured limit for that endpoint group ÷ total would-be-429s (plan-local target).
...
- **Eager-DO round-trip cost.** Every hosted rate check consults the DO (block state + authoritative `remaining`/`resetAt`). This doubles the CF rate-limit check cost (~1-5ms DO lookup added to the free binding). For the hot path (60/min messages), 2ms × 60 = 120ms/min added compute per user per minute. Accepted for V1 (D-021-3 — the lazy alternative breaks block enforcement and header accuracy).
...
- [ ] `rateLimitProcedure` is wired on every endpoint group the §Endpoint Wiring Ownership table assigns to this plan; the groups it assigns elsewhere are owned by their own surfaces (the SSE subscription surface, Plan-014 T14.14 / T14.15), and the dormant rows are covered by the dormant-row item below.
- [ ] The CP-021-6 concurrency-cap surface is exported for the store-side cap owner and not left module-private: the `endpoint-limits.ts` cap entries are readable, the canonical 429 builder and its concurrency-cap header subset are importable outside the tRPC middleware, and `rate_limit_trip_total{endpoint,tier}` accepts an increment from a caller that is not `rateLimitProcedure` — verified by a test that builds a cap refusal through the exported surface and asserts it byte-matches the window refusal minus the timing pair.
- [ ] `checkCapAdmission` + `recordCapViolation` are exported from the admission pipeline and exercised by a test proving a cap-row admission consumes **no** sliding-window bucket (counter unmoved across a cap refusal) while a banned identity is still refused `ratelimit.banned` and a genuine cap refusal appends exactly one violation to the escalation ladder (CP-021-6(d), 2026-08-25).
- [ ] WS per-frame admission implements drop-frame semantics: in-band `rate_limited` frame on counter trip (connection stays open); close `4029` only on active block, `4003` on ban; hosted consumption via CP-021-5, self-host via the relay node.
...
- [ ] Canonical metric families `rate_limit_trip_total{endpoint,tier}`, `rate_limit_block_total{window_size}`, `rate_limit_backend_error_total{backend}`, `rate_limit_failclosed_total{backend}`, `admin_ban_total{action}` are registered + emitted with the label guard (series < 50); self-host exposition rides the relay node; hosted degrades to structured logs (D-021-15).
...
- [ ] The dormant registry rows are not wired and no V1 surface has armed them — `approval.resolve` gated on the two `Spec-021 §ADR Triggers` conditions that could make it network-reachable (the topology trigger; the delegated/remote `approval.resolve` surface trigger), **`artifact.publish` on the network-reachable-publication trigger** (added 2026-08-25), `presence.heartbeat` on the V1.1 JSON-RPC-WS message-admission surface (`Spec-021 §Canonical Endpoint Group Registry` dormant-row semantics).
- [ ] Postgres TLS posture documented on the injected pool: `sslmode=verify-full` enforced at the relay node's config-parse locus per [Spec-027 row 5](../specs/027-self-host-secure-defaults.md#required-behavior) ([CVE-2024-10977](https://www.postgresql.org/support/security/CVE-2024-10977/), fixed in PG 17.1/16.5/15.9/14.14/13.17/12.21); the contract suite exercises the refusal-table assertion.

## Tier Placement

Tier 6 per cross-plan-dependencies.md. Strictly **downstream of the control-plane host** for the CP-021-1 tRPC mount, and downstream of [Plan-031](./031-remote-control.md)'s relay for the CP-021-5 frame seam — with the **SSE subscription surface's concurrency cap the one consumer that lands after this plan** (2026-08-25) and **upstream of the self-host relay node** (which instantiates `PostgresRateLimiter` + `wsRateLimit` + the admin routes inside its compose-deployed process, CP-021-3). Additionally: (a) upstream of the `event.subscribe` cap owner, which consumes the cap surface per CP-021-6 — deliberately deferred past this plan, so the edge stays acyclic rather than inverting this plan's own host dependency; (b) registers its metric families against Plan-020's §Prometheus /metrics Exposition contract (Tier 8 — doc-contract compliance only, no code consumed; CP-021-4) with self-host exposition mounted on the relay node.
