...
- [ ] The dormant registry rows are not wired and no V1 surface has armed them — `approval.resolve` gated on the two `Spec-021 §ADR Triggers` conditions that could make it network-reachable (the topology trigger; the delegated/remote `approval.resolve` surface trigger), **`artifact.publish` on the network-reachable-publication trigger** (added 2026-08-25, §6 node NS-80), `presence.heartbeat` on the V1.1 JSON-RPC-WS message-admission surface (`Spec-021 §Canonical Endpoint Group Registry` dormant-row semantics).
