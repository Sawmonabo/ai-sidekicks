...
- `packages/runtime-daemon/src/bootstrap/index.ts` (owned by Plan-007 — the documented wiring point, `Plan-007 §Daemon IPC + bootstrap (packages/runtime-daemon/src)`'s EXTEND row: owned-namespace `register*` calls wire here after `SecureDefaults.load`) — five `registerOnboarding*(registry, deps)` calls, per the Plan-002 `presence.*` precedent: the shipped `session.*`/`presence.*` binders under `src/ipc/handlers/` are the on-disk pattern.
