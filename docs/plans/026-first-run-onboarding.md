...
- **T7.3 — Desktop provider step in the existing walkthrough.**
  - **Files:** `apps/desktop/src/renderer/src/onboarding/ProviderStep.tsx` (CREATE), `apps/desktop/src/main/onboarding/walkthrough-host.ts` (EXTEND — fetch the projection and pass it in), `apps/desktop/src/main/onboarding/menu-entry.ts` (CREATE — the _Set up providers_ activation, alongside the existing _Set up collaboration_ entry)
  ...
  - **Tests:** `apps/desktop/e2e/onboarding.spec.ts` (EXTEND) — the step renders each readiness arm from a fixture projection; the renderer bundle contains no `providerAccount.*` call and no readiness computation; the _Set up providers_ entry activates Group B alone; and no rendered surface offers a token input.
...
- [ ] The I-026-1…I-026-13 invariant set holds — every task-declared `Verifies invariant:` assertion is backed by a green test or a recorded verification note, and every §Cross-Plan Obligation (CP-026-1…CP-026-5) is either satisfied with a return-cite from its provider or consumer plan or explicitly staged.
