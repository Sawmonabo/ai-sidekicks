...
- **Self-hosted operator first-run.** `docker-compose up` for the relay is a separate operator-facing flow owned by [Plan-031](./031-remote-control.md). This plan is the _client_-side onboarding.
...
- **Prompting on initial install, first daemon start, or first local session.** Single-user local-daemon mode reaches a working session without ever hitting this flow. Implementations that prompt ahead of the explicit activation trigger are a review rejection (`Spec-026 §Pitfalls To Avoid`). This holds for both step groups: the provider step is keyed to an account-plane refusal that has already happened, and a readiness test placed **ahead** of a provider run in order to prompt is the same rejection at a different point in the flow (I-026-11).
...
- [ ] [Plan-031](./031-remote-control.md) exposes the self-hostable relay's `GET /readyz` endpoint with a TLS-terminated HTTPS listener; Option 2's TOFU probe needs a reachable HTTPS certificate chain to pin.
...
- [ ] [Plan-031](./031-remote-control.md) serves the project-operated hosted relay so the hosted-sign-up redirect URL is a real, served URL. Without that deployment, Option 3 can be code-complete but not end-to-end testable.
...
- `apps/cli/src/exit-codes.ts` (owned by Plan-007 — Phase R3, T-007r-3-3) — **one call** into this plan's `account-plane-refusal-hint.ts` from the `error.data.type` discrimination that file already performs, so a run refused on the account plane routes the operator into Group B (T7.2). **No exit code is added, removed, or remapped** and the mapping stays total per I-007-14: the hint prints beside the existing rendering and changes no exit path. If the edit grows past the call, it is a review rejection — the recognition logic belongs in this plan's own file.
...
interface OnboardingSubmitChoiceRequest {
  ...
  deferred_validation?: boolean; // set true when offline at first-choice time per Spec-026 §Fallback Behavior
}
...
**Resolution.** Live. Plan-026 joins that extender list with four registrations and no other edit to the entry point; this plan's §Touched But Not Owned entry is the durable record. Without those registrations the plan promises a working `sidekicks onboarding start` and ships no path by which the binary reaches it. **Direction:** Extend [Plan-007](./007-local-ipc-and-daemon-control.md)'s `apps/cli/src/main.ts`. **Tasks:** T4.2.
...
| `onboarding.spki_mismatch` | 412 | Subsequent connection's SPKI differs from pinned value. Registered here, raised by the relay plan — see **CP-026-5**, the obligation row that carries this crossing; the relay-side event registration is explicitly deferred by `Spec-026 §Fallback Behavior`, so the return-cite is owed at that plan's own dispatch, not at this audit. |
...
**Precondition:** Phase 1 merged. Plan-023 has landed the keystore surface and the `safeStorage` backend probe (T2.4's desktop path); [Plan-031](./031-remote-control.md) exposes a TLS-terminated `GET /readyz` so T2.1's integration test has a real certificate chain to pin. Implementation Steps 4-7. These four modules are mutually independent and can be built in parallel once contracts land.
...
- **T3.4 — Contract-doc reconciliation: onboarding payloads and error codes.**
  ...
  - Doc-only reconciliation of the shipped shapes. `onboarding.spki_mismatch` is registered here and raised by the relay plan, never by this plan's service — the crossing is carried by CP-026-5, and the relay-side event registration is deferred by `Spec-026 §Fallback Behavior`, so the return-cite is owed at that plan's dispatch rather than here.
...
- CLI Option 2 against a test HTTPS self-host relay (testcontainer running the Plan-031 Node relay): paste admin token → verify SPKI pin in `[onboarding]`; verify admin token in keystore (mock keyring backend for CI). Subsequent connect with cert rotation → assert `onboarding.spki_mismatch` error code surfaces.
...
- Desktop E2E via Playwright `_electron` (per Plan-023's test harness): launch packaged app → activate _Set up relay_ → walkthrough appears → select Option 1 → verify the preload bridge's `presentChoice()` promise resolves → verify the daemon JSON-RPC `onboarding.submitChoice` was called.
...
Tier 9. Plan-026 is **strictly downstream** of:
...
- [Plan-031](./031-remote-control.md) (the self-hostable relay — Option 2's TOFU probe target; the hosted relay / control-plane serving Option 3's sign-up redirect endpoint, when deployed).
