...
- [ ] Plan-029 ships the node-local provider-account registry and its per-provider readiness projection (Phase 2). The provider-authentication step group consumes both and re-derives neither (CP-026-6). Enforcement is mechanical, not by tick: Phase 7's `external_plan_phase_merged` entry resolves this against Plan-029's own shipment manifest, so this box is a legibility record rather than a second gate — Phases 1-6 are unaffected by it.
...
**Precondition:** Phase 4 and Phase 5 merged (Group B ships on both surfaces, the same gate Phase 6 takes), **and** Plan-029 Phase 2 merged — the registry and readiness projection this phase consumes. Implementation Steps 23-25.
...
- **T7.1 — Group-B step model and trigger discipline in the daemon service.**
  - **Files:** `packages/runtime-daemon/src/onboarding/provider-step.ts` (CREATE), `packages/runtime-daemon/src/onboarding/service.ts` (EXTEND — the Group-B entry points)
  - The step model: read the per-provider readiness projection from Plan-029's `providerAccount.list` reply through the client SDK and pass it through **unmodified**; apply the three-trigger discipline, including the account-plane-refusal trigger; and expose per-provider register / sign-in-remedy / re-check / skip actions that delegate to `providerAccount.*`. Report a provider as set up only on the authenticated arm. Persist nothing and emit nothing.
  - **Tests:** `packages/runtime-daemon/src/onboarding/__tests__/provider-step.test.ts` (CREATE) — each of the three triggers fires and a fourth candidate (session creation, first launch, health check, and a pre-run readiness test) does not; a readiness projection is rendered byte-identically to the daemon's reply with no local recomputation; a re-check calls the probe exactly once and display alone calls it zero times; every non-authenticated arm reports not-set-up, `indeterminate` included; and no config, partial-state, keystore, or event write occurs on any path.
  - **Acceptance:** the step neither fires ahead of an admissible run nor computes a readiness value of its own.
  - **Spec coverage:** Spec-026 §Provider Authentication (Group B), Spec-026 §Default Behavior
  - **Verifies invariant:** I-026-11, I-026-13
  - **Consumes:** the readiness projection and the registry verbs ← Plan-029 T2.4, T2.5 (CP-026-6); the service machine ← T3.1

- **T7.2 — CLI provider step and readiness in `onboarding status`.**
  - **Files:** `apps/cli/src/commands/onboarding/start.ts` (EXTEND — the `--providers` flag), `apps/cli/src/commands/onboarding/status.ts` (EXTEND — the readiness block), `apps/cli/src/commands/onboarding/provider-step.ts` (CREATE — the interactive per-provider prompts)
  - `--providers` runs Group B alone; `status` prints per-provider readiness and, for any provider that is not ready, the remedy the daemon disclosed. Headless or no-reachable-browser prints readiness plus the out-of-band remedy and exits 2 with `onboarding.headless_required`. No new command file and no new clipanion registration, so `apps/cli/src/main.ts` keeps its four `.register()` calls.
  - **Tests:** `apps/cli/integration/onboarding.test.ts` (EXTEND) — `--providers` on a zero-account node offers registration per provider and leaves `[onboarding]` byte-identical; `status` prints the remedy for a husked home; the headless path exits 2 with the remedy and **no** credential override is accepted from any env var or flag; and a token supplied on stdin is not consumed by any prompt.
  - **Acceptance:** no CLI input path anywhere in this flow accepts provider credential material.
  ...
  - **Consumes:** the step model ← T7.1; the CLI command tree ← T4.1

- **T7.3 — Desktop provider step in the existing walkthrough.**
  - **Files:** `apps/desktop/src/renderer/src/onboarding/ProviderStep.tsx` (CREATE), `apps/desktop/src/main/onboarding/walkthrough-host.ts` (EXTEND — fetch the projection, pass it in, and export the Group-B activation entry point beside the Group-A one it already exposes)
  - The host fetches the projection in the main process and passes it to a view-only renderer step; renderer actions post back through the existing walkthrough channel. **No preload-bridge method is added** and `packages/contracts/src/desktop-bridge.ts` is untouched: Group B has no secret input, so I-026-4 holds by construction here rather than by enforcement.
  - **Tests:** `apps/desktop/e2e/onboarding.spec.ts` (EXTEND) — the step renders each readiness arm from a fixture projection; the renderer bundle contains no `providerAccount.*` call and no readiness computation; the Group-B activation entry point runs Group B alone and leaves Group A untouched; and no rendered surface offers a token input.
  - **Acceptance:** the renderer is a projection of the daemon's readiness and never a second source of it.
  - **Spec coverage:** Spec-026 §Provider Authentication (Group B), Spec-026 §Interfaces And Contracts
  - **Verifies invariant:** I-026-12, I-026-13
  - **Consumes:** the step model ← T7.1; the walkthrough host ← T5.3
...
- **T7.4 — End-to-end: a freshly onboarded node starts its first provider run.**
  ...
  - **Consumes:** T7.1, T7.2, T7.3; a registered, authenticated account ← Plan-029 Phase 3
...
- Steps 23, 24, 25 (the provider-authentication step group) consume steps 8-16 **AND** Plan-029 Phase 2. They are independent of steps 17-20: Group B has no relay dependency, so it neither blocks nor is blocked by the option-wiring work.
...
11. Provider-authentication step group on both surfaces (steps 23-25). Gate on Plan-029 Phase 2 merged.
...
- **No plan authors the desktop application menu — inherited, and named here rather than papered over.** `Spec-026 §Trigger` has required a _Set up collaboration_ menu activation since before this amendment, and `Spec-026 §Interfaces And Contracts` now names _Set up providers_ beside it. Neither label has an owning task: this plan's desktop steps author walkthrough and dialog surfaces only, and [Plan-023](./023-desktop-shell-and-renderer.md) — whose spec names a "platform-appropriate menu bar" — carries no menu module, no menu task, and no menu file anywhere in its body. T7.3 therefore exports a Group-B **activation entry point** from the walkthrough host beside the Group-A one and mints no menu file, because minting one would invent an integration point into a module no plan has committed to build. Consequence, stated plainly: until the menu module has an owner, both labels are reachable only through their CLI equivalents (`sidekicks onboarding start` and `--providers`) and the account-plane-refusal trigger, and Group B's desktop trigger surface is a strict subset of Group A's — the same subset, for the same reason. This amendment inherits the gap and does not widen it; the §Touched But Not Owned set stays at eight entries because no unowned file is touched. Closing it is a Plan-023 ownership question, not a Group-B one.
