...
- Define the internal driver interface for 10 operations (canonical long-form names per [api-payload-contracts.md:655](../architecture/contracts/api-payload-contracts.md) `interface ProviderDriver`): `createSession`, `resumeSession`, `startRun`, `interruptRun`, `applyIntervention`, `respondToRequest`, `closeSession`, `listModels`, `listModes`, `getCapabilities`. **Campaign forward-note (B6, 2026-07-07):** the amended Spec-005 surface adds four operations — `rollbackTo`, `setSessionGoal`, `clearSessionGoal`, `probeAuth` (14 post-campaign) — whose interface members, per-driver implementations, normalizers, and conformance tests land as ADDED Phase-3/Phase-4 tasks via the campaign's Plan-005 bundle; the Plan-016 goal service and the auth/rollback gates consume exactly that bundle (no task here silently absorbs them, and this plan is not completable-as-written for the amended surface until the bundle lands).
...
- **T1.1** — Author the 10-operation `ProviderDriver` interface in `packages/contracts/src/provider-driver.ts`.
  ...
  - Consumes: `SessionId` / `ChannelId` from `packages/contracts/src/session.ts` (Plan-001 Phase 2, Tier 1 — shipped); `PtyHost` contract from `packages/contracts/src/pty-host.ts` (Plan-024 Phase 2, Tier 1 — shipped); typed shape at api-payload-contracts.md:655 (`interface ProviderDriver`)
...
- **T1.2** — Author the 7-flag `DriverCapabilityFlag` literal-union type and `DriverCapabilities` shape in `packages/contracts/src/provider-driver.ts`.
  ...
  - Consumes: api-payload-contracts.md:175 (`type DriverCapabilityFlag` anchor), :948 (`interface DriverCapabilities` anchor)
...
- **T1.4** — Author `ApplyInterventionParams` + Zod-validated `DriverInterventionResultSchema` in `packages/contracts/src/provider-driver.ts`.
  ...
  - Consumes: api-payload-contracts.md:722 (`type ApplyInterventionParams` anchor), :758 (`interface DriverInterventionResult` anchor), :149 (cross-cutting `InterventionType` enum — resolution: co-locate the `InterventionType` re-export in `provider-driver.ts` since Plan-004's `runControl.ts` ships at Tier 5; Plan-004 imports the enum from Plan-005 at Tier 4)
...
- **T1.6** — Author the `DriverResumeResult` discriminated-union return shape for `resumeSession()` in `packages/contracts/src/provider-driver.ts`. Pattern matches existing `DriverInterventionResult` (T1.4): a `status`-discriminated union with explicit success and failure variants, validated via Zod.
  ...
  - Consumes: T1.1 `resumeSession` signature anchor; api-payload-contracts.md:759 `interface DriverInterventionResult` precedent (sibling shape)
...
- The `RuntimeBindingStore` exposes `findResumableBindings(...)` as a queryable seam where Plan-015 may add recovery-aware predicates (e.g., bindings with stale checkpoints, bindings marked recovery-needed via the dedicated `recovery_checkpoints` table at `local-sqlite-schema.md:1128`).
