...
Phase 1's T1.4 code authoring surfaced an internal naming inconsistency in the canonical contract doc: the two sibling driver result-envelope types used different conventions. `DriverResumeResult` (api-payload-contracts.md:779) leads with the `Driver` prefix, but its explicit sibling — the `applyIntervention` envelope at :701 — buried the qualifier in the middle as `InterventionDriverResult`. The doc is the defect; the fix is the doc, and Phase 1 code then mirrors the corrected doc (doc-first preserved).
