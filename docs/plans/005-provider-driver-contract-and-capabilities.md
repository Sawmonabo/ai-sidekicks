...
**Classification: additive (Status Flip Rule rows 1–2 — Plan-005 stays `approved`).** This amendment discharges the earlier campaign forward-notes' promise ("the amended Spec-005 surface's operations, per-driver implementations, normalizers, and conformance tests land as ADDED Phase-3/Phase-4 tasks via the campaign's Plan-005 bundle") as additive `#### Tasks` under Phases 1–4. It introduces **no** new plan invariant, cross-plan obligation, or phase: the R8 driver operations ride the existing `ProviderDriver` contract (additive operations + `applyIntervention` generic dispatch per [ADR-011](../decisions/011-generic-intervention-dispatch.md)), and every R8 event reuses an existing Spec-006 surface (`run_lifecycle` / `session_lifecycle` / `tool_activity` / `usage_telemetry` / `runtime_node_lifecycle`, CP-005-5) — so no Status flip. Governing docs: capability-enhancement campaign [design §4 W2 B10 row + §5.1 parity matrix](../superpowers/specs/2026-07-01-capability-enhancements-design.md), campaign plan Task 14.

**Count-claim re-derivation.** T1.7 / T1.8 widen the ratified ten-op / seven-flag base to the fourteen-op / thirteen-flag post-campaign surface. Whole-interface totals across Phases 1–4 (the Phase-1 Goal, the T1.3 wrapper rationale, the Phase-4 client-exposure reversal note, and the T4.1 / T4.3 spec-coverage lines) are re-derived to the post-campaign total here; each base task's own-deliverable count (T1.1 ten ops, T1.2 seven flags, and the T1.5 conformance test pinning the seven-flag base) stays at the ratified base it ships. Citation/consistency update — additive, no Status flip.

**Scope (counts re-derived from the tasks below).** **Eighteen corpus-item enrichments** — P0-1, P0-2, P0-3, P0-4, P0-5, P1-1, P1-2, P1-4, P1-5, P1-6, P2-1, P2-6, P2-7, P2-9, P2-10-L1, P3-1, P3-3, P3-4. **Ten CLI-currency duties** — C-1, C-4, C-5, C-7, C-8, C-12, C-14, C-16, C-17, and the regenerate-bindings rule (C-2 / C-3 additionally appear below as the P1-4 interactive-request mechanisms, and C-9 as the P0-1 rate-limit wire basis — mechanism citations for those corpus items, not counted among the ten currency duties). **Seven R8 parity driver legs** — rollback, session goals, callback-tool hosting, `subagentPolicy` pass-through, `executionPosture` mapping, the transport axis, and `realtime_*` suppression. Plus the P2-5 `Reference:` lines on T3.5–T3.10 (added above) and the P-2 phantom fix at T4.1 (applied in place — the prior `ipc/router.ts` cite was a file no plan creates). Each is named at the task that owns it.
...
#### Campaign Phase 3 driver enrichment (Codex + Claude)

- **T3.11 — Event-normalizer default-branch diagnostic + reorder buffer + telemetry producers (EXTENDs T3.5 Codex + T3.10 Claude).**
  - Files: `packages/runtime-daemon/src/provider/drivers/codex/event-normalizer.ts` (EXTEND T3.5); `packages/runtime-daemon/src/provider/drivers/claude/event-normalizer.ts` (EXTEND T3.10)
  ...
  - **P0-1 — default-branch diagnostic, never silent-drop.** Both normalizers consume the Plan-006 registry `EVENT_DISPOSITION_BY_KIND` (Plan-006 Phase 1's disposition map in `packages/contracts/src/event.ts`, campaign B11 — the code leg lands with Plan-006's Phase 1) as the single disposition source and mirror its `adopt` / `rename` / `correlate` / `discard` verdicts. Any wire kind that resolves to no registered `SessionEventType` — an unknown kind outside the census **or** an interim `typePending` kind whose literal has not landed — routes to a **typed `runtime_node.*` diagnostic default branch**: an operator-visible structured diagnostic (`{ provider, rawWireType, dispositionReason }`) surfaced on the daemon diagnostic channel, **never a `session_events` envelope and never a silent drop**; the kind moves to a real `eventType` when B18's W3 Spec-006 census amendment lands its literal (the Plan-006 T1.8 interim-disposition seam). The corpus checklist's `rate_limits` → `usage.context_window_update` gloss is **corrected here**: the Codex `rate_limits` account-quota kind maps to `usage.rate_limit_update` (account-quota utilization is not context-window utilization, Design §3.2) — Codex `account/rateLimits/updated` push + `account/rateLimits/read` pull both feed it (C-9). Unknown kinds fail open to this diagnostic; malformed frames fail closed per the boundary rule above; every correlation id copied verbatim.
  - **P2-1 — bounded reorder buffer with a first-class overflow diagnostic.** No global causal-order guarantee across aggregates; per-run order is preserved and tool events pair by `tool_use_id` (§9 BP-9). The normalize boundary carries a bounded reorder buffer parameterized `{ maxBufferedEvents, pairingTimeoutMs }`; on **overflow** it flushes in arrival order **and** emits a **first-class, queryable overflow diagnostic** — a timeline-visible diagnostic event **plus** an OpenTelemetry counter (`driver.reorder_buffer.overflow`). The overflow is never a silent flush: a silent flush degrades to invisible reordering, so the paired event+metric is load-bearing, not advisory. A pairing timeout (an unpaired `tool_use_id` past `pairingTimeoutMs`) surfaces the same first-class diagnostic pair.
  ...
  - **P1-6-producer — 3-tier cost resolution.** The driver sets `costStatus ∈ { priced, unpriced }` and `costSource ∈ { provider_reported, derived_exact, derived_family_prefix }` on `usage.cost_update`: (a) provider-emitted cost when the wire supplies it, **sanity-bounded** (non-negative, finite, below the configured absurdity ceiling; gross divergence from a derivable estimate emits a diagnostic); (b) else daemon-derived from token counts × the local per-model-family pricing table with family-prefix fallback; (c) else **fail-closed unpriced** (`usage.budget_warning { reason: 'unpriced-model' }`). The producer never halts and never branches on `costSource` — the B15 accountant owns the single ceiling (Spec-016 §Budget Policies, B6). The surveyed fail-open zero-cost terminal is deliberately not ported.
  ...
  - **`realtime_*` reserved-family handling.** The reserved `realtime_*` census family (Spec-006 B1) has no emitter in V1 — the normalizer routes any `thread/realtime/*` wire kind to the default-branch diagnostic (it is gated / `typePending`), and the driver suppresses the family at the source via Codex `optOutNotificationMethods` (C-16) until the R8 realtime gate lifts (R8 leg 7, T3.15).
...
- **T3.12 — Capability declaration + CLI-version floor + refresh cadence (EXTENDs T3.3 Codex + T3.8 Claude).**
  - Files: `packages/runtime-daemon/src/provider/drivers/codex/capabilities.ts` (EXTEND T3.3); `packages/runtime-daemon/src/provider/drivers/claude/capabilities.ts` (EXTEND T3.8)
  ...
  - **P2-9 — refresh cadence.** The capability-refresh caller polls per-runtime-node on a 15-minute bounded cadence with optional provider push where available; a refresh emits `runtime_node.capability_updated` (CP-005-5) — no new event type.
...
- **T3.13 — Tool metadata + MCP idempotency floor + MCP server-status census (EXTENDs T3.4 Codex + T3.9 Claude).**
  - Files: `packages/runtime-daemon/src/provider/drivers/codex/tools.ts` (EXTEND T3.4); `packages/runtime-daemon/src/provider/drivers/claude/tools.ts` (EXTEND T3.9)
  - **Spec coverage:** Spec-005 §Tool Metadata (MCP-discovered tool always `manual_reconcile_only`; class never derived from annotations); Spec-005 §Recovery Consequences (the durable-task-handle carve-out)
  - **Verifies invariant:** I-005-3
  - **P2-7 — MCP floor + load-bearing negative test.** An MCP-discovered tool is **always** `manual_reconcile_only`; the runtime MUST NOT derive `idempotency_class` from MCP `ToolAnnotations` (MCP 2025-11-25 binds clients to treat annotations as untrusted unless from trusted servers, MUST-strength — the MUST binds trust classification, not behavior derivation, so classes bind only on the operator-governed assignment surface, forward-ref Spec-028/B18). The **load-bearing negative test** asserts that a driver surfacing an MCP tool advertising `readOnlyHint` / `idempotentHint` still persists `manual_reconcile_only` — annotation self-claims never upgrade the class. **Durable task-handle persistence (Part-B carve-out (a), 2026-07-17):** where an MCP call is dispatched task-augmented under the experimental MCP 2025-11-25 Tasks utility, the receipt durably records the **receiver-generated `taskId` from the `CreateTaskResult` acceptance response** (the id exists only once the receiver accepts, so a crash before that response is durably stored leaves no handle and stays on the floor's halt) — the durable handle Spec-015's recovery dispatch polls via `tasks/get` to terminal status + `tasks/result` retrieval (result retrieval, never re-execution) instead of halting; a purged/expired or `input_required` task falls back to the halt with the handle preserved. This refines recovery dispatch without weakening the floor — the trust rule is untouched.
  - **P2-10-L1 — runtime MCP discovery census + live server-status ingest.** The driver produces a per-session MCP **server** inventory at init (name + status) plus status-change updates, unified into one enum `unknown | starting | connected | needs-auth | failed` — **servers only, never a per-server tool-list assumption** (support is not visibility, Spec-005 §Per-Driver Capability Matrix). Claude leg: the `claude mcp list` zero-billed-turn probe; Codex leg: `mcpServerStatus/list`. This per-session status feed is a driver **census/telemetry surface** (no new persisted table, no new CREATE owner — DO disposition) consumed by Spec-028/B18's status-and-health lane.
  - **C-1 — ephemeral `--mcp-config` as the primary Claude scoping mechanism.** Per-session MCP scoping is delivered by an ephemeral `--mcp-config` relaunch (the documented Claude launch-time mechanism) as the **primary** path; a live `mcp_set_servers` reconcile is docs-silent on current Claude Code / likely absent — verify-first at authoring, adopt only if confirmed.

- **T3.14 — Lifecycle + intervention + resume enrichment (EXTENDs T3.1/T3.2 Codex + T3.6/T3.7 Claude).**
  - Files: `packages/runtime-daemon/src/provider/drivers/codex/{lifecycle,intervention}.ts` (EXTEND T3.1/T3.2); `packages/runtime-daemon/src/provider/drivers/claude/{lifecycle,intervention}.ts` (EXTEND T3.6/T3.7)
  ...
  - **P0-4 — spawn-env hygiene.** The driver strips `CLAUDE_*` / `CLAUDECODE*` from the child environment and supplies config via `--settings`, never `~/.claude`; the Codex leg enforces the allow-listed spawn env natively with `shell_environment_policy` `inherit="none"` + explicit `set` (C-7) — the enforceable form of Plan-012's I-012-20.
  - **P0-5 — zero-turn auth probe.** `probeAuth` detects a logged-out provider before a turn is spent (`DriverAuthProbeResult`, `.strict()`; `driver.not_authenticated`). Codex leg cites the current probe surfaces `codex login status` / legacy `getAuthStatus` / `codex doctor --json` (redacted machine-readable health) (C-17); Claude leg uses its auth-state signal.
  - **P1-1 — intended-close discriminator.** Daemon-initiated `closeSession` stamps the intended-close flag on the terminal payload so a clean shutdown is not misclassified as a crash by the recovery classifier (Spec-006 B1).
  - **P1-2-driver — duplicate-terminal suppression.** The driver never emits a second terminal `run_lifecycle` event for the same `(runId, runVersion)` epoch — the driver-side leg of the at-most-once-terminal property whose primary guard is Plan-004 T3.7 and whose schema backstop is Plan-006's partial unique index (Spec-006 §Run Lifecycle (run_lifecycle)).
  - **P1-4-driver — interactive-request mechanisms (two-provider).** Claude leg: the `--permission-prompt-tool` stdio round-trip (`can_use_tool`: `{ tool_name, input }` → `{ behavior: allow | deny, updatedInput?, message? }`, C-2) + ephemeral `--mcp-config`; Codex leg: native `item/tool/requestUserInput` + `mcpServer/elicitation/request` (C-3). Both surface as the four `driver_ask.*` types (Spec-006 B1) that the B13 T2.8 normalizer routes into the Cedar pipeline (CP-005-7).
  ...
  - **P3-3 — resume-failure taxonomy.** The `failed` resume variant structurally omits `bindingId`; failure surfaces the closed `RecoveryCondition` — `recovery-needed` (generic) or `reauth-required` (mid-run credential/session expiry detected via C-5's `authentication_failed`, or at resume/probe) — routed by Plan-015's recovery dispatcher onto the existing `run.failed` event (no separate driver event, CP-005-5). The driver MUST NOT silently create a replacement session (I-005-5).

- **T3.15 — R8 parity driver legs (Codex + Claude).**
  - Files: `packages/runtime-daemon/src/provider/drivers/codex/{lifecycle,intervention,capabilities}.ts` + `packages/runtime-daemon/src/provider/drivers/claude/{lifecycle,intervention,capabilities}.ts` (EXTEND); driver `__fixtures__/`
  - **Spec coverage:** Spec-005 §Interfaces And Contracts (the seven parity legs); Spec-005 §Parity Capability Mechanism Grades (per-leg mechanism grades); Spec-012 §Required Behavior (execution-posture authorization, B20); Spec-016 §Provider-Native Subagents (single-supervisor invariant, B6)
  ...
  - The **seven R8 parity driver legs**, each recorded at its mechanism grade on the parity matrix (Spec-005 §Parity Capability Mechanism Grades):
    ...
    3. **Callback-tool hosting** — Codex `dynamicTools` + the `item/tool/call` server-request handler (native); Claude a daemon-hosted ephemeral MCP server via `--mcp-config` (native, MCP-shaped; tools surface as `mcp__<server>__<tool>`, the `--permission-prompt-tool` plumbing generalized). Every callback invocation flows through the Plan-012 approval pipeline (Cedar-governed identically to provider tools, B20) and lands as ordinary `tool_activity` rows.
    4. **`subagentPolicy` pass-through** — Claude `--agents` `AgentDefinition`s (per-subagent model/tools/permissionMode/effort/maxTurns; depth cap 5); Codex `[agents]` config. `maxConcurrent` enforces natively on Codex and is **boundary-serialized on Claude** (beyond-cap subagents hold at the daemon tool-call boundary until a slot frees — concurrency cap docs-silent on Claude); subagent lifecycle normalizes via T3.11; budget roll-up is run-scoped (single-supervisor invariant, no escape hatch, Spec-016 B6).
    ...
    6. **Transport axis** — Codex `stdio` default; `unix://` / `ws://` app-server listeners config-gated with bearer auth (`DriverTransportConfig`, credential a daemon-config reference). Claude has no local listener — remote Claude is Spec-024 cross-node dispatch, recorded as the parity mechanism (emulated, architectural).
    7. **`realtime_*` suppression** — the reserved `realtime_*` family is emitter-less in V1; the Codex driver suppresses `thread/realtime/*` via `optOutNotificationMethods` (C-16) until the upstream feature flag stabilizes and the R8 realtime gate lifts. No emulation is claimed (Claude exposes no realtime surface — absent).
  - **C-12 — native cost cap as defense-in-depth.** The Claude driver sets `--max-budget-usd` from the remaining session budget so the CLI cap sits beneath the authoritative BudgetAccountant (at cap the run stops with `error_max_budget_usd`, metadata intact, resumable — normalized into the budget-exhausted taxonomy); Codex `token_budget` is under development at the pin (watched via the driver bundles' execution-time currency pass, no backlog item).

#### Campaign Phase 4 SDK exposure

- **T4.8 — SDK/handler exposure + `RecoveryCondition` consume (EXTENDs T4.1/T4.2/T4.3).**
  ...
  - **Verifies invariant:** I-005-4
  - **P3-4 — `RecoveryCondition` consume (4 surfaces).** The hoisted named `RecoveryCondition` type (`recovery-needed` / `reauth-required`, in [api-payload-contracts.md](../architecture/contracts/api-payload-contracts.md), campaign B3 + PR-F4) is **referenced, never re-inlined**, at each of Plan-005's carrying surfaces: the resume `failed` variant, the `probeAuth` result, the recovery-dispatch return contract (T4.7), and the SDK-seam envelope. The `reauth-required` widening consumes C-5's mid-run `authentication_failed`.
  - **Parity-operation exposure.** No new client-facing `driver.*` verb is minted: the R8 parity operations are daemon-internal, invoked by orchestration on the in-daemon `ProviderRegistry` per §Phase 4 decision #2 (a verb that establishes or mutates session-or-run state stays orchestration-owned so `runtime_bindings` persistence + canonical event emission are never bypassed) — `rollbackTo` is reached through Plan-004's `rollback` intervention (campaign B9) and `setSessionGoal` / `clearSessionGoal` through Plan-016's goal service (campaign B15). `probeAuth` and the widened capability read are daemon-internal introspection surfaced through the existing `driver.listCapabilities` cache. The ratified seven client-facing methods are unchanged.
