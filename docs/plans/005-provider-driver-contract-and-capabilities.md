...
- **T1.1** — Author the 10-operation `ProviderDriver` interface in `packages/contracts/src/provider-driver.ts`.
  ...
  - Consumes: `SessionId` / `ChannelId` from `packages/contracts/src/session.ts` (Plan-001 Phase 2, Tier 1 — shipped); `PtyHost` contract from `packages/contracts/src/pty-host.ts` (Plan-024 Phase 2, Tier 1 — shipped); typed shape at api-payload-contracts.md:583 (`interface ProviderDriver`)
...
- **T1.4** — Author `ApplyInterventionParams` + Zod-validated `DriverInterventionResultSchema` in `packages/contracts/src/provider-driver.ts`.
  ...
  - Consumes: api-payload-contracts.md:618 (`interface ApplyInterventionParams` anchor), :639 (`interface DriverInterventionResult` anchor), :149 (cross-cutting `InterventionType` enum — resolution: co-locate the `InterventionType` re-export in `provider-driver.ts` since Plan-004's `runControl.ts` ships at Tier 5; Plan-004 imports the enum from Plan-005 at Tier 4)
...
- **T1.6** — Author the `DriverResumeResult` discriminated-union return shape for `resumeSession()` in `packages/contracts/src/provider-driver.ts`. Pattern matches existing `DriverInterventionResult` (T1.4): a `status`-discriminated union with explicit success and failure variants, validated via Zod.
  ...
  - Consumes: T1.1 `resumeSession` signature anchor; api-payload-contracts.md:639 `interface DriverInterventionResult` precedent (sibling shape)
...
**Interface-completeness assignment (execution-time clarification, PR #165).** The shipped `ProviderDriver` interface (`packages/contracts/src/provider-driver.ts:116`) declares 10 operations (Spec-005:43, :67-77). The per-task `Provides:` lines above enumerate 7 — the 5 lifecycle ops (T3.1/T3.6), `applyIntervention` (T3.2/T3.7), and `getCapabilities` (T3.3/T3.8). The remaining 3 — `respondToRequest`, `listModels`, `listModes` — are required for the interface to type-resolve when T3.1/T3.6 assemble the driver object in `index.ts` (T3.1's Spec coverage already cites "Spec-005:43 (10-op contract surface)"), so they are authored in this phase, assigned by responsibility per the §Phase 4 decision-#2 taxonomy: `respondToRequest` → `lifecycle.ts` (T3.1/T3.6) as a direct run-scoped control op alongside `interruptRun` (it has no capability-gated degraded path, so it is deliberately NOT part of the T3.2/T3.7 `applyIntervention` dispatcher); `listModels` + `listModes` → `capabilities.ts` (T3.3/T3.8) as "model/mode reads" introspection co-located with `getCapabilities`. No task is added, merged, or removed — this records op-to-file placement that the collapsed T3.6-T3.10 bullet and the 5-op T3.1/T3.6 `Provides:` line left implicit.

**Provider-driver substrate (execution-time clarification, PR #165).** Both V1 drivers wrap the participant's installed provider **CLI** and run on that CLI's interactive **subscription**, not a separately-billed raw provider API key — the "remote provider **APIs or services**" path of Spec-005:54 (each CLI is itself a local client of its provider's remote API; driver control and execution authority stay on the local runtime node per Spec-005:167). Wrapping a CLI is also what gives each driver a fixed built-in tool catalog to declare in T3.4 / T3.9.

- **Codex driver (T3.1-T3.5)** wraps `codex app-server` (line-delimited JSON-RPC over stdio per CP-005-3). Auth resolves through the ChatGPT sign-in that `codex login` performs by default (the binary's interactive subscription login); the driver never passes `codex login --with-api-key` (the `OPENAI_API_KEY`-from-stdin path that bypasses the subscription) — the symmetric analog of the Claude driver's never-`--bare` rule below.
- **Claude driver (T3.6-T3.10)** spawns `claude -p --output-format stream-json --input-format stream-json` and speaks the headless stream-json protocol over stdio. Auth resolves through the subscription token the binary reads in normal mode (`claude setup-token` — "requires Claude subscription"); the driver never runs `--bare` and never sets `ANTHROPIC_API_KEY` — the `--bare` API-key path, under which "OAuth and keychain are never read", i.e. the subscription is bypassed. Transport is **CLI-direct, not the Claude Agent SDK**: the two are capability-equivalent (the SDK spawns the same binary and the same stream-json protocol, exposing the identical built-in tool set), so CLI-direct is chosen for symmetry with the Codex `app-server` wrap — the daemon owns the subprocess via the PtyHost/worktree model, parses the native protocol, and normalizes to the six event families — to track the ground-truth CLI surface, and to avoid coupling to the SDK release cadence. Per-call tool approvals route through `--permission-prompt-tool` to a daemon-hosted MCP tool (the CLI equivalent of the SDK `canUseTool` callback), preserving Cedar gating at the daemon's approval boundary; the driver MUST wire this or a headless run stalls on the first un-granted permission. The choice is contained to `drivers/claude/{lifecycle,index}.ts` plus the normalizer's input-parsing half — it does not affect the `ProviderDriver` contract, the registry, the Codex driver, or the T3.9 catalog, so it is swappable later.
- **Tool catalogs are the providers' full built-in tool sets (T3.4 / T3.9).** Each is the static set of confirmed built-in leaf tools — Codex 0.141.0 and Claude Code 2.1.185 as installed — every entry carrying an explicit `idempotency_class`. MCP / app / skill tools are runtime-discovered and classified per-leaf at discovery; feature-gated or adjacency-only tools are documented exclusions that fall safe to the I-005-3 `manual_reconcile_only` default. Neither is baked into the static catalog.
...
2. **Plan-006 Phase 3 ratifies `providerFailureDetail?: string` on `run.failed` payload** ([`api-payload-contracts.md` `RunStateChangeEvent`](../architecture/contracts/api-payload-contracts.md) line ~996). Mirrors the `failed`-variant `providerFailureDetail` field of `DriverResumeResult` (line 657, shipped by Plan-005 T1.6) — Plan-015's recovery dispatcher reads the typed failure detail from the driver result and emits it on the canonical `run.failed` event so the audit log carries the operator-actionable reason without re-querying the driver. ADR-018-compliant additive-only MINOR addition (no consumer breakage; optional field).
...
Phase 1's T1.4 code authoring surfaced an internal naming inconsistency in the canonical contract doc: the two sibling driver result-envelope types used different conventions. `DriverResumeResult` (api-payload-contracts.md:652) leads with the `Driver` prefix, but its explicit sibling — the `applyIntervention` envelope at :639 — buried the qualifier in the middle as `InterventionDriverResult`. The doc is the defect; the fix is the doc, and Phase 1 code then mirrors the corrected doc (doc-first preserved).
