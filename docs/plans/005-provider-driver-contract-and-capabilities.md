...
- Define the internal driver interface for 10 operations (canonical long-form names per [api-payload-contracts.md:579](../architecture/contracts/api-payload-contracts.md) `interface ProviderDriver`): `createSession`, `resumeSession`, `startRun`, `interruptRun`, `applyIntervention`, `respondToRequest`, `closeSession`, `listModels`, `listModes`, `getCapabilities`.
...
- **T1.1** — Author the 10-operation `ProviderDriver` interface in `packages/contracts/src/provider-driver.ts`.
  ...
  - Consumes: `SessionId` / `RunId` / `ChannelId` from `packages/contracts/src/session.ts` (Plan-001 Phase 2, Tier 1 — shipped); `PtyHost` contract from `packages/contracts/src/pty-host.ts` (Plan-024 Phase 2, Tier 1 — shipped); typed shape at api-payload-contracts.md:579 (`interface ProviderDriver`)
...
- **T1.2** — Author the 7-flag `DriverCapabilityFlag` literal-union type and `DriverCapabilities` shape in `packages/contracts/src/provider-driver.ts`.
  ...
  - Consumes: api-payload-contracts.md:171 (`type DriverCapabilityFlag` anchor), :682 (`interface DriverCapabilities` anchor)
...
- **T1.4** — Author `ApplyInterventionParams` + Zod-validated `DriverInterventionResultSchema` in `packages/contracts/src/provider-driver.ts`.
  ...
  - Consumes: api-payload-contracts.md:614 (`interface ApplyInterventionParams` anchor), :635 (`interface InterventionDriverResult` anchor), :145 (cross-cutting `InterventionType` enum — resolution: co-locate the `InterventionType` re-export in `provider-driver.ts` since Plan-004's `runControl.ts` ships at Tier 5; Plan-004 imports the enum from Plan-005 at Tier 4)
...
- **T1.6** — Author the `DriverResumeResult` discriminated-union return shape for `resumeSession()` in `packages/contracts/src/provider-driver.ts`. Pattern matches existing `DriverInterventionResult` (T1.4): a `status`-discriminated union with explicit success and failure variants, validated via Zod.
  ...
  - Consumes: T1.1 `resumeSession` signature anchor; api-payload-contracts.md:635 `interface InterventionDriverResult` precedent (sibling shape)
...
- **T3.3** — Author Codex capability declaration + refresh.
  ...
  - Provides: `getCapabilities()` for Codex returning the V1 `GetCapabilitiesResult` wrapper with declared capability flags (matrix above) + Codex-declared per-tool metadata array. Refresh trigger emits `runtime_node.capability_updated` event per Spec-006:378 (existing event surface; payload `previousState` / `newState` carry the wrapper-shape contents) — see CP-005-5.
  - Estimate: 1 PR
...
- **T4.5** — Author daemon-side capability cache + invalidation.
  ...
  - Provides: Per-driver capability cache + invalidation on `runtime_node.capability_updated` event (Spec-006:378; subscribed via the daemon's audit-log fanout). SDK `listCapabilities()` reads the cache (no provider round-trip per call).
  - Estimate: 1 PR
...
**Capability-declaration + capability-change events reuse existing Spec-006 surface.** Spec-006:377-378 already enumerates two events in the `runtime_node_lifecycle` category that name "provider driver" as a capability type:
...
No new event types, no new category. The 7-flag matrix + per-tool metadata travel inside the existing `capabilityDetails` / `previousState` / `newState` payload fields. Plan-006 audit confirms these payload shapes are admissible under the existing event surface (Spec-006:378 already lists "driver version bump, tool addition" as a documented example of `capability_updated`).
...
The cross-cutting `InterventionType = "steer" | "interrupt" | "cancel"` enum (api-payload-contracts.md:145) is consumed by both Plan-004's `runControl.ts` (Tier 5) and Plan-005's `provider-driver.ts` (Tier 4). Resolution: Plan-005 Phase 1 (T1.4) co-locates the `InterventionType` re-export in `provider-driver.ts` since Plan-005 ships earlier. Plan-004 Tier 5 imports the enum from Plan-005 (Tier 4 → Tier 5 tier-consistent import).
