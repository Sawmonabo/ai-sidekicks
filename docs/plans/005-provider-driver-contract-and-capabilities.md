...
- **T2.4** — Author `driver_capabilities` cache writer logic.
  ...
  - Provides: Writes per-driver capability data on driver registration + on capability-refresh events. Writes per-flag rows to `driver_capabilities` (one row per `DriverCapabilityFlag` enum value with `supported INTEGER 0/1` and `refreshed_at`) AND per-tool rows to `driver_tools` (one row per `ProviderToolMetadata` with `idempotency_class` CHECK enum, optional `description`, and `refreshed_at`) so the daemon's two-phase command-receipt protocol (Spec-005:130-132) can read `idempotency_class` at crash-recovery dispatch time without round-tripping the driver. Reads on cold-start cache hydration union both tables to reconstruct `GetCapabilitiesResult = { capabilities: { flags, contractVersion }, tools: ProviderToolMetadata[] }`. Refresh trigger emits `runtime_node.capability_updated` event per Spec-006:376 (existing event surface; payload `previousState` / `newState` carry the reconstructed wrapper-shape contents) — see CP-005-5.
  - Estimate: 1 PR
