...
**Plan-003 plan-doc prose left intact (deliberate).** The same forward-dep narrative appears as point-in-time design prose in [Plan-003](./003-runtime-node-attach.md) (the Phase-2 cross-plan-amendment at line 83 and the implementation step at line 325: "do not yet exist" / "absent"). Those are Plan-003's own governance / implementation narrative, true as authored (PR #137, 2026-06-02), and explicitly scoped to **Plan-006 Tier 4** for removal when it binds the canonical `CapabilityDetails` (the documented removal trigger all three mentions share). The rewritten merge-neutral code comment no longer contradicts them, so leaving them is ownership discipline, not unresolved drift — Plan-006 Tier 4 retires the whole forward-dep narrative (code marker + both Plan-003 prose mentions) in one consistent pass when it owns the bind.
