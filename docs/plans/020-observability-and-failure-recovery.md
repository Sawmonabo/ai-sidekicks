...
**Cardinality ceiling (I-020-1).** Total emitted series across all five families MUST stay below 200 per daemon instance. Series-count assertion runs in integration tests; exceeding the ceiling is a violation of I-020-1 (not a warning), blocking merge until the allow-list tightens.
...
- **T1.1 — `HealthStatusRead` + `FailureDetailRead` contracts.**
  ...
  - Zod schemas + inferred types for `HealthStatusRead` (daemon / control-plane / provider / replay health over the `healthy` / `degraded` / `blocked` status categories) and `FailureDetailRead` (machine-readable failure category, optional recovery condition, human-readable summary). `FailureDetailRead.category` **imports** `RunFailureCategory` from the canonical contracts mirror; it never redefines the union, and the four arms are closed per §API And Transport Changes above — a fifth arm is a cross-plan contract change, not a Plan-020 edit. Policy or approval blockage is deliberately not an arm; it surfaces as `blockingReason` on T1.2's stuck-run shape.
  - **Tests:** `packages/contracts/src/__tests__/health.test.ts` (CREATE) — parse-accept one fixture per shape; parse-reject a status outside the three categories; parse-reject a `category` outside the four `RunFailureCategory` arms (the closure regression); assert the package-root barrel resolves both schemas.
...
- **T3.1 — `metrics-registry.ts`: allow-listed families with bounded labels.**
  ...
  - **Consumes:** the canonical control-plane family set ← [Plan-021](./021-rate-limiting-policy.md) (CP-020-1 — label-invariant doc contract; no daemon registration)
...
- **T3.3 — `metrics-auth.ts`: bind and auth secure-default gate.**
  ...
  - Default `METRICS_BIND=127.0.0.1:<port>`. A non-loopback bind without auth is a config-parse-time error. Non-loopback requires either `METRICS_AUTH=bearer` with a rotated token file or `METRICS_AUTH=mtls` with an operator-provided client-cert allow-list. `METRICS_BIND=off` disables the endpoint and emits the banner plus a `security.default.override=metrics_disabled` log event exactly once per startup. This is the contract Plan-025 consumes for the relay endpoint per CP-020-4.
  - **Tests:** `packages/runtime-daemon/src/observability/__tests__/metrics-auth.test.ts` (CREATE) — non-loopback bind without auth fails at parse time with an actionable error; bearer mode rejects missing and wrong tokens and invalidates old tokens on the next request after rotation; mtls mode rejects a client cert absent from the allow-list; `METRICS_BIND=off` disables the endpoint and emits the banner and log event exactly once.
...
- **T3.4 — Cardinality-ceiling integration test and CI wiring.**
  ...
  - Assert total emitted series across the five families stays below 200 per daemon instance under a fixture exercising every registered label combination, and wire the assertion into CI so a breach blocks merge.
...
- **T4.1 — `healthClient.ts` typed SDK surface.**
  ...
  - Expose health status, failure detail, stuck-run inspection, recovery-action requests, and the `DiagnosticRedactionPolicy` policy-state read over the typed SDK.
  ...
  - **Consumes:** contracts ← Phase 1; daemon services ← Phase 2
...
- **/metrics endpoint secure-default tests (Spec-027 row 9):**
  ...
  - Cardinality ceiling (I-020-1): integration test asserts total emitted series across the five families stays below 200 per daemon instance; exceeding ceiling blocks merge.
...
- [ ] Prometheus `/metrics` endpoint lands with the five allow-listed daemon metric families, bounded label sets, bearer-token / mTLS auth gate for non-loopback bind, and emission-time label enforcement verified by negative tests (I-020-2)
