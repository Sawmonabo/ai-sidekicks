...
| **Owned Spec-027 Rows** | 9 — Prometheus `/metrics` exposition (daemon endpoint + five daemon metric families (row 9a) + the bind/auth secure-default contract, which is shared: the same `METRICS_BIND` / `METRICS_AUTH` contract governs row 9b's relay endpoint); see [Spec-027 row 9](../specs/027-self-host-secure-defaults.md#required-behavior). The relay mounts the equivalent relay-side surface, owning row 9b's relay metric families and endpoint wiring while consuming this plan's bind/auth contract (CP-020-4). |
