...
- **T1.1 — `HealthStatusRead` + `FailureDetailRead` contracts.**
  ...
  - **Consumes:** `RunFailureCategory` ← [API Payload Contracts §Shared Enums](../architecture/contracts/api-payload-contracts.md#shared-enums) (closed four-arm union; import only); `RecoveryConditionSchema` / `RecoverySpanClassificationSchema` ← Plan-005 `packages/contracts/src/provider-driver.ts` (Plan-005 T4.8; import only)
