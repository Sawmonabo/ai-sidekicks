...
- `TimelineEntry` carries an optional `superseded` marker struct — `{runId, sourceEpoch, targetPosition}`, present exactly when the row's turn is superseded, absence meaning current — identical on `TimelineRead` windows and `TimelineSubscribe` replay ([API Payload Contracts](../architecture/contracts/api-payload-contracts.md) mirrors the shape); the live-stream `run.rolled_back` boundary entry carries the rewind cutoff for the client-side already-delivered-rows rule, with rows delivered after the boundary arriving marker-precomputed — a late straggler pre-marked exactly when its stamped attribution ranks above its epoch's cutoff, current at or below it; new-epoch rows unmarked — per `Spec-013 §Required Behavior`.
...
2. Implement daemon-owned timeline projection and replay-aware subscription delivery from canonical events — consuming Plan-004 T3.14's exported `supersededTurns(runId)` (CP-004-13) to compute each row's `superseded` marker on read and replay windows, marking live-appended rows at emission (a late pre-rollback straggler appends pre-marked exactly when its stamped attribution ranks above its epoch's cutoff and current when it ranks into the surviving prefix; a new-epoch row appends unmarked), emitting the `run.rolled_back` boundary entry with its rewind cutoff for the client-side rule over already-delivered rows, and composing the marker with audit-stub rows over compacted regions.
...
- Superseded-rendering tests proving live marking (already-delivered rows of the rewound run above the boundary's cutoff take the treatment; an above-cutoff late straggler delivered after the boundary arrives pre-marked while a straggler whose stamped `sourcePosition` ranks at or below its epoch's cutoff arrives current — the T3.14 late-delivered-sibling case; new-epoch rows never mark), read/replay marker parity with the live outcome, reused-ordinal correctness across the two-rollback sequence Plan-004 T3.14 pins (10→5, re-execute, 7→6 — the second epoch's re-executed turn 6 renders current while the first epoch's turn 6 stays superseded, the epoch-scoping `Spec-004 §Required Behavior` owns), and the compacted-stub composition (stub placeholder + superseded marker)
