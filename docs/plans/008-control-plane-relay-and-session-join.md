...
- **T-008b-1-1** (Files: `packages/control-plane/src/server/host.ts` (CREATE) + `packages/control-plane/src/server/feature-flag-gate.ts` (CREATE) + `packages/control-plane/src/server/production-deployment-gate.ts` (CREATE) + `packages/control-plane/wrangler.toml` (CREATE) + `packages/control-plane/.dev.vars.example` (CREATE) + `<repo-root>/.gitignore` (EXTEND with `.dev.vars` + `.dev.vars.*`); Verifies invariant: I-008-1 dual-gate; Spec coverage: per F-008b-1-06, NO Spec-008 AC at Tier 1) — Implement the tRPC v11 host scaffolding (Cloudflare Workers via `@trpc/server/adapters/fetch`'s `fetchRequestHandler`, per BL-104 resolution) behind I-008-1 dual-gate enforcement. The feature-flag gate (`env.CONTROL_PLANE_BOOTSTRAP_ENABLED === '1'` checked at request-entry, refuse-with-503 before router dispatch when unset) AND the production-deployment gate (`env.ENVIRONMENT !== 'production'` per F-008b-1-04 Workers reformulation, refuse-with-503 even with flag set when running under the `[env.production]` wrangler config) BOTH ship in this task.
  ...
  **Bootstrap flag location.** The flag is supplied locally via `packages/control-plane/.dev.vars` (`CONTROL_PLANE_BOOTSTRAP_ENABLED=1`), which `wrangler dev` reads automatically for workerd local development per [Cloudflare Workers Secrets docs](https://developers.cloudflare.com/workers/configuration/secrets/) ("Put secrets for use in local development in either a `.dev.vars` file or a `.env` file, in the same directory as the Wrangler configuration file"). `wrangler deploy` does NOT consult `.dev.vars` (deployed Workers receive secrets only via `wrangler secret put`, the dashboard, or `--secrets-file`), so the flag cannot reach a published Worker through the default deployment path. The live `.dev.vars` MUST be gitignored — root `.gitignore` extends with `.dev.vars` + `.dev.vars.*` per Cloudflare's explicit guidance ("Do not commit secrets to git. The `.dev.vars` and `.env` files should not committed to git"). A committed `packages/control-plane/.dev.vars.example` documents the expected keys (literal contents: `CONTROL_PLANE_BOOTSTRAP_ENABLED=1`) and instructs implementers to copy it to `.dev.vars` for local `wrangler dev` runs.
  ...
  Tests: T-008b-1-T1 handler refuses to serve without flag (I-008-1 gate #1); T-008b-1-T2 handler refuses with 503 in production environment even with flag set (I-008-1 gate #2); T-008b-1-T3 handler serves successfully with flag set in dev environment under `wrangler dev`. (Audit trail for prior `[env.dev]` and top-level-`[vars]` task drafts: see §Decision Log.)
