...
- [x] **SSE wire frame primitive declared (closed 2026-04-30).** Authoritative wire frame lives at [api-payload-contracts.md §SSE Wire Frame (Tier 1 Ratified)](../architecture/contracts/api-payload-contracts.md): Content-Type, `data:` encoding rules, `id:` cursor semantics, `retry:` directive, `Last-Event-ID` resumption, and heartbeat cadence are ratified. SSE adapter selection settled by BL-104: tRPC's shared `resolveResponse.ts` substrate via `@trpc/server/adapters/fetch` on Cloudflare Workers — see §Decision Log.
- [x] **tRPC procedures typed surface declared (closed 2026-04-30).** Per-procedure shape (query/mutation/subscription classification + Zod schema imports) is ratified at [api-payload-contracts.md §Tier 1 (cont.): Plan-008](../architecture/contracts/api-payload-contracts.md): `session.create` mutation, `session.read` query, `session.join` mutation, `session.subscribe` subscription (SSE-backed via tRPC fetch adapter).
...
- **T-008b-1-T7** (Verifies CP-008-1 SSE substrate) SSE connection lifecycle: open + send synthetic `EventEnvelope` + close on disconnect. Frame shape canonical at [api-payload-contracts.md §SSE Wire Frame (Tier 1 Ratified)](../architecture/contracts/api-payload-contracts.md) (closed 2026-04-30).
...
- [x] BLOCKED-ON-C6 governance pickup tracked: tRPC procedure-type assignments + canonical method-name strings ratified at [api-payload-contracts.md §Tier 1 (cont.): Plan-008](../architecture/contracts/api-payload-contracts.md) (closed 2026-04-30 — PR #22).
- [x] BLOCKED-ON-C6 governance pickup tracked: SSE wire frame primitive ratified at [api-payload-contracts.md §SSE Wire Frame (Tier 1 Ratified)](../architecture/contracts/api-payload-contracts.md) — Content-Type, `data:` encoding, `id:`/`retry:`/`Last-Event-ID` semantics, heartbeat cadence authoritative; tRPC adapter selection settled by BL-104 (closed 2026-04-30 — PR #22).
