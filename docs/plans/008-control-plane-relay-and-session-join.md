...
- `packages/control-plane/wrangler.toml` — Tier 1 bootstrap-deliverable: workerd local-dev + Cloudflare Workers deployment configuration. Top-level `[vars]` holds the development environment marker only (`ENVIRONMENT = "development"`), consumed by `wrangler dev` without an `--env` flag. The bootstrap feature flag is intentionally NOT placed in `wrangler.toml` — see `.dev.vars` bullet below for the security rationale. `[env.production.vars]` sets `ENVIRONMENT = "production"` for `wrangler deploy --env production`. This layout enforces I-008-1 gate #2 — see T-008b-1-1 for the full key/value matrix
- `packages/control-plane/.dev.vars` — Tier 1 bootstrap-deliverable: local-only secrets file (gitignored per Cloudflare guidance) supplying `CONTROL_PLANE_BOOTSTRAP_ENABLED=1` for `wrangler dev` workerd local development. `wrangler deploy` does NOT read this file (per [Cloudflare Workers Secrets docs](https://developers.cloudflare.com/workers/configuration/secrets/)), so the flag cannot reach a published Worker via the default deployment path. A committed `.dev.vars.example` documents the expected keys; root `.gitignore` adds `.dev.vars` + `.dev.vars.*`
...
- **T-008b-1-1** (Files: `packages/control-plane/src/server/host.ts` (CREATE) + `packages/control-plane/src/server/feature-flag-gate.ts` (CREATE) + `packages/control-plane/src/server/production-deployment-gate.ts` (CREATE) + `packages/control-plane/wrangler.toml` (CREATE) + `packages/control-plane/.dev.vars.example` (CREATE) + `<repo-root>/.gitignore` (EXTEND with `.dev.vars` + `.dev.vars.*`); Verifies invariant: I-008-1 dual-gate; Spec coverage: per F-008b-1-06, NO Spec-008 AC at Tier 1) — Implement the tRPC v11 host scaffolding (Cloudflare Workers via `@trpc/server/adapters/fetch`'s `fetchRequestHandler`, per BL-104 resolution) behind I-008-1 dual-gate enforcement. The feature-flag gate (`env.CONTROL_PLANE_BOOTSTRAP_ENABLED === '1'` checked at request-entry, refuse-with-503 before router dispatch when unset) AND the production-deployment gate (`env.ENVIRONMENT !== 'production'` per F-008b-1-04 Workers reformulation, refuse-with-503 even with flag set when running under the `[env.production]` wrangler config) BOTH ship in this task.

  **`wrangler.toml` layout.** Top-level `[vars]` is read by `wrangler dev` workerd local without an `--env` flag (since named `[env.NAME]` blocks load only when `--env`/`-e` or `CLOUDFLARE_ENV` is set). It carries the development environment marker only — `ENVIRONMENT = "development"`. The bootstrap flag MUST NOT be set in top-level `[vars]`: `wrangler deploy` (without `--env`/`-e` or `CLOUDFLARE_ENV`) targets the top-level environment, so a top-level `CONTROL_PLANE_BOOTSTRAP_ENABLED = "1"` would publish a publicly reachable Worker where both I-008-1 gates pass (`flag === '1'` AND `ENVIRONMENT !== 'production'`), contradicting the I-008-1 "defaulting to off" + "MUST NOT set the flag until Tier 5" invariants. `[env.production.vars]` sets `ENVIRONMENT = "production"` and MUST NOT set the bootstrap flag until Tier 5; `wrangler deploy --env production` consumes this block.

  **Bootstrap flag location.** The flag is supplied locally via `packages/control-plane/.dev.vars` (`CONTROL_PLANE_BOOTSTRAP_ENABLED=1`), which `wrangler dev` reads automatically for workerd local development per [Cloudflare Workers Secrets docs](https://developers.cloudflare.com/workers/configuration/secrets/) ("Put secrets for use in local development in either a `.dev.vars` file or a `.env` file, in the same directory as the Wrangler configuration file"). `wrangler deploy` does NOT consult `.dev.vars` (deployed Workers receive secrets only via `wrangler secret put`, the dashboard, or `--secrets-file`), so the flag cannot reach a published Worker through the default deployment path. The live `.dev.vars` MUST be gitignored — root `.gitignore` extends with `.dev.vars` + `.dev.vars.*` per Cloudflare's explicit guidance ("Do not commit secrets to git. The `.dev.vars` and `.env` files should not committed to git"). A committed `packages/control-plane/.dev.vars.example` documents the expected keys (literal contents: `CONTROL_PLANE_BOOTSTRAP_ENABLED=1`) and instructs implementers to copy it to `.dev.vars` for local `wrangler dev` runs.

  **Corrected misformulations (history).** Two earlier task drafts were corrected: (i) naming `[env.dev]` as the `wrangler dev` default — which would leave `env.ENVIRONMENT` unset and break T-008b-1-T3's `env.ENVIRONMENT === 'development'` assertion; (ii) placing `CONTROL_PLANE_BOOTSTRAP_ENABLED = "1"` in top-level `[vars]` — which would default `wrangler deploy` to a bootstrap-enabled production publish (the security hole described above). Both are corrected in this task definition.

  Tests: T-008b-1-T1 handler refuses to serve without flag (I-008-1 gate #1); T-008b-1-T2 handler refuses with 503 in production environment even with flag set (I-008-1 gate #2); T-008b-1-T3 handler serves successfully with flag set in dev environment under `wrangler dev`.
...
| 2026-04-30 | T-008b-1-1 wrangler.toml security correction (Codex P1, PR #20 round 3) | Codex flagged that the prior draft of T-008b-1-1 placed `CONTROL_PLANE_BOOTSTRAP_ENABLED = "1"` in top-level `[vars]`, which `wrangler deploy` (without `--env`/`-e` or `CLOUDFLARE_ENV`) targets — opening a path for an operator to publish a publicly reachable Worker where both I-008-1 gates pass (`flag === '1'` AND `ENVIRONMENT !== 'production'`), contradicting the I-008-1 "defaulting to off" + "MUST NOT set the flag until Tier 5" invariants. Fix: bootstrap flag relocated from top-level `[vars]` to `packages/control-plane/.dev.vars` (gitignored, local-only, never read by `wrangler deploy` per [Cloudflare Workers Secrets docs](https://developers.cloudflare.com/workers/configuration/secrets/)). Top-level `[vars]` retains only `ENVIRONMENT = "development"` (not the security-load-bearing key). I-008-1 gate semantics unchanged — both gates still enforce at runtime; this fix closes a configuration-default exposure surface (defense-in-depth structural separation between local-dev secrets and deployable config). Committed `packages/control-plane/.dev.vars.example` documents expected keys; root `.gitignore` extended with `.dev.vars` + `.dev.vars.*` per Cloudflare guidance. |
