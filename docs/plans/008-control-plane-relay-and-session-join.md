...
1. **Feature-flag gate.** Tier 1 PR must wire the bootstrap behind a feature flag (e.g., `CONTROL_PLANE_BOOTSTRAP_ENABLED=1`) that defaults off; production deployment configurations MUST NOT set the flag until Tier 5. Negative-path test: server refuses to start without the flag in `NODE_ENV=production`.
2. **Loopback-bind gate (per F-008b-1-04).** Bootstrap MUST default `BIND_HOST=127.0.0.1` (or platform-equivalent loopback) AND MUST refuse to start when `BIND_HOST` resolves to a non-loopback address even when the feature flag is set, in `NODE_ENV=production`. This gate is independent of the feature flag — the flag enables the bootstrap; the bind-host gate prevents accidental production exposure when an operator sets the flag for development reasons. Positive-path test: loopback bind succeeds with flag set. Negative-path test: `0.0.0.0` bind refused with flag set in `NODE_ENV=production`. Coordinate language with Spec-027's bind-time secure-defaults pattern (Plan-007's domain) — Spec-027 governs the daemon-side bind surface; I-008-1 is the SaaS-host parallel for the control-plane bootstrap.
...
Plan-008-bootstrap Phase 1 ships the tRPC v11 `sessionRouter` HTTP handlers + SSE substrate that Plan-001 Phase 5's control-plane-side `sessionClient` consumes. The contract surface includes (a) the per-procedure tRPC shapes (`SessionCreate` / `SessionRead` / `SessionJoin` query-or-mutation classification + Zod schema imports per F-008b-1-03 / BLOCKED-ON-C6), (b) the SSE wire frame primitive (Content-Type, `data:` encoding, `id:`/`retry:`/`Last-Event-ID` semantics per F-008b-1-01 / BLOCKED-ON-C6), and (c) the loopback-bind + feature-flag gate (per I-008-1) so Plan-001 Phase 5 integration tests against a development-mode bootstrap.
...
**Why bidirectional.** Plan-003 derivatively cites Plan-008's host runtime declaration (the Fastify-vs-Cloudflare-Workers contradiction at F-008b-1-02 cascades into Plan-003); resolving C-4 governance updates both Plan-008 AND Plan-003 in lockstep.
...
1. **[Tier 1: tRPC server skeleton + sessionRouter + SSE substrate; Tier 5: relay-negotiation + presence + reconnect contracts]** Define authenticated join, presence, reconnect, and relay-negotiation contracts. Include the relay wire format for relay messages. Per F-008b-1-07 expansion, the Tier 1 portion breaks into 4 sub-actions:
   - **(a)** Register tRPC v11 router on the host via the chosen adapter (BLOCKED-ON-C4 — adapter selection is downstream of ADR-014 runtime authorization; Fastify uses `@trpc/server/adapters/fastify`; Cloudflare Workers uses `@trpc/server/adapters/fetch`).
   - **(b)** Wire the I-008-1 dual-gate enforcement at the boot site: feature-flag check (`CONTROL_PLANE_BOOTSTRAP_ENABLED=1`) AND loopback-bind check (per F-008b-1-04 enforcement) — both refusal paths log + exit before any network listener binds.
   - **(c)** Register the SSE subscription adapter for `SessionSubscribe` (BLOCKED-ON-C6 — adapter selection follows from the SSE wire frame primitive landing in api-payload-contracts.md §Plan-008).
...
**Precondition:** [Plan-001](./001-shared-session-core.md) Phase 4 merged (the bootstrap wraps that PR's `packages/control-plane/src/sessions/session-directory-service.ts` — already shipped 2026-04-27 per [GitHub PR-#10](https://github.com/Sawmonabo/ai-sidekicks/pull/10)). Plan-001 Phase 2 schemas (`packages/contracts/src/session.ts` + `packages/contracts/src/event.ts`) merged. ADR-014 runtime authorization reconciled per §Preconditions BLOCKED-ON-C4 (or every Fastify reference in this Phase replaced with the C-4-resolved adapter).

- `packages/control-plane/src/server/` — host + tRPC v11 router registration scaffolding per [ADR-014](../decisions/014-trpc-control-plane-api.md). **Runtime: Fastify v5 (provisional, BLOCKED-ON-C4 governance pickup)** — ADR-014 currently authorizes Cloudflare Workers; until C-4 reconciles, every "Fastify host" reference in this plan body is provisional pending ADR-014 amendment OR Plan-008 amendment. Skeleton only — relay broker / presence register / invite handlers ship in Plan-008-remainder at Tier 5. Per §Invariants I-008-1, the bootstrap MUST be gated behind a feature flag (e.g., `CONTROL_PLANE_BOOTSTRAP_ENABLED=1`) defaulting to off AND must enforce the loopback-bind gate (per I-008-1 verification gate #2 + F-008b-1-04); production deployment configurations MUST NOT set the flag until Tier 5.
...
- `packages/control-plane/src/sessions/session-subscribe-sse.ts` — SSE transport plumbing for `SessionSubscribe`. The contract is request-only on the wire; the response is an `AsyncIterable<EventEnvelope>` SSE stream per `packages/contracts/src/session.ts:388`. Bootstrap supplies only the transport — event sourcing into the stream remains Plan-006's domain. **Wire frame BLOCKED-ON-C6 governance pickup** — until api-payload-contracts.md §Plan-008 (or sse-contracts.md) declares Content-Type, `data:` encoding, `id:`/`retry:`/`Last-Event-ID` semantics, heartbeat cadence, and tRPC adapter selection, this file ships behind a conservative inline shape: `Content-Type: text/event-stream; charset=utf-8`; `Cache-Control: no-store`; `X-Accel-Buffering: no`; one `EventEnvelope` per SSE event encoded as `data: <single-line JSON>`; `id:` carries the `EventCursor` value from Plan-006 (or placeholder string at Tier 1 pending Plan-006 widening); `retry: 5000`; on reconnect with `Last-Event-ID`, server emits all events strictly after that cursor; `event: heartbeat\ndata: {}\n\n` every 15s; tRPC v11 adapter is the official Fastify SSE link (BLOCKED-ON-C4 — Cloudflare Workers requires a different adapter).

#### Tasks

- **T-008b-1-1** (Files: `packages/control-plane/src/server/host.ts` (CREATE) + `packages/control-plane/src/server/feature-flag-gate.ts` (CREATE) + `packages/control-plane/src/server/loopback-bind-gate.ts` (CREATE); Verifies invariant: I-008-1 dual-gate; Spec coverage: per F-008b-1-06, NO Spec-008 AC at Tier 1) — Implement the tRPC v11 host scaffolding behind I-008-1 dual-gate enforcement. **BLOCKED-ON-C4** — runtime authorization (Fastify v5 vs Cloudflare Workers vs ADR-NNN) gates this task's adapter selection. The feature-flag gate (`CONTROL_PLANE_BOOTSTRAP_ENABLED=1` checked at boot, refuse-and-exit before any listener bind in `NODE_ENV=production`) AND the loopback-bind gate (`BIND_HOST` resolves to loopback, refuse `0.0.0.0` even when flag set in production per F-008b-1-04) BOTH ship in this task. Tests: T-008b-1-T1 server refuses to start without flag in `NODE_ENV=production` (I-008-1 gate #1); T-008b-1-T2 server refuses `BIND_HOST=0.0.0.0` with flag set in `NODE_ENV=production` (I-008-1 gate #2); T-008b-1-T3 server starts successfully with flag set + loopback bind in development.
...
- **T-008b-1-3** (Files: `packages/control-plane/src/sessions/session-subscribe-sse.ts` (CREATE) + `packages/control-plane/src/sessions/session-subscribe-sse.factory.ts` (CREATE); Verifies invariant: I-008-3 enforcement #1 (constructor injection); Spec coverage: §Cross-Plan Obligations CP-008-1 + CP-008-3) — Implement SSE substrate via `createSessionSubscribeSse(directoryService: SessionDirectoryService): SseHandler` factory. Conservative inline wire frame per the bullet above. **BLOCKED-ON-C6** — wire frame primitive may shift when api-payload-contracts.md §Plan-008 lands; the factory's frame-shaping logic is centralized so the C-6 resolution updates one file. **BLOCKED-ON-C4** — adapter selection (Fastify SSE link vs Workers SSE) downstream of ADR-014 reconciliation. Tests: T-008b-1-T7 SSE connection lifecycle (open + send synthetic EventEnvelope + close on disconnect); T-008b-1-T8 `Last-Event-ID` resumption (reconnect with header → server emits events strictly after cursor); T-008b-1-T9 heartbeat cadence (15s `event: heartbeat` frames in absence of data).
...
- **T-008b-1-T1** (Verifies I-008-1 gate #1) `packages/control-plane/test/server/feature-flag-gate.test.ts`: server refuses to start without `CONTROL_PLANE_BOOTSTRAP_ENABLED=1` in `NODE_ENV=production`; refusal logged + process exits before any listener bind.
- **T-008b-1-T2** (Verifies I-008-1 gate #2 + F-008b-1-04) `packages/control-plane/test/server/loopback-bind-gate.test.ts`: server refuses `BIND_HOST=0.0.0.0` with flag set in `NODE_ENV=production`; refusal logged + process exits.
- **T-008b-1-T3** Server starts successfully with flag set + loopback bind in development (`NODE_ENV=development`).
...
- [ ] Invariants I-008-1 (BOTH gates: feature flag AND loopback-bind), I-008-3 (constructor injection + ESLint rule + unit-test introspection — all 3 enforcement mechanisms) verified at Tier 1 scope
...
- [ ] BLOCKED-ON-C4 governance pickup tracked: ADR-014 runtime authorization reconciled — every "Fastify host" reference in Plan-008 either replaced with the C-4-resolved adapter OR ADR-014 amendment authorizes Fastify v5 for Tier 1 bootstrap
