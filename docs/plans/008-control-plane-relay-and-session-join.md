...
Plan-008-bootstrap's `sessionRouter` and `session-router.ts` MUST wrap the existing `packages/control-plane/src/sessions/session-directory-service.ts` shipped by Plan-001 PR #4. Re-implementing directory logic in the router would fork the lock-ordering invariant (per [Plan-001 §Invariants I-001-1](./001-shared-session-core.md#invariants)) and break the `Querier`-driven test pattern Plan-001 PR #5 strengthens.
...
- `packages/control-plane/src/sessions/session-router.ts` — typed tRPC procedures for `SessionCreate`, `SessionRead`, `SessionJoin` wrapping the existing `session-directory-service.ts` (Plan-001 PR #4). The router does not re-implement directory logic per §Invariants I-008-3.
