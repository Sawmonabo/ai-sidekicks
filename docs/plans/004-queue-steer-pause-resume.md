...
- **F2 (T4.7):** pending states defined — a `requested` / `accepted` response never exits the process: the CLI waits for the terminal state by idempotent replay (re-dispatching with the SAME `clientIdempotencyKey` minted at invocation start; the T1.2 replay contract returns the recorded row, never re-applies), bounded by the intervention's own expiry backstop; accepted→applied and mid-wait-expiry tests added.
