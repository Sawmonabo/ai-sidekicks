...
- **T2.3 — Orchestration-run admission pipeline.**
  ...
  - **Consumes:** `getRun` (CP-016-1); Plan-004 in-process admission (CP-016-7); T2.1/T2.5/T2.6 accessors — for a native-cap unpriced admission the T2.5 accessor returns the reserved family cap, and T2.3 **stamps it as `OrchestrationRunLinkCarrier.admittedUnpricedCapCents` (with the as-of-admission `admittedModelFamily`)** so the Plan-004 seam persists it on `run.queued` (the replay source for reservations and terminal debits; server-derived, never from the request).
