...
| **I-016-12** | Budgets are hard ceilings: `usage.budget_warning` fires exactly once per (scope, budget-type) 80% crossing; token/cost 100% crossings interrupt with `budget_exhausted` (the per-agent turn limit instead completes the run with `trigger: 'turn_limit'` — I-016-13); a session-cost ceiling interrupts all active runs and blocks admission until the owner raises the limit. | T2.5 |
