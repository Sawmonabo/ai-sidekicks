...
- **T2.10 — Session-goal service (campaign B6 2026-07-06).**
  ...
  - **Consumes:** T2.1 agents projection (configured personas); **Plan-005's `runtime-binding-store.ts` active-binding accessor** (`runtime_bindings` — the live provider legs and their binding ids; cross-plan seam CP — the fan-out target set and the intent payload's per-leg keys come from live bindings, never from the persona projection); `MethodRegistry` consumers arrive in T3.1.
