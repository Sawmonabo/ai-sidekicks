...
| **I-016-14** | A run with no appended event for its idle window is interrupted with `idle_timeout` — except under the Spec-024 pending-cross-node-dispatch hard-skip (campaign B7) (default 300000 ms; `OrchestrationRunConfig.idleTimeoutMs` override, persisted as the `run.queued` `effectiveRunConfig`). | T2.7 |
...
- **T2.5 — Budget accountant + budget surface.**
  ...
  - **Provides:** in-memory projection over `usage_telemetry` + `run.*` (replay-rebuilt, no accumulator table; per-run ceilings read from the durable `run.queued` `effectiveRunConfig` — D-016-5); once-per-(scope, type) 80% `usage.budget_warning` emission; 100% interrupt via the in-process entrypoint (`budget_exhausted`, D-016-7); session-cost ceiling fan-out (interrupt all active + admission block); `session_budgets` read/update service (row materializes with spec defaults on first touch); admission-block accessor for T2.3; the Spec-016 §Cost Derivation And Absent-Cost Semantics ladder (campaign B6 2026-07-06): 3-tier cost resolution (provider-emitted sanity-bounded → family-table derived with family-prefix fallback → fail-closed unpriced), the per-family off-threshold `usage.budget_warning { reason: 'unpriced-model', modelFamily }` + session-wide family admission block, native-cap-escape reservations (snapshot-at-admission persisted per run, released at terminal, immune to later cap updates), and committed-spend (observed + reserved) admission/threshold predicates surfacing `reservedCostCents` — task-level enrichment lands with the campaign's Plan-016 bundle.
...
- **T2.7 — Idle sweep.**
  ...
  - **Tests:** timer resets on matching-runId event; non-matching events don't reset; fires at the configured window; interrupt carries `idle_timeout`; timers rebuild after daemon restart from the persisted `effectiveRunConfig.idleTimeoutMs` (not re-merged session defaults).
...
- **T2.10 — Session-goal service (campaign B6 2026-07-06).**
  ...
  - **Verifies invariant:** —
  - **Tests:** projection rebuilds from events alone (update → clear → update); driver-op dispatch per parity leg; bounds refusal (empty, >4096, NUL).
