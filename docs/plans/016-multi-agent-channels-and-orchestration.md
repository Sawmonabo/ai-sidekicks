...
- **T2.10 — Session-goal service (campaign B6 2026-07-06).**
  - **Files:** `packages/runtime-daemon/src/orchestration/session-goal-service.ts` (NEW); owns `session_goal_dispatch_intents` rows — write-before-dispatch (payload: op, goal, prior goal, authenticated actor, per-leg pending/acked set), delete-on-append **and delete-on-failure-after-revert-convergence** (a refused mutation never survives to replay, and the intent persists until no leg remains `acked` — each successful compensating revert flips that leg to `reverted` in the persisted per-leg state, retrying immediately and on startup; further goal mutations for the session stay refused until convergence), startup replay completes only unknown-outcome intents: re-issue to pending legs, revert acked legs to prior + delete-without-append when the mutation cannot complete everywhere, and the recovered event carries the original actor (T1.3 creates the table).
