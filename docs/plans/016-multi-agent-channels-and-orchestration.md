...
| **I-016-14** | A run with no appended event for its idle window is interrupted with `idle_timeout` (default 300000 ms; `OrchestrationRunConfig.idleTimeoutMs` override, persisted as the `run.queued` `effectiveRunConfig`). | T2.7 |
