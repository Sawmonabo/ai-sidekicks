...
- Channel-level permission restrictions (V1-deferred per [Spec-016 §Resolved Questions](../specs/016-multi-agent-channels-and-orchestration.md#resolved-questions-and-v1-scope-decisions))
...
- [x] Paired spec is approved — re-checked 2026-07-18 by the campaign Task-28 / W1.5 batch spec re-promotion (supersedes the 2026-07-13 re-open note): Spec-016 returned to `approved`; its campaign amendment window closed.
...
- **T1.1 — `orchestration.ts` contract core + event payload schemas + union registration.**
  ...
  - **Tests:** schema acceptance/rejection rows per type; strict unknown-key rejection; round-robin-requires-order config validation; union discriminates all twelve types (fourteen once T1.2's `session.goal_*` registration lands); `channelCreatedPayloadSchema` back-compat (config absent still parses).
...
- **T1.3 — Migration: `channels` / `run_links` / `agents` / `session_budgets` / `session_goal_dispatch_intents`.**
  ...
  - **Provides:** the five tables (incl. `session_goal_dispatch_intents`, campaign B6) byte-matching [local-sqlite-schema.md §Channel and Orchestration Tables](../architecture/schemas/local-sqlite-schema.md#channel-and-orchestration-tables-plan-016) (CHECK constraints, indexes, defaults).
...
- **T2.3 — Orchestration-run admission pipeline.**
  ...
  - **Consumes:** `getRun` (CP-016-1); Plan-004 in-process admission (CP-016-7); T2.1/T2.5/T2.6 accessors — for a native-cap unpriced admission the T2.5 accessor returns the reserved family cap, and T2.3 **stamps it as `OrchestrationRunLinkCarrier.admittedUnpricedCapCents`, and stamps the as-of-admission `admittedModelFamily` on EVERY admission — not only native-cap ones — since derived pricing keys off it (the durable `run.queued` fields are path-independent: the ordinary `run.queueCreate` path stamps the same fields directly at the queue write per CP-004-10 — T2.3 owns wiring the resolver into that seam as an EXTEND of Plan-004's queue write, since Plan-004's tasks carry no pricing machinery; ordinary-path stamp test: an ordinary admission stamps the family — and the cap when native-cap — from the same resolver, no drift between paths)** so the Plan-004 seam persists it on `run.queued` (the replay source for reservations and terminal debits; server-derived, never from the request).
...
- Channel-level restriction policy remains V1-deferred (`Spec-016 §Resolved Questions and V1 Scope Decisions`); membership inheritance is the V1 posture.
