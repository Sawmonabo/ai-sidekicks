...
- **T2.7 — Idle sweep.**
  ...
  - **Consumes:** event-stream subscription (CP-016-4 read side); in-process intervention (CP-016-7); the caller-side pending-dispatch predicate (`sessionHasPendingCrossNodeDispatch`, [Spec-024 §Required Behavior](../specs/024-cross-node-dispatch-and-approval.md) — a cross-plan seam: the accessor lands with Plan-027 via the campaign's Plan-027 bundle; until it exists the sweep treats the predicate as constant-false, i.e. ordinary idle semantics, fail-safe) (campaign B7).
...
- **T2.10 — Session-goal service (campaign B6 2026-07-06).**
  ...
  - **Verifies invariant:** —
  - **Tests:** projection rebuilds from events alone (update → clear → update); driver-op dispatch per parity leg; live-leg driver failure surfaces the typed error and appends no event (no phantom goal); a dangling goal-dispatch intent replays at startup into a driver re-issue + event append with the original actor (crash between ack and append leaves no unaudited goal); a failed mutation deletes its intent and a post-failure restart re-applies nothing; multi-leg partial failure reverts acked legs and appends nothing; bounds refusal (empty, >4096, NUL).
...
- **T3.3 — Typed domain-error classes.**
  ...
  - **Provides:** `DaemonDomainError` subclasses for the thirteen §Channel / §Orchestration / §Agent codes plus the §Session `session.goal_delivery_failed` mapping (campaign B6) with `data.fields` shapes per [error-contracts.md](../architecture/contracts/error-contracts.md).
...
- **T3.6 — Typed-error wire round-trips.**
  - **Files:** `packages/runtime-daemon/src/ipc/__tests__/` (EXTEND).
  - **Provides:** end-to-end wire assertions for every refusal path (depth, active-child, pending, channel limit, queue depth, turn limit, budget, node locality, `channel.inactive`, `channel.name_reserved`, `agent.not_found`, `agent.not_ready`, `run.not_found` reuse, `session.goal_delivery_failed` — campaign B6).
  - **Consumes:** T3.1–T3.4.
