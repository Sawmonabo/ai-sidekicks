...
- The `run.queued` payload gains optional additive linkage fields `{agentId?, parentRunId?, linkType?, internalHelper?, producingNodeId?}` plus the admission-resolved `effectiveRunConfig?` and the server-stamped `admittedUnpricedCapCents?` (native-cap unpriced admissions) + `admittedModelFamily?` (every admission — derived pricing, family caps, and warnings all key off it) threaded from the in-process `OrchestrationRunLinkCarrier` (D-016-3, CP-004-10; ADR-018 additive-only).
...
- **T2.3 — Orchestration-run admission pipeline.**
  ...
  - **Consumes:** `getRun` (CP-016-1); Plan-004 in-process admission (CP-016-7); T2.1/T2.5/T2.6 accessors — for a native-cap unpriced admission the T2.5 accessor returns the reserved family cap, and T2.3 **stamps it as `OrchestrationRunLinkCarrier.admittedUnpricedCapCents`, and stamps the as-of-admission `admittedModelFamily` on EVERY admission — not only native-cap ones — since derived pricing keys off it** so the Plan-004 seam persists it on `run.queued` (the replay source for reservations and terminal debits; server-derived, never from the request).
