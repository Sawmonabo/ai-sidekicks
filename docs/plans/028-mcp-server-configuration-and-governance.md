...
The following invariants are **load-bearing** and MUST be preserved across all Plan-028 PRs and downstream extensions. Any change that would weaken or remove an invariant requires a coordinated cross-plan amendment (see cross-plan-dependencies.md).
...
Plan-028 EXTENDs the Plan-004-owned `command_receipts` table with one additive nullable column, `mcp_binding_digest`, carrying the governed binding a receipt's tool resolved from as a path-free keyed digest — the durable half of I-028-6. This follows the table's established EXTEND pattern exactly: Plan-015 added the BL-051 two-phase columns and Plan-005 added the additive nullable `mcp_task_id`, each under its **own** migration and neither editing a Plan-004 migration. The column is written from the same Plan-028 resolution output that already supplies `idempotency_class` to the receipt write, so no new write seam is introduced and no Plan-004-owned or Plan-015-owned file is modified. No [shared-postgres-schema.md §Lock Ordering Across Shared Tables](../architecture/schemas/shared-postgres-schema.md#lock-ordering-across-shared-tables) row is owed: every registrant there locks control-plane Postgres rows, whereas `command_receipts` and the `mcp_*` tables are local SQLite, whose single-writer transactions cannot deadlock across plans.

**Resolution.** `command_receipts` is Plan-004-owned; Plan-028 joins Plan-015 and Plan-005 as an extender, registered on the [local SQLite schema](../architecture/schemas/local-sqlite-schema.md) `command_receipts` block in the 2026-08-26 swap. Plan-004's Status does not move: the reciprocal is an extender-list entry on a table it owns — the [CP-007-15 consumer-row precedent](./007-local-ipc-and-daemon-control.md#cross-plan-obligations) for an obligation that rides existing ownership records rather than minting a reciprocal into the owner. The column lands with Plan-028's own Phase 4 migration, after Plan-004 Phase 1 has CREATEd the table — the same ordering Plan-005's `mcp_task_id` EXTEND already carries.
...
- `packages/runtime-daemon/src/bootstrap/index.ts` (EXTEND — Plan-007-owned) — one sanctioned recovery-attach wiring call per T28.4.7 (the Plan-010 `wireTurnSnapshotRetentionSweep` / Plan-016 `wireChannelDirectoryPublisher` class).
...
- **T28.4.3 — Cedar `mcp` action family registration and the deny-before-effect retrofit.**
  ...
  - Register the `mcp` action family additively through Plan-012's policy-module surface — no Plan-012-owned file is modified (CP-028-3, the CP-012-4 consumer pattern). Every one of the eight non-read operations evaluates `PermissionCheckService` **before any provider call or store write**; authorization is evaluated **before existence checks** so a deny is stable and leaks no inventory contents. A deny returns `mcp.governance_denied`; a caller who does not own this node returns `mcp.operator_scope_required` before any store or provider mutation, and the ownership predicate reads the authenticated caller and the node's ownership record only — never the transport the call arrived on, so an owner driving the node from a linked device is admitted exactly as one at the machine is (`Spec-031 §Parity by construction`). Retrofit the Phase 2–3 handlers off the `not_implemented` feature gate to full authorization in this task.
  - **Spec coverage:** Spec-028 §Authorization
  - **Verifies invariant:** I-028-4
...
- **T28.4.7 — Recovery-attach composition-root wiring and the non-vacuous production assertion.**
  - Files: `packages/runtime-daemon/src/bootstrap/index.ts` (EXTEND — Plan-007-owned; the sanctioned wiring-call edit)
...
- **T28.5.6 — CLI `sidekicks mcp` command group.**
  ...
  - Seven subcommands under the Plan-007 registered bin name; `mcp-watch.ts` tails `mcp.subscribe`. clipanion has no auto-discovery, so each file needs an explicit `.register()` on the `Cli` instance. Honors Plan-007's I-007-13 import isolation (only `@ai-sidekicks/client-sdk` / `@ai-sidekicks/contracts` / `clipanion` / Node built-ins), enforced by `apps/cli/eslint.config.mjs`. **These seven filenames are the pin audit finding F-028-12 discharges.** **`mcp-login.ts` prints the daemon-returned authorization URL as the command's own RESULT (2026-08-29)** — on the result stream `Plan-007 I-007-20` assigns results to, never as a diagnostic — because the URL is what the operator must open for the flow to proceed, and a login command that withholds it cannot complete. That is its only destination: it is not logged, not written to any file the CLI owns, and not re-emitted after settlement, and no token, authorization code, or PKCE material reaches either stream at all. The command then settles on the daemon's own completion, never on a timer.
...
- **2026-08-29 — The operator surface.** Status flipped `approved → review` and restored `approved` in the same diff, jointly with Spec-028, by the targeted readiness-audit delta riding the [Spec-005](./005-provider-driver-contract-and-capabilities.md) desktop-console parity amendment's own swap. This spec had specified a complete governance plane and named its operator surface exactly once, in passing, inside a bullet about a subscription. **Delivered in-diff:** `Spec-028 §The operator surface`, the [Spec-023](../specs/023-desktop-shell-and-renderer.md) MCP Servers View sketch (an operator-plane view on the 2026-08-26 Provider Accounts precedent, not a V1 Signature Feature), **CP-028-8** ⇄ Plan-023 CP-023-7 registering the owner-authors / host-mounts split on the precedent, and the T28.5.7 EXTEND taking that task from panel views to the full settings page with its four tests. **No wire surface is added**: the operation count stays **eleven**, the event count **five**, and the error registry **ten**, so no census moves and no table or column is minted. Dispatch order is unchanged — T28.5.7 already sat in Phase 5 behind the Plan-023-partial substrate, and CP-028-8 adds no YAML gate, since a page mounted before Plan-023's Tier-8 shell exists is unreachable rather than wrong. **Round-3 fold (2026-08-29).** The Spec-028 acceptance criterion this swap added prohibited rendering configuration **as such**, which a test implemented literally from it would have used to reject the very edit workflow §The operator surface, T28.5.7, and the Spec-023 criterion all require. It is narrowed to **withheld and secret** values — env-var values, header values, URL query values, tokens — with the daemon's redacted `McpServerConfigView` read-back explicitly permitted, so the three-way split reads the same way in all four places. Nothing normative moves and no census is touched.
