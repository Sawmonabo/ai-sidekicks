...
**Resolution.** On purge, the `session_memberships` rows referencing the purged participant are **anonymized in place** — `participant_id` is repointed to the per-purge tombstone identity rather than hard-DELETEd — so session-history membership lineage and the `sessions` → `session_memberships` referential graph survive the purge. Spec-022 keeps `session_invites` + `session_memberships` as anonymize-not-delete targets for exactly this referential-integrity reason (Spec-022:294-296 §Shred Fan-Out Path 2, rationale Spec-022:67). Reciprocal of Plan-022 CP-022-6 (encoded fix-in-place at the Tier-5 audit swap, satisfying Plan-022 I-022-19); shred-handler provider: Plan-022 (V1.1, owns the cross-store fan-out).
