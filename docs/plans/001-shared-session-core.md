...
| 2026-07-28 | `SessionService.append` guarded test-only + emitter seam decoupled (Plan-006 T3.1 precondition — guard + decouple legs) | Enhancement-lane PR #272, a post-completion change to this plan's shipped code authorized by the Plan-006 Phase 3 T3.1 precondition (added 2026-07-27 by PR #256 Codex round 3): `append()` now throws unless the service was constructed with the explicit test-only `allowUnsignedPlaceholderAppend` opt-in — the zero-fill placeholder rows it writes are exactly what Plan-006's `verifyRow` refuses fail-closed (`signature_placeholder`), so no production composition root can reach the unsigned path by accident. The option is typed as the literal `true` so it cannot be env-derived without a cast. Reads (`readEvents`/`replay`) need no opt-in. All 14 in-tree construction sites sit under `__tests__/`; every site that exercises `append` opts in (10 of 14), and the remaining four deliberately do not — two default-constructed reopen sites proving reads need no opt-in, plus the guard suite's negative control and read-only proof — so the opted-in green suite is not vacuous. Plan-003's `RuntimeNodeEventEmitter` was simultaneously re-typed onto the structural `SessionEventLog` seam — see Plan-003 §Progress Log §Notes, same date. The §Data And Storage Changes placeholder-convention bullet carries the discharge note. |
