...
| 2026-08-10 | Phase 2/3 test-file path errata + T2.1 direct-coverage completion (erratum) | Follow-up to the D5 path-correction erratum above — same root cause, swept as a unit. Both `packages/contracts/vitest.config.ts` and `packages/runtime-daemon/vitest.config.ts` discover tests via the `src/**/__tests__/**/*.test.ts` glob, so every Phase 2/3 Files-row path under a bare `test/` directory was never discoverable — and none ever existed in history: T2.1's `packages/contracts/test/session.test.ts` shipped (PR #8) as a three-file split `src/__tests__/session-id.test.ts` + `src/__tests__/session-create.test.ts` + `src/__tests__/session-join.test.ts`; T2.2's `test/event.test.ts` shipped as `src/__tests__/session-event.test.ts`; T2.3's `test/error.test.ts` shipped as `src/__tests__/error.test.ts`; T3.2's and T3.3's `session/test/` paths shipped (PR #9) at `session/__tests__/`. T3.4 is already adjudicated by the D5 erratum. The Files rows are kept verbatim per the keep-row-plus-erratum convention; future audits should reconcile the prose against the shipped reality. The split also left the T2.1 promise partially met: the SessionRead and SessionSubscribe schema families had NO direct unit coverage in the contracts package (exercised only transitively via consumer suites — daemon IPC handler tests, control-plane SSE factory), and `SessionJoinResponseSchema` likewise had none (the join suite pinned only the request-side hardening). Completed this date by `packages/contracts/src/__tests__/session-read.test.ts` + `packages/contracts/src/__tests__/session-subscribe.test.ts` plus a `SessionJoinResponseSchema` block appended to `packages/contracts/src/__tests__/session-join.test.ts` (this PR), pinning request/response strictness at every nesting level, the optional replay-cursor arms (`afterCursor` / `lastEventId`), RFC 3339 offset acceptance on snapshot datetimes, cursor length bounds, and the join-response field/UUID guards. |
