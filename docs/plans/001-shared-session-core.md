...
- `packages/control-plane/src/migrations/0001-initial.ts` migration creates `participants` (minimal anchor: `id`, `created_at`), `sessions` (with `min_client_version`), `session_memberships` (per § Data And Storage Changes), plus the canonical indexes per [Shared Postgres Schema §Sessions and Membership](../architecture/schemas/shared-postgres-schema.md#sessions-and-membership-plan-001-plan-002) (`idx_sessions_state`, `idx_session_memberships_session`, `idx_session_memberships_participant`).
