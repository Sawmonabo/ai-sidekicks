...
- **Windows kill-translation tests:**
  ...
  - Test K2 (Phase 3 sidecar): `packages/sidecar-rust-pty/src/kill_translation.rs::tests` — asserts `pty_session.rs::kill()` Windows path emits `CTRL_C_EVENT` then escalates `CTRL_BREAK_EVENT` on bounded-timeout; per I-024-1.
