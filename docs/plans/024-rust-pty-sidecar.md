...
- **Cwd / WSL / worktree tests:**
  ...
  - Test W3 (Phase 3, `windows-latest` CI only, gated by `RUN_W3_INTEGRATION=1` AND sidecar-binary availability via the four-tier resolver): `packages/runtime-daemon/test/pty/no-error-sharing-violation.test.ts` — spawns a long-running session with logical worktree cwd; asserts `git worktree remove <worktree-path>` succeeds without `ERROR_SHARING_VIOLATION` while session runs; per I-024-5.
