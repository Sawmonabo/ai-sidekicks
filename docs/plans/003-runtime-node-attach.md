...
| **Dependencies** | [Plan-001](./001-shared-session-core.md) (session model, forward-declared `session_events` integrity columns, and the `sessions.min_client_version` floor column — Plan-001 ships the physical column; Plan-003 picks up the read/write floor **semantics** at Tier 3). **The `runtime_node_attachments`/`runtime_node_presence` Postgres tables are Plan-003-owned, not Plan-001's** — Plan-003 CREATEs them in its own control-plane migration (Phase 3), per the `-- Owner: Plan-003` stamps in [shared-postgres-schema.md](../architecture/schemas/shared-postgres-schema.md); [Plan-007 bootstrap-deliverable](./007-local-ipc-and-daemon-control.md) (Tier 1 daemon JSON-RPC IPC substrate `packages/runtime-daemon/src/ipc/` incl. the namespace `registry.ts` and the `METHOD_NAME_FORMAT` guard — Plan-003 registers its runtime-node method handlers under it, an EXTEND of that Plan-007-owned substrate); [Plan-006](./006-session-event-taxonomy-and-audit-log.md) (`runtime_node.*` event taxonomy registration at Tier 4 — Plan-003 emits the 5 daemon-authored events in V1 at Tier 3 as event-shape stubs against the Plan-001 forward-declared columns; the per-event **payload-shape** Zod schemas are Plan-003-owned (CREATE — the 5 daemon-reachable shapes in Phase 2; the `degraded`/`revoked` durable-event schemas are V1.1-gated on the node-identity trust anchor per [ADR-017 §Server-Derived Runtime-Node Lifecycle Events](../decisions/017-shared-event-sourcing-scope.md#server-derived-runtime-node-lifecycle-events), their server-derived producers having no sound V1 author — V1 records those transitions as coordination records), while the additive Plan-006 Tier-4 follow-up is discriminated-union registration + the `EventEnvelope` integrity wrapper + `CapabilityDetails` binding, not the payload shapes); the Tier 1 control-plane bootstrap-deliverable (the tRPC v11 `sessionRouter` substrate, now owned by [Plan-031](./031-remote-control.md) — Plan-003's runtime-node calls — the four dual-transport mutations (`attach`/`heartbeat`/`capabilityupdate`/`detach`) plus the control-plane-only `runtimenode.roster` query added by the user-directed Phase 5 scope expansion (2026-06-09, PR #150) — cross the same control-plane transport as the sibling `runtimeNodeRouter`'s `runtimenode.*` procedures). [Spec-024](../specs/024-cross-node-dispatch-and-approval.md) is **not** a dependency for Plan-003 — historical Session H-interim header reference; cross-node dispatch implementation belongs to [Plan-027](./027-cross-node-dispatch-and-approval.md). |
...
### I-003-6 — Runtime-node transactions acquire locks in the canonical order, in the recorded modes
...
### CP-003-4 — `runtime_node_attachments` is a Path-2 user-erasure target (⇄ Plan-022 CP-022-6)

`runtime_node_attachments` (Plan-003-owned — the `-- Owner: Plan-003` stamp in [shared-postgres-schema.md §Runtime Node Attachments](../architecture/schemas/shared-postgres-schema.md#runtime-node-attachments-plan-003)) carries `user_id UUID NOT NULL REFERENCES users(id)`, so it is inside the `REFERENCES users(id)` inbound-foreign-key closure that [Plan-022](./022-data-retention-and-gdpr.md)'s GDPR Art. 17 Path-2 user-erasure fan-out must address ([CP-022-6](./022-data-retention-and-gdpr.md#cross-plan-obligations)). On a user-erasure request, the V1.1 shred handler **hard-DELETEs** the user's `runtime_node_attachments` rows: these are operational node-attach state (the durable node-attach audit trail lives in the crypto-shredded `runtime_node.*` event stream — Path 1 — not in this table), so there is no independent retention basis and no referential-integrity reason to anonymize-and-retain.

**Resolution.** This is the live in-tier reciprocal for Plan-022 CP-022-6, added fix-in-place at the Tier-5 audit swap (2026-05-31 — the F3 round-4 Codex finding that the Path-2 closure omitted this table). No Plan-003 V1 code change: the erasure handler is V1.1 (`Plan-022 §Non-Goals`), and `runtime_node_attachments` already exists with its `user_id` FK. This entry encodes the obligation so the closure carries no one-sided edge (Plan-022 I-022-19).

### CP-003-5 — Plan-006 T4.10 extends the runtime-node method registry with signing-key registration + resolution (⇄ Plan-006 CP-006-7 leg B)

Registered 2026-07-29 by the CP-006-7 two-leg amendment. The runtime-node surface is the carrier for daemon signing-key roster registration: `docs/architecture/security-architecture.md §Per-Event Daemon Signature` requires the daemon's session-scoped Ed25519 PUBLIC key to be registered against the session at admission time, keyed by `NodeId`, and the runtime-node control-plane surface is where node identity and session identity meet through an authenticated transport. [Plan-006 T4.10](./006-session-event-taxonomy-and-audit-log.md#cross-plan-obligations) EXTENDs two Plan-003-owned surfaces, none re-authored: (a) `packages/contracts/src/runtime-node.ts` gains four additive NEW schema exports — the `runtimenode.signingkeyregister` / `runtimenode.signingkeyroster` request/response pairs — with NO shipped schema mutated: every Plan-003 request/response schema in the file is `.strict()`, so a member added to a shipped shape (the amendment's original attach-request-field carrier, withdrawn in the same PR's review round) would break the new-daemon→old-control-plane skew direction at parse per `ADR-018 §Decision` #7, which is why the carrier is a new procedure pair rather than an attach field; (b) `packages/control-plane/src/runtime-nodes/runtime-node-router.factory.ts` takes two procedure-key registrations on the shared builder (the Plan-024 `leaseupdate` precedent) and `packages/client-sdk/src/runtimeNodeClient.ts` gains the two typed pass-throughs. The store itself is NOT a Plan-003 surface: the key persists register-once per `(session_id, node_id)` in the new Plan-006-owned `daemon_signing_public_keys` table (different-key registration refused with the typed `runtimenode.signingkeyregister_conflict` error before any row mutation — never a silent overwrite, mirroring Plan-006 T4.2's `refuse_on_rotation` posture; the 2026-08-11 admission-time registration decoupling deleted the round-5 `attachmentId` equality and its attachment-ownership sibling, so registration keys on admission with NO attachment read at all and the transaction no longer touches any Plan-003-owned row — the producer binding is the daemon-bound CP-006-13 credential, the widening named at `security-architecture.md §Per-Event Daemon Signature` — and the register and resolve refusals share the typed `runtimenode.permission_denied` (403) per the error-contracts §Runtime Node registry), deliberately NOT a `runtime_node_attachments` column — CP-003-4 above records that table's `Spec-022 §Shred Fan-Out` hard-DELETE disposition as operational node-attach state, while the verification key must SURVIVE user erasure to keep the retained crypto-shredded `runtime_node.*` stream and `event_log_anchors` rows verifiable; the register/resolve service lands as the Plan-006-owned sibling file `signing-key-service.ts` in this plan's `runtime-nodes/` directory (the Plan-024 `lease-projection-service.ts` per-file-crossing precedent). `attach-service.ts` and the attach wire contract are untouched.

**Resolution.** Plan-003's shipped phases are not re-opened — all five are shipped (Shipment Manifest), the attach wire contract is untouched (`Spec-003 §Interfaces And Contracts` remains accurate as pinned; the new procedures register in the api-payload-contracts method registry, the `runtimenode.leaseupdate` precedent), and the code leg belongs to Plan-006 T4.10 (Plan-006's Phase 4, the read-side/verifier phase — the placement matches the leg's consumer: it serves the verification-resolution surface, not Plan-006's append path), the same routing shape as the Spec-003 terminal-control amendment whose code leg belongs to Plan-024 Phase 3B. Plan-003's file records the obligation so the one-writer discipline carries the sanctioned edits rather than being silently violated.

### CP-003-6 — Renderer attach view forwards the server-minted `attachmentId` to the daemon (⇄ Plan-006 CP-006-14)

Registered 2026-08-01 by the Plan-006 T4.10 targeted readiness-audit delta; **RETRACTED 2026-08-11 by the admission-time registration decoupling** (reciprocal [Plan-006 CP-006-14](./006-session-event-taxonomy-and-audit-log.md#cross-plan-obligations), retracted in the same swap). The obligation existed to deliver the attach response's server-minted `runtime_node_attachments.id` to the daemon, because T4.10's register request presented it as a producer-binding equality and no daemon-side attach lifecycle exists to observe it. The decoupling removed that member and predicate outright — registration keys on session admission with the daemon-bound CP-006-13 credential as the producer binding, and the registrar's node identity resolves from the daemon's own shipped Phase-2 node registry (`node-registry.ts` over `node_trust_state`, a read-only constructor-injected consumption; no seam edit) — so the entire delivery chain this row sanctioned is deleted with the machinery that carried it: the renderer forward and reconnect replay never dispatch, `AttachFlow.tsx` keeps its shipped single-procedure header claims byte-intact (no re-derivation is owed), and the renderer-state-loss residual with its documented fresh-attach remediation dies too. Obligation numbers are never reused; this entry stands as the retraction record.

## Preconditions

- [x] Paired spec is approved — dated note (2026-07-18, retiring the 2026-07-13 note): Spec-003 returned to `approved` via the campaign Task-28 / W1.5 batch re-promotion. All five of this plan's phases are shipped (Shipment Manifest) — no code dispatch currently gates on this box; the amendment's code leg belongs to Plan-024 Phase 3B. **Re-opened 2026-08-03, scoped to the shared-terminal write-lease projection-conformance amendment:** [Spec-003](../specs/003-runtime-node-attach.md) pins two new normative §Required Behavior constraints on the `runtimenode.leaseupdate` projection — the `offline`-signal producer re-bind disjunct and the direction-asymmetric publish predicate — a Required-Behavior change, so the spec flips `approved → review` per the runbook's [§Spec-Status Promotion Gate](../operations/plan-implementation-readiness-audit-runbook.md#spec-status-promotion-gate) amendment rule, this box un-checks with it, and Status flips `approved → review` alongside (the [§Status Flip Rule](../operations/plan-implementation-readiness-audit-runbook.md#status-flip-rule) behavior-change row, reaching this plan through the paired-spec dependency rather than through any edit to its own body). The §Plan-readiness audit box below stays checked — the amendment mints no Plan-003 task, invariant, or cross-plan obligation, and its enforcement + test growth land on Plan-024's Phase 3B `T-024-3B-3` leg — so the restoring vehicle is the Spec-003 targeted readiness-audit delta, which re-checks this box and restores both Statuses in one swap (the PR #282 flip-and-restore shape). Dispatch impact is nil in practice: all five phases are shipped, so no lane-1 Plan-003 PR waits on this box. **Restored 2026-08-12** by the targeted readiness-audit delta this box named, which covers **two** amendment scopes in one vehicle under the campaign's dual-flip gate: the 2026-08-03 projection-conformance growth recorded above, and the BL-141 caller-authorization growth that [ADR-025](../decisions/025-runtime-node-control-plane-caller-authorization.md) mandates. The "mints no Plan-003 task, invariant, or cross-plan obligation" clause above is scoped to the **2026-08-03 amendment**, which remains true of it; the BL-141 scope does mint Plan-003 growth — T3.10–T3.12 and I-003-6 — and is therefore audited by the same delta rather than deferred, which is why the §Plan-readiness audit box below records its own re-open-and-re-check in the same swap.
- [x] Required ADRs are accepted
- [x] Blocking open questions are resolved or explicitly deferred
- [x] **Plan-readiness audit complete per [`docs/operations/plan-implementation-readiness-audit-runbook.md`](../operations/plan-implementation-readiness-audit-runbook.md)** — Tier 3 audit landed via; see [Status Promotion Gate §1](../operations/plan-implementation-readiness-audit-runbook.md#status-promotion-gate). Per-Phase `#### Tasks` blocks authored (Phases 1–5) and cite anchors corrected in-PR; the runtime-node table-CREATE-ownership self-misattribution (header + Phase 1) corrected to Plan-003-owned. Cross-cutting contract/schema fills (`clientVersion` on `RuntimeNodeAttachRequest`, the below-floor read-only attach representation, the JSON-RPC/tRPC method namespace, and the node↔session cardinality) plus the Spec-003 heartbeat degraded→offline threshold are dispositioned in this PR — the wire shapes in [`api-payload-contracts.md`](../architecture/contracts/api-payload-contracts.md) (the `clientVersion` field, the derived `readOnly` flag, and the Runtime-Node Method-Name Registry), the cardinality as [§Invariants I-003-5](#invariants), and the remainder in the per-task Contract-dependency notes below — per the [runbook §Cross-Tier Amendment Contingency](../operations/plan-implementation-readiness-audit-runbook.md#cross-tier-amendment-contingency); the `Spec-003 §Default Behavior` threshold addition re-discharges the Spec-Status Promotion Gate via a separate Spec-003 amendment PR ([BL-140](../archive/backlog-archive.md)). **Re-opened and re-checked 2026-08-12, scoped to the BL-141 caller-authorization growth ([ADR-025](../decisions/025-runtime-node-control-plane-caller-authorization.md)):** that amendment mints three Phase-3 tasks (T3.10–T3.12) and adds I-003-6, which is plan-body growth the runbook's [§Status Flip Rule](../operations/plan-implementation-readiness-audit-runbook.md#status-flip-rule) re-opens this box for. The growth and its readiness audit land in the **same** PR — the targeted delta restoring both Statuses — so this box is re-opened and re-checked within one commit and the marker never rests un-checked in a committed state; recorded here rather than left implicit so the swap is auditable. No re-audit of Phases 1–5 is implied: the delta's scope is the BL-141 growth plus the 2026-08-03 reconciliation growth, and Phases 1–5 remain covered by the Tier-3 audit landed via.
...
| --- | --- | --- | --- |
| C1 | `RuntimeNodeAttach payload validates required fields including client_version` | request schema | `Spec-003 §Interfaces And Contracts` (required fields), `Spec-003 §Required Behavior` (client_version) |
| C2 | `RuntimeNodeCapabilityUpdate payload supports add/remove/health-change variants` | mutation contract | `Spec-003 §Interfaces And Contracts` |
| C3 | `RuntimeNodeDetach payload validates session id + node id + reason` | retire contract | `Spec-003 §Interfaces And Contracts` |
...
| C5 | `VERSION_FLOOR_EXCEEDED error contract matches ADR-018 typed shape` | error contract | `Spec-003 §Required Behavior`, AC4 |
| C6 | `RuntimeNodeHeartbeat payload validates node id + health state` | presence-update contract | `Spec-003 §Interfaces And Contracts` |
...
| D2 | `Capability declaration service emits runtime_node.capability_declared on success` | event emission | `Spec-003 §State And Data Implications` |
| D3 | `runtime_node.online MUST NOT emit until runtime_node.capability_declared lands` | I-003-2 ordering invariant | I-003-2, `Spec-003 §Default Behavior` |
| D4 | `Detach emits runtime_node.offline; subsequent reconnect under same node id succeeds` | reconnect identity | `Spec-003 §Fallback Behavior`, `Spec-003 §Implementation Notes` |
...
| ID | Test | Asserts | Spec-003 AC / MUST |
| --- | --- | --- | --- |
| P1 | `RuntimeNodeAttach with NULL min_client_version floor admits all daemon versions` | NULL-floor permissive | `Spec-003 §Required Behavior` |
| P2 | `RuntimeNodeAttach with client_version >= floor admits with full read/write` | happy-path floor | `Spec-003 §Required Behavior`, AC4 |
| P3 | `RuntimeNodeAttach with client_version < floor admits in read-only state — node remains joined and reads succeed` | I-003-1 admit-in-read-only invariant | I-003-1, `Spec-003 §Required Behavior` |
...
| P5 | `Multiple runtime nodes can attach to the same session without changing session identity` | multi-node co-existence | AC3, `Spec-003 §Required Behavior` |
| P6 | `Heartbeat ingestion updates runtime_node_presence; missed heartbeat past threshold transitions health_state to degraded then offline (coordination record — no durable event in V1)` | health transitions | `Spec-003 §Default Behavior` (cadence; `degraded` older than `30s` / `offline` older than `60s`; control-plane sweep, no durable event) |
...
| ID | Test | Asserts | Spec-003 AC / MUST |
| --- | --- | --- | --- |
| I1 | `User opens a live session, then attaches a local runtime node — session not recreated` | live attach | AC1, `Spec-003 §Required Behavior` |
...
- All 23 enumerated tests above pass before Plan-003 is marked complete; Phase 5's renderer projection is gated by the manual multi-node attach smoke (the load-bearing floor and attach semantics are already proven by Phases 1–4 — C1–C6, D1–D6, P1–P6 and P9–P10, I1–I3). Automated renderer **component** coverage landed 2026-08-25 as the BL-131 exit-criterion-(b) backfill; the **E2E** coverage that replaces the manual smoke stays a criterion-gated V1.1 backfill tracked in BL-131 (not a V1/Tier-3 gate). **(Amended 2026-06-09, PR #150: the user-directed Phase 5 roster-read scope expansion adds automated contracts/control-plane/SDK coverage for `runtimenode.roster` — T5.0b–T5.0d — on top of the 23; the manual smoke remains the renderer-projection gate.)**

## Implementation Phase Sequence

Plan-003 implementation lands as a sequence of small PRs at Tier 3. Phases 1–4 ship the contract/daemon/control-plane/SDK layers; Phase 5 ships the renderer subtree after Phase 4 lands the SDK (the renderer substrate is created independently by Plan-023 Tier 1 Partial per BL-101 (a) resolution). Plan-003 depends only on Plan-001 completion.

### Phase 1 — Node Contracts + Migrations

**Precondition:** Plan-001 complete (Tier 1 substrate carve-outs + forward-declared `session_events` integrity columns + the `sessions.min_client_version` column in place). Plan-001 does **not** create the `runtime_node_attachments`/`runtime_node_presence` Postgres tables — those are Plan-003-owned and created in Phase 3 (see §Dependencies).

**Goal:** Tests C1–C6 go green; the daemon SQLite migration for `node_capabilities`/`node_trust_state` lands; Phase 1 confirms the Plan-001 upstream anchors (`users`, `sessions.min_client_version`) that the Plan-003 Postgres tables will FK/read (the Postgres tables themselves are created in Phase 3).

- `packages/contracts/src/runtime-node.ts` — `RuntimeNodeAttach`, `RuntimeNodeHeartbeat`, `RuntimeNodeCapabilityUpdate`, `RuntimeNodeDetach` payloads, plus the `NodeId` (string brand) and `NodeState` types. (The `NodeId`/`NodeIdSchema`/`NODE_ID_MAX_LEN` declarations were later hoisted verbatim to the dependency-free leaf `packages/contracts/src/node-id.ts` by Plan-009 T1.2 to break an import cycle; `runtime-node.ts` re-exports all three, and Plan-003 ownership is unchanged.)
- `packages/contracts/src/error.ts` — **no authoring.** The `VersionFloorExceededError` shape + `VersionFloorExceededErrorSchema` + `VERSION_FLOOR_EXCEEDED_CODE` (literal `version.floor_exceeded`) already shipped at Plan-001 T2.3. Plan-003 **imports** this shape; C5 asserts the existing export conforms to `ADR-018 §Decision` #10. Plan-003 owns the **emit site** (control-plane attach/write path, Phase 3), not the contract.
- Local SQLite migration (Plan-003-owned, daemon `0002-runtime-node.ts`): `node_capabilities`, `node_trust_state`.
- Confirm the Plan-001 upstream schema (read-only): the `users` identity anchor + `sessions.min_client_version` (Plan-003 FK-references / reads; does not duplicate-CREATE). The Plan-003-owned `runtime_node_attachments`/`runtime_node_presence` Postgres tables are created in the Phase 3 control-plane migration, **not** here and **not** by Plan-001.
...
- **Spec coverage:** Spec-003 §Interfaces And Contracts (RuntimeNodeAttach required fields), Spec-003 §Required Behavior (`client_version` floor field)
...
- **Spec coverage:** Spec-003 §Interfaces And Contracts (capability additions, removals, health changes)
...
- **Spec coverage:** Spec-003 §Interfaces And Contracts (RuntimeNodeHeartbeat updates presence/health; RuntimeNodeDetach retires/disconnects a node)
- **Verifies invariant:** none (contract-schema definition)
...
- **Spec coverage:** Spec-006 §Runtime Node Lifecycle (runtime_node_lifecycle) (payload base; the full seven-row `runtime_node.*` set the C4 conformance test re-derives — registered / online / degraded / offline / revoked / capability_declared / capability_updated)
...
- **Spec coverage:** Spec-003 §Required Behavior (typed `VERSION_FLOOR_EXCEEDED` on below-floor write), Spec-003 AC4
...
- **Spec coverage:** Spec-003 §State And Data Implications (runtime-node records durable for reconnect/audit)
...
- **Step:** Confirm — by reading the shipped Plan-001 migrations, NOT by creating tables — that (a) the Postgres `runtime_node_attachments`/`runtime_node_presence` tables are Plan-003-owned and created at Phase 3 (control-plane), and (b) Plan-001 already ships the `users` identity anchor and `sessions.min_client_version TEXT` that Plan-003 FK-references / reads. A guard that Plan-003 does not duplicate-CREATE upstream tables.
- **Test:** assert (via migration introspection) that no Plan-003 Phase-1 migration CREATEs `runtime_node_attachments`/`runtime_node_presence` (deferred to the Phase-3 Postgres migration), and that the Plan-001-shipped `users` anchor + `sessions.min_client_version` column are present for Plan-003 to FK-reference / read.
...
- **Spec coverage:** Spec-006 §Runtime Node Lifecycle (runtime_node_lifecycle) (payload base; session binding; per-event payload shapes); api-payload-contracts.md §Plan-006 (capability payload typing).
...
- **Spec coverage:** Spec-003 §State And Data Implications (durable runtime-node records), Spec-003 §Implementation Notes (node identity stable across reconnect), Spec-003 AC1.
...
- **Spec coverage:** Spec-003 §Default Behavior (least-privilege schedulability), Spec-003 §State And Data Implications (capability/trust changes emitted as session events — covers both declaration and update), Spec-003 §Pitfalls To Avoid (no implicit capability exposure on attach).
...
- **Spec coverage:** Spec-003 §State And Data Implications (capability/trust changes emitted as session events); Spec-006 §Runtime Node Lifecycle (runtime_node_lifecycle) (per-event payload shapes)
...
- **Spec coverage:** Spec-003 §Fallback Behavior (reconnect under same node identity after disconnect — durable-row behavior; node identity stable across reconnect).
...
- **Spec coverage:** Spec-003 §Default Behavior (online requires daemon-side declaration; the 2026-06-04 `capabilityupdate` amendment — the daemon self-report axis is `online ↔ degraded`).
...
- **Spec coverage:** Spec-003 §Required Behavior (NULL floor permits all daemons).
- **Verifies invariant:** I-003-5 (upsert reactivates an `offline` row; the `idx_node_attachments_active` partial unique index enforces single-active-session). Revocation terminality (a `revoked` row is not reactivated) is grounded in [runtime-node-model.md](../domain/runtime-node-model.md), verified by P10.
...
- **Spec coverage:** Spec-003 §Required Behavior (verify daemon version against floor; below-floor admitted read-only).
...
- **Spec coverage:** Spec-003 AC4 (`VERSION_FLOOR_EXCEEDED` on write, never ejected).
...
- **Spec coverage:** Spec-003 §Required Behavior (multiple runtime nodes per session), Spec-003 AC3 (multiple nodes coexist without changing session identity).
...
- **Spec coverage:** Spec-003 §Default Behavior (heartbeat cadence 15s; degraded `30s` / offline `60s` thresholds; control-plane staleness-sweep owner)
...
- **Step:** Author the detach method on `AttachService`. Detach resolves the node's single active attachment by `nodeId` (unambiguous per §Invariants I-003-5) and updates that `runtime_node_attachments.state` (→ `offline` for a clean disconnect, `revoked` for a trust revocation) and `runtime_node_presence.health_state`. The attach and detach paths write only `runtime_node_attachments` / `runtime_node_presence` — no other table is touched, asserted via test-visible query logging.
...
- **Spec coverage:** Spec-003 §Required Behavior (detach or offline state does not tear down the session).
- **Verifies invariant:** I-003-5 (the single active attachment is what detach resolves).
...
- **Spec coverage:** Spec-003 §Required Behavior (control plane coordinates discovery/presence; execution stays local).
...
- **Spec coverage:** Spec-003 §Required Behavior (caller authorization across the `runtimenode.*` surface, the uniform refusal, and the narrowed 409), Spec-003 §Default Behavior (the server-derived staleness sweep whose integrity the presence-forgery fix restores).
- **Verifies invariant:** I-003-6 (canonical lock order, recorded modes, two-phase resolve with re-verify); I-003-5 (the single-active-attachment resolution the two-phase pre-read keys on).
...
- **Spec coverage:** Spec-003 §Required Behavior (the session-ownership precondition on attach and on the roster read, and the version floor the level-1 `sessions` read resolves), Spec-003 §Interfaces And Contracts (the `RuntimeNodeRoster` faithful-projection contract the predicate must not narrow — every row verbatim, no server-side hiding).
- **Verifies invariant:** I-003-1 (admit-not-eject is untouched — a below-floor daemon that passes authorization is still admitted read-only); I-003-6 (level-1 then level-2 acquisition order and modes on the attach path).
...
- **Spec coverage:** Spec-003 §Required Behavior (every authorization arm the amendment pins, the uniform refusal, and the no-oracle rule), Spec-003 §Interfaces And Contracts (the roster projection stays faithful for a permitted caller).
- **Verifies invariant:** I-003-5 (the global single-active-attachment constraint); I-003-6 (order **and** mode, plus the two-phase re-verify).
...
- **Spec coverage:** Spec-003 §Interfaces And Contracts (RuntimeNodeAttach fields; RuntimeNodeHeartbeat updates presence and health; RuntimeNodeCapabilityUpdate add/remove/health variants; RuntimeNodeDetach retires a node).
...
- **Spec coverage:** Spec-003 AC1 (user attaches a local runtime node to an already active session), Spec-003 §Required Behavior (attach must not require session recreation).
...
- **Spec coverage:** Spec-003 AC2 (a degraded or offline node remains distinguishable from a healthy online node), Spec-003 §Fallback Behavior (capability-validation failure leaves the node `degraded` — the self-reported capability-health axis driven here), Spec-003 §Default Behavior (the `capabilityupdate` response `state` is the server-derived full `NodeState`; the effective roster state reconciles both independent axes).
...
- **Spec coverage:** Spec-003 AC4 (below-floor daemon admitted read-only, `VERSION_FLOOR_EXCEEDED` on subsequent write, never ejected).
...
**Scope expansion (user-directed 2026-06-09, PR #150).** Phase 5 additionally ships the full `runtimenode.roster` read path end-to-end — governance pin (T5.0a), contracts schemas (T5.0b), control-plane read service + the namespace's first router `query` (T5.0c), and the SDK roster query on the control-plane factory (T5.0d) — pulling the reconciled roster-read (presence × slot) **in-scope** rather than extending the T4.3 "downstream of this plan" deferral, per the standing hardened-implementation directive. T5.0a–T5.0d are orchestrator-introduced, so the audit 1:1-coverage rule applies only to T5.1–T5.4. The contract is pinned in [Spec-003 §Interfaces And Contracts (2026-06-09 amendment)](../specs/003-runtime-node-attach.md#interfaces-and-contracts) and the [Runtime-Node Method-Name Registry](../architecture/contracts/api-payload-contracts.md#runtime-node-method-name-registry-tier-3): `runtimenode.roster` is a **`query`** (its four namespace siblings are mutations), **control-plane tRPC only** — the roster is control-plane-owned cross-node state; a daemon knows only itself — and the read projects coordination records, authoring no durable event (no collision with the [ADR-017 §Server-Derived Runtime-Node Lifecycle Events](../decisions/017-shared-event-sourcing-scope.md#server-derived-runtime-node-lifecycle-events) V1.1 gate).
...
- **Spec coverage:** Spec-003 AC2 (a degraded or offline node remains distinguishable from a healthy online node), Spec-003 AC3 (multiple nodes coexist without changing session identity), Spec-003 §Required Behavior (multiple runtime nodes per session), Spec-003 §Default Behavior (both health axes reconciled client-side; the read-side projection now pinned).
...
- **Step:** author `RuntimeNodeRosterRequestSchema` (`.strict()`: `sessionId`), `RuntimeNodeRosterEntrySchema` (the nine-field entry pinned in `Spec-003 §Interfaces And Contracts` — `nodeId`, `userId`, `state: NodeState` carrying all five values, nullable `healthState` on the 3-value presence enum (`online | degraded | offline`, mirroring the `runtime_node_presence.health_state` CHECK), nullable `lastHeartbeatAt`, `readOnly`, `capabilities`, `clientVersion`, `attachedAt`), and `RuntimeNodeRosterResponseSchema` (`{ nodes: RuntimeNodeRosterEntry[] }` as built 2026-06-09; the 2026-07-13 campaign-B4 write-lease amendment extends the response contract with `controlHolder: UserId | null` — that schema extension is implemented by Plan-024's Phase 3B lease leg per `Spec-003 §Interfaces And Contracts`, not by re-opening this completed task — **landed 2026-09-06, and only the schema half:** `RuntimeNodeRosterResponseSchema` carries the key today, shipped under the [Plan-023](./023-desktop-shell-and-renderer.md) Phase 1C console settings family, where the runtime-nodes page renders the holder and a client parsing the shipped mirror could read it nowhere. The key is REQUIRED with a nullable value, so an omitted member is a parse failure rather than a silent "nobody holds it"; the value-producing `session_terminal_leases` join is still Plan-024 Phase 3B's), mirroring the registry wire shapes. Non-input projection schemas follow the file's single-`T` house style (the T2.0 precedent). Also re-anchor the T5.0a-drifted `Spec-003`/`api-payload-contracts.md` cites in this task's files (content-verified mappings carried in the 2026-06-09 scope-expansion dispatch DAG).
...
- **Spec coverage:** Spec-003 §Default Behavior (the wire carries both axes verbatim — no collapsed scalar), Spec-003 AC2 (`degraded`/`offline` representable on the wire), Spec-003 §Required Behavior (multiple runtime nodes per session — `nodes[]`).
- **Verifies invariant:** none (contract-schema definition).
...
- **Scope note:** orchestrator-introduced under the user-directed 2026-06-09 scope expansion.
- **Step:** add `readRoster(request: RuntimeNodeRosterRequest): Promise<RuntimeNodeRosterResponse>` on `AttachService`: SELECT **all** `runtime_node_attachments` rows for the session (all five `state` values verbatim — no server-side hiding; row count bounded by distinct nodes ever attached under the `(node_id, session_id)` uniqueness), LEFT JOIN `runtime_node_presence` on `node_id` carrying `health_state`/`last_heartbeat_at` verbatim (NULL until the node's first heartbeat), and derive `readOnly` per row from the stored `client_version` vs `sessions.min_client_version` (NULL floor → `false`; identical semantics to the attach-time verdict). The read MUST NOT derive staleness (the T3.6 sweep stays the single liveness-derivation writer) and MUST NOT collapse the two axes. (2026-07-13, campaign B4: the response additionally carries `controlHolder: UserId | null` from the `session_terminal_leases` join once Plan-024 Phase 3B ships the lease table — the read extension rides that leg per `Spec-003 §Interfaces And Contracts`; this completed task's attachments × presence join is unchanged until then. **Landed 2026-09-06, and only the projection:** this service composes `controlHolder: null` today, shipped under the [Plan-023](./023-desktop-shell-and-renderer.md) Phase 1C console settings family, because the schema key that landed in the same change is required and the only shipped producer of this reply would otherwise fail its own parse. `null` is a reading rather than a placeholder — no lease row can exist before the table does, and the read-time liveness predicate that will suppress a holder behind an offline producer serves the same value — so the join itself is still Plan-024 Phase 3B's.) Mount `runtimenode.roster` as the router's first `query` — control-plane tRPC only, no daemon JSON-RPC registration — and reconcile the router/SDK "all four are mutations / no query" comments in the touched files. Emit no durable event (ADR-017 non-collision). Also re-anchor the T5.0a-drifted `Spec-003`/`api-payload-contracts.md` cites in this task's files (content-verified mappings carried in the 2026-06-09 scope-expansion dispatch DAG).
...
- **Spec coverage:** Spec-003 AC2 (degraded/offline distinguishable in the roster read), Spec-003 AC3 (multiple nodes coexist), Spec-003 §Required Behavior (multiple runtime nodes per session; control plane coordinates discovery/presence, execution stays local; the derived read-only verdict against the floor), Spec-003 §Default Behavior (axes carried verbatim; reconciliation is the client's render-time concern).
...
- **Spec coverage:** Spec-003 AC2 (distinguishability through the client roster read), Spec-003 AC3 (multiple nodes coexist through the client), Spec-003 §Required Behavior (multiple runtime nodes per session).
- **Verifies invariant:** none (SDK transport wrapper).
...
- **Spec coverage:** Spec-003 AC2 (degraded/offline distinguishable from healthy online), Spec-003 AC3 (multiple nodes coexist without changing session identity), Spec-003 §Required Behavior (multiple runtime nodes per session), Spec-003 §Fallback Behavior (capability-validation failure keeps node degraded/offline, distinguishable from healthy).
...
- **Spec coverage:** Spec-003 AC1, Spec-003 AC2, Spec-003 AC4, Spec-003 §Fallback Behavior (node may reconnect under the same node identity).
- **Verifies invariant:** I-003-1 (end-to-end, bridge-routed).

After Phase 4 lands green at Tier 3, Plan-003's load-bearing semantics are complete. Phase 5 ships at Tier 3 after Phase 4 — the renderer substrate from Plan-023 Tier 1 Partial is independently in place from Tier 1, so the gating reduces to Plan-003's own SDK readiness. Phase 5's acceptance rests on the manual multi-node smoke (T5.4); the automated renderer **component** suites landed 2026-08-25 under [BL-131](../backlog.md) exit criterion (b), and the **E2E** that replaces the smoke stays a criterion-gated V1.1 backfill there under exit criterion (c) — neither is a Tier-3 gate.
...
shipped:
  ...
  - phase: 2
    ...
    spec_coverage:
      [
        "Spec-003 line 58",
        "Spec-003 line 59",
        "Spec-003 line 79 (reconnect-under-same-identity clause — a durable-row behavior; the session-ownership-intact clause is the I-003-3 invariant verified control-plane-side at P7/P8 in Phase 3)",
        "Spec-003 line 102",
        "Spec-003 line 103",
        "Spec-003 line 120",
        "Spec-003 line 126",
  ...
  - phase: 3
    ...
    verifies_invariant: [I-003-1, I-003-2, I-003-5]
    spec_coverage:
      [
        "Spec-003 line 47",
        "Spec-003 line 49",
