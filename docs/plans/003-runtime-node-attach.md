...
| ID | Test | Asserts | Spec-003 AC / MUST |
| --- | --- | --- | --- |
| I1 | `Participant opens a live session, then attaches a local runtime node — session not recreated` | live attach | AC1, line 50 |
...
- **Spec coverage:** Spec-003 line 103 (capability/trust changes emitted as session events); Spec-006 §Runtime Node Lifecycle (runtime_node_lifecycle) (per-event payload shapes)
...
- **Spec coverage:** Spec-003 line 47 (attach is a separate step), line 51 (detach/offline semantics).
...
- **Spec coverage:** Spec-003 line 131 (AC1 — attach a local runtime node to an active session), line 48 (attach includes node identity, capabilities, health, trust context), line 50 (attach must not require session recreation).
...
- **Spec coverage:** Spec-003 line 131 (AC1), line 132 (AC2), line 134 (AC4), line 79 (node may reconnect under the same node identity).
