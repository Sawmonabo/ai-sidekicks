...
- **Files:** `packages/contracts/src/runtime-node.ts`, `packages/contracts/src/__tests__/runtime-node.test.ts`.
- **Step:** Export the 7 `runtime_node.*` event-name string literals (`registered`, `online`, `degraded`, `offline`, `revoked`, `capability_declared`, `capability_updated`) as a frozen tuple, sourced verbatim from Spec-006 §Runtime Node Lifecycle (Spec-006 lines 407–413). The 2 `session.clock_*` events in the same category are Plan-015-owned and out of scope. Per CP-003-1, Plan-003 ships event-shape stubs only — these are name constants, not the `EventEnvelope` schema. The `category` literal `runtime_node_lifecycle` already exists in the `EventCategory` union.
- **Test (C4):** assert the exported 7-name set is exactly equal (as a sorted set) to the 7 `runtime_node.*` names in Spec-006 §Runtime Node Lifecycle — neither superset nor subset. Re-derive the expected set against Spec-006 lines 407–413; do not transcribe from a Plan-003 gloss.
...
- **Step:** Validate the capability declaration (only explicitly declared capabilities are persisted/schedulable, per Spec-003 line 58). Persist accepted capabilities to `node_capabilities` (PK `node_id + capability_key`). On a first declaration, emit `runtime_node.capability_declared` through the T2.3 wrapper (reduced base + `{capability, capabilityDetails}`, Spec-006 line 412). On a **change** to an already-declared capability (re-declare with different `capabilityDetails`), emit `runtime_node.capability_updated` through the T2.3 wrapper (reduced base + `{capability, previousState, newState}`, Spec-006 line 413 / [api-payload-contracts §Plan-006](../architecture/contracts/api-payload-contracts.md#plan-006--session-event-taxonomy)) carrying the prior and new detail snapshots — this is the Spec-003 line 99 "capability/trust changes emitted as session events" path. A re-declare with **identical** details is idempotent and emits **no** event (no spurious update spam). `previousState`/`newState` are interim-opaque (mirror the T2.0 schema; Plan-006 Tier 4 binds canonical `CapabilityDetails`).
- **Test (D2):** declare a capability → assert exactly one `runtime_node.capability_declared` event with the Spec-006 line 412 payload shape; re-declare the same capability with changed details → assert exactly one `runtime_node.capability_updated` event carrying the prior/new snapshots (Spec-006 line 413); re-declare with unchanged details → assert no further event appended (idempotent).
...
- **Spec coverage:** Spec-003 line 99 (capability/trust changes emitted as session events); Spec-006 lines 407-413 (per-event payload shapes)
