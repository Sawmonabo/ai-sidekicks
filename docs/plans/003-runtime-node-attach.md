...
- **Scope note:** orchestrator-introduced under the user-directed 2026-06-09 scope expansion.
- **Step:** add `readRoster(request: RuntimeNodeRosterRequest): Promise<RuntimeNodeRosterResponse>` on `AttachService`: SELECT **all** `runtime_node_attachments` rows for the session (all five `state` values verbatim — no server-side hiding; row count bounded by distinct nodes ever attached under the `(node_id, session_id)` uniqueness), LEFT JOIN `runtime_node_presence` on `node_id` carrying `health_state`/`last_heartbeat_at` verbatim (NULL until the node's first heartbeat), and derive `readOnly` per row from the stored `client_version` vs `sessions.min_client_version` (NULL floor → `false`; identical semantics to the attach-time verdict). The read MUST NOT derive staleness (the T3.6 sweep stays the single liveness-derivation writer) and MUST NOT collapse the two axes. (2026-07-13, campaign B4: the response additionally carries `controlHolder: ParticipantId | null` from the `session_terminal_leases` join once Plan-024 Phase 3B ships the lease table — the read extension rides that leg per Spec-003 §Interfaces; this completed task's attachments × presence join is unchanged until then.) Mount `runtimenode.roster` as the router's first `query` — control-plane tRPC only, no daemon JSON-RPC registration — and reconcile the router/SDK "all four are mutations / no query" comments in the touched files. Emit no durable event (ADR-017 non-collision). Also re-anchor the T5.0a-drifted `Spec-003`/`api-payload-contracts.md` cites in this task's files (content-verified mappings carried in the 2026-06-09 scope-expansion dispatch DAG).
