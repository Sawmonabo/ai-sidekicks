...
Registered 2026-08-01 by the Plan-006 T4.10 targeted readiness-audit delta. Plan-006's T4.10 registrar requires the attach response's server-minted `runtime_node_attachments.id` as an attempt precondition (the round-5 refusal-predicate equality recorded at CP-003-5 above), and that id is returned only in the attach response the client-driven attach flow receives in the renderer — no daemon-side attach lifecycle exists to observe it, so the daemon cannot self-serve the value. The delivery chain decomposes three ways, and this row sanctions the one Plan-003-owned leg: on attach success, the shipped Phase-5 attach view (`apps/desktop/src/renderer/src/runtime-node-attach/AttachFlow.tsx`, whose view state already holds `response.attachmentId`) forwards that id to Plan-006's `event.*`-namespace delivery method over the shipped Plan-023 `window.sidekicks` bridge (CP-003-3's projection surface; the bridge is method-agnostic, so no new transport obligation arises) — a bounded-retry forward at Codex PR #278 round 2, joined at round 4 by a reconnect replay on the same call: the delivery is idempotent last-write-wins, so it is retried on a bounded backoff while the view lives AND re-forwarded on each `daemon.status` transition to `connected` the view observes over Plan-007 T-007r-3-10's typed bridge topic, covering a daemon momentarily down at attach success and one down across the whole retry window alike (the topic's live backend arrives with Plan-023's Tier-8 bridge-registry mount and non-stub preload per Plan-007 CP-007-12's activation note — the same Tier-8 gate the attach-success forward and the whole shipped attach view already sit behind, so both triggers activate together and the replay adds no new tier gate). The daemon-side delivery method, its handler, and the registrar's injected attachment-id source are Plan-006's (T4.10, under the CP-006-4 registration mechanism) — durable-backed at rounds 2–3: the daemon commits the delivered pair to its dedicated `daemon_attachment_deliveries` table before acknowledging (acked-means-durable), so its restart needs no renderer replay, and a never-acked delivery is replayed on each reconnect the view observes; the residual that remains is renderer-state loss before any ack (the app quit before the daemon ever acknowledged), remediated by a documented fresh attach — it re-mints a new id and re-delivers, and until then the daemon's key stays off the roster with its anchors emitter-only-verifiable. **Resolution.** The call-site edit itself is performed by Plan-006 T4.10 as a fourth sanctioned seam edit under the CP-003-5 per-file-crossing shape — all five Plan-003 phases are shipped, so no Plan-003 phase exists to carry new work; one bounded-retry forward call on the existing success path plus its `daemon.status` reconnect re-forward subscription, and the header-commentary re-derivation the new call obligates (round 5) — the file's checked-in "only wire call" / "exactly one procedure" claims are restated as I-003-3's actual invariant (attach separate from membership/invite/session mutations) with the delivery forward named as sanctioned — no view re-authoring, no shipped wire shape touched. Reciprocal: [Plan-006 CP-006-14](./006-session-event-taxonomy-and-audit-log.md#cross-plan-obligations).

## Preconditions

- [ ] Paired spec is approved — dated note (2026-07-18, retiring the 2026-07-13 note): Spec-003 returned to `approved` via the campaign Task-28 / W1.5 batch re-promotion. All five of this plan's phases are shipped (Shipment Manifest) — no code dispatch currently gates on this box; the amendment's code leg belongs to Plan-024 Phase 3B. **Re-opened 2026-08-03, scoped to the shared-terminal write-lease projection-conformance amendment:** [Spec-003](../specs/003-runtime-node-attach.md) pins two new normative §Required Behavior constraints on the `runtimenode.leaseupdate` projection — the `offline`-signal producer re-bind disjunct and the direction-asymmetric publish predicate — a Required-Behavior change, so the spec flips `approved → review` per the runbook's [§Spec-Status Promotion Gate](../operations/plan-implementation-readiness-audit-runbook.md#spec-status-promotion-gate) amendment rule, this box un-checks with it, and Status flips `approved → review` alongside (the [§Status Flip Rule](../operations/plan-implementation-readiness-audit-runbook.md#status-flip-rule) behavior-change row, reaching this plan through the paired-spec dependency rather than through any edit to its own body). The §Plan-readiness audit box below stays checked — the amendment mints no Plan-003 task, invariant, or cross-plan obligation, and its enforcement + test growth land on Plan-024's Phase 3B `T-024-3B-3` leg — so the restoring vehicle is the Spec-003 targeted readiness-audit delta, which re-checks this box and restores both Statuses in one swap (the PR #282 flip-and-restore shape). Dispatch impact is nil in practice: all five phases are shipped, so no lane-1 Plan-003 PR waits on this box.
