...
### CP-003-5 — Plan-006 T4.10 extends the attach flow with daemon signing-key roster registration (⇄ Plan-006 CP-006-7 leg B)

Registered 2026-07-29 by the CP-006-7 two-leg amendment. The attach flow is the join-time carrier for daemon signing-key roster registration: `docs/architecture/security-architecture.md §Per-Event Daemon Signature` requires the daemon's session-scoped Ed25519 PUBLIC key to be registered in the session participant roster at join time, keyed by `NodeId`, and attach is where node identity and session identity first meet through an authenticated transport (session-create cannot carry it for a subsequent daemon in a multi-node session — that daemon's local session-materialization happens at attach, on a machine where session-create never ran; the obligation was previously misattributed to a Plan-002 session-create amendment). [Plan-006 T4.10](./006-session-event-taxonomy-and-audit-log.md#cross-plan-obligations) EXTENDs three Plan-003-owned surfaces, none re-authored: (a) `packages/contracts/src/runtime-node.ts` gains an additive OPTIONAL `daemonSigningPublicKey` member on `RuntimeNodeAttachRequestSchema` (the same extend-in-place shape as CP-003-1 leg (c); `ADR-018 §Decision` #8 additive-MINOR, so a pre-leg-B daemon omitting the field still parses); (b) `runtime_node_attachments` gains an additive nullable `daemon_signing_public_key BYTEA` column via a Plan-006-owned control-plane migration (the `command_receipts`/`mcp_task_id` §1 precedent — never an edit to Plan-003's `0003-runtime-nodes.ts`); (c) `attach-service.ts` takes sanctioned seam edits (the §2 Plan-024 `lease-projection` precedent): persist-on-attach, different-key-reconnect refusal with a typed conflict error (register-once — never a silent overwrite, mirroring Plan-006 T4.2's `refuse_on_rotation` posture), and `readRoster` projection exposure so verifiers have a `NodeId`-keyed resolution surface.

**Resolution.** Plan-003's shipped phases are not re-opened — all five are shipped (Shipment Manifest), and the code leg belongs to Plan-006 T4.10 (Plan-006's Phase 4, the read-side/verifier phase — the placement matches the leg's consumer: it serves the verification-resolution surface, not Plan-006's append path), the same routing shape as the Spec-003 terminal-control amendment whose code leg belongs to Plan-024 Phase 3B. Plan-003's file records the obligation so the §2 one-writer discipline carries the sanctioned edits rather than being silently violated.
