...
A newly attached runtime node MUST default to non-online state (e.g., `pending`/`degraded`) until the capability declaration succeeds. `runtime_node.online` MUST emit only after `runtime_node.capability_declared` (Spec-003 §Default Behavior line 57; §Implementation Notes line 115).

**Why load-bearing.** Marking a node online before capability validation succeeds would expose unvalidated capabilities to scheduler routing. Spec-003 explicitly forbids "implicit capability exposure on attach" (§Pitfalls line 122) — this invariant is the structural enforcement.
...
**Verification.** Test P9 asserts a node already actively attached to session A is refused a second active attach to session B (the partial unique constraint fires), while a reconnect to A after detach — its row left `offline` — succeeds via the T3.2 upsert (Spec-003 line 78: reconnect under the same node identity). Heartbeat- and detach-by-`nodeId` therefore resolve the node's one active row unambiguously.
...
| --- | --- | --- | --- |
| C1 | `RuntimeNodeAttach payload validates required fields including client_version` | request schema | line 82 (required fields), line 53 (client_version) |
...
| C4 | `runtime_node.* event names exactly match the 7-event taxonomy in Spec-006` | taxonomy conformance | Spec-006 §Runtime Node Lifecycle (lines 371–377) |
...
| ID | Test | Asserts | Spec-003 AC / MUST |
| --- | --- | --- | --- |
| D1 | `Node registry persists node identity across daemon restart` | stable identity (Spec-003 line 116) | AC1 |
| D2 | `Capability declaration service emits runtime_node.capability_declared on success` | event emission | line 99 |
...
| D4 | `Detach emits runtime_node.offline; subsequent reconnect under same node id succeeds` | reconnect identity | line 78, line 116 |
...
| P6 | `Heartbeat ingestion updates runtime_node_presence; missed heartbeat past threshold transitions health_state to degraded then offline (coordination record — no durable event in V1)` | health transitions | line 59 (cadence); line 60 (`degraded` older than `30s` / `offline` older than `60s`); line 61 (control-plane sweep; no durable event) |
...
| P9 | `Node actively attached to session A is refused a second active attach to session B with a typed CONFLICT (409) wire error; reconnect to A after detach reactivates the offline row` | I-003-5 single-active-session (partial unique index) | I-003-5, line 140 |
...
- **Spec coverage:** Spec-003 line 82 (RuntimeNodeAttach required fields), Spec-003 line 53 (`client_version` floor field)
...
- **Spec coverage:** Spec-003 line 83 (RuntimeNodeHeartbeat updates presence/health), line 85 (RuntimeNodeDetach retire/disconnect)
- **Verifies invariant:** none (contract-schema definition)

##### T1.4 — `runtime-node.ts`: `runtime_node.*` event-name taxonomy constants (C4 conformance)

- **Files:** `packages/contracts/src/runtime-node.ts`, `packages/contracts/src/__tests__/runtime-node.test.ts`.
- **Step:** Export the 7 `runtime_node.*` event-name string literals (`registered`, `online`, `degraded`, `offline`, `revoked`, `capability_declared`, `capability_updated`) as a frozen tuple, sourced verbatim from Spec-006 §Runtime Node Lifecycle (Spec-006 lines 375–381). The 2 `session.clock_*` events in the same category are Plan-015-owned and out of scope. Per CP-003-1, Plan-003 ships event-shape stubs only — these are name constants, not the `EventEnvelope` schema. The `category` literal `runtime_node_lifecycle` already exists in the `EventCategory` union.
- **Test (C4):** assert the exported 7-name set is exactly equal (as a sorted set) to the 7 `runtime_node.*` names in Spec-006 §Runtime Node Lifecycle — neither superset nor subset. Re-derive the expected set against Spec-006 lines 375–381; do not transcribe from a Plan-003 gloss.
- **Spec coverage:** Spec-006 lines 375-381 (runtime_node.registered)
...
- **Spec coverage:** Spec-003 line 53 (typed `VERSION_FLOOR_EXCEEDED` on below-floor write), Spec-003 AC4 (line 130)
...
- **Spec coverage:** Spec-003 line 98 (runtime-node records durable for reconnect/audit)
...
- **Spec coverage:** Spec-006 lines 375-381 (per-event payload shapes); api-payload-contracts.md §Plan-006 (capability payload typing).
...
- **Step:** Validate the capability declaration (only explicitly declared capabilities are persisted/schedulable, per Spec-003 line 58). Persist accepted capabilities to `node_capabilities` (PK `node_id + capability_key`). On a first declaration, emit `runtime_node.capability_declared` through the T2.3 wrapper (reduced base + `{capability, capabilityDetails}`, Spec-006 line 380). On a **change** to an already-declared capability (re-declare with different `capabilityDetails`), emit `runtime_node.capability_updated` through the T2.3 wrapper (reduced base + `{capability, previousState, newState}`, Spec-006 line 381 / [api-payload-contracts §Plan-006](../architecture/contracts/api-payload-contracts.md#plan-006--session-event-taxonomy)) carrying the prior and new detail snapshots — this is the Spec-003 line 99 "capability/trust changes emitted as session events" path. A re-declare with **identical** details is idempotent and emits **no** event (no spurious update spam). `previousState`/`newState` are interim-opaque (mirror the T2.0 schema; Plan-006 Tier 4 binds canonical `CapabilityDetails`).
- **Test (D2):** declare a capability → assert exactly one `runtime_node.capability_declared` event with the Spec-006 line 380 payload shape; re-declare the same capability with changed details → assert exactly one `runtime_node.capability_updated` event carrying the prior/new snapshots (Spec-006 line 381); re-declare with unchanged details → assert no further event appended (idempotent).
- **Spec coverage:** Spec-003 line 58 (least-privilege schedulability), line 99 (capability/trust changes emitted as session events — covers both declaration and update), line 122 (no implicit capability exposure on attach).
...
- **Spec coverage:** Spec-003 line 99 (capability/trust changes emitted as session events); Spec-006 lines 375-381 (per-event payload shapes)
...
- **Spec coverage:** Spec-003 line 57 (online only after capability declaration) — the gate D3 verifies (no `runtime_node.online` until a declaration row exists). Spec-003 line 76 (capability-validation FAILURE → `degraded`/`offline`, not healthy) is the sibling not-healthy principle; the `degraded`/`offline` it names are server-derived coordination-record transitions (Phase 3 T3.6), whose durable-event emission is V1.1-gated ([ADR-017 §Server-Derived Runtime-Node Lifecycle Events](../decisions/017-shared-event-sourcing-scope.md#server-derived-runtime-node-lifecycle-events)) — Phase 2 has no `degraded` emit shape; D3 exercises declaration ABSENCE via the :57 gate, not validation failure, so it codifies none of :76's emission. (Scoped 2026-06-02, PR #137, to match the T2.4/D3 test.)
...
- **Spec coverage:** Spec-003 line 78 (reconnect under same identity — a durable-row behavior; the active-membership-intact clause is the I-003-3 invariant verified control-plane-side at P7/P8 in Phase 3), line 116 (node identity stable across reconnect).
...
- **Spec coverage:** Spec-003 line 98 (durable runtime-node records for reconnect/audit).
...
- **Spec coverage:** Spec-003 line 49 (multiple runtime nodes per session), line 129 (AC3 — multiple nodes coexist without changing session identity).
...
- **Spec coverage:** Spec-003 line 52 (control plane coordinates discovery/presence; execution stays local), line 57 (online requires capability declaration), line 76 (capability-validation failure → `degraded`), line 65 (2026-06-04 `capabilityupdate`-semantics amendment).
...
- **Spec coverage:** Spec-003 line 82 (RuntimeNodeAttach fields), line 83 (RuntimeNodeHeartbeat updates presence and health), line 84 (RuntimeNodeCapabilityUpdate add/remove/health variants), line 85 (RuntimeNodeDetach retires a node).
...
- **Spec coverage:** Spec-003 line 127 (AC1 — participant attaches a local runtime node to an already active session), line 50 (attach must not require session recreation).
...
- **Step (amended 2026-06-09, PR #147 — drive `degraded` on the capability-health axis; the original heartbeat-driven roster-read is not a Phase-4 surface):** Attach two nodes to one live session through the SDK. Drive node A `registering → degraded` via `capabilityUpdate` with `healthChanges: { state: "degraded" }` (the self-reported capability-health axis — Spec-003 line 76 — explicitly permitted by the I-003-2 guard, which blocks only `registering → online`), and drive node B to a healthy `online` contrast via the permitted `registering → degraded → online` recovery. Assert distinguishability through the client: A's latest `capabilityUpdate` response `state` reads `degraded` while B's reads `online` (the response `state` is the server-derived full `NodeState` — Spec-003 line 72), and A is not absent — a subsequent capabilities-only `capabilityUpdate` on A still resolves its active attachment (a retired slot would surface the typed 409 refusal) and still reads `degraded`. Additionally exercise the SDK `heartbeat` path end-to-end: it resolves to the no-content `null` response, the `runtime_node_presence` row lands server-side, and the liveness axis stays independent of the capability axis (presence `health_state` still `online` after A's capability-`degraded`) — asserted at the persistence layer, since no Phase-4 client surface reads the presence axis. **Why amended:** the original step ("drive `degraded` via the Phase-3 heartbeat transition, then read the roster through the client") is unimplementable against the shipped Phase-4 surface — heartbeat/staleness transitions write only the `runtime_node_presence.health_state` liveness axis, which no Phase-4 SDK response surfaces, and the reconciled roster-read (presence × slot) is downstream of this plan (Spec-003 line 72 — per-axis storage "not pinned in this contract"; ADR-017 §Server-Derived Runtime-Node Lifecycle Events). **(Reconciled 2026-06-09, PR #150: the "downstream of this plan" roster-read deferral is superseded for the read path — the user-directed Phase 5 scope expansion ships the reconciled roster-read in this plan as the control-plane-only `runtimenode.roster` query (T5.0a–T5.0d), its projection now pinned in Spec-003 §Interfaces And Contracts while per-axis storage stays writer-owned; this T4.3 step itself is unchanged — the capability-health-axis I2 test remains as shipped in PR #147.)**
...
- **Spec coverage:** Spec-003 line 128 (AC2 — a degraded or offline node remains distinguishable from a healthy online node), line 76 (capability-validation failure leaves the node `degraded` — the self-reported capability-health axis driven here), line 72 (the `capabilityupdate` response `state` is the server-derived full `NodeState`; the effective roster state reconciles both independent axes).
...
- **Spec coverage:** Spec-003 line 130 (AC4 — below-floor daemon admitted read-only, `VERSION_FLOOR_EXCEEDED` on subsequent write, never ejected).
...
##### T5.0a — Governance: register `runtimenode.roster` + pin the roster-read contract (orchestrator-introduced, PR #150)
...
- **Scope note:** orchestrator-introduced under the user-directed 2026-06-09 scope expansion (PR #150); doc lands first in-PR — the doc never trails the code.
...
- **Spec coverage:** Spec-003 line 128 (AC2 — a degraded or offline node remains distinguishable from a healthy online node), line 129 (AC3 — multiple nodes coexist without changing session identity), line 49 (multiple runtime nodes per session), line 72 (both health axes reconciled client-side; the read-side projection now pinned).
- **Verifies invariant:** none (governance).

##### T5.0b — Contracts: `RuntimeNodeRoster*` request/entry/response Zod schemas (orchestrator-introduced, PR #150)

- **Files:** `packages/contracts/src/runtime-node.ts` (extend), `packages/contracts/src/__tests__/runtime-node.test.ts` (extend).
- **Scope note:** orchestrator-introduced under the user-directed 2026-06-09 scope expansion (PR #150).
- **Step:** author `RuntimeNodeRosterRequestSchema` (`.strict()`: `sessionId`), `RuntimeNodeRosterEntrySchema` (the nine-field entry pinned in Spec-003 §Interfaces And Contracts — `nodeId`, `participantId`, `state: NodeState` carrying all five values, nullable `healthState` on the 3-value presence enum (`online | degraded | offline`, mirroring the `runtime_node_presence.health_state` CHECK), nullable `lastHeartbeatAt`, `readOnly`, `capabilities`, `clientVersion`, `attachedAt`), and `RuntimeNodeRosterResponseSchema` (`{ nodes: RuntimeNodeRosterEntry[] }`), mirroring the registry wire shapes. Non-input projection schemas follow the file's single-`T` house style (the T2.0 precedent). Also re-anchor the T5.0a-drifted `Spec-003`/`api-payload-contracts.md` cites in this task's files (content-verified mappings carried in the PR #150 DAG).
...
- **Spec coverage:** Spec-003 line 72 (the wire carries both axes verbatim — no collapsed scalar), line 128 (AC2 — `degraded`/`offline` representable on the wire), line 49 (multiple runtime nodes per session — `nodes[]`).
- **Verifies invariant:** none (contract-schema definition).

##### T5.0c — Control plane: `AttachService.readRoster` + the `runtimenode.roster` router `query` (orchestrator-introduced, PR #150)
...
- **Spec coverage:** Spec-003 line 128 (AC2 — degraded/offline distinguishable in the roster read), line 129 (AC3 — multiple nodes coexist), line 49 (multiple runtime nodes per session), line 52 (control plane coordinates discovery/presence; execution stays local), line 53 (the derived read-only verdict against the floor), line 72 (axes carried verbatim; reconciliation is the client's render-time concern).
...
##### T5.0d — SDK: `roster` query on the control-plane runtime-node factory (orchestrator-introduced, PR #150)
...
- **Spec coverage:** Spec-003 line 128 (AC2 — distinguishability through the client roster read), line 129 (AC3 — multiple nodes coexist through the client), line 49 (multiple runtime nodes per session).
- **Verifies invariant:** none (SDK transport wrapper).
...
- **Spec coverage:** Spec-003 line 128 (AC2 — degraded/offline distinguishable from healthy online), line 129 (AC3 — multiple nodes coexist without changing session identity), line 49 (multiple runtime nodes per session), line 76 (capability-validation failure keeps node degraded/offline, distinguishable from healthy).
...
- **Test:** (no automated component test this Phase — covered by the T5.4 manual smoke; automated coverage backfilled per BL-131, V1.1.)
- **Spec coverage:** Spec-003 line 127 (AC1 — attach a local runtime node to an active session), line 47 (attach is a separate step from membership acceptance), line 48 (attach includes node identity, capabilities, health, trust context), line 50 (attach must not require session recreation).
...
- **Test:** (no automated component test this Phase — covered by the T5.4 manual smoke; automated coverage backfilled per BL-131, V1.1.)
- **Spec coverage:** Spec-003 line 130 (AC4 — below-floor admitted read-only, write returns typed `VERSION_FLOOR_EXCEEDED`, never ejected for the floor mismatch), line 53 (control plane verifies daemon version against the floor).
...
- **Spec coverage:** Spec-003 line 127 (AC1), line 128 (AC2), line 130 (AC4), line 51 (detach/offline must not revoke membership), line 78 (node may reconnect under the same node identity).
  ...
  - phase: 2
    ...
    spec_coverage:
      [
        "Spec-003 line 57",
        "Spec-003 line 58",
        "Spec-003 line 78 (reconnect-under-same-identity clause — a durable-row behavior; the active-membership-intact clause is the I-003-3 invariant verified control-plane-side at P7/P8 in Phase 3)",
        "Spec-003 line 98",
        "Spec-003 line 99",
        "Spec-003 line 116",
        "Spec-003 line 122",
        "Spec-006 lines 375, 376, 378, 380, 381",
  ...
  - phase: 4
    ...
    spec_coverage:
      [
        ...
        "Spec-003 line 127",
        "Spec-003 line 50",
        "Spec-003 line 128 (capability-health axis per the in-PR T4.3 plan-row amendment, 2026-06-09)",
        "Spec-003 line 76",
        "Spec-003 line 72",
        "Spec-003 AC4 line 130",
      ]
