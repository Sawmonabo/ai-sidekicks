...
`RuntimeNodeAttach` MUST NOT modify session_memberships, and `MembershipUpdate` MUST NOT trigger runtime-node detach as a side effect. Membership and node attach are independent surfaces that share a participant identity but otherwise compose orthogonally (`Spec-003 §Required Behavior`). **Amended 2026-08-12 (BL-141 / [ADR-025](../decisions/025-runtime-node-control-plane-caller-authorization.md) D11, ratified 2026-08-10):** the no-mutation property above is preserved verbatim, and a `FOR SHARE` **authorization read** of `session_memberships` is expressly permitted — reading a row to decide whether a caller may act is not modifying it, and the caller-authorization predicate cannot be evaluated without it. The permission is scoped to that mode and that purpose: the runtime-node transactions write nothing to `session_memberships`, and `FOR SHARE` is the weakest mode that conflicts with the `FOR NO KEY UPDATE` a suspend/revoke `UPDATE` acquires, so a revocation cannot commit between the authorization read and the mutation it authorizes. The `attach-service.ts#AttachService` header comments that today assert the table is never referenced become false with this amendment and are rewritten in the same commit as the guard (T3.10), not left to drift.
...
**Verification.** Test must assert that (a) `RuntimeNodeAttach` succeeds without any `session_memberships` mutation and (b) detaching a node leaves `session_memberships` unchanged. The audit-trail event sequence must show attach/detach and membership changes as distinct. The byte-unchanged-row assertion is **retained unweakened** under the 2026-08-12 amendment — an authorization read must leave the row byte-identical, which is exactly what distinguishes it from a mutation — and T3.12 additionally asserts the read is taken `FOR SHARE`, since `FOR KEY SHARE` would satisfy the byte-unchanged assertion while silently admitting the concurrent revocation this invariant's amendment exists to exclude.
...
Registered 2026-07-29 by the CP-006-7 two-leg amendment. The runtime-node surface is the join-time carrier for daemon signing-key roster registration: `docs/architecture/security-architecture.md §Per-Event Daemon Signature` requires the daemon's session-scoped Ed25519 PUBLIC key to be registered in the session participant roster at join time, keyed by `NodeId`, and the runtime-node control-plane surface is where node identity and session identity meet through an authenticated transport (session-create cannot carry it for a subsequent daemon in a multi-node session — that daemon's local session-materialization happens at its join-driven admission, on a machine where session-create never ran, and need not involve an attach at all, per the Spec-008 chat-only-member record the 2026-08-11 admission-time decoupling honors; the obligation was previously misattributed to a Plan-002 session-create amendment). [Plan-006 T4.10](./006-session-event-taxonomy-and-audit-log.md#cross-plan-obligations) EXTENDs two Plan-003-owned surfaces, none re-authored: (a) `packages/contracts/src/runtime-node.ts` gains four additive NEW schema exports — the `runtimenode.signingkeyregister` / `runtimenode.signingkeyroster` request/response pairs — with NO shipped schema mutated: every Plan-003 request/response schema in the file is `.strict()`, so a member added to a shipped shape (the amendment's original attach-request-field carrier, withdrawn in the same PR's review round) would break the new-daemon→old-control-plane skew direction at parse per `ADR-018 §Decision` #7, which is why the carrier is a new procedure pair rather than an attach field; (b) `packages/control-plane/src/runtime-nodes/runtime-node-router.factory.ts` takes two procedure-key registrations on the shared builder (the §2 Plan-024 `leaseupdate` precedent) and `packages/client-sdk/src/runtimeNodeClient.ts` gains the two typed pass-throughs. The store itself is NOT a Plan-003 surface: the key persists register-once per `(session_id, node_id)` in the new Plan-006-owned `daemon_signing_public_keys` table (different-key registration refused with the typed `runtimenode.signingkeyregister_conflict` error before any row mutation — never a silent overwrite, mirroring Plan-006 T4.2's `refuse_on_rotation` posture; the register authorization refusal — active membership, collapsed per the PR #274 round-4 amendment; the round-5 `attachmentId` equality and its attachment-ownership sibling were DELETED 2026-08-11 by the admission-time registration decoupling (the Spec-008-pinned leg, landed with the Plan-008/Spec-008 restoring targeted readiness-audit delta): registration keys on membership admission with NO attachment read at all, so the transaction no longer touches any Plan-003-owned row — the producer binding is the daemon-bound CP-006-13 credential at participant granularity, the widening named at `security-architecture.md §Per-Event Daemon Signature` — and the roster-membership refusal share the typed `runtimenode.permission_denied` (403) per the error-contracts §Runtime Node registry), deliberately NOT a `runtime_node_attachments` column — CP-003-4 above records that table's `Spec-022 §Shred Fan-Out` hard-DELETE disposition as operational node-attach state, while the verification key must SURVIVE participant erasure to keep the retained crypto-shredded `runtime_node.*` stream and `event_log_anchors` rows verifiable; the register/resolve service lands as the Plan-006-owned sibling file `signing-key-service.ts` in this plan's `runtime-nodes/` directory (the §2 Plan-024 `lease-projection-service.ts` per-file-crossing precedent). `attach-service.ts` and the attach wire contract are untouched.
...
- **Spec coverage:** Spec-003 §Fallback Behavior (reconnect under same node identity after disconnect — durable-row behavior; the active-membership-intact clause is I-003-3, verified control-plane-side at P7/P8 in Phase 3; node identity stable across reconnect).
...
- **Test (unit):** the classifier returns each of the four verdicts against a constructed row/membership pair; a suspended owner classifies `owner_inactive_member` rather than short-circuiting to permit; `heartbeat` for a `node_id` with no active attachment writes no `runtime_node_presence` row; `heartbeat` for an attachment owned by another participant writes nothing and refuses; `updateCapabilities` refuses a non-owner and its `SELECT` reads `participant_id`; `detach` by a fellow active member who does not own the row refuses with the typed 403 and mutates no row, while `detach` for a node with no attachment visible to the caller is the idempotent `null` no-op; the `capabilityupdate` 409 fires **only** for the `registering → online` state guard. Concurrency: an interleaving in which the attachment is detached and re-attached elsewhere between the pre-read and the lock is refused by the step-3 re-verify (this case fails without it).
- **Spec coverage:** Spec-003 line 46 (caller authorization precondition across the `runtimenode.*` surface — 2026-08-12 BL-141 amendment), Spec-003 line 90 (uniform refusal + the narrowed 409), Spec-003 line 73 (2026-08-12 amendment record on the five-procedure contract), Spec-003 line 62 (server-derived staleness sweep whose integrity the presence-forgery fix restores).
- **Verifies invariant:** I-003-3 (the amended form — a `FOR SHARE` authorization read is not a mutation; attach and detach still write no `session_memberships` row); I-003-6 (canonical lock order, recorded modes, two-phase resolve with re-verify); I-003-5 (the single-active-attachment resolution the two-phase pre-read keys on).

##### T3.11 — `attach` membership guard + identity pre-guard + roster membership predicate
...
- **Step:** Add `attach`'s two authorization axes. **Axis (a) — session membership (new enforcement):** inside the transaction take the level-1 `sessions` lock `FOR KEY SHARE` while reading `min_client_version`; zero rows is the uniform negative, which also replaces today's raw foreign-key-violation path with a typed refusal; then take the caller's `session_memberships` row at level 2 `FOR SHARE` and require `state = 'active'`; only then run the upsert. **Axis (b) — node-identity ownership (already structural):** the shipped upsert's `ON CONFLICT … DO UPDATE … WHERE runtime_node_attachments.participant_id = EXCLUDED.participant_id` predicate plus the active partial-unique index already forbid cross-owner takeover, so axis (b) needs **tests, not new enforcement**. **The pre-guard that makes axis (b) trustworthy:** axis (b) compares the row's owner against `EXCLUDED.participant_id`, which comes from the request body — so before the transaction opens, refuse when `canonicalizeUuid(callerParticipantId) !== canonicalizeUuid(validated.participantId)`. This is a cheap, allocation-free, pre-I/O ownership-spoof guard; without it an attacker sets `participantId` to the victim and the upsert predicate happily matches. Retain the router's existing self-check as transport-tier defence in depth, **canonicalize** it so it stops being case-sensitive, and map it through the typed `runtimenode.permission_denied` refusal rather than its shipped plain tRPC `UNAUTHORIZED`, so the transport tier and the service pre-guard produce the same uniform 403 with an `aisError` projection (ADR-025 D5/D9). Fold the membership predicate into `readRoster` as **one statement** driven from the caller's own membership row (`FROM session_memberships m LEFT JOIN runtime_node_attachments … WHERE m.session_id = $1 AND m.participant_id = $2 AND m.state = 'active'`): zero rows means not-an-active-member or no-such-session, both the uniform negative; rows with a null attachment id mean a visible session with no attachments, which is `{ nodes: [] }`. One `READ COMMITTED` statement is one snapshot, so no revocation interleaves between predicate and projection and the read needs **no lock and no transaction** — the shape `runtimenode.signingkeyroster` already ships, generalized. The roster stays a **faithful projection**: the predicate is at the session boundary, never a per-caller row filter, because filtering rows would break the projection contract and AC2 distinguishability.
- **Test (unit + integration):** a non-member's `attach` is refused with the uniform negative and writes no row; an `attach` naming a nonexistent session returns the typed refusal, not a raw constraint failure; an `attach` whose body `participantId` differs from the caller is refused before any I/O; a mixed-case `participantId` that canonicalizes to the caller is **accepted** (the canonicalization is not a stricter check by accident); a non-member's roster read is refused rather than returning an empty projection; an active member's roster read returns **every** row for the session with both health axes verbatim, including rows owned by other participants; a session the caller can see with no attachments returns `{ nodes: [] }`; the roster refusal for a nonexistent session and for a non-member are byte-identical.
- **Spec coverage:** Spec-003 line 46 (membership precondition on attach; session-scoped precondition on the roster read), Spec-003 line 87 (`RuntimeNodeRoster` faithful-projection contract the predicate must not narrow), Spec-003 line 92 (2026-06-09 roster amendment — every row verbatim, no server-side hiding), Spec-003 line 53 (the version floor the level-1 `sessions` read resolves).
- **Verifies invariant:** I-003-1 (admit-not-eject is untouched — a below-floor daemon that passes authorization is still admitted read-only); I-003-3 (the membership read is an authorization read, not a mutation); I-003-6 (level-1 then level-2 acquisition order and modes on the attach path).

##### T3.12 — Two-account IDOR suite + axis-(b) regressions + lock-ordering block

- **Files:** `packages/control-plane/src/runtime-nodes/__tests__/attach-service.test.ts`, `packages/control-plane/src/runtime-nodes/__tests__/heartbeat-service.test.ts`, `packages/control-plane/src/runtime-nodes/__tests__/runtime-node-router.test.ts`, `packages/control-plane/src/runtime-nodes/__tests__/caller-authorization.test.ts` (new), `packages/control-plane/src/memberships/__tests__/lock-ordering.test.ts` (EXTEND — the canonical lock-ordering suite and its logging-proxy `Querier` pattern; forking a runtime-nodes sibling would fracture the invariant's single home), `packages/control-plane/src/server/__tests__/host-runtime-node.test.ts`, and the cross-package ripple at `packages/client-sdk/test/runtimeNodeClient.integration.test.ts`.
- **Step:** Build the two-account IDOR suite — participant A owns an attachment, participant B is a _legitimate active member of the same session_ — and assert cross-owner refusal on all four mutating procedures at **three layers**: service, router, and HTTP. B being a genuine member is the point: it proves the guard is object-level and not merely a session gate. Add the roster enumeration case for a non-member and for a nonexistent session. Assert **oracle equality per procedure over its applicable negative causes** (ADR-025 D3/D4 — a blanket four-cause matrix is unsatisfiable: `attach` must succeed for a free node and its foreign-active arm is the deliberately distinct 409 residual, `roster` deliberately projects foreign-owned rows, and the `nodeId`-keyed procedures carry no session input): `attach` — unknown-session ≡ non-member, the pre-I/O identity-mismatch refusal carrying the same code; `heartbeat` / `capabilityupdate` — unknown node ≡ foreign-owned ≡ inactive-owner, byte-identical 403s (`capabilityupdate`'s 409 reserved for the state guard); `detach` — every invisible cause one byte-identical `null` no-op, the fellow-member cross-owner arm the same 403 as the other mutators; `roster` — unknown session ≡ non-member. A distinct message inside any pair reopens the oracle. Add the `owner_inactive_member` cases: a suspended owner is refused `attach` / `heartbeat` / `capabilityupdate` / `roster` and **permitted** `detach`, which moves only their own row toward `offline` and writes no membership. Add the concurrency case that fails without the two-phase resolve's step-3 re-verify. Add `attach` axis-(b) regressions: a body `participantId` naming another participant is refused pre-I/O, and the upsert's cross-owner predicate is separately pinned. Pin the ADR-025 accepted residual (known-id squatting, §Uniform-Negative Scope): with owner A's attachment detached, an active member B of another session attaches A's remembered `nodeId` there and **succeeds** with a row attributing B as owner, and A's subsequent reattach under the same id refuses 409 `runtimenode.attach_conflict` — asserted as the deliberately accepted shape, so any future change to it is a conscious contract move, not drift. Add the lock-ordering block asserting **order and mode** — order alone is insufficient, because a `session_memberships` lock taken `FOR KEY SHARE` instead of `FOR SHARE` is a silent no-op guard with **no runtime detection**, admitting a concurrent revocation between classification and mutation. Migrate the four existing suites, whose constructs currently succeed without a caller; the new required `callerParticipantId` parameter breaks them at compile time, which is the intended failure mode. **Note on `createCaller`:** `TRPCError.code` and `.cause` are visible through `createCaller`, but `shape.data.aisError` materializes only over the HTTP handler via the shared `errorFormatter` in `sessions/trpc.ts` — assert `code`/`cause` at the router tier and the `aisError` projection at the HTTP tier.
...
- **Spec coverage:** Spec-003 line 46 (every authorization arm the amendment pins, including the detach-only exemption and the no-oracle rule), Spec-003 line 90 (the uniform refusal and the narrowed 409), Spec-003 line 87 (the roster projection stays faithful for a permitted caller).
- **Verifies invariant:** I-003-3 (the byte-unchanged `session_memberships` assertion survives the amendment); I-003-5 (the global single-active-attachment constraint that makes the inactive-owner detach exemption necessary); I-003-6 (order **and** mode, plus the two-phase re-verify).
...
- **Test:** `apps/desktop/src/renderer/src/runtime-node-attach/__tests__/NodeRoster.test.tsx` (BL-131 exit-criterion-(b) backfill, 2026-08-25) — the three render states; subscribe-before-read ordering; the health-push re-read with its no-flicker, stale-response and cross-session cancellation guards; the below-floor read-only row rendered rather than dropped; a null liveness axis left absent rather than coerced; and the bridge-projection source scan. The two-client leg stays with the T5.4 manual smoke and the BL-131 exit-criterion-(c) E2E.
...
- **Test:** `apps/desktop/src/renderer/src/runtime-node-attach/__tests__/AttachFlow.test.tsx` + `apps/desktop/src/renderer/src/runtime-node-attach/__tests__/CapabilityDeclaration.test.tsx` (BL-131 exit-criterion-(b) backfill, 2026-08-25) — the four attach states; the composed attach payload with the session prop winning the spread; the structural double-fire guard (the pending branch renders no control); sync-throw normalization against the Tier-1 stub; attachment-target reset on a node or session change; the declared-set projection with its total value formatter; and the bridge-projection source scans. The I-003-3 renderer leg is asserted directly: attaching reaches the control plane exactly once, on the attach procedure alone.
...
- **Test:** manual two-client attach smoke; the automated **component** half of the BL-131 backfill landed 2026-08-25 (exit criterion (b)), and the two-client **E2E** that replaces this step remains owed as BL-131 exit criterion (c).
...
- **Heartbeat carries no caller-authorization predicate** (dated note, 2026-08-03 — routing only; this bullet mints no task, invariant, or cross-plan obligation). `runtimenode.heartbeat` resolves no caller today: `HeartbeatService.ingest` (T3.6) writes presence with no membership or ownership check, and the runtime-node router's AUTH POSTURE banner defers node-ownership verification to Tier 5. A suspended or revoked node-owner's daemon therefore keeps heartbeating, so its `runtime_node_presence.health_state` never reaches `offline`. **Interaction with the 2026-08-03 [Spec-003](../specs/003-runtime-node-attach.md) projection-conformance amendment:** the `offline` producer-re-bind disjunct that amendment adds to `runtimenode.leaseupdate` is reachable for a **dead** host (which stops heartbeating) but not for a **revoked-but-alive** one — that case is closed instead at the publish layer by the same amendment's direction-asymmetric membership predicate, so Plan-024's Phase 3B lease leg is correct without a heartbeat change and does not gate on one. Residual, named rather than hidden: while such a node stays alive it also stays the lease row's recorded producer, because neither hatch disjunct fires for it — not a V1-reachable migration blocker, since a V1 terminal host is the node-owner's own machine and no remote terminal-write leg exists for a successor to take over through (`Spec-003 §Required Behavior`). The parity itself is already owned and scheduled: the [bl-resolution campaign design §4.A Unit A](../superpowers/specs/2026-07-09-bl-resolution-campaign-design.md) decision 6 gives `HeartbeatService.ingest` the caller-classification guard (transaction + membership `FOR SHARE` + owned-attachment re-verify) under the ADR-025 it introduces, so this note opens nothing new — it records the interaction so the Phase 3B implementer does not re-derive it or mistake the residual for a lease defect. **Resolved in design 2026-08-10, recorded here 2026-08-12:** [ADR-025](../decisions/025-runtime-node-control-plane-caller-authorization.md) is `accepted`, and the heartbeat guard this bullet routed forward is now a named task — T3.10 gives `HeartbeatService.ingest` a transaction and the ownership-plus-active-membership predicate, so presence becomes writable only through an owned active attachment and a revoked-but-alive node's heartbeats stop refreshing `runtime_node_presence`; T3.12 carries its refusal and forgery cases. The routing framing above stands as authored (this bullet still mints nothing itself); what changed is that the parity it pointed at is scheduled on this plan rather than only in the campaign design. The lease-side conclusion is unaffected: Plan-024's Phase 3B leg still does not gate on a heartbeat change, and the recorded residual narrows once T3.10 ships because the dead-producer node stops being advertised live.
- **The five `runtimenode.*` control-plane procedures MUST NOT be exposed to untrusted multi-participant traffic until the T3.10–T3.12 ownership guard is enforced** (dated note, 2026-08-25 — re-homed from BL-141 §V1 Release Impact at that item's archival; this bullet mints no task, invariant, or cross-plan obligation). The procedures may land functionally — the single-participant and trusted-session execution paths are unaffected — but a collaboration product cannot ship a path where participant A mutates participant B's node. The gate is held structurally rather than by policy today: the control plane's `resolveCurrentParticipantId` throws until [Plan-008](./008-control-plane-relay-and-session-join.md) R1 lands PASETO validation ([ADR-010](../decisions/010-paseto-webauthn-mls-auth.md)), so no untrusted caller reaches these procedures, and the guard becomes production-live with no further Plan-003 code change when T-008r-1-5's host-wiring leg swaps that throw for the verified principal (`Spec-003 §Required Behavior`; [ADR-025](../decisions/025-runtime-node-control-plane-caller-authorization.md) D2). Sequencing consequence: if Plan-008 R1 is scheduled ahead of T3.10–T3.12, this plan's Phase-3 supplement becomes a release blocker for that leg.
