...
| ID | Test | Asserts | Spec-003 AC / MUST |
| --- | --- | --- | --- |
| I1 | `Participant opens a live session, then attaches a local runtime node — session not recreated` | live attach | AC1, `Spec-003 §Required Behavior` |
...
- **Step:** Author the single shared classifier `classifyRuntimeNodeCaller(transaction, callerParticipantId, attachmentRow) → 'attachment_owner' | 'not_visible'`. The owner check compares `canonicalizeUuid(attachmentRow.participant_id)` against `canonicalizeUuid(callerParticipantId)` using the shipped `packages/contracts/src/uuid-canonical.ts#canonicalizeUuid` (the branded `ParticipantId` schema admits mixed case; Postgres returns canonical lowercase). The verdict set is deliberately binary: the attachment row carries its owner, so ownership is decidable from the locked row alone and no second table read enters the predicate. Thread `callerParticipantId: ParticipantId` as an explicit parameter on all five service methods (never read from a tRPC context, never from the request body) so the services stay transport-agnostic for the dual-transport mutations. Wire the **two-phase resolve** into the three `nodeId`-keyed procedures (`heartbeat`, `capabilityupdate`, `detach`): an unlocked pre-read by `node_id` over the active states yielding a `(session_id, participant_id)` snapshot that authorizes nothing; then locks in canonical order — `capabilityupdate` first taking level 1 (`sessions … FOR SHARE`: its `VERSION_FLOOR_EXCEEDED` gate needs the floor verdict authoritative-at-write per D8's mode-table arm; `heartbeat` is version-invariant and `detach` reads no floor, so both skip level 1); then a **re-verify of the locked row's `(session_id, participant_id)` against that snapshot**, failing closed on mismatch — level-2 modes per procedure (ADR-025 D8): `capabilityupdate` and `detach` take `FOR NO KEY UPDATE` (mutator paths), `heartbeat` takes `FOR SHARE`, its mutation targeting `runtime_node_presence` so its attachment acquisition is the read-only authorization arm and concurrent heartbeat retries never serialize against each other. Extend `updateCapabilities`'s `SELECT` to include `participant_id` (today it cannot compare ownership at all) and narrow its `FOR UPDATE` to `FOR NO KEY UPDATE`. Give `heartbeat`'s `ingest` a transaction: presence is writable only through an owned active attachment, so an unowned or unattached `node_id` writes nothing. Split `runtimenode.capabilityupdate_conflict`'s overloaded 409 — the no-active-row arm becomes the uniform negative, the I-003-2 `registering`-cannot-be-driven-`online` state guard keeps the 409. Bind the `runtimenode.permission_denied` exception class in `runtime-nodes/errors.ts`. **Any future `runtimenode.*` procedure MUST obtain its verdict from this helper** — a procedure that does not call it has no verdict at all, which is the structural answer to per-call-site rot.
...
- **Spec coverage:** Spec-003 AC1 (participant attaches a local runtime node to an already active session), Spec-003 §Required Behavior (attach must not require session recreation).
...
- **The five `runtimenode.*` control-plane procedures MUST NOT be exposed to untrusted cross-account traffic until the T3.10–T3.12 ownership guard is enforced** (dated note, 2026-08-25 — re-homed from BL-141 §V1 Release Impact at that item's archival; this bullet mints no task, invariant, or cross-plan obligation). The procedures may land functionally — the owner's own execution paths are unaffected — but a hosted control plane cannot ship a path where one account mutates another account's node. The gate is held structurally rather than by policy today: the control plane's `resolveCurrentParticipantId` throws until the authenticated-principal wiring lands PASETO validation ([ADR-010](../decisions/010-paseto-webauthn-mls-auth.md)), so no untrusted caller reaches these procedures, and the guard becomes production-live with no further Plan-003 code change when that host-wiring leg swaps the throw for the verified principal (`Spec-003 §Required Behavior`; [ADR-025](../decisions/025-runtime-node-control-plane-caller-authorization.md) D2). Sequencing consequence: if that wiring is scheduled ahead of T3.10–T3.12, this plan's Phase-3 supplement becomes a release blocker for it.
