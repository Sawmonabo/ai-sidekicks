...
Files: `packages/client-sdk/src/eventClient.ts` (NEW) + test; `packages/client-sdk/src/index.ts` (EXTEND); `packages/client-sdk/src/sessionClient.ts` (EXTEND — SDK cursor correction: `createDaemonSessionClient` stops synthesizing the reconnect cursor from `event.id` and consumes the daemon-provided encoded `EventCursor` (T4.3 `encodeEventCursor`) verbatim, so replay and reconnect agree on cursor identity) + its test. Provides: `EventClient` interface (`readAfterCursor`, `readWindow`, `subscribe`); `createDaemonEventClient(client: JsonRpcClient): EventClient` factory (daemon-only per ADR-017 — no control-plane variant since event reads go through daemon IPC, not control-plane endpoints). Sequence-monotonicity gap-detection on `subscribe` per the live-stream gap-detection resolution. Depends: T4.6 Zod schemas; Plan-007 transport substrate. IdempotencyClass: SDK wrapper inherits transport semantics.
