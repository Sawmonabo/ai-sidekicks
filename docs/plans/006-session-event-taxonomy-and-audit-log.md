...
This plan covers the `EventEnvelope` contract and version semantics, the 130-event taxonomy registry across 19 categories per [Spec-006 §Event Type Summary](../specs/006-session-event-taxonomy-and-audit-log.md) (lines 519+521 — canonical count), append-only persistence with BLAKE3 hash chain and Ed25519 signatures over RFC 8785 JCS canonical bytes, Merkle anchor emission into the shared `event_log_anchors` table (metadata only per [ADR-017](../decisions/017-shared-event-sourcing-scope.md)), the PII-column indirection pattern (`pii_payload` ciphertext + `pii_ciphertext_digest`), replay reads, live subscriptions, and compaction to audit stubs.
...
- `packages/runtime-daemon/src/events/canonicalizer.ts` — RFC 8785 JCS emitter with mandatory UTF-16 code-unit lex-sort field ordering per the canonicalization resolution + Spec-006:563 amendment + RFC 3339 UTC millisecond `occurredAt`
...
- `packages/runtime-daemon/src/events/integrity-verifier.ts` — read-side chain/signature/anchor/stub-signature/scalar-binding verifier; emits `audit_integrity_verified` / `audit_integrity_failed` with the 11-value `failureMode` enum from Spec-006:444 (post amendment for the four `anchor_missing_for_compacted_range` / `anchor_signature_invalid` / `stub_signature_invalid` / `stub_scalar_mismatch` modes added by the post-compaction integrity protocol per Spec-006 §Post-Compaction Integrity)
...
| --- | --- | --- |
| `session_lifecycle` (session + channel/agent + repo/workspace/worktree) | 25 | [Plan-001](./001-shared-session-core.md) (7 `session.*`) + [Plan-016](./016-multi-agent-channels-and-orchestration.md) (7 `channel.*`/`agent.*` incl. `channel.unmuted`, Tier-6 audit D-016-12) + [Plan-009](./009-repo-attachment-and-workspace-binding.md) + [Plan-010](./010-worktree-lifecycle-and-execution-modes.md) (11 `repo.*`/`workspace.*`/`worktree.*`) |
...
| `usage_telemetry` | 4 | [Plan-005](./005-provider-driver-contract-and-capabilities.md) (3 driver-sourced) + [Plan-016](./016-multi-agent-channels-and-orchestration.md) (`usage.budget_warning` from the BudgetAccountant, Tier-6 audit A-016-6) |
...
- **Spec coverage:** Spec-006 line 529 (channel_arbitration), Spec-006 line 538 (cross_node_dispatch), Spec-006 line 540 (onboarding_lifecycle)
- **Verifies invariant:** I-006-1-01 (category/type bijection — the 19-category union)

##### T1.2 — Enumerate 130 `SessionEventType` literals + category registry

File: `packages/contracts/src/event.ts` (EXTEND). Provides: `SessionEventType` literal union of all 130 strings; `SESSION_EVENT_CATEGORY_BY_TYPE: ReadonlyMap<SessionEventType, EventCategory>`; per-category const arrays (`SESSION_LIFECYCLE_EVENT_TYPES`, etc.). Depends: T1.1; existing 3-variant `SessionEvent` union; existing registry pattern at `event.ts:431`. IdempotencyClass: N/A (type/data).

- **Spec coverage:** Spec-006 line 519 (Event Type Summary), Spec-006 line 521 (Total enumerated event types), Spec-006 lines 525-547
- **Verifies invariant:** I-006-1-01 (130-type / 19-category bijection), I-006-1-02 (event-type-string immutability)

##### T1.3 — Author named `EventEnvelopeSchema` with canonical field set

File: `packages/contracts/src/event.ts` (EXTEND — refactor existing `buildCommonShape()` output to a named export). Provides: `EventEnvelopeSchema: z.ZodType<EventEnvelope>`; `EventEnvelope` inferred interface; JSDoc citing Spec-006:556-568 + ADR-018 §Decision #1/#2/#6 (the field SET is canonical; serialized order is mandated by RFC 8785 §3.2.3 UTF-16 lex-sort — see Spec-006:563 amendment). Depends: T1.1, T1.2, existing `EventEnvelopeVersionSchema` brand. IdempotencyClass: N/A (type).

- **Spec coverage:** Spec-006 line 563 (Fields included — the canonical set)
...
- **Spec coverage:** Spec-006 line 384 (`runtime_node.capability_declared`), Spec-006 line 385 (`runtime_node.capability_updated`)
...
- **Spec coverage:** Spec-006 line 87 (`VERSION_FLOOR_EXCEEDED`)
...
File: `docs/architecture/contracts/api-payload-contracts.md` (EDIT Plan-006 §EventEnvelope block, ~line 705-723). Provides: Updated `EventCategory` doc-mirror to 19 entries + comment "16 categories total" → "19 categories total per Spec-006 §Event Type Summary (130 event types)". Already applied in this audit PR. Depends: T1.1. IdempotencyClass: N/A (docs).

- **Spec coverage:** Spec-006 line 519 (Event Type Summary), Spec-006 lines 523-548
...
- **Spec coverage:** Spec-006 line 563 (Fields included — the canonical set), local-sqlite-schema.md `session_events`
...
- **Spec coverage:** Spec-006 line 560 (RFC 8785), Spec-006 line 563 (Fields included — the canonical set), Spec-006 line 565 (occurredAt), Spec-006 line 566 (pii_ciphertext_digest)
...
- **Spec coverage:** Spec-006 line 552 (chained to its predecessor), Spec-006 line 568 (row_hash)
...
- **Spec coverage:** Spec-006 line 558 (same canonical form), Spec-006 line 560 (RFC 8785), Spec-006 line 563 (Fields included — the canonical set), Spec-006 line 564 (present-but-null), Spec-006 line 565 (occurredAt)
...
- **Spec coverage:** Spec-006 line 552 (chained to its predecessor), security-architecture.md §Audit Log Integrity (sequence=0 prev_hash = 32 zero bytes — line 372; Spec-006:552 delegates the genesis-seed value to this doc)
...
- **Spec coverage:** Spec-006 lines 602-604, Spec-006 line 636 (audit-stub projection), Spec-006 line 642 (Anchor-before-compaction protocol), Spec-006 line 491 (event.compacted), Spec-006 line 435 (never compacted), Spec-006 line 482 (never compacted)
...
- **Spec coverage:** Spec-006 line 572 (ANCHOR_INTERVAL_EVENTS), Spec-006 line 645 (force-fire), Spec-006 line 646 (pending_anchor_uploads)
...
- **Spec coverage:** Spec-006 line 490 (schema.migrated)
...
- **Spec coverage:** Spec-006 line 492 (event.shredded), Spec-006 line 566 (pii_ciphertext_digest), Spec-006 lines 602-604, Spec-006 line 572 (ANCHOR_INTERVAL_EVENTS), Spec-006 line 568 (row_hash), Spec-006 line 490 (schema.migrated)
...
- **Spec coverage:** Spec-006 line 70 (EventReadAfterCursor), Spec-006 line 71 (EventReadWindow), Spec-006 line 64 (audit-visible stub)
...
- **Spec coverage:** Spec-006 line 676 (replay cursor tracks)
...
- **Spec coverage:** Spec-006 line 72 (EventSubscription), Spec-006 line 58 (monotonically increasing sequence)
...
- **Spec coverage:** Spec-006 line 70 (EventReadAfterCursor), Spec-006 line 71 (EventReadWindow), Spec-006 line 72 (EventSubscription)
...
- **Spec coverage:** Spec-006 line 70 (EventReadAfterCursor), Spec-006 line 71 (EventReadWindow), Spec-006 line 72 (EventSubscription), Spec-006 line 63 (live-stream gap)
...
Files: `apps/desktop/src/renderer/src/timeline/CompactedStubSegment.tsx` (NEW) + test. Provides: React component consuming `EventEnvelope` carrying audit-stub payload `{id, sessionId, sequence, occurredAt, category, type, actor, compactedAt, retentionClass: 'audit_stub', summary}` per Spec-006:621-634; single-line summary render + dimmed/badged visual distinction from live events; **NEVER renders null or empty** (I-006-4-07: audit stubs are always visibly present). Depends: T4.7 `EventClient`; Phase 1; Phase 3 T3.2 audit-stub format; existing renderer substrate (Plan-002 P5/P6 bridge pattern). IdempotencyClass: N/A (UI).

- **Spec coverage:** Spec-006 line 679 (retentionClass), Spec-006 lines 621-634, Spec-006 line 636 (never a silent omission)
...
- **Spec coverage:** Spec-006 line 64 (audit-visible stub), Spec-006 line 679 (retentionClass)
...
- **`pii_ciphertext_digest` position + wire shape.** Resolved: Top-level field on `payload` (matches Spec-006:566 + Spec-022:342 literal reading); wire shape is **lowercase hex 64-char string** (matches BLAKE3 reference output convention; T2.3 golden vectors are reviewer-friendly in hex).
...
- **CP-006-7 — Plan-002 amendment for daemon_signing_key generation at session-create.** Plan-002 (Tier 1, closed) reopens via amendment-via-extension PR to extend session-create with `DaemonSigningKeySource.create(sessionId)` call + participant-roster public-key registration. Amendment is scope-limited to call-site + roster row write; no master-key crypto leaks into Plan-002. Spec-006:568 (the daemon public key is the NodeId-resolved public key from the session participant roster used for signature verification) is the governing semantic.
- **CP-006-8 — Plan-013 audit-stub renderer surface (forward-declared).** Phase 4 T4.8 ships `<CompactedStubSegment>` as the audit-stub render contract per Spec-006:636. The future Plan-013 (Timeline Rendering And Replay UI) MUST consume — not re-implement. Bidirectional once Plan-013 lands; one-sided forward-declared today (verified via `ls`: `docs/plans/013-*.md` does not exist).
...
- **I-006-1-01 — Category/type bijection.** Every event type belongs to exactly one category; `SESSION_EVENT_CATEGORY_BY_TYPE.size === 130` and `new Set(SESSION_EVENT_CATEGORY_BY_TYPE.values()).size === 19`. Enforced via Phase 1 type-level exhaustiveness check + runtime assertion. Cite: Spec-006:519-548.
- **I-006-1-02 — Event-type-string immutability.** Type strings are immutable wire identifiers; renames are forbidden once a type is registered. Enforced via build-time const-assertion + ADR-018 §Decision #8 (MINOR additive-only). Cite: Spec-006:556-568; ADR-018 §Decision #8.
- **I-006-1-03 — Envelope field set is fixed; serialized order is RFC 8785 lex-sort.** The Zod schema declares the canonical field set; RFC 8785 §3.2.3 mandates UTF-16 code-unit lex-sort serialization. JSDoc on `EventEnvelopeSchema` references the Spec-006:563 amendment. Enforced via Phase 2 T2.3 golden-vector tests + Spec-006:563 prose amendment. Cite: Spec-006:556-568 + amendment; RFC 8785 §3.2.3.
...
- **I-006-2-03 — Canonical bytes are byte-stable across implementations.** T2.3 golden-vector tests + RFC 8785 Appendix-A conformance vectors guarantee byte-identical output for semantically-equal envelopes. Cite: RFC 8785; Spec-006:556-568.
- **I-006-2-04 — Genesis `prev_hash` = 32 zero bytes; subsequent `prev_hash[n] = row_hash[n-1]`.** Enforced via T2.6 backfill migration + persistence-layer assertion. Cite: Spec-006:552; Plan-006:136; Plan-001:306.
- **I-006-2-05 — Signed bytes commit to `pii_ciphertext_digest`, not raw ciphertext.** `daemon_signature` covers `canonical_bytes(row)` which includes the digest but excludes `pii_payload`; post-shred verification still succeeds. T2.5 verifies. Cite: Spec-022:332-359; Spec-006:554.
...
- **I-006-2-07 — `audit_integrity` and `event_maintenance` categories carry no `pii_payload`.** Discriminated-union type on `RawEventInput`: events in these categories have `pii_payload: never`. Cite: Spec-006:431+478; Spec-006:474.
...
- **I-006-4-01 — Exactly-one verifier emission per range.** `verifyRange({sessionId, fromSeq, toSeq})` emits exactly ONE of `audit_integrity_verified` or `audit_integrity_failed`; never zero, never two. Consumers dedupe by `(verifierNodeId, fromSeq, toSeq, verifiedAt)`. Cite: Spec-006:443-444.
...
- **I-006-4-03 — `refuse_on_rotation` invariant enforcement.** Key-reuse detection halts ingest of further events from the colliding signer's `NodeId`; `key_reuse_detected` payload carries `rotationInvariantViolated: 'refuse_on_rotation'` verbatim. T4.2 test verifies. Cite: Spec-006:437.
- **I-006-4-04 — Replay across compacted regions returns audit stubs, never silent omission.** `readAfterCursor` / `readWindow` return all sequence positions; compacted positions carry `payload.retentionClass === 'audit_stub'`. T4.3 test asserts. Cite: Spec-006:64; Spec-006:611-615.
...
- **I-006-4-07 — Audit stubs render as visible summarized segments.** `<CompactedStubSegment>` never renders null or empty; empty `summary` still produces a visible "(compacted, no summary)" segment. T4.8 + T4.9 tests verify. Cite: Spec-006:64; Spec-006:636.
