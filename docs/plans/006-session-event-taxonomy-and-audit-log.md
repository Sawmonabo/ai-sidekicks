...
| 31 | `user_text` | Wire echo | correlate | correlation-only — folds into the originating app-sent user-message row via `correlation_id` (delivery confirmation of the pending send; no category and no new persisted type) |
...
- **Spec coverage:** Spec-006 line 520 (event.shredded), Spec-006 line 609 (pii_ciphertext_digest), Spec-006 lines 646-648, Spec-006 line 615 (ANCHOR_INTERVAL_EVENTS), Spec-006 line 611 (row_hash), Spec-006 line 518 (schema.migrated), Spec-006 line 570 (run_lifecycle terminal-event set — backstop-index fail-loud test), Spec-006 line 190 (terminal-key CHECK — trigger-pair INSERT/UPDATE abort tests)
...
Files: `packages/runtime-daemon/src/events/replay-service.ts` (NEW) + test; `packages/contracts/src/session.ts` (EXTEND — Plan-001-owned, Plan-006 extends per the [§2 shared-file ownership rule](../architecture/cross-plan-dependencies.md#ownership-rule)): the `encodeEventCursor` / `decodeEventCursor` pair beside `EventCursorSchema`, and the `SessionReadResponseSchema.timelineCursors.earliest` optional-field widening, each with contracts-test coverage; `packages/runtime-daemon/src/ipc/handlers/session-read.ts` (EXTEND — the `session.read` materializer populates `timelineCursors.earliest`) + its handler test. Provides: `readAfterCursor(params): Promise<EventReadAfterCursorResponse>` returning `{events, nextCursor, hasMore}`; `readWindow(params): Promise<EventReadWindowResponse>` bounded by `[fromSequence, toSequence]`; events from compacted regions returned as audit-stub envelopes with `payload.retentionClass === 'audit_stub'` (I-006-4-04 — never silent omission). **Cursor encoding (LOCKED): the canonical `EventCursor` is the event's `sequence` rendered as a decimal string under the existing opaque `EventCursor` brand — clients keep treating it as opaque; the decimal-`sequence` encoding is a daemon-side guarantee, materialized by the `encodeEventCursor(sequence: number): EventCursor` / `decodeEventCursor(cursor: EventCursor): number` pair exported from `packages/contracts/src/session.ts` beside `EventCursorSchema`. `decodeEventCursor` is strict-decimal — a cursor that is not a canonical decimal `sequence` (a legacy SDK-synthesized UUID, a corrupted value) raises the typed `CURSOR_UNRESOLVABLE` error (the contracts error-taxonomy pattern in `packages/contracts/src/error.ts`, e.g. `VERSION_FLOOR_EXCEEDED_CODE`), never a silent parse and never a dual-format fallback decoder. Cursor semantics: a cursor denotes the position *after* the consumed event of `sequence` S; the `afterCursor` parameter is **optional** — present ⇒ `readAfterCursor` scans strict `sequence > decodeEventCursor(afterCursor)` in ascending order; **absent ⇒ an inclusive read from the true floor** (the oldest surviving row), matching SSE first-connect semantics exactly (no `Last-Event-ID` header ⇒ replay from the start). `hasMore` is `true` exactly when the page cap leaves rows still beyond `nextCursor`, and `nextCursor` is `encodeEventCursor` of the last returned event's sequence (an empty page returns the input cursor unchanged with `hasMore: false`). Compacted ranges are gap-inverted — a compacted span is never a hole in the sequence scan; each compacted position surfaces its audit-stub envelope at that exact sequence, so the cursor advances monotonically across compacted ranges and a reader can never step past a compacted region without observing it (the I-006-4-04 no-silent-omission guarantee at the cursor layer). SSE resumption: the `Last-Event-ID` header value is `encodeEventCursor(envelope.sequence)`, derived from each delivered envelope's required `sequence` field (`EventEnvelope.sequence`, `packages/contracts/src/event.ts` — always present, so `LocalSubscriptionProducer<EventEnvelope>` already carries it and no notification-schema change is needed; T4.7 retires the old UUID synthesis) — a reconnecting subscriber resumes exactly after its last-delivered sequence, replaying any compacted-as-stub positions it had not yet seen. **Cursorless-floor-read recovery (shared primitive):** the `afterCursor`-omitted floor read plus full re-projection is the single recovery path that three failure modes converge on — a `CURSOR_UNRESOLVABLE` decode (legacy or corrupted cursor), gap detection against `earliest` (below), and the live-stream sequence-gap detection (T4.7) — so legacy cursors, corrupted cursors, and compaction/reconnect gaps recover through one tested mechanism rather than a per-format shim. Rationale: pre-1.0, no shipped external client holds a durable cursor (the UUID synthesis is in-repo only, retired here), so the format is fixed pre-release and the general recovery path is the durable mechanism. Also populates `timelineCursors.earliest?: EventCursor` — **advisory floor metadata, never a resume cursor** (the `SessionReadResponseSchema` widening + `session.read` materializer are in Files; architecture mirror in [api-payload-contracts.md](../architecture/contracts/api-payload-contracts.md)): the cursor of the oldest surviving row, a V1-constant `encodeEventCursor(0)` because compaction rewrites rows to audit stubs in place but never deletes them (it advances only if a future retention pass deletes rows — stated so the constant is not read as a guarantee). Its sole consumer role is **gap detection** — `decodeEventCursor(acknowledged) < decodeEventCursor(earliest)` ⇒ the acknowledged position fell below the surviving floor ⇒ events were lost ⇒ resync via the cursorless floor read above; a resume never passes `earliest` as `afterCursor`. Because the V1 floor is constant at `encodeEventCursor(0)` this check is dormant until a future retention pass moves it, but the wiring ships so the path is tested and correct when that lands. `earliest` is **optional** for version skew: a new daemon ALWAYS populates it, but an **absent** `earliest` ⇒ the responder predates the field, so the client cannot gap-detect and conservatively performs the cursorless floor read (the same safe path); it becomes required at the next MAJOR envelope version per the `ADR-018` additive-MINOR-now versioning ladder.** Depends: T4.6 Zod schemas; Phase 1 contracts; Phase 3 T3.2 compactor format; Plan-001 daemon-migration substrate; `ADR-018 §Decision` #6 + #9 (upcaster chain on read; unknown-MAJOR accept-and-stub). IdempotencyClass: `idempotent` (pure read).
...
Files: `packages/client-sdk/src/eventClient.ts` (NEW) + test; `packages/client-sdk/src/index.ts` (EXTEND); `packages/client-sdk/src/sessionClient.ts` (EXTEND — SDK cursor correction: `createDaemonSessionClient` stops synthesizing the reconnect cursor from `event.id` and instead derives it as `encodeEventCursor(envelope.sequence)` from each delivered envelope's required `sequence` field — `EventEnvelope.sequence` is always present (`packages/contracts/src/event.ts`), so T4.5's `LocalSubscriptionProducer<EventEnvelope>` already carries it and no notification-schema or `session-subscribe.ts` change is needed — and consumes the daemon-provided `nextCursor` verbatim on reads, so replay and reconnect agree on cursor identity) + its test, including a named round-trip test: subscribe → receive envelope → `encodeEventCursor(envelope.sequence)` → disconnect → `readAfterCursor` resume against a concurrent append → no gap, no duplicate. Provides: `EventClient` interface (`readAfterCursor`, `readWindow`, `subscribe`); `createDaemonEventClient(client: JsonRpcClient): EventClient` factory (daemon-only per ADR-017 — no control-plane variant since event reads go through daemon IPC, not control-plane endpoints). Sequence-monotonicity gap-detection on `subscribe` per the live-stream gap-detection resolution. Depends: T4.6 Zod schemas; Plan-007 transport substrate. IdempotencyClass: SDK wrapper inherits transport semantics.
