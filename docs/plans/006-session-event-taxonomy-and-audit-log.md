...
- **Golden-vector storage: inline hex vs JSON fixture.** Resolved: Inline hex strings in the test file. Matches crypto-paseto convention (`packages/crypto-paseto/src/__tests__/v4-local.test.ts#encryptV4LocalDeterministic` — the deterministic-variant golden-token tests); PR-diff is the audit trail for byte-stable artifacts.
