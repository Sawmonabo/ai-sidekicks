...
File: `packages/contracts/src/event.ts` (EXTEND with replay-shape Zod schemas; co-located per Plan-001's contracts package layout rather than a separate `replay-shapes.ts`). Provides: `EventReadAfterCursorRequestSchema` + `Response`; `EventReadWindowRequestSchema` + `Response`; `EventSubscriptionRequestSchema`; all `z.infer<>` typed exports. Mirrors api-payload-contracts.md:1068-1095 verbatim. Depends: Phase 1 contracts (`EventEnvelope`, `EventCursor`, `SessionId`). IdempotencyClass: N/A (type).
