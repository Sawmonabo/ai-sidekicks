...
The `agent_definitions` table and its migration; the `sidekick.*` daemon JSON-RPC namespace; the definition store and its case-insensitive uniqueness enforcement; the fail-closed resolver; the run-start integration that consumes it; the bridge's six `SessionCallbackTool` registrations, their session-scoped enablement, and their invocation handler; the CLI command surface; and the desktop definition-editor subtree handed to the shell's mount point.
...
Resolution copies the definition's values onto the run's own sidekick record when the run starts. No read path serving a running sidekick's configuration consults `agent_definitions`, and no foreign key binds a sidekick to the definition it was resolved from. Display is the one exception: a running sidekick's name, icon, and accent are read from the definition at the moment they are drawn.
...
- `apps/desktop/src/renderer/src/console/sidekicks/` (NEW) — the destination's own family, handed to Plan-021's mount under CP-027-2:
  ...
  - `library/` — `AgentLibrary.tsx`, `AgentCard.tsx`, `library-filters.ts`, `library-view.ts` (the read, delete and duplicate state machine).
  - `editor/` — `AgentEditor.tsx`, `InstructionsField.tsx`, `GoalField.tsx`, `PostureModeField.tsx`, `ToolAllowlistPicker.tsx`, `ProviderBindings.tsx`, `IconField.tsx`, `TryItPanel.tsx`.
...
- Five daemon JSON-RPC pairs under the new `sidekick` root: `agent.definitionCreate`, `agent.definitionUpdate`, `agent.definitionDelete`, `agent.definitionList`, and `agent.peerInvocationSet`. There is no separate read verb — `definitionList` returns full records, matching the `providerAccount.list` shape.
...
2. Build the definition store with storage-level case-insensitive uniqueness, then the `sidekick.*` handlers behind the `agent::manage` Cedar action.
...
- **T1.3 — Contract ↔ DDL conformance suite.**
  - **Files:** `packages/runtime-daemon/src/agents/__tests__/sidekick-definition-schema-conformance.test.ts` (NEW).
...
- **T5.2 — Peer-invocation enablement control.**
  ...
  - **Provides:** the session-scoped enable / disable control over `agent.peerInvocationSet`, stating that enabling grants automatic peer invocation for the remainder of the session and that disabling takes effect on the next invocation on every leg, including legs already running.
