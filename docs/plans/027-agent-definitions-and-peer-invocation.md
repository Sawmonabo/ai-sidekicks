...
The bridge's six verbs are registered at spawn unconditionally, and every invocation is adjudicated against `Action::"agent::invoke"` at call time, against the session's own permission level. The only filter over the registry is the resolved agent's tool allowlist; a Cedar denial is answered on the callback-tool result's `denied` arm, on the leg already running and with no respawn.
...
**Why load-bearing.** This is a correctness property, not a stylistic one. `callbackTools` rides only the session-creation and resume parameter shapes and there is no live-registry mutation seam, so a registry composed against session state is frozen at spawn: a widening would change nothing until the leg respawned, and — worse in the other direction — a narrowing would leave a live leg holding authority the operator believes they revoked until its next spawn boundary. Per-call adjudication makes both directions immediate and needs no seam that does not exist. It also matches the landed `workflow_start` precedent, the corpus's only other concrete session callback tool.

**Verification.** T4.1 asserting all six verbs present in a freshly composed registry and that the module reads no session state; T4.3 asserting a Cedar-denied call answers `denied` on an already-running leg with no respawn.
...
**Why surfaced here.** Definition mutation is authorized under `agent::manage` against a node-scoped resource, while each invocation is checked under `agent::invoke` against the session it runs in. Recording both actions together is what keeps "who may author the node's agents" and "who may run one from inside a session" separable.
...
**Why surfaced here.** Without the route, a session's bridge verbs reach a host that can neither authorize nor represent them, and the fail-closed backstop answers `denied` — the feature would ship inert while every doc claimed it worked.
...
- [ ] **Named-action callback dispatch route registered (CP-027-6)** — born unchecked 2026-08-26 at the round-2 review fold. Holds **T4.1 and T4.3 directly**, and nothing else: Phase 5 gates on Phase 3 rather than on Phase 4, so the library and the editor dispatch while these two are held. T1.x–T3.x, T4.4 and Phase 5 dispatch on tier order and their phase gates. Stated with the held set spelled out because a reader scheduling off a bare "Phase 4 is held" would defer Phase-5 work that nothing holds. The Plan-004 callback-tool host builds `approval.requestCreate`-shaped evaluation inputs keyed by `ApprovalCategory`, and `agent::invoke` deliberately has none (D-027-4), so a peer-invocation call cannot be represented in that input shape — not a prompting nuisance but an unrepresentable call. Checked when Plan-004 registers the named-action route CP-027-6 describes, in its own swap. Deliberately a box rather than a machine-checkable `external_plan_phase_merged` gate: the blocker is a contract Plan-004 has not yet authored, not a phase awaiting merge.
...
- **T2.2 — Definition-plane `agent.*` handlers and Cedar authorization.**
  ...
  - **Tests:** each verb refused for a principal lacking the action; a definition mutation authorized against the node-scoped resource descriptor, asserting the path invents no `sessionId`; a negative check asserting no handler in this module writes a `remembered_approval_rules` row, since a named-operation action cannot be represented as one; the definition-plane census asserted at four pairs.
