...
- **T5.2 — The session's peer-calling setting.**
  - **Files:** `apps/desktop/src/renderer/src/console/agents/agent-console/PeerInvocationSetting.tsx` (NEW).
  - **Provides:** the one-session setting over `agent.peerInvocationSet`, reading `Sidekicks may call each other` and starting off, stating that turning it on grants automatic peer invocation for the remainder of the session and that turning it off takes effect on the next invocation on every leg, including legs already running. No settings page carries it.
  - **Consumes:** T4.2 — the enablement leg this setting is presentation over, which is why Phase 5 gates on Phase 4; the `SessionReadResponse` enablement projection under CP-027-8, which is how the setting learns the current value on mount rather than folding raw events client-side; Plan-021's agent console mount per CP-027-2.
