...
- **CP-018-11 — Path-2 crypto-shred reciprocal (⇄ [Plan-022](./022-data-retention-and-gdpr.md) CP-022-6).** On a valid participant purge (`DELETE /participants/{id}/data`), the Plan-018-owned `participants` identity columns + `identity_mappings` rows are **hard-DELETEd** in the Spec-022 Postgres-side shred fan-out (Spec-022:298-300, §Shred Fan-Out Path 2). Reciprocal of Plan-022 CP-022-6 (encoded fix-in-place at the Tier-5 audit swap, satisfying Plan-022 I-022-19); shred-handler provider: Plan-022 (V1.1).
