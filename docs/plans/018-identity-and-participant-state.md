...
- **CP-018-11 — Path-2 crypto-shred reciprocal (⇄ [Plan-022](./022-data-retention-and-gdpr.md) CP-022-6).** On a valid participant purge (`DELETE /participants/{id}/data`), the Plan-018-owned `participants` identity columns + `identity_mappings` rows are **hard-DELETEd** in the Spec-022 Postgres-side shred fan-out (Spec-022:294-296, §Shred Fan-Out Path 2). Reciprocal of Plan-022 CP-022-6 (encoded fix-in-place at the Tier-5 audit swap, satisfying Plan-022 I-022-19); shred-handler provider: Plan-022 (V1.1).
...
- **T2.1 — Idempotent identity→participant upsert (participant-first FK ordering).**
  - Files: `packages/control-plane/src/participants/participant-mapping-service.ts` (CREATE); migration `packages/control-plane/src/migrations/0003-participants-identity.ts` (CREATE — `participants` ALTER (+`display_name`, +`identity_ref`, +`metadata`) + `identity_mappings` CREATE) wired into the Plan-001-owned `migration-runner.ts` as `{ version: 3, sql: PARTICIPANTS_IDENTITY_MIGRATION_SQL }` — the next free `schema_migrations` version after the v2 `session-invites` ship; Plan-018 lands before Plan-008-remainder so this takes **v3** and Plan-008's relay migration takes **v4** ([cross-plan-dependencies.md §5](../architecture/cross-plan-dependencies.md#5-canonical-build-order) Tier-5 migration allocation)
...
- **T4.5 — `RelayConnectionTokenIssuer` (relay connection-token custody, Plan-018-owned).**
  ...
  - Spec coverage: Spec-008:156 (300s-TTL `RelayNegotiationResponse.connectionToken`), Spec-008 §Relay Negotiation (connection-token issuance)
  ...
  - Consumes: `@ai-sidekicks/crypto-paseto` v4.public sign primitives (Plan-025 Tier 1 Partial, shipped PR #92); the `connectionToken` claim shape (Spec-008 §Relay Negotiation)
  ...
  - Note: independent of T4.1–T4.4 (no intra-phase dependency — consumes only the shipped crypto-paseto substrate + the Spec-008 claim shape); lands so Plan-018 ships the issuer before Plan-008-remainder R2/R3 per F-008r-C.
...
- **T4.6 — `participant.*` daemon-side IPC handlers (daemon-as-gateway proxy).**
  ...
  - Spec coverage: Spec-018:61,62,63 (the three reads/updates the handlers answer), Spec-018:69 (daemon-as-gateway transport boundary, ADR-008)
  ...
  - Note: mirrors the Plan-002 `presence.*` handler-registration precedent (`packages/runtime-daemon/src/ipc/handlers/{presence-subscribe,presence-read}.ts`, NS-26 / PR #108) and the Plan-022 `gdpr-stub-handlers.ts` precedent — each plan that adds a daemon-proxied namespace registers explicit handler files under `handlers/`. **Without this task the `participant.*` method strings T4.1 registers (CP-018-6) have no daemon-side responder — the SDK call resolves to `method-not-found` at runtime** (the F2 round-4 Codex finding). Independent of T4.1–T4.5 for unit-test purposes (handlers test against a stubbed control-plane service), but completes the runtime transport loop the client half assumes.
