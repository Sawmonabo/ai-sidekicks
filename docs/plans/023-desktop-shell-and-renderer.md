...
**Precondition:** None at the plan level. Tier 1 entry point. The placeholder `apps/desktop/src/index.ts` shipped by Plan-001 Phase 1 (PR #6) is repositioned during this PR per its forward-declaration comment ("split into `apps/desktop/{shell,renderer}` per Container Architecture").
