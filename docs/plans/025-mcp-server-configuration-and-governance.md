...
- **T28.5.7 — Desktop MCP-governance settings page (CP-025-6, CP-025-8).**
  ...
  - Panel views reading `mcp.list` / `mcp.get` and the `mcp.subscribe` stream **only** via the `window.desktopBridge` bridge — no direct daemon access from the renderer (the CP-014-11 pattern; live bridge verification arrives at Plan-021 Tier 7).
  ...
  - **Consumes:** the `window.desktopBridge` bridge stub + renderer substrate ← Plan-021-partial Tier 1 (shipped; prose-only precondition — shipped upstreams carry no YAML gate).
