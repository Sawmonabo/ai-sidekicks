...
The following invariants are **load-bearing** and MUST be preserved across all Plan-025 PRs and downstream extensions. Any change that would weaken or remove an invariant is agreed with the counterpart plans, not taken locally (see cross-plan-dependencies.md).

### I-025-1 — No credential custody

The daemon never persists, logs, relays, or embeds in events/errors any OAuth token, authorization code, PKCE material, bearer-token value, or env-var value belonging to an MCP server. The `mcp.oauthLogin` idempotency receipt stores its acknowledgment with `authorizationUrl` structurally omitted — launch URLs embed single-use PKCE state, so they are never durable and its replay is a URL-free acknowledgment. That URL's **sole carrier is the `mcp.oauthLogin` reply**: it appears on no durable and no broadcast destination, and each client consumes it at exactly one — the login command's own result stream, and transient renderer state cleared at settlement. The only durable auth trace is the `mcp.server_oauth_completed` event (identity + outcome).
...
The eleven `mcp.*` operations register against `MethodRegistry.register()` (`packages/contracts/src/jsonrpc-registry.ts`, shipped) when this plan lands — the Plan-006 CP-006-3 late-namespace pattern (`presence.*` precedent: namespace owners register when they land against the stable substrate); `mcp.subscribe` additionally rides Plan-006's streaming primitive (the `session.subscribe` long-lived consumer shape).
...
- SQLite census 52 → 55; the migration lands in Phase 1.
...
Plan-025 implementation lands as five PRs (one per phase). Each PR carries a `**Precondition:**` line so the merge order is reviewer-checkable. Code dispatch rides this plan's §Preconditions; no task carries an additional hold.
...
- **T28.1.4 — The five `mcp_governance` event payload schemas.**
  ...
  - **Verifies invariant:** I-025-1
  - **Consumes:** the five `mcp.*` type literals + the `mcp_governance` category ← Plan-005 T1.10 (shipped); `EventEnvelope` ← Plan-005 Phase 1 (shipped).
...
- **T28.1.5 — Error-code constants and typed daemon error classes.**
  ...
  - The ten `mcp.*` codes as typed constants matching error-contracts.md §MCP Governance byte-for-byte, plus the typed refusal classes subclassing `DaemonDomainError` in this Plan-025-owned module — keeping per-namespace classes out of the Plan-006 substrate (no per-namespace mapping-table maintenance; the Plan-008 `worktree-errors.ts` precedent), so each refusal reaches the wire as `data.type` with sanitized `data.fields` through the existing discriminator branch and `packages/runtime-daemon/src/ipc/domain-error.ts` is never edited.
...
- **T28.1.6 — Three-table migration module and runner registration.**
  ...
  - **Consumes:** `migration-runner.ts` guarded-block convention ← Plan-001 (shipped).
...
- **T28.4.3 — Cedar `mcp` action family registration and the deny-before-effect retrofit.**
  ...
  - **Spec coverage:** Spec-025 §Authorization
  - **Verifies invariant:** I-025-4
  - **Consumes:** `PermissionCheckService.check()` ← Plan-010 Phase 2 (§Precondition, `{plan: 010, phase: 2, status: merged}`; reciprocal return-cite on Plan-010 CP-010-4).
...
- **T28.4.6 — Drift-admission service registered through the Plan-003 `RunSetupGate` seam.**
  ...
  - **Spec coverage:** Spec-025 §Trust Governance
  - **Verifies invariant:** I-025-2
  - **Consumes:** the `RunSetupGate` registration seam ← Plan-003 Phase 3, CP-003-8 (§Precondition, `{plan: 003, phase: 3, status: merged}`; reciprocal extender enumeration on Plan-003 CP-003-8).
...
- **T28.4.7 — Recovery-attach composition-root wiring and the non-vacuous production assertion.**
  ...
  - **Spec coverage:** Spec-025 §Trust Governance
  - **Verifies invariant:** I-025-2
  - **Consumes:** the `assertAttachAdmissible` vacuous-default seam ← Plan-013 T15.3 (order-independent, no YAML gate; reciprocal already present in Plan-013's T15.3 attach-seam clause naming Plan-025 CP-025-5).
...
**Precondition:** Phase 4 merged; Plan-021-partial renderer substrate (shipped — prose-only precondition per CP-025-6: shipped upstreams carry no YAML gate).
...
- **T28.5.7 — Desktop MCP-governance settings page (CP-025-6, CP-025-8).**
  ...
  - **Consumes:** the `window.desktopBridge` bridge stub + renderer substrate ← Plan-021-partial (shipped; prose-only precondition — shipped upstreams carry no YAML gate).
