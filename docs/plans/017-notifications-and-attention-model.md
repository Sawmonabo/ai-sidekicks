...
| **Dependencies** | [Plan-011](./011-live-timeline-visibility-and-reasoning-surfaces.md) (timeline visibility), [Plan-028](./028-remote-control.md) (SSE catch-up cursor replay — [Spec-017 §Notification Delivery](../specs/017-notifications-and-attention-model.md#notification-delivery) routes reconnect catch-up for queued notifications through the relay's EventSource `Last-Event-ID` resumption, so undelivered notifications re-deliver rather than re-derive), [Plan-020](./020-data-retention-and-gdpr.md) (user-purge shred fan-out for the Plan-017-owned Postgres tables per CP-017-1 ⇄ CP-020-6), [Plan-005](./005-session-event-taxonomy-and-audit-log.md) (constructor-injected `DaemonCredentialProvider` credential seam — the CP-005-13 shape T3.1's `attention.notificationEmit` publisher authenticates through) — the relay and Plan-020 edges added by the Tier-7 audit, the Plan-005 edge by its PR #318 review round, all mirroring the dependency-graph rows |
...
- Deliver notifications over the existing control-plane SSE subscription rather than a new endpoint, per [Spec-017 §Desktop-to-Desktop Delivery](../specs/017-notifications-and-attention-model.md#desktop-to-desktop-delivery) and [ADR-014](../decisions/014-trpc-control-plane-api.md); [Spec-017 §Cross-Device Delivery](../specs/017-notifications-and-attention-model.md#cross-device-delivery) adds no webhook surface in V1.
...
- **Aggregate-carrier derivation rule (D-017-2).** [Spec-017 §Required Behavior](../specs/017-notifications-and-attention-model.md#required-behavior) requires attention at both run scope and session scope, and the shared `AttentionItem` shape in [api-payload-contracts.md](../architecture/contracts/api-payload-contracts.md) carries a single `trigger`, a single `severity`, and a single `sourceEventId` — leaving it unstated which field carries an aggregate over several unresolved contributors, and where a derived item's `sourceEventId` comes from. This plan fixes the rule; it changes no field shape:
