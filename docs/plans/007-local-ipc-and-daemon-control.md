...
- [x] [error-contracts.md §JSON-RPC Wire Mapping](../architecture/contracts/error-contracts.md#json-rpc-wire-mapping) declares JSON-RPC numeric-error-space (`-32700` / `-32600` / `-32601` / `-32602` / `-32603`) ↔ project dotted-namespace `data.type` two-layer envelope per JSON-RPC 2.0 §5.1 + RFC 7807 + LSP 3.17 ResponseError precedent; declares `unknown_setting` (`-32602`), `transport.unavailable` (`-32603`), and `resource.limit_exceeded` (`-32600`) Tier-1 domain identifiers with structured `data.fields` shapes — closed 2026-05-01 via [BL-103](../backlog.md) ratification.
