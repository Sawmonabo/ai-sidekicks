...
- [x] BLOCKED-ON-C7 closed 2026-05-01 via [BL-103](../backlog.md) ratification: [error-contracts.md §JSON-RPC Wire Mapping](../architecture/contracts/error-contracts.md#json-rpc-wire-mapping) declares the numeric ↔ dotted-namespace two-layer envelope (JSON-RPC 2.0 §5.1 `code` + `data.type` + `data.fields`) per RFC 7807 + LSP 3.17 ResponseError precedent; `unknown_setting`, `transport.unavailable`, `transport.message_too_large` Tier-1 domain identifiers shipped with structured `data.fields` shapes (the transport-layer 413-semantic `transport.message_too_large` is registered distinct from Spec-001's domain-quota 429-semantic `resource.limit_exceeded` per the canonicalization round-trip closed 2026-05-01).
