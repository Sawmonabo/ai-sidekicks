...
**Coverage scope note (retroactive, BL-113 audit).** Spec-007 §Acceptance Criteria coverage for `session.*` is verified by direct round-trip at `session.create` + `session.subscribe` (I-007-3-T1 + I-007-3-T3); `session.read` + `session.join` are verified by symmetry argument through the register-binding tests (`I-007-3-T5` + the handlers' shared `router.register` substrate). Per-method ACs (AC-N1..N4 for `session.create` / `session.read` / `session.join` / `session.subscribe`) are owed at the Spec-007 §Acceptance Criteria amendment — tracked separately via BL-114 (per F-007-3-08 + F-007-3-10).
