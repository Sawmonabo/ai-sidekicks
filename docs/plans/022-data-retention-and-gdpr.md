...
- **D-022-1 — Master-key custody Type-2 parameters → ratified.** (a) The spec's KEK-wrapped custody model (Argon2id V1 baseline, refuse-over-plaintext, reject keytar/base64) supersedes the plan's prior `2026-04-17` base64-plaintext custody — confirmed spec-canonical (`Spec-022 §Daemon Master Key`; Plan-006 T2.7; Plan-007 CP-007-8; [cross-plan-deps:26](../architecture/cross-plan-dependencies.md)). (b) The master-envelope AEAD AAD is the **full 50-byte header** (binding the version byte / KEK-branch discriminator), resolving the Spec-022 nonce-AAD-vs-full-header-AAD self-contradiction under `Spec-022 §Daemon Master Key` in favour of the full-header clause (`ADR-021 §Tier 2 — Argon2id-encrypted file (fallback)`). The Spec-022 back-fill fixing that self-contradiction lands in this swap. **Type 2.**
