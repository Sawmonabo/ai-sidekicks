...
- **CP-022-6 — Path-2 fan-out (⇄ Plan-001/002/003/014/018/019, + forward-declared Plan-027/BL-070) — the complete inbound-FK closure.** Path-2's deletion/anonymization set is the **complete `REFERENCES participants(id)` inbound-foreign-key closure** in [shared-postgres-schema.md](../architecture/schemas/shared-postgres-schema.md) (currently eleven rows — derived from the schema, not a hand-maintained list, so the set cannot silently drift; the count moved nine → eleven when the 2026-07-08 cross-node relay amendment added Plan-014's two `artifact_relay_*` tables; Spec-022 [§Shred Fan-Out](../specs/022-data-retention-and-gdpr.md#shred-fan-out) Path-2 Mechanism is reconciled to this closure at this swap per [§Ratified Design Decisions](#ratified-design-decisions-tier-5-audit-2026-05-30) D-022-7). Each owner carries the reciprocal:
