...
- **CP-022-6 — Path-2 fan-out (⇄ Plan-001/002/003/018/019, + forward-declared Plan-027/BL-070) — the complete inbound-FK closure.** Path-2's deletion/anonymization set is the **complete `REFERENCES participants(id)` inbound-foreign-key closure** in [shared-postgres-schema.md](../architecture/schemas/shared-postgres-schema.md) (currently nine rows — derived from the schema, not a hand-maintained list, so the set cannot silently drift; Spec-022 [§Shred Fan-Out](../specs/022-data-retention-and-gdpr.md#shred-fan-out) Path-2 Mechanism is reconciled to this closure at this swap per [§Ratified Design Decisions](#ratified-design-decisions-tier-5-audit-2026-05-30) D-022-7). Each owner carries the reciprocal:
  - **Live reciprocals (in-tier, encoded fix-in-place at this swap):** Plan-018 (`participants`, `identity_mappings` hard-DELETE — CP-018-11), Plan-019 (`notification_preferences` hard-DELETE — CP-019-1), Plan-002 (`session_invites.inviter_id` anonymize — CP-002-8), Plan-001 (`session_memberships` anonymize — CP-001-3), **Plan-003 (`runtime_node_attachments` hard-DELETE — [CP-003-4](./003-runtime-node-attach.md#cp-003-4--runtime_node_attachments-is-a-path-2-participant-erasure-target--plan-022-cp-022-6), new at this swap)**.
...
11. **Reserve Shred Fan-Out Orchestration surface (V1.1+).** Plan-022 is the V1 schema + write-path surface; the real participant-delete handler is V1.1+ (per §Non-Goals — not-implemented stub methods in V1). When the real handler ships in V1.1, it MUST execute these three paths in the order below before emitting `participant.purged`, per [Spec-022 §Shred Fan-Out](../specs/022-data-retention-and-gdpr.md#shred-fan-out):
    ...
    2. **Path 2 — Postgres hard DELETE / anonymize.** Operate over the complete `REFERENCES participants(id)` inbound-FK closure (currently nine rows; [shared-postgres-schema.md](../architecture/schemas/shared-postgres-schema.md)): hard-DELETE `participants`, `identity_mappings` (Plan-018), `notification_preferences` (Plan-019), `runtime_node_attachments` (Plan-003); anonymize `session_invites.inviter_id` (Plan-002) and `session_memberships.participant_id` (Plan-001) via `ON DELETE SET NULL` ([Spec-022 §Shred Fan-Out](../specs/022-data-retention-and-gdpr.md#shred-fan-out) Path 2 FK-safety — the four anonymize-class FKs reach nullable + `ON DELETE SET NULL`: the two in-V1 FKs relaxed by this plan's V1 control-plane migration T22.5.2, the two `revoked_*` FKs born nullable at their BL-070 build, D-022-7). The forward-declared owners complete when built/audited (`cross_node_dispatch_coordination`/Plan-027 Tier 9 hard-DELETE; `revoked_jtis`/`revoked_token_families`/BL-070 anonymize-`participant_id`-with-denylist-survival). Plan-022 is the orchestrator; each owner carries its reciprocal — see [§Cross-Plan Obligations](#cross-plan-obligations) CP-022-6. Any DELETE failure reports the whole path failed; daemon does not partially advance.
