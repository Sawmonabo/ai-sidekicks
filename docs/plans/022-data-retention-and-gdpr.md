...
Per [Spec-022 §PII Data Map](../specs/022-data-retention-and-gdpr.md#pii-data-map), three PII durability tiers are reachable from `DELETE /participants/{id}/data`. Plan-022 owns the V1 schema and write-path for the durable-tier SQLite side (below); the Postgres side of the durable tier is owned plan-by-plan — Plan-018 (`participants`, `identity_mappings`), Plan-002 (`session_invites`), Plan-019 (`notification_preferences`), Plan-001 (`session_memberships`); Plan-020 owns the bounded-retention tier; the telemetry-export tier is covered by Plan-020's default-deny outbound posture. Plan-022 is the orchestrator that reconciles the three tiers when the V1.1 deletion handler ships (see Implementation Step 11 + [§Signature Safety Under Shred](#signature-safety-under-shred)). These three durability tiers classify the participant's **PII content** by where it lives (the crypto-shred / anonymization / flush targets); the deletion handler's **relational** obligation is a distinct, broader lens — the complete `REFERENCES participants(id)` inbound-FK closure of CP-022-6, which additionally reaches pure-linkage and operational rows (`runtime_node_attachments`, `cross_node_dispatch_coordination`) plus the security-survival denylist (`revoked_jtis`/`revoked_token_families`) that FK-link to the participant without carrying a PII column.
...
11. **Reserve Shred Fan-Out Orchestration surface (V1.1+).** Plan-022 is the V1 schema + write-path surface; the real participant-delete handler is V1.1+ (per §Non-Goals — not-implemented stub methods in V1). When the real handler ships in V1.1, it MUST execute these three paths in the order below before emitting `participant.purged`, per [Spec-022 §Shred Fan-Out](../specs/022-data-retention-and-gdpr.md#shred-fan-out):
    ...
    2. **Path 2 — Postgres hard DELETE / anonymize.** Operate over the complete `REFERENCES participants(id)` inbound-FK closure (currently twelve rows; [shared-postgres-schema.md](../architecture/schemas/shared-postgres-schema.md)): hard-DELETE `participants`, `identity_mappings` (Plan-018), `notification_preferences` (Plan-019), `runtime_node_attachments` (Plan-003), `artifact_relay_recipients` (Plan-014 — the wrapped-CEK crypto-shred, CP-014-2); anonymize `session_invites.inviter_id` (Plan-002), `session_memberships.participant_id` (Plan-001), `artifact_relay_blobs.publisher_participant_id` (Plan-014 — born nullable at its Tier-7 build), and `session_terminal_leases.holder_participant_id` (Plan-024 Phase 3B — born nullable; auto-nulling frees the lease fail-closed) via `ON DELETE SET NULL` ([Spec-022 §Shred Fan-Out](../specs/022-data-retention-and-gdpr.md#shred-fan-out) Path 2 FK-safety — the six anonymize-class FKs reach nullable + `ON DELETE SET NULL`: the two in-V1 FKs relaxed by this plan's V1 control-plane migration T22.5.2, the two `revoked_*` FKs born nullable at their BL-070 build, and `artifact_relay_blobs.publisher_participant_id` / `session_terminal_leases.holder_participant_id` born nullable at their Plan-014 Tier-7 and Plan-024 Phase 3B builds, D-022-7). The forward-declared owners complete when built/audited (`cross_node_dispatch_coordination`/Plan-027 Tier 9 hard-DELETE; `revoked_jtis`/`revoked_token_families`/BL-070 anonymize-`participant_id`-with-denylist-survival). Plan-022 is the orchestrator; each owner carries its reciprocal — see [§Cross-Plan Obligations](#cross-plan-obligations) CP-022-6. Any DELETE failure reports the whole path failed; daemon does not partially advance.
