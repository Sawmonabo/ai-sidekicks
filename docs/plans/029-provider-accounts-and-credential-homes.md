...
- Refusal codes for the fail-closed paths in [Error Contracts](../architecture/contracts/error-contracts.md) — **four added** by the 2026-08-26 amendment (`signin_unsupported`, `signin_in_flight`, `token_class_refused`, `credential_seal_refused`), moving this plane's registered code census seven → eleven.
...
- **T2.7 — Non-interactive token admission and sealing.**
  ...
  - **Provides:** the [ADR-028](../decisions/028-provider-credential-custody-posture.md) D2 admission gate over the four conjunctive conditions, sealing through the [ADR-021](../decisions/021-cli-identity-key-storage-custody.md) ladder, the `logged_in_at` stamp on a token-mode registration, the mode-dispatched re-login-horizon estimate, and the **shred-with-the-account** obligation `Spec-022 §PII Data Map`'s account-plane posture records. Refuses rather than degrading where the ladder refuses.
