...
The following invariants are **load-bearing** and MUST be preserved across all Plan-029 PRs and downstream extensions. Any change that would weaken or remove an invariant requires a coordinated cross-plan amendment (see cross-plan-dependencies.md).
...
**Resolution.** Discharged at this plan's authoring audit: the global cost page read (T4.2) is a projection over the accountant's accessor with no independent fold, and the `Spec-016 §Cost Figure Display Consistency` clause (b) declaration is carried per figure.
...
- [x] Required ADRs are accepted (verified: ADR-006 `accepted`, ADR-012 `accepted`, ADR-017 `accepted`, ADR-018 `accepted`, ADR-021 `accepted`, ADR-028 `accepted`). **[ADR-028](../decisions/028-provider-credential-custody-posture.md) lands `accepted` in this same swap** — the same-PR-accepted shape [ADR-025](../decisions/025-runtime-node-control-plane-caller-authorization.md) established at — so this box is true at merge rather than pending a follow-on promotion. It was required, not optional: `Spec-029 §ADR Triggers` named this exact change as needing an ADR and not an amendment, and the ADR is the record that discharges it. [ADR-021](../decisions/021-cli-identity-key-storage-custody.md) joins the set as the custody ladder the D2 token is sealed through — a consumer relationship, adding no mechanism.
...
- `apps/cli/src/commands/` (CREATE — Plan-029-authored content inside Plan-007's `apps/cli` scaffold) + `apps/cli/src/main.ts` (EXTEND — registration calls)
