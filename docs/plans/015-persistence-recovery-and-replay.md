...
- [ ] **T15.4 — Expose recovery-status reads and renderer surfaces for degraded or blocked startup conditions.**
  ...
  - **Consumes (by SHAPE):**
    - `RecoveryStatusRead` → { overall, sessions: [{ state, lastReplayedSequence, failureCategory, recoveryCondition }] } ← `docs/architecture/contracts/api-payload-contracts.md §Plan-015 — Persistence Recovery And Replay`.
...
- [ ] **T15.5 — Resume position-compare + divergence halt-for-human (campaign B14; P1-5 with the R8 interplay guard).**
  ...
  - **Tests:** the CI divergence-injection suite — equal-position resume proceeds; each divergence direction halts in `waiting_for_input` with `recovery-needed` + the recorded classification; every `RecoverySpanClassification` value is exercised and `unclassifiable` handling is asserted identical to `irreversible`; a post-rollback resume compares against the `run.rolled_back` floor; the fresh-session catch fires; **with a firing negative control** (a seeded known-divergence fixture must fail the suite when the compare is disabled) — this suite is the named gate the future tiered-auto-resolution policy flip waits on.
  ...
  - **Consumes (by SHAPE):**
    ...
    - sanctioned divergence rows (`running | waiting_for_approval | paused → waiting_for_input`, startup reconciliation) + the three-outcome recovery decision rule ← [run-state-machine.md §Recovery Transitions](../domain/run-state-machine.md#recovery-transitions) (campaign B5).
