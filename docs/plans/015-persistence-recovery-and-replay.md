...
- **I-015-2 (idempotent rebuild):** `ProjectionRebuild` for a session over the same canonical event range yields identical projection state regardless of how many times it runs. Spec-015:70,346.
- **I-015-3 (no premature admission):** No mutable operation is accepted until startup recovery has restored canonical local truth or the node is in an explicit blocked/degraded state. Spec-015:54-55,347.
- **I-015-4 (no silent run loss):** A persisted driver handle that cannot be resumed transitions its run to `failed` with visible recovery-failure detail; it never silently disappears or restarts as a new run. Spec-015:61,348.
...
- [ ] **T15.1 — Finalize SQLite schema, receipts, and runtime-binding persistence needed for replay-safe restart.**
  ...
  - **Spec coverage:** Spec-015 Required Behavior — "Canonical local execution data must include session events, queue state, approvals, runtime bindings, and command receipts" (`docs/specs/015-persistence-recovery-and-replay.md:45`); Idempotency Protocol two-phase receipt commit (`docs/specs/015-persistence-recovery-and-replay.md:77-108`); Idempotency classes table (`docs/specs/015-persistence-recovery-and-replay.md:116-124`); Acceptance Criterion (`docs/specs/015-persistence-recovery-and-replay.md:346`).
  ...
  - **Consumes (EXTEND, by SHAPE):**
    ...
    - `tool.idempotency_class` per-tool declaration ← Spec-005 §Tool Metadata `docs/specs/015-persistence-recovery-and-replay.md:137`.
...
- [ ] **T15.2 — Implement replay rebuild and idempotent projection restoration on daemon startup.**
  ...
  - **Spec coverage:** Required Behavior — restart "projection rebuild from canonical events" (`docs/specs/015-persistence-recovery-and-replay.md:46-47`); "Replay must be possible without client memory or ad hoc transcript reconstruction" (`docs/specs/015-persistence-recovery-and-replay.md:50`); Interfaces — `ReplayReadAfterCursor` / `ProjectionRebuild` idempotent (`docs/specs/015-persistence-recovery-and-replay.md:69-70`); Acceptance Criterion (`docs/specs/015-persistence-recovery-and-replay.md:346`).
  - **Verifies invariant:** I-015-2
  - **Consumes (by SHAPE):**
    - `ReplayReadAfterCursor` { sessionId, afterSequence, limit } → { events: EventEnvelope[], nextSequence, hasMore } ← `docs/architecture/contracts/api-payload-contracts.md:2224-2232` (replay uses a sequence-position cursor `afterSequence: number`, intentionally distinct from `EventReadAfterCursor`'s opaque `EventCursor` brand — ratified D-015-1).
    - `ProjectionRebuild` { sessionId, force } → { sessionId, rebuiltProjections, asOfSequence } ← `docs/architecture/contracts/api-payload-contracts.md:2236-2243`.
    - `EventEnvelope` (incl. `version: EventEnvelopeVersion` "MAJOR.MINOR" at line 1009) ← `docs/architecture/contracts/api-payload-contracts.md:1008-1020`; ADR-018 versioning decisions (semver #1 line 35, floor #3 line 39, upcaster #6 line 45) `docs/decisions/018-cross-version-compatibility.md:35-51`.
...
- [ ] **T15.3 — Implement runtime-binding adoption or resume logic plus explicit failure transitions for in-flight runs.**
  ...
  - **Spec coverage:** Required Behavior restart steps — restoration of runtime bindings + resume-or-explicit-failure (`docs/specs/015-persistence-recovery-and-replay.md:48-49`); Default Behavior — recovery before mutable work; prefer adopting live sessions (`docs/specs/015-persistence-recovery-and-replay.md:55-56`); Fallback Behavior — un-resumable handle → `failed` with visible detail (`docs/specs/015-persistence-recovery-and-replay.md:61`); Idempotency-class recovery dispatch + recovery events (`docs/specs/015-persistence-recovery-and-replay.md:116-133`); Interfaces — `RuntimeBindingRead` (`docs/specs/015-persistence-recovery-and-replay.md:71`).
  ...
  - **Consumes (by SHAPE):**
    ...
    - `RuntimeBindingRead` { runId } → { runId, driverName, contractVersion, resumeHandle?, runtimeMetadata } ← `docs/architecture/contracts/api-payload-contracts.md:2247-2255`.
    - `DriverResumeResult` discriminated union incl. `recoveryCondition: RecoveryCondition` ← `docs/architecture/contracts/api-payload-contracts.md:761-768`; Spec-005 §Fallback Behavior `docs/specs/015-persistence-recovery-and-replay.md:122`.
    ...
    - `tool.replayed` / `tool.skipped_during_recovery` event types (category `tool_activity`), registered in Spec-006, taxonomy tracked by BL-064 ← `docs/specs/015-persistence-recovery-and-replay.md:126-133`.
    - `command_receipts` in-flight predicate (`started_at IS NOT NULL AND completed_at IS NULL`) ← `docs/specs/015-persistence-recovery-and-replay.md:112`.

- [ ] **T15.4 — Expose recovery-status reads and renderer surfaces for degraded or blocked startup conditions.**
  - Implement the `RecoveryStatusRead` daemon contract + `recoveryClient.ts` SDK method, and the renderer recovery-status surface that renders healthy / replaying / degraded / blocked states with machine-readable failure category + recovery condition. Block mutable operations when the local durable store is unavailable. Map `RunFailureCategory.projection failure` → `RecoveryStatusRead` state `degraded`; `RunFailureCategory.local persistence failure` → state `blocked`, per Spec-015:54,63.
  ...
  - **Spec coverage:** Default Behavior — block local mutable ops when store unavailable (`docs/specs/015-persistence-recovery-and-replay.md:54`); Fallback Behavior — degraded read-only + repair signals (`docs/specs/015-persistence-recovery-and-replay.md:63`); Interfaces — `RecoveryStatusRead` healthy/replaying/degraded/blocked (`docs/specs/015-persistence-recovery-and-replay.md:68`); Acceptance Criteria — block when unavailable + failure visible/auditable (`docs/specs/015-persistence-recovery-and-replay.md:347-348`).
  ...
  - **Consumes (by SHAPE):**
    - `RecoveryStatusRead` → { overall, sessions: [{ state, lastReplayedSequence, failureCategory, recoveryCondition }] } ← `docs/architecture/contracts/api-payload-contracts.md:2204-2217`.
    - `RunFailureCategory` enum ← `docs/architecture/contracts/api-payload-contracts.md:142-146`.
    - `RecoveryCondition` (`"recovery-needed" | "reauth-required"`) ← `docs/architecture/contracts/api-payload-contracts.md:779`.
...
- **D-015-1 — `ReplayReadAfterCursor` retains a sequence-position cursor (`afterSequence: number`), not the `EventCursor` brand.** The replay read advances over the canonical `session_events.sequence` integer (the `replay_cursors.last_sequence` state machine), so a numeric sequence position is the natural cursor and makes idempotent rebuild (I-015-2) trivially verifiable. The sibling `EventReadAfterCursor` uses the opaque `EventCursor` brand because client-facing event reads must tolerate cursor-format evolution (Plan-006). The two read models intentionally differ; reconciling them to a single brand is a non-blocking refinement **not** adopted in V1 — `api-payload-contracts.md:2224-2227` keeps `afterSequence: number`. This resolves the cursor-model reconcile the audit flagged as a non-blocking owner decision.
