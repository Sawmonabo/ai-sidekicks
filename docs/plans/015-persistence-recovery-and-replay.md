...
- **I-015-2 (idempotent rebuild):** `ProjectionRebuild` for a session over the same canonical event range yields identical projection state regardless of how many times it runs. Spec-015:68, 344.
- **I-015-3 (no premature admission):** No mutable operation is accepted until startup recovery has restored canonical local truth or the node is in an explicit blocked/degraded state. Spec-015:54-55, 345.
- **I-015-4 (no silent run loss):** A persisted driver handle that cannot be resumed transitions its run to `failed` with visible recovery-failure detail; it never silently disappears or restarts as a new run. Spec-015:60, 346.
...
- [ ] **T15.1 — Finalize SQLite schema, receipts, and runtime-binding persistence needed for replay-safe restart.**
  ...
  - **Consumes (EXTEND, by SHAPE):**
    ...
    - `tool.idempotency_class` per-tool declaration ← Spec-005 §Tool Metadata `docs/specs/015-persistence-recovery-and-replay.md:135`.
...
- [ ] **T15.2 — Implement replay rebuild and idempotent projection restoration on daemon startup.**
  ...
  - **Spec coverage:** Required Behavior — restart "projection rebuild from canonical events" (`docs/specs/015-persistence-recovery-and-replay.md:46-47`); "Replay must be possible without client memory or ad hoc transcript reconstruction" (`docs/specs/015-persistence-recovery-and-replay.md:50`); Interfaces — `ReplayReadAfterCursor` / `ProjectionRebuild` idempotent (`docs/specs/015-persistence-recovery-and-replay.md:67-68`); Acceptance Criterion (`docs/specs/015-persistence-recovery-and-replay.md:344`).
  - **Verifies invariant:** I-015-2
  - **Consumes (by SHAPE):**
    - `ReplayReadAfterCursor` { sessionId, afterSequence, limit } → { events: EventEnvelope[], nextSequence, hasMore } ← `docs/architecture/contracts/api-payload-contracts.md:2221-2229` (replay uses a sequence-position cursor `afterSequence: number`, intentionally distinct from `EventReadAfterCursor`'s opaque `EventCursor` brand — ratified D-015-1).
    - `ProjectionRebuild` { sessionId, force } → { sessionId, rebuiltProjections, asOfSequence } ← `docs/architecture/contracts/api-payload-contracts.md:2233-2240`.
    - `EventEnvelope` (incl. `version: EventEnvelopeVersion` "MAJOR.MINOR" at line 1009) ← `docs/architecture/contracts/api-payload-contracts.md:1005-1017`; ADR-018 versioning decisions (semver #1 line 35, floor #3 line 39, upcaster #6 line 45) `docs/decisions/018-cross-version-compatibility.md:35-51`.
...
- [ ] **T15.3 — Implement runtime-binding adoption or resume logic plus explicit failure transitions for in-flight runs.**
  ...
  - **Spec coverage:** Required Behavior restart steps — restoration of runtime bindings + resume-or-explicit-failure (`docs/specs/015-persistence-recovery-and-replay.md:48-49`); Default Behavior — recovery before mutable work; prefer adopting live sessions (`docs/specs/015-persistence-recovery-and-replay.md:55-56`); Fallback Behavior — un-resumable handle → `failed` with visible detail (`docs/specs/015-persistence-recovery-and-replay.md:60`); Idempotency-class recovery dispatch + recovery events (`docs/specs/015-persistence-recovery-and-replay.md:114-131`); Interfaces — `RuntimeBindingRead` (`docs/specs/015-persistence-recovery-and-replay.md:69`).
  ...
  - **Consumes (by SHAPE):**
    ...
    - `RuntimeBindingRead` { runId } → { runId, driverName, contractVersion, resumeHandle?, runtimeMetadata } ← `docs/architecture/contracts/api-payload-contracts.md:2244-2252`.
    - `DriverResumeResult` discriminated union incl. `recoveryCondition: RecoveryCondition` ← `docs/architecture/contracts/api-payload-contracts.md:761-768`; Spec-005 §Fallback Behavior `docs/specs/015-persistence-recovery-and-replay.md:120`.
    ...
    - `tool.replayed` / `tool.skipped_during_recovery` event types (category `tool_activity`), registered in Spec-006, taxonomy tracked by BL-064 ← `docs/specs/015-persistence-recovery-and-replay.md:124-131`.
    - `command_receipts` in-flight predicate (`started_at IS NOT NULL AND completed_at IS NULL`) ← `docs/specs/015-persistence-recovery-and-replay.md:110`.
...
- [ ] **T15.4 — Expose recovery-status reads and renderer surfaces for degraded or blocked startup conditions.**
  ...
  - **Consumes (by SHAPE):**
    - `RecoveryStatusRead` → { overall, sessions: [{ state, lastReplayedSequence, failureCategory, recoveryCondition }] } ← `docs/architecture/contracts/api-payload-contracts.md:2201-2214`.
...
- **D-015-1 — `ReplayReadAfterCursor` retains a sequence-position cursor (`afterSequence: number`), not the `EventCursor` brand.** The replay read advances over the canonical `session_events.sequence` integer (the `replay_cursors.last_sequence` state machine), so a numeric sequence position is the natural cursor and makes idempotent rebuild (I-015-2) trivially verifiable. The sibling `EventReadAfterCursor` uses the opaque `EventCursor` brand because client-facing event reads must tolerate cursor-format evolution (Plan-006). The two read models intentionally differ; reconciling them to a single brand is a non-blocking refinement **not** adopted in V1 — `api-payload-contracts.md:2221-2224` keeps `afterSequence: number`. This resolves the cursor-model reconcile the audit flagged as a non-blocking owner decision.
