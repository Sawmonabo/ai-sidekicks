...
> **Amendment (2026-08-25, first-run provider-authentication surfacing — the readiness derivation and sign-in handoff this plan owes [Plan-023](./023-first-run-onboarding.md)'s provider step; user-ratified).** Flips the previously-`approved` plan to `review` per the audit runbook's plan behavior-change row — it adds **I-026-9** and **I-026-10**, **CP-026-7**, and task **T2.5** — and **restores `approved` in the same diff** through the targeted readiness-audit delta riding it, the in-swap flip-and-restore shape the cohort established. `Spec-026` flips and restores in the same swap; the §Preconditions boxes below carry the scoped Re-opened / Delivered record. **The growth.** `providerAccount.list` gains a per-provider **readiness projection** derived from the registry — never a second source of truth, served from the account row's **stored** last-probe result so a registry read spawns no provider process, advisory only against the unchanged I-026-3 spawn gate — together with the credential-free-detection and remedy-disclosure rules the first-run surface consumes. **Mints no census-moving surface**: the namespace stays at **seven** verbs, no refusal code is registered, no table is added and no Plan-004 driver contract is widened; two nullable columns are added to the **unshipped** `provider_accounts` CREATE statement — the stored health reading and its observation timestamp, the reading Plan-026 T1.2's own migration test already expected a CHECK for and that the readiness projection reads. No table, no migration ordinal, and **no census move** (SQLite stays at 56).
...
- **No cost fold of its own.** Every cost figure this plan surfaces is supplied by the Plan-014 `BudgetAccountant`'s committed-spend accessor. This plan builds no second accountant and no spend table (`Spec-014 §Cost Figure Display Consistency` clause (a)).
...
- **No first-run onboarding surface.** The provider step of the first-run flow — its trigger, its walkthrough, and its CLI and desktop surfaces — is authored by [Plan-023](./023-first-run-onboarding.md) Phase 7 under CP-026-7. This plan supplies the registry that step registers into and the readiness derivation it reads (`Spec-026 §Node provider readiness and the sign-in handoff`), and authors no onboarding UI. Landed 2026-08-25; [BL-154](../archive/backlog-archive.md) is `completed`.

## Invariants

The following invariants are **load-bearing** and MUST be preserved across all Plan-026 PRs and downstream extensions. Any change that would weaken or remove an invariant requires a coordinated cross-plan amendment (see cross-plan-dependencies.md).
...
**Grounds in.** `Spec-026 §Account identity and credential generation`.
...
**Grounds in.** `Spec-026 §Validation at spawn — fail-closed`.
...
**Grounds in.** `Spec-026 §Credential homes and the constructed environment`.
...
**Grounds in.** `Spec-026 §Credential-refresh brokering`, which inherits the posture of `Spec-025 §Non-Goals`.
...
**Grounds in.** `Spec-026 §Concurrency Posture`.
...
**Grounds in.** `Spec-026 §Fallback Behavior`.
...
**Grounds in.** `Spec-026 §Node provider readiness and the sign-in handoff`.
...
**Grounds in.** `Spec-026 §Non-Goals`, `Spec-026 §Non-interactive token registration`, [ADR-028 §Decision](../decisions/028-provider-credential-custody-posture.md#decision).
...
**Grounds in.** `Spec-026 §Per-limit provider quota`, [Provider Wire Reference §claude](../reference/provider-wire/claude.md).
...
**Resolution.** Plan-004 Phase 3B (`Plan-004 §Phase 3B — Provider-account seam + typed usage-limit signal`), authored in the same amendment that registers this obligation. Plan-026 authors no file under the provider-driver tree; it supplies the registry read the Plan-004 tasks consume.
...
**Resolution.** Plan-004 Phase 3B, task T3.16, in the same amendment. Plan-026's consumption is read-only and adds no member to the signal.
...
**Resolution.** Plan-014 Phase 4B (`Plan-014 §Phase 4B — Session cost receipt`), authored in the same amendment. Plan-026 supplies data and asserts no display behavior of its own beyond the labeling rule stated in `Spec-026 §Billing mode`.

### CP-026-4 — Plan-026 joins the CP-014-16 committed-spend-fold consumer set

`Plan-014 §Cross-Plan Obligations` CP-014-16 obliges any plan that grows a cost-displaying surface to source every figure from the accountant's committed-spend accessor and to return-cite the obligation at its own audit. This plan's global cost page is such a surface.

**Resolution.** Discharged at this plan's authoring audit: the global cost page read (T4.2) is a projection over the accountant's accessor with no independent fold, and the `Spec-014 §Cost Figure Display Consistency` clause (b) declaration is carried per figure.

### CP-026-5 — Plan-011 renders the account-scoped provider quota

`Spec-011 §Rate-Limit Display` is amended by this bundle to the account-scoped, event-sourced form. The producer side — account identity and credential generation on the account-scoped quota event — is registered here; the renderer is Plan-011's.

**Resolution.** Plan-011's own rate-limit carrier precondition box names the lead-owned amendment that registers the producer side; this bundle is that amendment. The box check and the renderer task are Plan-011-owned work and are deliberately not authored here.
...
**Resolution.** Not a code dependency in either direction — the two plans share a substrate rather than an interface, and this plan writes nothing into a provider config file (`Spec-026 §Non-Goals`, inheriting `Spec-025 §Non-Goals`). Registered so the interaction is visible; Plan-025 return-cites at its next audit, per the open-consumer-set convention.

### CP-026-7 — Plan-023's first-run provider step consumes this plan's registry and readiness

**Obligation.** [Plan-023](./023-first-run-onboarding.md) Phase 7 authors the first-run provider-authentication step group. It registers accounts through this plan's `providerAccount.*` namespace rather than minting a second registry, reads this plan's per-provider readiness projection rather than re-deriving readiness from account fields, and renders the remedy this plan discloses rather than composing its own. `Plan-023 §Cross-Plan Obligations` CP-023-6 carries the reciprocal.
...
**Resolution.** Registered by the 2026-08-26 amendment in this same swap, with Spec-005 and Plan-005 flip-and-restored `approved` in it. The member is additive-optional and absent means the reserved `default` identifier, so no already-registered producer becomes invalid and no event type is minted — the taxonomy census does not move.

### CP-026-9 — Plan-011 renders per-limit quota and the second reading source

**Obligation.** `Spec-011 §Rate-Limit Display` gains the registry's newest stored reading as a **second sanctioned Source** beside the event, per-`limitId` consumer obligations, and a widened visibility rule: with a probe source in play, the absence of an indicator now also means _this account's observer is silenced_ or _this home is not a plan-limited one_, which is a different statement from the one the display previously made. Plan-011 owns the renderer.

**Resolution.** Producer side registered here (the durable window store, its precedence rule, and the notification that carries updates); Spec-011 and Plan-011 are flip-and-restored `approved` in this same swap for the consumer side. This plan asserts no display behavior beyond the precedence rule I-026-13 states.

### CP-026-10 — Plan-021 mounts the provider-management view this plan authors

**Obligation.** `Spec-021 §Provider Accounts And Cost View (→ Plan-026 Provider Accounts And Credential Homes)` already names Plan-026 as its owning plan. The 2026-08-26 amendment grows that view with the sign-in, token-supply, observer opt-out, and per-limit quota affordances. Plan-021 owns the desktop shell and the preload bridge the view mounts onto; Plan-026 authors the view's own subtree.

**Resolution.** Reciprocal, producer on this side: T4.4 authors `apps/desktop/src/renderer/src/provider-accounts/` and Plan-021 mounts it, the same shape CP-003-15 takes for the edit affordance. Spec-021 is flip-and-restored `approved` in this same swap; Plan-021 takes the mount row and no new task, because the subtree is Plan-026-authored content inside a Plan-021-owned tree.
...
- [x] Paired spec is approved — Spec-026 authored and promoted `approved` in this same bundle, its spec-status promotion gate cleared by the in-swap targeted readiness audit recorded below. **Re-opened 2026-08-25, scoped to the first-run provider-authentication amendment:** Spec-026 gained normative §Required Behavior, §Fallback Behavior, §Interfaces And Contracts, and §Acceptance Criteria text (the readiness derivation, the sign-in handoff, and the vendor authentication-policy constraints), so the spec flipped `approved → review` per the audit runbook's spec-amendment rule and this plan flips with it under the runbook's plan behavior-change row. **Delivered 2026-08-25** by the targeted readiness-audit delta riding the same diff, which audits the amendment, re-checks this box, and restores both to `approved`. **Re-opened 2026-08-26, scoped to the provider-account sign-in, health-observation, and per-limit-quota amendment:** Spec-026 gained normative §Scope, §Required Behavior, §Fallback Behavior, §Interfaces And Contracts, §State And Data Implications, §Pitfalls To Avoid, and §Acceptance Criteria text and **reversed a stated absolute** under [ADR-028](../decisions/028-provider-credential-custody-posture.md), so the spec flipped `approved → review` again and this plan flips with it. **Delivered 2026-08-26** by the targeted readiness-audit delta riding the same diff, which audits the amendment, re-checks this box, and restores both to `approved`.
- [x] Required ADRs are accepted (verified: ADR-006 `accepted`, ADR-012 `accepted`, ADR-017 `accepted`, ADR-018 `accepted`, ADR-021 `accepted`, ADR-028 `accepted`). **[ADR-028](../decisions/028-provider-credential-custody-posture.md) lands `accepted` in this same swap** — the same-PR-accepted shape [ADR-025](../decisions/025-runtime-node-control-plane-caller-authorization.md) established at — so this box is true at merge rather than pending a follow-on promotion. It was required, not optional: `Spec-026 §ADR Triggers` named this exact change as needing an ADR and not an amendment, and the ADR is the record that discharges it. [ADR-021](../decisions/021-cli-identity-key-storage-custody.md) joins the set as the custody ladder the D2 token is sealed through — a consumer relationship, adding no mechanism.
- [x] Blocking open questions are resolved — **the first-run provider-authentication surfacing gap is no longer deferred**. It was the one named gate this box carried ([BL-154](../archive/backlog-archive.md)); the 2026-08-25 amendment settles it on this side (`Spec-026 §Node provider readiness and the sign-in handoff`, I-026-9, I-026-10, T2.5, CP-026-7) and on the onboarding side (`Spec-023 §Provider Authentication (Group B)`, Plan-023 Phase 7), BL-154 is `completed` and archived, and `Spec-026 §Open Questions` records the closure. No open question remains on this plan.
- [x] **Plan-readiness audit complete per [`docs/operations/plan-implementation-readiness-audit-runbook.md`](../operations/plan-implementation-readiness-audit-runbook.md)** — **Re-opened 2026-08-25, scoped to the first-run provider-authentication amendment's growth** (I-026-9, I-026-10, CP-026-7, T2.5) and **Delivered 2026-08-25** by the targeted readiness-audit delta riding the same diff, which re-checks this box and restores `approved`. Original record: first-time targeted readiness audit taken **in this same bundle** against the plan as authored (the in-swap delta shape established by the 2026-08-17 amendment cohort). Gate walk recorded in the dated §Notes entry below. The audit certifies the four-phase `#### Tasks` decomposition across Phases 1–4 and the 4B supplement, I-026-1..8, CP-026-1..6, and the Spec-026 acceptance-criteria coverage mapping; it does **not** certify the carrier-held supplement phase below, whose producer is external and unlanded. **Re-opened 2026-08-26, scoped to the sign-in, health-observation, and per-limit-quota amendment's growth** (I-026-11, I-026-12, I-026-13, CP-026-8, CP-026-9, CP-026-10, T1.4, T2.6, T2.7, T3.6, T4.5, and the Phase-4 task growth on T4.3 and T4.4) and **Delivered 2026-08-26** by the targeted readiness-audit delta riding the same diff, which re-checks this box and restores `approved`. That delta additionally audits the ADR-028 dependency edge — a required ADR landing `accepted` in the same diff is certified against its own §Pre-Implementation Checklist rather than assumed, and all five of its boxes resolve.
- [x] **Turn-scoped effective principal carrier registered.** **Re-opened: no. Delivered 2026-08-26** by the canonical-transcript-authority and same-agent-provider-switch amendment, which registers the carrier as instance **(4)** of the [api-payload-contracts.md](../architecture/contracts/api-payload-contracts.md) §Authenticated Principal class rule: every `usage.cost_update` carries the daemon-resolved `effectivePrincipal` for the turn it meters ([Spec-005 §Usage Telemetry](../specs/005-session-event-taxonomy-and-audit-log.md#usage-telemetry-usage_telemetry)), a closed two-arm value whose user reference is required on the user arm and absent on the `system` arm. It rides the **metered row itself** rather than a turn-boundary event the fold would have to order against, so the axis reads a group key off each row and derives nothing from history — the class rule's own requirement, and the reason no per-turn table was minted. Resolved and published by [Plan-003](./003-queue-steer-pause-resume.md) T3.19 under CP-003-16, stamped onto the metered row by [Plan-004](./004-provider-driver-contract-and-capabilities.md)'s usage-emission path under CP-004-10, and read here. The stated degrade is unchanged: an unstamped turn attributes to the run starter. Per-turn billing attribution (`Spec-014 §Session Cost Receipt` — the run starter is billed by default, the intervener for turns they steered or edited) requires the turn-scoped effective principal and its admitting-principal stamp, registered by CP-003-14 and the [api-payload-contracts.md](../architecture/contracts/api-payload-contracts.md) §Authenticated Principal class rule. Until that registration lands, this plan's attribution leg has no principal to key on and would have to infer the billed party from run ownership alone — which is wrong exactly in the steered-turn case the requirement exists for. **Holds Phase 4B in whole** — the attribution leg is homed in its own supplement phase, and this box carries that phase's Gate-5 `precondition_box_checked` entry, so the hold it records and the hold the gate enforces are the same set. Phase 4's own tasks are unaffected.
...
- **T1.1 — Provider-account contracts.**
  ...
  - **Provides:** the `ProviderAccountId` brand; `ProviderName` (the two-member closed set); `BillingMode` (`subscription` \| `metered` \| `unknown`); `CredentialGeneration` (a positive integer); the `ProviderAccount` record shape; and the request/response pairs for register, remove, set-default, list, read, home-reset, and probe. Strict Zod schemas with unknown-key rejection. `credentialGeneration` is a **response-only** projection, and `accountId` is **daemon-minted** — a request carrying `credentialGeneration` never parses, and a register request carrying `accountId` parses only on the token re-supply arm (with `nonInteractiveToken` beside it, naming an already-registered account to repair — `accountId` alone is refused by the schema's superRefine), so a caller cannot assert an identity or a generation; `accountId` does appear on account-scoped requests (the list read scope, update, remove, set-default, home-reset, probe, and the sign-in and token re-supply selectors) as the selector for an identity the daemon already minted, which asserts nothing — a supplied id naming no registered account is refused, never created.
...
- **T2.4 — `providerAccount.*` wire namespace.**
  ...
  - **Provides:** the handler binders for the T1.1 pairs, the node-local operator-authority gate on **every verb in the namespace — the `list` read included, not only the mutating six** — and the SDK client marshalling wire values without deriving any of them. The read takes the same gate because its reply discloses a daemon-owned credential-home path in the readiness remedy (`Spec-026 §Authorization Posture`); gating it like an ordinary read would leave that path readable by any local caller that can reach the daemon socket.
...
- **T2.7 — Non-interactive token admission and sealing.**
  ...
  - **Provides:** the [ADR-028](../decisions/028-provider-credential-custody-posture.md) D2 admission gate over the five conjunctive conditions, sealing through the [ADR-021](../decisions/021-cli-identity-key-storage-custody.md) ladder, the `logged_in_at` stamp on a token-mode registration, the mode-dispatched re-login-horizon estimate, and the **shred-with-the-account** obligation `Spec-020 §PII Data Map`'s account-plane posture records. Refuses rather than degrading where the ladder refuses.
...
A supplement phase rather than a task inside Phase 4, because its gate is a **whole-phase** gate: Gate 5 resolves `precondition_box_checked` against the phase that declares it, so a carrier-held task sharing a phase with unheld ones would block all of them. Homing the held work alone here keeps the enforced hold and the recorded hold identical — the shape Plan-003's Phase-3B rollback cluster and Plan-007's Phase 2B already use.

#### Tasks

- **T4.1 — Per-user billing attribution.**
  - **Files:** `packages/runtime-daemon/src/accounts/provider-account-resolver.ts` (EXTEND).
  ...
  - **Consumes:** T3.1; the turn-scoped effective principal (external producer, per the §Preconditions box naming CP-003-14 and the api-payload §Authenticated Principal class rule).
  - **Spec coverage:** Spec-014 §Session Cost Receipt.
  - **Verifies invariant:** none — the attribution rule is stated by the Spec-014 clause above and asserted by Plan-014's receipt tasks, with this task supplying the account-side input.
  - **Tests:** an unsteered run attributes wholly to its starter; a steered turn attributes to the intervener and the remainder to the starter; an edit-and-resend composite attributes the resent turn to the editor; a system-initiated turn attributes to the system arm rather than to any user.
...
- **Phase 4B is carrier-held** on an external producer; Phase 4 ships without it, and the receipt degrades to account attribution without the user split until it lands. The degradation is visible rather than silent: the receipt's caused-by partition is absent, not wrong.
- **First-run provider authentication is surfaced as of 2026-08-25** — a fresh install meets [Plan-023](./023-first-run-onboarding.md)'s provider step rather than discovering the gap at the first refused run (CP-026-7; [BL-154](../archive/backlog-archive.md) `completed`). The residual this leaves is honest and named rather than closed: a credential-free probe cannot distinguish a healthy first-party login from one whose stored credential has since been destroyed, so the authenticated arm is necessary and not sufficient. It is bounded by the unchanged I-026-3 spawn gate and the per-account re-authentication runbook, and it is deliberately **not** closed by reading credential bytes — that would reverse the no-token-custody posture, which is an ADR-level reversal.
