...
- [ ] Plan-016 has landed the T2.7 idle-reaper seam that consults `runHasPendingCrossNodeDispatch(runId)` (Tier 6, campaign B15) — required by the liveness-registration task only, which is V1-dormant behind `orchestration.node_not_local`.
...
2. Add the Local SQLite migrations for `cross_node_dispatch_approvals` (caller and target daemons) and `cross_node_pending_dispatch` (caller daemon only) plus the Shared Postgres migration for `cross_node_dispatch_coordination`; update schema snapshots and migration tests (a `cross_node_pending_dispatch` fresh-DB apply, its two partial indexes, and the paired `closed_at` / `close_reason` CHECK).
