...
- [x] Paired spec is approved — **dated gate note (2026-07-06):** Spec-010 is temporarily `review` while the campaign B21 amendment (§Turn-Boundary Snapshots) sits in its review window; the campaign plan's Task 28 batch re-promotion restores `approved` after the W1.5 spec gate, and this plan's snapshot-phase code dispatch stays blocked until that restoration.
