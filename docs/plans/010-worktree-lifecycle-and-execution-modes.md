...
| Field | Value |
| --- | --- |
| **Status** | `review` (flipped from `approved` 2026-07-19 by the capability-enhancement campaign's B22 bundle — the new Phase 5 turn-snapshot service is a discrete named Phase, a Status Flip Rule row-3 change; re-earns `approved` through Plan-010's W2.5 targeted re-audit) |
| **NNN** | `010` |
...
- **T5.1 — Turn-boundary snapshot capture (`turn-snapshot-service.ts` create leg).**
  ...
  - `captureTurnSnapshot({executionRoot, runId, epoch, turnOrdinal})` implements the `Spec-010 §Turn-Boundary Snapshots` capture recipe verbatim: the out-of-worktree temp-index pipeline (`GIT_INDEX_FILE` at a scratch index outside the worktree → `git read-tree HEAD` → `git ls-files -co --exclude-per-directory=.gitignore -z | git -c core.autocrlf=false -c core.attributesFile=/dev/null update-index --add --remove -z --stdin` with `GIT_ATTR_NOSYSTEM=1` → `git write-tree` → `git -c i18n.commitEncoding=utf-8 commit-tree <tree> -p HEAD -m <fixed message>`), stamped with the daemon-owned six-var host-independence env set — `GIT_AUTHOR_NAME`/`GIT_AUTHOR_EMAIL`/`GIT_AUTHOR_DATE` + `GIT_COMMITTER_NAME`/`GIT_COMMITTER_EMAIL`/`GIT_COMMITTER_DATE` at a fixed UTC offset (author/committer dates are OID inputs, so ident env alone would still leak host wall-clock timezone) — then the create-only `git update-ref refs/sidekicks/runs/<runId>/epoch-<E>/turn-<N> <commit> ""` (compare-and-swap against ref absence). `<E>` is the run's execution epoch (`Spec-004 §Required Behavior` — 0 before any rollback, incremented per applied rollback; supplied by the caller, never derived here). Idempotence is **per-epoch**: a retried or duplicated capture of the same `(runId, epoch, turnOrdinal)` finds the ref already written and returns the recorded OID as idempotent success, while a post-rollback re-execution's identical `turnOrdinal` mints a fresh ref under its own `epoch-<E>` segment (the reused position ordinal never collides with, or silently resolves to, the superseded epoch's tree). Applicable to `worktree` / `branch` / `ephemeral clone` modes only (`read-only` snapshots nothing). Capture failure emits an OTel diagnostic and **never blocks or fails the turn** — the turn boundary completes regardless; snapshots are a recovery convenience, not a turn gate. Every git invocation is hook-neutralized per D-010-10 (empty `core.hooksPath`, `execFile` argv-only, no shell).
  - **Tests:** real-git temp-dir repos (no mocks): capture writes the epoch-namespaced ref whose commit's first parent is `HEAD`; the staged tree is byte-identical to `git add -A` under identical inputs including non-ignored untracked files; ignored untracked paths are excluded while a tracked file matching `.gitignore` is still captured; the recorded OID is stable across host `core.autocrlf` / `i18n.commitEncoding` / `core.excludesFile` variation (the pins hold); a duplicate capture of the same `(runId, epoch, turnOrdinal)` is idempotent (ref unmoved, recorded OID returned); a same-`turnOrdinal` capture under a new epoch mints a distinct ref; a `read-only` root captures nothing; an induced capture failure emits the diagnostic without throwing into the turn-boundary caller.
  - **Spec coverage:** Spec-010 §Turn-Boundary Snapshots (capture temp-index recipe, epoch-namespaced create-only refs, writable-modes-only applicability); Spec-004 §Required Behavior (execution-epoch definition)
...
- **T5.3 — Retention prune + clone-disposal boundary (`turn-snapshot-service.ts` prune leg).**
  ...
  - **Consumes:** capture leg + ref namespace ← T5.1; `run_execution_contexts.released_at` (run-terminal timestamp, the window anchor) ← T1.3 schema (stamped at terminal by the T3.2 gate at runtime); retention-window + sweep-cadence defaults ← daemon config (the T2.3 clone-TTL precedent); ephemeral-clone `on_run_complete` disposal ← T2.3 `ephemeral-clone-service.ts`
