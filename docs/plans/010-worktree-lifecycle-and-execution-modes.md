...
- **CP-010-5 — Worktree event registration with Plan-006.** Phase 1 registers the 5 `worktree.*` types — `worktree.created` (Spec-006 line 253), `worktree.ready` (Spec-006 line 254), `worktree.dirty` (Spec-006 line 255), `worktree.merged` (Spec-006 line 256), `worktree.retired` (Spec-006 line 257) — into the Plan-006-owned `SessionEventSchema` union as an additive-MINOR EXTEND, reusing Plan-009's `RepoWorkspaceLifecyclePayloadSchema` (it already carries `worktreeId?`); type names + `session_lifecycle` category mapping ship in Plan-006 T1.2. Phase 2 consumes `EventLogService.append` as the sole append path. This plan adds no rows to the Spec-006 registry — no `worktree.failed`, no clone events (D-010-11). **Direction:** Extend [Plan-006](./006-session-event-taxonomy-and-audit-log.md) (additive). **Tasks:** T1.1, T2.1.
...
- **T2.2 — `worktree-service.ts` + `worktree-errors.ts`: create / validateReuse / retire / cleanupPass.**
  ...
  - `worktree-service.ts`: `create({repoMountId, branchName, baseRef?, onCollision, sessionId, runId?})` — `branchName` is required at this seam; the Spec-010 line 126 slug rule lives in the exported `deriveWorktreeBranchName(summary?, runId)` helper (daemon-side only) that the derivation-owning callers invoke (the T3.2 gate's queue-summary slug; T2.4's runId-only fallback arm — D-010-19); resolve base per D-010-8 (mount HEAD; detached-HEAD + no `baseRef` → typed refusal); place the root per D-010-6; run `git worktree add` hook-neutralized per D-010-10; collision per D-010-7 (`refuse` → `worktree.branch_collision`; `suffix` → first-free ordinal via index-arbitrated insert-retry); row + `worktree.created`/`worktree.ready` via T2.1; creation failure → row `failed` (no event) + propagate for the caller's `failReprovision`. `validateReuse({reuseWorktreeId, branchName, acknowledgeDirtyCandidate?})` — the candidate must be live, branch-compatible, and clean-or-acknowledged (D-010-15), else `worktree.reuse_conflict`; incompatible never binds. `retire(worktreeId)` — refuse `worktree.retire_conflict` while the root is held busy; else state → `retired` + event (metadata preserved, I-010-9). `cleanupPass()` — detached-mount worktrees → retired (D-010-13c); async disk removal for retired-uncleaned rows stamping `cleaned_at` (D-010-13d, idempotent).
