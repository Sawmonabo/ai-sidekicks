...
| **I-010-23** | Fail-closed restore: `restoreToTurn` refuses (typed refusal, no leg applied) unless the current `HEAD` equals the resolved snapshot's recorded first parent — never an anti-diffed partial restore; a mid-sequence restore failure returns a convergent partial-restore result naming the failed command, never a silent success. | T5.2 |
...
- [ ] Phases 1 and 3 merged — the `run_execution_contexts` per-run execution-root binding (D-010-5) and the run-setup gate that writes it (D-010-16); the capture and restore legs operate on a run's resolved execution root
...
- **T5.1 — Turn-boundary snapshot capture (`turn-snapshot-service.ts` create leg).**
  ...
  - **Consumes:** execution root + runId + epoch + turnOrdinal ← caller (the turn-boundary capture hook resolves the root via `run_execution_contexts` from T3.2 and the epoch via the Plan-004 run engine per `Spec-004 §Required Behavior`); hook-neutralized git-invocation discipline ← Phase 2 (D-010-10)
- **T5.2 — Restore-to-ref op + lineage resolution (`turn-snapshot-service.ts` restore leg).**
  ...
  - `restoreToTurn({executionRoot, runId, targetPosition})` implements the `Spec-010 §Turn-Boundary Snapshots` restore contract. **Target resolution walks the current lineage** (Spec-010 assigns this resolution to this bundle): resolve the snapshot from the newest epoch whose recorded range contains `targetPosition`, an epoch inheriting the prefix at or below its rewind base from its parent epoch. **Fail-closed precondition:** the current `HEAD` must equal the resolved snapshot's recorded first parent (`<ref>^`); if branch history has advanced past the snapshot the restore refuses with a typed refusal — never an anti-diffed partial restore (post-snapshot branch commits are Spec-011's reviewable surface; rewind/revert is outside this recipe). The restore itself is the pinned three-step: `git -c core.autocrlf=false -c submodule.recurse=false read-tree --reset -u <ref>` → the untracked-delete pass (`git ls-files -o --exclude-per-directory=.gitignore -z`; unlink listed files, recursively remove trailing-slash nested-repo directory entries, prune emptied directories) **repeated to a fixpoint** (a turn-created untracked `.gitignore` shields what it ignores through the pass that deletes it — a single pass under-deletes) → the closing `git read-tree --reset HEAD` (index-only, no `-u`: captured-untracked files return to untracked status, tracked edits to unstaged modifications, no fabricated staged additions). An ignored untracked path colliding with a snapshot-tracked path is overwritten by the `read-tree --reset -u` leg and **enumerated in the restore result** (never silent loss); the submodule gitlink boundary holds (superproject `160000` gitlinks only, `submodule.recurse=false` pinned, each divergent gitlink enumerated). A mid-sequence restore failure returns a convergent partial-restore result naming the failed command (a fresh restore to the same target re-runs to the fixpoint); Plan-004's `rollback` maps it to the distinct `files-partially-restored` intervention disposition, never collapsed into `files-unrestored`. Plan-004's `rollback` intervention (campaign B9) owns the pre-dispatch validation, exclusive execution-root tenancy, and conversation leg per `Spec-004 §Required Behavior`.
  - **Tests:** real-git temp-dir repos, full capture→restore round-trip: worktree bytes byte-identical to the captured turn after restore; a `HEAD` advanced past the snapshot draws the typed refusal with no worktree mutation; post-snapshot untracked files are deleted including the fixpoint case (a turn-created untracked `.gitignore` plus the directory it ignores both removed across two passes) while snapshot-declared ignored paths (`node_modules`) survive every pass; an ignored path colliding with a snapshot-tracked path is overwritten and enumerated; interior submodule dirty/untracked state survives untouched and a divergent gitlink is enumerated; the closing index reset leaves no staged additions; lineage resolution picks the newest epoch whose range contains the target across a two-epoch fixture; a mid-sequence failure yields the partial-restore result naming the failed step.
  - **Spec coverage:** Spec-010 §Turn-Boundary Snapshots (restore two-step plus index close, fail-closed HEAD precondition, fixpoint untracked-delete, collision-overwrite enumeration, submodule gitlink boundary, lineage-walk target resolution); Spec-004 §Required Behavior (rollback consumes the restore leg under exclusive execution-root tenancy)
  - **Verifies invariant:** I-010-21, I-010-23
  - **Consumes:** capture leg + epoch-namespaced ref layout ← T5.1; execution root ← caller (resolved via `run_execution_contexts`, T3.2); pre-dispatch validation + exclusive tenancy + conversation leg ← Plan-004's `rollback` intervention (campaign B9, Task 13); hook-neutralized git-invocation discipline ← Phase 2 (D-010-10)
- **T5.3 — Retention prune + clone-disposal boundary (`turn-snapshot-service.ts` prune leg).**
  - **Files:** `packages/runtime-daemon/src/git/turn-snapshot-service.ts` (EXTEND), `packages/runtime-daemon/src/git/__tests__/turn-snapshot-service.test.ts` (EXTEND)
  - `pruneSnapshotsForRun(runId)` deletes a run's `refs/sidekicks/runs/<runId>/…` refs when the run's retention window closes (terminal state plus the configured window; the prune lives with the run-retention owner, invoked from the run-terminal path). **Clone-disposal boundary:** in `ephemeral clone` mode the snapshot refs live in the clone's own object store, so they share the clone's disposal lifecycle — an `on_run_complete` clone disposal takes the refs with it, and any later rollback of that run proceeds **conversation-only** (the file-leg disposition is recorded on the intervention outcome; Plan-004's `rollback` tasks own that detail — campaign B9). Disposal is not a retention violation: there is no clone left to restore into.
  - **Tests:** a run's snapshot refs are pruned on retention-window close and refs for a still-open run are retained; an ephemeral-clone run's refs vanish with the clone at `on_run_complete` disposal (the prune finds nothing to delete); pruning is idempotent (a second prune of an already-pruned run no-ops).
  - **Spec coverage:** Spec-010 §Turn-Boundary Snapshots (retention prune on window close; ephemeral-clone-disposal shares the clone lifecycle, post-disposal rollback conversation-only)
  - **Verifies invariant:** I-010-21
  - **Consumes:** capture leg + ref namespace ← T5.1; ephemeral-clone `on_run_complete` disposal ← T2.3 `ephemeral-clone-service.ts`; run-retention trigger (terminal state + window) ← the run-terminal path (Plan-004 run engine)
