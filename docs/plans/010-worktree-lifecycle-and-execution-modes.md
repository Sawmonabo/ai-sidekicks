...
- **T1.2 — Seven wire request/response pairs.**
  ...
  - Zod schemas + inferred types implementing api-payload-contracts.md §Plan-010 verbatim: `ExecutionModeSelectRequest/Response`; `ExecutionRootPrepareRequest/Response` (incl. `baseRef?`, `reuseWorktreeId?`, `acknowledgeDirtyCandidate?` — D-010-8/D-010-15; no wire `runId` — run binding is gate-supplied service-side); `WorktreeReuseCheckRequest/Response` (singular candidate; `reason?` populated when not clean or not compatible); `EphemeralClonePrepareRequest/Response` (no TTL on the wire — D-010-2; response reports effective `cleanupPolicy` + `expiresAt` + effective `branchName`); `EphemeralCloneDisposeRequest/Response`; `WorktreeRetireRequest/Response` (`Extract`-typed `retired`); `WorktreeStatusReadRequest/Response` (provenance-bearing arrays).
