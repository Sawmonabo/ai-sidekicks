...
- Five new JSON-RPC methods authored on the daemon side — wire names `onboarding.start` / `onboarding.submitChoice` / `onboarding.submitTelemetry` / `onboarding.reset` / `onboarding.read` per §API And Transport Changes. `Spec-023 §Interfaces And Contracts` names these operations abstractly (`OnboardingStart` …); the PascalCase symbols are this plan's request/response **type** names, never wire names.
- Config schema: `[onboarding]` block written to `$XDG_CONFIG_HOME/ai-sidekicks/config.toml` (or `%APPDATA%\ai-sidekicks\config.toml` on Windows) via `smol-toml` v1.6.1 per `Spec-023 §Persistence`.
...
- **Provider-authentication step group (Group B)** per `Spec-023 §Provider Authentication (Group B)`: a daemon-side step model in `packages/runtime-daemon/src/onboarding/provider-step.ts`, a `--providers` flag on the existing `sidekicks onboarding start` command plus per-provider readiness in `sidekicks onboarding status`, and a renderer step hosted by the existing walkthrough. It **consumes** [Plan-026](./026-provider-accounts-and-credential-homes.md)'s node-local `providerAccount.*` registry and its per-provider readiness projection (CP-023-6) and mints neither; it adds no `onboarding.*` method, no `[onboarding]` key, no partial-state entry, no keystore entry, and no event.
- Event emission for `onboarding.choice_made` / `onboarding.choice_reset` wired into the daemon event bus (already owned by Plan-005); payload shapes per `Spec-023 §Event Taxonomy Additions`. Spec-005 registration landed under BL-086 (completed 2026-04-18) in the `onboarding_lifecycle` category — this plan now consumes that registration rather than forward-declaring it.
...
- **Installer / package-manager bootstrap.** This plan does not ship brew / apt / npm post-install hooks or a `setup.exe` wizard. `Spec-023 §Scope` excludes this; first-run onboarding starts when the daemon does, not when the installer runs.
...
- **Enterprise SSO onboarding (OIDC / SAML).** Deferred to V1.1+ per `Spec-023 §Scope`'s out-of-scope list and BL-060.
...
- **Prompting on initial install, first daemon start, or first local session.** Single-user local-daemon mode reaches a working session without ever hitting this flow. Implementations that prompt ahead of the explicit activation trigger are a review rejection (`Spec-023 §Pitfalls To Avoid`). This holds for both step groups: the provider step is keyed to an account-plane refusal that has already happened, and a readiness test placed **ahead** of a provider run in order to prompt is the same rejection at a different point in the flow (I-023-11).
...
- `packages/runtime-daemon/src/onboarding/` — **created by this plan.**
  ...
  - `events.ts` — emits `onboarding.choice_made` / `onboarding.choice_reset` into the daemon event bus; payload shapes per `Spec-023 §Event Taxonomy Additions`.
...
- `apps/cli/src/env/` — **created by this plan.**
  ...
  - `env-override.ts` — reads `SIDEKICKS_ONBOARDING_CHOICE`, `SIDEKICKS_RELAY_URL` (equivalently `--relay-url`), `SIDEKICKS_HOSTED_TOKEN_STDIN` (equivalently `--hosted-token-stdin`), and `SIDEKICKS_TELEMETRY_OPT_IN` — the fourth being required for `Spec-023 §Telemetry Opt-In`'s no-silent-default rule to hold on the headless path, which `Spec-023 §Example Flows` already exercises; produces the same `OnboardingSubmitChoiceRequest` + `OnboardingSubmitTelemetryRequest` pair the interactive path produces.

### New desktop surface

- `apps/desktop/src/preload/onboarding.ts` — **created by this plan.** Extends `window.sidekicks` with the two `onboarding.*` methods from `Spec-023 §Desktop Surface`. Typed narrowly (no `any`) to remain inside Plan-021's narrow-preload-bridge contract.
...
- `packages/runtime-daemon/src/bootstrap/index.ts` (owned by Plan-006 — the documented wiring point, `Plan-006 §Daemon IPC + bootstrap (packages/runtime-daemon/src)`'s EXTEND row: owned-namespace `register*` calls wire here after `SecureDefaults.load`) — five `registerOnboarding*(registry, deps)` calls, per the shipped `presence.*` precedent: the shipped `session.*`/`presence.*` binders under `src/ipc/handlers/` are the on-disk pattern.
...
- `apps/desktop/src/main/bridge/onboarding.ts` (owned by Plan-021 — `Plan-021 §Main process`: Plan-021 authors the surface and registers the stubs; Plan-023 implements the flow logic) — replace the stub bodies of `onboarding.presentChoice` / `onboarding.telemetryPrompt` with delegation into this plan's `apps/desktop/src/main/onboarding/` modules. The `bridge/index.ts` handler registration is Plan-021's and is not touched.
...
Block (schema identical to `Spec-023 §Persistence`):
...
- Schema version: the top-level `schema_version` field (owned by Plan-006's config surface) bumps by 1 when this plan ships, so older daemons can detect and refuse (per `Spec-023 §Fallback Behavior` §Conflicting daemon).
...
7. **Author keystore-client wrapper.** In `packages/runtime-daemon/src/onboarding/keystore-client.ts`:
   ...
   - Desktop path (daemon runs as an Electron `utilityProcess` fork per Plan-021): forward to main-process `safeStorage` via the IPC bridge Plan-021 authors. Main-process `safeStorage.isEncryptionAvailable()` is checked at daemon boot; Linux-only `safeStorage.getSelectedStorageBackend()` must return something other than `'basic_text'` / `'unknown'` for us to persist secrets; otherwise refuse and log per `Spec-021 §Fallback Behavior`.
...
10. **Emit events.** In `packages/runtime-daemon/src/onboarding/events.ts`, emit `onboarding.choice_made` on final resolve and `onboarding.choice_reset` on reset into the event bus owned by Plan-005. Payload shapes per `Spec-023 §Event Taxonomy Additions` and §API And Transport Changes above. No secret fields.
11. **Author CLI prompts.** In `apps/cli/src/prompts/`:
    - `three-way-choice.ts` uses `@inquirer/prompts` v8.x `select({ message: 'Choose your relay deployment', default: 'free-public-relay', choices: [...] })` per [@inquirer/prompts README](https://github.com/SBoudrias/Inquirer.js). Default is Option 1 per `Spec-023 §Default Behavior`.
...
- CLI and desktop both call `onboarding.submitChoice` and produce the same `[onboarding]` block for a given `choice_id + relay_url + admin_token` input. This shared test suite is the primary guarantee that "the CLI and desktop surfaces produce identical persisted state" (`Spec-023 §Required Behavior` §Persistence).
...
- [ ] CLI commands `sidekicks onboarding reset|status` and `sidekicks telemetry set {on,off}` implement the `Spec-023 §Interfaces And Contracts` surface.
...
- [ ] Desktop preload bridge exposes `window.sidekicks.onboarding.{presentChoice, telemetryPrompt}` (the `Spec-023 §Desktop Surface` pair) with narrow typed surface; renderer never receives plaintext tokens.
