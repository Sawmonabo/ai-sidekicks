...
- **Spec-005 event registration.** The two `onboarding.*` events are emitted here and registered in the Spec-005 §Event Taxonomy table.
...
- **Enterprise SSO onboarding (OIDC / SAML).** Deferred to V1.1+ per [Spec-023 §Scope](../specs/023-first-run-onboarding.md#scope)'s out-of-scope list.
...
- [x] Spec-005's `onboarding_lifecycle` category carries `onboarding.choice_made` and `onboarding.choice_reset` with payload shapes matching this plan's emitter. This plan consumes the registered `EventType` union directly.
...
- `apps/desktop/src/renderer/src/onboarding/ProviderStep.tsx` — **created by this plan.** A view-only step in the existing walkthrough; the main-process host fetches the projection and passes it in, so **no preload-bridge method is added** and `packages/contracts/src/desktop-bridge.ts` is untouched.
...
- Events are emitted into the daemon event bus owned by Plan-005; this plan does not author the bus. Registration lives in Spec-005's §Event Taxonomy table, in the `onboarding_lifecycle` category.

## Invariants

Load-bearing constraints every Plan-023 PR — and every downstream extension — must preserve. Weakening or removing one is a change agreed with the counterpart plans, not a local edit. Each names the governing clause it grounds in, why it is load-bearing, and the task that verifies it.
...
**Obligation.** [Plan-021 §Main process](./021-desktop-shell-and-renderer.md#main-process) authors `apps/desktop/src/main/bridge/onboarding.ts` and registers its stubs; its remainder task T-021r-2-5 states the split explicitly — Plan-021 authors the surface, Plan-023 implements the flow logic.

**Resolution.** Live and reciprocal. Plan-023 replaces the stub bodies, adds the `onboarding` member to `DesktopBridge` together with its matching throwing stub in `createStubBridge()` (the factory's every-method-throws contract means the member and the stub must land together or the contracts package stops typechecking), and spreads the preload namespace — the four bridge-side entries in §Touched But Not Owned. The `bridge/index.ts` handler registration stays Plan-021's. **Direction:** Consume from [Plan-021](./021-desktop-shell-and-renderer.md). **Tasks:** T5.1, T5.2.

### CP-023-4 — `onboarding_lifecycle` event registration consumed from Spec-005 / Plan-005

**Obligation.** Both event types and their payload shapes are registered under Spec-005's `onboarding_lifecycle` category. Plan-005 owns the event bus, the taxonomy, and the registration; Plan-023 emits.
...
22. **Verify the Spec-005 registration.** Confirm Spec-005 §Event Taxonomy carries the `onboarding.choice_made` / `onboarding.choice_reset` entries in the `onboarding_lifecycle` category. Cross-check the registered payload shapes against this plan's `events.ts` emitter output — any drift between the Spec-005-registered payload and what this plan emits is a review-blocking mismatch and must be resolved by editing whichever side is wrong _before_ merge.
...
- **T3.5 — Spec-005 registration and payload-shape cross-check.**
  ...
  - Confirm Spec-005's `onboarding_lifecycle` category carries both event types with their registered payload shapes, and diff those against this plan's emitter output field by field. Drift is review-blocking and is resolved by editing whichever side is wrong **before** merge — never by emitting an unregistered shape.
  ...
  - **Spec coverage:** Spec-023 §Event Taxonomy Additions
  - **Verifies invariant:** I-023-8
  - **Consumes:** the `onboarding_lifecycle` registration ← [Plan-005](./005-session-event-taxonomy-and-audit-log.md) (CP-023-4)
...
- **T5.1 — Preload namespace, bridge type member, and the renderer import-ban extension.**
  - **Files:** `apps/desktop/src/preload/onboarding.ts` (CREATE), `apps/desktop/src/preload/index.ts` (EXTEND — one import + spread), `packages/contracts/src/desktop-bridge.ts` (EXTEND — `DesktopBridge` member plus its matching throwing `createStubBridge()` stub), `apps/desktop/eslint.config.mjs` (EXTEND — ban-list entries only)
...
- **T7.4 — End-to-end: a freshly onboarded node starts its first provider run.**
  ...
  - **Acceptance:** the first-run gap is closed on both surfaces, and closing it introduced no credential-disclosure path.
...
- Step 22 (the Spec-005 registration + payload-shape cross-check) is a read-only verification; any time before merge.
...
10. Spec-005 registration + payload-shape cross-check (step 22).
...
- **No plan authors the desktop application menu — inherited, and named here rather than papered over.** [Spec-023 §Trigger](../specs/023-first-run-onboarding.md#trigger) requires a _Set up relay_ menu activation, and [Spec-023 §Interfaces And Contracts](../specs/023-first-run-onboarding.md#interfaces-and-contracts) names _Set up providers_ beside it. Neither label has an owning task: this plan's desktop steps author walkthrough and dialog surfaces only, and [Plan-021](./021-desktop-shell-and-renderer.md) — whose spec names a "platform-appropriate menu bar" — carries no menu module, no menu task, and no menu file anywhere in its body. T7.3 therefore exports a Group-B **activation entry point** from the walkthrough host and mints no menu file, because minting one would invent an integration point into a module no plan has committed to build. **Group B's explicit trigger does not wait on that module**: `SetUpProvidersCommand.tsx` surfaces _Set up providers_ in the shell's existing renderer command surface, so all three of the spec's triggers are reachable on desktop today — an approved acceptance criterion cannot be left pending on an unowned module, and routing around it costs one renderer entry. The menu remains the _conventional_ home for both labels and would be the better one; when the menu module gets an owner, both entries become menu items calling the same activation point, which is a caller change and not a redesign. What stayed inherited and unclosed was Group A's `Set up relay` label, whose only desktop caller would be that menu. **Closed 2026-09-01:** Plan-021 now owns the application menu — `apps/desktop/src/main/menu.ts`, authored by its Phase-1B task T-021p-1B-2 — and that menu carries both labels as `Session` submenu entries calling this plan's exported activation entry point, each entry **absent** until this plan's desktop step registers the route it opens (the console's absent-not-disabled rule), so no menu entry ever points at a surface that does not exist — and the registration has an owner: T7.3 EXTENDs `apps/desktop/src/main/menu.ts` through Plan-021's typed `registerMenuSection` seam (T-021p-1B-2), gated on Plan-021 Phase 1B merged.

- **Walkthrough host-window accessibility.** The VS-Code-walkthrough-patterned renderer must expose keyboard navigation and screen-reader labels. Plan-023 does not author the full a11y audit; the walkthrough shell and the first-run frame are console routes held to [Spec-021 §Console Test Tiers](../specs/021-desktop-shell-and-renderer.md#console-test-tiers)' accessibility tier — zero `axe-core` violations at WCAG 2.2 AA on every route and dialog — with minimal landmark roles + focus management the floor below which the first-run UX is inaccessible. `axe-core` is MPL-2.0 and admitted as a never-distributed test dependency per [ADR-020 §License](../decisions/020-v1-deployment-model-and-oss-license.md#license). Acceptance test: that tier's sweep passes under the Playwright `_electron` harness.
...
- [ ] The Spec-005 §Event Taxonomy registration of `onboarding.choice_made` / `onboarding.choice_reset` (`onboarding_lifecycle` category) has been cross-checked against this plan's `events.ts` emitter output; any drift resolved before merge.
...
Plan-023 is **strictly downstream** of:
...
And **strictly upstream** of nothing — it is a leaf-node plan. CLI-first-release shippability is gated on Plan-006 only; desktop shippability is additionally gated on Plan-021. Plan-023's place in the build order is the dispatch group [cross-plan-dependencies §Dispatch groups](../architecture/cross-plan-dependencies.md#dispatch-groups) lists it in.
