...
| File                                     | Content                                                                                                                                                                                                                                                                                                                     |
| ---------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `design-audits/session-collaboration.md` | Audit of specs 001, 002, 013, 014, 016, 017 and plans 001, 002, 013, 014, 016. Covers session model, invites, membership, presence, channels, multi-agent orchestration, notifications. Identifies 6 critical gaps.                                                                                          |
| `design-audits/runtime-execution.md`     | Audit of specs 003, 004, 005, plans 003, 004, 005, run state machine, queue/intervention model. Covers run lifecycle, provider drivers, queue/steer/pause, event taxonomy. Identifies state machine gaps, spec contradictions, driver contract issues.                                                                      |
| `design-audits/git-workflow.md`          | Audit of specs 007, 008, 009, 010, 012, 015, plans 007, 009, 010, 012, 015, domain models, ADRs 004/006. Covers repo/workspace/worktree, git flow, diff attribution, PR prep, workflow authoring, approval model. Identifies workflow spec weakness as largest gap area.                                                    |
...
| File                       | Content                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `provider-wire/README.md`  | Family front-door: the four-grade TRUST vocabulary (Verified / Documented / Derived / Provisional) and the orthogonal five-value PROVENANCE axis (Generated schema / Upstream source / Official docs / Binary probe / Cross-reference), why they stay orthogonal (the rejected fifth trust grade), the pin / floor / degrade-per-capability policy, the nightly compatibility check that keeps the pin honest, and how consuming docs cite these files.                     |
| `provider-wire/codex.md`   | Codex `app-server` wire, pinned to `codex-cli 0.150.1` (floor `0.141.0`) and regenerated from the binary's own `generate-json-schema` / `generate-ts`: the legacy-vs-slash method namespace, the runtime experimental gate that silently drops 23 of 79 server notifications, the rollback-deprecation and `thread/fork` successor, and the goal / inject / turn-start / realtime (gated) / server-request callback shapes.                                                                                  |
| `provider-wire/claude.md`  | Claude Code headless CLI wire, pinned to `2.1.251` (supported floor `2.1.234`, ratified by the 2026-08-26 Spec-004 amendment) against a binary string census cross-checked with the docs census + changelog version anchors: the `--help`-non-authoritative rule and its converse, the flag surface, the 15-subtype control-request registry with its typed refusals, the experimental `get_usage` surface, and the recorded docs-vs-binary gaps.                                                                                                      |
...
| **BL-014b** Expand Spec-015              | Same as BL-014a plus Forge: [review.md Appendix A.3](forge/review.md#a3-workflows-discussions-plans-and-human-gates)                                                                                                                                                                                                    |
