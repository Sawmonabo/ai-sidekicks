...
**Consequence for the driver:** the conversation-rewind leg has a non-experimental successor available at the pin and should target `thread/fork` + `lastTurnId` rather than building on the deprecated method. The file-restore leg is unaffected either way — it was never the provider's.
