...
**What this reference does and does not settle.** Descriptively: `thread/rollback` carries an upstream deprecation at the pin, and `thread/fork` + `lastTurnId` is a non-experimental successor available there. Prescriptively: **nothing**. This is a non-governance reference file, and the two are not interchangeable methods for one behavior — a fork creates a **new thread**, where the deprecated path retains a single binding and rotates its delivery generation at an in-stream fence. `Spec-005` still requires the native method, and `Spec-004`'s rollback invariant is written against the retained-binding semantics, so adopting the successor would change rollback behavior rather than merely re-spell it. Whether the driver migrates is a spec amendment to make deliberately, not a consequence to read off this file. The file-restore leg is unaffected either way — it was never the provider's.
