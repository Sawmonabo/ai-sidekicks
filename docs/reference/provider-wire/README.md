...
| ---------------------- | ------------------------------ | ------------------- | --------------- | -------------------------------------------------------- |
| [`codex.md`](codex.md) | Codex CLI (`codex app-server`) | `codex-cli 0.150.1` | `0.141.0`       | the binary's own generated schema + the tagged upstream source |
| [`claude.md`](claude.md) | Claude Code headless CLI     | `2.1.251`           | `2.1.234`       | a string census of the pinned binary + the official docs census |
...
| **Upstream source** | The vendor's own published source, read at a **release tag** (e.g. `openai/codex` `rust-v0.150.1`). Records what the generator cannot: runtime gating, attribute markers, and dispatcher behavior that never reaches a generated type. Cite the tag, never a branch — `main` is not a pin. |
...
- **Pin to an exact version.** Each provider file names the CLI version its shapes were verified against and never floats. Codex pins to `0.150.1`; Claude pins to `2.1.251`. Where a behavior first appeared in a known release, the file records that release as a **version anchor** (the Claude changelog carries no dates, so version is the only stable anchor).
...
- **State the counting basis with any census number.** Method and notification counts in these files are taken over the **default (non-experimental) generation**, one entry per arm of the generated union root, keyed on that arm's `method` constant. `--experimental` generation yields larger numbers on the **request** roots but an identical set on `ServerNotification` (measured at `0.141.0`, `0.149.1` and `0.150.1`), so the two bases diverge per-root rather than uniformly and a differently-basised count is not comparable to these. Any future bump re-derives on this same basis or says which basis it used instead, and **measures both endpoints with one script in one run** — never one endpoint now and the other from memory, and never two endpoints counted by two different methods.

That last clause is written from a mistake, not from theory. The `0.149.1` re-pin's census was drafted, then "corrected" to different numbers by a second measurement that counted on a different basis, and the correction was the wrong one: it moved `ClientRequest` and `ServerNotification` off their true floor and pin values and invented a delta with them. On one of the two roots the blend is directly identifiable: the bad pass put `ServerNotification` at that pin at **77**, which is exactly the number of variants the tagged upstream source _declares_, where the binary _generates_ **75** (the two declared-but-ungenerated variants are named in [`codex.md`](codex.md), and the same two-arm gap holds at `0.150.1`, where the counts are 81 and 79). That number was a source-declaration count wearing a generated-schema label. The `ClientRequest` endpoint was wrong by three in the same direction and its cause was **not** isolated — so one root's blend is confirmed and the other's is merely suspected, which is itself the reason the rule below is stated as a procedure rather than as a warning about one known trap. The error was caught only by re-measuring **both** versions with a single script that counts top-level union arms — the basis stated above — and it would not have been caught by re-checking either version alone, because each measurement was internally consistent. A census is a comparison, so the counting method, not just the count, has to be held fixed across the endpoints being compared.

These rules are adopted campaign-wide; the capability-enhancements design §3.4 is the provenance record for the audit that established the first version of them (`../../archive/superpowers/specs/2026-07-01-capability-enhancements-design.md`).
...
- **Driver fixture paths are cited as plain text, not links.** The captured-wire fixtures the drivers test their normalizers against do not exist yet — they land with Plan-004 Phase 3. Each provider file names its fixtures path as text with that marker, so the cite records intent without a dangling link.
- **Do not restate a pinned version number in a governing doc.** A spec or plan that hardcodes "Claude Code 2.1.251" acquires a copy that this family's next re-pin cannot reach. Cite the file and let the version live in one place. A **version-anchored evidence** claim is the exception and is not a restated pin: "measured at `X`" records what a probe saw on a date and stays true after the pin moves, where "pinned at `X`" goes stale the moment it does. A re-pin therefore moves pin-pointers and leaves evidence anchors alone.
