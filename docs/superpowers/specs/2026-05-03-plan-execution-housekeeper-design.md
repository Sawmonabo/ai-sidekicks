...
(`cross-plan-dependencies.md` §6 — the `Upstream:` line of the `### NS-14: the retired tier plan-readiness audit — the retired invite-membership-and-presence plan` entry.)
...
- `- References: <markdown links + line cites>` — typically `[Plan-NNN](../../plans/NNN-...)` followed by `:NNN[-MMM]` line cite, comma-separated. May include literal repo-relative paths (e.g. `packages/runtime-daemon/src/bootstrap/secure-defaults-events.ts`, optionally with :24,35,59-style line cites).
...
1. **From `References:`:** parse markdown links matching `\[([^\]]+)\]\((\.\./[^)]+\.md)\)(:\d+(-\d+)?)?` to extract relative doc paths and optional line cites. Also parse bare-path tokens matching `[a-zA-Z0-9_./\-]+\.(md|ts|js|mjs|sql|rs|toml|json|ya?ml)(:\d+(,\d+)*(-\d+)?)?` to catch repo-root-relative source-file cites (e.g. `packages/runtime-daemon/src/bootstrap/secure-defaults-events.ts`, historically cited with :24,35,59-style line pins, from NS-11).
...
The lookup scans **only `### NS-...` heading titles**, NOT body Summary / References. Heading-only is deliberately conservative — body-prose matching reintroduces the brittle parsing surface the v3 reviewer surfaced (e.g., NS-14's Summary contains `the retired invite-membership-and-presence plan Phase 1-6` describing the audit's parallel subagents, which would create a false-positive match against any future the retired invite-membership-and-presence plan Phase 1 implementation PR). Heading text is authored intentionally to identify the work; body prose drifts.
...
3. **Plan-NNN + Tier-K exact match**: heading contains `Plan-<plan-NNN>` AND `Tier <tier-K>` (e.g., NS-03 matches `(023-partial, Tier 1)`; NS-14 matches `(002, the retired tier)`).
...
3. **Verify candidate vs diff.** Run three orthogonal checks per candidate (`verifyCandidate(candidate, gitDiff, args)`). ANY check failure → exit 2 with `manifest.verification_failures` populated; mechanical edits aborted.
   ...
   - **Plan-identity sanity check.** **Carve-out for cleanup / governance Types**: if candidate `- Type:` ∈ {`cleanup`, `cleanup (doc-only)`, `governance`, `governance (doc-only)`, `governance (load-bearing)`}, this check is SKIPped (parallel to the Type-signature permissivity for `cleanup` and the file-overlap SKIP for these Types — these shapes are explicit-candidate manual dispatches per §4.3.4 and their headings legitimately do not carry Plan-NNN / task-id / Tier-K tokens). Surface a `concerns` entry of `kind: plan_identity_skipped_for_manual_dispatch` for user awareness but do NOT halt. For all other Types, candidate's heading title MUST contain at least one of:
     ...
     - The `--tier` arg if passed, matched per **two** sub-rules (mirrors §4.3.2 rule 3 + rule 4):
       1. **Substring**: heading contains `Tier <tier-K>` literal (e.g., NS-14's heading `the retired tier plan-readiness audit — the retired invite-membership-and-presence plan` matches `--tier 2`).
...
7. **Route the plan's `## Done Checklist` to the subagent.** The script does not tick it. This step was originally a mechanical tick: open `docs/plans/NNN-*.md`, find the matching `### Phase N — ...` section, locate a `#### Done Checklist` sub-section nested inside it, and tick **all** boxes (rationale: Q11.7 — Phase E only fires after a complete Phase merges). **Retired 2026-07-27.** No plan in `docs/plans/` has ever carried that shape: every plan file, the template included, carries a single document-level `## Done Checklist` at the end instead — so the tick never fired once in production and the run returned a silent exit 3. That is a claim about the corpus as it stands, not as it stood the day this was written, and the next plan file added could falsify it silently — so it is enforced rather than asserted. `.claude/skills/plan-execution/scripts/__tests__/plan-done-checklist-corpus.test.mjs` enumerates `docs/plans/` at runtime and fails if any plan carries a phase-nested `Done Checklist` or anything other than exactly one at document level; it pins no file count (a pinned count would be the next stale number), its heading scan skips fenced and HTML-commented regions so only headings that actually render are counted, and it runs in CI on the plan-execution skill-tests leg. Repointing it at the real heading would have been worse than leaving it dead: those rows are evidence-bearing prose naming PRs, SHAs, and merge dates, and the checklist is plan-scoped rather than phase-scoped, so a blind tick on a mid-plan phase ship asserts done-ness both untraceably and early. The script instead emits `plan_done_checklist_evaluation` in `semantic_work_pending`, but only for a plan-bound run — `--plan` and `--phase` both present — whose plan file resolves to exactly one `docs/plans/<plan>-*.md`, and it declares that resolved path in `affected_files` so the subagent is authorized to edit it. A cleanup / governance / tier-audit run legally omits both flags and drops the item (a run with no plan cannot answer a plan-scoped question); a plan-bound run whose file does not resolve drops it as well and records a `plan_file_unresolved` entry in `warnings` instead. When it IS emitted, the subagent reads the plan and decides — tick the row with its evidence if due, or record `not due — phase N of M` if not. The item is evaluation-shaped (mirroring `ns_auto_create_evaluation`) so that the common "not due" answer is completed work rather than a `concerns` deferral that would make `DONE_WITH_CONCERNS` the default verdict.
...
4'. **Emit manifest stub** with `auto_create.reserved_ns_nn` and `auto_create.derived_title_seed` populated. `mechanical_edits.status_flip` / `mermaid_class_swap` are absent (the new node is added by the subagent, not the script). `semantic_work_pending` carries the auto-create-specific items `auto_create_compose_entry`, `auto_create_compose_mermaid_node`, `auto_create_derive_upstream`, plus `plan_done_checklist_evaluation` when the run is plan-bound and its plan file resolves — the one item shared with the `--candidate-ns` branch, since a phase shipped either way. A run without `--plan` / `--phase`, or one whose plan file does not resolve to exactly one path, omits the item on this branch too (the unresolved case adding a `plan_file_unresolved` entry to `warnings`), so the auto-create `semantic_work_pending` list is 3 or 4 items depending on plan-boundness.
...
| **NS-14** | `the retired tier plan-readiness audit — the retired invite-membership-and-presence plan` | `audit (doc-only)` | `(the retired invite-membership-and-presence plan, the retired tier)` → §4.3.2 rule 3 (Plan + Tier); 1 match. **Audit dispatch convention**: orchestrator parses `<tier-K>` from PR-tag `plan-readiness-audit-tier-K-complete` per `plan-implementation-readiness-audit-runbook.md`:209 precedent. | Type-sig: `audit (doc-only)` ↔ diff touches only the the retired invite-membership-and-presence plan doc + `docs/operations/audit-runbook.md` (no `packages/`) ✓; **file-overlap: SKIP** for audit Type ✓; plan-identity: heading has BOTH `the retired invite-membership-and-presence plan` and `the retired tier` ✓. **False-positive guard**: NS-14's body Summary contains `the retired invite-membership-and-presence plan Phase 1-6` (describing 6 parallel Phase audit subagents). §4.3.2 heading-only matching prevents a future the retired invite-membership-and-presence plan Phase 1 implementation PR from false-positive matching NS-14 — the heading does not contain `Phase 1` (only `the retired tier`), so a `(the retired invite-membership-and-presence plan, Phase 1)` lookup returns 0 candidates → `--auto-create`, correctly creating a new NS for the implementation work. | ✅ AUTO (audit SKIP) |
...
- **Lookup axis**: AUTO-PASS via Plan+Phase (rule 1; NS-01/05/07/09/10), AUTO-PASS via Plan+task-id (rule 2; NS-06/08), AUTO-PASS via Plan+Phase+task-id (rules 1+2; NS-02), AUTO-PASS via Tier-K substring (rule 3; NS-14 precedent), AUTO-PASS via range-form Tier-K arithmetic (rule 4; NS-15..NS-21 + ACTIONABLE-3), EXPLICIT-CANDIDATE PASS (NS-11), OUT-OF-SCOPE manual fall-through (NS-22), false-positive guard (the retired invite-membership-and-presence plan Phase 1 hypothetical).
...
- ~~Q11.6 Set-quantifier claim discovery mechanism~~ — resolved by Plan §Decisions-Locked D-2 to option (c): the subagent re-derives quantifying claims from `docs/architecture/cross-plan-dependencies.md` §6 prose only, and from nothing else. Live in SKILL.md's Phase E step 4.
- ~~Q11.7 Plan §Done Checklist matching algorithm~~ — moot. The mechanical tick this question asked about is **Retired 2026-07-27** (§5.1 step 7); Plan §Decisions-Locked D-3, as amended, narrows the decision to an evaluation the subagent performs rather than a tick the script applies. The corpus property the question turned on — that no plan carries a phase-nested `Done Checklist` — is now enforced at runtime by `.claude/skills/plan-execution/scripts/__tests__/plan-done-checklist-corpus.test.mjs` rather than assumed.
