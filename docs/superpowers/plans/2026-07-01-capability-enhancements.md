...
- [x] **Step 2:** Author the Amendment block covering all ten Design-§4-B1 bullets; anchors to re-verify: :183 (`runVersion`), :345–:352 (`usage_telemetry`). Mid-review addition (item (11)): the `executionPosture?` carrier — re-check anchors Spec-006 :97 (amendment item list) + :186 (run-state payload rule, `run.running` stamp) + the api-payload `RunStateChangeEvent` mirror comment.
