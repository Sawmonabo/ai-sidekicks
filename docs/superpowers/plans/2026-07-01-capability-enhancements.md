...
- [ ] **Step 3:** Verify in-PR: snapshot refs land under `refs/sidekicks/runs/<runId>/turn-<N>` in tests; tests assert an **untracked** file is captured by the snapshot (the temp-index recipe's load-bearing property), that restore removes post-snapshot files (`read-tree --reset -u` + fixpoint `clean -ffd` semantics — including a turn-created untracked `.gitignore` **and** the directory it ignored, which a single clean pass leaves behind), that after restore the index equals `HEAD` (a captured-untracked file is untracked again, never a staged addition — the closing index-reset leg), that the snapshot commit's author/committer ident **and dates** equal the daemon-owned fixed ident and the daemon-stamped fixed-offset instants, and that the snapshot OID is repo-config-independent — an `i18n.commitEncoding` setting must not change it, per the recipe's `-c i18n.commitEncoding=utf-8` pin (capture ran `-m` + explicit `GIT_AUTHOR_NAME/EMAIL/DATE` + `GIT_COMMITTER_NAME/EMAIL/DATE` env — no stdin message read, no passwd/gitconfig ident dependency, no host wall-clock/timezone or repo-config leak into the OID), and that an **ignored** path at a non-colliding location is neither captured nor deleted by restore while an ignored path shadowing a snapshot-tracked path is overwritten and enumerated in the restore result (the B21 ignored-path boundary plus its collision carve-out), that a restore attempted after a post-snapshot branch commit is refused with the typed refusal (the fail-closed `HEAD`-equals-snapshot-parent precondition); a snapshot failure path asserts diagnostic-not-blocking; prune-on-retention covered; `pnpm turbo run typecheck` + `pnpm test` green.
