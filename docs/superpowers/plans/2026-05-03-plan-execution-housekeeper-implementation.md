...
> **Superseded in part, 2026-07-27 — the plan-checklist tick was retired.** The **Goal** and **Architecture** above list "plan §Done Checklists" / "plan checklist tick" among the script's mechanical edits. `tickPlanDoneChecklist` keyed on a `#### Done Checklist` nested under `### Phase N`, and no file under `docs/plans/` has ever carried that shape — `git log -S'#### Done Checklist' -- docs/plans/` is empty across all history — so the deliverable shipped and then never fired. The never-fired half is an entailment rather than an independent observation: a predicate no file has ever satisfied cannot have matched, so it stands without a run record. Run repo-wide instead of path-scoped, the same probe returns only the two commits that ever introduced the string — `b83410a` (this plan) and `a0a0129` (the script and the fixtures written from it) — with every later commit touching it belonging to the PR that retired the tick. Every plan file, the template included, puts a document-level `## Done Checklist` at the end instead — enforced at runtime by `.claude/skills/plan-execution/scripts/__tests__/plan-done-checklist-corpus.test.mjs`, which enumerates `docs/plans/` and pins no file count. Repointing the tick there would have been worse than leaving it dead: those rows are evidence-bearing prose naming PRs, SHAs and dates, and the checklist is plan-scoped, so a blind tick on a mid-plan phase ship asserts done-ness with no trace and asserts it early. The function, its three call sites, the `plan_checklist_ticks` manifest field and the `plan_checklist_not_found` warning are removed; deciding whether a tick is due is now the subagent's `plan_done_checklist_evaluation` semantic-work item. Exit code 3, which only that path produced, is reserved and still routed.
>
...
### D-3 (resolves spec §11 Q11.7): Phase E evaluates the plan's Done Checklist and never ticks it unconditionally

> **Amended 2026-07-27 — narrowed, not reversed.** This decision was locked as "Phase E ticks ALL Phase boxes": when Phase E fired after a multi-Phase merge, the housekeeper would tick ALL boxes in the plan's `## Done Checklist` corresponding to the merged Phase, not just the most recent task. Its stated warrant was a verbatim quote of spec §5.1 step 7 — "Tick **all** boxes (rationale: Q11.7 — Phase E only fires after a complete Phase merges)" — and the spec now marks that sentence **Retired 2026-07-27**, so the warrant no longer stands on its own. What survives is the target this decision names: `## Done Checklist` at document level, one per plan file, is the correct heading. What is falsified is the phase-scoping and the unconditional tick — the checklist is plan-scoped and has never been nested under a `### Phase N`, so "boxes corresponding to the merged Phase" names a sub-structure no file under `docs/plans/` has ever carried.

Phase E does not tick this checklist mechanically. On a plan-bound run the subagent **evaluates** whether a tick is due for the phase that just shipped and records the decision either way under `semantic_edits.plan_done_checklist_evaluation`: due → tick the row with the evidence those rows carry (PR #, squash SHA, merge date); not due → record `not due — phase N of M`. Because the checklist is plan-scoped, "not due" is the ordinary answer on a mid-plan phase ship, and recording it IS completed work rather than a `concerns` deferral.

Rationale: the original trigger argument — Phase E only fires after a complete Phase merges — is still true, and was never sufficient. A complete Phase is not a complete Plan, and plan-completeness is what a `## Done Checklist` row asserts; the unconditional tick closed that gap by assuming it away. Canonical mechanism: `.claude/skills/plan-execution/references/post-merge-housekeeper-contract.md` § Manifest schema.
...
**[MERGE MECHANICS SUPERSEDED, 2026-07-27 — the decision itself stands]:** the bullets below record what PR #33 decided and remain true as provenance, but two of the invocations they name no longer describe the landed contract. Auto-merge is DISABLED on this repository — `gh pr merge --auto` fails with `Auto merge is not allowed for this repository (enablePullRequestAutoMerge)` (verified PR #119) — so the queue-on-CI mechanics the second and third bullets describe are not something the Phase E leg can do today, whatever was true when PR #33 landed. Both merge invocations also predate the sha pin: the landed form on each leg polls `gh pr view <PR#> --json mergeStateStatus,headRefOid` in ONE call until `mergeStateStatus` reads `CLEAN`, then merges that same observed head with `--match-head-commit <headRefOid>`. Canonical: `references/post-merge-housekeeper-contract.md` § Housekeeping commit landing, and `.claude/skills/plan-execution/SKILL.md` § Phase D.5 — Merge transition + § Phase E — Post-merge housekeeping. Do NOT edit the bullets below to match — they are the PR #33 record, not a description of current behavior.
...
### Task 2.3: Update path reference in the `preflight.mjs` usage-error message

**Files:**

- Modify: `.claude/skills/plan-execution/scripts/preflight.mjs#main` (usage-error message body)
...
- [ ] Task 2.3 — `preflight.mjs#main` path reference updated
...
- A new bullet at the top of Phase E noting "Per Plan §Decisions-Locked D-3: Phase E ticks ALL Done-Checklist boxes for the merged Phase, not just the most recent task — this matches the orchestrator's complete-Phase-merge trigger." **Superseded 2026-07-27:** D-3 was narrowed, so the bullet this instruction produced no longer states the decision. The live SKILL.md `### Phase E — Post-merge housekeeping` opening bullet is canonical and now reads "Phase E never ticks the plan's `## Done Checklist` unconditionally — it evaluates first, and a tick requires evidence." This line is retained as the provenance trail: a reader tracing why that bullet exists lands here, and Plan §Decisions-Locked D-3 as amended is what it now says.
...
The housekeeper subagent itself ticks these boxes in Phase E after PR 4 squash-merges. Per spec §10.2 bootstrap, PR 4 IS the housekeeper's first auto-run — so this checklist is the very first one the housekeeper ticks autonomously. **Amended 2026-07-27:** the partial-tick anomaly this note originally told the reader to watch for can no longer occur — per the amended Plan §Decisions-Locked D-3 the housekeeper ticks no plan checklist mechanically, so there is no tick that could land partially. Watch instead that a plan-bound run records a `plan_done_checklist_evaluation` decision at all; a run that silently omits it is the same write-only shape as the retired tick.
