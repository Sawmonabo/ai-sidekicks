...
7. **Round-3 refinements (Codex PR #327 round 3, 2026-08-12 — final round under the session's review cap).** Five cross-surface consistency fixes: `updateCapabilities` takes level 1 (`sessions … FOR SHARE` — the `VERSION_FLOOR_EXCEEDED` gate is authoritative-at-write; Step 10's rules and the lock-ordering assertions now match ADR-025 D8 and the dep-map's levels 1→2→3 cell); Step 7c's HTTP oracle compares the complete `aisError` envelope for one `nodeId` across two DB states (a message relaxation would have re-opened the oracle T3.12 closes); Plan-003 T3.12 extends the canonical `memberships/__tests__/lock-ordering.test.ts` rather than forking a runtime-nodes sibling; the classifier module is canonically `classify-caller.ts` on both task surfaces; and the BL-141 backlog citation gained its §Adjudication Record anchor.
...
> **BL-134 leg closed 2026-08-26 — premise-void, never executed, no successor.** The third clause of this task (the `+ BL-134 note` in the heading and DAG row, the "annotate BL-134 dormancy" Files bullet, the second half of Step 8, and the `BL-134` token in Step 10's drafted `Refs:` trailer) is retired rather than done. PR #356 ratified `clipanion@4.0.0-rc.4` as a **terminal** pin rather than a provisional one — folding the audited rc.4 defects into `Plan-007 §Phase R3 — Client Delivery (Tier 4)` as normative task text (§6 node NS-78) — and archived BL-134 outright as premise-void with five named re-evaluation triggers ([`backlog-archive.md` §BL-134](../../archive/backlog-archive.md#bl-134-clipanion-stable-v4-upgrade--lockfile-bump-for-plan-007-r3-pr-a-cli)). There is no live entry left to annotate and no "within one week of stable" trigger left to recalibrate: the watch surface this task assumed (an upstream stable-v4 tag) is the very premise the archive record falsifies. Step 8's checkbox stays **unticked** deliberately — its BL-131 half was executed, its BL-134 half never will be, and a tick would assert both.
>
