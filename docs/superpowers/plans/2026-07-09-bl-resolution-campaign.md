...
- Modify: `packages/control-plane/src/memberships/__tests__/lock-ordering.test.ts` (**I-003-6 regression block** — extend the shipped logging-proxy `Querier` technique that already pins I-002-4, with **per-procedure assertions matching Step 7's breakdown, not one blanket order**: the two-phase order — unlocked attachment pre-read < `session_memberships … FOR SHARE` < `runtime_node_attachments … FOR UPDATE` — for `detach` / `heartbeat.ingest` / `updateCapabilities` **only**; `attach` separately (`sessions … FOR KEY SHARE` first, membership `FOR SHARE` second, both before any `runtime_node_attachments` statement; it creates the row via upsert, so it has **no** unlocked pre-read and **no** attachment `FOR UPDATE` — asserting the blanket order against `attach` would fail the correct implementation); `readRoster` separately (membership `FOR SHARE` present, **no** `runtime_node_attachments` lock ever — Step 10's atomic read boundary). Cross-directory but correct: this file is the canonical home of the lock-ordering invariant, and forking a second one would fracture it).
...
- Produces:
  ...
  - `classifyRuntimeNodeCaller(transaction: Querier, callerParticipantId: ParticipantId, row: { participant_id: string; session_id: string }): Promise<RuntimeNodeCallerVerdict>` — used by `detach` / `updateCapabilities` / `heartbeat` (which have an existing row to classify), each after an **unlocked** pre-read of that row and before the `FOR UPDATE` re-resolve (I-003-6 lock order). It takes the level-2 `session_memberships … FOR SHARE` lock internally. `attach` and `readRoster` instead run a **direct `active`-membership SELECT** (no existing node row to classify — attach is creating one, roster is enumerating); **both are `FOR SHARE`** — `attach`'s inside its existing transaction, AFTER its level-1 `sessions … FOR KEY SHARE` floor read (Step 10's deadlock note), `readRoster`'s inside the transaction Step 10 wraps around the membership check + roster SELECT (the read-side authorization must be atomic with the enumeration it authorizes). (`Querier` is the transaction-callback type — `migration-runner.ts` passes a connection-bound `Querier` to `transaction(fn)`, not a narrower type; there is no `Queryable`.)
...
`readRoster` asserts: its membership `SELECT … FOR SHARE` runs through the in-transaction `Querier` (Step 10 wraps the method in a transaction), and **no** `runtime_node_attachments` lock statement ever appears — the roster enumeration itself is a plain read. Semantic two-connection interleave tests remain the Tier-5 integration-suite follow-up (named, not silently skipped).
...
Apply the same shape to `heartbeat`, `capabilityupdate` (keeping its surviving CONFLICT arm alongside the new NOT_FOUND arm), and `roster` (`.query`, NOT_FOUND on no-visible-session). **`attach` gains a NOT_FOUND catch-arm too** (for the new axis-a membership refusal) and is now threaded `callerParticipantId` — it retains its self-check, **canonicalized in this step**: `canonicalizeUuid(input.participantId) !== canonicalizeUuid(callerParticipantId)` → UNAUTHORIZED. The shipped check compares raw `!==`; the branded schema admits mixed-case UUIDs while `resolveCurrentParticipantId` returns the canonical form, so once the service pre-guard canonicalizes (Step 9), the raw router check would falsely 401 the true owner's mixed-case attach **before** the service could admit it — the same false-rejection class as the decision-3 owner check. UNAUTHORIZED stays the mismatch status (shipped contract); router-level pins: mismatched participant → UNAUTHORIZED (unchanged), same-participant case-varied `input.participantId` → **succeeds** (the canonicalization regression). It keeps its axis-b `attach_conflict` (409) path; the membership guard raises `RuntimeNodeNotFoundException` → NOT_FOUND. So `attach`'s catch-arm carries UNAUTHORIZED (self-check, pre-existing) + NOT_FOUND (axis-a, new) + CONFLICT (axis-b, shipped).
...
`readRoster` is exempt only from the **level-3** contract — it mutates nothing, so there is no `FOR UPDATE` + re-verify phase. Its membership read still takes the level-2 `FOR SHARE`, inside its own transaction (Step 10): the membership check is the read-side authorization boundary, and two unlocked statements would let a concurrent revoke commit between them, handing the revoked caller a roster computed from a snapshot **newer** than their authorization. **Do not "simplify" the nodeId-keyed mutators' phase-1 attachment pre-read into a locked read** — that is exactly the order inversion rule 1 forbids.

`attach` — gains `callerParticipantId` as its first param, opens the existing `this.#querier.transaction` with the **level-1 `sessions … FOR KEY SHARE` floor read** (rule 1's implicit-FK deadlock note — this statement subsumes the shipped unlocked floor read AND delivers the nonexistent-session 404), and runs the **axis-a membership guard** second. The change supersedes the floor-read comment (~:413-419) that currently says session authorization "is the router's gate (T3.8), not this service's" — correct that comment to record that the service now enforces membership:
...
async attach(callerParticipantId: ParticipantId, request: RuntimeNodeAttachRequest): Promise<RuntimeNodeAttachResponse> {
  ...
  return this.#querier.transaction(async (transaction) => {
    ...
    // doubles as the shipped floor read (step 1) AND the nonexistent-session 404
    ...
    // attachment row is created. A non-member yields the uniform not-found with the
...
`readRoster` — gains `callerParticipantId` first and **wraps both statements in `this.#querier.transaction`**: the membership check — the caller holds an `active` `session_memberships` row for `input.sessionId`, taken **`FOR SHARE`** — then the roster SELECT inside the same transaction. Absent membership → `RuntimeNodeNotFoundException` (identical for an unknown session and a session the caller isn't in). The lock and the transaction are load-bearing, not decorative: as two separate unlocked statements, a concurrent revoke can commit between them, so the roster would be computed from a snapshot **newer** than the authorization — the just-revoked caller could even receive attachments created after their revocation. `FOR SHARE` holds the membership fact stable through the enumeration (it conflicts with the revoke's `FOR NO KEY UPDATE`, PostgreSQL Table 13.3 — the same level-2 mode every other method takes, so all five now authorize under one idiom); the roster SELECT itself stays a plain read — **no** `runtime_node_attachments` lock. A member of a **visible-but-empty** session still gets `{ nodes: [] }` (the membership check passes; the roster SELECT returns no rows). This supersedes the shipped `{nodes: []}`-for-unknown-session posture. `readRoster` classifies by membership only — there is no per-row owner question for an enumeration. Comment the membership SELECT to record the I-003-3 read-vs-mutate distinction: roster now **references** `session_memberships` for authorization (under `FOR SHARE`) but never **mutates** it (the byte-unchanged property the I-003-3 test asserts still holds — a shared row lock reads, it does not write; Step 12.5).
...
- [ ] **Step 12: Migrate the existing suites to the new signatures + superseded postures.** This is the widest blast radius in the task; enumerate each in the PR body as a ratified Spec-003 contract change (PR-A1), not regression masking. The five service methods gained a `callerParticipantId` first parameter, so **every existing call site updates** — these are build-breaking (TS + runtime), not optional:
  ...
  3. **Heartbeat ingest presence-forgery close** — every successful `ctx.service.ingest` call site in the file, not only the ingest describe block. Four call sites migrate: the ingest suite's "creates a presence row … on first heartbeat (P6)" (~:182) and "updates the SAME row …" (~:196), **plus the two staleness/hysteresis recovery tests** — "restores a sweep-demoted degraded node to online …" (ingest at ~:293) and "resurrects a sweep-declared offline node to online …" (ingest at ~:317) — which today seed only `runtime_node_presence` before ingesting and would fail with `RuntimeNodeNotFoundException` instead of exercising liveness recovery. Each must first `seedParticipant` + `seedSession` + `seedMembership` + `seedAttachment` (active) for `NODE_ID` owned by `PARTICIPANT_ID`, then `ingest(PARTICIPANT_ID, …)`. (The invalid-`healthState` parse test at ~:211 needs no seeding — Zod rejects before the resolve.) Add a NEW negative test: `ingest(STRANGER_PARTICIPANT_ID, …)` on a never-attached node → `RuntimeNodeNotFoundException`, no presence row (moved from Step 7 if colocated).
