...
- [x] **Step 4:** Fixed BL-122: References **and** the Summary sentence (§Axis 2 → §Axis 1 — the cache text lives in §Axis 1; both forms swept, then re-grepped repo-wide for survivors); exit (a) re-pointed from `TURBO_TOKEN` (Vercel Remote Cache's bearer auth, which this design has no use for) to the HMAC signature key that is actually the only secret in play; the cache-cold baseline persisted onto the BL as **282 s** with its method (N=8, measured 2026-08-25 over the eight most recent merged PRs — the 218-223 s figure that lived only in these campaign records is superseded); measurement window recorded (≥5 PRs; below 30% → revert + record the negative result). ADR-023 §Decision Log entry added covering the provider adjudication (rharkor as the $0 GitHub-native option, riding **ADR-022's** "or equivalent" latitude — the phrase is not ADR-023's; ducktors fails the $0 constraint), the "deployed in our infrastructure" overstatement corrected to decided-now-implemented, the never-cache-a-test rule, and the F-8 mechanism correction.
