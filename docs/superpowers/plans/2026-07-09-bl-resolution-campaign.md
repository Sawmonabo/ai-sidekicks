...
6. **Round-2 refinements (Codex PR #327 round 2, 2026-08-12).** Three additions recorded across the corpus and mirrored below: the ADR-025 §Uniform-Negative Scope residual now names known-id post-detach squatting for roster-disclosed ids — pinned by an accepted-residual test in Plan-002 T3.12 and Step 7 below, closing mechanism the V1.1 node-identity trust anchor (ADR-017); the retired control-plane-relay-and-session-join plan's `T-008r-1-5` gains the host-wiring leg (the throwing `resolveCurrentParticipantId` swap + per-route verified-token tests) so I-008-4's runtimenode rows have an owned path to production-live; and Spec-002's under-lock mandate is scoped to the four mutations, stating the D7 lock-free roster exemption at the source. The design spec's §4.A carries a supersession note pointing at ADR-025 and this block.
...
- [x] **Step 6:** Amend the retired control-plane-relay-and-session-join plan I-008-4 to add `runtimenode.attach` / `heartbeat` / `capabilityupdate` / `detach` / `roster` to the gated-endpoint list (one-line rationale: the runtime-node guard consumes the same R1 middleware output). _(2026-08-12: executed in this same vehicle per correction 2, with the retired control-plane-relay-and-session-join plan flip-and-restored `approved` in the same swap and its §Preconditions plan-readiness box re-opened-and-re-checked scoped to this single leg. The retired control-plane-relay-and-session-join spec untouched — the leg is plan-body only.)_
 ...
 * row — no double-fetch.
 *
 * CALLER CONTRACT 1 — LOCK ORDER (I-002-6, extends I-002-4): the canonical order is
...
- Modify: the retired invite-membership-and-presence plan (Status → `review`; new Phase-2 tasks T2.6/T2.7/T2.8; new **CP-002-9** §Cross-Plan Obligations entry; re-open audit checkbox scoped to the new tasks; §Progress Log note).
...
| Plans | — | `002` (tasks + CP-002-9 + reflip), `003` (tasks + reflip), `008` (I-008-4 runtimenode via Task 2 + invite via Task 5; Tier-4 scope bullet) |
