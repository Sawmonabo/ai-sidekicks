...
- [x] **Step 11: Move BL-133 to the archive.** **Executed 2026-08-25 outside this campaign vehicle, and the closure note below is superseded** — the Plan-023 deep-link invite-confirmation amendment (cross-plan §6 node NS-81) moved the item, because the step as written closes exit (c) by hand-off alone while Plan-023 Phase 5 still gated on `{ type: bl_closed, ref: 133 }`; closing the item under that gate would have opened the phase against an unshipped endpoint. The landed closure instead gives (c) owning tasks (Plan-023 T-023r-5-5 / T-023r-6-3, invariants I-023-9 / I-023-10, obligation CP-023-5) and substitutes `{ type: plan_phase, plan: 002, phase: 2, status: merged }` for the circular gate. Original instruction retained below for provenance: Remove from `backlog.md`; append to the archive with the closure note: exit (a) **met** (the Spec-002 contract, exceeded — full invite router + hardened `aisError`/410 envelope, not just the preview contract); exit (b) **met-as-refined** — the endpoint ships on the **control-plane tRPC transport**; its daemon-as-gateway clause (ADR-008 surface, mirroring the `invite.accept` wire registration) is reassigned to **Plan-008 Tier-5** via Plan-002 **CP-002-9** (an obligation entry; the implementation task lands with Plan-008's Tier-5 leg). Name the **two** remaining hand-offs, each another plan's own step: the daemon-integration leg (IPC handlers + live control-plane caller) is **Plan-008 Tier-5**; exit (c) (Plan-023 Tier-8 deep-link consumption + two-client smoke) is **Plan-023 Tier-8**. Cites PR-B1/PR-B1p/PR-B2.
