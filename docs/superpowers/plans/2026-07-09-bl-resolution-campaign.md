...
- **Build to done (Design R1).** Every unit ends in tested code, not a plan entry. No slice is deferred except the named gates in Design §1.2 (Plan-019 rate-limit wiring; Plan-021 Tier-7 E2E + IPC; V1.1 thresholds), each with its gate named in the BL closure note.
...
| 3 | PR-A1p — Spec-002 + Plan-002 re-promotion | governance | 2 |
| 4 | PR-A2 — Unit A code + BL-141 archive | lane 4→1 | 3 |
| 5 | PR-B1 — Unit B docs | governance | 1 (merge-serialize census vs Task 2) |
| 6 | PR-B1p — the retired invite-membership-and-presence spec + Spec-019 + the retired invite-membership-and-presence plan re-promotion | governance | 5 |
...
- Modify: `docs/specs/002-runtime-node-attach.md` (already `review` since the 2026-08-03 V1 product-vision reconciliation flip — this amendment lands with no status transition; §Interfaces And Contracts + §Required Behavior authorization precondition for all five procedures — including **attach's session-membership precondition** (axis a); the four behavior changes recorded; dated Amendment block).
- Modify: `docs/architecture/contracts/error-contracts.md` (§Runtime Node — add **two** rows: `runtimenode.not_found` (404) + `runtimenode.permission_denied` (403); **and narrow** `runtimenode.capabilityupdate_conflict`'s description to the I-002-2 state guard. Per Step 4).
- Modify: `docs/plans/002-runtime-node-attach.md` (already `review` since the 2026-08-03 reconciliation flip — no status transition; new Phase-3 tasks T3.10/T3.11/T3.12; re-open the audit checkbox for the new tasks; §Progress Log note). **§Invariants takes two amendments:** (i) **I-002-3 amended** — its Verification note records that the runtime-node domain now _reads_ `session_memberships` for authorization under a `FOR SHARE` lock while still never mutating it; its two normative sentences (`RuntimeNodeAttach` MUST NOT modify `session_memberships`; `MembershipUpdate` MUST NOT trigger runtime-node detach as a side effect) are **unchanged and explicitly re-affirmed** — the `owner_inactive_member` detach carve-out exists so the second one can hold without stranding nodes. (ii) **I-002-6 added** — _any transaction touching both `session_memberships` and `runtime_node_attachments` MUST lock in the order `sessions` → `session_memberships` → `runtime_node_attachments`, counting implicit FK locks (an INSERT takes `FOR KEY SHARE` on each FK-parent row)_ — the counting clause is load-bearing: it is why `attach`'s upsert forces an explicit level-1 `sessions … FOR KEY SHARE` first (Task 4 Step 10), extending invariant 4 of the retired invite-membership-and-presence plan; verified by the `lock-ordering.test.ts` runtime-node block.
- Modify: the retired control-plane-relay-and-session-join plan (invariant 4 of the retired control-plane-relay-and-session-join plan endpoint list adds the five runtimenode procedures; the retired control-plane-relay-and-session-join plan promoted `approved` 2026-07-21 (W2.5 re-audit #239) — re-adjudicate the amendment under the Status Flip Rule in the PR, presumptively a flip: extending the security invariant's covered-endpoint set is a plan-body behavior change, a named flip trigger, so run flip → targeted re-audit → re-promotion with the PR's user pause as final arbiter).
- Modify: `README.md` (**both** §Project Status censuses — the plan-census and spec-census bullets carry **different totals**, so each takes its own delta. The **plan** bullet: Plan-002 `approved` → `review` — as of this writing `20 approved` → 19, `6 … in review` → 7, Plan-002 added to the named list. The **spec** bullet: Spec-002 `approved` → `review` — `21 approved` → 20, `6 … in review` → 7, Spec-002 added to the named list. Re-derive every count by counting, not arithmetic — the counting rule governs if intervening merges moved either census).

**Interfaces:** Produces ADR-025 (the caller-ownership decision) + the Spec-002 authorization preconditions + the two new `runtimenode.*` contract rows that PR-A2's code and reviews are conducted against.
...
2. **Step 6's the retired control-plane-relay-and-session-join plan premise was false.** The retired control-plane-relay-and-session-join plan was not `approved` at write time — the 2026-08-03 V1 product-vision reconciliation flipped it `review`, and its own restore vehicle (PR #323, 2026-08-11) landed without this leg. Step 6 was therefore executed in the same vehicle as Steps 2–5, with the retired control-plane-relay-and-session-join plan flip-and-restored `approved` in that swap, rather than as a separate flip → re-audit → re-promotion cycle.
...
4. **Neither Spec-002 nor Plan-002 took a `review` flip from this task.** Both were already `review` from the 2026-08-03 reconciliation, and the BL-141 growth was folded into the scope of that pair's restoring targeted readiness-audit delta — which is the vehicle these steps landed in — so the pair flipped once, to `approved`, rather than down and back up.
5. **Task 4's body below is rewritten in place, not overlaid (Codex PR #327 round 1).** The still-open code task originally carried the decision-4 contract in its executable text — `RUNTIME_NODE_NOT_FOUND_CODE`, `RuntimeNodePermissionDeniedException`, `NOT_FOUND` catch-arms, 404 assertions, a locked two-statement roster, and a blanket attachment `FOR UPDATE` — which, followed as written, would have produced code contradicting ADR-025, Spec-002, and the error registry. Every step, snippet, and table below now carries the ratified contract directly: the uniform 403, the single-statement lock-free roster (D7), per-procedure level-3 modes (D8 — `capabilityupdate`/`detach` `FOR NO KEY UPDATE`, `heartbeat` `FOR SHARE`), `detach`'s ratified negative split (`member_not_owner` → 403; every invisible cause → the idempotent `null`), and the router self-check mapped through the typed refusal (D5/D9).
6. **Round-2 refinements (Codex PR #327 round 2, 2026-08-12).** Three additions recorded across the corpus and mirrored below: the ADR-025 §Uniform-Negative Scope residual now names known-id post-detach squatting for roster-disclosed ids — pinned by an accepted-residual test in Plan-002 T3.12 and Step 7 below, closing mechanism the V1.1 node-identity trust anchor (ADR-017); the retired control-plane-relay-and-session-join plan's task r-1-5 of the retired control-plane-relay-and-session-join plan gains the host-wiring leg (the throwing `resolveCurrentParticipantId` swap + per-route verified-token tests) so invariant 4 of the retired control-plane-relay-and-session-join plan's runtimenode rows have an owned path to production-live; and Spec-002's under-lock mandate is scoped to the four mutations, stating the D7 lock-free roster exemption at the source. The design spec's §4.A carries a supersession note pointing at ADR-025 and this block.
7. **Round-3 refinements (Codex PR #327 round 3, 2026-08-12 — final round under the session's review cap).** Five cross-surface consistency fixes: `updateCapabilities` takes level 1 (`sessions … FOR SHARE` — the `VERSION_FLOOR_EXCEEDED` gate is authoritative-at-write; Step 10's rules and the lock-ordering assertions now match ADR-025 D8 and the dep-map's levels 1→2→3 cell); Step 7c's HTTP oracle compares the complete `aisError` envelope for one `nodeId` across two DB states (a message relaxation would have re-opened the oracle T3.12 closes); Plan-002 T3.12 extends the canonical `memberships/__tests__/lock-ordering.test.ts` rather than forking a runtime-nodes sibling; the classifier module is canonically `classify-caller.ts` on both task surfaces; and the BL-141 backlog citation gained its §Adjudication Record anchor.

- [x] **Step 1:** SPP-1 name=`a1-authz-docs`, branch=`docs/bl-141-authz`. _(2026-08-12: executed on the merged vehicle's branch `docs/plan-002-caller-auth-delta` instead — the Plan-002/Spec-002 restoring targeted readiness-audit delta, which carries this task's doc legs and Task 3's promotion legs in one PR under the Dual-flip gate.)_
- [x] **Step 2:** Author ADR-025 recording Design §4.A decisions 1–9 (decision 9 — extending the retired control-plane-relay-and-session-join plan's invariant 4 of the retired control-plane-relay-and-session-join plan gated-endpoint invariant to the five runtime-node procedures — is _recorded_ here as part of the authorization-hardening rationale; its file-level _implementation_, the invariant 4 of the retired control-plane-relay-and-session-join plan list edit, lands in Step 6) with antithesis (row-loading middleware — rejected for TOCTOU/double-fetch; Cedar now — rejected per cedar#1226 + ADR-012 approval-scope, with the named relational-growth revisit criterion; flat-404 everywhere — rejected where the corpus already discloses to members) + synthesis (transaction-interior predicate **plus the classify-then-mutate locking contract**, Design §4.A decision 1 §Locking discipline; two-tier `NOT_FOUND`/403; first-attacher-owns-`node_id`; **attach's two authorization axes — the shipped node-identity axis (b) and the newly-enforced session-membership axis (a), decision 5**; heartbeat/roster hardening). Include a **§Lock Order** subsection recording the new **I-002-6** canonical order (`sessions` → `session_memberships` → `runtime_node_attachments`, **counting implicit FK locks** — an INSERT takes `FOR KEY SHARE` on each FK-parent row, which is why `attach` opens with an explicit level-1 `sessions … FOR KEY SHARE` rather than letting its upsert acquire it after the membership lock, the ABBA inverse of `updateMembership`'s shipped order) as an extension of invariant 4 of the retired invite-membership-and-presence plan, the two-phase resolve it forces on the `nodeId`-keyed procedures, and the per-level lock modes: `FOR KEY SHARE` on `sessions` (attach only), `FOR SHARE` — not `FOR UPDATE`, and never `FOR KEY SHARE` — on the membership row, `FOR UPDATE` on the attachment. Record the **`owner_inactive_member` detach carve-out** with its rejected antithesis: reaping attachments from `MembershipService.updateMembership` is forbidden by Plan-002 I-002-3 (`MembershipUpdate` MUST NOT trigger a runtime-node detach) and would usurp the ADR-012 Cedar-gated `revoked` authority producer that `attach-service.ts` explicitly defers. Include a **§Uniform-Negative Scope** subsection recording, as an accepted residual with mitigation + revisit criterion, that `attach`'s 409 `attach_conflict` discloses one bit about a guessed `node_id` (Design §4.A decision 4 §Scope) — a Type-2 ADR must carry its own counterexample. Cite OWASP API1:2023, ASVS v5 8.2.2/8.3.1/8.3.3, cedar#1226, Matrix MSC3266 (Design §9 URLs). _(2026-08-12: ADR-025 landed `accepted` with eleven decision parts D1–D11 covering everything this step names — the transaction-interior predicate, the classify-then-mutate locking contract as D8 plus the new I-002-6, both attach axes, the heartbeat/roster hardening, the `owner_inactive_member` detach carve-out with its rejected reaping antithesis, and the `attach_conflict` one-bit residual with a named revisit criterion. Two deviations, both recorded in the ADR: the negative is 403 per correction 1 above, not the two-tier `NOT_FOUND`/403 this step names; and the `cedar#1226` citation is carried as **stale-and-corrected** (closed, resolved by PR #1256) rather than as a live blocker, with the rejection re-grounded on atomicity. Lock modes shipped as `FOR NO KEY UPDATE` on the attachment rather than `FOR UPDATE` — weakest-sufficient per PostgreSQL Table 13.3, the partial index excluding `state` from the escalation set. Matrix MSC3266 is not cited: no leg of the decision rests on it.)_
- [x] **Step 3:** Amend Spec-002 with the authorization preconditions and **all four behavior changes** (Design §4.A doc deliverables): a per-procedure precondition sentence for attach/heartbeat/capabilityupdate/detach (caller == active-node owner; cross-session → uniform not-found) + roster (session-membership predicate) + **attach's session-membership precondition (axis a)**. Record the four behavior changes explicitly: (1) attach's session-membership precondition is now **enforced** with a typed 404 (was required-but-unenforced — a non-member attach previously succeeded, a nonexistent-session attach raised a raw FK-500); (2) the heartbeat presence-forgery close; (3) the roster empty-for-unknown-session supersession; (4) `capabilityupdate`'s 409→404 split for the no-active-row arm. Add the I-002-3 read-vs-mutate note: attach/roster now **read** `session_memberships` for authorization while still never **mutating** it. Dated Amendment block (Spec-002/Spec-005 block precedent). Status is already `review` (2026-08-03 reconciliation flip) — record this amendment's flip reason in the dated Amendment block rather than a second transition. _(2026-08-12: executed as three in-place tail extensions under Spec-002's zero-net-line rule — §Required Behavior line 46 carries the substantive precondition, §Interfaces And Contracts line 90 the refusal contract, and the §Default Behavior blockquote the dated amendment record; `wc -l` is 174 before and after, so none of the 75 live `Spec-002 line NNN` cites moved. Behavior change (1) lands as a typed 403 refusal, not a 404, per correction 1; changes (2)–(4) as written. The read-vs-mutate note lands on Plan-002 I-002-3 itself, which is where the invariant text lives.)_
- [x] **Step 4:** Amend error-contracts §Runtime Node per Design §4.A: add the `runtimenode.not_found` (404) and `runtimenode.permission_denied` (403) rows verbatim from the Design's table (the `not_found` description covers roster/attach non-member/nonexistent-session too), and **narrow** `runtimenode.capabilityupdate_conflict`'s description to the I-002-2 `registering→online` state guard (its no-active-attachment arm moves to the new 404). Re-derive the §Runtime Node row count by counting. _(2026-08-12: the narrowing landed as written. The two-row addition did not: per correction 1 no `runtimenode.not_found` row exists, and the existing `runtimenode.permission_denied` row was extended with the four attach-surface call sites instead of a new row being added — so the §Runtime Node row count is unmoved and no re-derivation was owed.)_
- [x] **Step 5:** Add Plan-002 Phase-3 tasks (acceptance criteria tracing to ADR-025 + the Spec-002 rows): **T3.10** (the `classifyRuntimeNodeCaller` predicate + the two typed errors `RuntimeNodePermissionDeniedException`/`RuntimeNodePermissionDeniedException` + the `participant_id` SELECT-column extension on `updateCapabilities` + per-procedure negatives across heartbeat/capabilityupdate/detach), **T3.11** (the two session-scoped membership checks — **attach's axis-a session-membership guard AND the roster membership predicate**), **T3.12** (the two-account IDOR suite across the service / router / HTTP-envelope layers + the attach node-identity (axis-b) regression tests); Status already `review` (2026-08-03 reconciliation flip — no transition); re-open the audit checkbox scoped to the new tasks (Plan-012 precedent wording); fold the scoped audit-delta evidence into a §Progress Log note for PR-A1p to cite. _(2026-08-12: T3.10–T3.12 landed with the acceptance criteria this step names, minus the `RuntimeNodePermissionDeniedException` half of T3.10's typed-error pair, which correction 1 removes. Placed in Phase 3 rather than a new phase, with the `partially_shipped` transition recorded in the Phase-3 preamble. The audit checkbox was re-opened **and re-checked in the same commit** — growth and audit land together — so the evidence sits in the box text and in the §Preconditions record rather than in a §Progress Log note for a separate promotion PR, Task 3 having merged into this same vehicle.)_
- [x] **Step 6:** Amend invariant 4 of the retired control-plane-relay-and-session-join plan to add `runtimenode.attach` / `heartbeat` / `capabilityupdate` / `detach` / `roster` to the gated-endpoint list (one-line rationale: the runtime-node guard consumes the same R1 middleware output). _(2026-08-12: executed in this same vehicle per correction 2, with the retired control-plane-relay-and-session-join plan flip-and-restored `approved` in the same swap and its §Preconditions plan-readiness box re-opened-and-re-checked scoped to this single leg. The retired control-plane-relay-and-session-join spec untouched — the leg is plan-body only.)_
- [x] **Step 7:** Update **both** README censuses: the plan line (Plan-002 → `review`) **and** the spec line (Spec-002 → `review`: `21 approved` → 20, `6 … in review` → 7, Spec-002 added to the named list — README tracks a spec census beside the plan census; flipping only one lands a README that contradicts Spec-002's own header). Counts re-derived by counting. Then `grep -rn "Plan-002\|Spec-002" README.md CLAUDE.md docs/` for any other status claim. _(2026-08-12: executed as the **restore**-side census per correction 4 — both bullets move Plan-002 and Spec-002 `review → approved` and drop them from the named lists, not into them. Counts re-derived by counting; the ADR census moved too, ADR-025 being a new file. The grep sweep additionally discharged the 2026-08-03 flip-cohort clauses in README.md and CLAUDE.md, this pair being the cohort's last member.)_
- [x] **Step 8:** SPP-3 docs gates + `/ripple-check` (heading/cite/set-quantifier risk). SPP-4 with `plan-execution-spec-reviewer`. SPP-5/6 — subject `docs(repo): adr-025 runtime-node caller-ownership authorization`, body declares the Spec-002 + Plan-002 amendments (both already `review` per the 2026-08-03 reconciliation flip) + the invariant 4 of the retired control-plane-relay-and-session-join plan fix + the census ripple; trailer `Refs: ADR-025, Spec-002, Plan-002, the retired control-plane-relay-and-session-join plan, BL-141`. _(2026-08-12: the merged vehicle's subject — `docs(repo): restore Spec/Plan-002 approved via dual-scope caller-authorization delta` — **supersedes** the subject prescribed here, because the vehicle is the dual-scope restoring delta rather than a standalone ADR-025 PR. The trailer landed as prescribed.)_

---

### Task 3: PR-A1p — Spec-002 + Plan-002 re-promotion
...
**Files:** Modify: `docs/specs/002-runtime-node-attach.md` (`review` → `approved`); `docs/plans/002-runtime-node-attach.md` (`review` → `approved`; tick the re-opened audit checkbox); `README.md` (**both** census lines restored — plan + spec).

**Interfaces:** Produces the `approved` Spec-002 + Plan-002 that PR-A2's lane-1 preflight (Gate 7) requires.

- [x] **Step 1:** SPP-1 name=`a1p-authz-promote`, branch=`docs/bl-141-promote`. _(2026-08-12: no separate branch — Task 2 and Task 3 merged into one vehicle, `docs/plan-002-caller-auth-delta`, exactly as this task's Dual-flip gate permits.)_
- [x] **Step 2:** Flip Spec-002 + Plan-002 to `approved`; tick the audit checkbox. PR body cites runbook §Spec-Status Promotion Gate by name and the PR-A1 audit-delta evidence (Spec-024/NS-13b promotion-PR precedent): the new Plan-002 tasks trace to ADR-025 + the amended Spec-002 rows; dep-closure terminal (ADR-025 accepted in PR-A1; ADR-010/012 accepted); no open questions gating Required Behavior. For the Plan-002 half, also satisfy the **plan-side** §Status Promotion Gate elements (runbook:210–214): cite the Tier-2 audit-completion date + the `plan-readiness-audit-tier-3-complete` tag. **Dual-flip gate (2026-08-03):** Spec-002 + Plan-002 also carry the V1 product-vision reconciliation's `review` flip, whose restoring targeted readiness-audit delta is independent of BL-141's — flip to `approved` only after BOTH restoring deltas have landed, or land one combined targeted delta whose PR body declares coverage of both amendment scopes. _(2026-08-12: the **combined** arm was taken — one restoring targeted readiness-audit delta covering both amendment scopes, the 2026-08-03 reconciliation growth and the BL-141 growth, with the PR body declaring both. Spec-002 and Plan-002 flipped `review → approved` and all four Plan-002 §Preconditions boxes read `- [x]`; the plan-readiness box was re-opened by this PR's own growth and re-checked by this PR's audit in the same commit. The plan-side §Status Promotion Gate elements are satisfied by the Tier-2 audit landed via NS-15, cited in that box.)_
- [x] **Step 3:** Restore **both** README §Project Status censuses — per census, never one delta applied to both: the **plan** bullet returns Plan-002 to `approved` (`19 approved` → 20, `7 … in review` → 6, Plan-002 dropped from the named list) and the **spec** bullet returns Spec-002 to `approved` (`20 approved` → 21, `7 … in review` → 6, Spec-002 dropped from the named list). Counts re-derived by counting. _(2026-08-12: executed. The literal figures above are stale — sibling deltas moved both censuses between authoring and landing — so both bullets were re-derived by counting, as the step's own rule requires, and the ADR census was re-derived alongside them for the new ADR-025 file.)_
- [x] **Step 4:** SPP-3/4/5/6 — subject `docs(repo): re-promote spec-002 + plan-002 (adr-025 audit delta)`, trailer `Refs: Spec-002, Plan-002, BL-141`. _(2026-08-12: the merged vehicle's subject — `docs(repo): restore Spec/Plan-002 approved via dual-scope caller-authorization delta` — **supersedes** the subject prescribed here, and its trailer is the union `Refs: ADR-025, Spec-002, Plan-002, the retired control-plane-relay-and-session-join plan, BL-141`, since one PR carries both tasks.)_
...
- Modify: `packages/contracts/src/error.ts` (add `RUNTIME_NODE_PERMISSION_DENIED_CODE` — **only if** Plan-005 T4.10 has not already landed it, per ADR-025 D10's first-lander rule).
...
- [ ] **Step 4: Add the constant** to `packages/contracts/src/error.ts` (skip if Plan-005 T4.10 landed it first — D10), matching the sibling `*_CODE` export shape with explicit type annotations (TS9010, `--isolatedDeclarations`):
 ...
 * row — no double-fetch.
 *
 * CALLER CONTRACT 1 — LOCK ORDER (I-002-6, extends invariant 4 of the retired invite-membership-and-presence plan): the canonical order is
...
**Locking regression tests (Design §4.A decision 1 §Locking discipline; I-002-6).** PGlite is single-connection, so a true concurrent interleave is not directly expressible. **Do not invent a raw-source string scan** — this repo already has the right technique and a canonical home for it: `packages/control-plane/src/memberships/__tests__/lock-ordering.test.ts` wraps the test `Querier` in a **logging proxy** that captures every SQL statement, recursively re-wrapping the in-transaction `tx` and tagging it with a tx-scoped `querierId`, so assertions can pin both the **ordering** of lock statements and the fact that they ran through the in-tx `Querier` (gripping the held client rather than a side-checked-out connection). It already pins invariant 4 of the retired invite-membership-and-presence plan for `InviteService.acceptInvite` and `MembershipService.updateMembership` this way.
...
`readRoster` asserts: exactly **one** statement, carrying **no lock clause and no transaction wrapper** — the membership predicate and the enumeration are one `READ COMMITTED` snapshot (ADR-025 D7). Semantic two-connection interleave tests remain the Tier-4 integration-suite follow-up (named, not silently skipped).
...
// State A: NODE_ID is actively attached to a session the stranger cannot see.
const hidden = await postRuntimeNode(
  "heartbeat",
  { nodeId: NODE_ID, healthState: "online" },
  STRANGER_PARTICIPANT_ID,
);
// State B: retire the seeded row, so the SAME nodeId now has no active attachment.
await querier.query(`UPDATE runtime_node_attachments SET state = 'offline' WHERE node_id = $1`, [
  NODE_ID,
]);
...
// The COMPLETE envelope — message included — must be byte-identical across the two
// states: the refusal template names only the caller-supplied nodeId (ADR-025 D3),
// and the request is the same both times, so any difference is a server-side leak.
expect(hidden.body.error.data.aisError).toEqual(absent.body.error.data.aisError);
...
- [ ] **Step 10: Classify inside each service transaction, in canonical lock order.** All three `nodeId`-keyed mutators use the **two-phase resolve** (Design §4.A decision 1). `detach` is the reference implementation; `heartbeat.ingest` follows the identical shape, and `updateCapabilities` follows it with one insertion — a level-1 `sessions … FOR SHARE` acquisition between the phase-1 pre-read and the membership lock (rule 2 below):
...
async detach(callerParticipantId: ParticipantId, request: RuntimeNodeDetachRequest): Promise<null> {
  ...
  return this.#querier.transaction(async (transaction) => {
    ...
    // It MUST NOT lock: the canonical order (I-002-6) is
    ...
    // node_id, and I-002-3 forbids MembershipUpdate from detaching as a side effect.
...
async attach(callerParticipantId: ParticipantId, request: RuntimeNodeAttachRequest): Promise<RuntimeNodeAttachResponse> {
  ...
  return this.#querier.transaction(async (transaction) => {
    // I-002-6 LEVEL-1 ACQUISITION — deadlock-load-bearing, not decorative. The upsert
    ...
    // UPDATE` first, then the membership write; invariant 4 of the retired invite-membership-and-presence plan), and FOR UPDATE conflicts
    ...
    // 2) < the upsert's attachment write (level 3) — the full I-002-6 order; it is
    // why the nodeId-keyed mutators must pre-read their attachment row UNLOCKED.
    // I-002-3 still holds: a FOR SHARE row lock reads, it does not modify
    // session_memberships.
    const membership = await transaction.query<{ one: number }>(
      `SELECT 1 AS one FROM session_memberships
        WHERE session_id = $1 AND participant_id = $2 AND state = 'active'
...
- [ ] **Step 12: Migrate the existing suites to the new signatures + superseded postures.** This is the widest blast radius in the task; enumerate each in the PR body as a ratified Spec-002 contract change (PR-A1), not regression masking. The five service methods gained a `callerParticipantId` first parameter, so **every existing call site updates** — these are build-breaking (TS + runtime), not optional:
...
- [ ] **Step 13: Move BL-141 to the archive.** Remove the BL-141 entry from `docs/backlog.md`; append to `docs/archive/backlog-archive.md` under `## Completed: <merge date>` with a closure note: exit (a)–(d) met via ADR-025 + Spec-002 rows + Plan-002 T3.10–T3.12 + enforced code; production-live automatically at the retired control-plane-relay-and-session-join plan R1 (the identity substrate). **State the two refinements explicitly** (ADR-025 §Adjudication Record + D4/D7): (i) the unauthorized roster read returns the uniform `runtimenode.permission_denied` (403) as an enumeration-scoped negative, not a per-row 403 — there is no per-row authorization on that surface (D7's honesty note) — and the same code answers an unknown session, so no session-existence oracle opens; (ii) a `detach` naming a node with no attachment visible to the caller answers the shipped idempotent `null` no-op rather than a typed error — the per-procedure uniform negative, a typed error there being itself the oracle — while a fellow member's cross-owner `detach` refuses with the same 403 as the other mutators (the decision-3 rule). Both are deliberate readings of exit (c)/(d)'s "typed authorization error" letter, ratified in ADR-025, not silent narrowings. Cites PR-A1/PR-A1p/PR-A2. _(2026-08-25: the archive move landed early, out of this vehicle, by the backlog de-duplication chore PR — BL-141 was withdrawn to `docs/archive/backlog-archive.md` as a duplicate tracking surface, its entry already stating both refinements, with criterion (d) recorded as owned by T3.10–T3.12; the step stays open only for its residual — at PR-A2 merge, append the code-landing closure note (exit (d) met, merge date, PR number) to that archive entry.)_

- [ ] **Step 14:** SPP-3/4/5/6. Test Plan in the body enumerates, by name: the IDOR cases across the three layers (service / router / HTTP-envelope oracle); the uniform-negative oracle assertions (detach's invisible-arm `null` beside its cross-owner 403; heartbeat / capabilityupdate / roster / attach the uniform 403 — per procedure); and the Step-12 superseded-posture updates (the signature migration, the attach-happy-path membership seeding, the heartbeat ingest presence-forgery rewrite, the three capabilityupdate 409→403 flips, the roster split, the client-SDK leg) — flag each as the ratified Spec-002 contract change, not regression masking. Subject `feat(control-plane): enforce runtime-node caller-ownership (plan-002)`; trailer `Refs: Plan-002, ADR-025, BL-141`. Title carries the `Plan-002` token (lane 1).

---

### Task 5: PR-B1 — Unit B docs (BL-133)

> **Executed 2026-08-11 — merged vehicle (PR #322 / §6 node NS-57).** Tasks 5 and 6 were executed together in one PR — the Plan/the retired invite-membership-and-presence spec restoring targeted readiness-audit delta — instead of the planned two-PR B1 → B1p sequence: the 2026-08-03 V1 product-vision reconciliation (PR #284) had already flipped the retired invite-membership-and-presence spec / the retired invite-membership-and-presence plan `review` before this task dispatched, so the flip steps below were moot, and the delta authored the §4.B amendments, audited them jointly with the reconciliation growth and the channel-directory ingest carrier (BL-149, outside this task's charter but the same vehicle), and restored `approved` in the same swap — Spec-019 flip-and-restored beside them. Per-step deviations are noted inline; everything else executed as written.

**Design contract:** §4.B (decisions 1–10), §3.2 ground truth.

**Files:**

- Modify: the retired invite-membership-and-presence spec (Status → `review`; §Interfaces And Contracts `invite.preview` contract + two-tier error pin + the `create` inviter-binding note (`inviter` is server-bound to the authenticated caller, never trusted from the body — Design §4.B decision 10) + a note that the five invite refusals now project onto the `aisError` envelope with their error-contracts §Invite HTTP statuses; dated Amendment block).
- Modify: `docs/architecture/contracts/api-payload-contracts.md` (add `InvitePreviewRequest`/`InvitePreviewResponse` beside the retired tier invite trio; method-name registry note — flag the doc's `Invite*Request/Response` names vs code's `InviteCreate/InviteAccept/InviteRevoke` skew, don't silently reconcile).
- Modify: `docs/specs/019-rate-limiting-policy.md` (Status → `review`; add the `invite.preview` registry row).
- Modify: `docs/specs/021-desktop-shell-and-renderer.md` (§Deep-Link Invite Flow — one-line resolution pointer to the retired invite-membership-and-presence spec's new contract; stays `approved` — clarifying cross-ref).
- Modify: `docs/architecture/cross-plan-dependencies.md` (disambiguation lines: the invite **control-plane tRPC router** is owned by the retired invite-membership-and-presence plan and ships now; the **daemon-side invite leg** — IPC handlers + live control-plane caller — is the retired control-plane-relay-and-session-join plan's Tier-4 join/relay handoff).
- Modify: the retired control-plane-relay-and-session-join plan (invariant 4 of the retired control-plane-relay-and-session-join plan gated-endpoint list gains `invite.create`/`invite.accept`/`invite.revoke` + `invite.preview`'s recorded anonymous exclusion; the Tier-4 scope bullet names the daemon invite IPC leg; the retired control-plane-relay-and-session-join plan `approved` since 2026-07-21 (W2.5 re-audit #239) — re-adjudicate under the Status Flip Rule, amendment declared in the PR body).
...
- Modify: the retired invite-membership-and-presence plan (Status → `review`; new Phase-2 tasks T2.6/T2.7/T2.8; new **cross-plan row 9 of the retired invite-membership-and-presence plan** §Cross-Plan Obligations entry; re-open audit checkbox scoped to the new tasks; §Progress Log note).
- Modify: `README.md` (census: the retired invite-membership-and-presence plan + the retired invite-membership-and-presence spec + Spec-019 → `review`; re-derive counts).
...
- [x] **Step 2:** Author the the retired invite-membership-and-presence spec §Interfaces `invite.preview` contract: request `{ token }`; response `{ joinMode, expiresAt, sessionName: string | null, inviterDisplayName: string | null }` (raw UUIDs excluded; null fields documented Plan-016-fed / session-name-fed) — _deviation (2026-08-11, PR #322 Codex round 1): the landed response additionally carries `sessionId`; the Spec-021 confirmation step needs a target-session identity and V1 has no session-name producer, so an identifier-free response was unrenderable — the inviter id stays excluded_; explicitly non-consuming (idempotent — the single-use token survives); row-first-then-expiry error order matching accept (Design §4.B decision 1); two-tier errors reusing the §Invite vocabulary (uniform `invite.not_found` for garbage/unknown; distinct expired/revoked/already*accepted for MAC-valid). Add the `create` inviter-binding note: `InviteCreate.inviter` is authorization-checked against the resolved authenticated caller and never trusted from the body — the router passes `resolveCurrentParticipantId(ctx)` as the actor argument (Design §4.B decision 10). Add the note that the five invite refusals now project onto `aisError` (the migration to `AisWireException`) with their §Invite HTTP statuses, expired/revoked at 410 via the override map (Design §4.B decision 8). Dated Amendment block; flip Status `review`. *(2026-08-11: the flip was moot — the retired invite-membership-and-presence spec already `review` since PR #284; the delta authored the amendment and restored `approved` in the same swap.)\_
- [x] **Step 3:** Add the api-payload-contracts `InvitePreviewRequest`/`InvitePreviewResponse` interfaces beside the existing invite trio (Design §4.B decision 3 shapes); note the `invite.preview` method under the already-reserved `invite` namespace root, **recorded POST-only (tRPC mutation kind, deliberately — non-consuming read whose input is the raw token; a query would ride GET `?input=` per the client convention and leak tokens into logs/history; Design §4.B decision 3)**. Add the error-contracts §Error Response Shape **mechanism note** (Design §4.B decision 8, fourth bullet): registry codes whose pinned HTTP status has no native tRPC code are delivered via `AIS_WIRE_HTTP_STATUS_OVERRIDES` (`@ai-sidekicks/contracts` `error.ts`) — the `errorFormatter` stamps `data.httpStatus`, and the transport lifts `response.status` from that stamp **natively** (tRPC's `getHTTPStatusCode` reads each response's `error.data.httpStatus`; a uniform batch gets that status, a mixed batch gets 207 — no `responseMeta` is registered), unmapped codes ride their carrier code's native status; name the future registrants (`artifact.relay_expired` with Plan-012 Tasks 7–10; `approval.request_expired` only if it gains a control-plane tRPC surface — its V1 surface is daemon JSON-RPC where §JSON-RPC Wire Mapping governs).
- [x] **Step 4:** Add the Spec-019 registry row (Design §4.B decision 5 values); flip Status `review`. Re-derive the registry row count. _(2026-08-11: executed as a flip-and-restore in the same swap — registry 24 → 25, with one flagged widening beyond decision 5's literal: `invite.redeem_ip` stacks on the preview path too, keeping preview at-least-as-strict-as-accept on both axes, recorded in Spec-019 as the one deliberate exception to the most-specific-row rule.)_
- [x] **Step 5:** Add the Spec-021 §Deep-Link resolution pointer (declare in the PR body: clarifying cross-ref, Spec-021 stays `approved` per the runbook default rule). Add the cross-plan-dependencies disambiguation line (declare: runbook:218 re-audit trigger does not fire — clarification, not an ownership change). Add the **the retired control-plane-relay-and-session-join plan edits** (Design §4.B decisions 6/9): the invariant 4 of the retired control-plane-relay-and-session-join plan gated-endpoint list gains `invite.create`/`invite.accept`/`invite.revoke` (the §4.A decision-9 mirror — the three authenticated procedures PR-B2 makes reachable through the merged host), with `invite.preview` recorded in the invariant text as **deliberately excluded** (anonymous by design per §4.B decision 1; token entropy + the Spec-019 anonymous per-token-hash row carry its protection); widen the Tier-4 scope bullet ("Invite-acceptance handoff") to name the daemon-side invite IPC leg + live control-plane caller (decision 6 surface (c)). Declare in the PR body: invariant-list completion + scope naming, the retired control-plane-relay-and-session-join plan `approved` since 2026-07-21 (W2.5 re-audit #239) so the amendment carries its own Status Flip Rule adjudication — presumptively a flip (the invite triple extends invariant 4 of the retired control-plane-relay-and-session-join plan's covered-endpoint set; flip → targeted re-audit → re-promotion, the PR's user pause final); plus the observation for the retired control-plane-relay-and-session-join plan's next readiness re-audit that the Tier-1 session-directory procedures' Tier-4 gating disposition rides invariant 1 of the retired control-plane-relay-and-session-join plan's allow-list-widening obligation (pre-existing, not adjudicated by this campaign). _(2026-08-11: this step's "the retired control-plane-relay-and-session-join plan `approved` since 2026-07-21" premise was stale — the retired control-plane-relay-and-session-join plan flipped `review` 2026-08-03 with the reconciliation bundle, so the Status Flip Rule adjudication resolved differently than written: the invariant 4 of the retired control-plane-relay-and-session-join plan + Tier-4-scope edits land as growth on the already-open flip, audited by the retired control-plane-relay-and-session-join plan's own queued restoring delta (its audit box's owed scope widened to name them), with no flip performed by this vehicle — the superseded-instruction disposition recorded in the retired invite-membership-and-presence plan's audit box.)_
- [x] **Step 6:** Add the retired invite-membership-and-presence plan Phase-2 tasks (acceptance criteria tracing to the the retired invite-membership-and-presence spec rows), matching PR-B2's task decomposition (Design §4.B doc deliverables): **T2.6** (relocate `INVITE_*_CODE` to `contracts/src/error.ts` + the `AIS_WIRE_HTTP_STATUS_OVERRIDES` map + the `InvitePreview{Request,Response}` schemas + `previewInvite`), **T2.7** (migrate the five invite exceptions to `AisWireException` + `invite-router.factory.ts` + host merge + the `errorFormatter` 410 stamp — transport status lifts natively from `data.httpStatus`, no `responseMeta`), **T2.8** (test suite incl. the HTTP-layer envelope oracle + the mixed-batch 207 control). Append **cross-plan row 9 of the retired invite-membership-and-presence plan** to §Cross-Plan Obligations (the established cross-plan row 1 of the retired invite-membership-and-presence plan..8 pattern): the daemon-side invite leg — the `invite.*` IPC handlers under `packages/runtime-daemon/src/ipc/handlers/`, the live control-plane caller they delegate to, and `membershipClient.preview` — is consumed from the retired relay-plan remainder Tier 4 (join/relay handoff); the retired invite-membership-and-presence plan's T2.6–T2.8 own the control-plane tRPC transport only; the renderer's `NotImplementedAtTier1Error` posture stands until that leg lands; cross-ref the cross-plan-dependencies disambiguation lines. The daemon IPC handlers + live control-plane caller are **not** The retired invite-membership-and-presence plan tasks here — they are the retired control-plane-relay-and-session-join plan Tier-4. Flip Status `review`; re-open the audit checkbox scoped to the new tasks; fold audit-delta evidence into a §Progress Log note. _(2026-08-11: the flip/re-open halves were moot — both already open since PR #284; the delta re-checked the audit box with the three-growth-body scope enumeration and the restore record instead.)_
- [x] **Step 7:** README census (the retired invite-membership-and-presence plan + the retired invite-membership-and-presence spec + Spec-019 → `review`; counts re-derived). Merge-serialize vs PR-A1 (rebase census lines if PR-A1 landed first). _(2026-08-11: executed as the restore-side census — plans 21 `approved` / 5 `review`, specs 25 `approved` / 3 `review` — since flip and restore landed in one vehicle; no PR-A1 serialization arose.)_
- [x] **Step 8:** SPP-3 docs gates + `/ripple-check`. SPP-4 with `plan-execution-spec-reviewer`. SPP-5/6 — subject `docs(repo): the retired invite-membership-and-presence spec non-consuming invite.preview + wire contract`, body declares the three `review` flips + the two clarifying-only edits + the the retired control-plane-relay-and-session-join plan amendment (already `review`) + the error-contracts mechanism note + census ripple; trailer `Refs: the retired invite-membership-and-presence spec, Spec-019, Spec-021, the retired invite-membership-and-presence plan, the retired control-plane-relay-and-session-join plan, BL-133`. _(2026-08-11: shipped under the merged vehicle's subject `docs(repo): restore Spec/the retired invite-membership-and-presence plan approved via membership targeted delta`, its body carrying this task's declarations.)_

---

### Task 6: PR-B1p — the retired invite-membership-and-presence spec + Spec-019 + the retired invite-membership-and-presence plan re-promotion

> **Executed 2026-08-11 — merged into Task 5's vehicle (PR #322 / §6 node NS-57).** The separate promotion PR dissolved: the restoring targeted readiness-audit delta is itself the delivery-and-promotion vehicle (the Plan-005 PR #282 flip-and-restore-in-one-swap shape), so every step below executed inside PR #322.

**Files:** Modify: the retired invite-membership-and-presence spec + `docs/specs/019-…` + the retired invite-membership-and-presence plan (`review` → `approved`; tick the retired invite-membership-and-presence plan's re-opened audit checkbox); `README.md` (census restore).

- [x] **Step 1:** SPP-1 name=`b1p-invite-promote`, branch=`docs/bl-133-promote`. _(2026-08-11: no separate branch — merged vehicle.)_
- [x] **Step 2:** Flip all three to `approved`; tick the audit checkbox. PR body cites runbook §Spec-Status Promotion Gate by name + PR-B1 delta evidence (T2.6–T2.8 trace to the amended the retired invite-membership-and-presence spec rows; dep-closure terminal; no gating open questions). For the the retired invite-membership-and-presence plan half, also satisfy the **plan-side** §Status Promotion Gate elements (runbook:210–214): cite the retired tier audit-completion date + the `plan-readiness-audit-tier-2-complete` tag.
- [x] **Step 3:** Restore README census.
- [x] **Step 4:** SPP-3/4/5/6 — subject `docs(repo): re-promote the retired invite-membership-and-presence spec + spec-019 + the retired invite-membership-and-presence plan (invite delta)` (68 chars — `commitlint` `header-max-length` is 72; do not re-expand to `invite.preview delta`), trailer `Refs: the retired invite-membership-and-presence spec, Spec-019, the retired invite-membership-and-presence plan, BL-133`. _(2026-08-11: shipped under the merged vehicle's subject and trailer.)_

---

### Task 7: PR-B2 — Unit B code + BL-133 archive (lane 1 under the retired invite-membership-and-presence plan)
...
**Deliberately NOT in this task (Design §4.B decisions 4/6 — the retired control-plane-relay-and-session-join plan Tier-4 relay leg):** the daemon IPC invite handlers (`packages/runtime-daemon/src/ipc/handlers/invite-*.ts`) and `membershipClient.preview`. No daemon invite handler exists today and the daemon has no live control-plane caller; building a handler now only relocates the renderer's `NotImplementedAtTier1Error` to a handler delegating to a deferred caller (no usable capability, plus renderer-test churn). They land with the live caller at Tier-4.
...
- [ ] **Step 1:** SPP-1 name=`b2-invite-code`, branch=`feat/plan-invite-membership-invite-preview`.
...
async previewInvite(request: InvitePreviewRequest): Promise<InvitePreviewResponse> {
  ...
  if (isExpiredClaim(claims.expires_at, now)) {
    throw new InviteExpiredException(`InviteService.previewInvite: invite expired at ${claims.expires_at} (the retired invite-membership-and-presence spec §Token Security Properties).`);
  }
  // (6) State ladder in acceptInvite's order: revoked -> expired -> accepted.
  if (row.state === "revoked") throw new InviteRevokedException("InviteService.previewInvite: invite has been revoked (the retired invite-membership-and-presence spec §Invite Revocation).");
  ...
  if (row.state === "expired") throw new InviteExpiredException("InviteService.previewInvite: invite is in a persisted expired state (the retired invite-membership-and-presence spec §Token Security Properties).");
  if (row.state === "accepted") throw new InviteAlreadyAcceptedException("InviteService.previewInvite: invite has already been accepted (the retired invite-membership-and-presence spec §Token Security Properties).");
  // pending + non-expired: bounded metadata; display fields null until Plan-016 (display_name) / a session-name owner.
...
- [ ] **Step 11: Move BL-133 to the archive.** Remove from `backlog.md`; append to the archive with the closure note: exit (a) **met** (the the retired invite-membership-and-presence spec contract, exceeded — full invite router + hardened `aisError`/410 envelope, not just the preview contract); exit (b) **met-as-refined** — the endpoint ships on the **control-plane tRPC transport**; its daemon-as-gateway clause (ADR-008 surface, mirroring the `invite.accept` wire registration) is reassigned to **the retired control-plane-relay-and-session-join plan Tier-4** via the retired invite-membership-and-presence plan **cross-plan row 9 of the retired invite-membership-and-presence plan** (an obligation entry; the implementation task lands with the retired control-plane-relay-and-session-join plan's Tier-4 leg). Name the **two** remaining hand-offs, each another plan's own step: the daemon-integration leg (IPC handlers + live control-plane caller) is **the retired control-plane-relay-and-session-join plan Tier-4**; exit (c) (Plan-021 Tier-7 deep-link consumption + two-client smoke) is **Plan-021 Tier-7**. Cites PR-B1/PR-B1p/PR-B2.

- [ ] **Step 12:** SPP-3/4/5/6. Test Plan enumerates the non-consumption invariant, two-tier uniformity (tampered vs unknown-hash → `invite.not_found`), per-lifecycle-state preview, the HTTP-layer 410 envelope oracle, the mixed-batch 207 control, the preview POST-only kind pin (GET refused), and the now-reachable create/accept/revoke wire path. Subject `feat(control-plane): invite.preview + invite wire transport (the retired invite-membership-and-presence plan)` (70 chars — `commitlint` `header-max-length` is 72, so "non-consuming" lives in the body, not the subject); body declares the non-consumption invariant, the the retired control-plane-relay-and-session-join plan Tier-4 daemon-leg deferral + the cross-plan `trpc.ts`/`host.ts` edits (map-guarded, zero shipped-behavior change). Trailer `Refs: the retired invite-membership-and-presence plan, the retired invite-membership-and-presence spec, BL-133`. Title carries `the retired invite-membership-and-presence plan` (lane 1).

---

### Task 8: PR-C — Unit C tests + BL-131 rewrite + BL-134 note (lane 2)

> **BL-134 leg closed 2026-08-26 — premise-void, never executed, no successor.** The third clause of this task (the `+ BL-134 note` in the heading and DAG row, the "annotate BL-134 dormancy" Files bullet, the second half of Step 8, the `BL-134` token in Step 10's drafted `Refs:` trailer, and the `annotate BL-134` clause in the §File-structure ledger) is retired rather than done. PR #356 ratified `clipanion@4.0.0-rc.4` as a **terminal** pin rather than a provisional one — folding the audited rc.4 defects into `Plan-006 §Phase R3 — Client Delivery (Tier 3)` as normative task text (§6 node NS-78) — and archived BL-134 outright as premise-void with five named re-evaluation triggers ([`backlog-archive.md` §BL-134](../../archive/backlog-archive.md#bl-134-clipanion-stable-v4-upgrade--lockfile-bump-for-plan-006-r3-pr-a-cli)). There is no live entry left to annotate and no "within one week of stable" trigger left to recalibrate: the watch surface this task assumed (an upstream stable-v4 tag) is the very premise the archive record falsifies. Step 8's checkbox stays **unticked** deliberately — its BL-131 half was executed, its BL-134 half never will be, and a tick would assert both. The design's counterpart surfaces are swept in the same diff: its §4.C `Ride-along` clause, its §5 PR-C row, and its §6 end-state cell for BL-134 (whose archive-action cell PR #356 corrected on its own, as it did §1.2's premise bullet) — so Task 11 Step 1's per-BL sweep, which reads Design §6 end states as its oracle, no longer reports a dormancy annotation this campaign owes.
>
...
- Modify: `docs/plans/002-runtime-node-attach.md` (all **seven** BL-131 mentions — the four T5 Test-field notes + two §Verification restatements record the component-test half shipped; line 847's Shipment-Manifest note narrows "component/E2E" → "E2E"; note-level, no status flip).
...
- Modify: `pnpm-workspace.yaml` (catalog `vitest: ^4.1.5` → exact `4.1.5`; add `@vitest/coverage-v8: 4.1.5`).
- Modify: each vitest-workspace `package.json` (add `@vitest/coverage-v8: catalog:testing` devDep + a `test:coverage` script) + `vitest.config.ts` (consume the shared coverage factory; desktop scopes to the `renderer` project).
- Create: `vitest.shared.ts` (repo-root `sharedCoverageOptions()` factory; `tools/docs-corpus` mirrors it inline) + `tools/coverage-report.mjs` (job-summary renderer).
- Modify: `.github/workflows/ci.yml` (new advisory `coverage` job — non-required; `test:coverage`; `actions/upload-artifact`; first-party job-summary table).
- Modify: `turbo.json` + `apps/desktop/turbo.json` (a `test:coverage` task, `"cache": false`).
...
**Interfaces:** Consumes the workspace catalog + per-package vitest configs. Produces per-package `coverage/` artifacts + a job-summary surface.

- [x] **Step 1:** SPP-1 name=`tooling-decision-rows`, branch=`chore/coverage-turbo-cache-signing-rows` (NON-plan-scoped — lane 3). **Deviation from the drafted `chore/coverage-measurement-substrate`:** Tasks 9 and 10 plus the BL-108/112/124 corpus corrections ship as one PR, because all three edit `docs/decisions/023-v1-ci-cd-and-release-automation.md` §Decision Log and `docs/backlog.md`, and three concurrent branches over the same two files buy nothing but conflicts.
- [x] **Step 2:** Pin the `testing` catalog to exact `4.1.5` + add `@vitest/coverage-v8: 4.1.5`; `pnpm install`; `pnpm-lock.yaml` resolves both at `4.1.5` (peer-pin satisfied). **Correction to the drafted `4.1.10`:** the pin target is the version already resolved in the lockfile, not a bump. `@vitest/coverage-v8` declares an EXACT peer on its runner (`peerDependencies: { vitest: "4.1.5" }`, verified 2026-08-25 against the published manifest), so the two must move together — but moving the runner forward is a change to how every test in the repo executes, and it does not belong inside the PR that first turns coverage on. `latest` is 4.1.11 as of 2026-08-25; a bump is its own PR that edits both catalog lines.
- [x] **Step 3:** Author the repo-root `vitest.shared.ts` factory and consume it from six of the seven `vitest.config.ts` files — `tools/docs-corpus` mirrors the block inline, because `tsc -p tools/docs-corpus` pins `rootDir` to that directory and an import from above it fails the enforced `docs-corpus-typecheck` screen (TS6059); the mirror is annotated as hand-kept; add the coverage devDep + a `test:coverage` script to each package `package.json`. **Correction to the drafted per-config `coverage` block:** the options are defined once in the factory, because Vitest 4 resolves `coverage` root-only once `projects` exist and an inline per-config block would drift seven ways. **`coverage.include` is mandatory and is spelled out at every call site** — Vitest 4 removed `coverage.all` and inverted the denominator to "files imported during the run". Measured on this repo 2026-08-25 in `packages/contracts` with one deliberately unreferenced source file present: explicit include 26 files / 92.93% lines; v4 default 25 files / 93.46% — the untested file is invisible under the default, so a baseline collected that way cannot support a floor. `src/migrations/**` is excluded (inline-SQL DDL literals). Desktop scopes coverage to `src/renderer/**` only: its `main` project spawns a real Electron binary, which v8 cannot instrument from this process.
- [x] **Step 4:** Verified locally — `pnpm turbo run test:coverage` over the six workspace packages plus `pnpm exec vitest run --root tools/docs-corpus --coverage`, all green, each emitting `coverage/coverage-summary.json` + `lcov.info`. Denominator check (the v4 trap): the report's file count was compared against an independent `find`-derived source count per package and matched exactly at all seven — 6 / 25 / 24 / 7 / 60 / 11 / 17. The required `pnpm test` path is unchanged. Desktop coverage is invoked as `vitest run --project=renderer --coverage`, never through its nested `test` script.
- [x] **Step 5:** Added the advisory `coverage` job to `ci.yml`, absent from `ci-gate`'s `needs` (verified: still `[test-node22, test-node24, hook-tests-macos]`). **Correction to the drafted matrix + third-party reporter.** (a) One job, not a matrix mirroring the required Node split across legs: `tools/coverage-report.mjs` is fail-closed on a root that declares `test:coverage` and produced no report, and that check only carries signal if every root is measured in one workspace. The Node tier split is preserved by swapping the runtime mid-job (a second `actions/setup-node` before the control-plane step) — only `@ai-sidekicks/control-plane` runs on Node 24 and it is WASM-backed (PGlite), so no native binding crosses an ABI. `tools/docs-corpus` is invoked directly; it has no package.json and turbo cannot fan out to it. (b) `davelosert/vitest-coverage-report-action` is NOT used. It updates one marker-keyed PR comment, which two concurrent legs would race on; its threshold-icon feature regex-scans a package's vitest config, which finds nothing when the options live in a shared factory and Stage 1 sets no thresholds; and it needs the first `pull-requests: write` grant in the workflow that carries the required `ci-gate`. The shipped surface is a first-party `$GITHUB_STEP_SUMMARY` table plus `actions/upload-artifact` (`if-no-files-found: error`), which keeps the job at `contents: read`. This is the zero-spend half of the ratified choice between a free-tier coverage service and the artifact path; inline PR-comment rendering is the named upgrade if the summary tab proves too far from where reviewers look.
- [x] **Step 6:** Fixed the `failure-modes.md` Phase-A `NEEDS_CONTEXT` pointer (ride-along) — "CLAUDE.md doc-first discipline" → "AGENTS.md §Doc-First Discipline".
- [x] **Step 7:** Updated BL-123: Status `in_progress`; the >=5-PR baseline protocol and durable artifact pointers recorded; exit (a) rewritten from "installed in root `package.json` devDependencies" to the pnpm-workspace catalog mechanism this PR actually ships. ADR-023 §Decision Log entry added recording the advisory coverage job's design, the exact peer pin, the mandatory explicit `include`, the `"cache": false` measurement rule, and the deferral of thresholds to Stage 2.
- [x] **Step 8:** `pnpm turbo run test:coverage` (all green), repo-wide `prettier --write`, `eslint .`, docs-corpus pre-commit runner over the changed `.md`. Subject `chore(ci): wire vitest v8 coverage measurement substrate`; body declares lane 3 + the advisory-job design; `Refs: BL-123, ADR-023`. No plan token.
...
- Modify: `.github/workflows/ci.yml` (SHA-pinned `rharkor/caching-for-turbo` step in both test jobs; `TURBO_REMOTE_CACHE_SIGNATURE_KEY` at job-level `env:` from secret; `--cache=local:rw,remote:r` appended to **all eight** turbo invocations in the two cache-enabled jobs on `pull_request` — the advisory `coverage` job's two carry no switch and need none, having no caching action and no `TURBO_API`; job-scoped `permissions: { contents: read, actions: write }`). **Corrected 2026-08-25:** the `actions: write` half is retracted — `@actions/cache` authenticates with the runner-injected `ACTIONS_RUNTIME_TOKEN`, not `GITHUB_TOKEN`, so the grant bought the cache nothing while giving PR-authored code repo-wide Actions-API writes. Both jobs ship `contents: read` alone.
...
**Interfaces:** Consumes the GitHub Actions cache + `TURBO_REMOTE_CACHE_SIGNATURE_KEY`. Produces a warm-cache CI path measurable against the recorded cache-cold baseline.

- [x] **Step 1:** SPP-1 name=`tooling-decision-rows`, branch=`chore/coverage-turbo-cache-signing-rows` (NON-plan-scoped — lane 3). Ships with Tasks 9 and the BL-108/112/124 corrections; see Task 9 Step 1 for why the three units share one branch.
- [x] **Step 2:** Added `"remoteCache": { "signature": true }` to `turbo.json`. **Correction — the blocking gap this step alone leaves open.** With the cache on and `test` cacheable, a docs-only PR (the shape of all eight PRs in the measured sample) replays the previous test log instead of executing the suite. That is not a wall-clock win; it is the suite not running, and it is exactly what `ADR-023 §Failure Mode Analysis` F-8 pays 2-3 min per OS leg to prevent. Per the user-ratified scoping, the cached tasks are `build` / `typecheck` / `lint`, and **every task that executes a test carries `"cache": false`** — root `test` and `test:coverage`, plus the `test:renderer` / `test:smoke` legs in `apps/desktop/turbo.json` that the desktop `test` task delegates to (its own `"cache": false` does not reach them). Scoping the remote grant instead was rejected: `--cache=local:rw,remote:r` still leaves the local cache free to replay, and leaves `push` runs replaying too.
- [x] **Step 3:** Added the `rharkor/caching-for-turbo@2238fae6eb9a9936f92356f54cb3660200d105e7` (v2.5.1, SHA-pinned) step immediately after `Install dependencies` in both `test-node22` and `test-node24`; added job-scoped `permissions: { contents: read, actions: write }` and the job-level `env:` key to each. **Corrected 2026-08-25:** the `actions: write` half is retracted — `@actions/cache` authenticates with the runner-injected `ACTIONS_RUNTIME_TOKEN`, not `GITHUB_TOKEN`, so the grant bought the cache nothing while giving PR-authored code repo-wide Actions-API writes. Both jobs ship `contents: read` alone. **Navigate by job name + step name, not by line number** — the line cites this step carried (`ci.yml:43` / `:225` for the jobs, `:109`/`:147`/`:150`/`:153` and `:255`/`:258`/`:261`/`:264` for the invocations) had already drifted by 22 lines before execution and are removed rather than renumbered. **The read-only switch rides every turbo invocation, not only `test`:** four per job (`build`, `typecheck`, `lint`, `test`), eight in all, each appending `${{ github.event_name == 'pull_request' && '--cache=local:rw,remote:r' || '' }}`. Flagging only `test` would leave build / typecheck / lint writable from a fork.

  **Use the per-invocation `--cache` flag, not the job-level `TURBO_REMOTE_CACHE_READ_ONLY` env var.** The env var (<https://turborepo.dev/docs/reference/system-environment-variables>, documented as "Prevent writing to the Remote Cache - but still allow reading", verified 2026-07-10) is tempting because one job-level line would cover all four steps at once — but its parse of a _falsy_ string is undocumented, and that gap is a silent-failure trap. GitHub Actions renders `${{ github.event_name == 'pull_request' }}` as the string `"false"` on `push`; if Turborepo treats any non-empty value as truthy, `TURBO_REMOTE_CACHE_READ_ONLY=false` keeps the cache read-only on `push` too and disables **every** cache upload — a permanently cold cache with no error surfaced, which defeats the whole experiment. The `--cache` flag carries no such ambiguity: the conditional makes it either present (PR) or entirely absent (push), a binary with no string-truthiness question.
  ...
  **Declare `TURBO_REMOTE_CACHE_SIGNATURE_KEY` at `env:` on the JOB, not on the caching-action step.** GitHub Actions step-level `env:` is scoped to that one step and does not propagate forward, whereas the key is consumed later, by the `pnpm turbo run …` steps — Turborepo signs each uploaded artifact "using the value of the environment variable `TURBO_REMOTE_CACHE_SIGNATURE_KEY`" at run time (<https://turborepo.dev/docs/reference/configuration>, verified 2026-07-10). Putting it on the action step silently disables signing. Note also that a job-level `permissions:` map **replaces** the workflow-level one, so `contents: read` must be restated beside `actions: write` or the job's own `actions/checkout` loses its grant. **Corrected 2026-08-25:** the `actions: write` half is retracted — `@actions/cache` authenticates with the runner-injected `ACTIONS_RUNTIME_TOKEN`, not `GITHUB_TOKEN`, so the grant bought the cache nothing while giving PR-authored code repo-wide Actions-API writes. Both jobs ship `contents: read` alone.

  **`futureFlags.longerSignatureKey` is deliberately NOT set.** Measured on the pinned turbo 2.9.6, an empty key with `TURBO_API` set exits 1 under that flag — precisely the fork-PR and pre-secret state, since GitHub interpolates a withheld secret to `""` and the caching action always sets `TURBO_API`. Plain `remoteCache.signature: true` was re-verified on this repo 2026-08-25 under the same empty-key conditions: turbo warns (`TURBO_REMOTE_CACHE_SIGNATURE_KEY is too short (0 bytes) … This will become a fatal error in the next major version`) and **exits 0**, degrading to the local cache. 2.9.6's own warning names turbo 3.x as where that degradation expires; the expiry is recorded in `ADR-023 §Re-Evaluation Triggers` rather than left in this file.

- [x] **Step 4:** Fixed BL-122: References **and** the Summary sentence (§Axis 2 → §Axis 1 — the cache text lives in §Axis 1; both forms swept, then re-grepped repo-wide for survivors); exit (a) re-pointed from `TURBO_TOKEN` (Vercel Remote Cache's bearer auth, which this design has no use for) to the HMAC signature key that is actually the only secret in play; the cache-cold baseline persisted onto the BL as **282 s** with its method (N=8, measured 2026-08-25 over the eight most recent merged PRs — the 218-223 s figure that lived only in these campaign records is superseded); measurement window recorded (≥5 PRs; below 30% → revert + record the negative result). ADR-023 §Decision Log entry added covering the provider adjudication (rharkor as the $0 GitHub-native option. **Corrected 2026-08-25 during the closure pass, before merge:** this step originally read that ducktors "fails the $0 constraint" of **ADR-022's** "or equivalent" latitude. ADR-022 states no cost constraint — `$0` appears nowhere in it, and "or equivalent" attaches to the project name in the parenthetical. The real departure is from ADR-022's **self-hosting** commitment, and ADR-022 is amended in the same PR to record it), the "deployed in our infrastructure" overstatement corrected to decided-now-implemented, the never-cache-a-test rule, and the F-8 mechanism correction.
- [x] **Step 5:** Repo secret set — `openssl rand -hex 32 | gh secret set TURBO_REMOTE_CACHE_SIGNATURE_KEY` (32 random bytes hex-encoded; stdin keeps the value out of argv), confirmed present via `gh secret list`. Then SPP-3 / SPP-4 / SPP-5 / SPP-6. Subject `chore(ci): turbo remote-cache experiment via github actions cache`; body declares lane 3 + the fork-poisoning threat model + the keep-or-revert exit; `Refs: BL-122, ADR-023`. No plan token. **The threat model is stated honestly:** the controls that defend against fork cache-poisoning are GitHub withholding secrets from fork-originated workflows, the read-only `GITHUB_TOKEN` on `pull_request`, and `--cache=local:rw,remote:r` on PR events. The HMAC is **not** one of them — Turborepo documents `remoteCache.signature` as artifact **integrity** verification and states it "is not a security feature". `ADR-023 §Axis 1 — CI Workflow Architecture`'s "HMAC ≥32 bytes" requirement is still met; only the rationale is corrected.
- [ ] **Step 6: Measurement follow-up (post-merge, recorded on BL-122):** after ≥5 PRs, compare warm-cache `ci-gate` medians to the recorded 282 s cache-cold baseline; ≥30% reduction → keep + archive BL-122 with the data; below 30% → revert PR-E's wiring and record the negative result. **Known limit to carry into the comparison:** the OS is not part of Turborepo's cache key. Harmless while the two cacheable jobs cover disjoint packages on one runner image; the day `ADR-023 §Axis 1 — CI Workflow Architecture` adds macOS or Windows test legs, a per-OS `globalEnv` discriminator must land with them or a Linux result counts as a pass elsewhere. **Observed 2026-08-25:** 79% of a cache-cold run is the control-plane test step (in-process WASM Postgres re-running migrations per case) — and that step is now deliberately never cached, so it is the real wall-clock lever if CI ever feels slow, not something this experiment can move.
...
- [ ] **Step 2:** Status census: the retired invite-membership-and-presence spec, Spec-002 and Spec-019 + the retired invite-membership-and-presence plan and Plan-002 back at `approved`; README counts re-derived by counting; cross-plan-dependencies §6 coherent.
- [ ] **Step 3:** Contract-surface coherence: error-contracts §Runtime Node ↔ `packages/contracts` codes ↔ ADR-025 enumerate the same set; api-payload invite quartet ↔ `invite-router.factory.ts` ↔ the retired invite-membership-and-presence spec agree; `AIS_WIRE_HTTP_STATUS_OVERRIDES` keys/values ↔ error-contracts §Invite 410 rows ↔ the §Error Response Shape mechanism note agree; Spec-019 registry count re-derived. **Gated-endpoint completeness (Design §7.3):** The retired control-plane-relay-and-session-join plan invariant 4 of the retired control-plane-relay-and-session-join plan's list covers every authenticated procedure the campaign made reachable through the merged host (the five runtimenode + invite create/accept/revoke), with `invite.preview`'s anonymous exclusion recorded in the invariant text — no campaign-shipped procedure silently absent.
...
| Plans | — | `002` (tasks + cross-plan row 9 of the retired invite-membership-and-presence plan + reflip), `003` (tasks + reflip), `008` (invariant 4 of the retired control-plane-relay-and-session-join plan runtimenode via Task 2 + invite via Task 5; Tier-4 scope bullet) |
...
| runtime-daemon | — (invite IPC handlers deferred to the retired control-plane-relay-and-session-join plan Tier-4, Design §4.B decision 6) | — |
| client-sdk | — | `test/runtimeNodeClient.integration.test.ts` + `src/runtimeNodeClient.ts` comment sweep (Unit A roster-membership ripple; `membershipClient.preview` is the retired control-plane-relay-and-session-join plan Tier-4) |
...
- **Wiring completeness (anti-orphaning):** every net-new mechanism has a durable governance home a future implementer will actually read, and every deferral has a named owner recorded inside the owning plan — nothing ships discoverable only from this campaign doc. The map: runtime-node authz (ADR-025 + Spec-002 + error-contracts §Runtime Node + Plan-002 T3.10–12); attach membership guard (same set); `invite.preview` (the retired invite-membership-and-presence spec + api-payload-contracts + Spec-019 row + the retired invite-membership-and-presence plan T2.6); invite router + host merge (the retired invite-membership-and-presence spec + the retired invite-membership-and-presence plan T2.7 + cross-plan-dependencies + invariant 4 of the retired control-plane-relay-and-session-join plan, which gains invite create/accept/revoke so Tier-4 PASETO gating cannot skip them — preview's anonymous exclusion recorded); `AisWireException` migration (the retired invite-membership-and-presence spec note + the retired invite-membership-and-presence plan T2.7); `AIS_WIRE_HTTP_STATUS_OVERRIDES` + 410 delivery (error-contracts §Error Response Shape mechanism note naming the future registrants `artifact.relay_expired` / `approval.request_expired` + the retired invite-membership-and-presence plan tasks); coverage substrate (ADR-023 §Decision Log entry + BL-123 protocol); remote cache (ADR-023 §Decision Log entry + BL-122); renderer component tests (Plan-002's seven BL-131 mention flips); the daemon invite leg deferral (the retired invite-membership-and-presence plan **cross-plan row 9 of the retired invite-membership-and-presence plan** + the retired control-plane-relay-and-session-join plan Tier-4 scope bullet + cross-plan-dependencies lines — three surfaces, decision 6). Verified at closure by Task 11 Steps 1–3.
