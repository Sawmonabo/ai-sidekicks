...
- Modify: `docs/plans/008-control-plane-relay-and-session-join.md` (I-008-4 endpoint list adds the five runtimenode procedures; Plan-008 promoted `approved` 2026-07-21 (W2.5 re-audit #NNN) — re-adjudicate the amendment under the Status Flip Rule in the PR, expected stays-`approved` as defect-repair).
...
- [ ] **Step 3:** Restore **both** README §Project Status censuses — per census, never one delta applied to both: the **plan** bullet returns Plan-003 to `approved` (`19 approved` → 20, `7 … in review` → 6, Plan-003 dropped from the named list) and the **spec** bullet returns Spec-003 to `approved` (`20 approved` → 21, `7 … in review` → 6, Spec-003 dropped from the named list). Counts re-derived by counting.
...
- Modify: `docs/plans/008-control-plane-relay-and-session-join.md` (I-008-4 gated-endpoint list gains `invite.create`/`invite.accept`/`invite.revoke` + `invite.preview`'s recorded anonymous exclusion; the Tier-5 scope bullet names the daemon invite IPC leg; Plan-008 `approved` since 2026-07-21 (W2.5 re-audit #NNN) — re-adjudicate under the Status Flip Rule, amendment declared in the PR body).
