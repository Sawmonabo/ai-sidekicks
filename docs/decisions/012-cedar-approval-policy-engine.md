...
The daemon pins `@cedar-policy/cedar-wasm` at Cedar **v4.11** from **V1** (current stable as of 2026-06; verified against the npm registry at the Tier-5 audit; the resident evaluator ships at V1 per the 2026-07-02 amendment, and Plan-010 D-010-9 pins 4.11.x on the embedded set). Policy bundles (V1.1+) declare their target Cedar version in the manifest. Daemons reject bundles whose target version does not match the daemon's compiled cedar-wasm version. Cedar major-version upgrades are a coordinated daemon-image upgrade event and require an ADR-012 amendment.
