...
| 19 | Session time-travel (run rollback) | [Spec-004](../specs/004-queue-steer-pause-resume.md) (`rollback` intervention on the generic dispatch — to land via the campaign's B2 bundle, a named merge prerequisite before Plan-004 implements rollback; not yet in the authoritative spec), [ADR-017](./017-shared-event-sourcing-scope.md) (forward `run.rolled_back` event; the log never truncates — registered by the campaign's Spec-006 B1 amendment (merged in-tree via PR #173; the residual gate is Spec-006's `review → approved` status promotion — the W1.5 batch re-promotion gate, campaign Task 28)), [Spec-010](../specs/010-worktree-lifecycle-and-execution-modes.md) + [Plan-010](../plans/010-worktree-lifecycle-and-execution-modes.md) (the daemon-side turn-snapshot **file-restore** leg: Codex `thread/rollback` (upstream-deprecated beyond the `0.141.0` pin — Plan-005/Plan-010 re-verify against the then-installed binary per the receipt) reverts conversation only and leaves working-tree restoration to the daemon per the [provider-wire receipt](../reference/provider-wire/codex.md), so time-travel is NOT dispatchable on B1/B2 alone — the turn-snapshot service lands via the campaign's B21→B22→B23 chain (Task 26), gated **before Plan-004 Phase 3 rollback dispatch**; without it a rollback would emit `run.rolled_back` while leaving the worktree at the post-rollback state) |
| 20 | Session goals | [Spec-016](../specs/016-multi-agent-channels-and-orchestration.md) (per-session structured goal, set/clear RPC + `session.goal_*` events — landed 2026-07-06 via the campaign's B6 bundle (§Session Goals, goal RPCs + wire shapes in-tree); events registered by the Spec-006 B1 amendment (merged in-tree via PR #173) — residual gate for both specs = `review → approved` promotion, the W1.5 re-promotion gate / campaign Task 28) |
| 21 | Session callback tools | [Spec-005](../specs/005-provider-driver-contract-and-capabilities.md) (daemon-registered tool shape — landed 2026-07-05 via the campaign's B3 bundle (residual gate = Spec-005 `review → approved`, the W1.5 gate)), [Spec-012](../specs/012-approvals-permissions-and-trust-boundaries.md) (Cedar-governed identically to provider tools — B20 amendment, merged via PR #175); residual gate for both specs = `review → approved` promotion, the W1.5 gate / Task 28 |
...
- [RFC 9449 DPoP](https://datatracker.ietf.org/doc/html/rfc9449) + [W3C TAG capability-URLs finding](https://www.w3.org/2001/tag/doc/capability-urls/) + [Firefox Send](https://en.wikipedia.org/wiki/Firefox_Send) — sender-constrained authenticated fetch; the abuse post-mortem behind prohibiting capability URLs
...
- [Spec-021](../specs/021-rate-limiting-policy.md) — four artifact-relay registry rows (`artifact.upload.init`, `artifact.upload.chunk`, `artifact.fetch.authorize`, `artifact.fetch.chunk`)
