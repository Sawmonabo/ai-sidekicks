...
- Cross-Plan Dependencies — tier graph updated against this ADR per BL-054.
...
Remote provider transports and provider-native subagents were adopted in the same campaign but **amend existing features** (cross-node dispatch under Spec-024; orchestration under Spec-016) rather than minting feature numbers, so the count rises by six, not eight. The context-window fourth budget stays rejected on **correctness** grounds (estimated-grade window data driving hard interrupts would block co-users on false positives) — a correctness rejection, not a capability deferral, so it does not read against the R3 ruling.
