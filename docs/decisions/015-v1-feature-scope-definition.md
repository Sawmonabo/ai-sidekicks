...
**How decided:** Owner directive 2026-07-08 (pull forward into V1; hardened production design; publisher-online-only variant rejected), applied through the same four-criteria staff-level analysis as prior amendments (architectural correctness, modern 2025–2026 practice, regression surface, vulnerability surface) over the survey evidence. The relay holds ciphertext + per-participant wrapped keys only — it is trusted to hold bytes, never to read them (the existing relay trust model); integrity binds blob → signed publish event (not blob → itself), so a malicious relay cannot substitute content; the abuse surface is closed by authenticated-participant-only fetch plus quota/TTL envelopes.
