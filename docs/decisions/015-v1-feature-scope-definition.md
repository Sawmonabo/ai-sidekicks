...
| 20 | Session goals | [Spec-016](../specs/016-multi-agent-channels-and-orchestration.md) (per-session structured goal, set/clear RPC + `session.goal_*` events — to land via the campaign's B6 bundle, queued and not yet authored; events registered by the Spec-006 B1 amendment (merged in-tree via PR #173; residual gate = Spec-006's `review → approved` promotion — the W1.5 re-promotion gate, campaign Task 28) — both named merge prerequisites: the B6 Spec-016 goal RPC is queued and not yet authored; the B1 Spec-006 events are in-tree pending that promotion) |
