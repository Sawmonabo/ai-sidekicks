...
| 18 | MCP server configuration and governance | Spec-028 + Plan-028 (**landed 2026-07-22 via the campaign's B18 bundle, Task 24** — server-config CRUD, operator-managed trusted-server store with Cedar-gated per-tool overrides, zero-billed-turn status/health probing, server OAuth; Spec-028 promoted `approved` 2026-07-22 via the campaign's W3 gate; Plan-028 landed at `draft`, promoted `review` 2026-08-12 by its targeted readiness audit — `cross-plan-dependencies.md` §6 node NS-61 — and `approved` 2026-08-14 by its §Rollout Order step-2 promotion); authored net-new in the capability-enhancement campaign, R4 — see §Amendment History (amendment 2026-07-02) |
| 19 | Session time-travel (run rollback) | [Spec-004](../specs/004-queue-steer-pause-resume.md) (`rollback` intervention on the generic dispatch — landed via the campaign's B2 bundle (PR #205); Spec-004 re-promoted `approved` 2026-07-18 via the W1.5 gate / Task 28, re-flipped `review` 2026-08-03 by the V1 product-vision reconciliation amendment bundle, and restored `approved` 2026-08-08 by its run-control authorization targeted readiness-audit delta (#299 / NS-49); Plan-004 implements rollback), [ADR-017](./017-shared-event-sourcing-scope.md) (forward `run.rolled_back` event; the log never truncates — registered by the campaign's Spec-006 B1 amendment (merged in-tree via PR #173; Spec-006 re-promoted `approved` 2026-07-18 via the W1.5 batch gate, campaign Task 28)), [Spec-010](../specs/010-worktree-lifecycle-and-execution-modes.md) + [Plan-010](../plans/010-worktree-lifecycle-and-execution-modes.md) (the daemon-side turn-snapshot **file-restore** leg: the Codex conversation rewind (re-verified 2026-08-26 — `thread/rollback` carries its own deprecation in the generated type, so Spec-005 rebinds the rollback grade onto the non-experimental successor `thread/fork` at an inclusive `lastTurnId` boundary; the re-verification MUST is discharged, not deferred) reverts conversation only and leaves working-tree restoration to the daemon per the [provider-wire receipt](../reference/provider-wire/codex.md), so time-travel is NOT dispatchable on B1/B2 alone — the turn-snapshot service landed via the campaign's B21→B22→B23 chain (Task 26), gated **before Plan-004 rollback dispatch** — the gate held: the B23 leg shipped 2026-08-09 via PR #303 and the rollback cluster — Plan-004's Phase-3B explicit-label supplement since the NS-69 PR #346 round-1 re-home — remains undispatched, riding tier order plus its precondition legs; without it a rollback would emit `run.rolled_back` while leaving the worktree at the post-rollback state) |
