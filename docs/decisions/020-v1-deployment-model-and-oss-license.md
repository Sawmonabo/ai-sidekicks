...
| 2026-09-08 | Distributed OFL-1.1 font dependency admitted | `@ibm/plex-sans` (1.1.0) and `@ibm/plex-mono` (2.5.0), both SIL Open Font License 1.1, are admitted as **distributed** dependencies of `apps/desktop`: the console self-hosts the two ratified faces ([Spec-023 §Console Libraries](../specs/023-desktop-shell-and-renderer.md#console-libraries), the motion / fonts / icons row) rather than rendering in whichever face the host machine carries. OFL-1.1 sits outside the MIT / Apache-2.0 / BSD / ISC norm this ADR's Apache-2.0 commitment assumes for runtime dependencies, so it takes its own entry as the 2026-09-01 `axe-core` row requires of any bundled outside-norm use. The waiver holds because OFL-1.1's obligations are satisfied by construction here: the font files are shipped **unmodified** as build assets (Vite emits the package's own `woff2` bytes under a content hash and rewrites the reference — nothing is subsetted, renamed, or re-generated by this repo; verified at these pins by comparing the SHA-256 of all six emitted faces against the package sources, byte-identical in each case), so the Reserved Font Name clause is not engaged; each package carries its own `LICENSE.txt` at its root and declares `OFL-1.1` in `package.json`, and **reproducing that licence text in the packaged application is an open obligation on the packaging configuration**, which this repo does not yet carry — no release artifact is produced today, so this row records the requirement rather than reporting it met, and the phase that introduces the packaging configuration must include both licence files; and OFL's copyleft is scoped to the font software itself, which is bundled alongside the application rather than linked into it, so no obligation attaches to the Apache-2.0 source. A **modified** or **re-subsetted** face, or one distributed under the Reserved Font Name, is **not** covered and needs its own log entry. |
