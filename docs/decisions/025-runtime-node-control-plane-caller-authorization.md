...
`Plan-005 §Target Areas` already declares the CREATE: T4.10 "registers two typed registry-only codes — `RUNTIME_NODE_SIGNING_KEY_CONFLICT_CODE` … and `RUNTIME_NODE_PERMISSION_DENIED_CODE` (dotted `runtimenode.permission_denied`, 403)". That declaration stands; this ADR adds no ownership-map churn.
...
- If Plan-002 T3.10 lands first, it creates `RUNTIME_NODE_PERMISSION_DENIED_CODE` under the `error.ts` registration seam (the same seam Plan-002 already used for its three sibling codes) and records a one-line note at `Plan-005 §Cross-Plan Amendments`; T4.10 then consumes rather than creates. **Ordering is not a hard precondition on a P1 fix** — whichever lands first creates, and the loser's plan text is trued up in that same PR.
...
| 2026-08-12 | Codex PR #327 round 2 — three refinements | (1) §Uniform-Negative Scope's accepted residual re-scoped: node-id entropy bounds only callers the roster never showed the id to; the known-id post-detach squatting consequence for co-session observers is named and bounded (attributable row, fresh-id availability recovery at provenance-continuity cost, no attachment transition frees the slot per I-002-3), pinned by a T3.12 accepted-residual test, with the already-documented V1.1 node-identity trust anchor (ADR-017) as its closing mechanism. (2) §Mandated Work's Plan-028 amendment widened with the `Plan-028` host-wiring leg — the throwing `resolveCurrentUserId` swap plus per-route verified-token tests — so D2's zero-code-change promise has a named owner instead of an unowned assumption. (3) `Spec-002 §Required Behavior`'s under-lock mandate scoped to the four mutations at the source, stating D7's lock-free single-statement roster exemption where implementers read it; the campaign design spec's §4.A carries a supersession note so its superseded 404-model prose can no longer be followed as written. |
