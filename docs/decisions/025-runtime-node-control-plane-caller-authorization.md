...
> **Ratification record.** Authored for [BL-141](../archive/backlog-archive.md#bl-141-runtime-node-control-plane-caller-authorization-caller-owns-the-node-model) and ratified by the user on 2026-08-10: **Option A** (transaction-interior, row-locked ownership predicate) with **Reading 1** on the one point where this decision diverged from an already-ratified campaign design — every non-permitted verdict collapses into the contracted `runtimenode.permission_denied` (403), and `heartbeat`'s negative is a thrown 403 rather than a silent success-shaped no-op. The divergence and its adjudication are recorded, not erased, at §Adjudication Record — Resolved At Ratification. Enforcement lands in code under Plan-002 T3.10–T3.12; this ADR binds the design, not the shipment date.

## Context

The five `runtimenode.*` control-plane procedures — `attach`, `heartbeat`, `capabilityupdate`, `detach` (mutations) and `roster` (the control-plane-only query) — are shipped in `packages/control-plane/src/runtime-nodes/`. They are the surface through which a Local Runtime Daemon announces itself into a session, keeps its presence row fresh, refreshes its declared capabilities, and leaves.

[ADR-002](./002-local-execution-shared-control-plane.md) makes the daemon the execution authority and the control plane the shared-coordination authority; [ADR-007](./007-device-trust-and-permission-model.md) separates device trust from runtime-node trust from run approvals from tool grants, and holds that device trust alone never authorizes local execution on a node that has not accepted the session. Together they mean a runtime-node attachment row is an _account-owned_ object: it names the account on whose behalf a specific machine joined a specific session.

That ownership relation is presently unenforced. Four of the five procedures resolve their target row by `nodeId` (plus, for `attach`, `session_id`) with no predicate tying the row to the authenticated caller, and the fifth enumerates a session's whole roster with no session-ownership predicate at all.

## Problem Statement

How should the control plane decide that the caller of a `runtimenode.*` procedure is authorized to act on the attachment row that procedure resolves — and how should it refuse when they are not, without leaking whether the row or the session exists?

### Trigger

[BL-141](../archive/backlog-archive.md#bl-141-runtime-node-control-plane-caller-authorization-caller-owns-the-node-model) records a P1 horizontal-privilege-escalation (BOLA / IDOR) finding against the shipped code. Concretely, from the code as it stands:

- `runtime-node-router.factory.ts#createRuntimeNodeRouter` resolves a caller for `attach` **only**. Its `heartbeat`, `capabilityupdate`, `detach`, and `roster` procedures destructure `{ input }` alone and never call `deps.resolveCurrentUserId`. The factory's own AUTH POSTURE banner names both gaps as Tier-4-deferred.
- The one check that does exist compares the caller against **client-supplied input** (`input.userId !== current`), not against the persisted row — precisely the pattern [OWASP API1:2023](https://owasp.org/API-Security/editions/2023/en/0xa1-broken-object-level-authorization/) calls out as insufficient: "Comparing the user ID of the current session (e.g. by extracting it from the JWT token) with the vulnerable ID parameter isn't a sufficient solution to solve Broken Object Level Authorization (BOLA)." It is also a bare `!==` against a branded UUID whose schema admits mixed case.
- `attach-service.ts#AttachService` runs **zero** session-ownership queries: a caller who does not own the session at all can attach a node into it.
- `attach-service.ts#AttachService` `detach` issues `UPDATE runtime_node_attachments SET state = … WHERE node_id = $1 AND state IN (…)` — any caller who knows a `node_id` can evict a machine they do not own from a session.
- `attach-service.ts#AttachService` `updateCapabilities` selects `id, state, client_version, session_id … FOR UPDATE` and does not even _read_ `user_id`, so no ownership comparison is possible in the shipped shape.
...
- `attach-service.ts#AttachService` `readRoster` returns every `runtime_node_attachments` row for a session verbatim — including each row's `user_id` — to any caller, its header explicitly deferring "session existence/authorization" to "the router tier's concern", where nothing implements it.

The surface is not yet reachable by untrusted traffic (control-plane PASETO validation lands with Plan-028's control-plane auth phase per [ADR-010](./010-paseto-webauthn-mls-auth.md)), which is why this is a pre-exposure hardening decision rather than an incident. [Plan-002 §Risks And Blockers](../plans/002-runtime-node-attach.md#risks-and-blockers) states the constraint plainly (re-homed there from BL-141 §V1 Release Impact at that item's 2026-08-25 archival): the procedures MUST NOT be exposed to untrusted traffic until the ownership guard is enforced.

## Decision

**We adopt a direct ownership predicate, evaluated inside each service's own database transaction, against row state held under a row lock, in a fixed canonical lock order — Option A.** Cedar (Option B) and a pre-built Cedar seam (Option C) are rejected for this class of check, with a named revisit criterion.

The decision has eleven parts.

### D1 — The enforcement point is inside the service transaction, never a router-tier middleware

Authorization is computed from the same rows, in the same transaction, under the same locks as the mutation it authorizes. A tRPC middleware that pre-fetches the attachment row to authorize it would (a) double-fetch and (b) open a check-to-use window in which a concurrent `detach` or revocation commits between the middleware's read and the service's write. This satisfies [OWASP ASVS v5.0](https://github.com/OWASP/ASVS/blob/master/5.0/en/0x17-V8-Authorization.md) **8.3.1** ("Verify that the application enforces authorization rules at a trusted service layer") and **8.2.2** ("data-specific access is restricted to consumers with explicit permissions to specific data items to mitigate insecure direct object reference (IDOR) and broken object level authorization (BOLA)").

The router keeps exactly one job: resolve the authenticated caller and pass it down (D2).

### D2 — Caller identity flows as an explicit `callerUserId` service parameter

Every one of the five service methods takes `callerUserId: UserId` as an explicit parameter. The router resolves it once via `deps.resolveCurrentUserId(ctx)` and forwards it; the service never reaches into a tRPC context. This keeps the services transport-agnostic (the four mutations are dual-transport per [Spec-002 §Interfaces And Contracts](../specs/002-runtime-node-attach.md#interfaces-and-contracts)), makes the guard unit-testable without an HTTP layer, and means the guard becomes production-live with **zero code change** the moment Plan-028's control-plane auth phase makes `resolveCurrentUserId` return a PASETO-verified `sub` instead of throwing (the swap is an owned task, not an assumption: `Plan-028`'s host-wiring leg, per §Mandated Work).

Caller identity is never read from the request body. Client-supplied `userId` on `attach` is treated as an _assertion to be checked_, not as identity (D5).

### D3 — One classification helper with two verdicts

```
classifyRuntimeNodeCaller(transaction, callerUserId, attachmentRow)
  → "owner" | "not_visible"
```

- The **owner** sub-check compares `canonicalizeUuid(attachmentRow.user_id) === canonicalizeUuid(callerUserId)` — using the already-shipped `packages/contracts/src/uuid-canonical.ts#canonicalizeUuid`, because the branded `UserId` schema admits mixed case (RFC 9562 §4) while Postgres returns canonical lowercase.
- The **session** sub-check reads `sessions.owner_user_id` for the attachment's `session_id` and tests that it is the caller. It is a second, independent field: a caller may own a node row and still not own the session that row points into, and a caller may own a session without owning every node attached to it.
- Ownership does not move (`user-and-device-model.md §Session Ownership`), so unlike the role model this replaced there is no mid-flight authorization-loss race to serialize against: a caller either owned both objects when the transaction opened or did not. What still changes under the caller's feet is the attachment row itself, which is why D8's two-phase resolve and its re-verify step survive unchanged.

Authorization outcome per verdict:

| Verdict | `attach` | `heartbeat` | `capabilityupdate` | `detach` | `roster` |
| --- | --- | --- | --- | --- | --- |
| `owner` | permit | permit | permit | permit | permit (session ownership is the whole predicate) |
| `not_visible` | refuse | refuse | refuse | refuse (the idempotent no-op, D4) | refuse |

There is no longer a verdict in which a caller may detach a node but do nothing else. That arm existed because a suspended member could otherwise pin a `node_id` forever under `idx_node_attachments_active`, the **global** partial `UNIQUE(node_id)` index ([Plan-002 §Invariants](../plans/002-runtime-node-attach.md#invariants) I-002-5). Since ownership never lapses, a node's owner may always detach it, and the release valve the old arm supplied is now structural rather than a carve-out. I-002-3's prohibition on attach/detach mutating session state is untouched.

### D4 — `not_visible` maps to each procedure's own no-active-row response, never to a distinct one

This is the oracle-closing rule, and it is what makes the guard information-preserving rather than information-leaking. A caller must not be able to discriminate "this node does not exist", "this node exists but is not yours", "this session does not exist", and "this session is not yours" by observing different refusals.

| Procedure | Response for `not_visible` |
| --- | --- |
...
| `capabilityupdate` | the uniform negative — **the overloaded 409 is split**: today `runtimenode.capabilityupdate_conflict` covers both "no active row" and the I-002-2 `registering`-cannot-be-driven-`online` state guard. The no-active-row arm moves to the uniform negative; the genuine state-guard arm keeps the 409. |
| `roster` | the uniform negative for "nonexistent session OR a session the caller does not own"; `{ nodes: [], controlHolder: … }` **only** for a session the caller genuinely owns that has no attachments |
| `attach` | the uniform negative for an unowned session and a nonexistent session alike — superseding today's silent success and today's raw FK-violation 500 |

D3 leaves exactly one non-permitted verdict, so every refusal in the table above is the same refusal, and the no-oracle property is now structural rather than maintained by a mapping table: there is no second denial arm that could differ from the first. `detach` alone answers it as the idempotent `null` no-op rather than the 403, which discloses nothing either — it is byte-identical to detaching an already-`offline` node.

**Accepted residual, recorded rather than closed.** `attach` still answers `runtimenode.attach_conflict` (409) when a `node_id` is actively attached somewhere the caller cannot see. That is a one-bit "this node id is attached to _some_ session" channel and it is inherent to I-002-5's global uniqueness — the constraint's whole purpose is to be observable. Node-id unguessability — a **daemon-side property**, not a schema guarantee: `NodeId` is a bounded `TEXT` column (`packages/contracts/src/node-id.ts`), so the daemon's id-minting choice supplies the entropy — bounds this probe **only for callers the roster never showed the id to**, and [Plan-019](../plans/019-rate-limiting-policy.md)'s Tier-5 limits bound its rate. For anyone whose roster ever carried the node the id is **disclosed by design** — D7's faithful roster carries every entry's stable `nodeId`, and reconnect deliberately reuses it — and the sharper consequence for that population is **known-id squatting** (Codex PR #327 round 2): after the owner's `detach` frees the I-002-5 slot, a caller who owns _some_ session can attach the remembered id there, and the legitimate daemon's later reattach refuses 409 — an availability-plus-provenance-continuity attack, not merely the one-bit probe. Its accepted bounds, recorded honestly: the squatter must be an authenticated account owning some session; the squatted row is **fully attributable** (its `user_id` names the squatter in that session's faithful roster); the legitimate daemon recovers availability by minting a fresh node id, at the deliberate cost of the stolen id's provenance continuity ([Spec-002 §Pitfalls To Avoid](../specs/002-runtime-node-attach.md#pitfalls-to-avoid) names that cost); and — because I-002-3 forbids any session-state transition from reaping an attachment — no revocation frees the slot, only the squatter's own detach. T3.12 pins the squat as an explicit accepted-residual test so any future change to it is a conscious contract move, not drift. **Closing mechanism, already documented rather than newly deferred:** attach-time proof of node identity — reusing an existing `node_id` requires possession of its registered key material — is the V1.1 node-identity trust anchor gated at [ADR-017 §Server-Derived Runtime-Node Lifecycle Events](./017-shared-event-sourcing-scope.md#server-derived-runtime-node-lifecycle-events) (Plan-016 / Plan-002); when that anchor lands, this residual collapses with it. **Revisit criteria:** the trust anchor landing (closes the squat), and I-002-5 ever relaxing to per-session uniqueness (multi-session node sharing, deferred per [Spec-002 §Resolved Questions and V1 Scope Decisions](../specs/002-runtime-node-attach.md#resolved-questions-and-v1-scope-decisions)) — the 409 should then collapse into the uniform negative in the same change.

### D5 — `attach` carries two authorization axes, and the identity pre-guard is what makes the second one real

- **Axis (a) — session ownership (new).** Inside the transaction, take the level-1 `sessions` lock while reading `owner_user_id` and `min_client_version` (D8). Zero rows, or an `owner_user_id` that is not the caller → uniform negative, the same refusal for both (this also replaces the current raw-FK-violation path with a typed refusal). Only then run the upsert.
- **Axis (b) — node-identity ownership (already enforced structurally).** The shipped upsert's `ON CONFLICT … DO UPDATE … WHERE runtime_node_attachments.user_id = EXCLUDED.user_id` predicate plus the active partial-unique index already prevent one account from taking over another's `node_id` row. Axis (b) needs **tests**, not new enforcement code.
- **The pre-guard that makes axis (b) trustworthy.** Axis (b) compares the _row's_ owner against `EXCLUDED.user_id`, which comes from the request body. Without a check that the body's `userId` is the caller, an attacker simply sets `userId` to the victim and the upsert predicate happily matches. So before the transaction opens, `attach` refuses when `canonicalizeUuid(callerUserId) !== canonicalizeUuid(validated.userId)` — a cheap, allocation-free, pre-I/O ownership-spoof guard. The router's existing self-check is retained (defence in depth at the transport tier), canonicalized so it stops being case-sensitive, and re-thrown through the same typed `runtimenode.permission_denied` refusal the service pre-guard raises — its shipped plain tRPC `UNAUTHORIZED` would surface 401 with no `aisError` on the public transport, a refusal shape distinct from the uniform 403 that D9 forbids.

### D6 — `heartbeat` gains a transaction and the same resolve

`ingest` stops being an unconditional presence upsert. It runs the D8 two-phase resolve, and the presence row is writable **only through an owned, active attachment**. A `node_id` with no active attachment, or with an attachment the caller does not own, or pointing into a session the caller does not own, gets the uniform negative and writes nothing. This closes presence forgery, which today lets any caller keep an arbitrary node advertised as `online` and thereby defeat the T3.6 staleness sweep for that node.
...
### D7 — `roster` is a session-ownership predicate folded into the read, in one statement

The roster is a **session-scoped enumeration**, not a per-row ownership check: the session's owner enumerates its full node roster, because [Spec-002 §Interfaces And Contracts](../specs/002-runtime-node-attach.md#interfaces-and-contracts) requires a faithful projection (all five `state` values verbatim, both health axes, no server-side hiding) and AC2 distinguishability depends on it. Filtering rows by caller would break the spec's projection contract; the right predicate is at the session boundary.

Because the uniform negative collapses "not yours" and "no such session" (§Adjudication Record), the authorization decision needs **no lock and no transaction** — one `READ COMMITTED` statement is one snapshot, so nothing can interleave between predicate and projection. The prescribed shape drives the whole read from the session row itself:

```sql
SELECT s.owner_user_id,
       attachment.node_id, attachment.user_id, attachment.state, …
FROM sessions s
LEFT JOIN runtime_node_attachments attachment ON attachment.session_id = s.id
LEFT JOIN runtime_node_presence presence      ON presence.node_id = attachment.node_id
WHERE s.id = $1 AND s.owner_user_id = $2
```

Zero rows ⇒ a session the caller does not own, or no such session ⇒ uniform negative. One or more rows with `attachment.node_id IS NULL` ⇒ a visible session with no attachments ⇒ `{ nodes: [] }`. This is the shape [error-contracts.md §Runtime Node](../architecture/contracts/error-contracts.md#runtime-node) already ships for the sibling `runtimenode.signingkeyroster` surface ("the ownership predicate and the roster read are one SQL statement — one READ COMMITTED snapshot"), generalized. It is strictly simpler than the locked transaction the campaign design prescribed, and the simplification is a _consequence_ of the ratified Reading 1 (§Adjudication Record), not independent of it.

The `readOnly` per-row derivation keeps reading `sessions.min_client_version` in the same statement, and the `controlHolder` projection joins the same statement when the Plan-022 Phase 3B lease table lands.
...
That three-level sequence is the **union across registrants**, not any single registrant's own path. The Plan-005 T4.10 signing-key registrant instantiates levels 1 and 3 — the 2026-08-11 admission-time decoupling deleted its `runtime_node_attachments` acquisition, because no attachment row exists at admission time to read. This decision's runtime-node mutators register levels 1–2. Skipping a level is order-consistent and creates no cycle.

`I-002-6` (new) makes it binding for Plan-002. It is a **prefix-consistent extension** of the canonical order, not a competing declaration: the runtime-node mutators occupy the same levels in the same relative order, and nothing is re-ordered.
...
| --- | --- | --- | --- |
| 1 | `sessions` | `FOR KEY SHARE` on `attach`; `FOR SHARE` where the transaction must serialize against a floor change. This is also where the ownership predicate is read, and `FOR KEY SHARE` is sufficient for it because `owner_user_id` never changes — there is no concurrent writer to serialize against | `FOR KEY SHARE` "blocks other transactions from performing `DELETE` or any `UPDATE` that changes the key values" — exactly the existence guarantee `attach` needs — and it is the same mode the upsert's own foreign key takes implicitly, so no lock upgrade occurs. It does **not** conflict with `FOR NO KEY UPDATE`, the mode a floor-raising `UPDATE sessions SET min_client_version = …` acquires; that is acceptable here **only because the read-only verdict is derived, never persisted** — what the attachment row stores is `client_version`, and every later verdict site (`attach`, `capabilityupdate`, and each roster row) re-derives the answer from it against the then-current floor via `attach-service.ts#AttachService`'s shared `#deriveReadOnly`. A lost floor race therefore yields one stale response field, never durable corruption. A surface that needs the floor verdict to be authoritative-at-write takes `FOR SHARE` instead — the Plan-005 T4.10 registration surface does, and so does `capabilityupdate`'s `VERSION_FLOOR_EXCEEDED` write-admission gate: its `readOnly` **response projection** is the race-tolerable re-derived consumer above, but admitting or refusing the capability write is not. |
| 2 | `runtime_node_attachments` | `FOR NO KEY UPDATE` on mutator paths; `FOR SHARE` on read-only authorization | The mutators update `state` / `capabilities` / `client_version`. `state` sits only in a **partial** unique index, and PostgreSQL states that for the `FOR UPDATE`-escalation rule "partial indexes and expressional indexes are not considered" — so those updates take `FOR NO KEY UPDATE` natively, and matching that mode is sufficient. It conflicts with `detach`'s concurrent state `UPDATE`, with the other mutators, and with a Path-2 erasure `DELETE` (which takes `FOR UPDATE`). The shipped `updateCapabilities` `SELECT … FOR UPDATE` is strictly stronger and remains correct; T3.10 narrows it for uniformity. The read-only arm is `heartbeat`'s: its mutation targets `runtime_node_presence`, outside this order, so its attachment acquisition authorizes only (the two-phase resolve below records the per-procedure assignment). |

**Two-phase resolve** — required for `heartbeat`, `capabilityupdate`, and `detach`, which are keyed on `nodeId` alone and therefore cannot know their `session_id` before reading the attachment. Taking the attachment lock first to learn the session would invert levels 2 and 1 and deadlock ABBA against `attach`. So:

1. **Unlocked pre-read** by `node_id` over the active states, yielding a `(session_id, user_id)` snapshot. This read authorizes nothing.
2. Take level 1 (`sessions` — every procedure needs it now, because the ownership predicate is read there; `capabilityupdate` additionally needs the version floor authoritative-at-write and so takes `FOR SHARE` per the table's row-1 arm, while `heartbeat` is version-invariant and `detach` reads no floor, so both take `FOR KEY SHARE`), then level 2 — the attachment in that procedure's mode from the table above: `FOR NO KEY UPDATE` for the mutator paths (`capabilityupdate`, `detach`), `FOR SHARE` for `heartbeat`, whose mutation targets `runtime_node_presence` and whose attachment acquisition is therefore the read-only authorization arm — concurrent heartbeat retries never serialize against each other, while `FOR SHARE` still conflicts with a concurrent detach's `FOR NO KEY UPDATE`.
3. **Re-verify** the locked row's `(session_id, user_id)` against the step-1 snapshot. On mismatch — a concurrent detach and re-attach elsewhere — fail closed with the uniform negative. The daemon's next heartbeat re-attempts naturally; a bounded single retry is a permitted refinement, not a requirement.
...
See §Adjudication Record — Resolved At Ratification. Every non-permitted verdict on `attach` / `heartbeat` / `capabilityupdate` / `roster` collapses into the already-contracted `runtimenode.permission_denied` (domain code, HTTP 403, tRPC `FORBIDDEN`). `detach` alone keeps the idempotent `null` no-op (D4). No new error code is created.

### D10 — Symbol ownership for the refusal is Plan-005 T4.10's, consumed by Plan-002

`runtimenode.permission_denied` exists **as a doc row only**. `packages/contracts/src/error.ts` ships three `RUNTIME_NODE_*` codes (attach-conflict, attach-revoked, capabilityupdate-conflict) and `runtime-nodes/errors.ts` ships four `AisWireException` subclasses; neither carries a permission-denied member, and no `runtimenode.permission_denied` constant exists in code today.

[Plan-005 §Target Areas](../plans/005-session-event-taxonomy-and-audit-log.md#target-areas) already declares the CREATE: T4.10 "registers two typed registry-only codes — `RUNTIME_NODE_SIGNING_KEY_CONFLICT_CODE` … and `RUNTIME_NODE_PERMISSION_DENIED_CODE` (dotted `runtimenode.permission_denied`, 403)". That declaration stands; this ADR adds no ownership-map churn.

Consequently the Plan-002 T3.10 code leg (the former BL-141 code leg) **consumes** the symbol, and:

- If Plan-005 T4.10's contracts leg lands first, Plan-002 T3.10 imports it and adds only the `runtime-nodes/errors.ts` exception class binding.
- If Plan-002 T3.10 lands first, it creates `RUNTIME_NODE_PERMISSION_DENIED_CODE` under the `error.ts` registration seam (the same seam Plan-002 already used for its three sibling codes) and records a one-line note at [Plan-005 §Cross-Plan Amendments](../plans/005-session-event-taxonomy-and-audit-log.md#cross-plan-amendments); T4.10 then consumes rather than creates. **Ordering is not a hard precondition on a P1 fix** — whichever lands first creates, and the loser's plan text is trued up in that same PR.

### D11 — Invariant changes

- **I-002-3 (amended).** The invariant's "attach/detach never mutate session state" property is preserved verbatim; the amendment states that a `FOR KEY SHARE` **authorization read** of `sessions` is not a mutation and is expressly permitted. The invariant's Verification clause keeps requiring the byte-unchanged-row assertion.
- **I-002-6 (new).** Every control-plane transaction touching `runtime_node_attachments` acquires locks in the canonical order of D8, in the recorded modes, with the two-phase resolve for `nodeId`-keyed procedures. Verified by a lock-ordering regression test using the logging-proxy `Querier` pattern, authored with T3.12 as this surface's canonical lock-ordering suite.
...
**The predicate is genuinely two-field.** `caller == row.user_id AND caller == session.owner_user_id`. There is no hierarchy to traverse, no delegation, no attribute algebra, no time-of-day condition, no group nesting. Both facts are columns in tables the transaction is already reading. A policy language buys expressiveness that is not needed and costs an evaluation boundary that is actively harmful (above).

**The refusal surface is already contracted.** `error-contracts.md §Runtime Node` ships a `runtimenode.permission_denied` (403) row with an explicit no-oracle rationale and an explicit reservation of tRPC `NOT_FOUND` for a different purpose. Reusing it keeps one namespace with one refusal vocabulary, and the sibling `runtimenode.signingkeyroster` surface already demonstrates the single-statement ownership-predicate shape D7 adopts.
...
### Option A: Direct ownership predicate, transaction-interior, row-locked (Chosen)

- **What:** D1–D11 above. One shared classification helper, two verdicts, evaluated from locked rows inside each service transaction, refusals collapsed into one contracted 403.
...
- **What:** Model `User`, `Session`, `RuntimeNodeAttachment` as Cedar entities; author `permit(principal, action, resource) when { resource.owner == principal && resource.session.owner == principal }`; evaluate through the resident `@cedar-policy/cedar-wasm` engine on every `runtimenode.*` call.
...
- **What:** A `nodeOwnerProcedure` middleware that resolves the attachment by `nodeId`, checks ownership, and attaches the row to `ctx` for the service.
...
- **Why rejected:** It double-fetches, and — decisively — it authorizes a row read **outside** the service transaction, so a concurrent detach or revocation can commit between the middleware's read and the service's write. It also cannot participate in the canonical lock order, since it has no transaction to lock in. Notably, [trpc.io/docs/server/authorization](https://trpc.io/docs/server/authorization) offers no object-ownership primitive at all — its guidance stops at an `isAuthed` authentication middleware — so this is not a framework-blessed pattern being declined, it is a hand-rolled one. The framework's real contribution is kept (context-based caller resolution, D2); the row decision stays where the lock is.
...
`docs/archive/superpowers/specs/2026-07-09-bl-resolution-campaign-design.md` §3.1 decision 4 (Status `approved`, 2026-07-10) prescribed a **new** `runtimenode.not_found` code, HTTP 404 / tRPC `NOT_FOUND`, as the uniform negative. Three weeks later — 2026-07-31, Plan-005 T4.10, Codex PR #274 rounds 3–4 — `error-contracts.md §Runtime Node` shipped `runtimenode.permission_denied` (403) whose row ends: "deliberately never tRPC `NOT_FOUND`, which this namespace reserves as the old-control-plane procedure-absence discovery signal".
...
| Diagnostics lost | The session's owner mistyping a `node_id` sees 403 rather than 404. Non-security; recorded in §Negative | None — the 403/404 distinction survives for the owner, which is this reading's one genuine advantage |
...
| --- | --- | --- | --- |
| 1 | `resolveCurrentUserId` will return a cryptographically verified user when Plan-028's control-plane auth phase lands, with no signature change. | `runtime-node-router.factory.ts#createRuntimeNodeRouter` `RuntimeNodeRouterDeps` already types it `(ctx: SessionRouterContext) => UserId`; ADR-010 puts the caller's identity on the PASETO `sub`. | The guard authorizes a spoofable identity — it becomes decoration. Detection: the throwing stub is the current implementation, so the failure is loud, not silent. |
| 2 | `sessions.owner_user_id` is the correct authority signal for "may act in this session". | Ownership is the only session-level authority the model has, it is set once at creation, and it never moves (`user-and-device-model.md §Session Ownership`). | If a second authority signal is ever introduced (e.g. a delegated machine grant), the owner would be under-authorized nowhere but a delegate would be refused everywhere, and this predicate would need widening rather than replacing. |
| 3 | `runtime_node_attachments.user_id` is the true owner of the attachment and is never rewritten to another user. | The shipped upsert's `WHERE runtime_node_attachments.user_id = EXCLUDED.user_id` predicate forbids cross-owner rewrite; I-002-5's global partial unique index forbids a parallel row. | Ownership becomes forgeable and D3's owner sub-check is meaningless. Pinned by T3.12's axis-(b) regression tests. |
...
| 6 | Detach cannot be used to escalate. | `detach` can only move the caller's own row toward `offline`; it writes no authority and grants nothing. | A stale credential retains a live capability. Mitigated by the narrowness — one action, own row, monotonic toward inactive. |
| 7 | A node's owner can always free its global slot. | I-002-5's `idx_node_attachments_active` is `UNIQUE(node_id)` **globally**, not per session, and ownership never lapses, so the owner's detach is always available. | If ownership could ever be transferred or suspended, the slot could be pinned by an unreachable owner and an operator-side release would be needed. |
| 8 | No `runtimenode.*` client depends on the current unauthorized behavior. | The surface is pre-exposure; `resolveCurrentUserId` throws in production today. | The four behavior changes (D4/D5/D6/D7) break a live consumer. Detection: the four-suite migration plus the `client-sdk` integration ripple named in §Mandated Work. |
...
| Lock order violated by a new call site → ABBA deadlock under load | Low | Med | Postgres deadlock errors in production; the lock-ordering regression test in CI | I-002-6 plus the logging-proxy `Querier` test pattern from `runtime-nodes/__tests__/lock-ordering.test.ts` |
...
| A legitimate daemon is locked out mid-session by an over-strict ownership check | Low | High | Attach/heartbeat failures for honest clients | Ownership never lapses, so the predicate cannot turn false under a running daemon; admit-not-eject (I-002-1) is untouched — a below-floor daemon is still admitted read-only |
| Heartbeat's added transaction raises write load at 15s per node | Low | Low | Control-plane latency metrics | The transaction is three short indexed reads plus one upsert; Plan-019 rate limits bound the worst case |
| Plan-005 T4.10 and Plan-002 T3.10 both create `RUNTIME_NODE_PERMISSION_DENIED_CODE` | Med | Low | Merge conflict or duplicate export at typecheck | D10's first-lander rule with a same-PR true-up of the loser's plan text |
...
- **Blast radius:** `packages/control-plane/src/runtime-nodes/` (all four files), `packages/contracts/src/error.ts`, four control-plane test suites, `packages/client-sdk/test/runtimeNodeClient.integration.test.ts`, `Spec-002`, `Plan-002` (§Invariants + Phase 3), `Plan-028`, `error-contracts.md`, `shared-postgres-schema.md §Lock Ordering Across Shared Tables`.
...
- Refusals carry no ownership, attachment-existence, or session-existence oracle.
...
- No new wire types, no new dependency, no new error code (per the ratified Reading 1), and the guard goes production-live with zero further code change when Plan-028's control-plane auth phase lands.
...
- The session's owner mistyping a `node_id` loses the 403/404 diagnostic distinction. Non-security; the trade the no-oracle rule demands.
...
- `attach` and `detach` now read `sessions` for the ownership predicate, so any service header comment asserting that table is never referenced must be rewritten; the invariant's substance survives, its prose does not.

### Unknowns

- Whether cross-node dispatch (Plan-024 / Spec-022) will need a third principal type on this surface — a delegated operator acting for a node owner — which would trip the §Synthesis revisit criterion.
- Whether operators will want to author node-attach policy at all, or whether attach authority remains purely structural.
- Whether the `attach_conflict` one-bit channel proves practically exploitable under Plan-019's rate limits.
...
| Unowned-session roster enumeration refused | 100% | T3.12 roster enumeration case | `pending` — at T3.12 merge |
...
| Lock order and modes match I-002-6 | 100% of transactions touching `runtime_node_attachments` | Logging-proxy `Querier` lock-ordering regression test | `pending` — at T3.12 merge |
...
### Plan-002 — three Phase-3 tasks (existing tasks run T3.0–T3.9)

- **T3.10 — classification helper + typed refusal + per-procedure negatives.** Add `classifyRuntimeNodeCaller`; extend `updateCapabilities`'s `SELECT` to include `user_id` and narrow its `FOR UPDATE` to `FOR NO KEY UPDATE`; wire the two-phase resolve into `heartbeat` / `capabilityupdate` / `detach`; split `capabilityupdate`'s overloaded 409; bind the `runtimenode.permission_denied` exception class in `runtime-nodes/errors.ts` (creating `RUNTIME_NODE_PERMISSION_DENIED_CODE` in `packages/contracts/src/error.ts` only if Plan-005 T4.10 has not landed it — D10); rewrite the I-002-3 header comments in `attach-service.ts#AttachService` in the same commit.
- **T3.11 — `attach` ownership guard + roster ownership predicate.** Add `attach`'s pre-transaction identity pre-guard and its level-1/level-2 locks; canonicalize the router self-check; fold the ownership predicate into `readRoster`'s single statement per D7.
...
- **I-002-3 (amended)** per D11.
- **I-002-6 (new)** per D11 — the canonical lock order, modes, and two-phase resolve.

### Plan-028

Amend `Plan-028` to name the five `runtimenode.*` procedures in its gated-endpoint list, so the verified identity D2 depends on is contractually required rather than incidentally supplied. The same amendment (widened at Codex PR #327 round 2) grows `Plan-028` with the **host-wiring leg** that makes D2's zero-code-change promise come true: replace the host's throwing `resolveCurrentUserId` injection with the middleware-derived verified principal and exercise each `runtimenode.*` route under a verified token — without an owned wiring task, the middleware would exist while every runtime-node route kept throwing the Tier-4 deferral error.

### Suite migration (build-breaking)

`packages/control-plane/src/runtime-nodes/__tests__/attach-service.test.ts`, `heartbeat-service.test.ts`, `runtime-node-router.test.ts`, and `packages/control-plane/src/server/__tests__/host-runtime-node.test.ts`, plus the cross-package ripple into `packages/client-sdk/test/runtimeNodeClient.integration.test.ts`. Every one of these constructs calls that currently succeed without a caller; the four service methods' new required `callerUserId` parameter breaks them at compile time, which is the desired failure mode.
...
| `docs/archive/superpowers/specs/2026-07-09-bl-resolution-campaign-design.md` §3.1, §4.A | Repo (approved design) | The ratified content contract for this ADR: decisions 1–9, the three Plan-002 tasks, the invariant changes, and the suite-migration list. This ADR adopts 1–3 and 5–9 unchanged and **supersedes decision 4's negative shape**, ratified 2026-08-10 (§Adjudication Record); the campaign plan's Task 2 is corrected in the same PR | [2026-07-09-bl-resolution-campaign-design.md §4.A Unit A — BL-141 caller-ownership authorization (P1)](../archive/superpowers/specs/2026-07-09-bl-resolution-campaign-design.md#4a-unit-a--bl-141-caller-ownership-authorization-p1) |
...
- [ADR-007: Device Trust and Permission Model](./007-device-trust-and-permission-model.md) — the layering this decision operates inside; no layer is added, removed, or re-scoped
...
- [ADR-018: Cross Version Compatibility](./018-cross-version-compatibility.md) — I-002-1 admit-not-eject is untouched; a below-floor daemon is still admitted read-only

### Related Specs And Plans

- [Spec-002: Runtime Node Attach](../specs/002-runtime-node-attach.md)
- [Plan-002: Runtime Node Attach](../plans/002-runtime-node-attach.md)
- [Plan-005: Session Event Taxonomy And Audit Log](../plans/005-session-event-taxonomy-and-audit-log.md) — T4.10, the declared creator of `RUNTIME_NODE_PERMISSION_DENIED_CODE`
...
| 2026-08-10 | Ratified — `proposed → accepted` | User ratified **Option A** with **Reading 1**, including the second call (`heartbeat`'s negative is a thrown 403, not a silent no-op). §Open Adjudication For Ratification is rewritten as §Adjudication Record — Resolved At Ratification, preserving the comparison table as the record of what was weighed. Consequential same-PR edits, landed 2026-08-12: `error-contracts.md §Runtime Node` (`permission_denied` call sites extended; `capabilityupdate_conflict` narrowed to its surviving state-guard arm per D4), `cross-plan-dependencies.md` (level-2 reservation converted to a shipped registrant row per D8, and the existing row's `FOR KEY SHARE` no-op claim scoped to its floor-serialization purpose), `Spec-002` (§Required Behavior authorization precondition, zero net lines), `Plan-002` (T3.10–T3.12, I-002-3 amended, I-002-6 new), `Plan-028` (Plan-028's gated-endpoint list names the five `runtimenode.*` procedures — landed **in this same vehicle**, with Plan-028 flip-and-restored `approved` in the same swap, superseding the draft's bilateral-handoff plan, which assumed a Plan-028 restore vehicle that landed 2026-08-11 without this leg), and the campaign plan's Task 2 (decision-4 premise superseded). Both Pre-Implementation Checklist boxes resolved. D8's attribution of the four-level lock order was repaired in the same pass: the order is the union across registrants, and Plan-005 T4.10's own path lost its `runtime_node_attachments` level to the 2026-08-11 admission-time registration decoupling. Code enforcement is **not** in this PR — BL-141 stays open until T3.10–T3.12 ship. |
| 2026-08-12 | Codex PR #327 round 1 — five refinements | (1) `detach`'s negative split restored to the ratified campaign-design decision-3 rule: `member_not_owner` refuses with the uniform 403 — D4's table had briefly collapsed every non-permitted `detach` verdict into the idempotent `null`, contradicting D3's own refuse cell — and the `null` no-op is scoped to `not_visible`; `error-contracts.md`, Spec-002 §Required Behavior + §Fallback Behavior, and Plan-002 T3.10/T3.12 aligned in the same pass. (2) D8's two-phase step 2 scoped per procedure: `heartbeat`'s level-3 acquisition is `FOR SHARE` (read-only authorization — its mutation targets `runtime_node_presence`), `capabilityupdate`/`detach` take `FOR NO KEY UPDATE`, matching the mode table's own weakest-sufficient rule. (3) The attach router self-check re-throws the typed `runtimenode.permission_denied` refusal instead of a plain tRPC `UNAUTHORIZED` (D5/D9 — no 401-without-`aisError` on the public transport). (4) T3.12's oracle-equality matrix restated per procedure over applicable causes — the blanket four-cause matrix was unsatisfiable (`attach` must succeed on a free node, `roster` deliberately projects foreign rows, `nodeId`-keyed procedures carry no session input). (5) Two §References location cells gained durable anchors, and the campaign plan's still-open Task 4 was rewritten in place to this ADR's ratified contract. |
| 2026-08-12 | Codex PR #327 round 2 — three refinements | (1) §Uniform-Negative Scope's accepted residual re-scoped: node-id entropy bounds only callers the roster never showed the id to; the known-id post-detach squatting consequence for co-session observers is named and bounded (attributable row, fresh-id availability recovery at provenance-continuity cost, no attachment transition frees the slot per I-002-3), pinned by a T3.12 accepted-residual test, with the already-documented V1.1 node-identity trust anchor (ADR-017) as its closing mechanism. (2) §Mandated Work's Plan-028 amendment widened with the `Plan-028` host-wiring leg — the throwing `resolveCurrentUserId` swap plus per-route verified-token tests — so D2's zero-code-change promise has a named owner instead of an unowned assumption. (3) [Spec-002 §Required Behavior](../specs/002-runtime-node-attach.md#required-behavior)'s under-lock mandate scoped to the four mutations at the source, stating D7's lock-free single-statement roster exemption where implementers read it; the campaign design spec's §4.A carries a supersession note so its superseded 404-model prose can no longer be followed as written. |
| 2026-08-12 | Codex PR #327 round 3 — five consistency fixes (final round) | (1) D8 step 2 and the mode-table row-1 rationale now name `capabilityupdate` as the level-1 `FOR SHARE` taker — its `VERSION_FLOOR_EXCEEDED` write-admission gate needs the floor authoritative-at-write, while its `readOnly` response projection stays the race-tolerable re-derived consumer — aligning the campaign plan's Step 10 rules and lock-ordering assertions with the dep-map's levels 1→2→3 cell. (2) The campaign's HTTP envelope oracle compares the complete `aisError` (message included) for one `nodeId` across foreign-owned and never-attached states. (3) Plan-002 T3.12 owns the canonical lock-ordering suite for these procedures. (4) The classifier module is canonically `classify-caller.ts` across Plan-002 T3.10 and campaign Task 4. (5) The BL-141 backlog References row cites this ADR through its §Adjudication Record anchor. |
| 2026-08-25 | Pointer restatement — BL-141 archived as a duplicate tracking surface | No decision content changes. BL-141 was withdrawn to the backlog archive under the no-duplicate-backlog rule (its remaining criterion (d) is Plan-002 T3.10–T3.12, which this ADR already names as the enforcement leg); §Trigger now sources the exposure gate from [Plan-002 §Risks And Blockers](../plans/002-runtime-node-attach.md#risks-and-blockers), where it was re-homed in that plan's own voice; D10's first-lander clauses name Plan-002 T3.10 directly; and the two BL-141 links point at the archive entry. Status `accepted` unchanged; §Success Criteria's `pending — at T3.12 merge` row stands. |
