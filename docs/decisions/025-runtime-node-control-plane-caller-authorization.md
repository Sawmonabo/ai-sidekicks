...
[ADR-002](./002-local-execution-shared-control-plane.md) makes the daemon the execution authority and the control plane the shared-coordination authority; [ADR-007](./007-collaboration-trust-and-permission-model.md) separates device trust from runtime-node trust from run approvals from tool grants, and holds that device trust alone never authorizes local execution on a node that has not accepted the session. Together they mean a runtime-node attachment row is an _account-owned_ object: it names the account on whose behalf a specific machine joined a specific session.
...
- `runtime-node-router.factory.ts#createRuntimeNodeRouter` resolves a caller for `attach` **only**. Its `heartbeat`, `capabilityupdate`, `detach`, and `roster` procedures destructure `{ input }` alone and never call `deps.resolveCurrentUserId`. The factory's own AUTH POSTURE banner names both gaps as Tier-5-deferred.
...
The surface is not yet reachable by untrusted traffic (control-plane PASETO validation lands with Plan-031's control-plane auth phase per [ADR-010](./010-paseto-webauthn-mls-auth.md)), which is why this is a pre-exposure hardening decision rather than an incident. `Plan-003 §Risks And Blockers` states the constraint plainly (re-homed there from BL-141 §V1 Release Impact at that item's 2026-08-25 archival): the procedures MUST NOT be exposed to untrusted traffic until the ownership guard is enforced.
...
Every one of the five service methods takes `callerUserId: UserId` as an explicit parameter. The router resolves it once via `deps.resolveCurrentUserId(ctx)` and forwards it; the service never reaches into a tRPC context. This keeps the services transport-agnostic (the four mutations are dual-transport per `Spec-003 §Interfaces And Contracts`), makes the guard unit-testable without an HTTP layer, and means the guard becomes production-live with **zero code change** the moment Plan-031's control-plane auth phase makes `resolveCurrentUserId` return a PASETO-verified `sub` instead of throwing (the swap is an owned task, not an assumption: `Plan-031`'s host-wiring leg, per §Mandated Work).
...
There is no longer a verdict in which a caller may detach a node but do nothing else. That arm existed because a suspended member could otherwise pin a `node_id` forever under `idx_node_attachments_active`, the **global** partial `UNIQUE(node_id)` index (`Plan-003 §Invariants` I-003-5). Since ownership never lapses, a node's owner may always detach it, and the release valve the old arm supplied is now structural rather than a carve-out. I-003-3's prohibition on attach/detach mutating session state is untouched.
...
**Accepted residual, recorded rather than closed.** `attach` still answers `runtimenode.attach_conflict` (409) when a `node_id` is actively attached somewhere the caller cannot see. That is a one-bit "this node id is attached to _some_ session" channel and it is inherent to I-003-5's global uniqueness — the constraint's whole purpose is to be observable. Node-id unguessability — a **daemon-side property**, not a schema guarantee: `NodeId` is a bounded `TEXT` column (`packages/contracts/src/node-id.ts`), so the daemon's id-minting choice supplies the entropy — bounds this probe **only for callers the roster never showed the id to**, and [Plan-021](../plans/021-rate-limiting-policy.md)'s Tier-6 limits bound its rate. For anyone whose roster ever carried the node the id is **disclosed by design** — D7's faithful roster carries every entry's stable `nodeId`, and reconnect deliberately reuses it — and the sharper consequence for that population is **known-id squatting** (Codex PR #327 round 2): after the owner's `detach` frees the I-003-5 slot, a caller who owns _some_ session can attach the remembered id there, and the legitimate daemon's later reattach refuses 409 — an availability-plus-provenance-continuity attack, not merely the one-bit probe. Its accepted bounds, recorded honestly: the squatter must be an authenticated account owning some session; the squatted row is **fully attributable** (its `user_id` names the squatter in that session's faithful roster); the legitimate daemon recovers availability by minting a fresh node id, at the deliberate cost of the stolen id's provenance continuity (`Spec-003 §Pitfalls To Avoid` names that cost); and — because I-003-3 forbids any session-state transition from reaping an attachment — no revocation frees the slot, only the squatter's own detach. T3.12 pins the squat as an explicit accepted-residual test so any future change to it is a conscious contract move, not drift. **Closing mechanism, already documented rather than newly deferred:** attach-time proof of node identity — reusing an existing `node_id` requires possession of its registered key material — is the V1.1 node-identity trust anchor gated at [ADR-017 §Server-Derived Runtime-Node Lifecycle Events](./017-shared-event-sourcing-scope.md#server-derived-runtime-node-lifecycle-events) (Plan-018 / Plan-003); when that anchor lands, this residual collapses with it. **Revisit criteria:** the trust anchor landing (closes the squat), and I-003-5 ever relaxing to per-session uniqueness (multi-session node sharing, deferred per `Spec-003 §Resolved Questions and V1 Scope Decisions`) — the 409 should then collapse into the uniform negative in the same change.
...
The roster is a **session-scoped enumeration**, not a per-row ownership check: the session's owner enumerates its full node roster, because `Spec-003 §Interfaces And Contracts` requires a faithful projection (all five `state` values verbatim, both health axes, no server-side hiding) and AC2 distinguishability depends on it. Filtering rows by caller would break the spec's projection contract; the right predicate is at the session boundary.
...
That three-level sequence is the **union across registrants**, not any single registrant's own path. The Plan-006 T4.10 signing-key registrant instantiates levels 1 and 3 — the 2026-08-11 admission-time decoupling deleted its `runtime_node_attachments` acquisition, because no attachment row exists at admission time to read. This decision's runtime-node mutators register levels 1–2. Skipping a level is order-consistent and creates no cycle.

`I-003-6` (new) makes it binding for Plan-003. It is a **prefix-consistent extension** of the canonical order, not a competing declaration: the runtime-node mutators occupy the same levels in the same relative order, and nothing is re-ordered.
...
Consequently the Plan-003 T3.10 code leg (the former BL-141 code leg) **consumes** the symbol, and:
...
| 3 | `runtime_node_attachments.user_id` is the true owner of the attachment and is never rewritten to another user. | The shipped upsert's `WHERE runtime_node_attachments.user_id = EXCLUDED.user_id` predicate forbids cross-owner rewrite; I-003-5's global partial unique index forbids a parallel row. | Ownership becomes forgeable and D3's owner sub-check is meaningless. Pinned by T3.12's axis-(b) regression tests. |
...
| 7 | A node's owner can always free its global slot. | I-003-5's `idx_node_attachments_active` is `UNIQUE(node_id)` **globally**, not per session, and ownership never lapses, so the owner's detach is always available. | If ownership could ever be transferred or suspended, the slot could be pinned by an unreachable owner and an operator-side release would be needed. |
...
- No new wire types, no new dependency, no new error code (per the ratified Reading 1), and the guard goes production-live with zero further code change when Plan-031's control-plane auth phase lands.
...
**Honest scope of criterion (d).** The guard is enforced in code and exercised by tests, but production traffic stays behind the throwing `resolveCurrentUserId` until Plan-031's control-plane auth phase lands PASETO validation. This is the maximum honest satisfaction of (d) available pre-Tier-5; it is not a claim that untrusted traffic is now safely admitted.
...
Amend `Plan-031` to name the five `runtimenode.*` procedures in its gated-endpoint list, so the verified identity D2 depends on is contractually required rather than incidentally supplied. The same amendment (widened at Codex PR #327 round 2) grows `Plan-031` with the **host-wiring leg** that makes D2's zero-code-change promise come true: replace the host's throwing `resolveCurrentUserId` injection with the middleware-derived verified principal and exercise each `runtimenode.*` route under a verified token — without an owned wiring task, the middleware would exist while every runtime-node route kept throwing the Tier-5 deferral error.
