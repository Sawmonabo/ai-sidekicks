...
| 2026-08-12 | Codex PR #327 round 2 — three refinements | (1) §Uniform-Negative Scope's accepted residual re-scoped: node-id entropy bounds only callers the roster never showed the id to; the known-id post-detach squatting consequence for co-session observers is named and bounded (attributable row, fresh-id availability recovery at provenance-continuity cost, no membership transition frees the slot per I-003-3), pinned by a T3.12 accepted-residual test, with the already-documented V1.1 node-identity trust anchor (ADR-017) as its closing mechanism. (2) §Mandated Work's Plan-031 amendment widened with the `Plan-031` host-wiring leg — the throwing `resolveCurrentUserId` swap plus per-route verified-token tests — so D2's zero-code-change promise has a named owner instead of an unowned assumption. (3) `Spec-003 §Required Behavior`'s under-lock mandate scoped to the four mutations at the source, stating D7's lock-free single-statement roster exemption where implementers read it; the campaign design spec's §4.A carries a supersession note so its superseded 404-model prose can no longer be followed as written. |
| 2026-08-12 | Codex PR #327 round 3 — five consistency fixes (final round) | (1) D8 step 2 and the mode-table row-1 rationale now name `capabilityupdate` as the level-1 `FOR SHARE` taker — its `VERSION_FLOOR_EXCEEDED` write-admission gate needs the floor authoritative-at-write, while its `readOnly` response projection stays the race-tolerable re-derived consumer — aligning the campaign plan's Step 10 rules and lock-ordering assertions with the dep-map's levels 1→2→3 cell. (2) The campaign's HTTP envelope oracle compares the complete `aisError` (message included) for one `nodeId` across foreign-owned and never-attached states. (3) Plan-003 T3.12 extends the canonical `memberships` lock-ordering suite instead of creating a runtime-nodes sibling. (4) The classifier module is canonically `classify-caller.ts` across Plan-003 T3.10 and campaign Task 4. (5) The BL-141 backlog References row cites this ADR through its §Adjudication Record anchor. |
