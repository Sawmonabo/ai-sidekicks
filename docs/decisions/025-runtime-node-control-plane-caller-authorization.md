...
Every one of the five service methods takes `callerParticipantId: ParticipantId` as an explicit parameter. The router resolves it once via `deps.resolveCurrentParticipantId(ctx)` and forwards it; the service never reaches into a tRPC context. This keeps the services transport-agnostic (the four mutations are dual-transport per `Spec-003 §Interfaces And Contracts`), makes the guard unit-testable without an HTTP layer, and means the guard becomes production-live with **zero code change** the moment Plan-031's control-plane auth phase makes `resolveCurrentParticipantId` return a PASETO-verified `sub` instead of throwing (the swap is an owned task, not an assumption: `Plan-031`'s host-wiring leg, per §Mandated Work).
...
- **Axis (b) — node-identity ownership (already enforced structurally).** The shipped upsert's `ON CONFLICT … DO UPDATE … WHERE runtime_node_attachments.participant_id = EXCLUDED.participant_id` predicate plus the active partial-unique index already prevent one account from taking over another's `node_id` row. Axis (b) needs **tests**, not new enforcement code.
...
The lock order is the canonical one registered at [cross-plan-dependencies.md §Lock Ordering Across Shared Tables](../architecture/cross-plan-dependencies.md#lock-ordering-across-shared-tables):
...
**The predicate is genuinely two-field.** `caller == row.participant_id AND caller == session.owner_user_id`. There is no hierarchy to traverse, no delegation, no attribute algebra, no time-of-day condition, no group nesting. Both facts are columns in tables the transaction is already reading. A policy language buys expressiveness that is not needed and costs an evaluation boundary that is actively harmful (above).
...
- **What:** Model `Participant`, `Session`, `RuntimeNodeAttachment` as Cedar entities; author `permit(principal, action, resource) when { resource.owner == principal && resource.session.owner == principal }`; evaluate through the resident `@cedar-policy/cedar-wasm` engine on every `runtimenode.*` call.
...
**Honest scope of criterion (d).** The guard is enforced in code and exercised by tests, but production traffic stays behind the throwing `resolveCurrentParticipantId` until Plan-031's control-plane auth phase lands PASETO validation. This is the maximum honest satisfaction of (d) available pre-Tier-5; it is not a claim that untrusted traffic is now safely admitted.
...
Amend `Plan-031` to name the five `runtimenode.*` procedures in its gated-endpoint list, so the verified identity D2 depends on is contractually required rather than incidentally supplied. The same amendment (widened at Codex PR #327 round 2) grows `Plan-031` with the **host-wiring leg** that makes D2's zero-code-change promise come true: replace the host's throwing `resolveCurrentParticipantId` injection with the middleware-derived verified principal and exercise each `runtimenode.*` route under a verified token — without an owned wiring task, the middleware would exist while every runtime-node route kept throwing the Tier-5 deferral error.
...
| 2026-08-12 | Codex PR #327 round 2 — three refinements | (1) §Uniform-Negative Scope's accepted residual re-scoped: node-id entropy bounds only callers the roster never showed the id to; the known-id post-detach squatting consequence for co-session observers is named and bounded (attributable row, fresh-id availability recovery at provenance-continuity cost, no membership transition frees the slot per I-003-3), pinned by a T3.12 accepted-residual test, with the already-documented V1.1 node-identity trust anchor (ADR-017) as its closing mechanism. (2) §Mandated Work's Plan-031 amendment widened with the `Plan-031` host-wiring leg — the throwing `resolveCurrentParticipantId` swap plus per-route verified-token tests — so D2's zero-code-change promise has a named owner instead of an unowned assumption. (3) `Spec-003 §Required Behavior`'s under-lock mandate scoped to the four mutations at the source, stating D7's lock-free single-statement roster exemption where implementers read it; the campaign design spec's §4.A carries a supersession note so its superseded 404-model prose can no longer be followed as written. |
