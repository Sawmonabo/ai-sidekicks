...
2. Take level 1 (`sessions`, only if the procedure needs the floor: `capabilityupdate` does — its `VERSION_FLOOR_EXCEEDED` gate needs the verdict authoritative-at-write, so it takes `FOR SHARE` per the table's row-1 arm; `heartbeat` is version-invariant and `detach` reads no floor, so both skip it), then level 2 (`session_memberships` `FOR SHARE`), then level 3 — the attachment in that procedure's mode from the table above: `FOR NO KEY UPDATE` for the mutator paths (`capabilityupdate`, `detach`), `FOR SHARE` for `heartbeat`, whose mutation targets `runtime_node_presence` and whose attachment acquisition is therefore the read-only authorization arm — concurrent heartbeat retries never serialize against each other, while `FOR SHARE` still conflicts with a concurrent detach's `FOR NO KEY UPDATE`.
...
See §Adjudication Record — Resolved At Ratification. Every non-permitted verdict on `attach` / `heartbeat` / `capabilityupdate` / `roster` — and `detach`'s `member_not_owner` verdict, per the campaign design's decision-3 rule that `member_not_owner` always raises the typed refusal — collapses into the already-contracted `runtimenode.permission_denied` (domain code, HTTP 403, tRPC `FORBIDDEN`). `detach`'s `not_visible` verdict keeps the idempotent `null` no-op (D4), and its `owner_inactive_member` verdict is the D3 permit. No new error code is created.
...
| 2026-08-12 | Codex PR #327 round 2 — three refinements | (1) §Uniform-Negative Scope's accepted residual re-scoped: node-id entropy bounds only callers the roster never showed the id to; the known-id post-detach squatting consequence for co-session observers is named and bounded (attributable row, fresh-id availability recovery at provenance-continuity cost, no membership transition frees the slot per I-003-3), pinned by a T3.12 accepted-residual test, with the already-documented V1.1 node-identity trust anchor (ADR-017) as its closing mechanism. (2) §Mandated Work's Plan-008 amendment widened with the `T-008r-1-5` host-wiring leg — the throwing `resolveCurrentParticipantId` swap plus per-route verified-token tests — so D2's zero-code-change promise has a named owner instead of an unowned assumption. (3) `Spec-003 §Required Behavior`'s under-lock mandate scoped to the four mutations at the source, stating D7's lock-free single-statement roster exemption where implementers read it; the campaign design spec's §4.A carries a supersession note so its superseded 404-model prose can no longer be followed as written. |
