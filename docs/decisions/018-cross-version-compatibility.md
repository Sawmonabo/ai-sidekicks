...
The wire format carried between users is the `EventEnvelope`, defined in [Spec-005: Session Event Taxonomy and Audit Log](../specs/005-session-event-taxonomy-and-audit-log.md). [Spec-005 §Interfaces And Contracts](../specs/005-session-event-taxonomy-and-audit-log.md#interfaces-and-contracts) already declares `EventEnvelope` must be versioned and lists `version` as an envelope-level field in the canonical field set ([Spec-005 §Canonical Serialization Rules](../specs/005-session-event-taxonomy-and-audit-log.md#canonical-serialization-rules) — serialized order is RFC 8785 UTF-16 code-unit lex order, not list order). What Spec-005 does not yet document is the **semantics** of that field: who sets it, who validates it, what happens on mismatch, and how event-type evolution interacts with it.

Additionally, the event log is append-only and tamper-evident — row-chained via BLAKE3 and Ed25519-signed per [Spec-005 §Integrity Protocol](../specs/005-session-event-taxonomy-and-audit-log.md#integrity-protocol). Any version-evolution story must preserve that immutability: we cannot rewrite the log on upgrade, and stubbed unknowns must remain signature-verifiable forever.
...
- BL-064 (event-taxonomy cascade) will add ~20+ new event types mid-V1-lifecycle. The first envelope-relevant addition will arrive before Plan-013 lands, so the version scheme must be specified before then.
...
5. **Unknown event types MUST persist as version stubs, never dropped.** A version stub is a distinct artifact from the compaction stubs defined in [Spec-005 §Event Compaction Policy](../specs/005-session-event-taxonomy-and-audit-log.md#event-compaction-policy) — a compaction stub has had its `payload` removed, whereas a version stub preserves the full original canonical bytes verbatim. The version stub preserves those canonical bytes (so the Ed25519 signature remains verifiable per [Spec-005 §Integrity Protocol](../specs/005-session-event-taxonomy-and-audit-log.md#integrity-protocol)) plus a version-stub-metadata record (`original_version`, `original_type`, `received_at`, `stub_reason`). The canonical row's `.version` field stays the producer's original — version-stubbing is a read-side behavior, not a rewrite.
...
11. **Retro-replay durability contract.** Version stubs remain parseable for the full audit-retention lifetime. The version-stub-metadata schema is versioned separately from envelope `.version` so future stub-schema evolution is itself versionable. Compaction policy ([Spec-005 §Event Compaction Policy](../specs/005-session-event-taxonomy-and-audit-log.md#event-compaction-policy)) MUST NOT compact a version stub while it remains un-re-interpreted — compaction would remove the `payload` bytes that later upcasters need as input. Once a version stub has been re-interpreted at least once (the upcaster chain for its `(original_version, original_type)` has fired in at least one client session), the row is eligible for compaction on the normal Spec-005 schedule, and its Ed25519 signature-verifiability is relinquished at that point (the same trade-off every compacted event accepts — Spec-005 already documents that compaction removes `payload` from the signed canonical form).
...
| 4 | Receivers can safely persist unknown-type payloads without schema validation, because envelope-level Ed25519 signature still covers the payload bytes. | [Spec-005 §Canonical Serialization Rules](../specs/005-session-event-taxonomy-and-audit-log.md#canonical-serialization-rules) lists `payload` as a signed field regardless of type-registry state; signature verification is independent of type-handler registration. | Stubs persist unverified payloads; attack surface opens via unsigned-content replay. |
...
| Two peers cannot agree on `min_client_version` at join | Med | Low | `VERSION_FLOOR_EXCEEDED` / `VERSION_CEILING_EXCEEDED` typed errors | UX displays mutual upgrade path; session metadata shows which user is below/above |
...
| Session-metadata floor field desync across nodes | Low | Med | Control-plane is authoritative for session metadata ([ADR-004](./004-sqlite-local-state-and-postgres-control-plane.md)); reconciliation via Spec-002 runtime-node-attach | Peer reads floor from control plane at join; never trusts peer-reported floor |
...
- Actual upcaster-chain replay cost at scale; measured once Plan-013 and the first MINOR bump ship.
...
| Below-floor client write attempt yields typed `VERSION_FLOOR_EXCEEDED`, never crash | 100% of below-floor write attempts in contract-test matrix | CI contract tests | Plan-013 landing |
...
| Unknown-type events persist as version stubs with verifiable Ed25519 signatures | 100% of version stubs pass [Spec-005 §Integrity Protocol](../specs/005-session-event-taxonomy-and-audit-log.md#integrity-protocol) verification | CI integrity test | First MINOR bump |
...
| 2026-08-26 | Cross-reference — scope boundary recorded | Pointer row, no new semantics. [ADR-029](./029-canonical-transcript-is-authoritative.md) rests part of its case on a limit of this decision: the additive-optional / forward-compatible discipline here governs shapes the corpus defines, and can say nothing about a provider vendor's private, unversioned session-store format. That gap is the compatibility argument for treating a provider session as a disposable replay target rather than a record to depend on. Nothing in this ADR's scope, envelope contract, or skew policy changes; it stays `accepted`. |
| 2026-08-26 | Cross-reference — provider-CLI skew recorded as a distinct axis | Pointer row, no new semantics. This ADR governs skew in shapes **this corpus defines** — the `EventEnvelope` between two of our own daemons, where a floor is negotiable and unknowns are stubbed and re-emitted. Provider-CLI skew is a second axis with a different answer, because the vendor owns the shape and ships it faster than we can re-verify: [Spec-004 §Required Behavior](../specs/004-provider-driver-contract-and-capabilities.md#required-behavior) sets it as **below the floor, refuse; at or above it, admit and decide every capability individually by a zero-turn probe of the running build** — accept-and-stub has no analogue there, since a provider surface that moved cannot be stubbed and replayed, only detected and degraded. Its **enforcement is runtime and lives in the daemon** — the spawn-time floor gate Plan-004 T3.23 builds and tests. The scheduled `provider-compat.yml` nightly registered at [ADR-023 §Decision Log](./023-v1-ci-cd-and-release-automation.md#decision-log) is **regression evidence for that floor, not enforcement of it**: it installs each CLI at its floor, its pin, and `latest`, checks that those releases' provider surfaces still answer, and reports drift without gating a merge. It checks out no daemon code, runs no driver, and carries no below-floor arm, so it can pass while a daemon wrongly admits an older build — which is why the gate's own test, not this nightly, is what the policy rests on. Nothing in this ADR's scope, envelope contract, or skew policy changes; it stays `accepted`. |
| 2026-08-28 | Provider-CLI pins moved; floors unchanged | Pointer row, no new semantics. The recorded reference pins move to `codex-cli 0.150.1` and Claude Code `2.1.251`; the **floors do not move** and stay `codex-cli 0.141.0` / Claude Code `2.1.234` in [Spec-004 §Required Behavior](../specs/004-provider-driver-contract-and-capabilities.md#required-behavior), which remains their normative home. That split is this ADR's second axis working as specified: a pin is the version the wire references were measured at, a floor is the version admission refuses below, and moving the first moves neither the second nor any daemon behavior — admission is floor-compare only, so a build above the pin attaches and decides each capability by its own probe. What the hop actually changed, measured by full-list set difference between the two default generations rather than against a named subset: `ServerNotification` **75 → 79** arms, four **added** and none removed — `mcpServer/event/stream/notification`, `thread/realtime/item/started`, `thread/realtime/item/transcript/delta`, `thread/realtime/item/completed`, all four experimental-gated, taking the gated total to 23 of 79. `ClientRequest` (95), `ServerRequest` (10), and `ClientNotification` (1) are unmoved, and every generated params type the Codex reference reproduces is byte-identical across the hop. The three new realtime names are **additions beside** the older spellings and not renames of them, so the `thread/realtime/*` opt-out family is re-derived **eight → eleven** rather than re-spelled — a count [ADR-015 §Decision Log](./015-v1-feature-scope-definition.md#decision-log)'s 2026-08-26 feature-#23 note states at its own measurement date — date-anchored, so it stays as written — and one that [Spec-014 §Resolved Questions and V1 Scope Decisions](../specs/014-multi-agent-channels-and-orchestration.md#resolved-questions-and-v1-scope-decisions) restates from that note as an **undated** live scope-decision clause — corrected to eleven in that spec in this same change, an erratum-class count-currency edit moving no Status; that note's mechanism claim is unaffected, and the default and `--experimental` `ServerNotification` unions remain identical (79/79) at this pin. On the Claude side the flag surface and every recorded literal were re-verified at `2.1.251` while the schema-constructor census stays **carried** at `2.1.245` rather than restamped, per that reference's own §Version pin. Nothing in this ADR's scope, envelope contract, or skew policy changes; it stays `accepted`. |
