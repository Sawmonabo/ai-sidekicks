...
- [ADR-014](./014-trpc-control-plane-api.md) — tRPC v11 for the Control Plane.
...
| Build orchestrator | **Turborepo** with a remote cache backed by the GitHub Actions cache (`rharkor/caching-for-turbo`), signed artifacts, telemetry disabled. Amended 2026-08-25 (see [Decision Log](#decision-log)): this row read "self-hosted remote cache (`ducktors/turborepo-remote-cache` or equivalent)". | 2.9+ |
...
| Test runner | **Vitest** with `projects` configuration; **happy-dom + `@testing-library/react`** for renderer component tests (amended 2026-08-25 — see [Decision Log](#decision-log); Browser Mode reserved for assertions needing a real layout engine) | 4.x |
...
| SQLite binding | **better-sqlite3** with WAL at boot, prepared-statement caching for hot paths | 13.0.3 exact (Node-API; shipped 2026-09-01 by the Electron-44 pin-move PR — §Decision Log; no 12.x ships an ABI-149 prebuild, and 13.0.0 / 13.0.1 abort under the daemon's worker-terminate shape, so the pin is exact rather than a caret) |
| Postgres client | **pg** (`node-postgres`); `pg-listen` wrapper for LISTEN/NOTIFY | 8.20+ |
| Node target | **Two-tier**: Node 22 Maintenance LTS for daemon + Electron renderer (forced by ADR-016); Node 24 Active LTS for control plane + CLI | 22.14+ (shipped 2026-09-01, moved from 22.12+ with the `better-sqlite3` 13.x Node-API-10 prebuild — §Decision Log) / 24.x |
...
1. **Two-ABI native bindings.** The renderer (Electron 41 main + preload via `electron-v123` ABI) and the daemon (Node 22 native ABI) share `better-sqlite3` source but require different compiled binaries. _Amended 2026-09-01 (see [Decision Log](#decision-log)): the Electron-ABI axis of this constraint is dissolved for `better-sqlite3` by its 13.0.0 move to Node-API — one platform prebuild serves every Node and Electron version — and the V1 shell target is now Electron 44 (ABI 149, Node 24.19), which no 12.x release ever shipped a prebuild for. The isolated-linker rationale stands for any remaining per-ABI native module; the `pnpm rebuild` against Electron headers is no longer needed for this one. The `electron-v123` artifact name in the original text was also an erratum: Electron 41's ABI is 145, and 12.9.0 did ship `electron-v145` prebuilds._ pnpm's isolated linker is the only manager primitive that keeps each workspace's `node_modules` resolution-scoped, allowing `pnpm rebuild --filter=@ai-sidekicks/desktop better-sqlite3` against Electron headers without disturbing the daemon's Node 22 build (`pnpm/pnpm#9073`, `WiseLibs/better-sqlite3#1393`). Hoisted layouts (npm, Bun default, Yarn `node-modules`) put a single `better-sqlite3` at `node_modules/better-sqlite3` and break one runtime or the other.
2. **Hash-chain BLOB ergonomics.** Spec-005's integrity protocol (`prev_hash`, `row_hash`, `daemon_signature`, `user_signature`) operates on Buffers — BLAKE3, Ed25519, RFC 8785 JCS canonicalization all consume Node `Buffer`. better-sqlite3 returns BLOBs as `Buffer`; node:sqlite returns `Uint8Array`, requiring an extra `Buffer.from(view)` per read. Across event replay (read-heavy) this is allocation noise we eliminate by binding choice.
3. **Single-toolchain renderer + Node coverage.** Vitest 4.x with `projects` is the only stable runner that handles Node-class packages, type-only packages, _and_ the React renderer in one toolchain — the renderer project running under a DOM shim (happy-dom), with Browser Mode (Playwright provider, stable since 4.0 / 2025-10-22) available in the same runner for any assertion that later needs a real layout engine. The load-bearing claim is `projects`, not the DOM implementation: one runner, one config, every package. Pairing Vitest with the existing esbuild dependency from the JS-emit toolchain produces zero-config TS execution everywhere. (Amended 2026-08-25 — the original text named Browser Mode as the renderer vehicle; see [Decision Log](#decision-log).)
...
| --- | --- | --- | --- | --- |
| The Electron or Node ABI drifts away from a bundled `better-sqlite3` prebuild on a release | Low | High | `require` of the binding fails, or resolves outside `prebuilds/` — 13.x's prebuilds are Node-API and version-independent, so a host the tarball does not cover is the failure, not a recompile: the package's build script is denied in `pnpm-workspace.yaml`, so no source compile can silently stand in | Pin `better-sqlite3` exact; the `native-prebuilds` CI matrix asserts on each of the five target platforms that the binding the daemon resolves is that platform's tarball prebuild and that `build/Release/` holds no `.node`; vendor the prebuild or hold the Electron pin if a target loses coverage |
| Turborepo retires the open remote-cache HTTP API | Low | Medium | Vercel announcement; `turbo.json` schema breaks; the `rharkor/caching-for-turbo` proxy stops serving | Fall back to local-only caching, or stand up `ducktors/turborepo-remote-cache`, or migrate to Nx self-host (also free per Powerpack announcement). Amended 2026-08-25 (see [Decision Log](#decision-log)) — the row previously named the self-hostable cache as the thing deprecated, which stopped matching the posture when the backend moved to the GitHub Actions cache. |
| Vitest 4.x renderer environment regresses (happy-dom shim or, where used, Browser Mode) | Med | Medium | Renderer test suite fails after a Vitest or happy-dom update | Pin Vitest and happy-dom minors; escalate the `renderer` project to Browser Mode (Playwright provider, already supported by the same runner) — or fall back to Playwright Component Testing if the `projects` mechanism itself is at fault |
...
| `better-sqlite3` installs from a bundled Node-API prebuild on every V1 platform (re-derived 2026-09-01; supersedes the two-ABI rebuild criterion) | `pnpm install` on macOS x64/arm64, Linux x64/arm64, Windows x64 resolves a bundled `.node` for `better-sqlite3` 13.0.3 with no source compile, and the daemon suite passes on the moved tier-1 floor (Node 22.14) and on 24.x, and under Electron 44's Node ABI 149 | CI matrix run | `2026-09-08` |
...
| better-sqlite3 v13.0.0 release notes (2026-09-01 amendment) | Changelog | First N-API release: prebuilt binaries "should theoretically work across different versions of Node.js and Electron"; `prebuild-install` removed; prebuilds published inside the package; source compile at install as the fallback | <https://github.com/WiseLibs/better-sqlite3/releases/tag/v13.0.0> |
| better-sqlite3 releases 12.9.0 → 12.12.0 (2026-09-01 amendment) | Registry | Per-ABI Electron prebuilds top out at `electron-v145` (12.9.0 / 12.10.x) and `electron-v148` (12.11.2 / 12.12.0); no release carries `electron-v149`; 13.x releases carry no per-ABI assets at all | <https://github.com/WiseLibs/better-sqlite3/releases> |
| Electron releases feed (2026-09-01 amendment) | Registry | `modules` (Node ABI) 145 for 41.x, 146 for 42.x, 148 for 43.x, **149** for 44.x; 44.1.1 bundles Node 24.19.0, and so does 44.1.0, the exact pinned release (measured on the installed binary 2026-09-01: Electron 44.1.0 / Chromium 152.0.7977.65 / Node 24.19.0) | <https://releases.electronjs.org/releases.json> |
...
| SQLCipher project | Documentation | Whole-database single-master-key encryption — incompatible with Spec-020's per-user AES-GCM crypto-shred via key-row deletion; rejected | <https://www.zetetic.net/sqlcipher/> |
...
- [ADR-016: Electron Desktop Shell](./016-electron-desktop-shell.md) — sets the Electron floor that forces the tier-1 Node target (Electron 41 at authoring; amended 2026-09-01 to the 42 / 43 / 44 supported-branch subset with V1 on 44.x, whose bundled Node is 24.19; the tier-1 engines floor moves `>= 22.12.0` → `>= 22.14.0` with the `better-sqlite3` 13.x pin per §Decision Log 2026-09-01 — forced by the binding's Node-API-10 prebuild, not by Electron's Node).
...
- [Plan-001: Session Core](../plans/001-session-core.md) — first consumer of this toolchain; tier-entry plan that owns `0001-initial.ts`.
...
| 2026-08-25 | Amended | **§Decision row 2's remote cache is no longer self-hosted.** The row committed to a self-hosted remote cache; [ADR-023 §Axis 1 — CI Workflow Architecture](./023-v1-ci-cd-and-release-automation.md#axis-1--ci-workflow-architecture) now backs Turborepo's cache with the GitHub Actions cache through `rharkor/caching-for-turbo`, which is GitHub-hosted. Recorded here rather than left as a silent contradiction, and recorded honestly: ADR-022 sets **no** budget constraint — the string "$0" appears nowhere in this document, and row 2's "or equivalent" attaches to the project name in the parenthetical. An earlier draft of the ADR-023 amendment attributed a "$0 or equivalent" latitude to this row and reconciled against that; the real departure is from **self-hosting**, and it is deliberate. What self-hosting bought was independence from a vendor who could relicense the service — the concern this ADR's own assumption row 2 names. A GitHub Actions cache artifact is subject to no such relicensing, so the property is preserved by a different mechanism rather than abandoned. **Both external premises cited and narrowed 2026-08-25:** an earlier draft asserted flatly that the backend "costs nothing to run", which overstates a metered service. Per [GitHub's Actions billing documentation](https://docs.github.com/en/billing/managing-billing-for-your-products/about-billing-for-github-actions), cache storage is an **included allowance of 10 GB per repository**, and standard GitHub-hosted runner minutes are free in public repositories — so the accurate claim is that this repository's usage sits inside a free allowance, not that the service is unpriced; overage is billable where a repository raises the limit above the included amount. The [dependency-caching reference](https://docs.github.com/en/actions/reference/workflows-and-actions/dependency-caching) sets the eviction policy this trade actually inherits: "If you exceed the limit, GitHub will save the new cache but will begin evicting caches until the total size is less than the repository limit", oldest-by-last-access first, and "GitHub will remove any cache entries that have not been accessed in over 7 days". That 7-day rule is operationally load-bearing for the `BL-122` measurement window — a quiet week evicts the baseline and the next run is cache-cold through no fault of the wiring. On the relicensing half: what is retained is Turborepo's MIT-licensed CLI and its open HTTP cache API (the §Sources row above), which is what `rharkor/caching-for-turbo` implements; the exposure that remains is that API being retired, which is exactly what this ADR's re-pointed failure-mode row now names. Assumption row 2's remediation and the self-hostable-cache failure mode are re-pointed at the backend actually in use. Cost was never the deciding argument this ADR made; it is the deciding argument [ADR-023 §Decision Log](./023-v1-ci-cd-and-release-automation.md#decision-log) makes, on its own authority. |
...
| 2026-08-25 | Amended | **Renderer test environment: Browser Mode → happy-dom + `@testing-library/react`.** §Decision row 4 named Browser Mode (Playwright provider) as the renderer-test vehicle; the desktop app has in fact shipped renderer component tests in the Vitest `projects` `renderer` project under happy-dom + `@testing-library/react` since the first renderer suites landed, and the Plan-002 BL-131 backfill landed there on 2026-08-25. Browser Mode is reserved for assertions that genuinely need a real layout engine — none exist yet — and the Playwright dependency it drags in is not paid for coverage a DOM shim already gives. Playwright remains the E2E vehicle via Electron's `_electron` harness at Plan-021 Tier 7. **This is a real amendment, not a record-only note:** §Decision row 4, [ADR-022 §Thesis — Why This Combination](./022-v1-toolchain-selection.md#thesis--why-this-combination) point 3, [ADR-022 §Assumptions Audit](./022-v1-toolchain-selection.md#assumptions-audit) row 3, and the [ADR-022 §Failure Mode Analysis](./022-v1-toolchain-selection.md#failure-mode-analysis) renderer row are all rewritten in place in this same change, so an implementer reading any of them reaches the shipped choice. The load-bearing half of the decision — Vitest 4.x with `projects` as the single runner — is unchanged, which is why the ADR stays `accepted` rather than being superseded; what changed is the DOM implementation under one of its projects, a two-way door (escalating to Browser Mode is a per-project config edit in the same runner). Corrected at PR #355 review after an earlier record-only entry left the normative text contradicting the log. |
| 2026-09-01 | Amended | **Electron 44 floor ripple: `better-sqlite3` moves to the 13.x Node-API line.** [ADR-016](./016-electron-desktop-shell.md) is amended the same day to the 42 / 43 / 44 supported-branch subset with V1 on 44.x (Electron 41 end-of-support 2026-08-25). Electron 44 is Node ABI 149, and a re-enumeration of the better-sqlite3 releases finds no 12.x prebuild for it — 12.9.0 (the repo pin) stops at 145 and 12.12.0 at 148 — so under the shipped pin the daemon's binding would compile from source on every machine that runs it under the shell's `utilityProcess`. better-sqlite3 13.0.0 (2026-07-21) refactored to `node-addon-api`, bundles eight platform prebuilds in the tarball, and needs no per-ABI artifact, which dissolves §Thesis constraint 1's Electron-ABI axis for this module and retires the `pnpm rebuild` against Electron headers along with the CI-matrix ABI-poisoning hazard that rebuild step carried. **Decision:** `packages/runtime-daemon` bumps `better-sqlite3` `^12.9.0 → 13.0.3` (13.0.1 relaxed a cross-realm parameter-binding strictness regression and 13.0.2 fixed a worker-thread abort, both relevant to the daemon's worker-per-`Database` pattern — and the package's own `engines.node >= 22` is an overstatement: its prebuilds are compiled against Node-API 10, which Node exposes from 22.14.0 / 23.6.0, and on a host reporting Node-API 9 the first `new Database` dies `SIGSEGV` with no diagnostic (measured 2026-09-01: v22.12.0, v22.9.0, and v20.17.0 exit 139 silently; v24.18.0 loads), so **the tier-1 Node floor moves 22.12 → 22.14.0 in the same task** — the root and every package `engines.node`, the five CI `22.12` pins, and the contributor-facing floor sentences — which [ADR-023](./023-v1-ci-cd-and-release-automation.md)'s Node-floor resolution absorbs unchanged, since release CI already runs the Node 24 tier and the moved floor narrows the gap that resolution recorded rather than widening it) in the tooling PR that moves the desktop Electron pin, or ahead of it — the pin may not move first. Assumption 4 re-derived; the `electron-v123` erratum corrected at every site that carried it — §Thesis constraint 1, Assumption 4, and the §References prebuild-enumeration row (Electron 41's ABI was 145) — and the two-ABI rebuild retired at every site that asserted it: §Decision row 4's version cell, the §Failure Mode Analysis native-binding row (its detection signal is now a source compile in the install log), §Synthesis's reinforcement chain, and the §Success Criteria rebuild row, each rewritten in place on the 2026-08-25 entry's discipline so an implementer reading any of them reaches the shipped choice. **The Spec-013 gate.** [Spec-013 §Driver Pin](../specs/013-persistence-recovery-and-replay.md#driver-pin)'s upgrade policy requires any 13.x move to be re-evaluated against its §Pragmas overrides and its §Backup Policy atomicity contract before the pin moves; that gate binds the pin-move PR, [Plan-021](../plans/021-desktop-shell-and-renderer.md) T-021p-1B-4, which carries the Spec-013 pin amendment in-swap with the re-verification (pragma readback under 13.0.3 and the `.backup` contract re-probed) re-run on the pinned tree at merge. What the 13.x line changes is the binding's host interface, not SQLite's: 13.0.0 refactored to `node-addon-api` and added `db.explain` / `Statement.toString`, 13.0.1 relaxed a cross-realm parameter-binding regression, 13.0.2 fixed a worker-thread abort (#1507) and moved the embedded SQLite to 3.53.4, 13.0.3 changed only the Linux-arm64 build image — none touches `.backup`'s use of `sqlite3_backup_*` or the pragma path, which is why the re-evaluation is a test-run confirmation rather than a design review. **Re-evaluation, run 2026-09-01 ahead of the pin-move PR** (scratch worktree off `develop` at `f03d2e77`, Node v24.18.0, darwin-arm64): the daemon suite is identical under `^12.9.0` and `13.0.3` — 89 test files passed / 2 skipped, 4047 tests passed / 4 skipped / 1 todo — and typecheck is green repo-wide; the shipped `applyPragmas` reads back `journal_mode = wal`, `synchronous = 2`, `foreign_keys = 1`, `busy_timeout = 5000` with all 17 migrations applied, and `pragma`'s return shape is unchanged; the daemon ships **no** `.backup` caller today (the only two `backup` hits are prose comments), so [Spec-013 §Backup Policy](../specs/013-persistence-recovery-and-replay.md#backup-policy)'s atomicity contract has no suite yet and was re-probed directly — a 20 000-row WAL source copies to `remainingPages: 0` with the progress callback firing and `integrity_check` ok — the suite the policy names coming into existence with the backup writer; embedded SQLite 3.53.0 → 3.53.4; the binary that loads is the tarball's `prebuilds/darwin-arm64.node`, and `node-gyp` is still invoked at install yet builds nothing, both `binding.gyp` targets becoming `type: none` when the host prebuild exists, so the pin-move PR's install assertion checks provenance — the resolved binding path under `prebuilds/` and no `.node` under `build/Release/`, the one location a source build writes and, with `prebuild-install` gone from the 13.x line, nothing else does — rather than an absent `node-gyp` line, which appears on every green install; the runtime API diff is additive only (`Database.explain`, `Statement.toString`) against a daemon surface of exactly `prepare` / `exec` / `transaction` / `pragma` / `close`, with `SqliteError` never `instanceof`-tested; #1507 is not hypothetical for this repo — `session-service.test.ts` terminates worker fleets holding open handles, and under that shape 13.0.1 aborts `SIGABRT` (`FATAL ERROR: Error::ThrowAsJavaScriptException napi_throw`) while 13.0.3 survives 300 of 300 cycles, so 13.0.0 / 13.0.1 are unsafe here and 13.0.2 is the true floor beneath the 13.0.3 pin; the lockfile drops thirty transitive packages (`prebuild-install`, deprecated upstream, with `node-abi`, `tar-fs`, `tar-stream`, `simple-get`, and their tree) and adds one (`node-addon-api` 8.9.2); `@types/better-sqlite3` publishes no 13.x, so its pin stays and the two added methods are untyped and unused; and Electron 41.6.1's Node 24.15 (Node-API 10) loads the same prebuild under `ELECTRON_RUN_AS_NODE`, confirming the ABI-independence claim on the one Electron binary on hand. §Option F's `node:sqlite` rejection is unchanged — its BLOB return type and the Node 22 stability label are the reasons, and neither moves. **Re-run on the pinned tree 2026-09-01 by [Plan-021](../plans/021-desktop-shell-and-renderer.md) T-021p-1B-4, the PR that ships the move** (darwin-arm64, Node v24.18.0): the daemon suite is green under the landed `13.0.3` pin at the same 89 files / 4047 tests / 4 skipped / 1 todo the scratch re-evaluation recorded, and the two Spec-013 gate legs are re-verified there rather than merely re-asserted — the pragma readback is exact and its `SQLITE_DEFAULT_WAL_SYNCHRONOUS=1` premise is confirmed in the 13.x build's own `PRAGMA compile_options` (a connection reopening an existing WAL database reads `synchronous = 1` before the override), and `.backup` is re-probed directly for the Promise shape, `integrity_check ok`, the mutation-during-backup reflection, and the post-backup `wal_checkpoint(TRUNCATE)` truncating to zero bytes. The install assertion this entry specified ships in `.github/workflows/ci.yml` in its two-conjunct provenance form and is negative-controlled (a planted `build/Release/*.node` fails it). **One deviation from the authored Electron version, recorded here because this ADR owns the pin mechanics:** the desktop dependency lands as the **exact** `44.1.0` (`--save-exact`, no caret), not `44.1.1`. The exactness [Plan-021](../plans/021-desktop-shell-and-renderer.md) T-021p-1B-4 requires is delivered in full — no range, so a lockfile refresh cannot move the runtime outside the review every bump owes — and only the version digit deviates: 44.1.1 published 2026-09-01T18:25:51Z, roughly five hours before the pin-move install, and this workspace sets `minimumReleaseAge: 1440` (the supply-chain guard [ADR-023](./023-v1-ci-cd-and-release-automation.md) adopted), which is a resolution-time filter over published-at and not a range-satisfaction rule, so it refuses an exact `44.1.1` spec exactly as it refuses a `^44.1.1` range — measured: `ERR_PNPM_NO_MATURE_MATCHING_VERSION`, with no fallback — and there is therefore no lockfile shape that carries 44.1.1 on the day it published. 44.1.0 (published 2026-08-31T13:54:22Z, Chromium 152.0.7977.65 / Node 24.19.0 — the same Node the feed reports for 44.1.1) is the newest 44.x the guard admits, and [ADR-016 §Decision](./016-electron-desktop-shell.md#decision)'s normative rule is **44.x only**, which it satisfies; that decision's `44.1.1 at amendment` parenthetical is provenance about the registry `latest` that day and is unmoved. The gate is dated rather than vague: the guard matures 44.1.1 at 2026-09-02T18:25Z, after which moving the pin is a one-line exact bump plus a lockfile refresh. Weakening the guard with a `minimumReleaseAgeExclude` entry for `electron` was rejected: the exclusion would be permanent and unconditional, and the guard exists precisely for a same-day release of a dependency that ships a platform binary and runs with full Node privileges. Carried by the console-design amendment; Status stays `accepted`. |
