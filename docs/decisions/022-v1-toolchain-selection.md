...
| SQLite binding | **better-sqlite3** with WAL at boot, prepared-statement caching for hot paths | 13.0.3 (Node-API; `^12.9.0` as shipped until the Electron-44 pin-move PR — §Decision Log 2026-09-01; no 12.x ships an ABI-149 prebuild) |
| Postgres client | **pg** (`node-postgres`); `pg-listen` wrapper for LISTEN/NOTIFY | 8.20+ |
| Node target | **Two-tier**: Node 22 Maintenance LTS for daemon + Electron renderer (forced by ADR-016); Node 24 Active LTS for control plane + CLI | 22.14+ (moved 2026-09-01 from 22.12+ with the `better-sqlite3` 13.x Node-API-10 prebuild — §Decision Log) / 24.x |
...
| Electron releases feed (2026-09-01 amendment) | Registry | `modules` (Node ABI) 145 for 41.x, 146 for 42.x, 148 for 43.x, **149** for 44.x; 44.1.1 bundles Node 24.19.0 | <https://releases.electronjs.org/releases.json> |
