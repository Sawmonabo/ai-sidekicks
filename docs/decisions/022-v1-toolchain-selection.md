...
| 2026-08-25 | Reconciled (record only) | §Decision row 4's "Browser Mode (Playwright provider) for renderer tests" reading is superseded **as shipped**: renderer component tests run in the Vitest `projects` `renderer` project under **happy-dom + `@testing-library/react`**, which is what the desktop app has actually shipped since the Plan-002 Phase 6 renderer suites and what the Plan-003 BL-131 backfill landed on 2026-08-25. Browser Mode (`@vitest/browser-playwright`) is reserved for assertions that genuinely need a real layout engine — none exist yet — and the Playwright dependency it drags in is not paid for coverage a DOM shim already gives. The `projects` configuration itself (the row's load-bearing half) is unchanged, and Playwright remains the E2E vehicle via Electron's `_electron` harness at Plan-023 Tier 8. Recorded here rather than by rewriting row 4 because the Browser Mode reading is restated three more times in this ADR — `ADR-022 §Thesis — Why This Combination` point 3, `ADR-022 §Assumptions Audit` row 3, and an `ADR-022 §Failure Mode Analysis` row; a partial row edit would leave the ADR less internally consistent than one entry naming the divergence. No decision is reversed and the ADR stays `accepted`. |
