...
| Node target | **Two-tier**: Node 22 Maintenance LTS for daemon + Electron renderer (forced by ADR-016); Node 24 Active LTS for control plane + CLI | 22.14+ (the `better-sqlite3` 13.x prebuilds are compiled against Node-API 10, which Node exposes from 22.14.0; on a host reporting Node-API 9 the first `new Database` dies `SIGSEGV` with no diagnostic) / 24.x |
...
| Turborepo retires the open remote-cache HTTP API | Low | Medium | Vercel announcement; `turbo.json` schema breaks; the `rharkor/caching-for-turbo` proxy stops serving | Fall back to local-only caching, or stand up `ducktors/turborepo-remote-cache`, or migrate to Nx self-host (also free per Powerpack announcement). |
...
- [ADR-016: Electron Desktop Shell](./016-electron-desktop-shell.md) — sets the Electron floor that forces the tier-1 Node target: V1 runs on Electron 44.x, whose bundled Node is 24.19. The tier-1 `engines.node` floor is `>= 22.14.0`, forced by the `better-sqlite3` 13.x Node-API-10 prebuild rather than by Electron's Node.
