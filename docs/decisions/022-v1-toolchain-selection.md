...
| --- | --- | --- | --- | --- |
| The Electron or Node ABI drifts away from a bundled `better-sqlite3` prebuild on a release | Low | High | A source compile (`node-gyp`) for `better-sqlite3` appears in the `pnpm install` log — 13.x's prebuilds are Node-API and version-independent, so a compile means the bundled platform set itself changed | Pin `better-sqlite3` exact; CI asserts no `node-gyp` invocation for it in the install log; vendor the prebuild or hold the Electron pin if one appears |
