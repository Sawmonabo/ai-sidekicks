...
| 3 | The same JSON-RPC payloads can be reused verbatim over a WebSocket adapter, which carries the bidirectional device-channel subset only; [ADR-014](./014-trpc-control-plane-api.md) owns the browser/remote request-response transport. | ADR-008 positions WebSocket as the browser/remote transport, and JSON-RPC 2.0 is transport-agnostic. | If browser clients need a different payload shape, we would carry two serialization schemas. |
