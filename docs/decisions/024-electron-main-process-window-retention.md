...
> **Type guidance:** Two-way door. The decision is to keep a single `let mainWindow` module-scope reference. Reversal is a ≤5-line edit (remove the declaration + lint-disable + comment). No migration. Reversal cost is functionally zero and the blast radius is the single `apps/desktop/src/main/index.ts` file. The Antithesis, Synthesis and Failure Mode sections are here anyway, because they carry the measurement that falsifies the usual reason for keeping the reference.

---

## Context

The desktop shell main entrypoint at `apps/desktop/src/main/index.ts` creates exactly one `BrowserWindow` instance inside the `app.whenReady().then(...)` callback. The window is held by a module-scope `let` (declared next to the smoke-probe constants) with a `closed`-handler that nulls the reference back.

The widely repeated reason for that pattern is a garbage-collection claim: losing every JS reference to a window lets V8 collect it, which closes the window, which fires `window-all-closed` and quits the app — the desktop shell disappearing shortly after launch. This ADR records that the claim is false for the Electron we ship, and why the reference is kept regardless.
...
No test could be written that failed when the module-scope reference was removed. Designing one required first establishing that the failure mode is real, and the measurement below found that it is not.
...
If the wrapper cannot be collected while the native object lives, and `window-all-closed` fires from native-object removal rather than wrapper collection, then `let mainWindow` does not prevent the failure mode it is usually credited with preventing. "Losing all JS references allows the window to be garbage-collected" is not the operative mechanism in Electron 41.6.1.

### Synthesis — Why It Still Holds

The antithesis is empirically correct about Electron 41.6.1's mechanism. Two things follow from it:

1. **The header comment above the `let mainWindow` declaration** cites this ADR and names `BaseWindow::self_ref_` as the load-bearing mechanism. The user-side reference is defensive consistency with the canonical Electron community pattern, not the GC anchor.
2. **The Spec-021 acceptance criterion** encodes the observable lifecycle invariant directly — a `queryObjects(BrowserWindow)` count that holds stable across the probe's GC cycles and drops by at least one per window once every window is closed, with `window-all-closed` not firing during the probe iteration loop. It does not assert that the test fails when the module-scope reference is removed, because that is false in Electron 41.6.1; and it does not settle for a bare `>= 1`, which the fixed non-instance match satisfies with the instance gone.
...
- **Reversal cost:** ~5 minutes. Delete the `let mainWindow` declaration (1 line), the `closed`-handler nulling (3 lines), and the comment block (10 lines). Point this ADR's status at the replacement, or mark it `deprecated`. Update `apps/desktop/src/main/index.ts` to reference the new mechanism inline.
- **Blast radius:** Single file (`apps/desktop/src/main/index.ts`). Later multi-window lifecycle work that derives from this file would need to be re-derived from the new mechanism documentation, but no other file in the repo references `mainWindow` directly (verified by `grep -rn "mainWindow" --include="*.ts" .`).
...
| Future contributor reads the user-side reference, infers the false "this prevents GC" mechanism, propagates the belief to a derived doc | Medium — the comment is the only thing standing between a reader and the folklore | Low-Medium (no runtime impact; documentation drift) | The next edit to `apps/desktop/src/main/index.ts` reads the header comment | The comment cites ADR-024 inline; ADR-024 captures the falsification. The cite is the safeguard. |
...
- **Honest spec.** [Spec-021 §Acceptance Criteria](../specs/021-desktop-shell-and-renderer.md#acceptance-criteria) now encodes the observable lifecycle invariant directly (not the falsified "must FAIL when removed" predicate), and cites [ADR-024 §Antithesis — The Strongest Case Against](./024-electron-main-process-window-retention.md#antithesis--the-strongest-case-against) for the empirical mechanism truth.
...
- **Empirical no-op in the current fix-state.** The `let mainWindow` declaration does not prevent the failure mode it is credited with preventing. We carry one `let`, one comment, one lint-disable for asymmetric-risk reasons, not for current-fix-state mechanism reasons.
...
- Whether a multi-window app introduces a window that _is_ user-anchor-dependent — a transient settings window that does not go through `BaseWindow::self_ref_` for some reason we have not yet found. Reading the same Electron sources for the second-window case is the mitigation, and it happens when a second window lands.
