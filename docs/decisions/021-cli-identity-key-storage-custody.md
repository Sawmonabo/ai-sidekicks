...
| 2026-05-31 | Amended | Tier-5 plan-readiness-audit Codex review (PR #129): reconciled the tier-2 primitive-stack naming from `libsodium` to the actual `@noble` stack selected by [ADR-010](./010-paseto-webauthn-mls-auth.md) — `@noble/ciphers` (XChaCha20-Poly1305 AEAD) + `@noble/hashes` (`argon2id` KDF), both Cure53-audited, with `@noble/curves` additionally audited by Kudelski Security and Trail of Bits (Spec-008:111; Plan-022:223). The prior `libsodium` naming conflated the audited algorithms with the C library; `sodium-native` (a libsodium binding) stays a dependency **only** for `sodium_mlock` / `sodium_memzero` memory hygiene (Plan-022), never the file-tier cipher or KDF. No decision content changed — the OS-keystore → Argon2id-file → refuse ladder and every invariant are unchanged; this is a naming reconciliation, so the ADR stays `accepted`. |
