...
1. **Local daemon** -- Socket reachability plus a required 256-bit session token (mode 0600, rotated per daemon restart) presented by the Desktop Shell and CLI clients. The renderer is not a direct daemon client — all renderer-originated requests are brokered by the shell via the preload bridge. See [security-architecture.md §Local Daemon Authentication](../architecture/security-architecture.md#local-daemon-authentication-task-51) for the full authentication model. No network auth required.
...
3. **Relay (E2EE)** -- V1 ships pairwise X25519 ECDH + XChaCha20-Poly1305 via the audited `@noble/curves` and `@noble/ciphers` libraries. Each session establishes **ephemeral X25519 key pairs** per user (generated at session start, zeroed at session end), authenticated by each user's long-term Ed25519 identity key. A per-session symmetric key is derived via HKDF-SHA256 from the ECDH shared secret and used with XChaCha20-Poly1305 (24-byte random nonce, 16-byte authentication tag). This design provides **session-granularity forward secrecy** — compromise of the long-term Ed25519 identity key does not reveal past session keys because the X25519 material is discarded at session end. Messaging Layer Security (MLS, RFC 9420) via an audited implementation is the V1.1 upgrade path and will replace the pairwise layer once promotion gates are met (see Assumption #2 and Success Criteria). V1 pairwise sessions are capped at ≤10 active users to keep the N² fan-out cost bounded.
...
### Option E: Long-term X25519 per user with no ephemeral handshake (Rejected)

- **What:** Each user holds a long-term X25519 key; session traffic is encrypted with a symmetric key derived directly from the long-term ECDH shared secret.
- **Why rejected:** No forward secrecy. Compromise of any user's long-term X25519 key decrypts all past and future sessions with that peer. Option A's ephemeral-per-session design eliminates this failure mode at negligible additional cost.
...
- **Migration path:** Introduce a second auth backend behind a version-tagged token issuer, re-issue tokens during a deprecation window. For the V1 → V1.1 relay E2EE upgrade, add MLS behind a feature flag, run it in parallel with the pairwise layer during the soak period, negotiate cipher suite at session start based on user support, and promote MLS to default once the Success Criteria gates pass.
...
- V1 pairwise layer scales O(N²) in per-message work: sender encrypts plaintext once per recipient. V1 user cap (≤10) bounds the cost; MLS tree-based ratcheting at V1.1 reduces this to O(log N)
...
The in-house path at `packages/crypto-paseto/` avoids both failure modes by standing on two independently-audited primitive libraries (`@noble/curves` — audited by Cure53, Kudelski Security, and Trail of Bits; `@noble/ciphers` — audited by Cure53), which are already the V1 relay cryptographic adapter at Decision §3.
...
| 2026-04-19 | Amended | Added §PASETO v4 Implementation Library documenting the 2026-04-19 evaluation of `panva/paseto` (archived 2025-03-29, `v4.local` not implemented) and `paseto-ts` (single-maintainer, unaudited); recorded the in-house `packages/crypto-paseto/` as the authoritative implementation. Assumption #1 Evidence column updated to cite the new section (BL-067 resolution). |
| 2026-07-09 | Amended | The cross-node artifact relay ([Spec-014 §Cross-Node Artifact Relay (V1)](../specs/014-artifacts-files-and-attachments.md#cross-node-artifact-relay-v1), Codex round-2 hardening) introduces a **long-term artifact-encryption X25519 key class** — per user per node, private half under the [Spec-022](../specs/022-data-retention-and-gdpr.md) daemon master-key custody, public half distributed as an Ed25519-identity-signed attestation over the established pairwise-encrypted relay channels (a post-handshake control exchange; the `SessionKeyBundle` negotiation contract is unchanged). Artifact CEKs wrap to these durable keys because the session-ephemeral X25519 keys this ADR mandates are zeroed at session end — wrapping relay-held CEKs to them would orphan every pinned artifact on restart. Option E's rejection (no long-term X25519) is **session-traffic-scoped and stands**: relay event traffic keeps ephemeral pairwise keys and session-granularity forward secrecy; the artifact key class covers only durable-by-design artifact envelopes, with exposure bounded by relay TTL + refcount GC + crypto-shred. Spec-014 is authoritative for the artifact key model. |
| 2026-08-12 | Amended | Item 2 (control plane) clarified — scope only, no decision change, enumeration unchanged. The PASETO classes item 2 enumerates are the **control-plane** token classes (access `v4.public`, refresh `v4.local`). Cross-node dispatch introduces a **third, session-local class defined by [Spec-024 §Cross-Node Dispatch Request](../specs/024-cross-node-dispatch-and-approval.md#cross-node-dispatch-request), not by this ADR**: a dispatch-scoped `v4.public` token signed by the **caller's user identity key** (default expiry `created_at + 60s`, max 300 s; `req_hash` binding the canonical request body, `aud = target_node_id`, `cnf.jkt` DPoP thumbprint per RFC 9449), minted by the calling daemon and verified by the target daemon against the caller's roster-held long-term public key. It is never issued by, presented to, or refreshed through the control plane, and rides no OAuth 2.1 / RFC 8628 grant — so item 2's mechanisms neither cover nor constrain it. Spec-024 is authoritative for the dispatch token. Recorded by the Tier-9 plan-readiness audit ([Plan-027](../plans/027-cross-node-dispatch-and-approval.md)). |
| 2026-08-12 | Amended | Option-3 onboarding reconciliation recorded — scope only, no decision change. [Spec-026](../specs/026-first-run-onboarding.md)'s hosted-SaaS sign-up ([Plan-026](../plans/026-first-run-onboarding.md)) obtains a **one-shot bootstrap credential** over the RFC 8252 §7.3 loopback redirection path [ADR-020 §First-Run UX](./020-v1-deployment-model-and-oss-license.md#first-run-ux) permits; that scoped token is exchanged for a control-plane session on the control-plane join path, which is where item 2's three mechanisms bind — Device Authorization Grant (RFC 8628), DPoP sender-constraining, and refresh-token rotation. First-run onboarding therefore neither implements nor weakens them, and Plan-026 §Non-Goals now scopes all three out explicitly. Item 2 is unchanged. Recorded by the Tier-9 plan-readiness audit ([Plan-026](../plans/026-first-run-onboarding.md)). |
