...
| 2026-07-09 | Amended | The cross-node artifact relay ([Spec-012 §Cross-Node Artifact Relay (V1)](../specs/012-artifacts-files-and-attachments.md#cross-node-artifact-relay-v1), Codex round-2 hardening) introduces a **long-term artifact-encryption X25519 key class** — per user per node, private half under the [Spec-020](../specs/020-data-retention-and-gdpr.md) daemon master-key custody, public half distributed as an Ed25519-identity-signed attestation over the established pairwise-encrypted relay channels (a post-handshake control exchange; the `SessionKeyBundle` negotiation contract is unchanged). Artifact CEKs wrap to these durable keys because the session-ephemeral X25519 keys this ADR mandates are zeroed at session end — wrapping relay-held CEKs to them would orphan every pinned artifact on restart. Option E's rejection (no long-term X25519) is **session-traffic-scoped and stands**: relay event traffic keeps ephemeral pairwise keys and session-granularity forward secrecy; the artifact key class covers only durable-by-design artifact envelopes, with exposure bounded by relay TTL + refcount GC + crypto-shred. Spec-012 is authoritative for the artifact key model. |
| 2026-08-12 | Amended | Item 2 (control plane) clarified — scope only, no decision change, enumeration unchanged. The PASETO classes item 2 enumerates are the **control-plane** token classes (access `v4.public`, refresh `v4.local`). Cross-node dispatch introduces a **third, session-local class defined by [Spec-022 §Cross-Node Dispatch Request](../specs/022-cross-node-dispatch-and-approval.md#cross-node-dispatch-request), not by this ADR**: a dispatch-scoped `v4.public` token signed by the **caller's user identity key** (default expiry `created_at + 60s`, max 300 s; `req_hash` binding the canonical request body, `aud = target_node_id`, `cnf.jkt` DPoP thumbprint per RFC 9449), minted by the calling daemon and verified by the target daemon against the caller's roster-held long-term public key. It is never issued by, presented to, or refreshed through the control plane, and rides no OAuth 2.1 / RFC 8628 grant — so item 2's mechanisms neither cover nor constrain it. Spec-022 is authoritative for the dispatch token. Recorded by the Tier-8 plan-readiness audit ([Plan-024](../plans/024-cross-node-dispatch-and-approval.md)). |
| 2026-08-12 | Amended | Option-3 onboarding reconciliation recorded — scope only, no decision change. [Spec-023](../specs/023-first-run-onboarding.md)'s hosted-SaaS sign-up ([Plan-023](../plans/023-first-run-onboarding.md)) obtains a **one-shot bootstrap credential** over the RFC 8252 §7.3 loopback redirection path [ADR-020 §First-Run UX](./020-v1-deployment-model-and-oss-license.md#first-run-ux) permits; that scoped token is exchanged for a control-plane session on the control-plane join path, which is where item 2's three mechanisms bind — Device Authorization Grant (RFC 8628), DPoP sender-constraining, and refresh-token rotation. First-run onboarding therefore neither implements nor weakens them, and Plan-023 §Non-Goals now scopes all three out explicitly. Item 2 is unchanged. Recorded by the Tier-8 plan-readiness audit ([Plan-023](../plans/023-first-run-onboarding.md)). |
