...
1. **Local daemon** -- Socket reachability plus a required 256-bit session token (mode 0600, rotated per daemon restart) presented by the Desktop Shell and CLI clients. The renderer is not a direct daemon client — all renderer-originated requests are brokered by the shell via the preload bridge. See [security-architecture.md §Local Daemon Authentication](../architecture/security-architecture.md#local-daemon-authentication) for the full authentication model. No network auth required.
2. **Control plane** -- PASETO v4 tokens: access tokens (v4.public, 15 min), refresh tokens (v4.local, 7 days, rotated on use). OAuth 2.1 + PKCE mandatory. DPoP sender-constraining for access tokens. Device Authorization Grant (RFC 8628) for CLI (launches browser for primary auth). WebAuthn/Passkeys added at desktop launch. Cross-node dispatch uses a third, session-local token class that this ADR does not define: a dispatch-scoped `v4.public` token signed by the caller's own user identity key, minted by the calling daemon and verified by the target daemon against the caller's roster-held public key. It is never issued by, presented to, or refreshed through the control plane and rides no OAuth 2.1 or RFC 8628 grant; [Spec-022 §Cross-Node Dispatch Request](../specs/022-cross-node-dispatch-and-approval.md#cross-node-dispatch-request) is authoritative for it.
3. **Relay (E2EE)** -- V1 ships pairwise X25519 ECDH + XChaCha20-Poly1305 via the audited `@noble/curves` and `@noble/ciphers` libraries. Each session establishes **ephemeral X25519 key pairs** per user (generated at session start, zeroed at session end), authenticated by each user's long-term Ed25519 identity key. A per-session symmetric key is derived via HKDF-SHA256 from the ECDH shared secret and used with XChaCha20-Poly1305 (24-byte random nonce, 16-byte authentication tag). This design provides **session-granularity forward secrecy** — compromise of the long-term Ed25519 identity key does not reveal past session keys because the X25519 material is discarded at session end. Messaging Layer Security (MLS, RFC 9420) via an audited implementation is the V1.1 upgrade path and will replace the pairwise layer once promotion gates are met (see Assumption #2 and Success Criteria). V1 pairwise sessions are capped at ≤10 active users to keep the N² fan-out cost bounded. Durable artifact envelopes use a separate **long-term artifact-encryption X25519 key class** — one per user per node, private half under daemon master-key custody, public half distributed as an Ed25519-identity-signed attestation over the pairwise-encrypted relay channel — because the session-ephemeral keys are zeroed at session end and wrapping relay-held content-encryption keys to them would orphan every pinned artifact on restart. Relay event traffic keeps the ephemeral pairwise keys and their session-granularity forward secrecy; [Spec-012 §Cross-Node Artifact Relay](../specs/012-artifacts-files-and-attachments.md#cross-node-artifact-relay-v1) is authoritative for the artifact key model.
...
- **Why rejected:** No forward secrecy. Compromise of any user's long-term X25519 key decrypts all past and future sessions with that peer. Option A's ephemeral-per-session design eliminates this failure mode at negligible additional cost. The rejection covers session traffic; the separate long-term artifact-encryption key class in §Decision item 3 covers durable-by-design artifact envelopes only, with exposure bounded by relay TTL, refcount GC, and crypto-shred.

## Assumptions Audit

| # | Assumption | Evidence | What Breaks If Wrong |
| --- | --- | --- | --- |
| 1 | PASETO v4 libraries for Node/TypeScript are production-ready and will remain maintained. | See §PASETO v4 Implementation Library: in-house `packages/crypto-paseto/` on audited primitives (`@noble/curves` + `@noble/ciphers`); third-party `panva/paseto` and `paseto-ts` were evaluated and rejected. The PASETO v4 protocol itself is stable; `paseto.io` is the protocol reference. | We would need to fork or migrate to a different token format, re-issuing all access and refresh tokens. |
