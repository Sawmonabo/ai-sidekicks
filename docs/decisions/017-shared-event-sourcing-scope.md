...
- a **peer daemon** cannot _soundly_ — heartbeats are observed only by the control plane ([Spec-003 §Default Behavior](../specs/003-runtime-node-attach.md#default-behavior)), so a peer signing `runtime_node.offline` under its own roster key would merely rubber-stamp the control plane's verdict, which is the exact forgeability §Node-Scope Anchor Witnessing names.

This is the **failure-detector model** the industry uses and [Spec-003 §References](../specs/003-runtime-node-attach.md#references) already cites: Kubernetes' node controller derives `NotReady`/`Unknown` from missed heartbeats and records it as a **status condition in the central datastore (etcd) — not as a node-authored event**; SWIM/Lifeguard treat a node's self-report as a detector _input_, never the verdict; SPIFFE/SPIRE treat revocation as **authority-issued**, never subject-declared.

**Gated to V1.1** (on the same node-identity trust anchor as §Node-Scope Anchor Witnessing — Plan-018 / Plan-003): the durable `runtime_node.degraded` / `runtime_node.revoked` / heartbeat-loss `runtime_node.offline` session events and their per-event payload-shape schemas (Plan-003 §CP-003-1). Once the control plane holds a roster-resolvable node-identity key it becomes a sound authenticated authority and emits these directly. **V2** realizes the same authority model over MLS (the ADR-010 upgrade): the control plane registers as an authorized `external_senders` entity ([RFC 9420 §12.1.8](https://www.rfc-editor.org/rfc/rfc9420)) that _proposes_ the lifecycle change and group members _commit_ it.

The approved-spec [Spec-003](../specs/003-runtime-node-attach.md#state-and-data-implications) obligation that "capability declarations and trust posture changes must be emitted as session events" is satisfied for **capability declarations** in V1 — those are daemon-authored (`capability_declared` / `capability_updated`) by the declaring node, a live roster member — and **criterion-gated for the trust-posture (`revoked`) durable event** to the V1.1 node-identity anchor, with the `runtime_node_attachments.state = revoked` coordination record carrying the operational trust truth in the interim.

**Reversibility.** Additive, identical to §Node-Scope Anchor Witnessing: no V1 behavior is removed; the V1 coordination record stands, and the V1.1 durable events are added when the node-identity primitive ships. Re-evaluation trigger: the node-identity registry shipping in Plan-018 / Plan-003.
...
| 2026-06-06 | Amended — server-derived runtime-node lifecycle events scoped | Plan-003 Phase-3 reconciliation: the three **server-derived** runtime-node transitions (`degraded`, heartbeat-loss `offline`, `revoked`) are V1 **coordination records** (`runtime_node_presence.health_state` / `runtime_node_attachments.state`); their durable `runtime_node.*` session events + payload schemas are gated to V1.1 on the §Server-Derived Runtime-Node Lifecycle Events node-identity anchor (no sound V1 author — control plane has no roster identity, the node is dead/untrusted, a peer would rubber-stamp the verdict). Daemon-authored runtime-node events (the 5 daemon-reachable, incl. explicit-shutdown `offline` + capability declarations) unchanged. Core Option B decision unchanged; ADR stays `accepted`. |
| 2026-07-02 | Amended — local-log-authoritative resume + forward rollback event | Capability-enhancement campaign (B8). **P1-5**: on resume, when the provider's reported session position diverges from the daemon's recorded position, **the local log is authoritative** — the per-daemon `session_events` table with its monotonic per-session sequence (§Decision) is the source of truth; the reconciliation semantics (divergence ⇒ HALT-FOR-HUMAN, no silent re-emit and no silent discard) land in [Spec-015](../specs/015-persistence-recovery-and-replay.md) §Fallback via the campaign's B5 bundle (a named merge prerequisite before Plan-015 implements resume; not yet in the authoritative spec). **R8 rollback**: run time-travel (session time-travel, V1 feature #19 per [ADR-015](./015-v1-feature-scope-definition.md)) is modeled as a **forward** `run.rolled_back` event on the authoritative log — the log **never truncates and never rewrites**; provider-side rollback (the Codex conversation rewind / Claude fork-at-position) is an execution detail beneath the log. **The Codex half of that re-verification MUST is discharged (2026-08-26):** `thread/rollback` carries its own deprecation in the generated type, so Spec-005 rebinds the rollback grade onto the non-experimental successor `thread/fork` at an inclusive `lastTurnId` boundary and no driver registers the deprecated method ([`codex.md`](../reference/provider-wire/codex.md)). The Claude half stands: `--resume-session-at` is binary-probe-only, absent from the live CLI reference and known only from a binary string census (Binary probe provenance), so Plan-005 / Plan-010 MUST re-verify it against the then-installed binary before load-bearing use, per [`claude.md`](../reference/provider-wire/claude.md) §Gaps recorded. The daemon-side turn-snapshot leg is the durable file-restore path regardless, and rolled-back turns stay queryable history marked superseded by projection. `run.rolled_back` registers in the `run_lifecycle` family as **non-terminal** via the campaign's 2026-07-02 [Spec-006](../specs/006-session-event-taxonomy-and-audit-log.md) B1 amendment (in flight as PR #173 and not yet in the authoritative spec — a named merge prerequisite before any emitter exists); the [Spec-004](../specs/004-queue-steer-pause-resume.md) `rollback` intervention (campaign bundle B2, queued and not yet authored — a named merge prerequisite) consumes this ruling. Core Option B decision unchanged; ADR stays `accepted`. |
| 2026-07-13 | Amended — B1 + B5 + B2 merge-status reconciliation | Status reconciliation only (campaign B2, PR #205): the 2026-07-02 row's in-flight/queued/not-yet-in-spec gate clauses are superseded — the Spec-006 B1 amendment merged via PR #173, the [Spec-015](../specs/015-persistence-recovery-and-replay.md) B5 resume-divergence semantics merged 2026-07-06, and the [Spec-004](../specs/004-queue-steer-pause-resume.md) `rollback` intervention (campaign B2) merged via PR #205, so all three named merge prerequisites are satisfied; the residual gate for each amended spec is the W1.5 batch `review → approved` re-promotion (campaign Task 28). Rulings and core Option B decision unchanged; ADR stays `accepted`. |
| 2026-07-18 | Amended — W1.5 residual gate discharged | Status reconciliation only (campaign Task 28, W1.5 batch re-promotion): the 2026-07-13 row's residual-gate clause is superseded — [Spec-004](../specs/004-queue-steer-pause-resume.md), [Spec-006](../specs/006-session-event-taxonomy-and-audit-log.md), and [Spec-015](../specs/015-persistence-recovery-and-replay.md), with the other six W1-amended specs, re-promoted `review` → `approved` 2026-07-18 per the [runbook §Spec-Status Promotion Gate](../operations/plan-implementation-readiness-audit-runbook.md#spec-status-promotion-gate); implementation of the amended surfaces rides the campaign's W2 plan-task bundles. Rulings and core Option B decision unchanged; ADR stays `accepted`. |
| 2026-08-03 | Cross-reference — peer-to-joiner history backfill contracted on Spec-008 | Pointer row, no new semantics. The Decision's cross-participant distribution leg ("each receiving daemon decrypts the payload, validates it, and appends it to its own local log with its own per-session sequence number") had no contracted surface for a member who joins **after** the events were sent — pre-join relay ciphertext is undecryptable to them by design, and Option B leaves the control plane with no session log to serve. [Spec-008 §Peer History Backfill On Join (V1)](../specs/008-control-plane-relay-and-session-join.md#peer-history-backfill-on-join-v1) now contracts that leg: peers re-publish the joiner's entitled history from their per-daemon logs over the existing pairwise envelope — **each serving only the origin-signed rows it holds**, because this Decision's re-sequencing puts `sequence`, a signed canonical member, outside the origin's signature, so a received copy is re-signed by its receiver and unservable while origin-signed bytes live only where an origin daemon's own log lives — the joiner verifies each event against its **originating** daemon's roster-resolved key before appending, and the append itself is this Decision's receive path unchanged (own sequence, own chain, own signature) — so the §Negative (accepted trade-offs) framing is preserved verbatim: replay stays **reconstruction, not canonical read**, and the retained row attests receipt rather than origin authorship. No shared log, no new store, no ordering guarantee added; the single ordered cross-participant history remains Option A's V1.1 candidate. Core Option B decision unchanged; ADR stays `accepted`. |
| 2026-08-26 | Cross-reference — the ruling extended to the provider's own session store | Pointer row, no new semantics here. This ADR's 2026-07-02 row already settled that the local log is authoritative when a provider's reported session position diverges from the daemon's. [ADR-029](./029-canonical-transcript-is-authoritative.md) generalizes exactly that ruling from _position_ to _content_: a provider session is a replay target — derived, disposable, reconstructible from the canonical transcript — and no capability may depend on a provider-native continuity verb. Option B and the log's authority are unchanged; this ADR stays `accepted`. |
| 2026-08-26 | Discharged — the Codex half of the 2026-07-02 rewind re-verification MUST | Discharge row, no new semantics and no change to the forward-`run.rolled_back` ruling. The 2026-07-02 row bound Plan-005 / Plan-010 to re-verify `thread/rollback` "or its replacement" before driver code lands. Performed 2026-08-26: the deprecation is in the generated type itself, and [Spec-005 §Parity Capability Mechanism Grades](../specs/005-provider-driver-contract-and-capabilities.md#parity-capability-mechanism-grades) rebinds the leg onto the non-experimental successor `thread/fork` at an inclusive `lastTurnId` boundary — which mints a new thread, so the driver re-points the run's live provider binding rather than mutating one. The Claude half (`--resume-session-at`, binary-probe-only) is **not** discharged and its MUST stands. The log still never truncates; provider-side rewind remains an execution detail beneath it. |
