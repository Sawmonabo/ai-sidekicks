...
| 2026-05-31 | Amended — node-scope anchor witnessing scoped | Tier-4 plan-readiness audit (PR #129): daemon-scope (sentinel-partitioned) event chains are witnessed locally only in V1; control-plane upload deferred to V1.1, gated on a node-identity trust anchor + resolution surface (Plan-016 / Plan-002). Dissolves BL-137(c). Core Option B decision unchanged; ADR stays `accepted`. |
| 2026-06-06 | Amended — server-derived runtime-node lifecycle events scoped | Plan-002 Phase-3 reconciliation: the three **server-derived** runtime-node transitions (`degraded`, heartbeat-loss `offline`, `revoked`) are V1 **coordination records** (`runtime_node_presence.health_state` / `runtime_node_attachments.state`); their durable `runtime_node.*` session events + payload schemas are gated to V1.1 on the §Server-Derived Runtime-Node Lifecycle Events node-identity anchor (no sound V1 author — control plane has no roster identity, the node is dead/untrusted, a peer would rubber-stamp the verdict). Daemon-authored runtime-node events (the 5 daemon-reachable, incl. explicit-shutdown `offline` + capability declarations) unchanged. Core Option B decision unchanged; ADR stays `accepted`. |
| 2026-07-02 | Amended — local-log-authoritative resume + forward rollback event | Capability-enhancement campaign (B8). **P1-5**: on resume, when the provider's reported session position diverges from the daemon's recorded position, **the local log is authoritative** — the per-daemon `session_events` table with its monotonic per-session sequence (§Decision) is the source of truth; the reconciliation semantics (divergence ⇒ HALT-FOR-HUMAN, no silent re-emit and no silent discard) land in [Spec-013](../specs/013-persistence-recovery-and-replay.md) §Fallback via the campaign's B5 bundle (a named merge prerequisite before Plan-013 implements resume; not yet in the authoritative spec). **R8 rollback**: run time-travel (session time-travel, V1 feature #19 per [ADR-015](./015-v1-feature-scope-definition.md)) is modeled as a **forward** `run.rolled_back` event on the authoritative log — the log **never truncates and never rewrites**; provider-side rollback (the Codex conversation rewind / Claude fork-at-position) is an execution detail beneath the log. **The Codex half of that re-verification MUST is discharged (2026-08-26):** `thread/rollback` carries its own deprecation in the generated type, so Spec-004 rebinds the rollback grade onto the non-experimental successor `thread/fork` at an inclusive `lastTurnId` boundary and no driver registers the deprecated method ([`codex.md`](../reference/provider-wire/codex.md)). The Claude half stands: `--resume-session-at` is binary-probe-only, absent from the live CLI reference and known only from a binary string census (Binary probe provenance), so Plan-004 / Plan-008 MUST re-verify it against the then-installed binary before load-bearing use, per [`claude.md`](../reference/provider-wire/claude.md) §Gaps recorded. The daemon-side turn-snapshot leg is the durable file-restore path regardless, and rolled-back turns stay queryable history marked superseded by projection. `run.rolled_back` registers in the `run_lifecycle` family as **non-terminal** via the campaign's 2026-07-02 [Spec-005](../specs/005-session-event-taxonomy-and-audit-log.md) B1 amendment (in flight as PR #173 and not yet in the authoritative spec — a named merge prerequisite before any emitter exists); the [Spec-003](../specs/003-queue-steer-pause-resume.md) `rollback` intervention (campaign bundle B2, queued and not yet authored — a named merge prerequisite) consumes this ruling. Core Option B decision unchanged; ADR stays `accepted`. |
| 2026-07-13 | Amended — B1 + B5 + B2 merge-status reconciliation | Status reconciliation only (campaign B2, PR #205): the 2026-07-02 row's in-flight/queued/not-yet-in-spec gate clauses are superseded — the Spec-005 B1 amendment merged via PR #173, the [Spec-013](../specs/013-persistence-recovery-and-replay.md) B5 resume-divergence semantics merged 2026-07-06, and the [Spec-003](../specs/003-queue-steer-pause-resume.md) `rollback` intervention (campaign B2) merged via PR #205, so all three named merge prerequisites are satisfied; the residual gate for each amended spec is the W1.5 batch `review → approved` re-promotion (campaign Task 28). Rulings and core Option B decision unchanged; ADR stays `accepted`. |
| 2026-07-18 | Amended — W1.5 residual gate discharged | Status reconciliation only (campaign Task 28, W1.5 batch re-promotion): the 2026-07-13 row's residual-gate clause is superseded — [Spec-003](../specs/003-queue-steer-pause-resume.md), [Spec-005](../specs/005-session-event-taxonomy-and-audit-log.md), and [Spec-013](../specs/013-persistence-recovery-and-replay.md), with the other six W1-amended specs, re-promoted `review` → `approved` 2026-07-18 per the [runbook §Spec-Status Promotion Gate](../operations/plan-implementation-readiness-audit-runbook.md#spec-status-promotion-gate); implementation of the amended surfaces rides the campaign's W2 plan-task bundles. Rulings and core Option B decision unchanged; ADR stays `accepted`. |
...
| 2026-08-26 | Discharged — the Codex half of the 2026-07-02 rewind re-verification MUST | Discharge row, no new semantics and no change to the forward-`run.rolled_back` ruling. The 2026-07-02 row bound Plan-004 / Plan-008 to re-verify `thread/rollback` "or its replacement" before driver code lands. Performed 2026-08-26: the deprecation is in the generated type itself, and [Spec-004 §Parity Capability Mechanism Grades](../specs/004-provider-driver-contract-and-capabilities.md#parity-capability-mechanism-grades) rebinds the leg onto the non-experimental successor `thread/fork` at an inclusive `lastTurnId` boundary — which mints a new thread, so the driver re-points the run's live provider binding rather than mutating one. The Claude half (`--resume-session-at`, binary-probe-only) is **not** discharged and its MUST stands. The log still never truncates; provider-side rewind remains an execution detail beneath it. |
