...
- **Attribution in the review surface is per file and per turn, never per line.** Each file row names the newest turn that touched it beside its counts, and a commit row carries the mark and name of the sidekick that made it. No gutter carries per-line authorship and no file row takes a colour bar for who changed it; the line-level provenance this spec records lives in the commit trailers, not in the pane's chrome.
...
- While a run is live every control that would reach the sidekick waits, with the reason in its own tooltip.
...
- A file the sidekick edited more than once in one turn reaches the surface as ONE file: the daemon composes the edits into a single patch with true line numbers and one pair of counts, and the surface draws one row and one body for that path.
