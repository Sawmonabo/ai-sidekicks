...
| `driver_ask.canceled` | The ask was withdrawn — the provider retracted it, the run ended first, or a rollback rewound the turn that raised it ([Spec-004 §Required Behavior](./004-queue-steer-pause-resume.md#required-behavior), campaign B2). |
...
| `approval.canceled` | An approval request was canceled because its run ended before resolution, or because a rollback rewound the turn that raised it ([Spec-004 §Required Behavior](./004-queue-steer-pause-resume.md#required-behavior), campaign B2). |
