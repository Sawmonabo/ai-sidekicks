...
| `driver_ask.canceled` | The ask was withdrawn — the provider retracted it, the run ended first, the session closed first (Plan-012 T2.12's sweep, campaign B13), or a rollback rewound the turn that raised it ([Spec-004 §Required Behavior](./004-queue-steer-pause-resume.md#required-behavior), campaign B2). |
