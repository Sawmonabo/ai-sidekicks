...
| `audit_integrity_failed` | A read-side verifier detected a chain break, signature failure, anchor mismatch, or post-compaction integrity failure. Halts replay at the affected row and must be surfaced to operators. Promoted from §Integrity Events with category corrected from `session_lifecycle` → `audit_integrity`. The three `anchor_missing_for_compacted_range` / `anchor_signature_invalid` / `stub_signature_invalid` modes are additive-MINOR extensions for the post-compaction integrity protocol per §Post-Compaction Integrity. | base + `{treeSize, expectedRootHash, observedRootHash, failureMode ∈ ['hash_mismatch','signature_mismatch','anchor_mismatch','inclusion_proof_failed','consistency_proof_failed','log_file_missing','log_file_moved','anchor_missing_for_compacted_range','anchor_signature_invalid','stub_signature_invalid'], failurePath ∈ ['inclusion','consistency','signature'], offendingSeq?, detail}` |
...
**Verifier semantics on compacted rows.** For rows with `retention_class = 'audit_stub'`, the verifier runs **two** checks (both must pass):
...
A range verifies only when every compacted row passes both checks. For mixed ranges (some uncompacted, some `audit_stub`), the verifier performs per-row chain recomputation on the uncompacted prefix and the two compacted-row checks on the compacted suffix; all must pass for the range as a whole to verify.
...
- _Edit the visible stub_ (`summary`, `actor`, …): the modified `payload` bytes no longer match `stub_signature` → `stub_signature_invalid`. (This is the gap the prior anchor-only design missed: the anchor commits to the original `row_hash`, not to the post-compaction stub bytes.)
...
The three `failureMode` enum values added for the post-compaction protocol (`'anchor_missing_for_compacted_range'`, `'anchor_signature_invalid'`, `'stub_signature_invalid'`) are additive-MINOR extensions to the canonical 7-value enum at [§Audit Integrity (`audit_integrity`)](#audit-integrity-audit_integrity) per [ADR-018 §Decision #8](../decisions/018-cross-version-compatibility.md) (MINOR additive enum-value extension is sanctioned), taking the enum to 10 values. **Security amendment (approved spec):** the `stub_signature` per-row commitment was added 2026-05-28 in response to a P1 review finding (PR #124) that the prior anchor-only post-compaction design left the visible stub bytes unauthenticated; it is recorded here as a deliberate amendment to an approved spec, not a routine editorial change.
