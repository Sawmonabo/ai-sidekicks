...
- Execution success (`.executed`, `.completed`): `+ {approvalRecordRef, resultRef?}` — `resultRef` points to the local dispatch-result record; full result payload is not carried inline. The caller-side terminal observation (`.result_observed`) carries `+ {outcome: 'completed' | 'failed' | 'expired' | 'rejected', approvalRecordRef?, resultRef?, reason?}` — `outcome: 'completed'` is the result-delivery case (`resultRef` present); `'failed'` / `'expired'` record a relayed terminal-failure envelope (its `reason` carried through); `'rejected'` records the `DispatchReceive` rejection ack (campaign B7, 2026-07-13 — one caller-side observation event per target terminal; `.denied` is observed via `.approval_observed`).
