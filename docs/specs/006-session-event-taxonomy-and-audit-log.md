...
**Binding rule (event-level).** An event is emitted with `session_id` set to the reserved sentinel **iff its payload carries no real session context**. The discriminator is the event, not the category — a category may contain both shapes. The daemon-scope (sentinel-bound) set is:
