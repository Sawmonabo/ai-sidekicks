...
An audit stub retains the fields below — and, for a row whose source payload carries the cross-cutting `sourceEpoch` epoch-attribution field (§Event Type Enumeration), the `sourceEpoch` + `sourcePosition` pair together with the row's `runId`, and, for an accepted `run.rolled_back` boundary row, the `runId` + `runVersion` + `targetPosition` rewind cutoff — each preserved verbatim inside the stub projection per the compaction paragraph below:
