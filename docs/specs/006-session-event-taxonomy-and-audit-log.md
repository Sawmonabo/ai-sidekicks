...
This section enumerates every individual event type within each `EventCategory`. Each type is a string value for the `type` field of `EventEnvelope`. The payload fields listed are the category-specific fields carried inside the `payload` object; the envelope-level fields (`id`, `sessionId`, `sequence`, `occurredAt`, `category`, `actor`, `correlationId`, `causationId`, `version`) are always present and are not repeated here. One **cross-cutting payload field** spans categories (registered 2026-07-20 — the campaign-B9 CP-004-12 epoch-attribution carrier whose typed shape [Spec-004 §Required Behavior](004-queue-steer-pause-resume.md#required-behavior) delegates to this spec's plan bundle): `sourceEpoch?` — a nonnegative integer naming the **pre-rollback execution epoch** a late-appended **non-lifecycle** row is attributed to, stamped at ingestion by the run-engine's late-append leg (Plan-004 T3.11) exactly when the per-binding epoch cursor attributes the row to a superseded epoch (the atomic closed-pair `driver_ask.requested` + `driver_ask.canceled` late-append included), and absent on every current-epoch row — absence means current-epoch, and the stamp is never fabricated at read time. The field is admitted by the payload schemas of exactly the run-scoped families that late-append window covers — `assistant_output`, `tool_activity`, `usage_telemetry`, `artifact_publication`, and the `interactive_request` closed pair — and by no others (a `run_lifecycle` straggler is absorbed, never appended; non-run-scoped families carry no run epoch): every canonical `SessionEventSchema` payload branch of those five families, present now or registered later through the union-registration seam, MUST admit the optional field via the Plan-006 T1.9 fragment, so a stamped row validates through the canonical read path and is never rejected at subscription or replay validation. The field lives inside `payload`, so it sits in the §Canonical Serialization Rules canonical bytes — signed, hash-chained, and shred-safe like any payload field, with **no envelope-level field added** (the canonical-set membership is unchanged) and no version bump: this registration precedes [ADR-018 §Reversibility Assessment](../decisions/018-cross-version-compatibility.md#reversibility-assessment)'s point of no return — no non-test `"1.0"` emit exists as of 2026-07-20, the emitting phases being undispatched — so `sourceEpoch` is part of the v1.0 baseline payload contract from first emit, whereas the same field added after that point would be a MINOR envelope bump per `ADR-018 §Decision` #8's new-optional-field rule composed with §EventEnvelope Version Semantics' higher-MINOR preservation — and replays durably: the [Spec-004 §Driver-Level Rollback Mechanics](004-queue-steer-pause-resume.md#driver-level-rollback-mechanics) supersede projection consumes it for cross-epoch keying, and the audit-stub projection preserves it at compaction (§Compacted Event Format). Execution-epoch semantics — `0` before any rollback, advancing with each **accepted `run.rolled_back`** (a confirmed conversation rewind, regardless of the file-leg disposition) — are owned by [Spec-004 §Required Behavior](004-queue-steer-pause-resume.md#required-behavior) and [Run State Machine §Invariants](../domain/run-state-machine.md#invariants), not redefined here; the typed Zod shape (`SourceEpochSchema` + `SOURCE_EPOCH_PAYLOAD_KEY`) is [Plan-006](../plans/006-session-event-taxonomy-and-audit-log.md) T1.9's, in `packages/contracts/src/event.ts`, mirrored in [API Payload Contracts](../architecture/contracts/api-payload-contracts.md).
...
An audit stub retains the fields below — and, for a row whose source payload carries the cross-cutting `sourceEpoch` epoch-attribution field (§Event Type Enumeration), that field together with the row's `runId` and turn-position payload keys, and, for an accepted `run.rolled_back` boundary row, the `runId` + `runVersion` + `targetPosition` rewind cutoff — each preserved verbatim inside the stub projection per the compaction paragraph below:
