...
Lifecycle discipline: five terminal events — `.rejected`, `.denied`, `.failed`, `.expired`, `.completed` — are the **only** events that terminate a dispatch's lifecycle. Exactly one fires per `dispatchId`. Intermediate events may fire in sequence before the terminal event. `.executed` is intermediate: it records that the capability handler finished the action; the lifecycle is not terminated until `.completed` (target-side success: the capability handler finished and the result is ready for emission) or `.failed` (post-approval execution fault) fires. Result *delivery* to the caller is tracked separately from lifecycle termination: live-delivered results produce caller-side `.result_observed` after target `.completed`; if the caller was detached at `.completed` time, the target appends `.result_buffered` (a post-terminal delivery annotation, **not** a lifecycle state) and later delivery on caller reconnect produces caller-side `.result_observed`. The `.executed` / `.completed` distinction exists because audit reconstruction must be able to tell a successful-but-indeterminate-outcome (`.executed` with no terminal) from a fully-successful lifecycle (`.completed`); the `.completed` / delivery separation exists because Spec-024 §Execution And Result Emission Example Flow 3 explicitly allows the success-path lifecycle to terminate at the target *before* the caller reconnects and observes the result.
