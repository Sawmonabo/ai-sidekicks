...
| `dispatch.result_observed` | caller | Caller daemon observed the dispatch's terminal outcome and appended it to the local log; the payload's `outcome` field discriminates — `'completed'`: result payload received (live-delivered after target `.completed`, or via buffered-delivery after target `.result_buffered`); `'failed'` / `'expired'`: a relay-delivered terminal-failure envelope; `'rejected'`: the `DispatchReceive` rejection ack or a relayed `DispatchTerminalNotice` carrying a post-intake `dispatch.rejected` (Spec-024 §Interfaces And Contracts). Widened from result-payload-only by campaign B7 (2026-07-13) so each target terminal kind, **when the caller observes it**, maps to exactly one caller-side observation event (`.denied` is observed via `.approval_observed`); an observation closes the caller-local pending-dispatch window, while a terminal the caller never observes (relay partition, unreachable target) appends nothing — the window then closes on the caller-local clock bound alone, never on a synthetic observation, per Spec-024 §Cross-Node Failure Semantics. |
