...
| `dispatch.result_observed` | caller | Caller daemon observed the dispatch's terminal outcome and appended it to the local log; the payload's `outcome` field discriminates — `'completed'`: result payload received (live-delivered after target `.completed`, or via buffered-delivery after target `.result_buffered`); `'failed'` / `'expired'`: a relay-delivered terminal-failure envelope; `'rejected'`: the `DispatchReceive` rejection ack. Widened from result-payload-only by campaign B7 (2026-07-13) so every target terminal has exactly one caller-side observation event (`.denied` is observed via `.approval_observed`); the caller-local pending-dispatch window closes on the same observation per Spec-024 §Cross-Node Failure Semantics. |
