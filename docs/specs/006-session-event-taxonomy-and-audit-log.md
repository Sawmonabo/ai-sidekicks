...
Lifecycle discipline: five terminal events — `.rejected`, `.denied`, `.failed`, `.expired`, `.completed` — are the **only** events that terminate a dispatch's lifecycle. Exactly one fires per `dispatchId`. Intermediate events may fire in sequence before the terminal event. `.executed` is intermediate and fires **only on successful handler completion** — it records that the capability handler finished the action and returned a result. A handler-side error, result-emission error, or post-approval runtime fault never produces `.executed`; those paths go straight to the `.failed` terminal. In the normal success path `.executed` is immediately followed by `.completed` (target-side success: the capability handler finished and the result is ready for emission); the `.executed` / `.completed` split exists so audit reconstruction can distinguish a lost-terminal bug case (`.executed` recorded but no subsequent terminal in the log, meaning the inter-step emit was lost or the daemon crashed mid-sequence) from a fully-successful lifecycle (`.executed` → `.completed`). Result *delivery* to the caller is tracked separately from lifecycle termination: live-delivered results produce caller-side `.result_observed` after target `.completed`; if the caller was detached at `.completed` time, the target appends `.result_buffered` (a post-terminal delivery annotation, **not** a lifecycle state) and later delivery on caller reconnect produces caller-side `.result_observed`. The `.completed` / delivery separation exists because Spec-024 §Execution And Result Emission Example Flow 3 explicitly allows the success-path lifecycle to terminate at the target *before* the caller reconnects and observes the result.
