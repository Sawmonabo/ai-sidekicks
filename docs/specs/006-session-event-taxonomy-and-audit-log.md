...
| Type | Description |
| --- | --- |
| `run.queued` | A run has been created and placed in the queue. For orchestration-created runs the payload additionally carries the optional linkage fields `{agentId?, parentRunId?, linkType?, internalHelper?, producingNodeId?}` plus the admission-resolved `effectiveRunConfig?` (Tier-6 audit, D-016-3/D-016-5) and, on native-cap unpriced admissions, the server-stamped `admittedUnpricedCapCents?` (campaign B6 — never client-suppliable; the replay source for active reservations and the worst-case terminal debit) — the durable source from which the `run_links` projection, the round-robin turn cursor, and the per-run budget/idle enforcement values rebuild on replay; threaded from the in-process `OrchestrationRunLinkCarrier` ([api-payload-contracts §Plan-016](../architecture/contracts/api-payload-contracts.md)). |
