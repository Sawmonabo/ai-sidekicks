...
An audit stub retains the fields below — and, for a row attributed to a superseded epoch/turn (its source payload carries the cross-cutting `sourceEpoch` stamp, or the attribution is established at compaction time for a rolled-back run's rewound tail — §Event Type Enumeration and the compaction paragraph below), the `sourceEpoch` + `sourcePosition` pair together with the row's `runId`, and, for an accepted `run.rolled_back` boundary row, the `runId` + `runVersion` + `targetPosition` rewind cutoff — each preserved verbatim inside the stub projection per the compaction paragraph below:
...
```
{
  ...
  runId?: RunId,        // REQUIRED on terminal run_lifecycle rows and posture-bearing run.running rows (backstop-index key), on accepted run.rolled_back rows (rollback-boundary key), and on sourceEpoch-stamped rows (epochs are run-local — campaign B9)
