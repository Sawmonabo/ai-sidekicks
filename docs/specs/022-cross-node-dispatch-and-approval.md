...
- Cross-node shared artifacts and file replication (V1 per [ADR-015](../decisions/015-v1-feature-scope-definition.md), but owned by [Spec-012 §Cross-Node Artifact Relay](./012-artifacts-files-and-attachments.md#cross-node-artifact-relay-v1), not this spec; cross-node artifact movement within a single dispatch is in scope here only as the artifact carried on a dispatch result).
...
- The caller-side pending-dispatch record backing the idle-exemption predicate — written before relay send, closed at conclusion, carrying `expires_at` and the originating `runId` — is likewise Plan-024-owned; its table and schema row land with that plan.
...
- **Transport parity.** Cross-node dispatch is the **Claude leg of remote-provider parity**: Claude Code exposes no local network listener, so "drive a Claude session on another machine" is delivered by dispatching the run to that node's daemon — the mesh is the emulation. The Codex driver's native config-gated transport axis ([Spec-004 §Default Behavior](./004-provider-driver-contract-and-capabilities.md#default-behavior)) is the complementary leg. Parity is graded per the capability-parity mechanism vocabulary (`native | emulated | absent`), not claimed boolean.
