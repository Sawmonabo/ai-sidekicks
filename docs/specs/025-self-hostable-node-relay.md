...
- **Cloudflare DO sharding envelope.** The hosted deployment uses Cloudflare Durable Objects with the Hibernation API; per Cloudflare's 2025-04 GA of SQLite-backed Durable Objects, one DO can hold up to 32,768 hibernating WebSockets. This number is relevant to [Deployment Topology](../architecture/deployment-topology.md) §Relay Scaling Strategy (hosted backend), **not** to this spec. If `deployment-topology.md` currently states a different number, that is a documentation drift to file separately; this spec's self-host path is sized for small-team deployments, not the DO ceiling.
...
| Cloudflare Durable Objects Hibernation API | Documentation | 32,768 hibernating WebSockets per DO; SQLite backend GA 2025-04-07 | https://developers.cloudflare.com/durable-objects/api/websockets/#websocket-hibernation-api |
