...
- [ ] A supplied path that does not exist, a path whose git metadata is present but unusable, a working tree whose registration is missing or does not name it back, a derived main-checkout root that fails its bidirectional verification (the linked worktree of a `--separate-git-dir` repository lands here), and a git pointer file that is not a regular file each fail repo attach explicitly rather than being attributed to an ancestor repository.
...
- The main-checkout root is DERIVED, then VERIFIED — never assumed. When the supplied path's own git directory IS the common directory (canonically equal), the enclosing working tree is the repository's main working tree and the canonical root is that tree's own top level; this is the arm a `--separate-git-dir` main checkout takes, and deriving from the common directory's filesystem location there would name an unrelated directory. When the two differ (a linked worktree), the candidate is the common directory's parent, accepted only when the common directory's basename is `.git` AND the candidate's own `.git` entry resolves back to that same common directory; a candidate failing either screen is refused with the typed root-resolution error, naming attach-from-the-main-checkout as the recovery. Git records no main-working-tree pointer for a `--separate-git-dir` repository — no `core.worktree` is written, and its own `git worktree list` reports the git directory itself as the main working tree ([git-worktree(1)](https://git-scm.com/docs/git-worktree)) — so a linked worktree of such a repository lands on exactly that refusal arm by design: identity for that repository is established by attaching its main checkout, after which the worktree binds through the registration arm of §Local Trust Envelope (V1 Definition).
