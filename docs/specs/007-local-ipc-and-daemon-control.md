...
- [ ] **AC-N2** (`session.read`) — handler returns a `SessionRead`-shape result for a known session id (per [Spec-001 §Interfaces And Contracts](./001-shared-session-core.md#interfaces-and-contracts)); an unknown id rejects with `-32602 InvalidParams` + `data.type: "session.not_found"` per [error-contracts.md](../architecture/contracts/error-contracts.md). Wire shape inherits [Spec-001 §Required Behavior](./001-shared-session-core.md#required-behavior). Structural invariants (I-007-6 duplicate-registration rejection at register-time + I-007-7 schema-validates-before-dispatch) inherit via the shared `router.register` substrate, proven by Plan-007 Phase 3 test ID `I-007-3-T5` against `session.create`'s identical binding shape; the per-method binding lives at `packages/runtime-daemon/src/ipc/handlers/session-read.ts`. _A dedicated `session.read` handler round-trip test is honest-deferred per [BL-117](../backlog.md) (not a BL-114 blocker; the substrate-symmetry argument satisfies the Phase 1-3 retroactive audit scope per BL-113 findings F-007-3-08 + F-007-3-10)._
- [ ] **AC-N3** (`session.join`) — handler appends a `session.joined` event to the session's event log; a second-client `SessionJoin` returns the same session id, existing membership state, and full event history per [Spec-001 AC4](./001-shared-session-core.md#acceptance-criteria). Wire shape inherits [Spec-001 §Interfaces And Contracts](./001-shared-session-core.md#interfaces-and-contracts). Structural invariants inherit via Plan-007 Phase 3 test ID `I-007-3-T5`'s substrate-symmetry; the per-method binding lives at `packages/runtime-daemon/src/ipc/handlers/session-join.ts`. End-to-end join-event observation is indirectly exercised through Plan-007 Phase 3 test ID `I-007-3-T3`'s subscribe streaming-primitive integration (a `session.joined` event reaching the subscribe-side observer would route through the same `sub.next(event)` validation path). _A dedicated `session.join` handler round-trip test (with a real `session.joined` event fixture replacing T3's `session.created` fixture) is honest-deferred per [BL-117](../backlog.md)._
