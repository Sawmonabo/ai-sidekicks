...
3. Main process resolves the display metadata needed to render a confirmation (target session identity, inviter identity) and emits a bridge event carrying that metadata plus an opaque invite reference — never the raw token — to the renderer
