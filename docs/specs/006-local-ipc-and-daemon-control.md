...
- A subscription delivers changes in frames rather than one message per change: at most 50 events or 16 ms of them to a frame, whichever comes first, the window opening on the first event so nothing waits longer than it, and every change in a frame carrying its own cursor. A consumer that falls behind is not waited for. The daemon drops for it, marks the gap on the next frame that fits, and the consumer repairs by reading again from its cursor — taking a snapshot read past the gap where the gap is further back than the record can fill — so a consumer is never silently short and a slow reader never holds the daemon up.
- The default local IPC transport must be OS-local:
  - Unix domain socket on Unix-like platforms
  - named pipe on Windows
- **The socket path has a hard length limit, so the daemon's run directory is short by design.** A Unix domain socket address is a fixed-size field in the operating system's own structure — the `sun_path` field of `sockaddr_un`, 104 bytes on macOS, 108 on Linux (`unix(7)`, read 2026-09-21) — and it is not a path the platform will truncate or redirect: a path past the limit simply cannot be bound. The failure is also badly reported by at least one client library above it, which surfaces it as a malformed address rather than as a length, so a caller reading the message looks for a typo in a path that is spelled correctly. The daemon therefore keeps its run directory short enough that the socket fits with room for the file name, and where it is handed a longer one it **refuses to bind and says the path is too long for a socket on this platform**, naming the limit and the length it was given, rather than passing the platform's own message through.
...
- Machine-wide daemon configuration is read and written through this surface, one place per value and no second one: how long diagnostic logs are kept, how long finished sessions and workflow-run step data are kept, the port a workflow started by a web request listens on, the time limit after which a run is ended, the memory cap for tool processes, `Max steps per turn`, and the two diagnostic switches — recording traces, and recording an event-replay log, which names the file it writes. Four of them carry a shape of their own: the run limit is no limit by default and is otherwise `30 minutes`, `1 hour`, `4 hours`, `12 hours` or `24 hours`; the memory cap is empty until a size is given, and empty means no cap; `Max steps per turn` reads `Unlimited` by default, which is what both providers do on their own, and carries one line saying what the number bounds and that a session can set its own number in its inspector — a number typed here governs sessions started later and reaches a session already running at its next turn, and `Unlimited` puts the providers' own behaviour back, what the bound does and how each provider realizes it being [Spec-003 §The Step Bound On A Turn](./003-queue-steer-pause-resume.md#the-step-bound-on-a-turn)'s and the per-session override's own write being [Spec-001 §Interfaces And Contracts](./001-session-core.md#interfaces-and-contracts)'s; and both diagnostic switches are off until they are turned on. Each value's own policy belongs to the spec that owns it: the diagnostic-log bound, and with it the warning the daemon records when that bound is set past thirty days, to [Spec-018 §PII in Diagnostics](./018-observability-and-failure-recovery.md#pii-in-diagnostics), the finished-session bound and the workflow-run step-data bound to [Spec-020 §Retention Policy](./020-data-retention-and-gdpr.md#retention-policy). What this surface owes is that the value is set in one place, that a value the operating system will not let the daemon enforce says so where it is set, and that a refused value — something that is not a port, something that is not a size — is refused in place with nothing written and the field left as it was; a port already taken is answered the same way, saying that nothing is listening, and the daemon never moves to another port of its own.
...
- **What the daemon brings back first.** Where a shared Codex service was running, the daemon starts that service again on the same socket before anything else it restores, so a terminal Codex session attached to it reattaches by itself — a session whose service is back within about five seconds reconnects on its own, and one is given up on at about twenty-four — and never notices the restart ([Codex wire reference §The shared `app-server` a terminal `codex` joins](../reference/provider-wire/codex.md#the-shared-app-server-a-terminal-codex-joins), measured 2026-09-21 at `0.155.1`). A session that has been without its service for longer than that window has given up and is not recovered by the daemon at all: it is the person's own process, and what brings it back is the provider's own resume ([Spec-014 §Sessions Talking To Each Other](./014-multi-agent-channels-and-orchestration.md#sessions-talking-to-each-other)).
- The daemon's own supervision facts are read by the surface that shows them and edited nowhere: whether it is running and since when, its version, and how much of the machine's processor and memory it is using — each reading stamped with the time it was taken and taken again only when it is asked for, never sampled on a timer. Stop and restart are the two supervised controls; each confirms before it acts and each says that work in flight on this machine stops, and, where the shared Codex service is running, the confirmation names the terminal Codex sessions inside it, because stopping the service strands them until the person runs the provider's own resume ([Spec-014 §Sessions Talking To Each Other](./014-multi-agent-channels-and-orchestration.md#sessions-talking-to-each-other)). The daemon is updated separately from the desktop app: the update shows the step it is on, offers a cancel for as long as the daemon still allows one, and settles by naming the version it came from and the one it reached, or both versions and the reason where it rolled back, or that this machine is already at the newest version.
