...
- Budget roll-up: subagent usage aggregates into the run's own budgets — the §Budget Policies accountant sees it through the run's `usage` events, so subagents provide no budget escape hatch. Derived pricing resolves **per usage row**: a row wire-attributed to a model outside the run's admitted family keys off THAT model's family, falling back to `admittedModelFamily` when the wire carries no attribution — a differently-modeled subagent is never priced under its parent's rates. And the admission-preemption rule above extends to spawn: `subagentPolicy.definitions[]` models resolve against the pricing table at spawn, and a definition naming an unpriced-family model on a capless leg is refused fail-closed (same `driver_capless` vocabulary) unless a native-cap reservation covers the run ([Spec-006 §Tool Activity](006-session-event-taxonomy-and-audit-log.md) records the same aggregation rule on `subagent.completed`, campaign B1).
