...
- [ ] A participant or node joining the session after publish receives the explicit `artifact.no_access_key` error (distinct from `artifact.fetch_unauthorized`; never a silent failure) and gains access when the online publisher re-publishes (idempotent re-pin adding recipient rows).
- [ ] Artifact-key generation and attestation happen at node attach (join-sequence obligation, never lazily at first fetch): every node whose attach completed before a publish appears in that publish's wrapped-key recipient set, with the publishing daemon stopped afterward.
...
- V1 decision (2026-07-09, Codex rounds 5–6): the relay fetch token is **`(participant, node)`-scoped**, the binding made at mint per RFC 9449 §5 — the issuer resolves `node_id` from the authenticated node-attach context of the `ArtifactFetchAuthorize` call, stamps `(participant, node)` as issuer-signed claims, and sets `cnf` from that call's DPoP proof (a bare `cnf`/jkt thumbprint is possession material, not an identity, so no thumbprint→node registry exists to maintain). Participant-only scoping was insufficient once recipient rows became per-node: the relay could neither select the correct wrapped CEK for a multi-node participant nor safely attribute an `ArtifactFetchComplete` ack, and a caller-supplied node id would let one compromised node mark its siblings delivered. The ack is idempotent, so a lost ack response is retry-safe.
...
- V1 decision (2026-07-09, Codex round-8): relay-side integrity is **digest-level only** — the signed chunk manifest and `cekCommitment` are E2E payload the relay never parses, so per-chunk, key-commitment, and plaintext verification are exclusively recipient-side; the relay's one keyless check is re-hashing the assembled ciphertext against the declared CAS digest at `ArtifactUploadComplete` (nothing pins unverified), and a re-pin re-verifies the stored copy before short-circuiting — so publisher re-publish also repairs at-rest corruption, closing the healing loop that recipients-detect-only verification would otherwise leave open.
