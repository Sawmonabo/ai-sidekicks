...
- [Spec-029: Provider Accounts And Credential Homes](./029-provider-accounts-and-credential-homes.md) — the node-local provider-account registry, the readiness derivation, and the vendor authentication-policy constraints that Group B composes and never re-specifies.
...
| [Spec-029: Provider Accounts And Credential Homes](./029-provider-accounts-and-credential-homes.md) | Provider-account registry, credential homes, the readiness projection Group B renders, fail-closed spawn validation, and the vendor authentication-policy constraints. |
