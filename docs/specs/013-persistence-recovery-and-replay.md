...
- If a restore has to skip a file, the dry run names the count before the restore runs and says which file and why — a symbolic link, a hard link, something that is not a file, a file whose folder moved — and the rest are still put back. A skip is never silent and never cancels the restore, and what a shell command wrote is reported as outside the checkpoint store rather than reported as restored.
...
- **A checkpoint holds one copy per content version of a file, and a file created since the checkpoint is recorded as new.** The dry run names such a file as one the restore will remove, and the restore removes it, so the working folder after a restore holds what it held at that point and nothing the agent added since.
...
- A session's file checkpoint copies live with that session outside any checkout, grow no table, are retained when the session is archived, and are deleted with the session when it is purged.
