...
- Secret custody internals — master-key custody, envelope formats, and tier-1/tier-2 storage rules are owned by `Spec-020 §Daemon Master Key`. This spec only states _that_ secrets are generated on first run.
...
- **Spec-020 interaction**: secrets generated in row 3 are the master key custody domain of `Spec-020 §Daemon Master Key`. Envelope format and tier-1/tier-2 storage rules are owned by that spec; Spec-024 only asserts _that_ they are generated on first run.
