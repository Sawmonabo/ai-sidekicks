...
- Authority split with the later persistence-hardening plan (backup internals) and with Spec-020 (secret custody internals).
...
- Backup internals — WAL checkpoint cadence, retention pruning, rotation atomicity, and crash recovery are owned by a later persistence-hardening plan. This spec only states _that_ backup is on by default.
...
| 6 | **SQLite online-backup job on by default** | Daemon enables SQLite online-backup via the online-backup API. Default cadence: session-end event + nightly full. Default destination: `./data/backups/` inside the daemon data dir. Directory MUST be `0700`. | `BACKUP_DIR=<path>`, `BACKUP_CADENCE=off`. Disabling backup MUST emit a loud banner and a `security.default.override=backup_disabled` log event on every startup. Plug-in point for S3/GCS/similar is specified by Plan-001 and the later persistence-hardening plan, not by this spec. | Data durability is a security property; silent session loss is a breach class. Default-off has failed in other OSS self-host projects. This spec states the default is **on**; the persistence-hardening plan owns how it works (see §Authority Split). | Plan-001 (SQLite writer) + the persistence-hardening plan (backup internals) |
...
Row 6 of §Required Behavior states _that_ SQLite online-backup is on by default. It does NOT state _how backup works_. The downstream persistence-hardening plan owns:
...
When this spec and the persistence-hardening plan conflict, that plan governs backup internals; this spec governs defaults. This separation keeps Spec-024 stable as an operator-facing declaration even as backup internals evolve.
...
- **Short-lived cert profile (row 1)**: LE's 6-day `shortlived` profile went GA 2026-01-15. Not adopted for V1 default because DNS-01 wildcard issuance under a 6-day profile means daily DNS writes, and not every DNS provider plugin is rate-limit-safe at that cadence. Plan-028 may expose `ACME_PROFILE=shortlived` as an opt-in.
...
- [ ] `/metrics` endpoint exposes all five documented row-9a daemon counter families in Prometheus v0.0.4 format when scraped on loopback. Scraping on a non-loopback bind without the configured auth credential — bearer token or client certificate — is rejected.
...
- **Behavior 6 plug-in point shape**: S3 / GCS / custom credential surface. Owned by the persistence-hardening plan + Plan-001; spec defers.
