...
- Mutations (`mcp.upsertServer`, `mcp.removeServer`, `mcp.setEnabled`) target a binding and MUST route through the provider-native mechanism for the target provider, per the capability table above. The only writable scope in V1 is `user` on both providers; a mutation targeting a `project` or `local` binding fails with `mcp.config_scope_unsupported` and actionable guidance naming the file to edit.
...
- **Claude, live leg (opportunistic):** when the running CLI/SDK pair satisfies the version floor (capability-probed at driver spawn, not assumed from the pin) in a streaming-input session, the daemon additionally applies the mutation live via `setMcpServers` and MUST reconcile the returned `{added, removed, errors}` against the requested delta; partial failures surface per-server, not as a blanket success. The call replaces the named-server set but keeps unnamed plugin-provided servers — the daemon MUST always send the full desired named-server set, never a delta. The response grade reports the most immediate application achieved: `live_reconcile` when the live leg applied (the durable leg having also completed), `user_config_write` otherwise.
...
- A trust grant MUST bind to the binding's current **base-config hash**: `b3:`-prefixed BLAKE3 over the RFC 8785 JCS canonicalization of the normalized server config (the same canonical-bytes discipline as [Spec-006 §Integrity Protocol](./006-session-event-taxonomy-and-audit-log.md#integrity-protocol)), computed **excluding the daemon-managed override-projection fields** (`enabled_tools` / `disabled_tools` / `tools.<t>.approval_mode` — the fields the daemon itself materializes for governed tool overrides, which carry their own Cedar-gated audit trail in the override store). Without the exclusion, the daemon's own governed override write would change the hash and revoke the very trust that authorized it. Trust means "the operator trusts _this_ configuration", not the name.
...
- **Revocation neutralizes weakening (atomic):** whenever trust is revoked — operator revoke or drift auto-revoke — safety-weakening override facets for that binding stop applying in the same operation: daemon-enforced weakenings (Claude approval modes, idempotency-class assignments everywhere) are conditioned on current trust at resolution time and lapse immediately, and Codex-materialized native weakening fields MUST be rewritten to the safe defaults before the revocation returns, emitting `mcp.tool_override_changed` for each reverted facet. A revoked server never keeps a weakened posture it was granted while trusted.
...
- Enforcement is provider-graded, and the override response reports it **per facet** (see the payload contract): for Codex, `enabled`/`approvalMode` facets are materialized into the native config fields (`enabled_tools` / `disabled_tools` / `tools.<t>.approval_mode`) on write — grade `user_config_write`; for Claude, the daemon enforces them at its own approval layer (Spec-012 evaluation sees the override before prompting) — grade `daemon_enforced`, effective immediately for subsequent evaluations. The `idempotencyClass` facet is provider-independent and always `daemon_enforced`: it feeds the Spec-005 tool-metadata resolution layer, which downstream consumers (recovery/replay per Spec-015) already read — consumers never read the override table directly. Safety-weakening facets are effective only while the binding's trust holds (see §Trust Governance — revocation neutralizes weakening).
...
- Every non-read operation (`mcp.upsertServer`, `mcp.removeServer`, `mcp.setEnabled`, `mcp.setTrust`, `mcp.setToolOverride`, `mcp.clearToolOverride`, `mcp.oauthLogin`, `mcp.reconnect` — eight of the ten) MUST be authorized through the Plan-012 Cedar evaluation surface (`PermissionCheckService`) under a dedicated `mcp` Cedar action family before any provider call or store write. Read operations (`mcp.list`, `mcp.get`) are operator-readable without a Cedar mutation check.
- Of the eight Cedar-gated operations, **seven are governance mutations** bound by the one-event-per-mutation audit invariant (the six config/trust/override mutations plus `mcp.oauthLogin`, whose audit event is `mcp.server_oauth_completed`). **`mcp.reconnect` is an operational command, not a governance mutation**: it changes no daemon store and no provider config, so it carries no dedicated governance event — its observable effect is audited through the `mcp.server_status_changed` transitions it induces, and a reconnect attempt that produces no status transition intentionally leaves no governance-event row (the attempt is still Cedar-evaluated, and its failures surface as typed errors on the response).
...
- **Non-user-scope mutation (either provider):** `mcp.config_scope_unsupported` with the resolved config-file path in the _message guidance only_ (never in event payloads).
...
- **IPC operations** (10, JSON-RPC per [ADR-009](../decisions/009-json-rpc-ipc-wire-format.md); registered in the Plan-007 `MethodRegistry` at Plan-028's tier per the `presence.*` late-namespace precedent): `mcp.list`, `mcp.get`, `mcp.upsertServer`, `mcp.removeServer`, `mcp.setEnabled`, `mcp.setTrust`, `mcp.setToolOverride`, `mcp.clearToolOverride`, `mcp.oauthLogin`, `mcp.reconnect`. Requests address the scope-qualified binding; `mcp.upsertServer` carries the fully-typed `McpServerConfigInput` union; mutation responses carry application grades (per-facet for overrides). Typed request/response mirrors live in [API Payload Contracts §Plan-028 — MCP Governance Contract Surfaces](../architecture/contracts/api-payload-contracts.md#plan-028--mcp-governance-contract-surfaces).
...
- **Two new daemon-local tables** (node-scoped; DDL in [Local SQLite Schema §MCP Governance Tables (Plan-028)](../architecture/schemas/local-sqlite-schema.md#mcp-governance-tables-plan-028)): `mcp_server_trust` (scope-qualified binding identity, `trusted`, base-config `b3:`-hash binding, grant/revoke provenance, and the Claude enabled overlay) and `mcp_tool_overrides` (per-tool facets, FK-cascaded to the trust row). SQLite census 52 → 54.
...
- **Claude composition model.** The daemon drives `claude mcp add-json` / `claude mcp remove` at `--scope user` as the sanctioned durable-write mechanism, and declares each run's server set via ephemeral config (`--mcp-config` + `--strict-mcp-config`) composed as a **snapshot of the observed user scope plus governance overlays** at run start — determinism means the session sees exactly the composed snapshot, immune to mid-run ambient drift, while ambient edits (operators using the human CLI directly) flow into the next composition and the inventory. Claude `project` (`.mcp.json`) and `local` scopes are observed read-only and deliberately **excluded from composition** in V1: strict-mode composition bypasses the provider's native project-file trust prompting, so auto-including them would launder project-file trust; their entries carry `effectiveInRuns: false`.
- **Config-hash normalization.** Hash inputs normalize the server config to the daemon's canonical shape (sorted keys via JCS, provider-native field names preserved) so cosmetic reordering does not flip hashes, but any semantic change to the base config (URL, headers, env, command, timeouts, `required`, auth) does. The daemon-managed override-projection fields are excluded per §Trust Governance — they are governed and audited through the override store, not the trust binding.
...
- [ ] Non-user-scope mutation attempts (Codex project; Claude project/local) fail with `mcp.config_scope_unsupported`; the target file is byte-identical after the attempt
...
- [ ] Each of the seven governance mutations emits its event exactly once, with sentinel binding for node-scope events and real-session binding for session-feed status observations; `mcp.reconnect` emits no governance event beyond the status transitions it induces
