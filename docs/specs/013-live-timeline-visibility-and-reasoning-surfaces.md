...
> **Amendment (2026-08-26, provider-switch timeline note — amends the previously-`approved` spec; the header flips to `review` under the audit runbook's spec-amendment rule and is restored `approved` by the paired [Plan-013](../plans/013-live-timeline-visibility-and-reasoning-surfaces.md) targeted readiness-audit delta riding this same diff; [cross-plan-dependencies.md §6](../architecture/cross-plan-dependencies.md) node NS-84).** One run-state subtype row: `agent.provider_switched` renders as a **single status line**, not a message and not a block. The consumer rule is the one this spec already applies everywhere — the row is composed from the event payload verbatim and the renderer derives nothing, so a switch that declared no losses renders without a loss clause rather than being decorated with a generic caution. Adds [Plan-013](../plans/013-live-timeline-visibility-and-reasoning-surfaces.md) T4.9 under CP-016-17's consumer side; mints no event type, wire member, or error code, so no census moves.
