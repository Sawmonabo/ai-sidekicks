...
- `channel.directoryPublish` (2026-08-11, [BL-149](../archive/backlog-archive.md#bl-149-plan-002-channel-directory-ingest-carrier-registration-four-legs)) — the **idempotent channel-directory ingest mutation**, the Plan-002-owned consuming half of the [Spec-016 §Interfaces And Contracts](016-multi-agent-channels-and-orchestration.md#interfaces-and-contracts) D-016-22 publication (producer: Plan-016 T2.14 / CP-016-15, reciprocal [Plan-002 CP-002-10](../plans/002-invite-membership-and-presence.md#cross-plan-obligations)). A **daemon-called** mutation on the attach-authenticated daemon channel (the `runtimenode.signingkeyregister` posture — a daemon publishes after successful attach; participants never call it), carrying one publication: `{channelId, sessionId, name?, state, kind, memberPair?}` plus the origin-ordering keys (`originNodeId`, `originSeq`, envelope `occurredAt` + `id`). The ingest upsert into [`session_channel_directory`](../architecture/schemas/shared-postgres-schema.md#session-channel-directory-plan-002) is a **deterministic last-writer-wins register fold** over at-least-once, arbitrarily reordered, duplicated publications: same origin → higher `originSeq` wins; different origins → `(occurredAt, id)` lexicographic; with **terminal-state dominance on the `state` axis only** — `archived` (terminal, Plan-016 D-016-12) dominates any non-terminal record regardless of comparator, so a delayed mute/unmute straggler can never resurrect an archived row. The **create-once disclosure binding is an independent axis**: `kind` and the canonicalized member pair bind exactly once, from the creation publication whose authenticated calling daemon matches the payload's `originNodeId`, and are immutable to the ingest thereafter — a creation publication arriving after an `archived` record still binds them on the terminal row without reviving it; conflicting origin-authenticated creation claims collapse the row's disclosure fields permanently (`disclosure_collapsed`); healing publications from non-origin daemons are honored for **existence and ordering only**, never for disclosure fields. Everything unresolvable fails **closed**: a publication whose caller's node identity cannot be resolved binds nothing disclosure-bearing, and a directory row whose kind is `NULL`, unrecognized, or collapsed is treated by every consumer — the `ChannelList` filter above first — **as if `direct`** (omitted for everyone outside a resolvable member pair; a `NULL` pair member matches no caller, so participant erasure narrows visibility, never widens it — the [Spec-022 §Shred Fan-Out](022-data-retention-and-gdpr.md#shred-fan-out) `ON DELETE SET NULL` severance is a DB-side act, not an ingest write, and does not breach the create-once rule). The ingest acknowledges only after the durable upsert commits (the producer's at-least-once retry keys on that acknowledgment) and performs no other side effect — no event append (the daemon-local `channel.*` stream stays canonical per ADR-017), no fan-out.
