...
- **A chat becomes a project in place.** Converting a chat to a project copies the managed workspace's files into the attached repository, and from then on the session is a project session: the same session id, the same timeline, the same history. Nothing beyond that copy is migrated and nothing is auto-merged, and the managed workspace and its own history are kept rather than deleted. The conversion changes the session's binding; it never mints a second session. **It never overwrites a file the repository already has:** such a file is not copied and stays in the kept managed workspace. The one act row the conversion appends to the session's timeline counts what was copied and what was not, and opens to name each file that was not copied with its reason and a way to open the chat's copy; the daemon also sends the session's agent one short message naming those files, so the agent can say so and offer to bring them across. Nothing is quietly replaced and nothing is quietly lost.
...
- **The runtime never stops a session's provider process while the session exists.** The daemon starts no stop of its own: no idle timer, no memory-pressure stop, and no fixed bound of any kind on how long a provider process may live. It ends when the person ends the session, or when the provider exits on its own. The reason is what lives inside that process: both providers keep their own background work, their pending wake-ups and their helpers there, so a stop of the runtime's own would end work nobody was ever shown. Memory is therefore a matter of what is **started** rather than of what is killed. A process is **replaced** only by an act the person takes on the session itself — switching its provider, which ends the run and starts the other provider ([Spec-014 §Same-Agent Provider Switch](./014-multi-agent-channels-and-orchestration.md#same-agent-provider-switch)); moving it to another account, which reopens its conversation in that account's own credential home ([Spec-026 §Moving a session to another account](./026-provider-accounts-and-credential-homes.md#moving-a-session-to-another-account)); or restarting it after the provider died — and each of those is a replacement the person asked for, not a reclamation. Stopping or restarting the background service itself is the same kind of act and is confirmed as one, saying that work in flight on the machine stops ([Spec-006 §Required Behavior](./006-local-ipc-and-daemon-control.md#required-behavior)); merely closing the desktop window is **not**, and leaves every session and process going. An idle window on a **run** is a different thing: the person sets it, the daemon arms a timer only where they have, and it interrupts that run without stopping any process ([Spec-014 §Stop Conditions](./014-multi-agent-channels-and-orchestration.md#stop-conditions)).
...
The runtime adds no count limit of its own: not on channels, runs or agents in a session, and not on how many messages a session may queue. How many agents a provider will run at once, and how deep they may nest, is that provider's own limit under the person's own provider settings; where a provider refuses, its refusal is shown in its own words. What a session may spend is its budget, which the person sets.
...
- `session.maxStepsUpdate` writes this session's own `Max steps per turn` override on the session record, or clears it so the session falls back to the machine's own value; an absent override is the ordinary state and means unlimited unless the machine's value says otherwise. What the bound does and how each provider realizes it are [Spec-003 §The Step Bound On A Turn](./003-queue-steer-pause-resume.md#the-step-bound-on-a-turn)'s, and nothing about the bound is a budget or an intervention.
- **A session's address rides its snapshot, not a read of its own.** The address another session writes to when it messages this one — the string the session inspector offers as `Copy address` ([Spec-014 §Sessions Talking To Each Other](./014-multi-agent-channels-and-orchestration.md#sessions-talking-to-each-other)) — is a member of the snapshot `session.read` answers, because the one surface that shows it already reads that snapshot and a second verb would be a second source for one fact. It is present only while the session has a live provider process to be reached at, and it is re-read rather than remembered, since a restarted process is reached at a new address.
...
- The session record additionally carries this session's own `Max steps per turn` override, absent where the person set none. It is durable because a session keeps its own bound across a restart, and it is on the session rather than on a run because a run does not outlive the turn it bounds. The session's address is **not** on the record: it is read from the live provider process the session is bound to, so nothing stale is ever handed out.
...
- [ ] AC7 — Concurrent channels and runs are supported without timeline corruption, with no count limit of the runtime's own on either.
...
- [ ] AC10 — A chat converted to a project keeps its session id and its full timeline, its managed workspace and history are kept, and a file the repository already has is left untouched, stays in the managed workspace, is named on the conversion's own timeline row, and is named to the session's agent in one short message.
