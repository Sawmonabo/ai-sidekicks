# Spec-023: First-Run Three-Way-Choice Onboarding
...
- Re-deriving the three-way-choice semantics. [ADR-020 §First-Run UX](../decisions/020-v1-deployment-model-and-oss-license.md#first-run-ux) is authoritative; this spec implements it.
- Re-specifying OS-keystore mechanics, WebAuthn orchestration, or the preload bridge. Spec-021 is authoritative; this spec composes those capabilities through the bridge.
...
- Owning provider sign-in, or specifying the provider-account registry. The flow hands the operator to each provider's own first-party sign-in and never collects, stores, or intermediates provider credentials; [Spec-026](026-provider-accounts-and-credential-homes.md) is authoritative for the registry, the credential homes, the readiness derivation, and the vendor authentication-policy constraints, and this spec composes those surfaces rather than re-specifying them.
...
- [Spec-006: Local IPC And Daemon Control](./006-local-ipc-and-daemon-control.md) — `DaemonStatusRead` and the typed config surface both clients consume.
- [Spec-021: Desktop Shell And Renderer](./021-desktop-shell-and-renderer.md) — desktop-side surface composes the preload bridge for keystore, WebAuthn, deep-link, and OS dialogs; this spec does not restate Spec-021's mechanisms.
- [Spec-005: Session Event Taxonomy And Audit Log](./005-session-event-taxonomy-and-audit-log.md) — destination for the two new `onboarding.*` events introduced by this spec.
- [Spec-026: Provider Accounts And Credential Homes](./026-provider-accounts-and-credential-homes.md) — the node-local provider-account registry, the readiness derivation, and the vendor authentication-policy constraints that Group B composes and never re-specifies.
...
- **Option 2 (`self-host`).** The UI must prompt for: the relay URL (`https://…`); the admin-issued relay token (paste, never echoed on CLI); a TLS-fingerprint-trust confirmation step after the daemon's first reachability probe against `GET /readyz` at the URL. The daemon must pin the certificate's SubjectPublicKeyInfo hash on confirmation (TOFU). The admin token must be written to the OS keystore via the same keystore surface Spec-021 defines; never to a plaintext config field.
- **Option 3 (`hosted-saas`).** The UI must open the system browser to the hosted sign-up URL (a configurable constant in daemon config) with a one-shot PKCE state parameter. The daemon must listen on a `127.0.0.1:<ephemeral>/callback` loopback endpoint (desktop may alternatively register a `sidekicks://` deep-link handler per Spec-021) and accept exactly one inbound callback bearing the scoped token and the matching PKCE state. The scoped token must be written to the OS keystore. The loopback listener must bind only to `127.0.0.1` and must close within 5 minutes or on first use, whichever comes first.
...
Secrets (`self-host` admin token; `hosted-saas` scoped token) are never written to `config.toml`. They go through the Spec-021 keystore surface (`safeStorage` on desktop, `@napi-rs/keyring` with try/catch existence-check for CLI).
...
2. After a provider run has been refused on the account plane ([Spec-026 §Fallback Behavior](026-provider-accounts-and-credential-homes.md#fallback-behavior)). The refusal is the trigger, so the operator meets the step at the moment it is relevant, carrying the refusal's own remedy.
...
**This step elects no account.** A provider whose accounts include none marked current is reported as not ready with its own remedy, and the mark is moved on the Providers settings page rather than here: this step registers accounts and hands the operator to the provider's own sign-in, and it elects no account, not even where there is a single candidate ([Spec-026 §Selection at run start](026-provider-accounts-and-credential-homes.md#selection-at-run-start)).
...
The same commands are reachable through the short alias `sk`, which [Plan-006](../plans/006-local-ipc-and-daemon-control.md) registers beside `sidekicks` in one `bin` map; the long form above stays the unambiguous spelling, because an unrelated `sk` already on the user's `PATH` wins or loses on `PATH` order alone (`which -a sk` reports which).

### Desktop Surface

Desktop composes the flow through two bridges exposed by the Spec-021 preload contract:
...
Per Spec-006's typed control surface, the following request/response methods must be added:
...
Request/response schemas belong in [api-payload-contracts.md](../architecture/contracts/api-payload-contracts.md) (added by Plan-023); the shape follows the Spec-006 authenticated-principal model.
...
**The provider-authentication step group emits no event, and this spec's `onboarding_lifecycle` category does not grow.** The provider-account registry is node-local, non-evented daemon configuration ([Spec-026 §State And Data Implications](026-provider-accounts-and-credential-homes.md#state-and-data-implications)), and Group B writes nothing of its own beside it — no config key, no marker, no partial-state entry (§Provider Authentication (Group B), §State And Data Implications). Emitting a provider-setup event would place node-local account facts into a session-scoped, relayed, durable log they are deliberately excluded from. The two existing `onboarding.*` events are unchanged in name, payload, and category.
...
- **Keystore probing.** Per Spec-021's keystore surface, Desktop uses Electron `safeStorage.isEncryptionAvailable()` + `safeStorage.getSelectedStorageBackend()` ([docs](https://www.electronjs.org/docs/latest/api/safe-storage)). CLI (Node-only) uses [`@napi-rs/keyring`](https://github.com/Brooooooklyn/keyring-node) v1.2.0, which lacks an existence-check API — wrap reads in try/catch and treat `not-found` as the unresolved-partial-state signal.
- **Config schema version.** Bump the `config.toml` top-level `schema_version` when adding `[onboarding]` so older daemon binaries do not misread it; follows the Spec-006 version-negotiation posture.
...
- **Prompting on initial install or first session create.** The flow is keyed to explicit activation, not first-launch. Prompting earlier breaks local-only and offline modes and conflicts with [ADR-020 §First-Run UX](../decisions/020-v1-deployment-model-and-oss-license.md#first-run-ux). Implementations that put the prompt on install-time must be rejected in review.
...
- **Leaking secrets into renderer (Desktop Option 2 / Option 3).** The renderer must never handle the paste-box or the callback token directly. All secret input flows through main-process dialogs per [Spec-021 §Trust Stance](./021-desktop-shell-and-renderer.md#trust-stance). An implementation that renders the admin-token input via `<input type="password">` in React has already leaked the secret into the renderer address space.
...
- [ ] Secrets (Option 2 admin token, Option 3 scoped token) are persisted via the Spec-021 keystore surface only; `config.toml` never contains them.
...
| [Spec-021: Desktop Shell And Renderer](./021-desktop-shell-and-renderer.md) | Keystore surface, `safeStorage` usage, WebAuthn orchestration, preload-bridge pattern, deep-link handler registration. This spec composes those mechanisms; it does not re-specify them. |
| [Spec-006: Local IPC And Daemon Control](./006-local-ipc-and-daemon-control.md) | Typed control surface the onboarding RPCs extend; `DaemonStatusRead` semantics. |
| [Spec-005: Session Event Taxonomy And Audit Log](./005-session-event-taxonomy-and-audit-log.md) | Destination for the two new `onboarding.*` events. |
| [Spec-026: Provider Accounts And Credential Homes](./026-provider-accounts-and-credential-homes.md) | Provider-account registry, credential homes, the readiness projection Group B renders, fail-closed spawn validation, and the vendor authentication-policy constraints. |
