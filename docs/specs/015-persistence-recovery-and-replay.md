...
**Upgrade policy.** Minor and patch bumps within the `^12.9.0` range are auto-applied by the package manager; any future major (`13.x.y`) must be explicitly re-evaluated against the pragma overrides in §Pragmas and the `.backup()` atomicity contract in §Backup Policy before the pin moves. Since V1's durability posture depends on `synchronous = FULL` and on the undocumented fsync behavior of `.backup()`, a surprise major bump that changed either behavior would be invisible without this gate. _Landed-fact annotation (2026-09-01; no Status flip)._ [ADR-022 §Decision Log](../decisions/022-v1-toolchain-selection.md#decision-log) has decided the first major move this policy anticipates: `packages/runtime-daemon` moves to `better-sqlite3` **13.0.3** — the first Node-API line, SQLite 3.53.4 embedded — because Electron 44's Node ABI 149 has no 12.x prebuild and V1 targets 44.x per [ADR-016 §Decision](../decisions/016-electron-desktop-shell.md#decision). The gate above binds the PR that moves the pin, [Plan-023](../plans/023-desktop-shell-and-renderer.md) T-023p-1B-4: it amends this section's pin sentence (the ordinary spec-amendment flip-and-restore) only with the §Pragmas overrides re-verified by readback and the §Backup Policy atomicity contract re-probed under 13.0.3 — the daemon ships no `.backup()` caller yet, so that contract has no suite until the backup writer lands and the direct API probe stands in — re-running on the pinned tree the scratch-tree re-evaluation the same ADR-022 entry already records (suite identical under both pins; pragma readback exact; the 13.0.1 worker-terminate abort reproduced and closed on 13.0.3). The same task moves the tier-1 Node floor to 22.14.0, below which the 13.x Node-API-10 prebuild segfaults at first open. Until that PR merges, the pin above is the shipped one.
