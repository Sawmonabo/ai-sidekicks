...
V1 pins **`better-sqlite3@13.0.3`** exact as the local SQLite driver (moved 2026-09-01 from `^12.9.0`; see §Upgrade policy below for the gate this move discharged). 13.0.3 was released 2026-08-05, is the first Node-API line — eight platform prebuilds bundled in the tarball, ABI-independent across Node and Electron, with no `prebuild-install` — and embeds SQLite **3.53.4** (up from 3.53.0 on 12.9.0). Its published `engines.node >= 22` **overstates the real floor**: the prebuilds are compiled against Node-API 10, which Node exposes from 22.14.0 / 23.6.0, and a Node-API-9 host dies `SIGSEGV` with no diagnostic at the first `new Database()` ([ADR-022 §Decision Log](../decisions/022-v1-toolchain-selection.md#decision-log), 2026-09-01), which is why the tier-1 `engines.node` floor moved to `>= 22.14.0` in the same task. The pin is **exact** rather than a caret because the 13.0.x line's own history is the reason for the version chosen: 13.0.0 / 13.0.1 abort `SIGABRT` under this repo's worker-terminate shape (WiseLibs/better-sqlite3#1507, fixed in 13.0.2), so a range that could resolve below 13.0.2 would resolve to a binding this daemon crashes on. Recommended Node runtime is 24 LTS. The alternative built-in `node:sqlite` module is not yet viable for V1 — on Node 24.15.0 LTS and Node 25.9.0 Current its stability index is still `1.2 — Release candidate` ([nodejs.org/api/sqlite.html](https://nodejs.org/api/sqlite.html), fetched 2026-04-19). Re-evaluate pre-V2 once `node:sqlite` graduates to Stability 2 (Stable).
