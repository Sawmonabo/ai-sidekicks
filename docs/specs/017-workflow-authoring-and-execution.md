...
**What the engine deliberately does not consume.** The account-plane `usage.rate_limit_update` snapshot — payload `{provider, windowMins, usedPercent, resetsAt?}` — is the nearest existing surface and is **not** an input to any rule in this section. [Spec-006 §Usage Telemetry](006-session-event-taxonomy-and-audit-log.md#usage-telemetry-usage_telemetry) scopes it as "a display/quota surface only, kept off the security/admission plane"; its `resetsAt` carries no provenance stamp; and its own consumer obligations are staleness defenses that resolve **fail-open to the freshest reading**. Arming an execution schedule off a display surface that is licensed to be stale and stamps no provenance would be precisely the guess the arming rule below refuses to make. It stays what it is: a quota display.

**Fail-closed until the carrier lands.** While no typed signal exists, nothing in this section is reachable: a provider refusing for a spent allowance is seen by the engine as an ordinary failure and travels the ordinary retry-and-fail path. That is the honest V1 state, stated rather than implied — the park, the schedule, and the attention fold activate together when the driver carrier registers, and none of them is approximated in the meantime by reading text. The supply cadence the carrier will ride is already fixed: [Spec-005 §Resolved Questions and V1 Scope Decisions](005-provider-driver-contract-and-capabilities.md#resolved-questions-and-v1-scope-decisions)'s bounded 15-minute per-runtime-node refresh with optional provider push (the Codex `account/rateLimits/updated` push alongside the `account/rateLimits/read` pull), and correctness here does not depend on the push arm.
...
**Recorded residual, with its named gate.** V1 ships these as engine state-machine rules; the operator control surface that invokes them is **not** minted here. The wire registry carries no run-cancel, run-resume, or re-pin operation, and §Event types (SA-19) carries no run-cancelled lifecycle type — both predate this amendment (the `cancelled` run status has had no named producer since it was declared). Naming that surface is a separate, deliberate contract addition; this amendment states the semantics it must satisfy and mints neither the operation nor the event type, so the eleven-method registry and the 23-type / five-category taxonomy are unchanged. The auto-resume path of SA-40 is engine-internal and needs no wire producer, so it is unaffected by this residual. The addition is tracked as [BL-151](../backlog.md), and it carries one structural constraint this spec fixes now: the operation set and the run-cancelled lifecycle event MUST mint **together**, because a durable cancellation the log cannot reconstruct would break §Truth vs projection vs ephemeral (SA-25) — a projection rebuild would replay the last `workflow.phase_suspended` payload, restore its schedule and attention key, and resurrect a run the operator cancelled. Until BL-151 lands, no shipped path durably produces the `cancelled` status, so the record-without-event divergence is unreachable rather than latent.
...
- **`workflow_lifecycle` (6):** `workflow.created`, `workflow.started`, `workflow.gated`, `workflow.failed`, `workflow.completed`, `workflow.resumed` (payload extended 2026-08-16 with an additive-optional version re-pin member naming the version a repaired run left and the one it joined — §Frozen-definition repair (SA-41); additive MINOR under [ADR-018](../decisions/018-cross-version-compatibility.md))
...
**Recorded residual.** This rule governs the composer path. The steer and queue-submit paths carry free text into provider turns and are Spec-004 / Spec-005 / Spec-006 surface; neutralizing leading-`/` text at the driver boundary — including whether the provider process is spawned with its own command dispatch disabled — is tracked as BL-148 against the Plan-005 driver build. Until BL-148 lands, chat-borne workflow start is collision-proof by interception, the composer's provider-bound paths refuse all leading-`/` input (the escape above stays inactive there, so the composer never produces provider-bound slash text), and steered and queued free text arriving from non-composer surfaces retains the provider-dispatch hazard: a bounded, named residual, not a silent one.
