...
- Event taxonomy extensions (cataloged in [Spec-006](./006-session-event-taxonomy-and-audit-log.md) under category `cross_node_dispatch`): 13 events across caller-side and target-side lifecycles. Caller-side (3): `dispatch.sent`, `dispatch.approval_observed`, `dispatch.result_observed`. Target-side intermediate (5): `dispatch.received`, `dispatch.approval_requested`, `dispatch.approved`, `dispatch.executed`, `dispatch.result_buffered`. Target-side terminal (5 — one per dispatch): `dispatch.rejected` (pre-approval failure), `dispatch.denied` (approval refusal), `dispatch.failed` (post-approval execution failure), `dispatch.expired` (mid-flight token timeout), `dispatch.completed` (success). Plus `runtime_node.capability_declared` (already owned by Spec-003).
