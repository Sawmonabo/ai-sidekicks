...
| **Depends On** | [Runtime Node Model](../domain/runtime-node-model.md), [Session Model](../domain/session-model.md), [User And Device Model](../domain/user-and-device-model.md), [Component Architecture Local Daemon](../architecture/component-architecture-local-daemon.md), [Security Architecture](../architecture/security-architecture.md), [Runtime Node Attach](./003-runtime-node-attach.md), [Approvals, Permissions, and Trust Boundaries](./012-approvals-permissions-and-trust-boundaries.md) |
| **Implementation Plan** | [Plan-027: Cross-Node Dispatch And Approval](../plans/027-cross-node-dispatch-and-approval.md) |

> **Amendment (2026-07-06, capability-enhancement campaign B7; re-authored 2026-07-12 on post-delta develop with the exemption predicate carrying the run-scoped name reconciled to the landed Plan-016 T2.7 seam — amends the previously-`approved` spec; the header is flipped to `review` for the amendment's review window per the audit runbook's spec-amendment rule, and the campaign plan's Task 28 batch re-promotion restores `approved` after the W1.5 spec gate; dependent plans' code dispatch stays census-gated on that restoration).** This bundle lands the dispatch-liveness session half and the transport-parity framing: (1) a pending outbound cross-node dispatch exempts its **initiating run** from idle reaping via the run-scoped `runHasPendingCrossNodeDispatch(runId)` hard-skip predicate — each dispatch record carries its **originating `runId`**, so unrelated runs in the same session keep ordinary idle semantics, and every pending window is self-terminating and caller-local (§Required Behavior, new bullet under §Cross-Node Failure Semantics; the target-owner-unreachable bullet gains the caller-side liveness clause; a new Acceptance Criterion; a §State And Data Implications bullet pinning the caller-local pending-dispatch record's ownership) — the backing table and schema row are deliberately NOT here (they land with [Plan-027](../plans/027-cross-node-dispatch-and-approval.md) via the campaign's B17 bundle, keeping the CREATE with its owner; the accessor code lands with the same bundle and is consumed by [Plan-016](../plans/016-multi-agent-channels-and-orchestration.md) T2.7's idle-reaper seam); (2) an R8 transport-parity note — cross-node dispatch is the Claude leg of remote-provider parity, complementary to the Codex driver's native transport axis, and **provider-run** dispatch results adopt the [Spec-005](./005-provider-driver-contract-and-capabilities.md) `structured_output` typing (campaign B3, C-13; output schema carried in the canonical `action_payload.output_schema` field — non-provider capability handlers keep their own result contracts) (§Interfaces And Contracts; §Implementation Notes).
...
- Session creation and attachment semantics (covered by [Spec-001](./001-shared-session-core.md)).
...
- **Pending outbound dispatch exempts the initiating run from idle reaping (campaign B7).** While `runHasPendingCrossNodeDispatch(runId)` holds for a run — true while the session has at least one outbound dispatch whose record names `runId` as its **originating run** and whose pending window is open (the predicate is run-scoped; the accessor lands with [Plan-027](../plans/027-cross-node-dispatch-and-approval.md)'s campaign bundle and is consumed by [Plan-016](../plans/016-multi-agent-channels-and-orchestration.md) T2.7) — the run-idle sweep ([Spec-016 §Stop Conditions](./016-multi-agent-channels-and-orchestration.md#stop-conditions), `idle_timeout`) must **hard-skip that dispatch-initiating run** (unrelated runs in the same session keep ordinary idle semantics): a skip, not an activity-timestamp bump, because a bump decays and re-fires after another idle window while approval waits legitimately span the full token lifetime (the surveyed reaper-exemption pattern). A dispatch's pending window closes on whichever comes first: (a) the caller observes its conclusion — **any observed terminal outcome, not only success or denial**: a relay-delivered result or terminal-failure envelope (`DispatchTerminalNotice`, §Interfaces And Contracts — the target-side `dispatch.failed` / `dispatch.expired` terminals **and the post-receipt `dispatch.rejected` terminal**: a step-5 Cedar-evaluation failure — a class-rule deny that does not route to the approval flow, or `policy_engine_error` — lands after intake validation passed and the `DispatchReceive` ack returned `"received"`, so without the relayed notice the caller would hold the window open to the clock bound for a terminal the target has already recorded), or an intake rejection returned synchronously in the `DispatchReceive` ack (`status: "rejected"` — steps 1–4: token, body binding, replay, capability; the same target-side `dispatch.rejected` terminal, observed without a relay leg and with no `dispatch.received` ever emitted) — each recorded caller-side as `dispatch.result_observed` with the matching `outcome` discriminator (`'completed' | 'failed' | 'expired' | 'rejected'`; the [Spec-006](./006-session-event-taxonomy-and-audit-log.md) row carries the widened payload, so the observation violates no taxonomy) — or a denial via `dispatch.approval_observed` per §Example Flows; an observed conclusion closes the window immediately, so the caller never idle-protects a dispatch it already knows is over (the authoritative caller-side predicate enumeration is [Plan-027](../plans/027-cross-node-dispatch-and-approval.md)'s, landing with the campaign's Plan-027 bundle), or (b) **the caller-local expiry bound elapses**: `caller_token.exp` plus the result-buffer window (+ 5 minutes — the partner-detach result-buffer bullet below) — an independent caller-clock trigger requiring no observation of any target-side event, so an unreachable target daemon or relay partition can never hold the exemption open past the bound, and a restarted caller rebuilds every pending window from its own caller-local pending-dispatch records alone — written before relay send, carrying `expires_at` and the originating `runId`, and closed in place at conclusion, so a rebuild never resurrects a concluded window (the backing table lands with [Plan-027](../plans/027-cross-node-dispatch-and-approval.md) via the campaign's B17 bundle; the `dispatch.sent` event stays base-payload per [Spec-006](./006-session-event-taxonomy-and-audit-log.md) — it is the audit record, not the rebuild source). With several overlapping dispatches from the same initiating run, that run's exemption releases when its **last** pending window closes. The dispatch's own expiry semantics (caller-token expiry, clock skew) are unchanged.
...
- [Spec-012 — Approvals, Permissions, And Trust Boundaries](./012-approvals-permissions-and-trust-boundaries.md) — approval categories and the reaching-a-session-does-not-imply-execution invariant.
