...
- `DispatchResult(dispatch_id, result_payload, result_signature) -> (delivered to caller via relay)` — final result emission after execution. `result_payload` adopts the [Spec-005](./005-provider-driver-contract-and-capabilities.md) `structured_output` typing (campaign B3, C-13): when the executing driver declares the flag and the dispatch names an output schema, the payload is the schema-constrained final output; otherwise it is the provider's unconstrained final text (Spec-005's matrix row names Spec-024 dispatch results as a consumer of this flag — campaign B7 closes that loop from this side).
