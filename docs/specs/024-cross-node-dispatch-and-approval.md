...
- **R8 transport parity (campaign B7).** Cross-node dispatch is the **Claude leg of remote-provider parity**: Claude Code exposes no local network listener, so "drive a Claude session on another machine" is delivered by dispatching the run to that node's daemon — the mesh is the emulation. The Codex driver's native config-gated transport axis ([Spec-005 §Default Behavior](./005-provider-driver-contract-and-capabilities.md), campaign B3) is the complementary leg. Parity is graded per the capability-parity mechanism vocabulary (`native | emulated | absent`), not claimed boolean.
