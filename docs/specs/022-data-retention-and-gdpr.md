...
- Export must additionally include the decrypted participant-authored bodies of `interventions` and `queue_items` rows whose `pii_participant_id` stamps the participant — on `interventions`, the [Spec-004](004-queue-steer-pause-resume.md) edit-and-resend composite's `replacementSend` body and the steer directive text (the composite's copy added 2026-08-17, PR #344 round-2 fold; steer joined 2026-08-18); on `queue_items`, the queued send body (2026-08-18). Text that exists only on a refused or suppressed intervention row, or on a queue item drained or cancelled without ever reaching the timeline, is still the participant's authored content, and the event-scoped selector alone would miss all of it.
...
- The session cost receipt adds no export path of its own (2026-08-18, provider-account amendment). It is a read-time fold over `usage_telemetry` rows the event-scoped selector above already covers ([Spec-016 §Session Cost Receipt](016-multi-agent-channels-and-orchestration.md#session-cost-receipt)), so exporting those rows exports everything the receipt shows about the participant; the export must not carry a separately-computed per-participant spend tally, which would be exactly the second accountant [Spec-016 §Cost Figure Display Consistency](016-multi-agent-channels-and-orchestration.md#cost-figure-display-consistency) forbids. The receipt's per-participant axis is a **new use of an already-mapped identifier**, not a new PII path: it reads the admitting-principal reference whose durable carrier is the `interventions.admitting_principal_id` row above, and that row's disposition — the participant-reference posture recorded below — governs it unchanged. A use does not earn its own map row; the column it reads already has one.
- `session_events.content_payload` adds no export path of its own (2026-08-30, the machine-authored prose amendment), and the omission is a **recorded disposition with a named limit rather than a silent gap**. Where the event-scoped selector already finds a row — because the participant authored it, or because the row's own metadata names them — the machine-authored body is exported with it, decrypted under the session content key rather than `participant_keys`; the body is part of what that event _is_, and it is readable after key deletion because it is the session's record and not the erased participant's, which leaves the ordering rule in the bullet above unaffected. **The limit, stated plainly** (2026-08-30, Codex PR #383 round 1): a mention occurring **only inside** an encrypted machine-authored body — an assistant reply or tool result that names the participant in prose the metadata does not index — is **not discoverable** by that selector, so the first bullet's "or mentioning" reach does not extend into this column. That is a deliberate consequence of the column carrying no participant linkage of any kind, and the alternatives are worse than the limit. A durable participant→content association would be a **new PII path**: an index binding every person named in a machine body to that body, built for erasure and immediately becoming a second thing to erase. A `content_payload`-keyed selector has nothing to key on and would return every agent's output in every session the participant ever joined — other members' session record, not this data subject's personal data. And a decrypt-and-scan of every session's whole content on each request would mint a new DSAR mechanism with no producing task, performing a fresh decrypt over co-owned content on one member's request — a governance question this column is not the place to settle. The export requirement is therefore **narrowed here rather than claimed satisfied**: it reaches every event the metadata selector identifies, and the bodies of those events, and no further.
- Export covers a participant's session data and does not reach the node's provider-account registry, which is operator-plane machine configuration rather than any participant's session content (2026-08-18, provider-account amendment). The registry's own access and erasure disposition — including the operator-serviced handling of a `display_label` that names a person — is recorded in [§PII Data Map](#pii-data-map) rather than added as an obligation here, because the participant-scoped export selector has no linkage by which to find that table.
...
- `GET /participants/{id}/export` must return a JSON export of all events associated with the participant, decrypted with their key — plus the decrypted intervention-row and queue-item bodies stamped with the participant (the [Spec-004](004-queue-steer-pause-resume.md) at-rest copies per §Data Export).
...
| `session_events` (SQLite) | `content_payload` | **Machine-authored** session content, not a data subject's authored text — the assistant message body, the reasoning-update body, and tool-call arguments / result / error bodies (added 2026-08-30, the machine-authored prose amendment). It is mapped here **because it can quote a participant**, not because a participant wrote it: an assistant reply restates what was asked and a tool result can contain a file the participant named. Sealed under a **session**-scoped key ([Local SQLite Schema §Session Events](../architecture/schemas/local-sqlite-schema.md#session-events-plan-001-extended-by-plans-006-008-015)), so it carries no `pii_participant_id` stamp and no participant selector reaches it | Until any ONE [Spec-006 §Event Compaction Policy](006-session-event-taxonomy-and-audit-log.md#event-compaction-policy) trigger names the row a candidate — the same window full payloads take, and for a content-bearing session the 500 MB storage trigger rather than the 90-day age trigger is typically the binding one. Compaction removes the ciphertext (set NULL) alongside `pii_payload`; the audit stub that survives carries none of it | **Outside the participant-keyed fan-out** — the artifact-payload posture below, applied unchanged. The ciphertext dies with the row's compaction, with the row itself on session purge, or with daemon master-key destruction ([§Daemon Master Key](#daemon-master-key)); deliberately **not** a per-participant sweep, on the same reading `artifact_manifests.relay_cek_ciphertext` above takes. Erasure severs the participant's **reach** — Path 1 destroys the key sealing everything they authored — not the session's record of what its agents did, which every other member of that session co-owns. **What this row does not claim:** with no per-row key of its own there is no crypto-shred here, and NULL-out is not secure overwrite (SQLite frees the page to the freelist and the WAL may retain the image until checkpoint and `VACUUM`), so the honest statement is that the ciphertext becomes unaddressable and dies with its row, not that it is provably destroyed on demand |
| `interventions` (SQLite) | `pii_payload` | Participant-authored intervention body — the rollback replacement-send message body (the [Spec-004](004-queue-steer-pause-resume.md) edit-and-resend composite's at-rest copy; added 2026-08-17, PR #344 round-2 fold) and the steer directive text (joined 2026-08-18) | 90 days (full), then NULLed by the daemon retention pass — a plain NULL-out, not a stub: no digest or anchor binding attaches to this ciphertext, unlike `session_events`; the non-PII intervention audit record is retained for the row's lifetime | Crypto-shred via participant key deletion — same `participant_keys` DEK as `session_events.pii_payload`, so Path 1 shreds both copies identically (`pii_participant_id` is the selector stamp) |
| `queue_items` (SQLite) | `pii_payload` | Participant-authored queued send body — the follow-up message a participant queues against an active run, and the composite's admitted replacement (added 2026-08-18) | Row lifetime: queue rows are durable and never deleted on drain (the row is canonical for audit and replay, and is never evented), so the ciphertext persists until crypto-shred | Crypto-shred via participant key deletion — same `participant_keys` DEK as `session_events.pii_payload` and `interventions.pii_payload`, so Path 1 shreds every copy of one send in a single act (`pii_participant_id` is the selector stamp — the only participant-naming column on this table, and the reason the fan-out can address it at all) |
| `interventions` (SQLite) | `admitting_principal_id` | Participant reference — identifier-class, not content: the daemon-resolved principal recorded as the intervention's admitting authority (added 2026-08-18) | Row lifetime | **Retained**, not erased — the participant-reference posture below |
...
| `provider_accounts` (SQLite, daemon-local) | `display_label` | Operator-chosen free text naming one provider account — routinely an email address or a person's name, since that is how an operator tells two accounts apart | Account-registration lifetime; node-local, and neither the label nor any other registry column is relayed or uploaded to the control plane | **Outside the participant-keyed fan-out** — the table carries no participant linkage, so no selector reaches it and none is invented (an invented one would delete a machine's ability to execute, not a person's data). The label dies with the registration: it is editable in place and removed with the account ([Spec-029 §The account registry](029-provider-accounts-and-credential-homes.md#the-account-registry)). An erasure request naming a person whose name or address an operator typed into a label is therefore serviced on that operator's own node by renaming or removing the registration, which is this spec's disposition for the column; an access request from that same person is answered the same way — read from the registry directly by the operator, since the participant-scoped export selector has no linkage that reaches this table. There is no credential row beside it to shred — see the account-plane posture note below |
| `provider_accounts` (SQLite, daemon-local) | `observed_auth_mode`-associated identity (the provider-reported email, organization id, and organization name a health observation may surface) | **Provider-reported, not operator-typed** — where a provider's own status surface reports the account's email and organization for a credential home, a health observation may carry them to the operator so two accounts of one provider are tellable apart. Unlike `display_label` this is not free text an operator chose: it is a real identifier belonging to a real person or organization, learned from the provider (2026-08-26, sign-in and health-observation amendment) | Account-registration lifetime, and re-observed rather than accumulated: a later observation **replaces** the prior value rather than appending, so no history of an account's past identities is retained | **Outside the participant-keyed fan-out, on the same grounds as `display_label`** — the row carries no participant linkage, so no selector reaches it and none is invented. It dies with the registration. Two obligations are specific to this class and do not apply to the operator-typed label: it is **never rendered outside the node-local, node-operator-authorized surfaces** that already carry the registry (no event payload, no relayed payload, no log line), and an erasure request naming that person is serviced by removing the account registration, which takes the value with it. It is deliberately not made independently erasable: a registry row without the identity it was observed under would name a credential home the operator can no longer attribute, which trades one person's data for every operator's ability to tell their accounts apart |
| `provider_account_usage_windows` (SQLite, daemon-local) | — (no PII column) | Quota readings keyed by account and limit identifier. Carries no participant identity, no operator-authored text, and no credential material; `limit_id` and `label` are provider-authored infrastructure strings | Newest reading per key only — superseded readings are replaced, not accumulated, so the table holds no usage history to mine | **Outside the participant-keyed fan-out**, and outside it for a stronger reason than its parent: the table has no PII column at all. Cascades away with its account's registration |
...
**Scope.** Three SQLite tables, all shredded by the one key deletion: every `session_events` row with a non-NULL `pii_payload` authored by or containing the participant's PII; every `interventions` row whose `pii_participant_id` stamps the participant (the [Spec-004](004-queue-steer-pause-resume.md) replacement-send body added 2026-08-17, PR #344 round-2 fold, the steer directive text 2026-08-18); and every `queue_items` row whose `pii_participant_id` stamps the participant (2026-08-18 — queue rows are durable and never deleted on drain, so a participant's queued send bodies outlive the runs that consumed them). All three encrypt under the same per-participant DEK, so the single key deletion shreds every copy of one send in one act, and none of them needs a row DELETE. The SQL selector uses the durable participant-id stamp on each row, not the ciphertext (which is opaque). These three are the complete set of durable-tier SQLite tables holding participant-authored content **under a per-participant erasure selector** — the qualifier is load-bearing, and the excluded classes are each excluded for a stated reason rather than by oversight: artifact manifests and their CAS payloads are participant-authored but deliberately outside per-participant erasure as shared work product (the artifact-payload posture note above, which erasure addresses by severing reach rather than by a content selector); the `interventions` and `queue_items` participant-**reference** columns are identifier-class and retained per the §PII Data Map participant-reference posture; and `artifact_encryption_keys` is key material deleted by the mechanism above rather than a content selector. Membership-level references in other participants' events remain, but their `pii_payload` columns — if any — are encrypted under a different participant's key and are not affected.
