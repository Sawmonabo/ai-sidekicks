...
- `session_events.content_payload` adds no export path of its own (2026-08-30, the machine-authored prose amendment), and the omission is a **recorded disposition rather than a gap**. The first bullet's selector already reaches every event authored by **or mentioning** the participant, and this column hangs off those same rows: exporting an event exports its machine-authored body along with it, because the body is part of what that event _is_. What the export must **not** grow is a `content_payload`-keyed selector, because there is nothing for one to key on — the column carries no `pii_participant_id` and no participant linkage of any kind, its sealing key being session-scoped by design ([§PII Data Map](#pii-data-map)). A selector invented for it would return every agent's output in every session the participant ever joined, which is other members' session record and not this data subject's personal data. Decryption for export uses the session content key rather than `participant_keys`, so the ordering rule in the bullet above is unaffected: this content is readable after key deletion and is deliberately still there, being the session's record and not the erased participant's.
