...
| **Depends On** | Spec-007, Spec-020, Spec-021, Spec-022, Spec-026, Spec-031; ADR-010, ADR-012, ADR-020; `docs/architecture/deployment-topology.md`, `docs/architecture/security-architecture.md` |
...
- Bind-address reconciliation between the relay's container internals (owned by [Spec-031](./031-remote-control.md)) and this spec (daemon host posture).
...
- **`security.default.override` log event schema** (rows 2, 5, 6, 8, 9): structured log with fields `behavior` (integer 1–10), `effective_value` (string), `banner_printed_at` (ISO-8601), and an OPTIONAL `row` (`7a`/`7b` as string). The `row` field is the `7a`/`7b` sub-row discriminator and is **omitted** for the single-integer `behavior` overrides — rows 2/5/6/8/9 carry no `7a`/`7b` sub-row, so they emit no `row`. Row 9's `9a` (daemon, Plan-020) / `9b` (relay, Plan-031) split is a process-scope distinction — each process emits its own `metrics_disabled` override under `behavior: 9` — not a `7a`/`7b`-style payload discriminator, so it too omits `row`. (Tier-7 audit content-only amendment marking `row` OPTIONAL — its `7a`/`7b` domain never intersects the override-emitting rows 2/5/6/8/9, leaving it structurally dead for every applicable row; Spec-027 stays `approved`. Row 9 was added to the applicable set by the round-18 review — its `metrics_disabled` override had been absent from this annotation.)
- **`/metrics` endpoint** (rows 9a daemon / 9b relay): Prometheus v0.0.4 exposition format. Counter names and semantics listed in the companion doc. Bind/auth contract shape owned by Plan-020; relay wiring owned by Plan-031.
...
- **ACME client selection (row 1)**: this spec requires RFC 9773 ARI support. The `acme-client` npm package (publishlab/node-acme-client v5.4.x) is flagged "Inactive" by Snyk as of 2026-04 with last major publish mid-2024 and no documented ARI support. Plan-031 will vendor-review `acme-client`, fork it, or implement ARI directly against the `/renewalInfo` endpoint. The spec does not pin a library.
...
- **Short-lived cert profile (row 1)**: LE's 6-day `shortlived` profile went GA 2026-01-15. Not adopted for V1 default because Dwildcard issuance under a 6-day profile means daily DNS writes, and not every DNS provider plugin is rate-limit-safe at that cadence. Plan-031 may expose `ACME_PROFILE=shortlived` as an opt-in.
- **Platform-specific self-update swap rules (row 7b)**:
  ...
  - **macOS**: standalone Mach-O binaries CAN be notarized (Apple's notary service creates a ticket), but the ticket cannot be stapled to them per [Apple notarytool docs](https://developer.apple.com/documentation/security/customizing-the-notarization-workflow) ("Although tickets are created for standalone binaries, it's not currently possible to staple tickets to them"). Gatekeeper on a network-connected machine fetches the ticket via `gktool` at first launch; offline first-launch produces a quarantine warning. V1 SHIPS macOS as a notarized `.pkg` so the ticket can be stapled and offline first-launch is clean; self-update verifies the replacement `.pkg` is also notarized + stapled before swap.
...
- **Caddy random-challenge picking**: do NOT adopt Caddy's "try a random ACME challenge type and learn over time" heuristic for auditable deployments. Deterministically prefer Dwhen credentials are configured, else HTTP-01.
...
- **Short-lived LE profile (6-day) opt-in**: Plan-031 decides whether to expose `ACME_PROFILE=shortlived` in V1 or defer to V1.1.
- **OAuth auth method on Postgres 18**: not explicitly required by row 5 (which allows scram-sha-256). If an operator is on PG 18 with `oauth` configured, does that count as "weak"? Spec currently treats it as acceptable; Plan-031 may revisit.

## Resolved Questions and V1 Scope Decisions

- **Behavior 9 `/metrics` auth on non-loopback — resolved 2026-06-10, retired from §Open Questions 2026-08-10.** The scheme is no longer deferred: rows 9a and 9b both require either `METRICS_AUTH=bearer` (rotated token file) or `METRICS_AUTH=mtls` (operator-provided client-cert allow-list) on any non-loopback bind, and missing auth on a non-loopback bind is a config-parse-time error. Plan-020 owns the bind/auth contract for both surfaces; Plan-031 consumes it for the relay endpoint. The answer landed with the 2026-06-10 Tier-6 readiness-audit pass (Plan-021 D-021-8) that rewrote rows 9a/9b; this list simply still carried the superseded "bearer token is assumed" framing. Retiring it against the ratified rows is reconciliation, not new design, so Spec-027 stays `approved` (Tier-8 readiness audit — Plan-020 walk).
