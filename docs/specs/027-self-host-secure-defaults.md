...
- **`security.default.override` log event schema** (rows 2, 5, 6, 8): structured log with fields `behavior` (integer 1–10), `effective_value` (string), `banner_printed_at` (ISO-8601), and an OPTIONAL `row` (`7a`/`7b` as string). The `row` field is the `7a`/`7b` sub-row discriminator and is **omitted** for the single-integer `behavior` overrides — rows 2/5/6/8 carry no `7a`/`7b` sub-row, so they emit no `row`. (NS-19 Tier-7 audit content-only amendment marking `row` OPTIONAL — its `7a`/`7b` domain never intersected the rows-2/5/6/8 applicability, leaving it structurally dead for every applicable row; Spec-027 stays `approved`, recorded as Plan-025 D-025-5.)
