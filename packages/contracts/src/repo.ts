// Repo-mount and workspace contracts — the branded `RepoMountId` / `WorkspaceId`
// scalars, the four canonical repo/workspace enums, the derived `RepoMountHealth`
// projection, and the shared repo/workspace/worktree lifecycle event payload.
...
// completing the mount half above.
//
...
//
// The TRANSPORT and TYPING notes on block above govern these three unchanged:
...
export const WorkspaceBindRequestSchema: z.ZodType<WorkspaceBindRequest, WorkspaceBindRequest> = z
  .object({
    ...
    // keeps traversal off `RepoAttachRequest.localPath` above.
    //
