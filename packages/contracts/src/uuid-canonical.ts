 ...
 * i.e. `z.string().uuid()`) are therefore case-INSENSITIVE and accept an
 * uppercase or mixed-case UUID unchanged (the validator performs no
 * normalization). Whenever a UUID is used as a **Map key** or a **hash
 * input**, two case-variants of one logical id must collapse to a single key
 * or they split / lose state — so callers MUST canonicalize first.
 *
