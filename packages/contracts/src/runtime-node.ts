...
// (T1.3, same file — api-payload-contracts.md:512-514). Hoisted to a single
...
// --------------------------------------------------------------------------
//
// Canonical wire: api-payload-contracts.md:496-509. The request carries the
...
export const RuntimeNodeAttachResponseSchema: z.ZodType<RuntimeNodeAttachResponse> = z
  .object({
    // `attachmentId`: the wire contract (api-payload-contracts.md:505) types
...
//
// Canonical wire: api-payload-contracts.md:518-528. Method
...
// the broad field T1.2 shipped (api-payload-contracts.md:522); the request→
...
// --------------------------------------------------------------------------
//
// Canonical wire: api-payload-contracts.md:511-516. Method
...
// body (api-payload-contracts.md:516,569): the resolver returns `null`, which
...
//
// Canonical wire: api-payload-contracts.md:530-535. Method `runtimenode.detach`
...
// No-content response. The wire payload is literally `null`, NOT a 204 empty
// body (api-payload-contracts.md:535,571): the resolver returns `null`, which
// tRPC serializes as an ordinary HTTP 200 success envelope
// `{ result: { data: null } }`, and the JSON-RPC daemon transport returns
// `result: null`. Both are validated by this `z.null()` schema, so
...
// --------------------------------------------------------------------------
//
// Canonical wire: api-payload-contracts.md:537-557 (registry row at :571,
...
//
// BOTH-AXES STANCE (Spec-003 line 72, never-mask): each entry carries BOTH
...
export const RuntimeNodeRosterEntrySchema: z.ZodType<RuntimeNodeRosterEntry> = z
  .object({
    ...
    // Spec-003 AC2 (line 128) needs `degraded`/`offline` nodes visible and
    // distinguishable from healthy ones.
    state: NodeStateSchema,
    // LIVENESS axis — `runtime_node_presence.health_state` carried VERBATIM:
    // the 3-value sweep-owned presence enum (shared-postgres-schema.md:223),
...
export const RuntimeNodeRosterResponseSchema: z.ZodType<RuntimeNodeRosterResponse> = z
  .object({
    ...
    // Spec-003 line 49 / AC3 (line 129: multiple runtime nodes coexist without
...
// `session.clock_corrected` (Spec-006:414-415) sit in the SAME EventCategory
// (`runtime_node_lifecycle`) but retain the `session.` prefix by name-
// preservation (Spec-006:417 / ADR-018 §Decision #8 — an event-type rename is
...
// `runtime_node.*` prefix set ONLY (Spec-006:407-413), exactly per the C4
...
// (Spec-006:401). `sessionId` is `.optional()` per Spec-006's `sessionId?` base:
// the daemon always populates it for `runtime_node.*` events (Spec-006:403,
...
// Payload` (api-payload-contracts.md §Plan-006, lines 779-791) carry NO base /
...
// --------------------------------------------------------------------------
//
// Spec-006:407 ("base + {capabilities[], nodeVersion, platform}"). Emitted by the
...
// --------------------------------------------------------------------------
//
// Spec-006:408 ("base"). Emitted by the T2.1/T2.2 path only AFTER
...
// --------------------------------------------------------------------------
//
// Spec-006:410 ("base + {lastHeartbeatAt, reason ∈ ['heartbeat_lost',
...
// --------------------------------------------------------------------------
//
// Spec-006:412 ("base + {capability, capabilityDetails}"). Emitted by the T2.2
...
// listing omits (the canonical doc documents extensions inline, per Spec-006:401).
//
...
// tools: NormalizedProviderToolMetadata[]}`, api-payload-contracts.md:1003-1007)
...
// diff capability snapshots structurally — api-payload-contracts.md:1018-1023),
