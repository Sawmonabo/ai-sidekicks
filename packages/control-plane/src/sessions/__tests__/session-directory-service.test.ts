...
// Post Plan-002 Amendment 2 (PR #102) the canonical-path applies every
// registered version (v1 + v2, plus v3 after Plan-003 Phase 3); the dedicated
...
describe("applyMigrations — idempotency", () => {
  it("re-running applyMigrations on a migrated database is a no-op", async () => {
    ...
    // `MIGRATIONS` array — extended to `[v1, v2, v3]` by Plan-003 Phase 3
    // (the runtime-node attachments/presence migration). The canonical-path
    // bootstrap leaves ALL version anchor rows in schema_migrations, so the
    // assertion shape tracks the registered set. The expanded canonical-path
    // coverage (R1+R2) lives in the dedicated `migration-runner.test.ts` test
    // file; this assertion remains the composition-level proof that running
    // the migration runner through the directory-service test fixture
    // preserves the same idempotency.
  ...
  it("applyMigrations is concurrency-safe — concurrent calls on the same fresh database serialize via advisory lock (Codex R8)", async () => {
    ...
    //       exactly once (`schema_migrations` carries the v1 + v2 + v3
    //       anchor rows — v3 added by Plan-003 Phase 3; `participants`
    //       table exists from v1).
    ...
    try {
      ...
      // The runner iterates `[v1, v2, v3]` (v3 added by Plan-003 Phase 3),
      // so ALL anchor rows must be present (a regression that drops a
      // version would surface here as a shorter array).
