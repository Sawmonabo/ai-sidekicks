// Control-plane runtime-node domain exceptions (
//
...
// cross-session info-leak (`RUNTIME_NODE_*_CODE`).
//
// Transport wiring is LIVE as of: the runtime-node router catch-arms
...
// subclass onto `shape.data.aisError` via the base `instanceof` (collapsed the
// per-class formatter branches onto the base and landed the deferred
// runtime-node projection). also ADDS the version-floor throwable below
...
// distinct from the two attach-time refusals.
//
// Throw discipline (identical to SessionDirectoryService): throw from inside the
// `Querier.transaction(...)` callback OR the service body. The `pg.Pool` and
// PGlite adapters both auto-run `ROLLBACK` on throw and re-raise, so a thrown
// refusal leaves `runtime_node_attachments` byte-for-byte unchanged (the
// no-mutation property conflict/revoked tests assert).
//
...
/**
 * Thrown by `AttachService.attach` for BOTH attach conflict triggers — one
 * typed code, two call sites, distinct messages:
 *   1. Cross-session active collision (P9 /: a node has at most one attachment in
 *      an active state across all sessions). The node is already actively attached
 *      to a DIFFERENT session — the collision the partial-unique
...
/**
 * Thrown by `AttachService.updateCapabilities` for BOTH of the
 ...
 *      (`runtime_node_attachments.state`), so there is no active row — and thus no
 *      `{nodeId, state, updatedAt}` to return. A liveness-only staleness sweep
 *      does NOT trigger this refusal: the sweep writes only
 ...
 *      reconcile at READ time). This mirrors `attach`'s defensive posture (the
 *      service does not assume the router pre-validated the node's liveness).
 *   2. State-context guard: the request would drive a `registering` attachment to
 *      `online`. The wire VALUE `online` is legal (the 2-value
 ...
 *      DAEMON-side capability declaration, and the control plane is not the
 *      declaration authority. The permitted capability-health axis is `online <->
 *      degraded`. The other illegal transitions (`offline`/`revoked`) never reach
 *      this method: `healthChanges.state` is the 2-value enum, so they are
 *      rejected at the SCHEMA boundary, not here.
 *
 ...
 * (the formatter wiring landed alongside theirs).
 */
 ...
 * session's `min_client_version` floor — attempts the capability WRITE. This is
 * the typed `VERSION_FLOOR_EXCEEDED` write-refusal: the node was admitted
 * (admit-not-eject) and its reads succeed, but the write is refused. The throw
 * rolls the transaction back, leaving the attachment row byte-for-byte unchanged,
 * so the node stays JOINED — never ejected for the floor mismatch.
 *
 ...
 *     CODE, emitted via the generic `aisError {code, message}` envelope (canonical
