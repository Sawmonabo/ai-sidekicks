// AttachService — Phase 5: runtime-node attach + detach +
// capability-update + roster-read handler.
//
// Responsibilities (attach flow + version-floor verdict + detach
// + Capability update + roster read):
//   * attach — admit a runtime node into a session by upserting its
//     `runtime_node_attachments` row, returning the wire `RuntimeNodeAttachResponse`
//     (`{attachmentId, state, readOnly, attachedAt}`).
...
//         (P3, `readOnly = true`) — admit-not-eject.
...
//   * detach — the clean-disconnect `offline` transition ("an explicit `detach`
//     retires the node"). Moves the node's SINGLE active attachment to `offline`
//     across two orthogonal axes: the SLOT axis (`runtime_node_attachments.state ->
//     offline`, guarded to active states) and the LIVENESS axis
//     (`runtime_node_presence.health_state -> offline`, UPDATE- only). Resolves the
//     one active row by `nodeId` alone (the request carries no `sessionId`).
//     Idempotent: a node with no active attachment is a clean `null` no-op (presence
//     untouched). Writes the terminal state `offline` ONLY — it is NOT a `revoked`
//     producer (see the detach scope note under Cross-task boundaries).
//   * updateCapabilities — the capability-declaration refresh ("the node declares its
//     capabilities"). Resolves the node's SINGLE active attachment by `nodeId` (the
//     request carries no `sessionId`), then refreshes its `capabilities` discovery
//     snapshot and applies the resolved next-state, all inside ONE
//     `Querier.transaction(...)`. Enforces TWO refusals that share
//     `runtime-nodes/errors.ts`'s typed family:
//       - `VERSION_FLOOR_EXCEEDED` write-refusal — a below-floor node (admitted
//         read-only at attach) is denied the capability WRITE. The read-only
//         verdict is RE-DERIVED here from the CURRENT session floor
...
//       - registering->online guard — driving a still-`registering` attachment to
//         `online` is refused (the typed
//         `RuntimeNodeCapabilityUpdateConflictException`): bringing a node online
//         requires a successful daemon-side capability declaration, and the control
//         plane is not the declaration authority.
...
//   * readRoster — the session roster projection. ONE read-only SELECT returning EVERY
//     `runtime_node_attachments` row for the session — all five `state` values
//     verbatim, no server-side hiding (needs degraded/offline nodes visible) —
//     LEFT-JOINed with the heartbeat-owned presence axis (NULL until the node's first
//     beat) and carrying a per-row `readOnly` verdict derived AT READ TIME via the
//     SAME `#deriveReadOnly` comparator attach uses (the read-side surfacing of
//     admit-not-eject). Joins THIS class rather than a second service: the roster is a
//     projection over the SAME `runtime_node_attachments` row lifecycle attach /
//     detach / updateCapabilities own ("not a fragmented second service class"
//     cohesion rationale). Mounted by the sibling router as `runtimenode.roster` — the
//     namespace's FIRST (and only) query, control-plane tRPC ONLY.
//
...
//   * Enforced at the DATABASE layer by the partial-unique
//     `idx_node_attachments_active` (one active-state row per node across all
//     sessions). The service does not re-check this in application code — it
//     lets the index raise `23505` on the cross-session second-active-attach and
//     translates that to the typed conflict refusal (the no-TOCTOU posture: the
//     unique index is the single source of truth, so two racing attaches cannot
//     both win).
//
...
//     this task ships only the `Querier`-injected service, so the test substrate
//     (in-process PGlite) and the eventual production surface (`pg.Pool`) stay
...
//     THROWN here in this service. The attach-time logic only sets the read-only
//     VERDICT (`#deriveReadOnly`: a below-floor daemon is admitted `readOnly =
//     true`, admit-not-eject); the typed write-refusal is ENFORCED in this file's
...
//     `runtime_node_attachments.state = 'revoked'`. NOT in scope here: `revoked` is a
//     trust decision issued by an authority ABOUT the node (never self-asserted),
//     whose proper home is a Cedar- gated authority surface, NOT this daemon-facing
//     detach (a daemon cannot self-revoke; the wire `RuntimeNodeDetachRequest` carries
//     no disposition discriminator). The `revoked` STATE itself + attach's P10
//     terminal-re-attach refusal already ship — only the ungated producer is deferred.
//     detach writes the terminal state `offline` ONLY.
//   * Liveness (staleness) derivation — heartbeat sweep, the SINGLE
//     liveness-derivation writer. readRoster carries
//     `runtime_node_presence.health_state` / `last_heartbeat_at` VERBATIM and never
//     ages a heartbeat into a health verdict — deriving at read time would create a
//     second, racing liveness author (per-axis single-writer — "the read never derives
//     staleness").
//   * The tRPC router + errorFormatter that lift the typed `.code` onto the wire
//     envelope and map each refusal to HTTP 409 / tRPC `CONFLICT`
...
// The partial-unique index whose `23505` signals cross-session
...
// node advances to `online` only after registration/capability handshake
// (out of scope here); attach lands it at `registering`.
const REGISTERING_STATE: NodeState = "registering";

// The terminal state an explicit `detach` writes. Reused for BOTH detach writes (the
// same literal is valid in both column domains): the attachment SLOT axis
// (`runtime_node_attachments.state` — the clean-disconnect terminal state) and the
// presence LIVENESS axis (`runtime_node_presence.health_state` — the
// `online|degraded|offline` CHECK domain). It is the same liveness-death value
// staleness sweep derives at 60s, which an explicit detach effects IMMEDIATELY instead
// of waiting for heartbeat staleness ("an explicit `detach` retires the node").
...
// and the session's `min_client_version` floor (the per-row `readOnly`
...
// code?, constraint? }`) without depending on a driver-specific class. Used to
// distinguish active-index `23505` (which becomes the typed conflict refusal)
// from every other thrown error (which rethrows unchanged).
...
export class AttachService {
  ...
  /**
   * Attach a runtime node to a session.
   *
  ...
  async attach(request: RuntimeNodeAttachRequest): Promise<RuntimeNodeAttachResponse> {
    ...
    return this.#querier.transaction(async (transaction) => {
      ...
      // line 104). A missing session row surfaces as `floorRow === undefined`;
      // we treat that as "no floor" here — session existence/authorization is
      // the router's gate, not this service's (the FK on
      // `runtime_node_attachments.session_id` would also reject an INSERT
      // against a non-existent session at step 3).
      ...
      // persisted here: `state` is hard-pinned to `registering` — a node advances to
      // `online` ONLY via the capability- declaration handshake, and presence/health
      // lands on the first heartbeat (owns `runtime_node_presence`). Do NOT "fix" the
      // dropped field by writing health at attach; that would skip the handshake gate.
      ...
      try {
        ...
        // The partial-unique `idx_node_attachments_active` raised `23505` because
        // this node already holds an active-state row in ANOTHER session. The
        // `23505` can originate from EITHER statement phase, both translated
        // identically:
  ...
  /**
   * Detach a runtime node — the clean-disconnect `offline` transition ("an explicit
   * `detach` retires the node").
   *
   ...
   * row per node across all sessions, so `nodeId` resolves it unambiguously —
   * the request carries no `sessionId` (the wire shape is `{nodeId, reason?}`).
   * The retirement moves TWO orthogonal axes to `offline`: the attachment SLOT
   * axis (`runtime_node_attachments.state`) and the presence LIVENESS axis
   * (`runtime_node_presence.health_state`).
   *
   * Scope (settled): detach writes the TERMINAL state `offline` ONLY. It is NOT a
   * `revoked` producer. `revoked` is an authority-issued trust decision ABOUT the node
   * (never self-asserted), gated on a Cedar / authorization surface Phase 3 does not
   * ship; the only V1 caller of detach (the daemon-initiated `runtimenode.detach`) is
   * a clean disconnect, and a daemon cannot self-revoke. The `revoked` STATE +
   * attach's P10 terminal re-attach refusal already ship; this method declines to
   * invent an ungated, uncalled trust-mutation producer (its home is the future
   * authority surface).
   *
  ...
  async detach(request: RuntimeNodeDetachRequest): Promise<null> {
    ...
    // event that would carry an audit `reason` is V1.1-gated (no control-plane
    // event log in V1). Do NOT "fix" the dropped field by inventing a column —
    // that would lead the schema.
    ...
    return this.#querier.transaction(async (transaction) => {
      ...
      // (2) Liveness axis — effect the node's liveness-death immediately, the same
      // `health_state = 'offline'` staleness sweep derives at 60s, without waiting for
      // heartbeat staleness. UPDATE-ONLY, never an upsert/INSERT: presence rows are
      // heartbeat-owned (creates the row on the first beat), and
      // `runtime_node_presence.last_heartbeat_at` is `NOT NULL` with no default — an
      // INSERT here would be both wrong (detach is not a heartbeat) and unsatisfiable
      // (no timestamp to write). A node that never heartbeated has no presence row, so
      // this is a clean 0-row no-op. `health_state = 'offline'` is CHECK-valid (the
      // presence domain is `online|degraded|offline`).
   ...
   * `runtimenode.capabilityupdate` coordination-snapshot refresh
   ...
   * event log; the daemon's node-capability service stays the event writer. This
   * method only updates the coordination row.
   *
   ...
   *   `healthChanges.state` is the 2-value `RuntimeNodeHealthState` (`online |
   *   degraded`), so `offline` / `revoked` are UNCONSTRUCTABLE at the schema
   *   boundary (make-illegal-states-unrepresentable); this method never sees
   *   them.
   ...
   *   admitted READ-ONLY at attach because its declared `clientVersion` is below the
   *   session's `min_client_version` floor — attempts this capability WRITE (the typed
   *   `VERSION_FLOOR_EXCEEDED` write-refusal). The node is NOT ejected — the throw
   ...
   *   (1) no active attachment exists — the SLOT axis was retired by a `detach` or
   *   `revoke` (there is no `{nodeId, state, updatedAt}` to return).
   *
   * Transaction sequence (ONE commit boundary):
   ...
   *      unambiguous), selecting `client_version` + `session_id` too (the
   *      floor-gate at step 3 needs both).
   *   2. Zero rows -> the no-active-attachment refusal (case 1 above).
   *   3. Version-floor write-gate: re-derive the read-only verdict at WRITE time from
   *      the CURRENT session floor + the version the daemon declared at attach. A
   *      below-floor node is refused with `VersionFloorExceededException` (typed,
   *      never-eject — the throw rolls back, the attachment row is untouched, the node
   *      stays joined). This precedes guard: a permission denial (read-only node) is
   *      decided BEFORE the transition-legality check.
   *   4. Guard: a `registering` row + a requested `online` health is the one
   *      residual state-context refusal (case 2). EXPLICITLY ALLOWED (not
   *      guarded): `registering -> degraded` (capability-validation failure
   *      leaves the node degraded), `degraded -> online` (recovery), and a
   *      capabilities-only refresh on any active row.
   ...
   * heartbeat — else staleness sweep marks it offline (a de-facto ejection) —
   * and MUST be able to `detach` — else it is trapped joined. Gating either
   * would violate the never-eject guarantee. `heartbeat` (the separate
   * `HeartbeatService`) and `detach` are therefore naturally ungated; the
   * capability WRITE is the one mutation a read-only node is denied.
   */
  async updateCapabilities(
    ...
    // write (no control-plane event log; the daemon's node-capability service is
    // the event writer). Do NOT "fix" the dropped field by inventing a column —
    // that would lead the schema. This method reads only `healthChanges?.state`
    // (the liveness transition) below.

    return this.#querier.transaction(async (transaction) => {
      // (1) Resolve the node's SINGLE active attachment FOR UPDATE. The `state IN
      ...
      // `detach` or `revoke`) matches no row (the typed refusal below). A
      // liveness-only staleness sweep does NOT retire the slot (it writes only
      // `runtime_node_presence.health_state`), so a swept node keeps an active
      // slot and still matches this band. FOR UPDATE locks the row for the
      // transaction's duration so a concurrent detach cannot retire it between
      // this read and the UPDATE below. Single-table query, so the lock covers
      // only the attachment row.
      //
      ...
      // (2) No active attachment -> typed 409 refusal (NOT a 500, NOT a null no-op).
      // capabilityupdate MUST return `{nodeId, state, updatedAt}`; with no active row
      // there is no state to return. This is a legitimate production race — a late
      // update arriving after a `detach` OR `revoke` retired the SLOT axis
      // (`runtime_node_attachments.state`). This mirrors attach's defensive posture
      // (the service does not assume the router pre-validated liveness). The message
      // names `nodeId` ONLY (no session, no state — no info-leak).
      ...
      // Re-derive the read-only verdict at WRITE time from the CURRENT session floor
      // and the version the daemon declared at attach (stored `client_version`). A
      // below-floor node was admitted read-only; this is where its capability WRITE is
      // refused — typed, never-eject (the throw rolls the transaction back, leaving
      // the attachment row byte-unchanged, so the node stays joined). This precedes
      // state-context guard below: a permission denial (the node is read-only) is
      // decided BEFORE we consider whether the requested transition is legal.
      //
      ...
      // (4) guard — the ONE residual state-context refusal. The wire VALUE `online` is
      // legal (the 2-value `RuntimeNodeHealthState`), but applying it to a
      // still-`registering` attachment is not: bringing a node `online` requires a
      // successful DAEMON-side capability declaration, and the control plane is not
      // the declaration authority. EXPLICITLY ALLOWED and NOT guarded here:
      //   - `registering -> degraded` (a capability-validation failure
      //     leaves the node degraded);
      ...
      // case. The message names the `nodeId` + the PUBLIC RULE, never the node's
      // current `state` (no info-leak — a caller learns the rule, not the node's
      // internal liveness position).
   ...
   * coordination-record projection (wire shapes registry row) table).
   *
   * FAITHFUL PROJECTION: returns EVERY `runtime_node_attachments` row for the
   * session — all five `state` values verbatim (`registering | online | degraded |
   * offline | revoked`), no server-side hiding — because the distinguishability
   * requires `degraded` / `offline` nodes visible, and the row count is bounded by
   * distinct nodes ever attached (`UNIQUE(node_id, session_id)`).
   *
   * BOTH AXES VERBATIM, NEVER COLLAPSED: each entry carries the SLOT axis
   * (`runtime_node_attachments.state`) AND the LIVENESS axis
   * (`runtime_node_presence.health_state` + `last_heartbeat_at`, LEFT-JOINed on
   * `node_id` and NULL until the node's first heartbeat — presence rows are
   * heartbeat-owned) with NO collapsed health scalar. Reconciling the two axes is the
   * CLIENT's render-time concern; a row whose axes disagree (e.g. slot `online` with
   * swept liveness `offline`) round-trips as stored.
   *
   * NO STALENESS DERIVATION: the read NEVER ages `last_heartbeat_at` into a health
   * verdict — staleness sweep stays the SINGLE liveness-derivation writer. A
   * stale-but-unswept row reports its stored `health_state` verbatim; deriving here
   * would create a second, racing liveness author.
   *
   * DERIVED `readOnly`: computed per row AT READ TIME from the stored `client_version`
   * against the session's CURRENT `min_client_version` floor via the same
   * `#deriveReadOnly` comparator the attach-time verdict and write-gate use —
   * identical semantics by construction (NULL floor -> `false`). This is the read-side
   * surfacing of admit-not-eject: a below-floor node appears in the roster joined, its
   * `state` untouched, with `readOnly = true` — never hidden, never ejected. The
   * stored `client_version` is parsed+branded (`EventEnvelopeVersionSchema.parse`,
   * never an `as` cast) so a corrupted stored version fails CLOSED at the read
   * boundary rather than reaching the comparator as NaN.
   *
   ...
   * `runtime_node.*` event (the control plane has no event log; the read
   * PROJECTS coordination records, so it does not collide with which governs
   * durable event AUTHORSHIP, not coordination-record reads).
   *
  ...
  async readRoster(request: RuntimeNodeRosterRequest): Promise<RuntimeNodeRosterResponse> {
    ...
    // table and its `runtimenode.leaseupdate` producer both belong to lease leg;
    // until that ships nothing writes a lease row for this session, so no live
    // holder exists to advertise and `null` is exactly what a join against an
    // empty relation would yield. The member is required on the wire so a client
    // cannot read "nobody holds it" out of an absent key, and the two `null`
    // readings — a free lease, and one suppressed behind an offline producer —
    // are deliberately indistinguishable here as they are on every other path.
   ...
   *     never eject — the node stays joined and reads succeed; the write refusal
   *     (`VERSION_FLOOR_EXCEEDED`) is enforced downstream, not here.
   *
   * Isolating the verdict behind this private seam keeps the upsert path stable:
   * the call site (the transaction body) is unchanged across -> extension — only
   * this method's non-NULL branch gained the comparison. The seam is now shared
   * by THREE call sites — the attach-time verdict write-gate's re-derivation,
   * and per-row roster projection — so all three stay semantically identical by
   * construction (one comparator, no drift).
   *
  ...
  #deriveReadOnly(floor: string | null, clientVersion: EventEnvelopeVersion): boolean {
    ...
    // admit in read-only, never eject.
