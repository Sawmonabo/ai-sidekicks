...
// HTTP path exercises all four here — the runtime-node refusals, all
// code+message-only, so `aisError` carries NO `details` key:
// `VersionFloorExceededException`, `RuntimeNodeAttachConflictException`,
// `RuntimeNodeAttachRevokedException`, and
...
// caller. They prove the formatter (a single `AisWireException` base
// `instanceof`) projects EVERY concrete subclass uniformly: the four
// runtime-node refusals take the `cause.details === undefined` branch, so
// `aisError` carries NO `details` — the version-floor write-refusal, the attach
// cross-session conflict, the attach revoked-row refusal, and the
// capability-update conflict.
