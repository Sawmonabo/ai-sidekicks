...
// traversed, and a resolved root the daemon cannot open for enumeration —
...
//
// It IS asserted to be a directory the daemon can ENUMERATE, and that is a
// containment question rather than a usability one once you follow where the
// value goes. Its only purpose is to become `workspaces.fs_root` — the root a
...
// filesystem from changing between validation and use: a directory can be
// replaced by a file, have its mode changed, or be removed after the probe and
// before the spawn, exactly as a resolved path can be re-pointed after the
// `realpath`. TOCTOU hardening at the execution boundary is out of scope for a
// bind-time check.
...
// asserted nowhere in an ubuntu-only CI.
//
// The readability probe is the one seam the two modules SHARE rather than
// duplicate. It is declared here and imported by the resolver — that direction,
// because the resolver already imports the comparison helpers below, and the
// reverse edge would close a real import cycle. Sharing the declaration is what
// keeps the two answers identical, which is the argument those helpers make.
//
// Both modules need it for one reason: each RETURNS a path for persistence
// rather than consuming it, so nothing downstream asks on its behalf and each
// must ask itself. The question is the same on both sides — can the daemon open
// this directory for enumeration? — because both values become roots a process
// is later expected to run inside.
//
// It replaced a `stat` that asked only whether the resolved root was a
// DIRECTORY. Opening SUBSUMES that, since a regular file rejects `ENOTDIR`, and
// adds what `stat` cannot answer: a `0111` directory stats as a directory and
// lists for nobody. Bind is exactly where such a root would be written to
// `workspaces.fs_root`.
 ...
 *   4. the module-local `resolveDeps` defaulting helper below — not
 *      byte-identical, and not meant to be: the resolver additionally defaults
 *      three git-execution members (`executeFile`, `gitExecutablePath`,
 *      `gitCommandTimeoutMs`) this module has no use for. That is the ONLY
 *      divergence. This module's member set is otherwise a strict SUBSET of the
 *      resolver's, and every member they share defaults to the same thing —
 *      `platformPath` to `node:path`, `realpath` to the twinned
 *      `DEFAULT_REALPATH` (entry 6), and `probeDirectoryReadable` to the one
 *      binding declared here and imported there, which cannot diverge at all.
 *      The `??`-per-member idiom matches, and a member added to the SHARED part
 ...
 * deliberate divergence entry 4 records. The fifth is a CONTAINMENT rule, so
 ...
 * above, which is why a seam both modules now carry added no seventh twin.
 * Sharing is a third relationship, and the only one a COMPILER enforces — the
 * sixth twin is held by tests, the first five by this note alone. Prefer it
 * whenever the import direction allows: every surface moved from the list to
 * the shared set is one this note no longer has to police.
 */
...
export interface TrustEnvelopeValidatorDeps {
  ...
  /**
   * Defaults to `DEFAULT_DIRECTORY_READABILITY_PROBE` below. Asked exactly one
   * question, of the RESOLVED root: can it be opened for enumeration?
   *
   * `opendir` rather than `access`: the probe IS the operation, so its answer
   * cannot disagree with the act it predicts. `access(2)` answers a related but
   * different question — it "uses the real user ID in place of the effective
   * user ID" — and its own SECURITY CONSIDERATIONS reject the pre-flight shape
   * outright: "The actual access control decision should be made by attempting
   * to execute the relevant system call while holding the applicable
   * credentials … even though access() may have predicted success." `readdir`
   * would be equally honest and is the wrong size: it materializes an entire
   * listing neither module reads, on a directory whose contents are unbounded.
   * Opening and closing settles the question and keeps nothing.
   *
   ...
   * hard-wired probe would refuse every win32 acceptance test; one POSIX case is
   * worse still, since `/srv` exists on a Linux runner and not on macOS, which
   * would make that verdict a property of the host.
   *
   * Nothing here distinguishes a symlink, as the `stat`-versus-`lstat` question
   * did not: the probed value is `realpath`'s OUTPUT, whose final component is
   * already resolved.
   */
 ...
 * garbage collector gets to it, and this runs on every successful validation
 * and every successful resolution. The close is awaited so a failure to release
 * surfaces here rather than as an unhandled rejection later.
 */
...
export class TrustEnvelopeValidator {
  ...
  public async validateExecutionRoot(candidate: WorkspaceExecutionRootCandidate): Promise<string> {
    ...
    // Step 6 — the resolved root must be a directory the daemon can OPEN FOR
    // ENUMERATION. Containment says where the path is; this says whether it can
    // serve as `workspaces.fs_root` at all, and nothing downstream re-asks
    // (header). One probe answers both halves: a regular file rejects `ENOTDIR`,
    // a `0111` directory rejects `EACCES`. A probe that throws is refused for
    // the reason a `realpath` that throws is — an unprovable property is not a
    // satisfied one. Last, so every earlier refusal keeps its own cause.
    try {
