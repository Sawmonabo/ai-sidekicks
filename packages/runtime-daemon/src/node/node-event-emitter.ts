...
//     REQUIRED at this boundary (Spec-006:370 — the daemon always populates
...
interface RuntimeNodeEmitBase {
  ...
  // daemon always populates the session_id (Spec-006:370). Also the
...
export class RuntimeNodeEventEmitter {
  ...
  /**
   * Emit `runtime_node.registered` (Spec-006:374). Returns the persisted
  ...
  /**
   * Emit `runtime_node.online` (Spec-006:375). The T2.4 ordering gate only
  ...
  /**
   * Emit `runtime_node.offline` (Spec-006:377). Phase 2's T2.5 detach
  ...
  /**
   * Emit `runtime_node.capability_declared` (Spec-006:379). Reduced base +
  ...
  /**
   * Emit `runtime_node.capability_updated` (Spec-006:380). Here
