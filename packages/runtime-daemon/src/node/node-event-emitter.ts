...
//     payload-shape schema's `.parse()` (validation seam, in place of
//     ad-hoc objects). The PARSED output — not the caller's input object —
//     is what gets persisted, so storage always reflects the schema's
//     normalized shape.
//   * Reconciles the two fields that appear in BOTH the envelope and the payload
//     (`sessionId`, `actor`): one input value populates both, so a caller cannot
//     make the envelope and payload disagree. `sessionId` is REQUIRED at this
//     boundary (the daemon always populates the session_id of the attachment a
//     `runtime_node.*` event describes), even though the payload schemas type it
//     `.optional()` to mirror base.
...
//
// What this module NO LONGER does (moved by the re-point — do not
// reinstate here):
//   * Allocates the per-session `sequence`. the log-derive allocator
//     (`readEvents`, last + 1) and the `nextSequence` injection seam are
//     BOTH gone, and their removal is the point rather than a
...
//     anchor. Phase 2 defines the 5 schema-backed shapes only.
//
...
// matching the existing event convention (session-service.test.ts fixtures
// all carry "1.0").
//
...
// Structural on purpose — it names no concrete class precondition ("the shipped
// `RuntimeNodeEventEmitter` re-pointed off" `SessionService.append`): the two
// methods are the exact surface the emitter consumes, so any implementation
// satisfies it without a nominal dependency.
//
// ASYNC-TRANSACTIONAL BY CONTRACT — re-point INVERTED this seam, and the
// inversion is worth reading carefully because the old contract said the
// exact opposite.
//
...
// typed against its own parameter and return types so a signature change
// there fails THIS compile rather than drifting. `readEvents` is GONE:
// it existed only to feed the log-derive sequence allocator, which the
// append path now owns (see the header). Keeping a dead read on the seam
// would oblige every test fake to implement a method nothing calls.
...
export interface RuntimeNodeEventEmitterDeps {
  ...
  // Monotonic clock for `monotonic_ns` (within-daemon ordering only).
  // Injectable so the D6 can drive non-monotonic values THROUGH this emitter
...
// (no consumer imports it — call via the `EmitXInput` types). Used to
// tighten the internal append helper's `payload` parameter from
// `Record<string, unknown>` to "a payload that has already cleared a
...
// the payload schemas type `sessionId` as `.optional()` (base), but the
// envelope REQUIRES it as the per-session partition/sequence key — so a
...
export interface EmitOfflineInput extends RuntimeNodeEmitBase {
  ...
  // Phase 2's detach producer passes `explicit_shutdown`; the full enum is
  // honored at the schema so Phase 3 adds producers, not a shape change.
...
export interface EmitCapabilityUpdatedInput extends RuntimeNodeEmitBase {
  ...
  // contracts payload type, whose canonical-first tolerant union admits
  // both producers uncast: the typed driver-capabilities-writer snapshot
  // (`CapabilityDetails`) and node-capability-service's JSON-round-tripped
  // records.
...
function narrowAppendReceipt(value: unknown): EventLogAppendReceipt {
  if (typeof value !== "object" || value === null) {
    throw new Error(
      `SessionEventLog.append resolved to ${value === null ? "null" : `a value of type ${typeof value}`} instead of an EventLogAppendReceipt: this seam is the EventLogService.append, whose receipt carries the id, sequence and 32-byte row hash the append committed.`,
    );
  }
...
export class RuntimeNodeEventEmitter {
  ...
  /**
   * Returns the persisted `AppendableEvent` so the caller (registry) can read
  ...
  /**
   * Ordering gate only calls this AFTER a successful
   * `runtime_node.capability_declared` — the gate lives in the producer, not
   * here.
   */
  ...
  /**
   * Phase 2's detach producer passes `reason: "explicit_shutdown"`; the
   * heartbeat-driven reasons are Phase 3.
   */
  ...
  /**
   * Reduced base + `{capability, capabilityDetails}` — capability events are NOT
   * NodeState transitions, so they carry no `previousState`/`newState:
   * NodeState`.
   */
  ...
  /**
   * Here `previousState`/`newState` are `CapabilityDetails` SNAPSHOTS (NOT
   * NodeState); the contracts payload schemas bind the canonical
   * `CapabilityDetails` canonical-first over a tolerant record arm, and the
   * input seam carries that same union via indexed access, so typed snapshots
   * arrive uncast.
   */
    ...
    if (!isThenable(appendResult)) {
      throw new Error(
        "SessionEventLog.append did not return a promise: this seam is async-transactional " +
          "(the EventLogService.append serializes on withSessionAppendLock and" +
