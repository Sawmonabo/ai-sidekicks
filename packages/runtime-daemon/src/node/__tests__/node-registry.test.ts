...
//   * D1 (Plan-003 T2.1 / Spec-003:78,90, AC1): register a node, CLOSE + REOPEN
...
//   * Spec-003:96 (no implicit capability exposure on attach): registering a node
...
// does NOT populate the capability table (Spec-003:96, no implicit capability
...
// ----------------------------------------------------------------------------
// D1 — durable identity recovered across DB reopen (Spec-003:78,90, AC1)
// ----------------------------------------------------------------------------
...
// ----------------------------------------------------------------------------
// Happy-path emit shape (Spec-006:374) + envelope wiring + Spec-003:96
// (no implicit capability exposure on attach)
// ----------------------------------------------------------------------------

describe("NodeRegistry — emits runtime_node.registered (Spec-006:374)", () => {
  it("lands exactly one runtime_node.registered event with the Spec-006:374 payload shape and exposes NO capability (Spec-003:96)", () => {
    const registry: NodeRegistry = makeRegistry();
    registry.register({
      nodeId: NODE_ID,
      sessionId: SESSION_ID,
      ...
      // `node_capabilities` rows (Spec-003:96, asserted below).
      capabilities: { "provider-driver": { contractVersion: "1.0" } },
      nodeVersion: "1.4.2",
      platform: "darwin-arm64",
    });
    ...
    // Spec-003:96 — no implicit capability exposure on attach: even though the
