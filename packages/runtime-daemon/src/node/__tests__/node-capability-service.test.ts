...
//     their heartbeat/admin producers), so D3 codifies NONE of :76.
...
// (capability/trust changes emitted as session events), line 127 (serial re-attach
...
// ABSENCE via the :57 gate, not validation failure. Spec-003:109 — no implicit
...
describe("NodeCapabilityService — change-detection is node-scoped, not session-scoped (Model B)", () => {
  it("treats an identical re-declare in a DIFFERENT session as a no-op — no capability_declared in the second session", () => {
    ...
    // supported serial re-attach (Spec-003:127). `node_capabilities` is node-keyed
