 ...
 * method (Spec-006:410 authors the full enum; V1.1 adds a producer, not a shape).
 *
...
export class NodeRegistry {
   ...
   * (Spec-006:410) for the explicit-shutdown trigger and LEAVES THE
   ...
   * producer, not this method (Spec-006:410 authors the full enum so Phase 3 adds
  ...
  detach(input: DetachNodeInput): void {
    this.#emitter.emitOffline({
      ...
      // clock, never null (Spec-006:410 / `z.iso.datetime`).
