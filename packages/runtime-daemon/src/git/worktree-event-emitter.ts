// Worktree lifecycle event emission — the single seam every Phase-2 worktree
// state transition appends its event through.
//
...
// --------------------------------------------------------------------------
// Type → post-transition state. mapping, in one place.
// --------------------------------------------------------------------------
//
...
// and — the part that carries at COMPILE time — confines its values to the
// five evented states. Annotating the values `WorktreeState` would have
...
// its union arms reject a `worktree.created` carrying a state), so this is a
// mapping two surfaces hold rather than a local convention.
...
// MAJOR.MINOR matching the daemon's existing convention and the `1.0` the
// sibling repo/workspace emitter writes for the same family.
//
...
// holds an identical constant): that module belongs to a different owner, the
// declared consumes here are `EventLogService` + contracts only, and a shared version constant
// would assert a lockstep neither plan owes the other — the same reasoning
// the precedent gives for declaring its own append-seam interface instead of
// importing runtime-node's identically-shaped one.
...
/**
 * Structural on purpose — it names no concrete class, and it is typed against
 * the append path's OWN parameter and return types so a signature change there
 * fails THIS compile rather than drifting. Declared locally rather than
 * imported emitter's identically-shaped seam: sharing that export would create
 ...
 * immediately before it. That is the mechanism the "transactionally with the
 * row write" rests on: a throwing INSERT rolls the row write back, and a
 * refusal before the transaction opens means the prelude never runs at all.
 */
 ...
 * worktree), so the two-shape split precedent needed — where the six events
 * divide by subject between mounts and workspaces — collapses here.
 *
...
export interface EmitWorktreeEventInput {
  ...
  // The worktree this event describes. REQUIRED on every one of the five — the
  // emitter obligation, restated in `WorktreeLifecyclePayload`'s own doc
  // comment: `worktreeId` is populated on every `worktree.*` emission. The
  ...
  // same transaction as the INSERT, immediately before it. This is how the
  // "transactionally with the row write" is discharged: the `worktrees`
  // INSERT/UPDATE rides down here rather than running in a transaction of the
  // producer's own. The constraints on what may go in one (synchronous, same
  // connection, and no read whose result shapes THIS row's PAYLOAD — which was
  // canonicalized and signed before the transaction opened) are documented on
...
export class WorktreeEventEmitter {
   ...
   * `creating`. Resolves to the append receipt so the producer can read the
   * `sequence` the append path ASSIGNED rather than one this emitter guessed.
   */
   ...
   * stamps `cleaned_at` afterwards, so a `retired` event never implies the
   * root is gone.
   */
  ...
  async #appendWorktreeEvent(
    ...
    // type → category bijection is asserted, and the strict layer refuses an
    // envelope whose category disagrees with its type — so a hardcoded
    ...
    // event row, silently undoing the row/event atomicity depends on. A
    // tripwire, not a recovery path: by the time we look, the implementation's
    // work has already happened. It exists to make such wiring LOUD on the
    // first emit any test exercises, rather than report success over a
    // half-written pair. No cast: `isThenable` already takes `unknown`, and a
    // runtime check reads whatever value the seam actually returned — the
