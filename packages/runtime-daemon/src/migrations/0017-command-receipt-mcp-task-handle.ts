// Version-17 migration: the MCP Tasks durable recovery handle on the
// `command_receipts` substrate.
//
...
// column ORDER is the shape the last extender leaves behind and not a shape any
// single incremental migration can reproduce. ADD COLUMN` appends, so this column
// lands after `created_at` rather than between `completed_at` and
// `mcp_binding_digest`. Physical ordinal is not part of the contract — every
// reader names columns.
//
// ----------------------------------------------------------------------------
// Why ADD COLUMN carries this CHECK, and the does not
// ----------------------------------------------------------------------------
//
...
// `session_events.retention_class`, and exactly the distinction draws: a
// single-column NULL-permitting CHECK is ALTER-ADD-COLUMN-addable, while a
// TABLE-level (multi-column) CHECK requires the 12-step rebuild of
// sqlite.org/lang_altertable.html#otheralter.
//
// Paying that cost here would be worse than redundant — a rebuild DROPs and
// re-CREATEs a SHIPPED table, so it would have to hand-carry
// `idx_command_receipts_run` and would silently discard any column a sibling
// extender landed first. An append discards nothing.
//
// ----------------------------------------------------------------------------
...
// opaque SQLITE_CONSTRAINT from a driver stack frame. That is defense-in-depth
// convention the `runtime_bindings` provider-declared strings already follow;
// the two bounds are the same 256 literal by construction, the recorder
// exporting the constant this DDL states.
//
...
// `manual_reconcile_only` halt. recovery reads a non-NULL handle and polls
