...
// headers, the per-column comments, and the two partial-unique indexes) so
// the inline constant stays in lockstep with the canonical doc (the audited
// DDL there IS the migration content).
//
// ----------------------------------------------------------------------------
// Scope (this migration — version 4)
// ----------------------------------------------------------------------------
//
// Owns the physical CREATE for four Local SQLite tables:
//
//   * worktrees              — per-mount live checkouts with creating-session
//                              / creating-run provenance; the
...
//   tables that no migration creates yet. SQLite resolves FK targets lazily
//   at DML time, so these CREATE TABLE statements apply cleanly on a fresh
//   database; under `foreign_keys = ON` any INSERT into the referencing
//   columns fails until the parent tables exist. That failure is a
//   statement-compile error — `SQLITE_ERROR: no such table:
...
//   insert path exists until which chains after the migration. Do NOT strip
//   or weaken the REFERENCES clauses.
//
// Contract↔DDL lockstep (DDL half): conformance test string-extracts the
// CHECK clauses from this exported constant and compares the parsed sets
// against the `worktree.ts` contract enums — keep the constant exported and
// the CHECK clauses byte-verbatim.
//
// The `schema_version` anchor table itself is this migration only
// INSERTs its version-4 row.

export const WORKTREE_LIFECYCLE_MIGRATION_SQL: string = `
CREATE TABLE worktrees (
  id                    TEXT PRIMARY KEY,
  repo_mount_id         TEXT NOT NULL REFERENCES repo_mounts(id),
  created_by_session_id TEXT NOT NULL,              -- creating-session provenance (session ids are event-sourced — no FK, matching session_id columns elsewhere)
  ...
  fs_root               TEXT NOT NULL,              -- filesystem path to worktree (under the daemon execution-roots dir)
...
-- on disk (any non-retired, non-failed state, including 'merged') still holds the branch. Race
-- arbiter for the provenance-split collision policy.
...
CREATE TABLE ephemeral_clones (
  ...
  clone_root      TEXT NOT NULL,              -- filesystem path (under the daemon execution-roots dir)
  branch_name     TEXT NOT NULL,              -- head branch inside the clone (caller-supplied or slug-derived at prepare; the status read exposes it)
  ...
  expires_at      TEXT NOT NULL,              -- TTL deadline (daemon config, default 24h)
  created_at      TEXT NOT NULL,
  updated_at      TEXT NOT NULL,
...
CREATE UNIQUE INDEX idx_branch_contexts_worktree_workspace ON branch_contexts(worktree_id, workspace_id) WHERE worktree_id IS NOT NULL;  -- one binding row per (workspace, worktree) — upsert; the worktree-keyed BranchContextRead resolves on the pair

-- Run_id is event-sourced (runs live in the event log, not a table) — PRIMARY KEY without FK.
-- released_at stamps run-terminal release; a terminal-source rollback clears it atomically with the
-- run's re-open, and a rollback composite ending without a confirmed rewind restores it (campaign
-- B2 — the campaign's bundle owns the implementing task).
