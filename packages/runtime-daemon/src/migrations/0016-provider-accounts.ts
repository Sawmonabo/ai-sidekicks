// Version-16 migration: the node-local provider-account registry and its
// per-limit quota-window store.
//
...
//
// Blank lines are dropped on both sides. The rest is this file's alone, and the
// doc has no counterpart for it, because the doc says the same things in prose
// OUTSIDE its fences: the two per-table banner BLOCKS — each a `-- ---` rule
// line, a few description lines, and a closing `-- ---` rule line, so it is the
// whole block that is elided and not only the rules — and the trailing
// `schema_version` INSERT. After those, exactly one difference remains, and it
// is deliberate: the doc's identity-column comment closes with three lines
// recording WHEN the three observed-identity columns were added and which
// review found them missing. That is corpus provenance rather than a constraint
// on the column, so the comment here stops at "never carried on an error."
//
// ----------------------------------------------------------------------------
...
// D2 token lives ladder's store. There is therefore no column here to leak, to
// log, or to shred.
//
...
//     (accessed 2026-08-31). NULLs compare distinct in that index, so two
...
CREATE TABLE provider_accounts (
  ...
  billing_mode          TEXT NOT NULL
                        CHECK(billing_mode IN ('subscription', 'metered', 'unknown')),  -- how this account is charged; unknown is the honest-absence arm, never a synonym for metered; drives cost labeling, never cost derivation
  is_default            INTEGER NOT NULL DEFAULT 0
                        CHECK(is_default IN (0, 1)),  -- exactly one default per provider, enforced by the partial unique index below
  health_state          TEXT
                        CHECK(health_state IS NULL OR health_state IN ('authenticated', 'reauth_required', 'home_missing', 'indeterminate')),  -- the STORED outcome of the last validation of this account: the driver's authentication probe reading together with the credential-home observation taken at that same moment. NULL until a probe has ever been taken, which the wire renders as indeterminate — NOT as a failure and never as authenticated. This is the column the readiness projection reads; a registry read never re-derives it, so a read spawns no provider process and opens no credential file.
  ...
  observed_auth_mode    TEXT
                        CHECK(observed_auth_mode IS NULL OR observed_auth_mode IN ('oauth_subscription', 'oauth_token', 'api_key', 'external', 'none', 'unknown')),  -- the authentication mode the provider's OWN status surface reports for this home, OBSERVED and never assumed. NULL until observed; unknown is the distinct arm for "observed, but the provider named a mode this daemon does not recognize" — a tolerant arm so a vendor adding a mode does not fail an observation closed. oauth_token is D2 class and is what admits a token-mode account; the token VALUE is not here and is in no column of any table.
  last_refresh_observed_at TEXT,  -- RFC 3339 UTC of the most recent credential refresh the daemon has OBSERVED to have completed for this home, read from the provider's own durable marker where it publishes one. NULL = not observed, never "fine". Drives the freshness reading; the daemon never CAUSES a refresh to produce it.
  ...
  -- (provider_accounts row); they are never logged, never evented, and never carried on an error.
...
-- Exactly one default account per provider. A partial unique index rather than application-level
-- enforcement: two concurrent set-default calls racing on the same provider would both read "no
-- other default" and both write one, and the resulting ambiguity would be resolved silently at the
-- next spawn by whichever row sorted first — binding a run, and its spend, to an account the
-- operator did not choose. The database refuses the second writer instead.
CREATE UNIQUE INDEX provider_accounts_one_default_per_provider
  ON provider_accounts(provider)
  WHERE is_default = 1;

-- Exactly one account per credential home, across every provider. Two rows sharing a home share
-- its credentials: the daemon builds each spawn environment from this path, so a duplicate reduces
-- per-account isolation to a naming convention — one account's re-authentication rewrites the
-- other's credentials in place, and spend keyed to two identities is drawn from one. Deliberately
-- NOT scoped per provider: two providers pointed at one home is the same collision, and the path
-- is what the spawn environment carries either way. The database refuses the second writer
-- instead.
...
CREATE TABLE provider_account_usage_windows (
  ...
  limit_id      TEXT NOT NULL,  -- the provider's own limit identifier, carried verbatim as an untrusted provider-adjacent string. A reading that names no limit takes the reserved value 'default'so a provider publishing a single window needs no special case and the pre- single-window shape stays valid as the degenerate case. NOT enumerated by a CHECK: the provider's limit set is an open, versioned vocabulary and a closed CHECK would fail a reading closed the moment a vendor adds a window.
