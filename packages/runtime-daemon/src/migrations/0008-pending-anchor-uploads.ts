// Version-8 migration: the durable Merkle-anchor upload queue.
//
...
// the `0NNN-` placeholder for a not-yet-assigned migration (the `0005` stamp
// shows the concrete spelling once assigned), and this file is the assignment.
//
// ----------------------------------------------------------------------------
// Why a durable table and not an in-memory retry queue
// ----------------------------------------------------------------------------
//
// An anchor is a SIGNED commitment. Once `MerkleAnchorService` computes a Merkle
// root over a range and signs it with the daemon's Ed25519 key, that signature is
// the artifact an auditor will check years later — and Ed25519 is deterministic (RFC
// 8032 section 5.1.6), so re-signing the same root reproduces the same 64 bytes.
//
...
// which is precisely the hole step 1 exists to prevent before a compactor
// discards the underlying bytes — or (b) re-anchor blindly on a fresh
// cadence window with different bounds, producing overlapping anchors that
// an auditor cannot reconcile against the compaction ranges. Durability is
// what makes "anchored" a fact about the system rather than a fact about the
// current process.
//
// This is also why step 3 pins that landing a row HERE — not a successful
// control-plane upload — is what satisfies the anchor-before-compaction
// precondition. The control plane is a witness, not the system of record;
// requiring it to be reachable would make compaction fail during a partition,
// and a daemon that cannot compact during a partition eventually cannot
// append.
//
// ----------------------------------------------------------------------------
...
// (`IdempotencyClass: idempotent` on plan row) under re-entry — a retried
// compaction re-derives the same bounds, hits the key, and the service
// returns the already-queued row instead of enqueueing a duplicate.
//
...
// never an exact-start match. The [1,5000] anchor covers a compaction of
// [2000,3000]; an exact-match probe would miss it and force a redundant
// re-anchor.
//
// ----------------------------------------------------------------------------
...
// here too, as the durable LOCAL witness. Their `uploaded_at` therefore stays
// NULL by design — the upload worker filters them out rather than failing on
// them, so "NULL uploaded_at" alone is not an operator-actionable backlog
// signal for these rows.
//
...
-- coverage semantics in the constraint comment below). Node-scope (sentinel session_id) chains
-- queue their local Merkle anchors here too, as the durable LOCAL witness. In V1 those
-- sentinel-partitioned rows are NOT upload candidates -- the upload worker selects session-scoped
-- rows only (the sentinel cannot satisfy event_log_anchors' non-null session_id FK, and node-scope
-- control-plane witnessing is a V1.1 extension). Their uploaded_at stays NULL by design in V1.
CREATE TABLE pending_anchor_uploads (
  ...
  merkle_root         BLOB NOT NULL,         -- BLAKE3 Merkle root over row_hash leaves (RFC 9162 section 2.1.1 MTH: split at largest power of two, 0x00/0x01 domain separation)
  root_signature      BLOB NOT NULL,         -- Ed25519 by daemon_signing_keys.sealed_private_key over the anchor CLAIM ({endSequence, merkleRoot, nodeId, sessionId, startSequence}, RFC 8785)
  ...
  -- Durable retry/backoff state (partition-anchor queue durability decision, resolved): survives
  -- daemon restart so upload retry resumes and the last failure is queryable for operator triage
  -- post-restart.
  ...
  -- end_sequence is part of the key: a cadence anchor [1,1000] and a wider compaction-covering
  -- anchor [1,5000] share start_sequence=1 and MUST coexist. "Covering anchor exists" is a COVERAGE
  -- query (start_sequence <= range_start AND end_sequence >= range_end), NOT an exact-start match;
  -- the key only dedups genuine re-fires of the identical range.
