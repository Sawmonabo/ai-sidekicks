// Version-14 migration: the console-parity capability flags.
//
...
//
// The canonical schema source-of-truth is whose widening note already declares
// the seventeen-value CHECK and names this as the third and last wave of rows.
// This file applies that note; it does not decide it.
//
// ----------------------------------------------------------------------------
...
// is a fact about OWNERSHIP rather than an omission: version 13 is the
...
// no other.
//
// The declarations the two shipped drivers carry at this task are NOT all
...
// most cells until the first capability refresh runs. Writing the declaration
// here would make the cache assert a capability no build has been asked about,
// which is exactly the unprobed `true` exists to forbid.
//
// ----------------------------------------------------------------------------
...
-- altered in place, and version 11 froze this one at fourteen literals that
-- admit none of the three flags added here. The successor shape is the canonical
-- block widened CHECK included.
-- ---------------------------------------------------------------------------
...
-- Backfill (undeclared capability = unsupported). Exactly the THREE flags this task
-- adds, for every driver_name already cached. Runs AFTER the rename: the three
-- literals would be rejected by the superseded fourteen-value CHECK. refreshed_at is
-- copied from the driver's own newest row, never the migration's wall clock, so a
-- cache the driver never answered cannot read as freshly refreshed. DO NOTHING rather
-- than INSERT OR IGNORE, so a mistyped literal fails loud on the CHECK instead of
-- silently short-counting the row set; the WHERE true disambiguates the UPSERT's ON
-- from a join's ON per sqlite.org/lang_upsert.html.
