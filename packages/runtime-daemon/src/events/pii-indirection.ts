...
// of order is a TypeScript error rather than a runtime integrity bug:
//
//   `RawEventInput`
...
//
// THIS MODULE PRODUCES `pii_payload`; IT DOES NOT PERSIST IT. The brand is what
// makes that split enforceable: a persistence helper typing its column parameter
// `PiiPayloadCiphertext | null` can be handed a non-null value only by code that
// ran this pipeline, so "sole write path" becomes a compile error for everyone
// else instead of a convention. the shipped `SessionService.append` hardcodes
// `pii_payload = NULL` (no ciphertext parameter exists yet); the
...
//
// IDEMPOTENCY CLASS: `manual_reconcile_only`, and deliberately NOT `idempotent`. The
// PII cipher is AES-256-GCM under a fresh random 96-bit nonce per write, which NIST SP
// 800-38D section 8.2 requires precisely because nonce reuse under one key is
// catastrophic — so a re-run produces different ciphertext, a different digest,
// different canonical bytes, and a different `row_hash` / `daemon_signature`.
// Re-running this codec after a partial failure therefore mints a SECOND, unrelated
// commitment rather than reproducing the first; recovery is operator-driven
// reconciliation. The contrast with `signer.ts` is exact and worth holding onto:
// Ed25519 derives its nonce from the key and the message (RFC 8032 section 5.1.6), so
// IS `idempotent` and its re-runs reproduce bytes. For that reason the refusals this
// module can raise FROM THE INPUT ALONE are ordered before the encrypt step, so a
// rejected append costs no nonce — three of them hoisted out of the downstream stages
// that still run them: the sequence guard, the `prevHash` guard, and the signing-key
// shape check `ed25519.sign` performs at the bottom of its own signature call. That
// third one fronts a LIBRARY guard rather than a sibling module's, which makes it the
// one hoist that is NARROWER than what it stands in for instead of identical to it;
// the guard itself says why that is still only ever an early refusal. TWO CLASSES STAY
// BEHIND THE ENCRYPT and `writeEventWithPii` names both: the shape of the encryptor's
// own RESULT, which does not exist until it has run, and `payload`'s own RFC 8785
// refusals. Those last are not one story — the nesting ceiling admits no honest
// pre-check short of a second canonicalization of the same row, while the RFC 8785
// `writeEventWithPii`'s own note draws that line rather than letting the harder case
// speak for both.
//
// `PiiEncryptor` is an INTERFACE owned here ships the AES-256-GCM
// implementation in `crypto/pii-codec.ts` and the composition root injects it.
// cannot import — it sits a tier below — so nothing in this file reaches for
// that module, and the same holds for the `splitPii` partition owns:
// `RawEventInput` is SHAPED to receive its `{payload, piiPayload}` output, and
// this module never calls it. also obliges the composition root to inject the
// real codec — never a test stub — with a runtime assertion against the stub
// outside tests. Neither the stub nor that assertion lives in this file: owns a
// test-only stub and the daemon bootstrap owns the assertion.
//
...
// `ipc/domain-error.ts`'s `DaemonDomainError` under an `event.*` code
// registered.
//
...
// land inside the SIGNED canonical bytes, and every independent verifier reads
// them by that spelling — a fold to the surrounding camelCase would not tidy
// anything, it would break verification on every existing PII row), and why the
// digest's ENCODING is deliberately unpinned. What stays here is the reasoning
// that is about this codec:
//
 ...
 *
 * A persistence helper typing its column parameter `PiiPayloadCiphertext | null`
 * can then only receive a non-null value from code that ran encrypt → digest →
 * embed → canonicalize → sign in order, because nothing else can produce the
 * brand. This is also why
 * {@link PiiEncryptor} returns a BARE `Uint8Array`: were the interface to return
 * the brand, its implementation would have to mint one to satisfy the signature,
 * and any caller could then invoke the codec directly and hand the result to the
 * INSERT — bypassing the digest, the embed, and the signature in a single
 * well-typed call, defeating together.
 *
 * Brand shape mirrors the contracts-package convention (`SessionId`,
 * `EventEnvelopeVersion`), the `CanonicalBytes`, and the `Ed25519PrivateKey`. No
 * constructor, mint helper, or brand symbol is exported.
 */
 ...
 *
 * The third link chain, and the one that leaves the module: the persisting
 * caller is handed THIS type rather than a bare `EventEnvelope`, so "the row I
 * am about to write is the row that was signed" is carried in the type instead
 * of asserted in a comment. Producible only by the embed stage below, which
 * itself requires a {@link PiiPayloadCiphertext} and so cannot run before the
 * encrypt stage.
 *
...
// --------------------------------------------------------------------------
//
// The second partition this module seals, and the reason it is HERE rather than in a module of
// its own: this is where the Encrypt-Then-Digest-Then-Sign order lives, and a second sealing
// module would be a second place for that order to drift. states the rule as a sole-write-path
// obligation — callers MUST NOT produce `content_payload` bytes anywhere else.
//
// A row may carry both, one, or neither — and a row carrying both runs the order
// ONCE over both partitions, so the one-canonicalization -per-row rule is not
// weakened by a second ciphertext.
 ...
 * unregistered census type still travels the accept-and-stub envelope path, and
 * that tolerance is deliberately preserved for what it is about: a reader "MUST
 * persist an envelope whose `type` it cannot interpret as a version stub —
 * never drop or reject it". It is about TYPES, never about admitting reserved
 * codec-owned MEMBERS. The discriminator is mechanical:
 ...
 * registered narrowly — it refuses "a write whose `payload` carries a PII-tagged
 * field with no `pii_ciphertext_digest`" — and its
 * `DaemonPiiSplitBypassDetailsSchema` is `.strict()` on `fieldPath` alone.
 * Reusing that code for a content-key refusal would make a registered contract
 * describe something it does not describe. This branch mints no wire error code,
 * so the refusal lands as an internal typed error — the same disposition {@link
 * assertRegisteredVariantParses} already takes on this same append path for the
 * same class of caller-supplied defect.
 *
 ...
 * follows it — so no length prefix is needed to keep two distinct pairs from
 * colliding.
 */
 ...
 * either fail the RFC 8785 section 3.2.2.2 well-formed-strings guard downstream or
...
export interface PiiEncryptionRequest {
   ...
   * It is deliberately the same serializer the canonical bytes use (the
 ...
 * use. The read path (and the Phase-4 task that implements it) declares its own
 * counterpart against the plaintext convention documented on {@link
 * PiiEncryptionRequest.plaintext}.
 */
 ...
 *
 * `payload` and `piiPayload` are the two halves of the `splitPii` partition,
 * which the EMITTER runs at the call site (a pure classification per its own
 * invariant).
 *
 * `piiPayload` is REQUIRED, not optional: this codec is the PII write path, and
 * an event with no PII has no business here. the plain append path handles
 ...
 * substitution, by a quieter route. buys its way out of the analogous hazard
 * with a defensive copy of `prevHash`, but that is 32 fixed bytes; a payload
 * tree of unbounded depth is not the same purchase, and deep-copying would
 * charge every append for it while only moving the alias to the caller's side.
 *
...
export interface PiiEventWriteResult {
   ...
   * the four column values it writes would reopen exactly the reach-back that
   * rule closes. `signer.ts` makes the same call for `SignedRow.prevHash`,
...
/**
 * The RFC 8032 section 5.1.5 secret-seed width, re-spelled here for the pre-encrypt
 ...
 * its `Ed25519PrivateKey` note defers the width to `ed25519.sign` on purpose — and
 * the `signing-key-source.ts` keeps its own `ED25519_KEY_LENGTH` module-private, so
 * reaching either would mean widening a sibling's export surface for one integer.
 *
 ...
 * canonicalized once and signed once, with both digests embedded in the same
 * step — which is why the stages below name each partition rather than splitting
 * into two numbered lists.
 *
 ...
 *      INSIDE the canonical form (excludes both columns themselves and requires
 *      the digest in their place). The content partition embeds two further
 *      members the seal step determines — `contentLength`, and
 *      `contentTruncated` only when the bound fired.
 *   5. CANONICALIZE — over the digest-bearing envelope.
 *   6. SIGN — over those canonical bytes: `row_hash` and
 *      `daemon_signature` in one call, one canonicalization per
 *      row.
 *
 * `prevHash` is the link — the immediately-prior row's `row_hash` for this
 * `session_id`, or `GENESIS_PREV_HASH` at `sequence = 0` — and it is a PARAMETER
 * because reading it is only correct under the per-session append lock owns.
 * This module holds no lock and touches no database, so resolving the link here
 * would race every concurrent append. `signRow` refuses a `prevHash` that is not
 * 32 bytes and remains the authority on that width; this function refuses the
 * same value EARLIER, for the reason below.
 *
 ...
 *      could reach the shipped code: before a second partition existed,
 *      "neither" was unreachable and the half-present shape threw much later and
 *      about the wrong thing, inside `canonicalizeJson(undefined)`. Each arm's
 *      own note carries why it refuses rather than degrades.
 *
 ...
 *   3. A `sequence` that is not a safe integer. HOISTED from the
 ...
 *   4. A non-canonical `occurredAt` (the `normalizeOccurredAt`), then the PII
 *      partition's own serialization refusals (the `canonicalizeJson` over
 *      `piiPayload`: its nesting ceiling, its RFC 8785 section 3.2.2.2 Unicode
 ...
 *   5. A `prevHash` that is not 32 bytes. HOISTED from the `signRow`, and
 ...
 *   - `input.payload`'s OWN RFC 8785 REFUSALS — the nesting ceiling, its RFC
 *     8785 section 3.2.2.2 Unicode well-formedness guard, and
 *     `canonicalize@3.0.0`'s `NaN` / `Infinity` / circular-reference throws —
 *     fire inside `canonicalizeEvent`, at recipe step 5. A pre-check that
 *     disagrees with the guard it stands in for would make the ordering claim
 *     above FALSE for the boundary case, which is worse than refusing late and
 *     saying so. THAT DEPTH-OFFSET ARGUMENT DOES NOT CARRY THE WELL-FORMEDNESS
 *     GUARD, and it is not stretched to: well-formedness is a property of each
 *     string alone, so a standalone walk over `input.payload` would agree with
 *     the real guard exactly, at every input. It stays late for the weaker
 *     reason only — exports no well-formedness checker either, and hoisting one
 *     would reorder a refusal sequence this block fixes deliberately — which is
 *     a residual worth naming rather than a forced placement.
 *     `canonicalizeJson`'s no-JSON-representation guard is NOT in this class: it
 *     fires only for a TOP-LEVEL value with no JSON representation, and
 *     `canonicalizeEvent` always hands it an object, so it is reachable only
 *     through the `piiPayload` call at 4 — above the encrypt.
...
export async function writeEventWithPii(
  ...
  // LAYER 2 — and the narrowing the rest of this function stands on.
  //
  ...
  // is the SOLE WRITE PATH for `content_payload`, so a check that lived above it
  // would be bypassed by the next caller to reach the codec directly — and the
  // append path already routes `options.content` straight through. One guard on
  // the write path is worth more than a guard on each caller, because the second
  // kind is only ever as complete as the last review.
  //
  ...
  if (input.content !== undefined && !Object.hasOwn(BODY_BEARING_EVENT_TYPES, input.type)) {
    throw new Error(
      `writeEventWithPii refuses a content partition on event type ${JSON.stringify(input.type)}: machine-authored prose is sealed only for ${Object.keys(BODY_BEARING_EVENT_TYPES).join(", ")}, the types whose registered payload declares the codec's content members. Any other type would be signed carrying members its own schema rejects. Append this event through the plain append path instead.`,
    );
  }
  ...
  if (Object.hasOwn(input.payload, PII_CIPHERTEXT_DIGEST_PAYLOAD_KEY)) {
    throw new Error(
      `writeEventWithPii refuses an event whose payload already carries ${PII_CIPHERTEXT_DIGEST_PAYLOAD_KEY}: this codec is the only producer of that member, and it is not re-runnable — AES-256-GCM uses a fresh random nonce per write, so a second pass would mint an unrelated ciphertext and digest.`,
    );
  }
  ...
  // REFUSAL 3 of the order documented above — the `assertRepresentableSequence`,
  // hoisted ahead of the encrypt. `sequence` travels as an IEEE-754 double and
  // holds integers faithfully only to 2^53 − 1; past that two genuinely
  // different events canonicalize to identical bytes and collide inside the very
  // chain builds to make tampering detectable. That verdict is answerable from
  // `input.sequence` and nothing else, and the predicate is
  // `Number.isSafeInteger` — precisely the, and total — so the early copy cannot
  // disagree with the late one on any input. the guard still runs inside
  // `canonicalizeEvent` at recipe step 5 and stays the authority.
  //
  ...
  // a fixed point, so the second pass returns the identical string.
  ...
  // REFUSAL 5 of the order documented above — the `prevHash` guard, hoisted to
  // just ahead of the encrypt. A wrong-width link hashes happily and mints a
  // signature over a chain input no verifier can ever reproduce: an untampered
  // row that fails forever.
  //
  // Supplies this argument by reading the previous row's `row_hash` back out of
  // SQLite, and a `BLOB` column can hand back a JS `string` — which is why the
  // test is BYTE-NESS as well as width, the same conjunction `signRow`
  // documents. A 32-CHARACTER string clears a bare length check and then coerces
  // to near-zero bytes through `TypedArray.prototype.set`.
  //
  ...
  if (!(prevHash instanceof Uint8Array) || prevHash.length !== CHAIN_HASH_LENGTH) {
    throw new Error(
      `writeEventWithPii requires a ${CHAIN_HASH_LENGTH}-byte Uint8Array prev_hash — the previous row_hash for this session, or GENESIS_PREV_HASH at sequence 0 — received ${describeByteShape(prevHash)}. Refused before the encrypt step so a rejected append costs no AES-256-GCM nonce; signRow re-checks the same value when the row is signed.`,
    );
  }
  ...
  // `Ed25519PrivateKey` has exactly one mint site in the workspace — the
  ...
  // `verifyEd25519` names for the public half ("the unvalidated `as
  // Ed25519PublicKey` cast").
  //
  ...
  if (
    ...
    throw new Error(
      `writeEventWithPii requires a ${ED25519_PRIVATE_KEY_LENGTH}-byte Uint8Array daemon signing key per RFC 8032 section 5.1.5; received ${describeByteShape(daemonSigningKey)}. That is a key-custody or key-routing bug — the signing-key-source.ts is the only site that may mint this type. Refused before the encrypt step so a rejected append costs no AES-256-GCM nonce; ed25519.sign re-checks the same value when the row is signed.`,
    );
  }
  ...
  if (contentInput !== undefined) {
    ...
    //
    // The canonical payload path already treats ill-formed strings this way: the
    // `canonicalizeJson` runs the RFC 8785 section 3.2.2.2 well-formedness guard and
    // refuses rather than substituting. This arm holds the CONTENT partition to the
    // same rule, which is what keeps one row's two text partitions answering the same
    // way about the same defect.
    //
  ...
  if (piiCarrying !== undefined && piiPlaintext !== undefined) {
    ...
    // The declared `Uint8Array` is a claim about a value that crossed injection
    // boundary — an implementation this module does not own, does not import, and
    // may briefly be a stub. Empty is refused too — no AEAD emits a zero-length
    // output, so it means the implementation returned nothing while claiming
    // success. Width is NOT checked: this interface does not fix an AEAD, so its
    // ciphertext has no width this module is entitled to assert.
    if (!(encryptorResult instanceof Uint8Array) || encryptorResult.length === 0) {
      throw new Error(
        `PiiEncryptor.encrypt must return non-empty Uint8Array ciphertext for the session_events.pii_payload column received ${describeByteShape(encryptorResult)}. That is an injection bug boundary, not a tampered row.`,
      );
    }

    // The ONE site where `PiiPayloadCiphertext` enters the type system — and it
    // brands an OWNED COPY, not the encryptor's own array.
    //
    // `PiiEncryptor` is an injection boundary: it fixes a return TYPE and says
    // nothing about the LIFETIME of the buffer behind it, so a conforming
    // implementation may hand back a reusable scratch array and overwrite it on
    // its next call. Nothing here can forbid that — owns the implementation, sits
    // a tier above, and never consults this file. Copying costs one allocation of
    // the ciphertext's width per PII append; no width is named here because the
    // interface fixes no AEAD.
    //
    ...
    // first would trade one refusal that names boundary and routes to the
    // encryptor's author for one of those, so the guard runs first and this line
    // only ever copies bytes it has already been told are bytes.
    //
  ...
  // ONE embed over BOTH partitions, which is what keeps the
  ...
  // is what keeps from resolving the column from a second source that could
  // disagree with the signed one. Safe to echo by reference where the ciphertext
  // was not — a `string` is immutable, so there is no aliasing hazard to close.
  return {
 ...
 * so this function is unreachable before it. Kept as one function because the
 * digest has no legitimate existence apart from the embed — a digest computed
 * and then NOT embedded is precisely the failure the ordering invariant exists
 * to prevent.
 *
 * The digest covers the ciphertext EXACTLY as the column will hold it.
 *
...
function embedCiphertextDigest(
  ...
  //
  // That invariant — and leg 1's title, "carries `pii_ciphertext_digest` and
  ...
  // Envelope members projected one at a time, never spread from `input`: `input`
  ...
  // twelfth envelope member added in contracts breaks this literal instead of
  ...
  // `EventEnvelope` rather than a double cast — the register.
 ...
 * binding, the same shape as the `occurred_at` hole the `isCanonicalOccurredAt`
 * closes. Keeping mint and check in one place is the structural half of the fix:
 * a future editor cannot touch one without the other in view.
 *
 * NOT part of `verifyRow`. Emitting the verdict is `Security Architecture `
 * rule 2's job, as a POSTcondition rather than a precondition: the digest lives
 * inside the signed payload, so it is only worth trusting once the Ed25519
 ...
 *
 * The fourth row is the one worth naming, because it is not what a reviewer asks
 * for when they ask for a digest comparison: a signed digest with no ciphertext
 * under it means the column was emptied AFTER signing, and the signature still
 * verifies green over the digest that outlived its subject. That is evidence
 * destruction, and comparing hashes alone would never see it. A legitimately
 * compacted row does not land here — NULLs `pii_payload` and replaces `payload`
 * with a stub projection that carries no digest, so it lands in row two.
 *
 * ONE BOUNDARY ON THAT READING IS PINNED RATHER THAN SILENT: it holds for rows
 * this daemon ORIGINATED. A row that arrived from another machine would land in
 * row four BY DESIGN, because `pii_payload` never crosses the machine boundary
 * while the signed digest inside the canonical payload does — so nothing was
 * destroyed anywhere and row four would be reporting a transport property as
 * evidence destruction. No such receive path is wired: every row this predicate
...
/**
 * The COLUMN-AND-KEY-PARAMETERIZED core both ciphertext-digest checks
 * run.
 *
 ...
 *
 * TOTAL AND NEVER-THROWING, on ground its callers document: a verifier consumes
 * it inside a range walk, where one throw would abort the walk and silence audit
...
/**
 * (the sixteenth verification mode `content_ciphertext_digest_unbound`.)
 *
 ...
 *
 * DELIBERATELY UNWIRED, on precedent its two PII siblings follow: this module
 * exports the pure predicate and the verifier task emits the verdict. Emitting
 * one from here would be Phase-3 code deciding a Phase-4 question, and
 ...
 *
 * TOTAL AND NEVER-THROWING, on ground the whole family shares: a verifier
 * consumes this inside a range walk, where one throw would abort the walk and
 * silence audit of the entire remaining tail. All three parameters are
 ...
 * shape `isPiiOwnerStampBound` documents for the stamp column adds.
 *
 * STILL UNWIRED TO A VERDICT. Like its siblings this reports a boolean and names
 * no `failureMode`; `integrity-verifier.ts` is the file and does not exist.
 ...
 * walk and silence audit of the entire remaining tail. Both parameters are
 * `unknown` because both will arrive from SQLite. Sited adjacent to the mint, so
 * mint and check cannot drift apart.
 *
 * The four states:
 *
 ...
 * and every payload schema is `.strict()`, so a verbatim parse would have
 * refused 100% of the PII route. The registration removed the reason rather than
 * the symptom: there is now nothing to excise, and the guard judges the signed
 * row exactly as it will be read back.
 */
export function assertRegisteredVariantParses(
  ...
  throw new Error(
    `${seam.name} refuses to sign an event of type ${JSON.stringify(envelope.type)} that its own registered SessionEventSchema variant rejects: ${describeStrictLayerIssues(parsed.error.issues)}. Signing it would chain a row that fails the strict layer on the way back out — permanently unreadable as anything but a stub. ${seam.timing}`,
  );
}
 ...
 *
 * The `canonicalizeEvent` accepts any `EventEnvelope`, correctly: it serves the
 * whole daemon, and most events carry no PII. This one-line wrapper is what
 * closes the {@link EventWithPiiDigest} → `CanonicalBytes` link of on the PII
 * path without narrowing that shared entry point for everyone else, and
 ...
 * It is not a module-wide prohibition, since the wider entry point must be
 ...
 * {@link describeByteShape} answers in that same shape for a genuinely different
 * reason: a refused signing key IS secret material.
 *
