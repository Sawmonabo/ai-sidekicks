...
//
// EXACTLY TWO OF THE THREE BIND — its intake needs a registered rejection reason for
// each, an over-deep `action_payload` and an ill-formed one — and those two differ in
// KIND: the ceiling is POLICY, renegotiated here rather than forked around, while the
// well-formedness refusal is an RFC 8785 section 3.2.2.2 normative MUST, not
// renegotiable in either direction. registers those two, and scopes the renegotiation
// affordance to the ceiling alone. The `toJSON` refusal deliberately adds NO third
// row, because no body can reach it: that intake arrives via `JSON.parse`, whose
// output has no function-valued member anywhere — `{"toJSON":"x"}` yields a STRING,
...
// `NaN is not allowed`, `Infinity is not allowed`, and `Circular reference
// detected`. All three are narrow rather than holes — the untrusted path arrives
// via `JSON.parse`, which produces no `NaN`, no `Infinity`, and no cycle — but an
// in-process caller reaches the first two with a plain `canonicalizeJson({
// sequence: NaN })`, so the inventory names them rather than claiming this module
// owns every throw. The THIRD is now SHADOWED in both shapes that could reach it:
// an own-property cycle drives the depth walk past the ceiling and gets this
// module's depth wording, and a cycle reachable only through a `toJSON` result is
// refused by `assertNoToJsonOverride` before the serializer ever runs. What is
// left of it is the non-idempotent-accessor residual `assertWithinCanonicalDepth`
// documents — a getter handing the guards an acyclic tree and the serializer a
// cyclic one — so it stays inventoried as that residual rather than as a live
// path. The `NaN` example is entry-point-specific: the SAME defect routed through
// `canonicalizeEvent` meets `assertRepresentableSequence` first and gets this
// module's wording instead, since a non-integer fails the safe-integer predicate
// too. Nothing discriminates a canonicalization failure from any other throw yet,
// and a class minted before a `catch` branches on it is speculative. The
// migration target when a consumer first needs to discriminate is
// `DaemonDomainError`, which already owns the JSON-RPC wire projection, under an
// `event.*` code registered — a NEW row there, since that file's namespace today
// holds only cursor code.
//
...
export function normalizeOccurredAt(occurredAt: string): string {
  ...
  // `-00:00` IS ACCEPTED, DELIBERATELY. RFC 3339 section 4.3 distinguishes it from
  // `+00:00`/`Z` — but only to annotate that the producer's LOCAL offset is unknown,
  // on the section's own premise that "the time in UTC is known". The instant is
  // identical `-0`, and `x - (-0) === x`), and RFC 9557 section 2.2 — Standards Track,
  // `Updates: 3339` — has since reassigned that annotation to `Z` itself. So the fold
  // below discards no time, only provenance the canonical form has no member to carry.
  // This is the module's stated policy at the header applied as written: normalize
  // where the rewrite BOTH preserves the instant AND lands in the canonical form.
  // Refusing here would instead hard-reject conformant input from a future non-JS
  // producer.
  //
 ...
 * guard of its own, and routes UNTRUSTED dispatch request bodies through this
 * entry point. PARSEABLE nesting costs TWO bytes per level — every `[` needs
 * its `]` — so a ~10 KB body buys depth ~5,000, far enough to turn the
 * serializer into `RangeError: Maximum call stack size exceeded`. A body-size
 * cap set anywhere near a realistic request still admits depths well past the
 * overflow point below, so the bound has to be on depth itself.
 *
 * Why 64: measured on Node 22.12 with the default stack, `canonicalize` survives
 * to depth ~1500 (nested arrays) / ~3500 (nested objects) before overflowing.
 * That was measured from a shallow test caller, so a daemon handler already deep
 * in an async chain has strictly less stack left — which is exactly why the
 * margin here is ~23x rather than ~2x. Against that headroom, no real caller
 * comes close. Arbitrary nesting reaches this module only through the
 * OPEN-RECORD fields the contracts declare, and every one of them sits shallow:
 ...
 * untrusted path — `JSON.parse` output carries no `toJSON` and no accessors.
 *
...
//   3. THE RFC 8785 section 3.2.2.2 REFUSAL IS EVADED: `{ toJSON: () => ({ a: "\ud800" }) }`
 ...
 * {@link assertNoLoneSurrogate} makes and for the same reason: the PII codec
...
// --------------------------------------------------------------------------
// Unicode well-formedness — the refusal RFC 8785 section 3.2.2.2 MANDATES.
// --------------------------------------------------------------------------
//
// RFC 8785 section 3.2.2.2 closes with a normative Note, quoted verbatim: "Since
...
// error." It is the same normative register as which this module already honors
// through `canonicalize@3.0.0`'s own two throws — so the two Notes are treated
// alike rather than one being read as advisory.
//
...
// arrives via `JSON.parse`, which produces no `NaN`, no `Infinity`, and no
...
// case — `canonicalizeEvent` does not parse, and the bodies never meet
// `EventEnvelopeSchema` at all.
//
 ...
 * UNITS, which is the level RFC 8785 section 3.2.2.2 legislates at and the level a lone
 ...
 *
 * THE OFFENDING TEXT IS NEVER INTERPOLATED. the PII codec calls
 * `canonicalizeJson(input.piiPayload)` directly, so this guard runs over PII
 * PLAINTEXT and its message reaches logs. Four throw sites are reachable through
 * that call — the depth ceiling, the `toJSON` refusal, the
 ...
 * name, at any depth.
 *
 * BOTH POSITIONS, because RFC 8785 section 3.2.2.2 legislates over both in one breath
 * — "For JSON string data (which includes JSON object property names as well)".
 * Property names are not an edge case here: they are lex-sorted into the byte order
 * so an ill-formed name is load-bearing twice over.
 *
 ...
 *
 * DIVERGES FROM THE SERIALIZED TREE IN ONE DIRECTION ONLY — THE CONSERVATIVE ONE
 * — which is what lets its verdict stand for the bytes. (A third shape is gone
 * with the bypass — a `toJSON` returning a SUBSET of the own-property tree left
 * the surplus checked but never serialized.) Neither remaining shape is
 * reachable on the untrusted path, where `JSON.parse` output carries no
 * `toJSON`, no accessors, no `undefined`, and no symbols.
 */
 ...
 *
 * The generic entry point, and the one names: the `request_body_hash`
 * digests an RFC-8785-canonicalized request body, and MUST consume this
 * function rather than re-implement the scheme.
...
export function canonicalizeJson(value: unknown): CanonicalBytes {
  ...
  if (typeof canonicalText !== "string") {
    ...
    // why this branch is a no-output guard, not a general error funnel. routes
    // arbitrary request bodies through this same entry point, so the guard
    // closes a live hole rather than a hypothetical one.
    throw new Error(
      "RFC 8785 canonicalization produced no output: the value has no JSON representation (undefined, a function, or a symbol).",
    );
  }
  // RFC 8785 section 3.2.1 specifies UTF-8 as the canonical output encoding. The
 ...
 * vector set binds this module to — includes unsafe integers and `1e30` as
 * inputs with mandated outputs, so a generic unsafe-number refusal would fail
 * the specification the module implements. Second, the two entry points carry
 * different contracts, and this one is event-specific: `sequence` is declared
 * an INTEGER, so a value that is not a faithful integer violates its own
 * contract, whereas an arbitrary `payload` number is declared a JSON NUMBER —
 * a double — and collapses relative to nothing. `canonicalizeJson` stays the
 * generic serializer hands untrusted bodies; event semantics live here.
 *
 ...
 * `null`, as requires, and `actor` is the canonical set's UNIQUE member that is
...
export function canonicalizeEvent(envelope: EventEnvelope): CanonicalBytes {
  ...
  // clean under this mapped type and under any other. That residue is the
  // golden vectors' to close, by construction rather than by annotation.
  //
  // Declaration order is not load-bearing — RFC 8785 section 3.2.3 mandates a UTF-16
  ...
  // and JSON has no `undefined`: an optional member holding `undefined` IS an
  // absent key on the wire — that is exactly what the wire schema's
  // `.optional()` yields for a key the producer never sent — so it is dropped
  // here, while `actor: null` (the canonical set's only nullable member) is
  // kept and serializes as `null`. Dropping the key in this module rather than
  // leaning on the serializer's handling of `undefined` keeps the
  // absent-vs-null distinction a property of the code that owns it. Both
  // no-value `actor` states — absent and `null` — reach this loop and emit
  // DIFFERENT bytes, which is why the precondition above puts the
  // absent-vs-null choice on the caller: storage cannot tell the two apart on
  // the way back, so this module must be handed the shape the row will hold.
