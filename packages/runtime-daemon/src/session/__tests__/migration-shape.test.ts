...
//   * Spec-005:43 — driver-contract operations persist provider session
//     handles (runtime_bindings is the persistence surface).
//   * Spec-005:47 — provider-owned resume handles persisted separately from
...
describe("0003-runtime-bindings migration shape", () => {
  ...
  it("pins the column shape, single-column PK, and resume_handle nullability of `runtime_bindings`", () => {
    ...
    // Spec-005:47 — resume_handle (provider-owned handle) is persisted
  ...
  it("enforces the capability_flag CHECK on `driver_capabilities`", () => {
    ...
    // canonical excluded flag per Spec-005:46 + ADR-011.
    expect(() => {
      insertFlag("pause");
    }).toThrow(/CHECK constraint failed/i);
  });
