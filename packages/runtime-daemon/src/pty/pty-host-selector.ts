...
//
// Env-var override grammar (F-024-2-07 / `Plan-024 §API And Transport Changes`)
// ----------------------------------------------------
//
 ...
 * back to the platform default with a `console.warn` per `Plan-024 §API And Transport Changes`.
 */
...
export function selectPtyHost(deps?: Partial<PtyHostSelectorDeps>): PtyHost {
  ...
  // `Plan-024 §API And Transport Changes` we emit a warn AND fall back to the platform default
