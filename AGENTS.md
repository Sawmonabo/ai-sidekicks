...
- **Code under `packages/` + `apps/`** — `` `path/to/file.ts#exportedSymbol` `` (path, `#`, an exported identifier or test name present in the file). New raw `` `path.ts:NNN` `` line-pins into these trees are denied by the docs-corpus gate.
...
A durable cite may carry a free advisory locator — e.g. `` `packages/x/foo.ts#parseFrontmatter` (near the retry loop) `` — no gate reads the parenthetical.
