...
**Boundary rule (lane 1 vs lane 2).** A task already recorded in the Shipment Manifest is shipped; later fixes or improvements to that code are lane 2 (or lane 4 when they alter the contract) — never a second lane-1 pass over the same entry. Lane 1 is reserved for work that adds or completes manifest entries. The title-token side of this boundary is CI-enforced: the docs-corpus workflow's `lane-boundary` job (`tools/docs-corpus/bin/lane-boundary-check.ts`) fails any PR whose title cites `Plan-NNN` (case-insensitive) while its diff touches material paths (`packages/`, `apps/`, `.github/`) without also touching that plan's `docs/plans/NNN-*.md`.
