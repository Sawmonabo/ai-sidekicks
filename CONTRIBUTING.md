...
`pnpm install` provisions the [lefthook](https://github.com/evilmartians/lefthook) chain (`lefthook install` runs via `prepare` script). `lint-staged` runs first, because it rewrites and re-stages fixable files; the three screens — `gitleaks` + `docs-anchor-check` + `docs-corpus-checks` — then run in parallel with each other, after that rewrite rather than beside it, so no screen observes a half-rewritten substrate. `gitleaks --staged` and `docs-corpus-checks` read the index lint-staged produced — the blob that commits; `docs-anchor-check` (lychee) reads the worktree lint-staged left behind, so it can still diverge from the index when unstaged edits are present — CI's full-repo docs-corpus sweep is the authoritative gate there. `commitlint` runs on the message locally; no CI workflow re-runs it — the squash-merge subject is set explicitly at merge time. CI re-runs the pre-commit checks as the authoritative gate per [ADR-023 §Axis 2](docs/decisions/023-v1-ci-cd-and-release-automation.md#axis-2--pre-commit-hook-framework).
