...
`pnpm install` provisions the [lefthook](https://github.com/evilmartians/lefthook) chain (`lefthook install` runs via `prepare` script). `lint-staged` runs first, because it rewrites and re-stages fixable files; the three screens — `gitleaks` + `docs-anchor-check` + `docs-corpus-checks` — then run in parallel with each other, after that rewrite rather than beside it, so no screen observes a half-rewritten substrate. All three screen the index lint-staged produced — the blob set that commits: `gitleaks --staged` and `docs-corpus-checks` read it directly, and `docs-anchor-check` (lychee) runs against a `git checkout-index` materialization of it, so a partially staged file is checked as the version that commits rather than as the worktree copy carrying unstaged edits. CI's full-repo docs-corpus sweep remains the authoritative gate. `commitlint` runs on the message locally; no CI workflow re-runs it — the squash-merge subject is set explicitly at merge time. CI re-runs the pre-commit checks as the authoritative gate per [ADR-023 §Axis 2](docs/decisions/023-v1-ci-cd-and-release-automation.md#axis-2--pre-commit-hook-framework).
...
- **New raw line-number cites into `packages/`/`apps/`, or into specs/plans/ADRs from code comments** — use the durable forms per [AGENTS.md §Durable-Cite Rule](AGENTS.md#durable-cite-rule-repo-internal-citations) (`path#symbol` for code targets, backticked `Spec-NNN §Heading` for governance targets); the docs-corpus gate denies new raw line-pins into volatile trees.
