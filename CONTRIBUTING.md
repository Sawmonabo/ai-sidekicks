...
`develop` is the integration branch; `main` is the release branch and the only protected one. Cut every branch from `develop` as `<type>/<topic>` in kebab-case, where `type` is one of `feat`, `fix`, `refactor`, `chore`, `docs`, `test`, `ci`. Squash-merge back into `develop`; `develop` merges into `main` only at a release.
...
Open the PR with the whole description in the first `gh pr create` call. CI runs (`ci-gate`, `docs-corpus-gate`) and the Codex bot reviews; on `develop` both are information for the reviewer, not gates. Squash-merge when you decide; the squash subject becomes permanent history, so keep it commitlint-valid including the ` (#NNN)` suffix. Delete the branch afterwards.
...
`git worktree add .worktrees/<name> -b <branch> develop`. Run `pnpm install` inside a fresh worktree. Never remove a worktree another session is using (see `CLAUDE.md` rule 4). Never `git stash` while more than one worktree is live; `refs/stash` is shared.
