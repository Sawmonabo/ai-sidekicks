// The console's single-reading chokepoints, as `no-restricted-syntax` SELECTORS.
//
// A SIBLING OF THE FLAT CONFIG, not a second config: `eslint.config.mjs` composes the
// selectors below into its own blocks, so there is still exactly one place a reviewer
// reads to know what the console may not write. They live here because the bans and
// their derivations had grown past the point where the import boundary above them was
// still findable in the same file — the package's own rule is that a file holds one
// concept and splits on the seam between two, and the import boundary and the syntax
// bans are two.
//
// SELECTORS AND NOT A CONFIG BLOCK, which is forced rather than chosen. Flat config
// REPLACES a rule's options at the LAST matching config object, so a block declared here
// and spread into the array would delete every console-scoped selector `eslint.config.mjs`
// declares for the same files — or be deleted by them, depending on order. Exporting the
// selectors lets that file state one union per file class, which is the only shape under
// which two authors can both add a selector and both survive.
//
// Everything the selectors need to say about themselves is said beside them. Only
// `eslint.config.mjs` imports this module.
...
/**
 * The console's TIME-reading bans.
 *
 * Not import bans, so `no-restricted-imports` can express none of them. Scope is decided
 * by `eslint.config.mjs`, which composes them into the console, shell, and console-tier
 * unions — the tiers included, because a fixture composed with `Date.parse` records the
 * HOST's zone into an expectation the surface under test is then measured against, which
 * is the same defect as the production one arriving through the file that is supposed to
 * catch it.
 *
 * ONE SELECTOR LIVED HERE AND IS GONE — count-free on purpose, since the list grows. It
 * banned importing `normalizeWireRejection` from `src/shared/wire-errors.ts`, back when a
 * function of that name lived in both that module and `console/core/wire-rejection.ts`
 * with two different return types, so an import from the wrong one compiled wherever the
 * result was only rendered. The shared function is now `wireRejectionToError`, named for
 * what it answers, so the collision is closed at its source and `tsc` catches the stale
 * import first and better (`error TS2305: Module ... has no exported member`). A lint
 * rule whose only reachable target is a symbol that does not exist guards nothing.
 *
 * TWO CATCH-STRINGIFICATION SELECTORS LIVED HERE AND ARE GONE TOO. They banned
 ...
 * reached neither arm, and `"" + error`, `error.toString()`, and every
 * `.catch((error) => …)` form are not inside a `CatchClause` at all. esquery has no
 * backreference, so a selector cannot bind a catch parameter and compare it to the
 * identifier being stringified. A selector that catches half a class reads exactly like
 * one that catches the class, so the claim is a reviewer's — `apps/desktop/AGENTS.md`
 * states it — rather than a half-selector nobody can trust.
 */
export const CONSOLE_TIME_READING_SELECTORS = [
  {
 ...
 *
 * Both are negative controls that have to CALL the banned API to demonstrate that
 * `Date.parse` answers a number for a value RFC 3339 refuses. A ban nobody can show the
 * cost of is a ban nobody keeps. Two entries that used to sit here were never earned:
 * `core/clock.ts` was excused as the console's one `Date.now` seam, and `Date.now`
 * matches no selector above; `core/wire-rejection.ts` was excused for the `String(catch)`
 * rule that no longer exists and that it only ever named in prose. Both lint clean
 * without an entry. What they cost while they sat here is the real thing: an exempt file
 * is exempt from EVERY selector composed for it, so a `Date.parse` could have landed in
 * either one and the gate would have stayed green.
 */
export const CONSOLE_TIME_READING_EXEMPT_FILES = [
  "src/renderer/src/console/core/instant.test.ts",
  "src/renderer/src/console/primitives/figures/wire-figures.time.test.ts",
];

/**
 * The console's exported-collection ban.
 *
 * Its own export because it is a different claim about a different subject — module
 * shape rather than wire readings — and because `eslint.config.mjs` composes the two
 * into different unions: the time bans are lifted for the two negative controls above,
