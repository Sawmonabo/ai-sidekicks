 ...
 *
 * A HARNESS WRITES THE OFFENDING EDGE TOO, and is exempt for the reason the intra-console
 * deep-import rule exempts one: a `.test-support` module is part of a suite rather than
 * something that ships, and neither remedy the rule offers is open to it — a door cannot
 * publish a fixture helper (`barrel-census` fails a specifier no production module reads)
 * and there is nowhere below to hoist one to. Three modules, one edge written twice and
 * the legal shape once: a subtraction widened past `.test-support` takes the production
 * violation with it and this control reads an empty list.
 */
