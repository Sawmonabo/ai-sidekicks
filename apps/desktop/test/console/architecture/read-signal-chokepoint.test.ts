 ...
 *
 * Nine of its ten: `driver.applyIntervention` alone reaches the daemon through a
 * surface the console has not registered a reply shape for yet, so the partition says
 * nothing about it and neither does this claim. A reading below that is the instrument
 * having gone blind rather than the claim holding.
 */
const CLASSIFIED_BLOCKED_METHOD_COUNT = 9;
