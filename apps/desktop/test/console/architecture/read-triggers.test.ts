 ...
 */
const CONSOLE_READINGS: readonly ReadingClassCensus[] = consoleSourceModules({})
  .flatMap((module) =>
    readingsIn(censusClasses(module.displayPath, readConsoleSourceModule(module))),
  )
  // Subtracted HERE and not inside `readingsIn`, so the controls below still drive the
  // unnarrowed rule and a name on the writer list cannot make the gate stop biting.
  .filter((reading) => !BRIDGE_HOLDING_WRITERS.has(reading.className));
