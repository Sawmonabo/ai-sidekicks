...
// The load-failure document — Plan-023 Phase 1B (T-023p-1B-2).
//
...
//
// The document is GENERATED in the main process and served from this handler.
// It is deliberately not a file emitted into the renderer bundle, because the
// failure it reports is "the renderer bundle could not be loaded" — a fallback
// document living in the tree that just failed is a fallback that is missing
// exactly when it is needed. Being generated also makes containment trivially
// true rather than checked: this path never reaches `resolveRendererAsset`, so
// no filesystem call happens on it at all and there is no root for it to escape.
//
...
/**
 * Longest reason rendered. A reason is assembled from an Electron error
 * message, which is neither bounded nor authored by us; a document that grew
 * with it would be a main-process memory cost driven by the failure itself.
 */
const LOAD_FAILURE_REASON_MAX_LENGTH = 300;

/** Builds the URL a window loads to display `reason`. */
export function buildLoadFailureUrl(reason: string): string {
  const bounded = reason.slice(0, LOAD_FAILURE_REASON_MAX_LENGTH);
 ...
 */
function matchLoadFailureRequest(url: string): string | null {
  let parsedUrl: URL;
  try {
