...
// `executionPosture` stamped on `run.running` — and, when there is none, says that
// posture is set by policy at run start rather than implying nobody set one.
//
...
import { Chip, Nothing, formatCount } from "../../../console/primitives/index.js";
import type { PostureChipModel } from "./chip-models.js";

/** What the chip says when the daemon has stamped no posture on this run yet. */
const POLICY_NOTICE = "Posture is set by policy when the run starts.";
...
export function PostureChip(props: PostureChipProps): React.JSX.Element {
  ...
  if (posture === undefined) {
    return <Nothing kind="not-checked" title="Posture not stamped" detail={POLICY_NOTICE} />;
  }
  return (
