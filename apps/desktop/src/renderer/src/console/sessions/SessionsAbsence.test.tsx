...
// that failed used to render as the same block, under a sentence saying the node's own
// directory answered nothing here. That is true of exactly one of them, and false of
// the other in the way that matters most — a closed channel reported as an idle
// console.
//
...
describe("SessionsAbsence — one absence per reason there is none", () => {
  ...
  it("negative control: a failed read never claims the console did not ask", async () => {
    ...
    // render the `not-checked` block, whose sentence asserts that the console is
    // holding none and the node volunteered none — a claim contradicted by the refusal
    // printed beside it.
    renderAbsence({ status: "unavailable", refusal: await failedDirectoryRefusal() });

    expect(screen.queryByText(/answered nothing here/u)).toBeNull();
    expect(screen.queryByText("This console is not holding any sessions.")).toBeNull();
  });
  ...
  it("says the console did not ask when no wire is registered for the read", () => {
    ...
    expect(screen.getByText("This console is not holding any sessions.")).toBeTruthy();
    expect(screen.getByText(/answered nothing here/u)).toBeTruthy();
  });
