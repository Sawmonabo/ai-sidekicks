...
//
// The decision itself is not made here. `rows/session-directory-rows.ts` owns
// `sessionsAbsenceKindFor`, so this file renders a kind it is handed and the merge
// and the absence agree by construction rather than by two switches written to
// match. A refused directory with no session open is `not-checked`: the console did
// not ask the daemon, and must not report "there are none" for a question it never
// put. A SERVED directory with no rows is `empty`, because that question was put and
