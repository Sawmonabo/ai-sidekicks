 ...
 * declared beside the overlay that binds it, and two surfaces PRINT it: the overlay
 * itself and the whole-surface absence, which is a primitive. So the primitive was
 * reaching up into palette for a string, which is the inversion the paragraph above
 * describes and the reason chord printing moved down in the first place. The BINDING
 * still belongs to the overlay — it is the only reader that hands this to
 * `parseChord` — but the LITERAL is console-wide vocabulary, and one home for it is
 * what keeps two hints spelling one chord.
 */
