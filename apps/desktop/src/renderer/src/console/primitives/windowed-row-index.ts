...
export interface WindowedRovingIndexOptions {
   ...
   *
   * A CLAMP IS NOT ENOUGH, and the diff pane's changed-file list is where that was
   * measured: a filter narrows five hundred rows to three, the remembered move at
   * row 499 clamps to row 2, and clearing the filter restores the move to 499 while
   * the window still sits at the top — leaving every mounted row untabbable and the
   * whole list outside the page's tab order. Clamping keeps a position the
   * participant never moved to; identity drops the move instead, which puts the tab
   * stop back on `anchorIndex`.
   *
   * Compared by identity and nothing else, so a caller passes the array it drew —
   * whatever value it has that is one sequence. Absent means the caller's set never
   * changes identity, which is every list whose rows are not filtered.
   */
...
export function useWindowedRovingIndex(options: WindowedRovingIndexOptions): WindowedRovingIndex {
  ...
  // A move stands only inside the sequence it was made in: an index into a sequence
  // that no longer exists addresses a different row, or none.
  const movedToIndex =
