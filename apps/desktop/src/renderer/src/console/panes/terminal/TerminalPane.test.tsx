 ...
 *
 * Both render through the same inline primitive, and the pane now has two reads that
 * can refuse — the output stream and the caller's identity. A bare class query would
 * match whichever landed first, so every output case scopes itself OUT of the lease
 * line rather than asserting on a count.
 */
function outputRefusal(region: HTMLElement): Element | null {
  return (
