// Which user this window is, for ATTRIBUTION: asked once, kept only when it
// answers, and never rendered.
//
// NAMED FOR THE POLICY IT OWNS AND NOT FOR THE WIRE IT CALLS. Two families held a class
// called `CallerUserRead`, in two files of that name, wrapping the same growth
...
//
// THE REFUSAL IS ABSORBED HERE AND ON PURPOSE. `causationUserId` is optional on
...
// for a user to do about it — and it is deliberately NOT turned into a
...
import { callerUserIdentityFrom } from "../../seats/index.js";

export interface CallerUserAttributionOptions {
 ...
 *
 * A PROMISE HELD RATHER THAN A VALUE: the read answers which user this window is,
 ...
 * act that needs it rather than started at `start()`, because a gate a user never
 ...
 */
export class CallerUserAttribution {
  readonly #bridge: ConsoleBridge;
  readonly #sessionId: string;

  #pending: Promise<string | undefined> | undefined;

  public constructor(options: CallerUserAttributionOptions) {
    this.#bridge = options.bridge;
    this.#sessionId = options.sessionId;
  }

  /**
   * Which user this window is, for an act's causation — or the honest absence.
   *
  ...
  public async read(): Promise<string | undefined> {
    ...
    if (userId === undefined && this.#pending === pending) {
   ...
   *
   * The served/refused narrowing comes from `seats/identity/caller-user.ts`, which is the
  ...
  async #ask(): Promise<string | undefined> {
    try {
      const identity = callerUserIdentityFrom(
        await this.#bridge.growth.callerUserRead({ sessionId: this.#sessionId }),
      );
