...
describe("gateSummaryLine", () => {
  it("gives every arm its own line, and none of them a number nothing read", () => {
    expect(gateSummaryLine({ kind: "not-checked" })).toBe("not checked");
    expect(gateSummaryLine({ kind: "preparing" })).toBe("reading");
    expect(gateSummaryLine({ kind: "no-context", executionMode: "read-only" })).toBe(
      "no context in read-only mode",
    );
    expect(gateSummaryLine({ kind: "refused", message: "the daemon said no" })).toBe("refused");
  });
});
