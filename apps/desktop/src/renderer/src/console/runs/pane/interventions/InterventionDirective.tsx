// What a user directed, where the console may still read it.
//
...
/**
 * What the user directed, where the console may still read it.
 *
 * The body-unavailable arm is a SENTENCE and not an empty line: a steer directive and
 * a replacement-send body rest encrypted under the authoring user's key, so a
...
export function InterventionDirective(props: {
  ...
  return directive.availability === "available" ? (
    ...
    <p className="meridian-interventions__detail">
      The directive rests encrypted under the authoring user&apos;s key, which this node no longer
      holds. The audit record stands; its text cannot be read.
    </p>
  );
}
