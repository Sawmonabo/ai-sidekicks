...
//
// THE FRAME CANNOT SEE IT. The dialog family runs under `modal="trap-focus"`, which
// traps focus and leaves inerting the app root to the shell — so a reader following
// the accessibility tree is handled and one navigating by STRUCTURE is not: the rail
// and the whole route surface stay reachable underneath. The shell has the guard —
// `AppFrame`'s `modalOverlayOpen`, which hangs `inert` on the background — and cannot
// arm it for a view family's card, because `console-view-family-isolation` forbids
// `frame/` from naming one. So the card publishes into the WINDOW store and the frame
// folds it with the palette's own state.
//
// HOISTED ON THE SECOND USE, which is this module's whole reason to exist. Two
// window-scoped overlays hold this wiring — the sign-in card and the onboarding
...
// it — a hook cannot state a prop for a component it does not render, and a hook that
// returned one would be read as though it had.
...
/**
 * Publish a window-scoped dialog's open state into the window's claim register.
 *
...
export function useModalSurfaceLifetime(frameStore: FrameStore, isOpen: boolean): void {
  ...
  useEffect(() => {
    ...
    return () => {
      frameStore.modalSurfaceClaims.release(claimId);
    };
  }, [frameStore, claimId, isOpen]);
}
