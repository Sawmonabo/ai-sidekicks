...
// `open-session-entry.ts` and `session-store-registry.ts`, and those two take their names
// by their own specifiers rather than through this door. Routing them here would close a
// cycle `no-circular` fails: this door is an edge to `read-triggers.ts`, which reads
// `session/index.js`, which publishes `session-hooks.ts`, which reads
// `session-store-registry.ts` — so a fifth edge back to this door completes the ring. The
// deep edge is the remedy the package's module-shape rule names for exactly this shape,
// and it is why `ApplyQueue` — whose only sibling reader is one of those two — is not
