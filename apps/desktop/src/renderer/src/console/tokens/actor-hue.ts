// The user hue system — design-language rule 2.
//
// Hue answers "who", everywhere: attribution edges, session-header rings, typing
...
// any of them:
//
//   1. **Join-log order.** Allocation walks users in the order the
...
//   2. **Hash-mod-twelve preferred step.** A user's first choice is
//      `hash(userId) mod 12`, so a person tends to keep a color across
//      sessions without any stored assignment.
//   3. **Clockwise next-free resolution.** A taken preferred step walks +1 until
//      a free step is found. Rule 2 puts it as: two users adjacent on the
...
//      users some step must repeat. Inventing a thirteenth hue would break
...
//      a glyph. Two users can then share a hue and still never be
//      confused, because the ring differs.
//
// **Departures free nothing.** A step allocated to a user stays theirs for
// the session's lifetime. Freeing it would let a later joiner inherit a departed
// user's color, which rewrites the meaning of every row that user
...
import { actorHue, actorHueTokenName } from "./tokens.js";

/**
 * The ring treatments, indexed by wrap lap. A session with more than
 * `ACTOR_HUE_STEPS * RING_TREATMENTS.length` users exhausts the
 * distinct treatments and reuses the last one — which is a real limit, stated
 * here rather than hidden: at 49 users in one session the session header has
...
export type ActorRingTreatment = (typeof RING_TREATMENTS)[number];

/** One user's place on the wheel. */
export interface ActorHueAssignment {
  ...
  /** True when this assignment shares its step with an earlier user. */
 ...
 */
export class ActorHueAllocator {
  ...
  /**
   * Admit a user in join-log order and return its assignment. Idempotent:
   * re-admitting a user already on the wheel returns the same assignment
   * and allocates nothing, which is what makes a re-join keep its color.
   *
   * AN AGENT'S ID IS ADMITTED EXACTLY AS A USER'S IS, and that is deliberate
   * rather than incidental. The store feeds this wheel from each event's `actorId`,
   * which the wire registers as a user id, an agent id, or nobody, with no
   ...
   * let one collide with a user it sits next to. The parameter keeps the
   * `userId` spelling every caller and helper here already uses; what it names
  ...
  public admit(userId: string): ActorHueAssignment {
    ...
    const preferredStep = preferredHueStep(userId);
  ...
  /**
   * The assignment a user already holds, or `undefined` when the wheel has
   * never seen it. Reading is side-effect free — a renderer that asks about an
   * unknown user must render the unrecognized shape, not silently mint an
  ...
  /** How many users the wheel has admitted. */
   ...
   * occupants. Below twelve users "fewest" is zero and this is exactly the
  ...
  #nextFreeStepFrom(preferredStep: number): number {
    ...
    for (let offset = 1; offset < ACTOR_HUE_STEPS; offset += 1) {
      const candidateStep = (preferredStep + offset) % ACTOR_HUE_STEPS;
 ...
 * anagrams, and user ids in this corpus are UUIDs whose characters are
...
/** The step a user would take if the wheel were empty. */
export function preferredHueStep(userId: string): number {
  return hashUserId(userId) % ACTOR_HUE_STEPS;
}
