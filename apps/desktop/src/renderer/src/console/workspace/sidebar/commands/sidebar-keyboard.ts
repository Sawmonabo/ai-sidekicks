// The sidebar's DOM-free cursor, as commands and chords rather than a keydown switch.
//
// The design track states it: a cursor where `j` / `k` move, `Enter` opens, and
// `Space` expands, with the cursor in STORE state so a re-render never loses it. Every
// clause of that is already built — `SidebarModel` holds the cursor outside the tree,
// `CommandRegistry` is the console's one command list, and `KeyBindingTable` is its one
// dispatch — so this module contributes; it implements no keyboard of its own.
//
// WHY A SIDEBAR-SCOPED REGISTRY AND TABLE, NOT THE WINDOW'S
//
// These chords are bare letters. A window-global `j` would take the letter away from
// every other surface in the console, which is exactly the failure the keybinding
// table's conflict rule exists to make loud — and it would be a real conflict, not a
// false one. The table takes any `EventTarget`, so the sidebar installs one listener on
// its own element and the chords are live only while focus is inside it.
// `CommandRegistry` is documented as one instance per window for the palette; a second
// instance for a scoped surface is the same class doing the same job at a smaller
// scope, not a second command system.
//
// AND WHY THE PALETTE SEAT BESIDE IT IS STILL ONE SEAT. `sidebar-command-seat.ts`
// carries the three acts a person reaches for from OUTSIDE the sidebar, which is where
// they are when they reach for it; these four are only meaningful once focus is inside
// the column, where a window-scoped palette row would be a second way to do what the
// key under the finger already does.
//
// THE FILTER FIELD IS HANDLED BY THE TABLE, NOT BY A GUARD HERE. `KeyBinding` defaults
// `allowInTextInput` to false and the table's own `isTextEntryTarget` decides what a
// text field is. Typing `j` into the filter therefore types a `j`, with no branch in
// this file that could disagree with the palette about what counts as text entry.
 ...
 * sections whose subject has a SESSION-scoped pane kind. `artifacts` and `repos` open
 * panes over one artifact or one worktree — a row's act, reached through the section
 * body's own `openPane` — and a header that opened an entity pane with no entity would
 * be the deck guessing which one. A section with no entry here expands instead, which
 * is the other half of what the cursor's `Enter` means.
 *
 * Each address carries no `entity` member at all rather than one holding `undefined`:
 * those kinds' arms of `ConsolePaneAddress` have none, which is how the seat states
 * that the pane is a view of the session and not of a row.
 */
 ...
 *
 * `KeyJ` / `KeyK` rather than `j` / `k`: the chord parser matches on `code` for the
 * letter forms, so the binding survives a layout where `j` is somewhere else.
 */
...
export interface SidebarCommandTargets {
   ...
   *
   * The cursor is DOM-free — it lives in the model — but a person driving by keyboard
   * still has to be told where they are, and the platform's own answer to that is
   * focus. Handed in rather than reached for so this module holds no DOM.
   */
  readonly focusSection: (sectionId: SidebarSectionId) => void;
}
 ...
 * about its own: every act reads live state, so they are per sidebar and are removed
 * when it unmounts.
 */
export function sidebarCursorCommands(
  model: SidebarModel,
    ...
    },
    {
      ...
      run: () => {
        ...
        if (address === undefined) {
          // No pane for this section, so `Enter` means the other thing it means: open
          // the section. Deliberately not a toggle — a person pressing Enter on an open
          // section wanted it open, and shutting it would be the surface arguing with
          // them.
