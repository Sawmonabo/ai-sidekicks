...
//
// The three acts are the ones a person cannot otherwise reach without a pointer: put
// the keyboard into the sidebar, collapse or expand the column, and put the keyboard
// in the filter field. All three are contributed at COMPOSITION time so they are in
// the palette and its chord table from the first frame, and all three resolve their
// target at PRESS time, because the sidebar comes and goes with the route while the
// command is built once per window.
//
...
//
// WHY THE CURSOR IS NOT HERE. The cursor's four acts are bare letters live only while
// focus is inside the column, so they belong to a sidebar-scoped table rather than to
// the window's palette — `sidebar-keyboard.ts` states that in full. These three are the
// window's, because a person reaching for the sidebar is by definition not in it yet.
...
export interface SidebarActs {
  /** Put the keyboard on the section the sidebar's cursor is holding. */
  readonly focusSidebar: () => void;
  readonly toggleSidebarCollapsed: () => void;
  /** Put the keyboard in the filter field, opening the column if it is shut. */
 ...
 * settings page needs to know the sidebar is not here, and naming which of the acts
 * they reached for would answer a question they did not ask.
 */
...
export function sidebarPaletteCommands(acts: SidebarActs): readonly ConsoleCommand[] {
  return [
    {
    ...
    },
    {
      ...
      title: "Filter the session sidebar",
      group: SIDEBAR_COMMAND_GROUP,
      when: WHEN_SESSION_ACTIVE,
      keywords: ["sidebar", "filter", "search", "find"],
      run: acts.focusSidebarFilter,
    },
  ];
}
 ...
 * the ones a person builds muscle memory for; three more on a surface reachable by one
 * Tab press would spend three chords on a keystroke that is already free.
 */
export function registerSidebarCommands(
  ...
  surface.contribute({
    ...
    commands: sidebarPaletteCommands(actsOnTheMountedSidebar(seat)),
    keyBindings: [],
  });
}
 ...
 *
 * Written out rather than derived from a name list, so a FOURTH act added to
...
function actsOnTheMountedSidebar(seat: MountedSidebarSeat): SidebarActs {
  return {
    ...
    focusSidebarFilter: () => {
      performOnMountedSidebar(seat, "focusSidebarFilter");
    },
  };
}
