...
import {
  ...
  sidebarPaletteCommands,
  type SidebarActs,
} from "./sidebar-command-seat.js";
...
interface SpyingSidebarActs extends SidebarActs {
  ...
  readonly focusSidebarFilter: Mock<() => void>;
}

function acts(): SpyingSidebarActs {
  return {
    focusSidebar: vi.fn<() => void>(),
    toggleSidebarCollapsed: vi.fn<() => void>(),
    focusSidebarFilter: vi.fn<() => void>(),
  };
}
...
describe("the sidebar's palette rows", () => {
  it("offers focus, collapse, and filter, all scoped to a window with a session", () => {
    const commands = sidebarPaletteCommands(acts());
