// The three reads the session header puts, and the one shape all three settle into.
//
// WHY THE HEADER READS ANYTHING AT ALL. The session store answers what the LOG says —
// what happened, and when. Three of the things the header has to render are not in the
// log and never will be: a session's display title and its state are projections the
// daemon serves, the node's health is a measurement, and the committed spend is one
// accountant's figure this renderer is forbidden to compute. Each is a read, and each
// is on the growth slate.
//
...
// and each renders the same three facts: nothing has answered yet, here is the answer,
 ...
 * the four-state seed rule: every one of these three reads has a subject the header
 * either holds or does not, and where it does not the header renders no reading at all
...
/**
 * Put one of the header's reads, and hold its answer against the subject it was put for.
 *
 ...
 * surface: a header with no session is one whose reads have not resolved, and the
