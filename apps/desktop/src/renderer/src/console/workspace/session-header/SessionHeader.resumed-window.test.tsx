// The session header over a window that opens partway through its session's log.
//
// THE CASE THIS FAMILY HAD NO ANSWER FOR. A session's stream replays from the position
// this user was last acknowledged at, so a resumed read establishes a window
...
// its count falls to zero, and the header printed its all-clear line over a run that was
// still blocked.
//
// A SUITE OF ITS OWN rather than a case in `SessionHeader.absence.test.tsx`, because the
// subject is not an absence the header renders: it is what the STORE knows about rows it
...
function barOver(sessionStore: ReturnType<typeof storeWith>): HTMLElement {
  return renderBar(<SessionHeader sessionId={SESSION_ID} sessionStore={sessionStore} />);
}

describe("SessionHeader — a window that opens partway through the log", () => {
  it("negative control: never prints the all-clear line over rows it was not sent", () => {
    ...
    const sessionStore = storeWith([], { readFromCursor: ACKNOWLEDGED_CURSOR });
  ...
  it("counts a blocked run the base state carried, rather than reporting it unread", () => {
    ...
    const sessionStore = storeWith([], {
      ...
      entities: [{ kind: "run", id: "run-below-the-head", state: "waiting_for_approval" }],
    });
