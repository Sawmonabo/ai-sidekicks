...
// currency's minor-unit digits to size its own fraction bounds. The session header sits
...
describe("SessionHeaderSpend — the committed figure is formatted once per figure", () => {
  it("formats one total once, however many times the bar repaints", () => {
    const formatter = vi.mocked(formatCentsAsCurrency);
    formatter.mockClear();

    const { rerender, container } = render(<SessionHeaderSpend spend={served(123_45)} />);
    expect(formatter).toHaveBeenCalledTimes(1);
    const figure = container.querySelector(".meridian-session-header__spend .meridian-figure");
    ...
    rerender(<SessionHeaderSpend spend={served(123_45)} />);
    rerender(<SessionHeaderSpend spend={served(123_45, "partially-priced")} />);
    expect(container.querySelector(".meridian-session-header__spend-qualifier")?.textContent).toBe(
      "at least",
    );
    expect(formatter).toHaveBeenCalledTimes(1);
    ...
    rerender(<SessionHeaderSpend spend={served(200_00)} />);
    expect(formatter).toHaveBeenCalledTimes(2);
    expect(
      container.querySelector(".meridian-session-header__spend .meridian-figure")?.textContent,
  ...
  it("formats nothing at all while the read has served no figure", () => {
    ...
    const { container } = render(<SessionHeaderSpend spend={{ status: "reading" }} />);
    expect(formatter).not.toHaveBeenCalled();
    expect(container.querySelector(".meridian-session-header__spend")).toBeNull();
  });
});
