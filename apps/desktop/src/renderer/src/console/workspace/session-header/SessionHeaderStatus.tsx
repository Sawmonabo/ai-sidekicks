...
// how much, and it is here because the session header is the one strip a person keeps in
...
import { type SessionHeaderHealthVerdict } from "./model/session-header-readings.js";

export interface SessionHeaderStatusProps {
  /**
   ...
   */
  readonly verdict: SessionHeaderHealthVerdict;
}

export function SessionHeaderStatus(props: SessionHeaderStatusProps): React.JSX.Element | null {
  const { verdict } = props;
  ...
  if (verdict.kind === "unchecked") {
    return (
      <span className="meridian-session-header__status">
        <Nothing
          kind="not-checked"
          title="Node health"
