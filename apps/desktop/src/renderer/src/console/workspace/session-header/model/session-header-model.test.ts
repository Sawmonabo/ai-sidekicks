...
import { deriveSessionHeader } from "./session-header-model.js";
import {
  sessionHeaderOver,
  castEvent,
  ledgerOver,
  withRun,
} from "./session-header-model.test-support.js";

const AGENT = "agent-architect";

describe("deriveSessionHeader — the all-clear line", () => {
  it("says nothing needs you only when nothing does", () => {
    expect(sessionHeaderOver([castEvent(1, AGENT, "run.running")]).standing).toBe("all-clear");
    expect(sessionHeaderOver([castEvent(1, AGENT, "run.waiting_for_approval")]).standing).toBe(
      "attention",
    );
  });
  ...
  it("refuses to claim all-clear over an incomplete projection", () => {
    ...
    // no standing to make.
    const model = sessionHeaderOver([], { isDegraded: true });
    expect(model.standing).toBe("attention");
  ...
  it("refuses to claim all-clear while the node's health mark is amber", () => {
    ...
    const model = sessionHeaderOver([], { isNodeUnwell: true });
    expect(model.standing).toBe("attention");
    expect(model.outstandingAskCount).toBe(0);
  });
  ...
  it("says the count is partial where the window opened partway through the log", () => {
    ...
    const model = deriveSessionHeader({
  ...
  it("negative control: the same spotless log with a healthy node IS the all-clear", () => {
    ...
    expect(sessionHeaderOver([]).standing).toBe("all-clear");
  });
});
