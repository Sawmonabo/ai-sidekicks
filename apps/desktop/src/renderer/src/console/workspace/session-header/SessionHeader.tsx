...
import { SessionHeaderBody } from "./SessionHeaderBody.js";
import { SessionHeaderIdentity } from "./identity/SessionHeaderIdentity.js";
import { SessionHeaderSkeleton } from "./SessionHeaderSkeleton.js";
import { SessionHeaderStatus } from "./SessionHeaderStatus.js";
import {
  useSessionHeaderGrowthPort,
  useSessionHeaderHealth,
  useSessionHeaderIdentity,
  useSessionHeaderSpend,
} from "./model/session-header-readings.js";
...
export function SessionHeader(props: SessionHeaderProps): React.JSX.Element {
  const growth = useSessionHeaderGrowthPort();
  const identity = useSessionHeaderIdentity(growth, props.sessionId);
  const healthVerdict = useSessionHeaderHealth(growth, props.sessionId);
  const spend = useSessionHeaderSpend(growth, props.sessionId);

  return (
    <header className="meridian-session-header" aria-label="Session">
      <SessionHeaderIdentity sessionId={props.sessionId} identity={identity} />
      <SessionHeaderStatus verdict={healthVerdict} />
