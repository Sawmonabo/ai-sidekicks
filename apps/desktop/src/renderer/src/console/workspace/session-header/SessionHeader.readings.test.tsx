...
//
// `SessionHeader.absence.test.tsx` next door is this suite's negative half and stays that
...
import {
  ...
  renderBar,
  storeWith,
} from "./SessionHeader.test-support.js";
...
function scenarioAnswering(replies: ConsoleScenario["replies"]): ConsoleScenario {
  return { ...CAST_BAR_SILENT_SCENARIO, id: "session-header-answers", replies };
}
...
function sessionReadReply(state: string, title?: string): ConsoleScenario["replies"][number] {
  return {
    ...
    result: {
      ...
      timelineCursors: { latest: "session-header-cursor-1" },
    },
  };
}
...
describe("the session header — the node's health, in one mark", () => {
  it("counts the components that are not healthy and names them", async () => {
    ...
    const status = bar.querySelector(".meridian-session-header__status");
  ...
  it("says one component in the singular", async () => {
    ...
    expect(bar.querySelector(".meridian-session-header__status")?.textContent).toContain(
  ...
  it("negative control: a healthy node gets no mark at all", async () => {
    ...
    expect(bar.querySelector(".meridian-session-header__status")).toBeNull();
  });
...
describe("the session header — the accountant's figure", () => {
  it("renders the committed figure the accountant settled, and sums nothing", async () => {
    ...
    const spend = bar.querySelector(".meridian-session-header__spend");
  ...
  it("negative control: an unanswered read draws no figure at all", async () => {
    ...
    expect(bar.querySelector(".meridian-session-header__spend")).toBeNull();
    expect(bar.querySelector(".meridian-session-header__all-clear")?.textContent).not.toMatch(
      /\$|\d+\.\d\d/,
    );
  });
});
