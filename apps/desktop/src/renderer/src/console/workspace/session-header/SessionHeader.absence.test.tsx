...
//
// What the header DOES render off an answered read is `SessionHeader.readings.test.tsx`.
...
describe("SessionHeader — what the console has not measured", () => {
  it("draws the spend figure as unread rather than as a zero", () => {
    ...
    const allClear = bar.querySelector(".meridian-session-header__all-clear");
...
describe("SessionHeader — what is waiting on you", () => {
  ...
  it("counts the blocked run even while a newer ordinary row followed it", () => {
    const bar = renderBar(
      <SessionHeader
        ...
        sessionStore={storeWith(blockedInOneRunBusyInAnother())}
      />,
    );
...
describe("SessionHeader — the all-clear line", () => {
