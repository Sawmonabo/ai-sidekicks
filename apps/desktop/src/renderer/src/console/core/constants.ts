...
// This module is that place for the WHOLE console, view families included. It
// once said the opposite — that a family adds its own module beside its subtree —
// and that sentence is what two families were written against before anything
// checked the rule: a ceiling declared inside the feature that spends it is a
// ceiling nobody audits, which is the finding `cap-constant-home.test.ts` closes.
// A family reaches its bounds through this family's door, as every other symbol
// it borrows from here already does.
//
