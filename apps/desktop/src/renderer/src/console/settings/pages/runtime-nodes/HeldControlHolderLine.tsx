...
// store.
//
// ONE WHEEL, NEVER A SECOND. `ActorHueAllocator` allocates in join-log order and
...
// wheel for every user the session opened with. A later admission — a
// user who joins while this page is open — reaches the mark on the block's
...
export function HeldControlHolderLine(props: {
  ...
  const hueAssignment = useMemo(
    () => (sessionInitialised ? hueAllocator.assignmentFor(props.userId) : undefined),
    [hueAllocator, props.userId, sessionInitialised],
  );

  return (
    <ControlHolderLine
      reading={{ kind: "held", userId: props.userId }}
