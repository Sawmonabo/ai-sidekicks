...
// participant as its subject, so with none it refuses every focus and every reconnect
...
// names a participant, and the reading held for that participant reads its own set on
...
export class ScheduledCallerParticipantRead implements ReadTriggerTarget {
   ...
   *
   * The question is which participant this CALLER is, which no event in any session's
  ...
  readonly #bridge: ConsoleBridge;
  readonly #sessionId: string | undefined;
  readonly #changes = new Emitter<void>("caller participant read change");
   ...
   */
  public snapshot(): CallerParticipantReading | undefined {
    return this.#reading;
  }
  ...
  async #read(): Promise<void> {
    ...
    try {
      const outcome = await this.#bridge.growth.callerParticipantRead({ sessionId });
      round.settle(() => {
      ...
      round.settle(() => {
        this.#publish({
          kind: "unreadable",
          refusal: consoleRefusalFrom(rejection, CALLER_PARTICIPANT_ORIGIN),
        });
      });
    }
  }

  #publish(reading: CallerParticipantReading): void {
