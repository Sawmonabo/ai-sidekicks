...
//
// Split from `LeaseLine.tsx` so that module declares one component. The history is one
// click away rather than on the line, and every row NAMES ITS REASON — the sentence
// comes from `lease-transition.ts`'s table, which is total over the closed reason set,
// so the three automatic reasons cannot collapse into one.
//
...
// says forty-one. Without a notice a person answering "why did the shell move" is
// shown a feed that looks exhaustive and is not, with nothing saying which
        ...
        {state.transitions.map((transition) => (
          ...
          >
            <p className="meridian-lease-line__sentence">
              {terminalLeaseTransitionSentence(transition)}
            </p>
          </LedgerRow>
        ))}
      </div>
