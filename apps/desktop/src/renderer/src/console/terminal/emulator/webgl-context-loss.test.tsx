...
describe("two panes on one session", () => {
  it("spend two contexts, and one pane's teardown leaves the other drawing", () => {
    ...
    const firstPane = new XtermTerminalAdapter({ terminalId: sessionTerminalId, pool });
    liveAdapters.push(firstPane);
    firstPane.attach(attachedHost());
    const secondPane = new XtermTerminalAdapter({ terminalId: sessionTerminalId, pool });
    liveAdapters.push(secondPane);
  ...
  it("negative control: the survivor's own teardown then gives up the last hold", () => {
    ...
    const pool = new TerminalRendererPool();
    const onlyPane = new XtermTerminalAdapter({ terminalId: "solo-session", pool });
    liveAdapters.push(onlyPane);
