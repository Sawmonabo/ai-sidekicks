...
// renderer root directly. Three of them now mount inside a console-authored
// surface, and `seats/absorbed-surfaces.ts` holds those mounts — it had to, because
// the surfaces that mount them are four different view families and a view family
// cannot reach into the frame.
//
 ...
 * because no console-authored workspace surface has landed. The other three are
 * mounted by the console surfaces that absorbed them, through the helpers the seats
 * door publishes.
 *
 * The components each family exports beyond these four take inputs no route carries
 * — an invite token, an attach draft — so a route cannot supply them and a slot for
 * them would be a slot nothing could ever fill. The invite acceptance view is
 * exactly that case and is mounted by its caller, which holds the token.
 */
