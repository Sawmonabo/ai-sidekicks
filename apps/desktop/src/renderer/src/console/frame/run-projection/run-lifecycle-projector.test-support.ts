...
//
// Both suites drive a payload no scenario scripts, and each needs the same session to
// deliver it on. The constant was written twice — once in each file, with the same
// bytes — and the fold under test REFUSES a beat whose payload names another session,
// so two suites that had drifted apart on the value would each still pass while testing
// opposite halves of that one guard. Declared here instead, on the package's
// shared-code rule: a helper used by two modules lives once.
//
