...
//
// THIS IS THE PANE'S BODY AND NOT THE PANE. `BrowserPane.tsx` is what the deck mounts:
// it asks the host for this pane's view first and renders this body only once there is
// one to steer, or once it knows nobody ever asked for one. Every read below opens a
// subscription against a `paneId`, so a body mounted ahead of that answer would be five
// questions about a view that may not exist — which is the whole reason the two are
// separate components rather than one with a branch inside it.
//
...
export function BrowserPaneChrome(context: PaneContextOf<"browser">): React.JSX.Element {
