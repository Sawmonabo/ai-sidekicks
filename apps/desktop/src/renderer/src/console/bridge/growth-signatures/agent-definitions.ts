// The agent-definition plane: the node-local registry's list, create, update,
...
export interface AgentDefinitionGrowthSignatures {
  // agent definition — the registry's four definition pairs in its own order, and the
  ...
  agentDefinitionList: {
    ...
    value: readonly AgentDefinition[];
  };
  agentDefinitionCreate: { request: AgentDefinitionDraft; value: AgentDefinition };
  ...
  // landed on one and not the other. `agent-definition.ts` says why the draft and
  // the stored row stay two shapes while the two WRITES stay one.
  agentDefinitionUpdate: {
    request: { readonly definitionId: string } & Partial<AgentDefinitionDraft>;
  ...
  agentPeerInvocationSet: { request: PeerInvocationSetRequest; value: PeerInvocationReading };
}
