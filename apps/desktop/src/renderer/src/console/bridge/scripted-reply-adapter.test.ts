...
//
// Every case drives the real `ScenarioEngine` and the real adapter. A double would be
// asserting its own arithmetic over `abandoned`, which is a state only the engine's
// own teardown produces — and the point of the case is that the adapter reports the
...
import { ScenarioEngine, type ConsoleScenario, type ScenarioReply } from "./scenario.js";
...
function scenarioScripting(replies: readonly ScenarioReply[]): ConsoleScenario {
  return {
    id: "scripted-reply-adapter-probe",
    label: "Adapter probe",
    purpose: "Drives the port-side scripted-reply adapter over one settlement at a time.",
...
/**
 * The run read, asked through the adapter with the refusing fallback the port uses.
 *
 ...
 * makes.
 */
function readRunThroughAdapter(
  engine: ScenarioEngine,
): Promise<GrowthOutcome<WorkflowRunSnapshot>> {
  return answerFromScriptedReply(engine, RUN_READ_CALL, "workflowRunRead", () =>
    growthUnavailable("workflowRunRead"),
  );
}

describe("the scripted-reply adapter — the caller decides what an unscripted script means", () => {
  it("hands back the caller's own answer when the scenario scripts nothing", async () => {
    ...
    const snapshot = await readRunThroughAdapter(engine);

    // Two callers, two meanings of "the script said nothing", and the adapter imposes
    // neither: one operation has an empty form that is a real answer and the other has
    // none at all. An adapter that chose for them would be wrong for one of the two.
  ...
  it("serves a scripted reply verbatim, so the fallback never runs against a stated fact", async () => {
    ...
    const outcome = await readRunThroughAdapter(engine);

    expect(outcome.status).toBe("served");
    if (outcome.status === "served") {
      // Identity: the value is the scenario's own object rather than a copy the
      // adapter built, which is what "verbatim" has to mean for an `unknown` result.
  ...
  it("throws a scripted daemon refusal verbatim and unwrapped", async () => {
    ...
    // deliberately arbitrary — the adapter must not read it, which is what identity on
    ...
    await expect(readRunThroughAdapter(engine)).rejects.toBe(refusal);
    engine.dispose();
  });
  ...
  it("carries the seam's own diagnosis rather than composing a second one", async () => {
    ...
    const throughAdapter = engineScripting([heldRunReadReply()]);
    const throughSeam = engineScripting([heldRunReadReply()]);
    const adapterPending = readRunThroughAdapter(throughAdapter);
    const seamPending = settleScriptedReply(throughSeam, RUN_READ_CALL);

    throughAdapter.dispose();
    throughSeam.dispose();

    const outcome = await adapterPending;
  ...
  it("negative control: the same held read serves once the caller advances the clock", async () => {
    // Without this, an adapter that refused every parked read would pass the case
    ...
    const pending = readRunThroughAdapter(engine);
