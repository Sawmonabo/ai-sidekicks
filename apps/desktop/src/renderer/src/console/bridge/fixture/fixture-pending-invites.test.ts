...
async function drain(
  ...
  for await (const invite of feed.events) {
    // The scenario feed carries only the ready arm; any other frame here is the
    // fixture misbehaving, and the case should say so rather than skip it.
    if (invite.status !== "ready") {
      throw new Error(`the pending feed served a ${invite.status} frame`);
    }
