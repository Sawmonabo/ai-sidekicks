...
describe("the fixture growth port — what it serves, and what it still refuses", () => {
  ...
  it("still refuses a served workflow read under a scenario that scripts no workflow", async () => {
    // The counter-arm the two claims above deliberately leave out, and the one this
    // header used to promise from a suite that was never written. A served operation is
    // a claim about the PORT and not about the script: under the flagship, which
    // scripts no workflow at all, the two snapshot reads have no subject to answer for
    // — and the honest answer is a refusal rather than an invented empty form, which
    // would render "this run has no phases" about a run nothing asked after.
    const bridge = createFixtureBridge({ scenario: FLAGSHIP_SCENARIO });

    for (const operationId of ["workflowRunRead", "workflowPhaseOutputRead"] as const) {
