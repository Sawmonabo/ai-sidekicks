...
// two do not carry the same members and were never meant to: the queue event family
// fixes the canonical payload at `{sessionId, queueItemId, channelId?, state}`,
// while `QueueItemSummarySchema` (`packages/contracts/src/runControl.ts`)
...
// what the event registry registers was REFUSED as unprojectable — and the only way
...
// is `QueueItemListResponse` — `{items: QueueItemSummary[]}`, the rows themselves, as
// the run-control wire registry declares them. The fixture's stand-in for a daemon
// read is a scripted reply, so a
