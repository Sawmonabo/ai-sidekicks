...
// `GrowthUnavailable` comes off for the same reason and in the same diff. The one
// surface that named the type read the directory's refusal, and it now reads the
// settlement's `SettledReadRefusal` instead, which is the shape a settled read
// actually carries; the outcome union still travels, on `GrowthReading` below.
...
// goal editor first). `stampedExecutionPostureOf` is the composer's posture chip's:
// it parses the candidate against the registered `RunStateChangeEvent` shape, which
// is the whole point — a surface checking two members loosely and asserting the
