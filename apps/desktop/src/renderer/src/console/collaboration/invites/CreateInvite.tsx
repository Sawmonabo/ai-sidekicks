...
import { consoleClockFor, heldIdAsWireId, type ConsoleBridge } from "../../bridge/index.js";
...
export function CreateInvite(props: CreateInviteProps): React.JSX.Element {
  ...
  const { value: identity, publish: publishIdentity } = useSubjectScopedState<
  ...
  useEffect(() => {
    ...
    // asked and a settlement arriving after a re-address publishes nowhere.
    void bridge.growth.callerParticipantRead({ sessionId }).then(
      (outcome) => {
        publishIdentity(
      ...
      // The port resolves with an outcome, so a rejection has no arm in its
      // vocabulary. Left unhandled it published nothing and the send control stayed
      // closed for the life of the window over a call that had already failed.
      (rejection: unknown) => {
        publishIdentity({
          status: "refused",
          refusal: consoleRefusalFrom(rejection, CREATE_INVITE_ORIGIN),
        });
      },
    );
  }, [bridge, sessionId, publishIdentity]);
