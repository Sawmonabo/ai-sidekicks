...
//
// The sections are filled by the composer family (`goal`, `runs`, `approvals`), the
// session surfaces family (`channels`, `agents`), and the repos family (`repos`,
// `artifacts`). Three branches, one sidebar: the failure this file exists for is two
// of them claiming one section, and the second failure is the render order drifting
// to whichever module happened to evaluate first.
//
...
// the whole tuple to a literal rather than counting it.
...
/**
 * Every section the sidebar renders, in render order.
 *
 * Written out here rather than derived, so a section added to or dropped from the
 * seat's own tuple fails this file instead of silently changing what a person reads.
 * `repos` covers repos and worktrees as one section, named for the entity kind its
 * cards open panes for.
 */
const EXPECTED_SIDEBAR_SECTION_IDS: readonly string[] = [
  "goal",
  "channels",
  "runs",
  "agents",
...
describe("sidebar sections — the closed set", () => {
  it("carries every section in render order", () => {
    ...
    // the order check.
    expect([...SIDEBAR_SECTION_IDS]).toStrictEqual([...EXPECTED_SIDEBAR_SECTION_IDS]);
  });
  ...
  it("negative control: the two sections a shorter substrate had no seat for", () => {
    // Without this the comparison above would pass over a literal edited to match a
    // tuple that had dropped them — which is exactly how the substrate came to omit
    // both while claiming to be closed. Named one by one, so removing either fails
...
describe("sidebar sections — the module-scope door", () => {
  ...
  it("negative control: an unfilled section has no body", () => {
    ...
    expect(sidebarSectionRenderer("channels")).toBeUndefined();
  });
});
