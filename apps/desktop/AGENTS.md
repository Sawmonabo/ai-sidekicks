...
- A budget marked `enforced` is reachable from the aggregate `test` script _and_ from a CI job. If it is not wired, its status is `n/a` naming the wiring task — never `enforced` and unrun. The two `enforced` rows today, `renderer-initial-bundle` and `terminal-instance-memory`, are both reached through the `console-bundle` tier, which is on both.
