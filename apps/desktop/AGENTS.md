...
- Every stylesheet reads only custom properties the console itself defines: a `var(--…)` naming a token no sheet declares resolves to nothing and the declaration is dropped, which renders as a missing colour rather than as an error. Review rule — no tool in this package lints CSS (`stylelint` is the standard-tool home for it and is not adopted).
