...
- A stylesheet is imported by the barrel of the directory that OWNS it — a family door, or the door of a sub-directory that owns a surface of its own — and by no component. A family door pulling in the sheets of its own sub-directories is that shape rather than an exception to it: the owner is what the rule keys on, not the depth. What it forbids is a sheet reaching ACROSS an ownership boundary — one directory's barrel importing another's sheet — which puts a surface's rules on the document for every session that never opens that surface and makes one directory the reason another is styled at all.
