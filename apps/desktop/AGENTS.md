...
- A stylesheet enters through the barrel of whatever OWNS it — the family, a sub-module that owns a surface of its own, or a lazily-loaded chunk — and through no component, which is the rule `test/console/architecture/stylesheet-edges.test.ts` holds every module to. A sheet pulled into a family sheet from another directory is the shape this forbids: it puts a surface's rules on the initial document for every session that never opens that surface, and it makes one directory the reason another is styled at all. The sub-module case is a pane body with its own door — `workflows/pane/run/` and `workflows/pane/builder/` are the two today. The chunk case is a directory reached only by an `import()` — `workflows/pane/run/phase-graph/` is the one today — whose own `index.ts` is the module that `import()` names, so its sheets ride the chunk instead of the initial document without any component holding a stylesheet edge.
...
- Shared scaffolding lives once, and one home per role. Cross-process roles go in `test/helpers/` — the `vi.mock("electron")` factory in `test/helpers/electron-mock.ts`, and any second one is rejected. Console roles go in `test/console/`: `console-harness.tsx` (render harness), `electron-harness.ts` (spawn-and-scan), `phase-graph-settled.ts` (graph readiness — whether a lazily-drawn canvas is fitted and still enough for a tier to read), and `console-source-modules.ts` (the renderer source roots and the walk over them — the home for "where the console is", so a second tier that needs the path imports it rather than resolving one of its own) are the residents; a temp-directory helper and a script runner take `temp-dir.ts` and `run-script.ts` there when first needed. A mount of one family's surfaces is not a shared role and does not join them: it goes in `test/console/surfaces/<family>.tsx`, one file per family, so the tier root stays the set every tier can reuse. A tier that hand-rolls a role another tier already has is rejected.
