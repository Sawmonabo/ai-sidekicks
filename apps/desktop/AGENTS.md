...
- A budget marked `enforced` is reachable from the aggregate `test` script _and_ from a CI job, and its `measuredBy` names a harness that holds the subject it bounds. If it is not wired, its status is `n/a` naming the wiring task — never `enforced` and unrun. WHICH rows are gated, and which tier measures each one, is `budgets.json`'s to say and is not restated here: a roster in prose is a second home for a value that has one, and the closed list that stood here named four of the nine rows that file marks `enforced`.
