...
- A family's CSS is imported from that family's barrel and from nowhere else, and each sheet sits flat beside the modules it dresses — one per sub-module directory, named for that directory, with a sheet whose shapes more than one of them reaches for at the family root — never in a `styles/` directory of its own.
