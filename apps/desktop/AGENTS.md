...
That marker has two halves, and they are deleted together in the PR that imports the symbol. The tag sits where knip reports the finding, which for a barrelled symbol is the BARREL's own export specifier — knip reads per-specifier tags on an `export { … } from`. Whether the re-exported DECLARATION also needs one is decided by running the gate, not by reading this paragraph: excluding the barrel's specifier takes the re-export edge with it, so a declaration reached only through that line is then reported unreferenced and carries the JSDoc tag too — measured on `renderAbsorbedParticipantRoster`, whose declaration knip reports the moment the tag is spelled as a line comment. Where the gate does not need it, the declaration carries the same claim as a one-line `// Consumed by T-023p-1C-<n>` comment, so the symbol names its consumer where a reader meets it and a tag knip does not need cannot fail the run. `--treat-tag-hints-as-errors` is what makes the deletion mandatory: the moment a consumer imports the symbol, the surviving tag fails the run.
...
- The console reaches `src/shared/` through a layer family that owns the concern — `core/` for the wire strings and the rejection readers, `routing/` for the auxiliary-route grammar — and a VIEW family importing it directly fails `structure:layering` under `console-view-family-shared-through-core`. The cross-process leaf sits on no rung of the console DAG, so no ordering rule says anything about that edge and a second reading of a shared shape could otherwise land above the layer that owns it.
...
- A budget marked `enforced` is reachable from the aggregate `test` script _and_ from a CI job, and its `measuredBy` names a harness that holds the subject it bounds — the WHOLE subject: the terminal row measures a real `terminal` pane, resolved through the deck's own registry and mounted in the built console by the `define`-gated fixture pane harness on a live WebGL2 context, and fails on a fallback-renderer reading rather than measuring a narrower one. If it is not wired, its status is `n/a` naming the wiring task — never `enforced` and unrun. WHICH rows are gated, and which tier measures each one, is `budgets.json`'s to say and is not restated here: a roster in prose is a second home for a value that has one, and the closed list that last stood here named three of the eleven rows that file marks `enforced`.
- One row KIND carries a rule the JSON cannot: a hardware-dependent row prints its figure on every runner and compares it only on the pinned CI runner class, which `test/console/endurance/pinned-runner-class.ts` decides for the whole tier. Off that class the reading is reported and gates nothing — which is not the same as `n/a`, and is never written as one.
