...
- A stylesheet enters through the barrel of the family or of the lazily-loaded chunk that owns it, and through no component. The second case is a directory reached only by an `import()` — `workflows/pane/run/phase-graph/` is the one today — whose own `index.ts` is the module that `import()` names, so its sheets ride the chunk instead of the initial document without any component holding a stylesheet edge.
