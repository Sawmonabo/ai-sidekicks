...
| Local Database | SQLite (WAL mode, 58 tables) |
...
| 14 | Artifacts (local + cross-node) | Diffs, files, and attachments; shared artifacts stay fetchable while the publishing node is offline via an eager relay pin of E2EE ciphertext (ADR-015 amendment 2026-07-08; threat-model-scoped 2026-08-08, and time-bounded 2026-08-26 to a **live** pin — the guarantee ends at the artifact's retention TTL — see that ADR's §Decision Log for the two accepted V1 residuals, their named closure, and the time bound) |
