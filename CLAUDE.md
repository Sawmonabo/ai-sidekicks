...
- **Plan-readiness audit** ([runbook](docs/operations/plan-implementation-readiness-audit-runbook.md), methodology PR #14, Tier 1 pilot PR #15) gates code from Tiers 2-9 — each tier's plans must clear the audit before that tier's code PRs land. The plan-template Precondition checkbox blocks `draft → review` for new plans; pre-audit `approved` plans need a retroactive audit pass first. **Tiers 2-7 audits are complete** (NS-14 + NS-15..NS-19; the Tier-7 audit landed via PR #160), so those tiers' `approved` plans are code-cleared; **Tiers 8-9 audits remain** — two tier-PRs owed (Tier 8 includes Plan-017's `review → approved` promotion).
